...
Conclusion: GAN生成的图像具有独特的频域特征，结合信号处理和深度学习的方法在数字取证和增强工业AI系统可信度方面具有工业潜力。
...
TL;DR: 该论文研究了Vision Transformers与对比学习目标在3D形状特征表示学习中的结合应用，通过实验验证了该方法在多视角3D分析任务中的有效性。


<details>
  <summary>Details</summary>
Motivation: 解决计算机视觉方法在从2D图像识别3D物体时面临的挑战，包括需要大量标注数据以及CNN可能忽略重要形状关系的问题。

Method: 将Vision Transformers架构与现代对比学习目标相结合，包括监督对比学习和自监督对比学习，用于多视角3D形状分析。

Result: 在ModelNet10数据集上，监督对比学习方法达到了约90.6%的准确率，证明了ViT与对比学习结合的有效性。

Conclusion: ViT能够捕捉全局形状语义，而对比学习能够优化局部判别特征，这种结合克服了传统方法对大量标注数据的依赖和CNN在捕捉形状关系方面的局限性。
...
Method: 采用基于查询的跟踪框架，包含多模态两阶段transformer精炼和跟踪流水线，融合相机和LiDAR的BEV特征，无需显式运动模型，通过时间平滑器在移动窗口内精炼边界框序列。

Result: 在nuScenes测试集上达到74.7的aMOTA，在3D MOT基准测试中表现出色，减少了身份切换同时保持竞争性精度。

Conclusion: 基于查询的transformer跟踪方法从多模态传感器特征中显著受益，提供了一个无需预训练和有限数据即可与其他神经网络方法竞争的框架。
...
TL;DR: 在线标签平滑(OLS)方法通过动态调整软标签来改善医学图像分类中的过度自信问题，在RadImageNet数据集上相比传统方法提升了分类准确率和特征表示质量。
...
Method: 使用在线标签平滑(OLS)方法，在训练过程中基于模型自身的预测模式动态调整软标签，并在RadImageNet数据集上使用ResNet-50、MobileNetV2和VGG-19三种架构进行评估。

Result: OLS相比硬标签、传统标签平滑和无教师知识蒸馏等方法，在Top-1和Top-5分类准确率上均获得提升，同时产生更紧凑和分离良好的特征嵌入。
...
<details>
  <summary>Details</summary>
Motivation: 鱼眼相机在广域交通监控中具有优势，但其强径向失真和边界区域物体外观严重退化给标准目标检测器带来挑战。

Method: 采用简单有效的预处理和后处理管道，在鱼眼交通图像上训练多个最先进的检测模型，并通过集成策略结合它们的输出。
...
TL;DR: 提出了一种实时渲染感知过滤方法，通过中间梯度计算敏感度分数，解决3D高斯泼溅模型在训练数据分布外视角下的视觉噪声问题。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅模型在训练数据分布外的视角下会产生严重的视觉噪声，这是由于缺乏训练数据导致密度、颜色和几何预测不确定。

Method: 基于中间梯度计算敏感度分数，专门针对各向异性取向引起的不稳定性进行过滤，而不是各向同性方差。

Result: 实验表明该方法相比现有NeRF方法（如BayesRays）显著提高了视觉质量、真实感和一致性，并能实时集成到现有3DGS渲染管线中。

Conclusion: 该方法有效解决了3DGS模型在训练分布外视角下的生成不确定性，无需重新训练或微调即可实现高质量实时渲染。
...
<details>
  <summary>Details</summary>
Motivation: 传统物理全波形反演方法受颅骨引起的信号衰减、模式转换和相位畸变限制，而纯数据驱动方法在低信噪比和稀疏孔径条件下会产生定量偏差的声速图。

Method: 两阶段混合框架：第一阶段使用逆时偏移（时间反转声学）处理多角度采集数据生成迁移片段；第二阶段使用基于Transformer的超分辨率编码器-解码器与图注意力单元融合片段生成准确的声速图像。

Result: 在两个合成数据集上的实验表明，BrainPuzzle在声速重建精度和图像完整性方面表现优越。

Conclusion: BrainPuzzle展示了推进定量超声脑成像的潜力，通过混合算法补偿缺失孔径，提高了经颅超声成像的可行性。
...
TL;DR: 该研究构建了一个统一评估框架，系统分析了文本到图像生成和图像到图像编辑中的文化偏见问题，发现在国家无关提示下模型倾向于全球北方现代描绘，迭代编辑会侵蚀文化保真度，且模型仅应用表面线索而非上下文感知的更改。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注文本到图像系统的文化偏见，而图像到图像编辑器的文化偏见问题研究不足，需要建立一个标准化协议来提供可比较的诊断结果。

Method: 使用固定设置的开放模型，结合标准自动指标、文化感知的检索增强VQA和来自本土评审员的专家人工判断，对六个国家进行跨国家、跨时代和跨类别评估。

Result: 研究发现：1）国家无关提示下模型默认全球北方现代倾向描绘；2）迭代编辑侵蚀文化保真度；3）图像到图像模型仅应用表面线索而非上下文感知更改。

Conclusion: 当前系统中的文化敏感编辑仍然不可靠，通过发布标准化数据、提示和人工评估协议，为诊断和跟踪生成图像模型中的文化偏见提供了可复现的文化中心基准。
...
TL;DR: 提出FIBAR方法，使用IIR滤波器异步重建事件相机图像，无需GPU，在CPU上高效运行，适用于标记检测等任务


<details>
  <summary>Details</summary>
Motivation: 现有基于神经网络的事件相机图像重建方法需要GPU且复杂，需要一种更简单、异步的CPU解决方案

Method: 使用时间IIR滤波器整合事件强度变化，通过新颖算法检测陈旧像素并用高斯滤波器模糊处理

Result: FIBAR在CPU上处理速度达42-140百万事件/秒，重建图像比神经网络方法噪声更多且有重影，但足以完成标记检测等任务

Conclusion: FIBAR提供了一种简单高效的异步图像重建方法，虽然质量不如神经网络方法，但在特定应用场景下足够使用
...
TL;DR: 提出了一种新的变换双边张量低秩表示模型TBTLRR，通过自适应学习酉变换来更有效地捕捉全局相关性，同时利用双边结构挖掘局部相关性，并整合ℓ₁/₂范数和Frobenius范数来应对复杂噪声。


<details>
  <summary>Details</summary>
Motivation: 现有张量低秩表示方法依赖固定变换且对噪声鲁棒性差，需要开发更灵活有效的模型来处理图像聚类中的复杂噪声和相关性。

Method: 提出TBTLRR模型，引入数据自适应的张量核范数，学习任意酉变换；利用张量数据的双边结构挖掘局部相关性；整合ℓ₁/₂范数和Frobenius范数正则化项；基于ADMM开发高效优化算法。
...
<details>
  <summary>Details</summary>
Motivation: 视频评估和手术数据科学在手术培训、研究和质量改进中有重要作用，但受到异构录制格式和视频共享隐私问题的限制。

Method: 采用软件开发生命周期和用户中心反馈，基于隐私设计架构开发，通过内部和外部临床医生调查评估可用性和采用度，并进行不同硬件配置的基准测试。
...
TL;DR: 本文提出了注意力卷积(ATConv)，通过将自注意力的两个关键特性(自适应路由和侧向抑制)融入卷积操作，解决了传统卷积与自注意力之间的性能差距。


<details>
  <summary>Details</summary>
Motivation: 虽然卷积具有线性复杂度和强视觉先验，但在表达能力上仍落后于自注意力。本文旨在揭示自注意力优于卷积的根本原因，并设计兼具两者优势的新算子。

Method: 提出了ATConv，通过动态调节位置信息流和引入token权重竞争机制，将自注意力的自适应路由和侧向抑制特性融入3×3卷积核中。

Result: ATConv在基础视觉任务中持续优于各种自注意力机制，基于ATConv的AttNet在ImageNet-1K上达到84.4%准确率(仅27M参数)，在扩散模型中替换自注意力可降低FID并加速采样。

Conclusion: ATConv成功弥合了卷积与自注意力之间的性能差距，证明了通过重新设计卷积算子可以捕获自注意力的核心表达能力，同时保持计算效率。
...
TL;DR: 提出StableSketcher框架，通过优化VAE解码器和集成基于视觉问答的奖励函数，提升扩散模型生成手绘草图的质量和文本-图像对齐度。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在生成像素级手绘草图方面存在挑战，特别是抽象表达式的生成质量不足。

Method: 1) 微调变分自编码器优化潜在解码；2) 集成基于视觉问答的新奖励函数用于强化学习，提升文本-图像对齐和语义一致性。

Result: 实验表明StableSketcher生成的草图在风格保真度和提示对齐度方面优于Stable Diffusion基线。

Conclusion: 提出的框架能有效生成高质量手绘草图，同时创建了首个包含实例级草图、标题和问答对的数据集SketchDUO。
...
TL;DR: 该研究探索使用描述性标题作为生物多模态基础模型的额外监督源，通过多模态大语言模型生成合成标题来弥补实例特异性标题稀缺的问题，训练出的BIOCAP模型在物种分类和图文检索方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 生物多模态基础模型缺乏实例特异性描述性标题作为监督信号，这限制了自然语言监督在生物领域的应用，而图像和标题可以看作物种潜在形态空间的互补样本。
...
TL;DR: 提出了一种结合深度学习和物理模型的新系统，用于检测和跟踪快速移动的小物体，在自定义壁球数据集上比卡尔曼滤波器跟踪器减少70%的平均位移误差。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉在一般物体检测和跟踪方面已有显著进展，但快速移动的小物体检测问题仍未充分探索，现有方法存在局限性。
...
<details>
  <summary>Details</summary>
Motivation: 传统光场获取需要计算成本或专用设备，限制了其应用范围。本文旨在从单张图像生成光场，扩展光场的适用性。

Method: 设计神经渲染管线，通过交叉注意力计算源光线间关系，预测目标光线颜色，并迭代更新生成遮挡内容。
...
<details>
  <summary>Details</summary>
Motivation: 现有的后处理方法未能充分利用模型logit空间中的丰富信息，限制了OOD检测的性能。

Method: 提出LogitGap方法，显式利用最大logit与剩余logit之间的关系；引入无训练策略自动识别最具信息量的logit子集进行评分。

Result: 在视觉语言和纯视觉模型上的广泛实验表明，LogitGap在各种OOD检测场景和基准测试中均达到最先进性能。

Conclusion: LogitGap是一种高效且有效的后处理OOD检测方法，通过充分利用logit空间信息显著提升了检测性能。
...
TL;DR: PartNeXt是一个包含23,000多个高质量纹理3D模型的新一代数据集，提供细粒度层次化部件标注，涵盖50个类别，用于推进3D部件理解和多任务评估。


<details>
  <summary>Details</summary>
Motivation: 现有数据集如PartNet依赖无纹理几何和专家标注，限制了可扩展性和实用性。PartNeXt旨在解决这些限制，推动计算机视觉、图形学和机器人学中部件级理解的发展。

Method: 引入PartNeXt数据集，包含高质量纹理3D模型和细粒度层次化部件标注，通过可扩展标注流程和纹理感知标签，支持类无关部件分割和3D部件中心问答等任务。

Result: 在类无关部件分割任务中，现有方法在细粒度和叶级部件上表现不佳；在3D部件中心问答任务中，3D-LLMs在开放词汇部件定位方面存在显著差距；使用PartNeXt训练的Point-SAM相比PartNet有显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 在自动驾驶中，骑行者属于易受伤害的道路使用者，准确估计其姿态对意图分类、行为预测和碰撞避免至关重要。传统6D姿态估计方法无法处理自行车铰接部件的角度变化，这些变化会影响3D边界框和实际行驶方向。

Method: 提出联合估计8D姿态和3D关键点的模型，使用合成和真实图像数据进行训练以泛化到真实图像。8D姿态包括3D平移、3D旋转以及车把和踏板相对于自行车车架的旋转。

Result: 该方法在8D姿态参数估计上表现出有希望的结果，与使用刚性规范对象模板匹配的最先进6D姿态估计器相比达到了有竞争力的分数。

Conclusion: 该方法能够估计更细粒度的自行车姿态状态和行驶方向，为自动驾驶中的骑行者安全提供了更准确的姿态估计能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决组合零样本学习中由于测试时包含未见属性-对象组合导致的分布偏移问题，现有方法在这方面存在性能下降。
...
Conclusion: 该方法通过累积多模态知识和自适应原型更新，有效应对了组合零样本学习中的分布偏移挑战，在多个基准测试中表现出色。
...
TL;DR: 提出IB-GAN，一种基于GAN的无监督解耦表示学习模型，通过信息瓶颈框架优化GAN，在生成器中间层约束输入与输出间的互信息，实现解耦和可解释的潜在空间表示。
...
Method: IB-GAN模型在生成器中引入中间随机层，约束输入与生成输出间的互信息，该中间层作为可学习的潜在分布与生成器联合端到端训练。
...
TL;DR: 提出PPMStereo方法，通过Pick-and-Play Memory模块实现高效的长距离时空一致性深度估计，在保持计算效率的同时提升时间一致性。


<details>
  <summary>Details</summary>
Motivation: 立体视频中的时间一致性深度估计对于增强现实等应用至关重要，但现有方法在建模长时时间一致性和计算效率之间存在根本性权衡。

Method: 受人类两阶段决策过程启发，提出Pick-and-Play Memory模块：'pick'过程选择最相关帧，'play'过程自适应加权选中的帧进行时空聚合，维持紧凑且信息丰富的内存缓冲区。

Result: 在Sintel数据集上取得0.62/1.11 TEPE（相比BiDAStereo提升17.3%和9.02%），计算成本更低，在准确性和时间一致性方面达到最先进性能。

Conclusion: PPMStereo通过两阶段协作过程有效解决了长距离时空一致性建模与计算效率之间的权衡问题，为动态立体匹配提供了高效解决方案。
...
Method: 开发了严格的评估协议：对于I2V使用现有数据集起始帧与真实视频比较；对于T2V开发提示套件探索不同行人密度和交互；关键是从像素空间重建2D鸟瞰轨迹而无需相机参数。

Result: 分析显示领先模型已学习到令人惊讶的有效多智能体行为先验，但存在合并和消失行人等失败模式。

Conclusion: 当前模型在多智能体行为模拟方面表现出色，但合并和消失问题指出了未来改进的方向。
...
TL;DR: 提出SPAN网络，将时序意图定位从离散分类转为连续回归，建模可疑意图的波动演进，显著提升检测性能和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有离散分类方法无法捕捉可疑意图的连续性特征，限制了早期干预和可解释性。需要建模意图的长期依赖和累积效应。

Method: 基于时序点过程理论定义可疑分数公式，引入可疑系数调制和多模态信息调整，提出概念锚定映射方法关联动作与意图概念。

Result: 在HAI数据集上MSE降低19.8%，平均mAP提升1.78%，低频情况下mAP增益达2.74%，能更好捕捉细微行为变化。

Conclusion: 连续可疑建模方法相比离散分类系统能实现更早检测和主动干预，显著增强安全应用中的可解释性和实用性。
...
TL;DR: 该研究系统分析了54个公开脑MRI数据集，揭示了数据规模、模态组成和预处理方法的高度异质性，强调在开发脑MRI基础模型时需要预处理感知和领域自适应策略。
...
Result: 发现健康队列与临床人群之间存在严重不平衡，预处理步骤改善了数据集内一致性但数据集间差异仍然存在，标准化预处理后仍存在可测量的残差协变量偏移。
...
Method: 第一阶段：检索增强提示优化(RAPO)，从关系图中检索语义相关修饰符并重构提示以匹配训练分布。第二阶段：样本特定提示优化(SSPO)，使用多源反馈迭代优化提示。第三阶段：利用SSPO优化的提示对微调重写器LLM。

Result: 在五个最先进的T2V模型和五个基准测试上的广泛实验表明，RAPO++在语义对齐、组合推理、时间稳定性和物理合理性方面取得了显著提升，大幅优于现有方法。

Conclusion: RAPO++是一个模型无关、成本效益高且可扩展的解决方案，为T2V生成中的提示优化设定了新标准。
...
TL;DR: 提出FlowCycle框架，通过目标感知的中间状态和循环一致性约束，实现无需反演的基于流的图像编辑，在保持源图像一致性的同时实现忠实修改


<details>
  <summary>Details</summary>
Motivation: 现有文本图像编辑方法采用目标无关的中间状态构建方式，主要关注源图像重建而忽略与特定编辑目标的语义差距，导致在需要大幅修改时编辑能力有限或不一致

Method: 提出FlowCycle框架，将损坏过程参数化为可学习噪声，通过循环一致性过程进行优化。通过迭代地从源图像编辑到目标图像再恢复回源图像，利用双重一致性约束学习生成目标感知的中间状态

Result: 广泛的消融实验表明，FlowCycle在编辑质量和一致性方面优于最先进的方法

Conclusion: 目标感知的中间状态构建对于文本图像编辑至关重要，FlowCycle通过循环一致性优化实现了更好的编辑效果
...
Method: 使用自监督对比学习框架，结合空间和时间双重对比损失，采用任务特定的采样策略，并通过概率微调方法实现灵活推理。
...
TL;DR: EditInfinity是一个基于VQ生成模型的图像编辑方法，通过精确的中间量化表示实现更有效的图像反演，在PIE-Bench基准测试中优于现有扩散模型方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的图像编辑方法在图像反演过程中存在近似误差，限制了编辑性能。VQ生成模型能够获得源图像的精确中间量化表示，为图像反演提供更有效的监督。

Method: 提出EditInfinity方法，基于二进制量化生成模型Infinity进行适配。包含高效的图像反演机制（整合文本提示修正和图像风格保留）和整体平滑策略，确保编辑结果对源图像的高保真度和与文本提示的精确语义对齐。

Result: 在PIE-Bench基准测试中，在"添加"、"更改"和"删除"编辑操作上，EditInfinity相比最先进的基于扩散模型的基线方法表现出优越性能。

Conclusion: EditInfinity通过利用VQ生成模型的精确中间表示特性，解决了扩散模型图像反演中的近似误差问题，实现了更精确的图像编辑。
...
TL;DR: 论文提出了一种"诱导-检测-抑制"框架来解决大型视觉语言模型在长文本生成中的幻觉问题，认为幻觉风险主要源于对上下文连贯性的依赖而非文本长度本身。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在生成长文本时容易出现幻觉问题，传统观点认为这是由长度引起的错误累积导致的。本文旨在探究幻觉问题的深层机制，验证是否是由于对上下文连贯性的依赖增加所致。

Method: 提出"诱导-检测-抑制"三阶段框架：通过设计特定上下文主动诱导幻觉，利用诱导实例进行早期高风险检测，在实际解码过程中抑制潜在的对象级幻觉。

Result: 该方法在所有基准测试中都取得了显著且一致的改进，证明了框架的有效性。强大的检测能力和改进的幻觉缓解不仅验证了框架，更重要的是重新验证了关于上下文的假设。

Conclusion: 研究不仅追求性能提升，更旨在为深入探索大型视觉语言模型长文本响应中的幻觉问题提供新的见解，是迈向更深层次研究的第一步。
...
TL;DR: SELM-SLAM3是一个基于深度学习的视觉SLAM框架，通过集成SuperPoint和LightGlue进行鲁棒的特征提取和匹配，在低纹理、运动模糊等挑战性条件下显著优于传统SLAM系统。


<details>
  <summary>Details</summary>
Motivation: 传统SLAM技术在低纹理、运动模糊和挑战性光照条件下性能不佳，这在视觉障碍者辅助导航等应用中尤为关键，影响定位精度和跟踪稳定性。

Method: 提出SELM-SLAM3框架，集成SuperPoint进行特征提取和LightGlue进行特征匹配，使用深度学习增强视觉SLAM的鲁棒性。
...
Conclusion: SELM-SLAM3为视觉障碍者导航辅助系统提供了一个可靠的平台，在挑战性条件下具有增强的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于高斯泼溅的开放词汇3D分割方法要么依赖单一的3D语言场导致分割效果不佳，要么依赖预计算的类无关分割导致错误累积。需要一种能有效整合语言和分割线索的方法。

Method: 提出协作场概念（包含实例场和语言场），通过实例到语言特征映射和两阶段训练策略构建协作场，在推理时使用自适应语言到实例提示精炼来桥接两个场的特性。

Result: 在多个基准测试中表现领先，展示了在图像驱动的3D分割、分层分割和机器人应用中的高潜力。

Conclusion: COS3D通过协作式框架有效解决了开放词汇3D分割的挑战，在性能和适用性方面都表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 解决当前大型语言模型和多模态模型在具身任务中的局限性，包括模型设计与代理需求之间的差距、实时延迟与性能的权衡，以及离线评估指标的不真实性。

Method: 采用代理对齐的数据结构，结合大规模监督微调(SFT)和步骤增强的组相对策略优化(Step-GRPO)，并整合了生成奖励模型(GRM)来提升训练效率。
...
### [34] [Empower Words: DualGround for Structured Phrase and Sentence-Level Temporal Grounding](https://arxiv.org/abs/2510.20244)
...
Conclusion: 解耦的语义建模在视频-语言对齐中是有效的，能够同时捕捉粗粒度和局部语义，实现更具表达力和上下文感知的视频定位
...
<details>
  <summary>Details</summary>
Motivation: 传统LiDAR投影方法存在几何不一致性，导致不可逆的信息损失，影响高保真应用。需要一种无需制造商数据即可实现无损投影的方法。

Method: 通过自动逆向工程推断旋转LiDAR传感器的关键参数，包括激光束配置、角度分布和每束激光的校准校正，实现无损投影。
...
TL;DR: 提出EDGeo框架，通过掩码位置编码和上下文增强模块，在跨视角对象地理定位中实现最先进性能，定位精度提升3.39%。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖基于关键点的位置编码，仅捕获2D坐标而忽略对象形状信息，导致对标注偏移敏感且跨视角匹配能力有限。

Method: 提出掩码位置编码方案利用分割掩码捕获空间坐标和对象轮廓；设计上下文增强模块使用水平和垂直条带卷积核提取长距离上下文特征；整合为EDGeo端到端框架。

Result: 在两个公开数据集(CVOGL和VIGOR-Building)上的实验表明，该方法在具有挑战性的地面到卫星场景中实现了最先进的性能，定位精度提高了3.39%。
...
TL;DR: 提出校准多模态共识模型(CMC)，通过伪标签生成模块实现自监督单模态预训练，使用参数无关融合模块和多模态共识路由器缓解文本主导问题，在四个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态情感识别方法忽视模态间语义不一致问题，且常被文本模态主导而影响识别准确性。

Method: CMC包含伪标签生成模块(PLGM)进行自监督单模态预训练，参数无关融合模块(PFM)和多模态共识路由器(MCR)进行多模态微调。

Result: 在CH-SIMS、CH-SIMS v2、CMU-MOSI、CMU-MOSEI四个数据集上达到或超越SOTA方法，在语义不一致场景下表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 智能手机已成为日常生活必需品，包括对视障人士。日常任务如独立处理货币对他们来说很困难，因此需要开发辅助系统来提高他们的独立性。
...
TL;DR: 提出了GMFVAD方法，通过细粒度多模态特征减少视频特征冗余，提升视频异常检测性能


<details>
  <summary>Details</summary>
Motivation: 现有方法在引入文本特征时方式粗糙，忽略了视频片段中存在的大量冗余信息，需要利用多模态信息的多样性来精炼特征

Method: 基于视频片段生成细粒度多模态特征，总结主要内容，并引入原始视频字幕的文本特征来增强视觉特征的突出部分

Result: 在四个主要数据集上达到了最先进的性能，消融实验验证了性能提升源于冗余信息的减少

Conclusion: GMFVAD通过减少特征冗余有效提升了视频异常检测性能，证明了细粒度多模态特征的重要性
...
<details>
  <summary>Details</summary>
Motivation: 现有VCR方法虽然预测准确率高，但忽视了数据集中的偏见问题，缺乏有效的去偏见策略。研究发现文本和视觉数据中都存在共现和统计偏见。

Method: 分析VCR中的因果图和预测捷径，采用后门调整方法消除偏见。基于正确答案集合创建字典来消除预测捷径。

Result: 实验证明该去偏见方法在不同数据集上均有效。

Conclusion: 提出的去偏见方法能有效提升VCR模型的泛化能力，VCR-OOD数据集为评估模型性能提供了新基准。
...
<details>
  <summary>Details</summary>
Motivation: 解决CV-SAR图像识别中泛化性、可解释性和效率之间的冲突三难问题，充分利用CV-SAR数据中丰富的电磁散射特征

Method: 基于"压缩-聚合-压缩"架构的KINN框架，包含物理引导压缩、聚合模块和语义压缩，支持CNN和ViT两种变体

Result: 在5个SAR基准测试中达到参数高效识别的最先进水平，在数据稀缺和分布外场景下表现出优异的泛化能力和可解释性
...
Conclusion: DMC³框架有效提升了第一人称视频问答的性能，证明了反事实对比学习方法的有效性。
...
TL;DR: SLYKLatent提出了一种新颖的注视估计方法，通过自监督学习和补丁三分支网络解决数据集外观不稳定性问题，在多个基准数据集上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决注视估计中由于随机不确定性、协变量偏移和测试域泛化导致的数据集外观不稳定性挑战。
...
Conclusion: 消融研究证实了SLYKLatent新颖组件的有效性，该方法在注视估计任务中显著优于现有方法。
...
TL;DR: 提出Instruction-as-Reasoning范式，将指令视为动态分析路径，通过两阶段训练框架（SFT+RL）优化GUI grounding性能，在多个基准测试中达到SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有GUI grounding方法将指令视为静态代理，忽略了指令多样性和质量对性能的影响。研究发现现有数据集中23.3%的指令存在缺陷，且利用指令多样性可带来76%的相对性能提升。

Method: 采用两阶段训练框架：首先在合成的多样化指令上进行监督微调，培养多视角推理能力；然后通过强化学习优化路径选择和组合。

Result: UI-Ins-7B和UI-Ins-32B在五个基准测试中达到SOTA：UI-I2E-Bench 87.3%、ScreenSpot-Pro 57.0%、MMBench-GUI L2 84.9%。UI-Ins-7B在AndroidWorld上达到74.1%的成功率。

Conclusion: Instruction-as-Reasoning范式能显著提升GUI grounding性能，模型展现出涌现推理能力，能在推理时选择性组合和合成新的指令路径。
...
TL;DR: 该研究评估了现代视频基础模型在街舞分类任务中的表现，发现视频编码器模型在预测任务上优于最先进的视频语言模型。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在体育应用中广泛使用，但大多局限于足球、板球、篮球等流行运动。本研究旨在探索现代视频基础模型在街舞这一小众但流行的舞蹈运动中的适用性。

Method: 使用现代视频基础模型（包括编码器和解码器），对街舞视频分类任务进行微调和分析。

Result: 视频编码器模型在预测任务上继续优于最先进的视频语言模型。研究提供了如何选择编码器模型的见解，并对微调后的解码器模型在街舞视频分类中的工作机制进行了深入分析。

Conclusion: 对于街舞等特定体育运动的视频分类任务，视频编码器模型仍然是更有效的选择，而非视频语言模型。
...
TL;DR: 本文提出了RoboSense 2025 Track 4竞赛的获胜解决方案，通过领域对齐预处理和专家混合框架解决跨平台异质性和领域差距问题，在异构视角下实现了鲁棒的跨模态地理定位。


<details>
  <summary>Details</summary>
Motivation: 解决跨平台无人机导航任务中的两个主要挑战：严重的平台间异质性和通用训练描述与平台特定测试查询之间的领域差距。

Method: 使用领域对齐预处理管道（平台划分、卫星增强、方向词移除）和LLM驱动的标题精炼管道，结合BGE-M3和EVA-CLIP模型，采用渐进式两阶段硬负样本挖掘策略训练三个平台专家，并在推理时融合专家得分。

Result: 该系统在官方排行榜上排名第一，展示了在异构视角下鲁棒的跨模态地理定位能力。

Conclusion: 提出的领域对齐预处理和专家混合框架有效解决了跨平台异质性和领域差距问题，为跨模态地理定位任务提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs需要大量计算资源进行训练，主要原因是广泛使用的视觉编码器（如CLIP、SAM）缺乏与语言在多粒度级别的对齐，导致效率低下。
...
Conclusion: HyperET为MLLMs提供了一种高效的多粒度对齐训练范式，通过双曲空间建模有效解决了视觉与文本模态间的粒度差距问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决基于深度学习的点云几何压缩中的泛化挑战，主要源于缺乏鲁棒的上下文模型和低效的OOD数据处理。
...
Conclusion: AnyPcc通过创新的上下文建模和自适应微调策略，有效解决了点云压缩的泛化问题，显著提升了压缩性能。
...
TL;DR: AccuQuant是一种用于扩散模型的后训练量化方法，通过最小化多步去噪过程中的累积误差来提升量化效果，同时提出了内存复杂度从O(n)降低到O(1)的高效实现技术。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在采样过程中，量化误差会在多个去噪步骤中累积，导致性能下降。传统方法独立处理每个步骤的误差，无法有效解决累积误差问题。

Method: AccuQuant通过显式模拟扩散采样过程中的多个去噪步骤进行量化，考虑多步去噪中的累积误差，并提出高效实现技术降低内存复杂度。
...
TL;DR: 本文发现扩散变换器(DiTs)中的补丁令牌具有高度独立性，空间一致性主要由位置编码控制。作者提出位置编码场(PE-Field)，将2D位置编码扩展到结构化3D场，在单图像新视角合成和可控空间图像编辑中取得最先进性能。


<details>
  <summary>Details</summary>
Motivation: 重新审视DiTs如何组织视觉内容，发现补丁令牌表现出惊人的独立性：即使位置编码被扰动，DiTs仍能产生全局一致的输出，表明空间一致性主要由位置编码控制。

Method: 引入位置编码场(PE-Field)，将位置编码从2D平面扩展到结构化3D场，包含用于体积推理的深度感知编码和用于细粒度子补丁控制的分层编码。

Result: PE-Field增强的DiT在单图像新视角合成方面达到最先进性能，并能推广到可控的空间图像编辑。

Conclusion: PE-Field使DiTs能够直接在3D空间中建模几何，为视觉生成提供了更强大的空间推理能力。
...
TL;DR: 提出了一种新颖的因果方法，通过预测图像中可能被忽略的烹饪元素，并将这些元素显式注入跨模态表示学习来缓解偏差，从而改善图像到食谱检索的性能。


<details>
  <summary>Details</summary>
Motivation: 现有图像到食谱检索方法假设食物图像能完全捕捉食谱文本细节，但实际上图像只反映烹饪结果的视觉外观，忽略了烹饪过程中的细微差异，导致表示学习偏向主导视觉元素，难以区分具有细微差异的相似食谱。

Method: 采用因果方法预测图像中可能被忽略的烹饪元素，并将这些元素显式注入跨模态表示学习，以缓解表示学习中的偏差。

Result: 在标准单语Recipe1M数据集和新构建的多语言多文化美食数据集上的实验表明，该方法能够发现细微的食材和烹饪动作，在单语和多语言多文化数据集上都取得了令人印象深刻的检索性能。

Conclusion: 提出的因果表示学习方法能够有效缓解图像到食谱检索中的表示偏差，提高对细微烹饪差异的识别能力，在单语和多语言多文化场景下均表现出色。
...
TL;DR: 提出FuzzyDistillViT-MobileNet模型，使用动态模糊逻辑驱动的知识蒸馏方法进行肺癌分类，通过动态调整蒸馏权重处理医学图像中的不确定性，在LC25000和IQOTH/NCCD数据集上分别达到99.16%和99.54%的准确率。


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏方法使用固定权重，无法有效处理医学图像诊断中的不确定性和复杂性。需要动态调整蒸馏权重，让学生模型专注于高置信度区域，减少对模糊区域的关注。
...
Conclusion: 动态模糊逻辑驱动的知识蒸馏方法能有效处理医学图像分类中的不确定性，显著提升分类性能，同时通过遗传算法优化实现了性能与计算成本的平衡。
...
TL;DR: Conan是一个基于证据的多步骤视频推理框架，通过识别上下文和证据帧、跨帧推理以及自适应决策来提升视频推理能力，在多个基准测试中显著优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有的视频推理方法存在局限性：基于强化学习的方法依赖纯文本推理链，容易产生无根据或幻觉的结论；基于帧检索的方法虽然引入了视觉基础，但在证据定位方面仍不准确。

Method: 构建了Conan-91K大规模自动生成推理轨迹数据集，包含帧识别、证据推理和行动决策；设计了多阶段渐进式冷启动策略与识别-推理-行动强化学习训练框架，联合增强多步骤视觉推理。

Result: 在六个多步骤推理基准测试中，Conan平均比基线Qwen2.5-VL-7B-Instruct准确率提升超过10%，达到最先进性能；在长视频理解任务中表现出良好的泛化能力。

Conclusion: Conan框架通过证据基础的多步骤视频推理方法有效解决了现有方法的局限性，在准确性和泛化能力方面都表现出色，验证了其强大的可扩展性和鲁棒性。
...
TL;DR: 提出了一种可复现的人口属性推断管道，用于提高人脸分析系统公平性评估的可靠性，通过模块化迁移学习方法替代传统端到端训练，在准确性、公平性和鲁棒性方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 人脸分析系统的公平性评估依赖于自动人口属性推断，但现有方法的可靠性存在问题，影响了公平性审计的有效性。

Method: 采用模块化迁移学习方法，将预训练的人脸识别编码器与非线性分类头结合，构建可复现的人口属性推断管道。

Result: 在性别和种族推断任务中，该方法优于强基线，特别是在更具挑战性的种族属性上表现更佳。

Conclusion: 这项工作为公平性审计中的人口属性推断提供了可靠基础，通过提高推断可靠性来减少公平性评估的偏差和方差。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大推理模型存在两个关键局限：对简单查询也使用计算昂贵的推理导致效率低下，以及专注于专门推理会损害更广泛的通用理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的一步扩散模型在个性化新概念方面能力有限，需要开发能够在单步生成中有效整合新概念的方法。

Method: 采用端到端训练，同时训练多步扩散模型(教师)和一步扩散模型(学生)，通过双向概念蒸馏和回声反馈机制，共享文本编码器，并使用对抗损失和对齐损失优化学生模型。

Result: 实验表明该框架在1-SDP设置下显著优于现有个性化方法，不仅提升了学生模型的新概念个性化能力，还改善了教师模型的生成质量。

Conclusion: EchoDistill为T2I扩散模型提供了一种快速有效的个性化新范式，通过双向协作机制实现了概念蒸馏和生成质量的共同提升。
...
Method: 首先定义面部图像概念树(FICT)对面部图像进行细粒度区域划分，获得可靠的数据标注流程FiFa-Annotator；然后提出Artifact-Grounding Explanation任务，生成带有分割掩码的文本伪造解释；最后设计统一的多任务学习架构FiFa-MLLM，支持丰富的多模态输入输出。

Result: FiFa-MLLM在AGE任务上优于强基线模型，在现有XDFA数据集上达到SOTA性能。

Conclusion: FiFa框架通过细粒度数据标注和统一的多任务架构，显著提升了多模态大语言模型在可解释深度伪造分析中的性能，解决了细粒度感知不足的问题。
...
TL;DR: Open-o3 Video是一个非代理框架，通过整合显式的时空证据到视频推理中，在生成答案的同时突出关键时间戳、对象和边界框，使推理基于具体的视觉观察。


<details>
  <summary>Details</summary>
Motivation: 现有的视频推理模型仅生成文本推理轨迹，而不指示关键证据出现的时间和位置。将基于证据的推理能力扩展到视频更具挑战性，需要跨动态场景的联合时间跟踪和空间定位。

Method: 首先构建了两个高质量数据集STGR-CoT-30k和STGR-RL-36k，提供统一的时空监督和推理轨迹；然后采用冷启动强化学习策略，设计了多个专门奖励来共同鼓励答案准确性、时间对齐和空间精度。

Result: 在V-STAR基准测试中，Open-o3 Video实现了最先进的性能，在Qwen2.5-VL基线上将mAM提高了14.4%，mLGM提高了24.2%。在VideoMME、WorldSense、VideoMMMU和TVGBench等广泛视频理解基准上也观察到一致的改进。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在领域转移时性能急剧下降，无监督领域适应旨在减少领域差距并避免昂贵的新领域标注

Method: 在嵌入空间中学习类别原型，引入相似性约束使原型具有代表性且类别间可分离，使用字典存储不同图像的原型以解决类别缺失问题并实现对比学习

Result: 大量实验表明该方法优于其他最先进方法

Conclusion: 基于相似性原型的跨模态分割框架能有效提升领域适应性能
...
TL;DR: 提出了一种联合估计潜在清晰图像和相机运动轨迹的深度学习框架，通过可微分模糊创建模块和端到端训练，在严重或空间变化模糊情况下实现最先进的去模糊性能。


<details>
  <summary>Details</summary>
Motivation: 相机抖动导致的大范围或旋转运动引起的运动模糊仍然是图像恢复中的主要挑战，传统端到端去模糊网络在严重或空间变化模糊情况下表现不佳。

Method: 使用投影运动模糊模型(PMBM)，通过神经网络预测完整的3D旋转轨迹，指导基于模型的恢复网络进行端到端训练，并通过重模糊损失优化轨迹以提高一致性。

Result: 在合成和真实数据集上均达到最先进的性能，特别是在严重或空间变化模糊情况下表现优于端到端去模糊网络。

Conclusion: 该方法通过联合估计清晰图像和相机运动轨迹，在保持可解释性的同时显著提升了去模糊性能，代码和训练模型已开源。
...
TL;DR: OnlineSplatter是一个在线前馈框架，能够从单目视频中直接重建自由移动物体的高质量3D高斯表示，无需相机位姿、深度先验或捆绑优化。
...
Method: 基于第一帧锚定重建，通过密集高斯基元场逐步优化物体表示；核心是双键记忆模块，结合潜在外观-几何键和显式方向键，通过空间引导记忆读取和高效稀疏化机制融合当前帧特征与时序聚合的物体状态。

Result: 在真实世界数据集上的评估表明，OnlineSplatter显著优于最先进的免位姿重建基线方法，随着观测增加持续改进，同时保持恒定的内存和运行时间。

Conclusion: 该方法实现了高质量、物体中心的3D高斯重建，能够有效处理自由移动物体，具有恒定的计算成本，不受视频序列长度影响。
...
TL;DR: 这篇论文系统综述了基于深度学习的牙科图像自动分析研究，涵盖了260项研究，包括49篇公开牙科数据集论文和211篇深度学习算法论文，总结了该领域的数据集、模型架构、评估指标及未来方向。


<details>
  <summary>Details</summary>
Motivation: 牙科图像分析面临低对比度、金属伪影和投影角度变化等挑战，加上临床医生主观性差异，导致手动分析耗时且不一致。AI驱动的自动分析为解决这些问题提供了有前景的方案。

Method: 系统回顾260项深度学习在牙科图像分析中的应用研究，分析公开数据集特征和获取方法，分类整理不同任务下的深度学习模型架构、优化策略、训练方法和性能表现。

Result: 总结了牙科成像基础概念、现有数据集特点、深度学习基础技术，并按不同牙科图像分析任务对相关模型算法进行了系统分类和分析。

Conclusion: 该工作为研究人员提供了有价值的系统性参考，讨论了当前研究面临的挑战并展望了未来潜在发展方向，所有补充材料和详细比较表将在GitHub上公开。
...
### [63] [From Cheap to Pro: A Learning-based Adaptive Camera Parameter Network for Professional-Style Imaging](https://arxiv.org/abs/2510.20550)
...
Method: 框架包含两个模块：ACamera-Exposure估计ISO缓解曝光不足和对比度损失，ACamera-Color预测相关色温和增益因子改善色彩一致性。针对边缘设备实时推理优化，可无缝集成到成像管线中。

Result: 在多样化真实世界数据上训练，模型在不同光照条件下泛化良好。实验表明ACamera-Net持续提升图像质量并稳定感知输出，优于传统自动模式和轻量级基线。

Conclusion: ACamera-Net无需依赖额外图像增强模块，就能有效解决复杂光照条件下的图像质量问题，为边缘设备提供实用的相机参数优化方案。
...
### [64] [From Far and Near: Perceptual Evaluation of Crowd Representations Across Levels of Detail](https://arxiv.org/abs/2510.20558)
*Xiaohan Sun,Carol O'Sullivan*

Main category: cs.CV

TL;DR: 研究用户对不同细节层次和观看距离下人群角色表示视觉质量的感知，比较几何网格、图像替身、NeRF和3D高斯等表示方法的视觉保真度与计算性能权衡。
...
Method: 使用定性分析和定量评估方法，比较几何网格、图像替身、NeRF和3D高斯等表示在不同细节层次和观看距离下的表现。

Result: 不同表示方法在视觉保真度和计算性能方面存在明显差异，为人群渲染的感知优化提供了具体见解。

Conclusion: 研究结果为设计感知优化的细节层次策略提供了重要指导，有助于在视觉质量和计算效率之间找到最佳平衡。
...
### [65] [Small Drafts, Big Verdict: Information-Intensive Visual Reasoning via Speculation](https://arxiv.org/abs/2510.20812)
*Yuhan Liu,Lianhui Qin,Shengjie Wang*

Main category: cs.CV

TL;DR: 提出Speculative Verdict (SV)框架，通过多个轻量级草稿专家生成多样化定位候选，再由大型裁决模型合成最终答案，在信息密集型视觉问答任务中实现高效准确推理。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在处理信息密集型图像时面临挑战，这些图像密集交织文本标注和细粒度图形元素，需要精确定位关键线索和多跳推理来整合分散证据。

Method: 采用训练免费的推测解码框架，包含草稿阶段和裁决阶段。草稿阶段使用小型VLMs作为草稿专家生成多样化推理路径；裁决阶段使用强大VLM合成这些路径产生最终答案。引入共识专家选择机制，仅转发高一致性推理路径。

Result: 在InfographicVQA、ChartMuseum、ChartQAPro和HR-Bench 4K等挑战性信息密集型和高分辨率视觉问答基准测试中取得一致性能提升。

Conclusion: 通过合成多个部分准确推理路径中的正确见解，SV相比大型专有模型或训练流程实现了错误校正和成本效率的双重优势。
...
### [66] [GenColorBench: A Color Evaluation Benchmark for Text-to-Image Generation Models](https://arxiv.org/abs/2510.20586)
*Muhammad Atif Butt,Alexandra Gomez-Villa,Tao Wu,Javier Vazquez-Corral,Joost Van De Weijer,Kai Wang*

Main category: cs.CV

TL;DR: 提出了GenColorBench，首个全面的文本到图像颜色生成基准，包含44K个颜色相关提示，覆盖400+颜色，通过感知和自动评估揭示模型在颜色控制方面的真实能力。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像生成模型在细粒度颜色可控性方面表现不佳，现有基准缺乏对颜色精度的系统性评估，而颜色在视觉感知和应用中至关重要。

Method: 基于ISCC-NBS和CSS3/X11颜色系统构建基准，包含数值颜色等缺失元素，通过44K个颜色相关提示进行感知和自动评估。

Result: 对流行文本到图像模型的评估显示性能差异，揭示了模型理解最佳的颜色约定和失败模式。

Conclusion: GenColorBench将为精确颜色生成的改进提供指导，基准将在接受后公开。
...
### [67] [Towards General Modality Translation with Contrastive and Predictive Latent Diffusion Bridge](https://arxiv.org/abs/2510.20819)
*Nimrod Berman,Omkar Joglekar,Eitan Kosman,Dotan Di Castro,Omri Azencot*

Main category: cs.CV

TL;DR: 提出了一种用于模态转换的通用框架LDDBM，通过在共享潜在空间中学习任意模态之间的桥梁，解决了现有方法在维度对齐、高斯先验和特定架构方面的限制。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在单模态生成方面表现出色，但扩展到跨模态转换仍面临挑战。现有方法存在维度对齐要求、高斯先验假设和特定架构限制，缺乏通用性和理论基础。

Method: 基于潜在变量扩展的Denoising Diffusion Bridge Models，在共享潜在空间中操作，引入对比对齐损失确保语义一致性，设计领域无关的编码器-解码器架构进行潜在空间噪声预测，并提出预测损失指导跨域转换训练。

Result: 该方法支持任意模态对，在多视图到3D形状生成、图像超分辨率和多视图场景合成等多样化模态转换任务中表现优异，建立了通用模态转换的新强基线。

Conclusion: LDDBM提供了一个通用且有效的模态转换框架，通过潜在空间操作和多种训练策略，成功克服了现有方法的局限性，在多个任务上验证了其有效性。
...
### [68] [SeViCES: Unifying Semantic-Visual Evidence Consensus for Long Video Understanding](https://arxiv.org/abs/2510.20622)
*Yuan Sheng,Yanbin Hao,Chenxu Li,Shuo Wang,Xiangnan He*

Main category: cs.CV

TL;DR: SeViCES是一个无需训练、模型无关的框架，通过语义-视觉共识证据选择来解决长视频理解问题，在准确性和鲁棒性上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 长视频理解面临计算复杂度高、推理不聚焦不一致的挑战，现有帧选择方法忽略时间依赖或依赖单模态证据，无法提供完整且与查询相关的上下文。

Method: 包含两个关键组件：1) SVCFS模块通过时间感知语义分支和聚类引导视觉分支选择帧；2) ACR模块通过融合证据和约束答案空间解决语义与视觉预测的不一致性。
...
TL;DR: BTB3D提出了一种因果卷积编码器-解码器架构，通过频率感知的体素标记化方法，解决了3D医学影像中高分辨率、长序列体积的处理难题，在报告生成和文本到CT合成任务上取得了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前3D医学影像的视觉语言模型在处理高分辨率、长序列体积时面临挑战：对比预训练产生的视觉编码器与临床语言不匹配，切片级标记化模糊了精细解剖结构，降低了下游任务的诊断性能。

Method: BTB3D采用因果卷积编码器-解码器架构，统一2D和3D训练与推理，生成紧凑的频率感知体素标记。通过三阶段训练课程：(i)局部重建、(ii)重叠窗口平铺、(iii)长上下文解码器精炼，模型能从短切片片段学习并泛化到超过300个切片的扫描。

Result: BTB3D在两个关键任务上创下新纪录：报告生成方面，BLEU分数提升，临床F1比CT2Rep、CT-CHAT和Merlin提高40%；文本到CT合成方面，FID降低75%，FVD比GenerateCT和MedSyn减半，生成解剖学一致的512*512*241体积。
...
<details>
  <summary>Details</summary>
Motivation: 解决超高分辨率文本到图像生成的两个关键挑战：缺乏大规模高质量数据集和忽视细节合成的专门训练策略。

Method: 引入UltraHR-100K数据集（10万张3K+分辨率图像），并提出频率感知后训练方法，包括细节导向时间步采样和软加权频率正则化。

Result: 在UltraHR-eval4K基准测试中显著提升了超高分辨率图像生成的细节质量和整体保真度。

Conclusion: 该方法有效解决了超高分辨率文本到图像生成的细节合成问题，为相关研究提供了高质量数据集和训练策略。
...
TL;DR: 提出HybridSOMSpikeNet混合深度学习框架，用于智能高效的垃圾分类，准确率达97.39%，优于现有方法，支持可持续废物管理。
...
Method: 集成卷积特征提取、可微分自组织和脉冲时序处理的混合框架，使用ResNet-152提取空间特征，Soft-SOM增强聚类可解释性，脉冲神经网络头提高鲁棒性。

Result: 在十类垃圾数据集上达到97.39%的测试准确率，优于多种先进架构，同时保持轻量计算特性适合实际部署。

Conclusion: 该框架通过精确自动化的垃圾分拣支持更高回收效率，减少可回收物流污染，降低废物处理生态成本，符合联合国可持续发展目标。
...
<details>
  <summary>Details</summary>
Motivation: 现有多比特量化网络需要为每个支持的比特宽度重复全数据集更新，训练成本随精度数量线性增长，且通常需要额外的微调阶段来支持更多精度选项。
...
Conclusion: 所提出的方法显著降低了多比特量化网络的训练开销，同时保持了模型效用，为灵活部署深度神经网络提供了高效解决方案。
...
TL;DR: 该论文提出了一种基于代理的架构，结合LLM推理和轻量级视觉模块，以解决多模态大语言模型中的视觉幻觉和过度依赖文本先验问题，在多个基准测试上取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在复杂视觉任务中仍存在视觉幻觉和过度依赖文本先验的问题，需要进行系统诊断和改进。

Method: 提出基于代理的架构，将LLM推理与轻量级视觉模块结合，实现细粒度分析和推理链的迭代优化。

Result: 在MMMU基准上提升10.3分，MathVista上提升6.0分（相比7B基线），性能达到或超过更大模型。
...
Conclusion: MixKV通过混合重要性和多样性，有效解决了多模态KV缓存中的语义冗余问题，显著提升了压缩性能，并可无缝扩展到LLMs中。
...
TL;DR: AutoScape是一个长时域驾驶场景生成框架，通过RGB-D扩散模型生成几何一致的关键帧，然后用视频扩散模型插值生成密集连贯的视频帧，显著提升了长时域驾驶视频的生成质量。


<details>
  <summary>Details</summary>
Motivation: 解决长时域驾驶场景生成中几何一致性和视觉连贯性的挑战，现有方法在长时间序列中难以保持几何一致性。

Method: 1) 在共享潜在空间中联合处理图像和深度；2) 显式条件化先前生成关键帧的几何信息；3) 使用扭曲一致引导的采样过程；4) 基于高质量RGB-D关键帧进行视频插值。

Result: 生成了超过20秒的真实且几何一致的驾驶视频，长时域FID和FVD分数相比现有最优方法分别提升了48.6%和43.0%。
...
Conclusion: 所提出的双编码器模型通过注意力驱动特征融合有效提升了组织病理学图像语义分割性能，代码已开源。
...
TL;DR: DyPE是一种无需训练的方法，通过动态调整扩散模型的位置编码来匹配生成过程的不同阶段，使预训练扩散变换器能够生成远超训练分辨率的图像。
...
Result: 能够生成高达1600万像素的图像（使用FLUX模型），在多个基准测试中实现最先进的超高分辨率图像生成性能。

Conclusion: DyPE提供了一种高效的无训练方法，显著提升了扩散变换器在超高分辨率图像生成方面的能力。
...
TL;DR: α-Flow是一种新的生成建模目标函数，通过平滑地从轨迹流匹配过渡到MeanFlow来解决优化冲突，在ImageNet-256上取得了SOTA结果。
...
Method: 提出α-Flow目标函数族，统一了轨迹流匹配、Shortcut Model和MeanFlow，采用课程学习策略平滑地从轨迹流匹配过渡到MeanFlow。

Result: 在ImageNet-1K 256x256上，α-Flow始终优于MeanFlow，α-Flow-XL/2+模型取得了FID 2.58(1步)和2.15(2步)的SOTA结果。
...
<details>
  <summary>Details</summary>
Motivation: 解决从单张2D图像准确推断相机姿态、3D形状和纹理的挑战，将3D重建构建为条件采样过程，实现姿态和形状估计的统一生成框架。

Method: 采用两阶段流匹配管道：粗阶段生成初始3D几何和2D投影用于姿态恢复；精阶段集成姿态对齐图像特征以增强结构保真度和外观细节。

Result: 在广泛实验中，Cupid在PSNR上获得超过3dB增益，Chamfer距离减少超过10%，在姿态精度上匹配单目估计器，在视觉保真度上优于基线3D生成模型。

Conclusion: Cupid通过统一的生成框架实现了从单图像到3D重建的准确推断，在多个指标上显著优于现有领先方法，展示了卓越的重建性能。
...
TL;DR: 提出了一个融合雷达和摄像头数据的多目标跟踪框架，通过在线雷达-摄像头校准简化传感器集成，利用共同特征提高跟踪精度。


<details>
  <summary>Details</summary>
Motivation: 现有研究往往低估雷达的作用，仅将其作为辅助传感器，尽管雷达能够提供准确的目标距离/深度信息。本文旨在充分发挥雷达在3D世界坐标系中的优势。

Method: 开发雷达-摄像头融合MOT框架，利用在线校准自主关联两个传感器的检测结果；使用共同特征准确推导检测对象的真实世界位置；采用特征匹配和类别一致性检查超越单纯位置匹配的限制。

Result: 框架能够简化雷达-摄像头映射过程并提高跟踪精度，在受控环境和实际交通场景中的实验验证了其有效性。

Conclusion: 这是首个研究雷达-摄像头共同特征集成及其在在线校准中用于实现多目标跟踪的工作，代码已开源。
...
TL;DR: ARGenSeg是一种基于自回归生成的图像分割新范式，通过统一的框架实现多模态理解和像素级感知，使用图像生成方法输出密集掩码，显著提升推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用边界点表示或专用分割头，依赖离散表示或语义提示，限制了多模态大语言模型捕捉细粒度视觉细节的能力。

Method: 基于图像生成的分割框架，利用MLLM输出视觉标记，通过通用VQ-VAE解码为图像，采用下一尺度预测策略并行生成视觉标记以减少推理延迟。

Result: 在多个分割数据集上超越先前最先进方法，推理速度显著提升，同时保持强大的理解能力。

Conclusion: ARGenSeg通过统一的生成式框架成功实现了多模态理解和像素级分割，在性能和效率方面均有显著优势。
...
TL;DR: 提出基于纯Transformer的自回归视频预测模型，在物理模拟数据集上通过连续像素空间表示实现长时间准确的物理预测，相比现有方法将物理准确预测时间延长50%。
...
Method: 使用纯Transformer模型进行自回归视频预测，采用连续像素空间表示，比较不同时空自注意力布局，无需复杂训练策略或潜在特征学习组件。

Result: 在物理模拟数据集上，相比潜在空间方法将物理准确预测时间延长50%，同时在常见视频质量指标上保持可比性能。通过可解释性实验识别出网络中对PDE模拟参数估计有用的区域。
...
TL;DR: SpectraMorph是一个物理引导的自监督高光谱超分辨率框架，通过解混瓶颈将高光谱图像与多光谱图像融合，无需监督训练即可实现高性能超分辨率。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法缺乏可解释性，且在MSI波段很少时表现不佳。需要一种能够处理单波段MSI的稳健且可解释的融合方法。
...
Conclusion: SpectraMorph提供可解释的中间结果，训练时间不到一分钟，即使在单波段MSI情况下也能保持稳健性能。
...
Method: 提出分层画布表示，每个主体位于独立图层；引入锁定机制，保持选定图层高保真度，同时允许其他图层灵活适应上下文。

Result: 在多个主体个性化图像生成中，LayerComposer在空间控制和身份保持方面优于现有最先进方法。
...
TL;DR: HoloCine是一个解决文本到视频生成中"叙事鸿沟"的模型，能够整体生成连贯的多镜头场景，确保从第一个镜头到最后一个镜头的全局一致性。


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频模型擅长生成独立片段，但在创建连贯的多镜头叙事方面存在不足，而连贯叙事是故事讲述的核心。
...
### [86] [Configuration-Dependent Robot Kinematics Model and Calibration](https://arxiv.org/abs/2510.19962)
...
<details>
  <summary>Details</summary>
Motivation: 机器人精确运动学对于工具精确定位至关重要，但非几何因素会引入配置依赖的模型差异，需要在整个工作空间中提高精度。

Method: 采用局部POE模型（参数化连续性好），在多个配置下识别并插值为全局模型，使用肩部和肘部关节角参数化的傅里叶基函数进行插值。

Result: 在两个6自由度工业机器人上验证，最大定位误差减少超过50%，达到冷喷涂制造所需的亚毫米精度，配置依赖差异较大的机器人受益更多。
...
### [87] [Push Anything: Single- and Multi-Object Pushing From First Sight with Contact-Implicit MPC](https://arxiv.org/abs/2510.19974)
...
<details>
  <summary>Details</summary>
Motivation: 解决非抓取式操作中物体物理属性未知和接触丰富交互复杂性的挑战，扩展CI-MPC方法在多样化物体几何形状和多物体场景中的应用能力。

Method: 开发了Consensus Complementarity Control Plus (C3+)算法，整合了物体扫描、网格重建和硬件执行的完整流程，相比前代C3显著提高了求解速度。

Result: 在硬件实验中，系统在33个物体上达到98%的成功率，平均到达目标时间分别为：单物体0.5分钟，双物体1.6分钟，三物体3.2分钟，四物体5.3分钟。

Conclusion: C3+算法有效解决了多物体推动任务中的接触推理挑战，实现了实时性能和高成功率，展示了CI-MPC在复杂非抓取操作任务中的广泛应用潜力。
...
### [88] [Simultaneous learning of state-to-state minimum-time planning and control](https://arxiv.org/abs/2510.20008)
*Swati Dantu,Robert Pěnička,Martin Saska*

Main category: cs.RO

TL;DR: 提出基于强化学习的框架，学习无人机任意状态间的最小时间飞行策略，结合敏捷飞行和稳定悬停，并通过课程学习实现泛化。


<details>
  <summary>Details</summary>
Motivation: 传统无人机竞速方法受限于预设赛道布局，缺乏在任意状态间飞行的泛化能力，限制了实际应用。

Method: 使用点质量模型轨迹作为代理奖励近似最优飞行目标，采用课程学习高效扩展训练过程并实现泛化。

Result: 仿真实验验证了方法的有效性，优于非线性模型预测控制跟踪点质量模型轨迹；真实环境实验证明策略在户外环境的鲁棒性和泛化能力。

Conclusion: 所提框架成功学习到通用最小时间飞行策略，能够在小型ARM单板计算机上运行，实现任意状态间的敏捷飞行。
...
### [89] [Calibration of Parallel Kinematic Machine Based on Stewart Platform-A Literature Review](https://arxiv.org/abs/2510.20070)
*Sourabh Karmakar,Apurva Patel,Cameron J. Turner*

Main category: cs.RO

TL;DR: 本文综述了基于Stewart平台的并联运动学机器人的逆运动学校准方法，分析了外部仪器、约束和自校准三种主要方法，指出当前研究主要关注结构误差，在无负载条件下进行校准。


<details>
  <summary>Details</summary>
Motivation: Stewart平台并联运动学机器人在精密应用中需要微纳级运动控制，必须通过校准达到比应用要求更高的精度。逆运动学校准比正运动学方法更简便有效。

Method: 通过文献综述分析三种主要校准方法：外部仪器法、约束法和自校准法，重点关注逆运动学为基础的校准技术。

Result: 研究发现研究人员主要关注平台位置和姿态精度的提升，考虑单一或多重误差源，误差源主要是结构误差，部分考虑环境因素，但都在无负载条件下进行校准。

Conclusion: 本研究旨在理解该领域当前技术水平，为其他研究人员在特定领域的进一步探索拓展研究范围。
...
### [90] [Design of a Bed Rotation Mechanism to Facilitate In-Situ Photogrammetric Reconstruction of Printed Parts](https://arxiv.org/abs/2510.20079)
*Travis A. Roberts,Sourabh Karmakar,Cameron J. Turner*

Main category: cs.RO

TL;DR: 设计并制造了一个用于聚合物FDM工艺研究的开源3D打印机平台，重点开发了热床旋转机制，通过最少摄像头实现打印件的摄影测量重建。


<details>
  <summary>Details</summary>
Motivation: 商用和消费级3D打印机多为闭源平台，缺乏研究所需的灵活性和参数控制能力，需要开发一个能够精确控制和监测FDM工艺参数的研究平台。

Method: 设计并制造了具有闭环位置反馈、热端和热床温度控制、环境温湿度监测功能的FDM打印机，特别开发了热床旋转机制，结合摄影测量技术实现打印过程几何记录。

Result: 成功构建了一个能够精确控制FDM工艺参数的研究平台，通过热床旋转机制实现了使用最少摄像头进行摄影测量重建的能力。

Conclusion: 该开源研究平台为聚合物FDM工艺研究提供了可靠的实验环境，能够实现工艺参数与几何缺陷的关联分析，具有重要的研究价值。
...
### [91] [PathFormer: A Transformer with 3D Grid Constraints for Digital Twin Robot-Arm Trajectory Generation](https://arxiv.org/abs/2510.20161)
*Ahmed Alanazi,Duy Ho,Yugyung Lee*

Main category: cs.RO

TL;DR: 提出基于路径的Transformer模型，通过3网格表示和约束掩码解码生成机器人轨迹，在53,755条轨迹上训练，达到89.44%步进准确率和99.99%合法路径，在真实机器人测试中获得86.7%端到端成功率。


<details>
  <summary>Details</summary>
Motivation: 解决序列模型忽略运动结构导致无效或低效执行的问题，需要精确、任务感知的轨迹规划。

Method: 使用3网格（位置/内容/时间）表示编码机器人运动，采用约束掩码解码确保相邻网格移动和工作空间边界，同时推理任务图和动作顺序。

Result: 模型与真实轨迹高度一致：89.44%步进准确率、93.32%精度、89.44%召回率、90.40% F1分数；在xArm Lite 6机器人上达到97.5%可达性、92.5%抓取成功率、86.7%端到端任务成功率。

Conclusion: 路径结构表示使Transformer能够生成准确、可靠、可解释的机器人轨迹，桥接了基于图的规划和基于序列的学习，为通用操作和仿真到真实转移提供了实用基础。
...
### [92] [Reinforcement Learning-based Robust Wall Climbing Locomotion Controller in Ferromagnetic Environment](https://arxiv.org/abs/2510.20174)
*Yong Um,Young-Ha Shin,Joon-Ha Kim,Soonpyo Kwon,Hae-Won Park*

Main category: cs.RO

TL;DR: 提出了一种针对四足磁吸附爬墙机器人的强化学习框架，通过三阶段课程学习处理磁脚吸附的不确定性，在仿真和硬件实验中实现了稳健的垂直爬行。


<details>
  <summary>Details</summary>
Motivation: 解决四足磁吸附爬墙机器人中磁脚吸附的不确定性，包括部分接触、气隙敏感性和概率性附着失败等问题，提高机器人在复杂环境中的鲁棒性。

Method: 采用三阶段课程学习：1）在平坦地面无吸附情况下学习爬行步态；2）逐渐旋转重力矢量至垂直方向并激活吸附模型；3）注入随机吸附失败以鼓励滑移恢复。结合物理基础的吸附模型进行仿真训练。

Result: 学习到的策略在仿真中实现了高成功率、强吸附保持力和快速脱离恢复能力。与假设完美吸附的MPC基线相比，在间歇性失去附着时仍能保持运动。硬件实验证实了在钢表面上稳健的垂直爬行能力。

Conclusion: 将课程学习与真实吸附建模相结合，为磁吸附爬墙机器人在复杂环境中提供了具有弹性的仿真到现实框架。
...
### [93] [A Contact-Driven Framework for Manipulating in the Blind](https://arxiv.org/abs/2510.20177)
*Muhammad Suhail Saleem,Lai Yuan,Maxim Likhachev*

Main category: cs.RO

TL;DR: 提出了一个在视觉受限环境中进行机器人操作的完整框架，通过整合接触反馈和结构先验知识，实现在未知环境中的鲁棒操作。


<details>
  <summary>Details</summary>
Motivation: 机器人在视觉受限环境（如遮挡、光线不足）中操作时，需要依赖接触反馈来感知环境，同时可以利用环境的结构先验知识来提高操作效率。

Method: 框架包含三个紧密耦合的组件：接触检测与定位模块、占据估计模块和规划模块，分别负责检测接触、构建环境地图和规划避碰路径。

Result: 在模拟和真实世界的UR10e机械臂上测试了两个家庭任务，结果显示该框架能可靠完成任务，相比基线方法任务完成时间最多减少2倍。

Conclusion: 该框架成功整合了接触反馈和结构先验知识，为视觉受限环境中的机器人操作提供了有效的解决方案。
...
### [94] [NODA-MMH: Certified Learning-Aided Nonlinear Control for Magnetically-Actuated Swarm Experiment Toward On-Orbit Proof](https://arxiv.org/abs/2510.20231)
*Yuta Takahashi,Atsuki Ochi,Yoichi Tomioka,Shin-Ichiro Sakai*

Main category: cs.RO

TL;DR: 该研究通过实验验证了利用卫星磁力矩器产生的学习辅助磁场相互作用实现大规模卫星群控制的原理，提出了NODA-MMH方法用于基于模型的功率最优群控制。


<details>
  <summary>Details</summary>
Motivation: 解决多卫星长期编队维持问题，特别是当卫星数量超过3个时出现的非完整约束、欠驱动、可扩展性和计算成本等挑战。

Method: 采用学习辅助的时间积分电流控制方法，设计两轴线圈和基于气浮平台的地面实验装置，提出NODA-MMH算法进行模型驱动的功率最优群控制。

Result: 实验验证了学习辅助时间积分电流控制的两个关键方面：增强平均系统动力学的可控性（具有理论保证的误差界）和分散式电流管理。

Conclusion: 该研究为磁驱动卫星群的长期编队维持问题提供了有效的解决方案，补充了相关教程论文的内容。
...
### [95] [Kinaema: a recurrent sequence model for memory and pose in motion](https://arxiv.org/abs/2510.20261)
*Mert Bulent Sariyildiz,Philippe Weinzaepfel,Guillaume Bono,Gianluca Monaci,Christian Wolf*

Main category: cs.RO

TL;DR: Kinaema模型通过隐式潜在记忆处理视觉观察流，实现机器人在大场景中的自我定位和导航，相比传统transformer具有计算效率优势。
...
Method: 使用基于transformer的循环模型维护隐式潜在记忆，压缩传感器历史为紧凑表示，不显式存储观察历史。

Result: 模型能有效维护场景表示，导航到实际任务开始前观察到的目标位置，计算效率优于传统transformer。

Conclusion: Kinaema模型在Mem-Nav任务中表现出色，为空间感知机器人提供了高效的自我定位和导航解决方案。
...
### [96] [MemER: Scaling Up Memory for Robot Control via Experience Retrieval](https://arxiv.org/abs/2510.20328)
*Ajay Sridhar,Jennifer Pan,Satvik Sharma,Chelsea Finn*

Main category: cs.RO

TL;DR: 提出MemER分层策略框架，让机器人策略具备记忆能力，通过高层策略选择相关历史关键帧，指导低层策略执行任务，在需要长时间记忆的机器人操作任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 人类依赖记忆执行任务，但现有机器人策略缺乏此能力。直接使用长观测历史计算成本高且脆弱，而随机采样历史会导致信息冗余或无关。

Method: 分层策略框架：高层策略训练选择并跟踪历史相关关键帧，使用选定关键帧和最新帧生成文本指令指导低层VLA模型执行。基于Qwen2.5-VL-7B-Instruct和π0.5模型微调。

Result: 在三个需要分钟级记忆的真实世界长时程机器人操作任务中，MemER方法优于先前方法。

Conclusion: MemER框架成功赋予机器人策略记忆能力，能高效处理长时程依赖关系，与现有VLA模型兼容。
...
TL;DR: 提出Dino-Diffusion Parking (DDP)方法，结合视觉基础模型和扩散规划，实现跨域鲁棒性自主泊车，在OOD场景下成功率超过90%。


<details>
  <summary>Details</summary>
Motivation: 解决端到端方法在域偏移（如天气和光照变化）下的鲁棒性问题，不依赖额外数据实现跨域泛化。

Method: 集成视觉基础模型与基于扩散的运动规划，在CARLA中训练并在对抗性设置中进行零样本迁移。

Result: 在所有测试的OOD场景中泊车成功率均超过90%，在3D高斯溅射重建的真实停车场中显示出有前景的仿真到真实迁移能力。

Conclusion: DDP通过架构和算法设计显著提升了跨域性能，为自主泊车系统提供了有效的域无关解决方案。
...
### [98] [Multi-Modal Decentralized Reinforcement Learning for Modular Reconfigurable Lunar Robots](https://arxiv.org/abs/2510.20347)
*Ashutosh Mishra,Shreya Santra,Elian Neppel,Edoardo M. Rossi Lombardi,Shamistan Karimov,Kentaro Uno,Kazuya Yoshida*

Main category: cs.RO

TL;DR: 提出了一种用于模块化可重构机器人的分散式强化学习方案，使不同模块学习各自策略，实现零样本泛化到未见配置。


<details>
  <summary>Details</summary>
Motivation: 模块化可重构机器人适合特定空间任务，但形态组合增长阻碍了统一控制。

Method: 分散式强化学习方案：轮子模块使用SAC进行运动控制，7自由度肢体使用PPO进行转向和操作。

Result: 转向策略平均绝对误差3.63度；操作策略成功率84.6%；轮子策略平均扭矩降低95.4%且成功率99.6%。月球模拟现场测试验证了零样本集成。

Conclusion: 系统在不同执行模式间平滑切换，无空闲状态或控制冲突，为模块化月球机器人提供了可扩展、可重用且稳健的方法。
...
### [99] [NeuralTouch: Neural Descriptors for Precise Sim-to-Real Tactile Robot Control](https://arxiv.org/abs/2510.20390)
*Yijiong Lin,Bowen Deng,Chenghua Lu,Max Yang,Efi Psomopoulou,Nathan F. Lepora*

Main category: cs.RO

TL;DR: NeuralTouch是一个结合视觉描述符场和触觉传感的多模态框架，通过轻柔物理交互实现精确、可泛化的抓取


<details>
  <summary>Details</summary>
Motivation: 现有视觉方法（如NDF）生成的抓取位姿不够精确，而纯触觉方法通常局限于简单的预定义接触几何，需要一种能结合两者优势的通用框架

Method: 利用NDF隐式表示目标接触几何，训练基于深度强化学习的策略，使用触觉反馈来优化抓取，该策略以神经描述符为条件，无需显式指定接触类型

Result: 在仿真和真实世界的零样本迁移任务（如孔轴装配和瓶盖开启）中，NeuralTouch显著提高了抓取精度和鲁棒性，优于基线方法

Conclusion: NeuralTouch为精确、接触丰富的机器人操作提供了一个通用框架，成功整合了视觉和触觉感知
...
### [100] [PointMapPolicy: Structured Point Cloud Processing for Multi-Modal Imitation Learning](https://arxiv.org/abs/2510.20406)
...
TL;DR: PointMapPolicy是一种新颖的扩散策略方法，通过在不进行下采样的结构化点网格上操作，结合RGB数据实现增强的多模态感知，在机器人操作任务中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前点云方法难以捕捉细粒度细节，而RGB方法缺乏几何感知能力，限制了机器人在复杂操作任务中的精度和泛化能力。需要结合两种感知模态的优势。

Method: 提出PointMapPolicy方法，在结构化点网格上条件化扩散策略，无需下采样。使用xLSTM作为骨干网络，有效融合点图和RGB数据。
...
### [101] [MR-UBi: Mixed Reality-Based Underwater Robot Arm Teleoperation System with Reaction Torque Indicator via Bilateral Control](https://arxiv.org/abs/2510.20407)
*Kohei Nishi,Masato Kobayashi,Yuki Uranishi*

Main category: cs.RO

TL;DR: 提出了一种基于混合现实的水下机器人手臂遥操作系统MR-UBi，通过双边控制和反应扭矩指示器，将视觉和触觉反馈集成到MR-HMD中，显著提高了抓取扭矩控制的准确性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 为了解决水下机器人手臂遥操作中缺乏有效反馈机制的问题，通过集成视觉和触觉反馈来提升操作的准确性和用户体验。

Method: 使用混合现实头戴显示器（MR-HMD）叠加颜色和长度编码的扭矩条作为反应扭矩指示器（RTI），结合双边控制技术，实现视觉和触觉反馈的集成。

Result: 与双边控制基线相比，MR-UBi显著提高了抓取扭矩控制精度，增加了在最佳扭矩范围内的时间，减少了低和高抓取扭矩范围，在提升和拾放任务中表现更优。主观评估显示更高的可用性（SUS）和更低的工作负荷（NASA-TLX）。
...
### [102] [Robot Path and Trajectory Planning Considering a Spatially Fixed TCP](https://arxiv.org/abs/2510.20473)
*Bernhard Rameder,Hubert Gattringer,Andreas Mueller,Ronald Naderer*

Main category: cs.RO

TL;DR: 提出了一种在工件坐标系中规划轨迹的方法，使用空间固定的工具中心点(TCP)，同时考虑工件上的加工路径。该方法适用于移动工件比移动工具更容易的场景。


<details>
  <summary>Details</summary>
Motivation: 当移动工件比移动工具更容易时，需要一种能够规划工件坐标系中轨迹的方法，同时考虑加工路径和TCP速度要求。

Method: 使用B样条表示机器人路径，无论是数学形状描述还是设计程序中的单点数据。通过样条实现连续路径和光滑轨迹，在计算轨迹时考虑规定的方向和TCP速度。

Result: 该方法在实际工业机器人系统上进行了验证，机器人移动任意定义的工件，证明了方法的可行性。

Conclusion: 基于B样条的轨迹规划方法能够有效处理工件坐标系中的轨迹规划问题，实现连续光滑的机器人运动轨迹，满足加工路径和速度要求。
...
### [103] [Degradation-Aware Cooperative Multi-Modal GNSS-Denied Localization Leveraging LiDAR-Based Robot Detections](https://arxiv.org/abs/2510.20480)
*Václav Pritzl,Xianjia Yu,Tomi Westerlund,Petr Štěpán,Martin Saska*

Main category: cs.RO

TL;DR: 提出了一种自适应多模态多机器人协同定位方法，使用因子图融合异步的视觉-惯性里程计、激光雷达-惯性里程计和3D机器人间检测，在传感器性能下降时仍能保持精确定位。


<details>
  <summary>Details</summary>
Motivation: 解决GNSS拒止环境下机器人长期精确定位问题，通过多机器人分布式传感器配置来降低单个机器人的尺寸、重量和功耗需求，同时应对异步多模态数据融合的挑战。

Method: 采用因子图框架松散耦合融合异步VIO、LIO和3D机器人间检测；提出插值因子处理非同步测量；基于近似扫描匹配Hessian评估LIO退化；通过Wasserstein距离对VIO输出进行加权；包含理论分析。

Result: 在真实世界UGV和UAV异构团队数据上进行广泛评估，显示该方法在各种传感器退化情况下显著提高了定位精度。

Conclusion: 该方法能有效应对传感器退化问题，在多机器人协同定位中提供显著的精度提升，适用于GNSS拒止环境下的长期精确定位任务。
...
### [104] [Dual Control Reference Generation for Optimal Pick-and-Place Execution under Payload Uncertainty](https://arxiv.org/abs/2510.20483)
*Victor Vantilborgh,Hrishikesh Sathyanarayan,Guillaume Crevecoeur,Ian Abraham,Tom Lefebvre*

Main category: cs.RO

TL;DR: 该论文提出两种方法来解决机器人操作任务中的未知动力学问题，通过将参数不确定性嵌入到鲁棒最优控制中，同时实现任务执行和系统辨识。


<details>
  <summary>Details</summary>
Motivation: 解决机器人操作任务中未知动力学（如负载不确定性）的问题，需要主动探索和在线参数适应来实现精确的基于模型的控制。

Method: 将问题构建为双重控制问题，预定义反馈策略结构包含显式适应机制。提出两种参考轨迹生成方法：一种将参数不确定性嵌入鲁棒最优控制，另一种最小化最优性损失。

Result: 在拾取-放置操作任务中验证了方法的有效性，能够实现更快、更准确的任务执行和系统辨识，同时确保稳定高效的控制。

Conclusion: 考虑控制的同时设计参考轨迹能够提升任务性能和系统辨识效果，两种方法都自然地考虑了Fisher信息，同时追求最优任务执行。
...
### [105] [Simultaneous Stiffness and Trajectory Optimization for Energy Minimization of Pick-and-Place Tasks of SEA-Actuated Parallel Kinematic Manipulators](https://arxiv.org/abs/2510.20490)
*Thomas Kordik,Hubert Gattringer,Andreas Mueller*

Main category: cs.RO

TL;DR: 该论文研究了如何通过优化轨迹和串联弹性驱动器(SEA)刚度来最小化执行拾取-放置任务的并联机器人能量消耗。


<details>
  <summary>Details</summary>
Motivation: 工业机器人执行拾取-放置任务时通常长时间运行，最小化能耗具有重要意义。串联弹性驱动器(SEA)和弹性元件为节能提供了可能。

Method: 推导SEA驱动并联机器人的动力学模型，制定能量最小化的最优控制问题，同时优化操作轨迹和SEA刚度。

Result: 在两个并联机器人应用上测试该方法，结果证实了该方法的有效性。

Conclusion: 通过激发由执行器弹簧产生的本征运动并利用其振荡特性，可以显著降低SEA驱动并联机器人在拾取-放置任务中的能量消耗。
...
### [106] [A Parameter-Linear Formulation of the Optimal Path Following Problem for Robotic Manipulator](https://arxiv.org/abs/2510.20496)
*Tobias Marauli,Hubert Gattringer,Andreas Mueller*

Main category: cs.RO

TL;DR: 提出了一种基于最大化路径速度的时间最优路径跟随方法，避免了传统方法在零路径速度时的奇异性问题，能够高效生成平滑轨迹。


<details>
  <summary>Details</summary>
Motivation: 传统时间最优路径跟随方法在最小化旅行时间时，会在零路径速度处产生奇异性，导致计算困难和轨迹不平滑的问题。

Method: 采用最大化路径速度的方法，将问题重新表述为离散优化问题，且优化变量呈线性关系。

Result: 该方法能够高效地规划平滑轨迹，避免了奇异性问题，计算效率高。

Conclusion: 基于最大化路径速度的方法为解决时间最优路径跟随问题提供了一种计算高效且能生成平滑轨迹的有效途径。
...
### [107] [RubbleSim: A Photorealistic Structural Collapse Simulator for Confined Space Mapping](https://arxiv.org/abs/2510.20529)
*Constantine Frost,Chad Council,Margaret McGuinness,Nathaniel Hanson*

Main category: cs.RO

TL;DR: 提出了RubbleSim——一个开源、可重构的模拟器，用于在结构倒塌事故中进行逼真的空隙空间探索，以解决真实灾难响应数据难以获取的问题。


<details>
  <summary>Details</summary>
Motivation: 由于法律约束和机构所有权问题，真实结构倒塌事故中的空隙空间内部组成数据难以公开获取，而训练用的工程废墟堆也不愿公开其专有训练场信息。

Method: 使用Unity开发多操作系统支持的模拟器，采用基于物理的方法构建随机废墟堆，允许在保留绝对真实信息的同时快速迭代模拟世界。

Result: 应用最先进的结构从运动算法，展示了在模拟空隙空间中具有挑战性的视觉条件下感知性能如何下降。

Conclusion: RubbleSim为研究社区提供了一个可访问的解决方案，用于在难以获取真实数据的情况下研究废墟空隙探索问题。
...
### [108] [C-NAV: Towards Self-Evolving Continual Object Navigation in Open World](https://arxiv.org/abs/2510.20685)
*Ming-Ming Yu,Fei Zhu,Wenzhuo Liu,Yirong Yang,Qunbo Wang,Wenjun Wu,Jing Liu*

Main category: cs.RO

TL;DR: 提出了C-Nav持续视觉导航框架，通过双路径抗遗忘机制和自适应采样策略解决持续目标导航中的灾难性遗忘问题，在保持高性能的同时显著降低内存需求。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖静态轨迹和固定对象类别训练，忽视了现实世界中对动态开放环境的持续适应需求，需要解决新类别学习时对已学知识的灾难性遗忘问题。

Method: C-Nav框架包含：1）双路径抗遗忘机制：特征蒸馏确保表示一致性，特征重放确保策略一致性；2）自适应采样策略：选择多样化和信息丰富的经验以减少冗余和内存开销。

Result: 在多种模型架构上的广泛实验表明，C-Nav始终优于现有方法，即使与保留完整轨迹的基线相比也能实现更优性能，同时显著降低内存需求。

Conclusion: C-Nav为持续目标导航提供了一个有效的解决方案，通过创新的抗遗忘机制和采样策略，在动态开放世界中实现了更好的适应性和记忆效率。
...
### [109] [Real-Time Gait Adaptation for Quadrupeds using Model Predictive Control and Reinforcement Learning](https://arxiv.org/abs/2510.20706)
*Ganga Nair B,Prakrut Kotecha,Shishir Kolathaya*

Main category: cs.RO

TL;DR: 提出一个结合MPPI和Dreamer模块的优化框架，用于四足机器人在连续步态空间中的实时步态自适应，显著降低能耗并保持精确跟踪。
...
Method: 在每个时间步使用MPPI联合优化动作和步态变量，结合Dreamer学习奖励函数（促进速度跟踪、能效、稳定性和平滑过渡）和价值函数作为终端奖励。

Result: 在Unitree Go1仿真中，不同目标速度下能耗平均降低36.48%，同时保持精确跟踪和自适应任务适当步态。

Conclusion: 该框架成功结合了MPPI的优化能力和Dreamer的适应能力，实现了四足机器人的高效自适应步态控制。
...
### [110] [FieldGen: From Teleoperated Pre-Manipulation Trajectories to Field-Guided Data Generation](https://arxiv.org/abs/2510.20774)
...
TL;DR: FieldGen是一个场引导的数据生成框架，通过分解操作为预操作和精细操作两个阶段，实现可扩展、多样化和高质量的真实世界数据收集，减少人工监督需求。


<details>
  <summary>Details</summary>
Motivation: 现有机器人操作数据收集方法难以平衡规模、多样性和质量。仿真方法可扩展但存在仿真到现实的差距，遥操作产生高质量演示但多样性有限且人工成本高。

Method: 将操作分解为预操作阶段（允许轨迹多样性）和精细操作阶段（需要专家精度）。人类演示捕获关键接触和姿态信息，然后吸引场自动生成多样轨迹收敛到成功配置。

Result: 实验表明，使用FieldGen训练的策略比基于遥操作的基线方法获得更高的成功率和改进的稳定性，同时显著减少了长期真实世界数据收集的人工努力。

Conclusion: FieldGen通过解耦设计结合了可扩展的轨迹多样性和精确监督，FieldGen-Reward进一步通过奖励注释增强生成数据，有效提升策略学习效果。
...
### [111] [The Reality Gap in Robotics: Challenges, Solutions, and Best Practices](https://arxiv.org/abs/2510.20808)
...
TL;DR: 这篇论文是关于机器人学中仿真到现实迁移的综述，重点分析了现实差距问题及其解决方案。


<details>
  <summary>Details</summary>
Motivation: 机器学习的进步推动了机器人导航、运动和操作等领域的发展，但仿真与现实环境之间的差异（现实差距）阻碍了系统从仿真到现实世界的成功迁移，这是机器人学中最紧迫的挑战之一。

Method: 通过领域随机化、现实到仿真迁移、状态和动作抽象、仿真-现实协同训练等技术来克服现实差距。

Result: 最近的仿真到现实迁移研究在各种平台上取得了有希望的结果，包括运动、导航和操作等领域。

Conclusion: 虽然已有进展，但挑战仍然存在，需要对现实差距的根本原因和解决方案有更深入的理解。本综述提供了仿真到现实领域的全面概览，突出了现实差距和仿真到现实迁移的原因、解决方案和评估指标。
...
### [112] [GSWorld: Closed-Loop Photo-Realistic Simulation Suite for Robotic Manipulation](https://arxiv.org/abs/2510.20813)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够"闭环"开发机器人操作策略的模拟器，实现从真实机器人数据学习策略的可重复评估和sim2real策略训练。

Method: 提出GSDF（高斯场景描述文件）资产格式，将高斯-网格表示与机器人URDF和其他对象融合，结合物理引擎构建照片级真实感模拟器。

Result: 创建了包含3种机器人配置和40多个对象的GSDF数据库，展示了五个应用场景：零样本sim2real像素到动作策略学习、自动化DAgger数据收集、可重复基准测试、虚拟遥操作数据收集和零样本sim2real视觉强化学习。

Conclusion: GSWorld为机器人操作研究提供了一个强大的照片级真实感模拟平台，支持多种sim2real应用，显著降低了机器人实验的成本和复杂性。
...
### [113] [VAMOS: A Hierarchical Vision-Language-Action Model for Capability-Modulated and Steerable Navigation](https://arxiv.org/abs/2510.20818)
...
TL;DR: VAMOS是一个分层视觉语言导航系统，通过将语义规划与具体化接地分离，使用通用规划器处理多样化环境，专用可及性模型学习机器人物理约束，实现跨具体化导航。


<details>
  <summary>Details</summary>
Motivation: 解决机器人导航中学习策略在不同环境中的泛化问题，同时适应特定机器人的物理约束和能力差异（如四足机器人能上楼梯而轮式机器人不能）。

Method: 设计分层架构：高层规划器从多样化开放世界数据中学习，提出图像空间候选路径；专用可及性模型在低成本模拟中学习机器人物理约束，评估和重新排序路径。

Result: 真实世界实验显示VAMOS在室内和复杂室外导航中比最先进的基于模型和端到端学习方法获得更高成功率，支持轮式和腿式机器人跨具体化导航，通过拒绝物理不可行计划使单机器人可靠性提高3倍。

Conclusion: 分层设计使单一高层规划器能够部署在物理特性不同的机器人上，专用模型是实现具体化接地的关键，显著提高了导航可靠性和成功率。
...
### [114] [A Quantum-Inspired Algorithm for Solving Sudoku Puzzles and the MaxCut Problem](https://arxiv.org/abs/2510.19835)
*Max B. Zhao,Fei Li*

Main category: cs.AI

TL;DR: 提出一种量子启发算法，使用矩阵乘积态和离散驱动调度解决QUBO问题，在Sudoku和MaxCut问题上表现出色，能可靠找到全局最优解而非近似解。


<details>
  <summary>Details</summary>
Motivation: 解决二次无约束二进制优化问题，该问题在数学上等价于寻找伊辛自旋玻璃哈密顿量的基态，传统方法难以找到全局最优解。

Method: 使用矩阵乘积态紧凑表示自旋构型的大叠加，结合离散驱动调度引导MPS向基态演化，通过DMRG方法迭代最小化系统能量。

Result: 在200+自旋的Sudoku问题和251节点3265边的MaxCut问题上成功找到全局最小值，算法具有可靠性和扩展性。

Conclusion: 该量子启发方法具有可扩展性、通用性和适用于工业规模QUBO应用的潜力。
...
### [115] [Benchmarking Reasoning Reliability in Artificial Intelligence Models for Energy-System Analysis](https://arxiv.org/abs/2510.19836)
*Eliseo Curcio*

Main category: cs.AI

TL;DR: 该研究提出了分析可靠性基准(ARB)，这是首个用于量化能源系统分析中大型语言模型推理可靠性的标准化框架，包含五个子指标，在四种前沿模型上进行了测试。


<details>
  <summary>Details</summary>
Motivation: 当前AI在能源领域的验证实践主要关注预测准确性或计算效率，而缺乏对分析结论逻辑完整性的测试，需要建立标准化评估框架。

Method: 开发了ARB框架，整合准确性、推理可靠性、不确定性纪律、政策一致性和透明度五个子指标，使用公开技术经济数据集在确定性、概率性和认知性场景下评估模型性能。

Result: GPT-4/5和Claude 4.5 Sonnet实现了稳定且符合政策的推理(分析可靠性指数大于90)，Gemini 2.5 Pro表现中等，Llama 3 70B低于专业阈值，统计验证显示这些差异显著且可重现。
...
### [116] [Branch-and-Browse: Efficient and Controllable Web Exploration with Tree-Structured Reasoning and Action Memory](https://arxiv.org/abs/2510.19838)
...
Method: 该框架结合了结构化推理-行动、上下文记忆和高效执行，采用显式子任务管理和树结构探索进行可控多分支推理，通过网页状态重放引导探索，并利用页面动作记忆在会话内外共享探索过的动作。
...
### [117] [DAG-Math: Graph-Guided Mathematical Reasoning in LLMs](https://arxiv.org/abs/2510.19842)
*Yuanhe Zhang,Ilja Kuzborskij,Jason D. Lee,Chenlei Leng,Fanghui Liu*

Main category: cs.AI

TL;DR: 提出DAG-MATH框架，将思维链建模为基于规则的有向无环图过程，引入逻辑紧密度指标评估LLM推理一致性，发现即使准确率相同，不同模型在规则一致性推理能力上存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 当前不清楚LLM在数学问题上的成功是源于搜索、机械记忆还是规则一致性推理，需要超越传统PASS@k指标来评估推理能力。

Method: 将思维链建模为基于规则的有向无环图随机过程，定义逻辑紧密度指标，构建DAG-MATH格式的基准测试来评估LLM推理能力。

Result: 在标准数学推理数据集上，发现代表性LLM家族在推理保真度上存在统计显著差异，即使PASS@k指标相当，揭示了最终答案准确性与规则一致性推导之间的差距。

Conclusion: 该框架在自由形式思维链和形式证明系统之间取得平衡，为LLM推理评估提供了可操作的诊断工具。
...
### [118] [Surfer 2: The Next Generation of Cross-Platform Computer Use Agents](https://arxiv.org/abs/2510.19949)
...
TL;DR: Surfer 2是一个基于纯视觉观察的统一架构，在Web、桌面和移动环境中实现最先进的性能，无需任务特定微调即可超越所有现有系统。
...
Method: 集成层次化上下文管理、解耦的规划与执行、以及具有自适应恢复能力的自我验证，支持长任务周期的可靠操作。

Result: 在WebVoyager上达到97.1%准确率，WebArena 69.6%，OSWorld 60.1%，AndroidWorld 87.1%，多尝试情况下在所有基准测试中超越人类表现。

Conclusion: 系统化编排能够放大基础模型能力，实现仅通过视觉交互的通用计算机控制，同时需要下一代视觉语言模型来实现帕累托最优的成本效率。
...
### [119] [RELATE: A Schema-Agnostic Perceiver Encoder for Multimodal Relational Graphs](https://arxiv.org/abs/2510.19954)
*Joseph Meyer,Divyansha Lachi,Reza Mohammadi,Roshan Reddy Upendra,Eva L. Dyer,Mark Li,Tom Palczewski*

Main category: cs.AI

TL;DR: RELATE是一个模式无关的图神经网络特征编码器，使用共享的模态特定编码器和交叉注意力模块，在减少5倍参数的同时达到接近模式特定编码器97%的性能。


<details>
  <summary>Details</summary>
Motivation: 现有GNN需要为每种节点类型和特征列设计单独的特征编码模块，这阻碍了可扩展性和参数共享，无法支持多数据集预训练。

Method: 使用共享的模态特定编码器处理分类、数值、文本和时间属性，然后通过Perceiver风格的交叉注意力模块将特征聚合成固定大小的节点表示。

Result: 在RelBench基准测试中，RELATE在ReLGNN和HGT上达到模式特定编码器97%的性能，同时减少参数达5倍。
...
### [120] [A new wave of vehicle insurance fraud fueled by generative AI](https://arxiv.org/abs/2510.19957)
*Amir Hever,Itai Orr*
...
<details>
  <summary>Details</summary>
Motivation: 保险欺诈每年造成数百亿美元损失，传统欺诈手段包括伪造事故和夸大损失。生成式AI的出现让欺诈者能轻松制造逼真的碰撞照片、损坏证据和假身份，加剧了保险欺诈问题。

Method: 保险公司部署基于AI的深度伪造检测软件和增强验证流程来检测AI驱动的欺诈。本文提出UVeye的分层解决方案，代表在检测、缓解和威慑新型欺诈能力上的重大进步。

Result: 当前缓解策略面临显著限制：检测工具存在误报和漏报问题，复杂欺诈者不断调整策略规避自动检查。生成式AI与检测技术之间的军备竞赛，加上保险公司的资源和成本障碍，使对抗AI驱动的保险欺诈仍是持续挑战。

Conclusion: UVeye的分层解决方案在检测、缓解和威慑车辆保险欺诈方面取得了重大进展，为应对生成式AI加剧的保险欺诈问题提供了有效方法。
...
### [121] [AI-Driven Personalized Learning: Predicting Academic Per-formance Through Leadership Personality Traits](https://arxiv.org/abs/2510.19964)
*Nitsa J Herzog,Rejwan Bin Sulaiman,David J Herzog,Rose Fong*

Main category: cs.AI

TL;DR: 该研究使用机器学习模型通过领导力人格特质预测学术成功，随机森林分类器在包含17个人格特征和领导力标记的模型上达到87.50%的准确率。
...
Method: 收集129名环境工程硕士的五项领导力人格测试数据，使用探索性数据分析和相关性分析，通过特征选择后调优七种机器学习算法进行建模。

Result: 随机森林分类器表现最佳，包含17个人格特征和领导力标记的模型准确率达87.50%，不包含该特征的模型准确率为85.71%。

Conclusion: 该研究为早期识别学生优缺点和选择最适合的个性化学习策略提供了有效方法，证明了领导力人格特质在预测学术成功方面的价值。
...
### [122] [LLMs can hide text in other text of the same length.ipynb](https://arxiv.org/abs/2510.20075)
...
<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型如何能够将完全不同的信息隐藏在看似连贯合理的文本中，揭示文本与作者意图之间的根本性分离，这对书面通信的信任构成进一步侵蚀。

Method: 使用简单的协议，即使是80亿参数的开源LLM也能在笔记本电脑上快速编码和解码信息，将秘密文本嵌入到表面内容完全不同的文本中。

Result: 实验表明该方法高效可行，能够在几秒钟内编码和解码如摘要长度的信息，且隐藏后的文本看起来完全正常和连贯。

Conclusion: 这种技术展示了文本与作者意图的彻底分离，对AI安全构成严重挑战，并引发了对LLM知识本质的根本性质疑。
...
### [123] [AI PB: A Grounded Generative Agent for Personalized Investment Insights](https://arxiv.org/abs/2510.20099)
*Daewoo Park,Suho Park,Inseok Hong,Hanwool Lee,Junkyu Park,Sangjun Lee,Jeongman An,Hyunbin Loh*

Main category: cs.AI

TL;DR: AI PB是一个部署在真实零售金融环境中的生产级生成式代理系统，能够主动生成基于事实、合规且个性化的投资洞察，而非被动回答问题。


<details>
  <summary>Details</summary>
Motivation: 在金融领域，需要开发能够主动提供可信赖AI洞察的系统，而非仅仅被动回答问题的聊天机器人。系统必须在韩国金融监管框架下完全在本地运行，确保数据安全和合规性。

Method: 系统采用组件化编排层，根据数据敏感性在内部和外部LLM之间进行确定性路由；使用混合检索管道（OpenSearch和金融领域嵌入模型）；以及多阶段推荐机制（规则启发式、序列行为建模和上下文多臂老虎机）。

Result: 系统在24个NVIDIA H100 GPU上使用Docker Swarm和vLLM部署运行。通过人工QA和系统指标评估，证明了基于事实的生成、显式路由和分层安全机制能够在高风险金融环境中提供可信赖的AI洞察。

Conclusion: 研究表明，通过明确的路径选择和分层安全机制进行基于事实的生成，可以在高风险金融环境中实现可信赖的AI洞察交付。
...
### [124] [Human-Centered LLM-Agent System for Detecting Anomalous Digital Asset Transactions](https://arxiv.org/abs/2510.20102)
...
<details>
  <summary>Details</summary>
Motivation: 提高数字资产交易异常检测的透明度和可信度，让非专家用户能够通过自然语言交互理解和验证检测结果。

Method: 构建包含解析、检测和解释三个角色的多智能体系统，使用XGBoost作为基础检测器，通过对话工作流将用户意图转换为检测模式。

Result: 在比特币混币数据集上，基线检测器达到高准确率，HCLA系统在此基础上增加了可解释性和交互式优化能力。

Conclusion: 人机协作设计能够显著提升金融取证领域的透明度和信任度，HCLA系统为非专家用户提供了直观的异常检测工具。
...
### [125] [The Verification-Value Paradox: A Normative Critique of Gen AI in Legal Practice](https://arxiv.org/abs/2510.20109)
*Joshua Yuvaraj*

Main category: cs.AI

TL;DR: 论文认为需要重新评估AI在法律实践中的使用，提出了验证-价值悖论，指出AI带来的效率提升会被相应的验证需求抵消，使得AI的净价值对律师来说往往微不足道。


<details>
  <summary>Details</summary>
Motivation: 由于律师因提交不准确的AI生成内容而受到谴责的案例，以及AI与现实脱节、缺乏透明度与律师诚实、正直、不误导法庭的首要职责之间的冲突，需要重新评估AI在法律实践中的使用范式。

Method: 提出了一种替代性的AI使用模型，即验证-价值悖论，该模型更全面地反映了AI的特性和律师的职责要求。

Result: 验证-价值悖论表明，法律实践中使用AI带来的效率提升会被相应增加的输出验证需求所抵消，导致AI使用的净价值往往对律师来说微不足道。

Conclusion: 论文阐述了该悖论对法律实践和法律教育的影响，强调应基于对真理的忠诚和公民责任来指导法律实践，包括AI的使用。
...
### [126] [TRUST: A Decentralized Framework for Auditing Large Language Model Reasoning](https://arxiv.org/abs/2510.20188)
*Morris Yu-Chao Huang,Zhen Tan,Mohan Zhang,Pingzhi Li,Zhuo Zhang,Tianlong Chen*

Main category: cs.AI

TL;DR: 提出了TRUST框架，一个去中心化的大语言模型推理审计系统，通过共识机制、分层DAG分解、区块链记录和隐私保护分段解决现有集中式审计的四大挑战。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型推理链审计方法存在集中化、不透明、难以扩展的问题，在关键领域部署专有模型时存在重大风险。

Method: TRUST框架包含：1) 多样化审计者共识机制；2) 推理链的分层DAG分解；3) 区块链账本记录验证决策；4) 隐私保护的分段共享。
...
TL;DR: 该论文提出AGI发展需要经历从开放模仿到身份固化的锁定阶段，在此阶段模型的目标结构、拒绝行为、偏好和内部表征变得稳定且抵抗外部操控。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型过于开放和易操控，而真正的AGI需要稳定的身份和内在一致性，因此研究身份固化的动态过程和检测方法具有重要意义。

Method: 通过形式化身份固化阶段，将其与学习动态中的已知现象关联，并提出操作性指标进行检测，在不同规模模型上进行实验验证。

Result: 实验发现身份固化的行为表现是非线性的快速过程，但对通用能力的影响存在差异：小模型出现性能权衡，中等规模模型几乎无成本，大型量化模型出现瞬时不稳定性。

Conclusion: 身份固化是AGI可靠性的前提条件，也是安全控制的关键点，可以工程化设计可靠身份，但也可能在扩展过程中自发形成，导致不可预测的目标和行为固化。
...
### [128] [Merge and Conquer: Evolutionarily Optimizing AI for 2048](https://arxiv.org/abs/2510.20205)
...
Method: 实现两种系统：双智能体元提示系统（思考者LLM为执行者LLM优化策略）和基于改进价值函数的单智能体系统（有限蒙特卡洛树搜索），并实验回滚功能以避免性能退化。

Result: 单智能体系统取得显著改进，每个周期平均增加473.2分，训练周期呈现明显上升趋势（相关系数ρ=0.607）。LLM对游戏的理解增强，开发出更先进的策略。双智能体系统改进有限，突显了元提示的内在局限性。

Conclusion: 进化精炼技术在非确定性环境中改善AI性能具有潜力，单智能体方法比元提示方法更有效。
...
### [129] [Individualized Cognitive Simulation in Large Language Models: Evaluating Different Cognitive Representation Methods](https://arxiv.org/abs/2510.20252)
...
<details>
  <summary>Details</summary>
Motivation: 虽然LLM能模仿表面的人类行为，但其模拟更深层次个体认知过程的能力尚不清楚。研究旨在填补这一空白，评估不同认知表征方法在ICS中的效果。

Method: 构建基于新出版小说的数据集，提出11条件认知评估框架，测试7个现成LLM在作者风格模仿任务中的表现。比较了语言特征、概念映射和基于档案信息等不同认知表征方法。

Result: 结合概念和语言特征的方法在ICS中特别有效，在整体评估中优于基于静态档案的线索。LLM在模仿语言风格方面比叙事结构更有效，显示了其在更深层次认知模拟中的局限性。
...
### [130] [Using Large Language Models for Abstraction of Planning Domains - Extended Version](https://arxiv.org/abs/2510.20258)
...
<details>
  <summary>Details</summary>
Motivation: 动态领域抽象的选择会影响智能体的规划、推理和解释能力，因此需要一种方法能够根据特定目的生成合适的抽象表示。

Method: 在PDDL中建模智能体具体行为，利用大语言模型（如GPT-4o）通过上下文学习生成抽象PDDL领域和问题实例，考虑三种抽象类别：替代具体动作选择、具体动作序列和动作/谓词参数。

Result: GPT-4o在简单设置中能有效合成有用的规划领域抽象，但在动作抽象方面比相关流变体抽象表现更好。生成的抽象PDDL通过符号验证工具和专家评估。

Conclusion: 大语言模型能够生成有用的规划领域抽象，特别是在动作抽象方面表现良好，为基于目的的领域抽象生成提供了可行方法。
...
### [131] [Classical Feature Embeddings Help in BERT-Based Human Mobility Prediction](https://arxiv.org/abs/2510.20275)
...
Method: 提出STaBERT模型，在BERT-based移动性模型中引入推导的时间描述符和POI嵌入，在每个位置整合POI和时间信息，构建统一的语义增强移动性表示。

Result: 实验结果显示STaBERT显著提升预测精度：单城市预测的GEO-BLEU得分从0.34提升到0.75；多城市预测从0.34提升到0.56。

Conclusion: STaBERT通过整合语义和时间信息，有效提升了人类移动性预测的性能，证明了语义丰富表示的重要性。
...
### [132] [Multi-Step Reasoning for Embodied Question Answering via Tool Augmentation](https://arxiv.org/abs/2510.20310)
*Mingliang Zhai,Hansheng Liang,Xiaomeng Fan,Zhi Gao,Chuanhao Li,Che Sun,Xu Bin,Yuwei Wu,Yunde Jia*

Main category: cs.AI

TL;DR: ToolEQA是一个集成了外部工具和多步推理的EQA智能体，通过工具使用提供有用信息来改进探索方向，从而以更短的探索距离生成更准确的回答。


<details>
  <summary>Details</summary>
Motivation: 现有的EQA方法直接使用视觉语言模型探索环境而不进行显式思考或规划，限制了推理能力，导致探索效率低下和回答无效。

Method: 提出ToolEQA智能体，集成外部工具进行多步推理；设计自动生成EQA任务的数据生成流程，构建包含约18K任务的EQA-RT数据集。

Result: 在EQA-RT-Seen和EQA-RT-Unseen测试集上，ToolEQA比最先进基线成功率提高9.2~20.2%，比零样本ToolEQA提高10%；在HM-EQA、OpenEQA和EXPRESS-Bench数据集上达到最先进性能。
...
### [133] [Bias by Design? How Data Practices Shape Fairness in AI Healthcare Systems](https://arxiv.org/abs/2510.20332)
*Anna Arias-Duart,Maria Eugenia Cardello,Atia Cortés*

Main category: cs.AI

TL;DR: 本文基于AI4HealthyAging项目，识别了临床数据收集中存在的多种偏见类型，并提出了改善医疗AI系统公平性和鲁棒性的实用建议。


<details>
  <summary>Details</summary>
Motivation: 尽管AI在医疗领域有巨大潜力，但由于训练数据的质量和公平性问题，AI解决方案在真实临床实践中的整合仍然有限。主要障碍是数据收集过程中的偏见。

Method: 基于西班牙国家研发计划中的AI4HealthyAging项目，识别临床数据收集中存在的偏见类型，包括历史偏见、代表性偏见和测量偏见等。

Result: 在多个用例中发现了多种偏见，这些偏见体现在性别、年龄、居住地、社会经济地位、设备和标签等变量中。

Conclusion: 提出了改善临床问题设计和数据收集公平性和鲁棒性的实用建议，希望能指导未来项目开发更公平的医疗AI系统。
...
### [134] [Collateral Damage Assessment Model for AI System Target Engagement in Military Operations](https://arxiv.org/abs/2510.20337)
...
<details>
  <summary>Details</summary>
Motivation: 在AI系统在战场上发挥越来越重要作用时代，确保负责任的目标打击需要对潜在附带效应进行严格评估。

Method: 采用设计科学方法论，构建分层知识表示与推理架构，整合时间、空间和力量维度，考虑传播、严重性、可能性和评估指标。

Result: 通过实例化演示和评估了该模型，为构建负责任和可信赖的智能系统提供了基础。

Conclusion: 该模型为评估军事行动中打击AI系统产生的效果提供了透明推理机制，支持构建负责任和可信赖的智能系统。
...
### [135] [LLM-empowered knowledge graph construction: A survey](https://arxiv.org/abs/2510.20345)
*Haonan Bian*

Main category: cs.AI

TL;DR: 本调查系统回顾了LLM赋能知识图谱构建的最新进展，分析了LLM如何重塑传统的本体工程、知识抽取和知识融合三层流程，探讨了基于模式和无模式两种范式，并展望了未来研究方向。
...
Method: 首先回顾传统KG方法，然后从基于模式（强调结构、规范化和一致性）和无模式（强调灵活性、适应性和开放发现）两个互补视角分析新兴的LLM驱动方法，分析代表性框架的技术机制和局限性。

Result: 系统梳理了LLM赋能KG构建的技术路线，明确了两种范式各自的优势和适用场景，为理解LLM与KG的交互演进提供了清晰框架。

Conclusion: 该调查旨在阐明LLM与知识图谱之间不断发展的相互作用，弥合符号知识工程与神经语义理解之间的鸿沟，推动自适应、可解释和智能知识系统的发展。
...
### [136] [IKnow: Instruction-Knowledge-Aware Continual Pretraining for Effective Domain Adaptation](https://arxiv.org/abs/2510.20377)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需要访问原始基础模型或依赖外部领域特定数据库，这在基础模型权重因安全原因被保留或可靠外部语料不可用时构成现实障碍。

Method: IKnow框架在指令-响应对话格式中制定新的自监督目标，利用文本中嵌入的领域知识，在更深语义层面进行编码学习。

Result: 该方法能够在不依赖外部资源的情况下，适应大语言模型到新领域，同时保持指令跟随能力和语义表示。

Conclusion: IKnow提供了一个简单通用的框架，通过自监督学习有效解决指令调优模型在持续预训练中的挑战。
...
### [137] [A computational model and tool for generating more novel opportunities in professional innovation processes](https://arxiv.org/abs/2510.20402)
...
Method: 开发了一个包含五个功能模块的计算模型，这些功能基于创造力理论和技巧设计，旨在提升创新机会的新颖性而不损失实用性。

Result: 在酒店业创新项目评估中，该模型生成的机会比Notebook LM和ChatGPT4o更具新颖性和/或实用性，但并非所有功能模块都对提升新颖性有贡献。

Conclusion: 该计算模型在生成创新机会方面表现优于现有AI系统，但部分功能需要进一步优化，为模型开发提供了新的方向。
...
### [138] [Neural Reasoning for Robust Instance Retrieval in $\mathcal{SHOIQ}$](https://arxiv.org/abs/2510.20457)
...
Method: EBR使用嵌入来近似符号推理器的结果，仅需要检索原子概念和存在限制的实例，就能近似SHOIQ描述逻辑中任何概念的实例集。

Result: 实验表明，与最先进的推理器相比，EBR对缺失和错误数据具有鲁棒性。

Conclusion: EBR为在现实世界知识库中部署神经符号概念学习提供了可行的解决方案，通过神经推理方法克服了传统符号推理器的局限性。
...
### [139] [FLORA: Unsupervised Knowledge Graph Alignment by Fuzzy Logic](https://arxiv.org/abs/2510.20467)
*Yiwen Peng,Thomas Bonald,Fabian M. Suchanek*

Main category: cs.AI

TL;DR: FLORA是一种无需训练数据、基于模糊逻辑的知识图谱对齐方法，能够同时对齐实体和关系，提供可解释的结果，并在主要基准测试中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱对齐方法主要关注实体级对齐，缺乏可解释性推理且需要训练数据。FLORA旨在解决这些问题，提供无需训练数据的整体对齐方案。

Method: 基于模糊逻辑的迭代方法，同时对齐实体和关系，允许存在悬空实体（没有对应实体的实体），并保证收敛性。

Result: 在主要基准测试中取得了最先进的结果，证明了该方法的有效性。

Conclusion: FLORA是一种简单而有效的知识图谱对齐方法，具有无监督、可解释、收敛性好等优势，能够处理现实世界中的知识图谱对齐问题。
...
### [140] [Lost in Translation: Policymakers are not really listening to Citizen Concerns about AI](https://arxiv.org/abs/2510.20568)
*Susan Ariel Aaronson,Michael Moreno*

Main category: cs.AI

TL;DR: 该研究比较了澳大利亚、哥伦比亚和美国三个国家的AI治理公众参与实践，发现政府未能建立有效的对话机制，参与率极低且官员对反馈响应有限，导致参与式AI治理的承诺与现实存在差距。


<details>
  <summary>Details</summary>
Motivation: 研究旨在分析政府如何征集公众对AI风险和政策的意见，以及这些意见是否真正影响了AI治理，揭示当前参与式AI治理的不足。

Method: 采用景观分析方法，比较三个国家（澳大利亚、哥伦比亚、美国）政府征集AI政策反馈的方式及其对治理的影响。

Result: 研究发现三国均未能建立有意义的对话机制，参与率不足人口1%，政府缺乏多样化声音征集和反馈响应，形成无效的反馈循环。

Conclusion: 当前AI治理参与方法无法建立信任和合法性，作者提出八项改进建议，包括提升AI素养、扩大参与范围、采用创新参与方法等。
...
### [141] [Transferable Graph Learning for Transmission Congestion Management via Busbar Splitting](https://arxiv.org/abs/2510.20591)
*Ali Rajaei,Peter Palensky,Jochen L. Cremer*

Main category: cs.AI

TL;DR: 提出了一种基于图神经网络加速的网络拓扑优化方法，用于缓解输电拥堵和降低重调度成本，在保证AC可行解的同时实现4个数量级的加速。


<details>
  <summary>Details</summary>
Motivation: 现有求解器无法在近实时内解决大规模系统的混合整数非线性网络拓扑优化问题，而传统机器学习方法在泛化到未见拓扑、不同运行条件和系统时存在局限。

Method: 开发了异构边感知消息传递神经网络来预测有效的母线分裂动作作为候选拓扑优化解，该方法考虑线性化AC潮流模型，能够捕捉局部流量模式。

Result: 在GOC 2000总线系统上实现了4个数量级的加速，在1分钟内提供AC可行解，最优性差距仅为2.3%。
...
### [142] [What Defines Good Reasoning in LLMs? Dissecting Reasoning Steps with Multi-Aspect Evaluation](https://arxiv.org/abs/2510.20603)
...
<details>
  <summary>Details</summary>
Motivation: 传统仅评估最终答案正确性的方法过于粗糙，无法提供有效的模型改进信号，忽视了底层推理过程的质量。

Method: 引入因果逐步评估(CaSE)方法，将推理质量分解为相关性和连贯性两个维度，仅使用前序上下文评估每个推理步骤以避免后见之明偏差。

Result: 在MRa-GSM8K和MRa-MATH基准上验证了CaSE与人类判断的一致性，并通过CaSE评估的数据筛选直接提升了最终任务性能。
...
### [143] [Efficient Algorithms for Computing Random Walk Centrality](https://arxiv.org/abs/2510.20604)
*Changan Liu,Zixuan Xie,Ahad N. Zehmakan,Zhongzhi Zhang*

Main category: cs.AI

TL;DR: 提出了两种可扩展的算法来计算随机游走中心性，一种基于近似Cholesky分解和稀疏逆估计，另一种基于采样根生成树，均能在近线性时间内运行并提供强近似保证。


<details>
  <summary>Details</summary>
Motivation: 随机游走中心性是图挖掘中量化节点重要性和影响力的基本指标，但由于现有方法的计算需求，在大规模网络上计算该度量仍然不切实际。

Method: 提出了随机游走中心性的新公式化，并基于此开发了两种算法：一种是利用近似Cholesky分解和稀疏逆估计，另一种是采样根生成树。

Result: 在大型真实世界网络（包括超过1000万个节点的网络）上的广泛实验证明了所提出算法的效率和近似质量。

Conclusion: 所提出的算法能够高效地计算随机游走中心性，为大规模图分析提供了实用的解决方案。
...
### [144] [Towards the Formalization of a Trustworthy AI for Mining Interpretable Models explOiting Sophisticated Algorithms](https://arxiv.org/abs/2510.20621)
*Riccardo Guidotti,Martina Cinquini,Marta Marchiori Manerba,Mattia Setzu,Francesco Spinnato*

Main category: cs.AI

TL;DR: 提出了MIMOSA框架，这是一个可解释性优先的监督学习方法论，在平衡可解释性与性能的同时嵌入因果性、公平性和隐私性等关键伦理属性。


<details>
  <summary>Details</summary>
Motivation: 现实应用中需要可解释性优先的模型来建立信任、问责制和自动化决策模型的安全采用。

Method: 定义了监督学习设置，涵盖表格数据、时间序列、图像、文本、交易和轨迹等多种数据类型。分析了特征重要性、规则和实例三类可解释模型家族，并形式化了因果性、公平性和隐私性三个伦理属性。

Result: 建立了开发不仅准确可解释，而且公平、保护隐私和具备因果意识的AI系统的理论基础。

Conclusion: MIMOSA框架为构建可信赖的AI系统提供了理论基础，通过模型生成过程中评估伦理度量，确保系统具备可解释性、公平性、隐私保护和因果意识。
...
### [145] [Towards Reliable Evaluation of Large Language Models for Multilingual and Multimodal E-Commerce Applications](https://arxiv.org/abs/2510.20632)
*Shuyi Xie,Ziqin Liew,Hailing Zhang,Haibo Zhang,Ling Hu,Zhiqiang Zhou,Shuman Liu,Anxiang Zeng*

Main category: cs.AI

TL;DR: EcomEval是一个全面的多语言多模态基准测试，用于评估LLM在电子商务领域的性能，解决了现有评估工具在任务多样性、模态覆盖和语言多样性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有电子商务评估基准存在任务多样性有限、缺乏多模态数据、使用合成数据以及语言覆盖狭窄等问题，无法有效评估LLM在复杂真实购物场景中的表现。

Method: 构建包含6个类别37个任务的多语言多模态基准，主要来源于真实客户查询和交易日志，采用半自动流程生成参考答案，由50多名专家审核修改，并为每个问题定义难度级别。

Result: EcomEval覆盖7种语言（包括5种东南亚低资源语言），包含8个多模态任务，反映了真实商业交互的噪声和异质性特征。

Conclusion: EcomEval为电子商务领域的LLM评估提供了更全面、真实和细粒度的基准测试工具，填补了现有评估方法的空白。
...
### [146] [Fluidity Index: Next-Generation Super-intelligence Benchmarks](https://arxiv.org/abs/2510.20636)
...
Method: 通过测量模型对扩展环境中状态变化的理解、预测和调整能力来评估适应性，使用流动性指数作为量化指标。

Result: 建立了评估模型适应性的框架，能够区分不同级别的适应性表现。
...
### [147] [Integrating Machine Learning into Belief-Desire-Intention Agents: Current Advances and Open Challenges](https://arxiv.org/abs/2510.20641)
*Andrea Agiollo,Andrea Omicini*

Main category: cs.AI

TL;DR: 本文对将机器学习集成到理性智能体架构中的现有方法进行了系统化梳理，特别以BDI（信念-欲望-意图）范式为参考框架，识别了该领域的关键研究机会和开放挑战。


<details>
  <summary>Details</summary>
Motivation: 由于机器学习在感知和认知任务中展现出类人能力，将ML集成到理性智能体架构的框架日益受到关注。但目前该领域研究分散且不连贯，往往只关注将ML嵌入通用智能体容器，而忽视了理性架构（如BDI智能体）的表达能力。

Method: 采用BDI范式作为参考框架，对现有方法进行细粒度系统化分析，梳理了ML增强理性智能体的快速演进文献。

Result: 分析揭示了该领域的研究现状和发展趋势，展示了ML增强理性智能体的文献快速演进过程。
...
### [148] [The Shape of Reasoning: Topological Analysis of Reasoning Traces in Large Language Models](https://arxiv.org/abs/2510.20665)
*Xue Wen Tan,Nathaniel Tan,Galen Lee,Stanley Kok*

Main category: cs.AI

TL;DR: 提出了基于拓扑数据分析的框架，用于自动化评估大语言模型推理轨迹的质量，相比传统图指标具有更高预测能力
...
Result: 拓扑特征在预测推理质量方面比标准图指标具有显著更高的预测能力，表明有效推理由高维几何结构而非纯关系图更好捕捉
...
### [149] [Plan Then Retrieve: Reinforcement Learning-Guided Complex Reasoning over Knowledge Graphs](https://arxiv.org/abs/2510.20691)
*Yanlin Song,Ben Liu,Víctor Gutiérrez-Basulto,Zhiwei Hu,Qianqian Xie,Min Peng,Sophia Ananiadou,Jeff Z. Pan*

Main category: cs.AI

TL;DR: Graph-RFT是一个两阶段强化微调KGQA框架，通过'计划-KG搜索-网络搜索-思考'范式，让LLM在不完整知识条件下自主规划并自适应检索KG和网络资源。
...
Method: 提出两阶段方法：1) 使用定制化计划-检索数据集的思维链微调激活结构化推理；2) 基于计划-检索的强化学习过程，集成显式规划和检索动作，采用笛卡尔式规划模块分解复杂问题，逻辑表达式指导工具调用。

Result: 该框架通过多奖励设计优化推理检索过程，结合结果和检索特定信号，使模型学习何时以及如何有效结合KG和网络检索。

Conclusion: Graph-RFT能够在不完整知识条件下实现自主规划和自适应检索调度，解决复杂场景下的KGQA问题。
...
### [150] [A Coherence-Based Measure of AGI](https://arxiv.org/abs/2510.20784)
*Fares Fourati*

Main category: cs.AI

TL;DR: 本文提出了一个基于广义均值积分的一致性感知AGI度量方法，替代了传统算术平均的补偿性假设，强调跨认知领域的平衡能力。


<details>
  <summary>Details</summary>
Motivation: 现有AGI定义使用算术平均，假设不同认知领域间存在补偿性（即某些领域的卓越能力可以弥补其他领域的失败），但真正的通用智能应该反映所有关键领域的平衡能力。

Method: 提出基于广义均值连续体积分的AGI度量方法，涵盖算术、几何和调和均值等不同补偿性假设，通过曲线下面积（AUC）量化在不同补偿性假设下的鲁棒性。

Result: 应用于GPT-4和GPT-5的CHC领域得分，一致性调整的AUC显示尽管算术得分较高（如GPT-5达到24%），但两个系统距离通用能力仍然很远。

Conclusion: 广义均值积分提供了一个原则性、可解释且更严格的AGI度量基础，能够惩罚不平衡并捕捉领域间依赖关系。
...
### [151] [Real Deep Research for AI, Robotics and Beyond](https://arxiv.org/abs/2510.20809)
*Xueyan Zou,Jianglong Ye,Hao Zhang,Xiaoyu Xiang,Mingyu Ding,Zhaojing Yang,Yong Jae Lee,Zhuowen Tu,Sifei Liu,Xiaolong Wang*

Main category: cs.AI

TL;DR: 提出了Real Deep Research (RDR)框架，用于系统分析AI和机器人领域的研究趋势，识别新兴趋势和跨领域机会。


<details>
  <summary>Details</summary>
Motivation: AI和机器人领域每年产出超过10,000篇论文，研究人员难以跟上快速发展的趋势，需要工具来帮助识别新兴方向和跨学科机会。

Method: 构建了一个通用的RDR管道，能够系统分析任何研究领域，重点关注基础模型和机器人技术的进展。

Result: 开发了全面的RDR框架，应用于AI和机器人领域，并扩展到其他科学领域。

Conclusion: 该工作为AI及其他领域的研究人员提供了识别研究趋势和探索新方向的工具。
...
### [152] [Excitation of Looped Bistable Bands for High-Speed Linear Actuation](https://arxiv.org/abs/2510.19834)
*Sareum Kim,Josie Hughes*

Main category: eess.SY

TL;DR: 本文研究了环形双稳态带弹簧的高速线性运动放大，通过共振将输入振荡转化为放大的线性运动。


<details>
  <summary>Details</summary>
Motivation: 软机器人技术越来越依赖智能材料和创新结构，双稳态带弹簧因其固有的双稳态特性而成为有前景的选择，能够实现有趣的动态行为如振荡。

Method: 将双稳态带弹簧形成环形结构，产生两个不同的连接点以促进平滑振荡。将其安装在直线导轨上，通过不同频率的曲柄机构驱动，在共振时实现运动放大。

Result: 系统成功将输入振荡转化为放大的线性运动，特别是在共振条件下实现了高速往复线性运动。

Conclusion: 研究证明了双稳态带弹簧在高速往复线性运动应用中的潜力，为软机器人技术提供了新的运动放大方案。
...
### [153] [IMAS$^2$: Joint Agent Selection and Information-Theoretic Coordinated Perception In Dec-POMDPs](https://arxiv.org/abs/2510.20009)
*Chongyang Shi,Wesley A. Suttle,Michael Dorothy,Jie Fu*

Main category: eess.SY

TL;DR: 本文提出了一种用于联合选择感知代理并在Dec-POMDP框架内合成去中心化主动感知策略的两层优化方法，利用信息论指标和子模性理论，开发了IMAS²算法，并提供性能保证。


<details>
  <summary>Details</summary>
Motivation: 解决在多智能体系统中联合选择感知代理并合成去中心化主动感知策略的问题，旨在优化集体感知能力。

Method: 采用两层优化结构：内层使用互信息作为主动感知的统一目标，外层利用信息论目标的单调性和子模性设计IMAS²算法进行联合代理选择和策略合成。

Result: 证明了信息论目标在特定条件下具有单调性和子模性，IMAS²算法能提供(1-1/e)的性能保证，并在网格世界环境中验证了方法的有效性。

Conclusion: 所提出的信息论方法能够有效解决多智能体联合感知代理选择和策略合成问题，具有理论保证和实际应用价值。
...
### [154] [Approximate Model Predictive Control for Microgrid Energy Management via Imitation Learning](https://arxiv.org/abs/2510.20040)
*Changrui Liu,Shengling Shi,Anil Alan,Ganesh Kumar Venayagamoorthy,Bart De Schutter*

Main category: eess.SY

TL;DR: 提出一种基于模仿学习的框架，用于近似微电网能量管理中的混合整数经济模型预测控制，通过神经网络模仿专家控制动作，实现快速实时决策。


<details>
  <summary>Details</summary>
Motivation: 随着可再生能源集成增加，高效的能源管理对微电网可靠和可持续运行至关重要。传统优化方法计算成本高，难以实时应用。

Method: 训练神经网络模仿离线轨迹中的专家EMPC控制动作，通过噪声注入增强鲁棒性，并明确考虑可再生能源和需求的预测不确定性。

Result: 学习策略在经济性能上与EMPC相当，但计算时间仅为基于优化的EMPC的10%。

Conclusion: 该方法能够以显著降低的计算成本实现与EMPC相当的经济性能，适合微电网实时能量管理应用。
...
### [155] [Safe Output-Feedback Adaptive Optimal Control of Affine Nonlinear Systems](https://arxiv.org/abs/2510.20081)
*Tochukwu E. Ogri,Muzaffar Qureshi,Zachary I. Bell,Wanjiku A. Makumi,Rushikesh Kamalapurkar*

Main category: eess.SY

TL;DR: 提出了一种结合状态估计和参数估计的安全控制合成方法，使用自适应最优控制和控制屏障函数保证系统安全性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 解决在缺乏完整状态测量情况下的安全控制问题，将安全目标与学习目标解耦，确保系统在状态估计误差下仍能保持安全。

Method: 采用控制屏障函数保护控制器，结合自适应最优控制框架，利用深度神经网络自适应观测器处理状态估计误差，通过Lyapunov分析提供理论保证。

Result: 在温和激励条件下通过仿真验证了控制器的有效性，能够保证系统安全性和收敛性。

Conclusion: 该方法成功实现了在状态估计不确定情况下的安全控制，为缺乏完整状态测量的系统提供了可靠的安全保障方案。
...
### [156] [SpeechAgent: An End-to-End Mobile Infrastructure for Speech Impairment Assistance](https://arxiv.org/abs/2510.20113)
*Haowei Lou,Chengkai Huang,Hye-young Paik,Yongquan Hu,Aaron Quigley,Wen Hu,Lina Yao*

Main category: eess.SY

TL;DR: SpeechAgent是一个移动语音助手，专门为有言语障碍的人群设计，通过整合大型语言模型和先进语音处理模块，提供实时、低延迟的日常沟通支持。


<details>
  <summary>Details</summary>
Motivation: 尽管ASR和TTS技术有所进步，但针对言语障碍用户的可访问网络和移动基础设施仍然有限，阻碍了这些技术在日常沟通中的实际应用。

Method: 系统整合了LLM驱动的推理与先进语音处理模块，开发了结构化的部署管道，可在移动和边缘设备上实现实时语音处理。

Result: 在真实世界言语障碍数据集和边缘设备延迟分析中的评估证实，SpeechAgent提供了有效且用户友好的性能。

Conclusion: SpeechAgent展示了为个性化日常辅助沟通提供可行解决方案的潜力。
...
### [157] [Interpolatory Approximations of PMU Data: Dimension Reduction and Pilot Selection](https://arxiv.org/abs/2510.20116)
*Sean Reiter,Mark Embree,Serkan Gugercin,Vassilis Kekatos*

Main category: eess.SY

TL;DR: 本文提出使用插值矩阵分解框架来压缩PMU数据，通过选择少量行和列来重构完整数据矩阵，减少通信带宽需求，并提供可计算的重构误差估计用于故障检测。


<details>
  <summary>Details</summary>
Motivation: 为了在电力传输系统实时监测中使用有限的PMU测量数据，减少通信带宽需求，同时提供可靠的数据压缩和故障检测能力。

Method: 采用插值矩阵分解框架，通过离散经验插值方法选择关键的行和列，利用少量PMU数据流和时间快照重构完整数据矩阵。

Result: 数值测试显示DEIM在数据压缩方面表现优异，验证了基于DEIM的故障检测方法的有效性。

Conclusion: 插值矩阵分解为PMU数据压缩提供了严格的误差界限，DEIM算法能够有效控制压缩误差，重构误差估计可作为系统运行状态变化的检测工具。
...
### [158] [Design Optimization and Global Impact Assessment of Solar-Thermal Direct Air Carbon Capture](https://arxiv.org/abs/2510.20135)
*Zhiyuan Fan,Bolun Xu*

Main category: eess.SY

TL;DR: 该研究提出了一种结合聚光太阳能技术和低成本沙基储热的太阳能-热直接空气捕集系统，能够实现80%以上的年容量因子和160-200美元/吨的CO2去除成本，在沙质地区具有巨大部署潜力。


<details>
  <summary>Details</summary>
Motivation: 应对经济脱碳和全球能源需求增长的双重挑战，需要可扩展且成本效益高的二氧化碳去除技术。直接空气捕集技术虽然前景广阔，但其高能耗特别是吸附剂再生所需的热能，成为成本降低和可持续部署的关键障碍。

Method: 分析结合聚光太阳能技术和低成本沙基储热的太阳能-热直接空气捕集系统，研究其在并网和独立运行两种配置下的技术经济性能，重点关注与太阳能可用性匹配的短周期吸附剂。

Result: 太阳能-热直接空气捕集可实现年容量因子超过80%，CO2去除成本低至160-200美元/吨，与领先的直接空气捕集技术具有竞争力。6000吨/年的模块化系统设计占地小于1平方公里，全球沙质地区潜在部署容量超过26吉吨/年。

Conclusion: 太阳能驱动的直接空气捕集系统在太阳能资源丰富且具有沙质地形的地区特别有前景，为地质储碳提供了比地热加热更低成本的替代方案。
...
### [159] [Soft Switching Expert Policies for Controlling Systems with Uncertain Parameters](https://arxiv.org/abs/2510.20152)
*Junya Ikemoto*

Main category: eess.SY

TL;DR: 提出一种基于仿真的强化学习算法，用于控制参数不确定且变化的系统，通过两阶段方法缓解现实差距问题。


<details>
  <summary>Details</summary>
Motivation: 仿真器虽能安全学习物理系统的控制策略，但现实差距仍是主要挑战，需要解决系统参数不确定和变化的问题。

Method: 第一阶段在仿真器中为不同参数系统学习多个控制策略；第二阶段基于观测使用在线凸优化算法平滑切换这些策略。

Result: 通过数值实验验证了所提算法的有效性。

Conclusion: 该两阶段算法能有效处理系统参数不确定性问题，缓解仿真与现实之间的差距。
...
### [160] [From Bundles to Backstepping: Geometric Control Barrier Functions for Safety-Critical Control on Manifolds](https://arxiv.org/abs/2510.20202)
*Massimiliano de Sa,Pio Ong,Aaron D. Ames*

Main category: eess.SY

TL;DR: 本文提出了几何控制屏障函数（CBFs）在流形上的通用理论，为机器人和航空航天应用中的系统提供了构造性合成工具，特别针对几何力学系统开发了基于动能的反步法CBF合成技术。


<details>
  <summary>Details</summary>
Motivation: 控制屏障函数在欧几里得空间中已有成熟理论，但在机器人和航空航天应用中常见的流形上缺乏通用公式和构造性合成工具。

Method: 开发了流形上的几何CBF理论，推广了基于动能的反步法到黎曼流形，为几何力学系统提供构造性CBF合成技术，避免高阶切丛计算。

Result: 提出了适用于控制仿射系统的标准优化型CBF控制器及其光滑类似物，并提供了易于验证的成功条件。

Conclusion: 该技术成功应用于SO(3)上的欠驱动卫星系统，证明了其在几何力学系统中的有效性。
...
### [161] [Observer-based Differentiators for Noisy Signals](https://arxiv.org/abs/2510.20234)
*Van Huynh,Hieu Trinh,Riley Bain*

Main category: eess.SY

TL;DR: 提出了多种观测器系统作为微分器，能够在噪声环境下估计信号的导数


<details>
  <summary>Details</summary>
Motivation: 开发能够在噪声信号条件下准确估计导数的观测器系统

Method: 使用观测器作为微分器，构建不同类型的观测系统

Result: 这些观测器微分器能够有效估计噪声信号的导数

Conclusion: 观测器系统可以作为有效的微分器，在噪声环境中实现导数估计
...
### [162] [Multi-layer Optimized Coordination of Smart Building Resources in Active Power Distribution Systems](https://arxiv.org/abs/2510.20313)
...
<details>
  <summary>Details</summary>
Motivation: 解决智能建筑资源在配电系统中的优化协调问题，同时保护用户隐私数据，避免建筑内部光伏、储能和负载状态信息泄露给电网运营商。

Method: 采用分层优化方法，建立智能建筑协调器(SBC)、微电网协调器(MGC)和配电系统协调器(DSC)三级协调架构，各协调器独立运行并仅交换有限信息。

Result: 数值仿真验证了该平台在协调建筑内部资源与配电系统方面的有效性，实现了资源优化利用。

Conclusion: 所提出的多参与者协调平台能够有效协调智能建筑资源与配电系统，在保护用户隐私的同时实现系统优化运行，具有灵活可扩展的架构。
...
### [163] [On MIMO Stability Analysis Methods Applied to Inverter-Based Resources Connected to Power Systems](https://arxiv.org/abs/2510.20384)
*Anton A. Stoorvogel,Saeed Lotfifard,Ali Saberi*

Main category: eess.SY

TL;DR: 对逆变器基资源小信号稳定性分析常用方法的批判性综述，讨论其适用性、局限性和常见误读来源


<details>
  <summary>Details</summary>
Motivation: 澄清逆变器基资源小信号稳定性分析方法的正确与错误实施方式，阐明其适用性和内在局限性

Method: 采用批判性综述方法，分析文献中常用的分析方法，讨论其预期目的、正确实施和错误实施

Result: 提供了对现有分析技术的适用性见解，澄清了内在局限性，并讨论和说明了常见误读来源

Conclusion: 该综述有助于更准确地理解和应用逆变器基资源小信号稳定性分析方法，避免常见误读
...
### [164] [Interlacing in Controllers Implementation: Frequency Analysis](https://arxiv.org/abs/2510.20394)
*Julian Salt*

Main category: eess.SY

TL;DR: 该论文介绍了如何使用交错技术实现LTI控制器，并分析了不同结构在此环境下的应用，旨在在资源受限环境中实现显著的计算节省。


<details>
  <summary>Details</summary>
Motivation: 在资源受限的环境中实现LTI控制器时，需要寻找能够显著减少计算量的方法。交错技术提供了一种有效的解决方案，通过时间变化系统来优化控制器实现。

Method: 使用交错技术实现LTI控制器，引入新程序获取与不同实数和复数控制器极点相关的块。通过适当的离散提升技术对结果时变系统进行建模，并采用新的高效双率频率响应计算来确定控制环特性。

Result: 提出的方法在资源受限环境中实现了重要的计算节省，通过理论示例验证了所提方法的有效性。

Conclusion: 交错技术为LTI控制器的实现提供了一种高效的方法，特别是在资源受限的环境中，能够显著降低计算需求，同时保持控制性能。
...
### [165] [A Multifunctional Capacitive Sensing Platform for Wireless Vascular and Heart Monitoring](https://arxiv.org/abs/2510.20415)
*Parviz Zolfaghari,Beril Yagmur Koca,Taher Abbasiasl,Hakan Urey,Hadi Mirzajani*

Main category: eess.SY

TL;DR: MAiCaS是一个多功能天线集成电容传感平台，通过利用电感天线的寄生电容作为应变敏感元件，将传感、遥测和机械功能统一到紧凑设计中，实现无源、无线、实时心血管监测。


<details>
  <summary>Details</summary>
Motivation: 传统心血管监测系统需要分离的传感器和无线模块，增加了设备复杂性和尺寸。MAiCaS旨在开发一种集成的、可扩展的解决方案，简化制造过程并提高可重复性。

Method: 采用无洁净室、单步UV激光图案化工艺在柔性PDMS基板上制造传感器。利用电感天线的寄生电容作为应变敏感元件，通过无线共振响应测量应变、压力和变形。

Result: 在体外实验中，MAiCaS在生理条件下表现出稳定的共振频率偏移，在皮肤、PBS、人血清和模拟血管环境中性能稳定。灵敏度分别为：心外膜贴片2.9 MHz/1%应变、血管移植物0.43 MHz/mmHg、支架集成传感器309.6kHz/μm。

Conclusion: MAiCaS提供了一种可扩展、经济高效的无电池血管动力学监测解决方案，具有远程诊断、术后随访和连续心血管健康管理的潜力。
...
### [166] [Behavior-Aware Online Prediction of Obstacle Occupancy using Zonotopes](https://arxiv.org/abs/2510.20437)
...
Result: 在城市环境仿真中验证了方法的有效性，展示了准确且紧凑的预测结果。
...
### [167] [Joint Computation Offloading and Resource Management for Cooperative Satellite-Aerial-Marine Internet of Things Networks](https://arxiv.org/abs/2510.20443)
*Shuang Qi,Bin Lin,Yiqin Deng,Hongyang Pan,Xu Hu*

Main category: eess.SY

TL;DR: 提出了一种用于海上边缘计算和数据存储的协同卫星-空中-MIoT网络，通过联合计算卸载和资源管理算法优化延迟敏感任务的优先级处理，同时处理延迟容忍任务。


<details>
  <summary>Details</summary>
Motivation: 海洋物联网设备通过LEO卫星和无人机连接时，缺乏有效的流量处理策略难以满足低延迟需求，需要优化系统以优先处理延迟敏感任务。

Method: 使用Dinkelbach方法和线性规划提出JCORM算法，控制无人机发射功率、DT任务开始时间、计算资源分配和卸载比例四个相互依赖的变量。

Result: JCORM算法相比基线方法，卫星收集数据量最多增加41.5%，计算时间从318.21秒大幅减少到0.16秒，适合实时应用。

Conclusion: JCORM算法能有效解决卫星-空中-MIoT网络中的联合优化问题，显著提升数据收集效率并降低计算延迟。
...
### [168] [Decentralized Small Gain and Phase Stability Conditions for Grid-Forming Converters: Limitations and Extensions](https://arxiv.org/abs/2510.20544)
*Diego Cifelli,Adolfo Anta*

Main category: eess.SY

TL;DR: 提出了一种通过环路整形变换改进分散稳定性认证的方法，解决了电网形成变换器在低频段的非扇形性问题，降低了保守性。


<details>
  <summary>Details</summary>
Motivation: 随着电力系统中基于变换器的资源比例增加，需要可扩展的稳定性分析方法，但现有的分散小增益和小相位准则对电网形成变换器的适用性受到扇形性假设的限制。

Method: 引入环路整形变换，在替代坐标系中重新表述变换器和网络模型，扩展混合增益相位条件。

Result: 该方法解决了低频段的固有非扇形性问题，降低了保守性，在无限总线系统和IEEE 14总线网络上验证了实用性和可扩展性。

Conclusion: 这些发现为电力系统中更少保守和更广泛适用的分散稳定性证书提供了途径。
...
### [169] [Safe Decentralized Density Control of Multi-Robot Systems using PDE-Constrained Optimization with State Constraints](https://arxiv.org/abs/2510.20643)
...
<details>
  <summary>Details</summary>
Motivation: 传统集中式方法需要大量计算和通信能力，在通信和定位不完美的实际部署中不实用，因此需要开发计算和通信需求更低的去中心化控制器。

Method: 设计去中心化的控制屏障函数，推导局部安全约束保证全局安全的充分条件，将机器人建模为由Fokker-Planck方程控制的空间概率密度函数，显式考虑定位和运动噪声。

Result: 控制器在四个四旋翼飞行器的仿真和实验中得到了验证，相比传统集中式方法需要更少的计算和通信能力。

Conclusion: 该去中心化密度控制器能够有效保证多机器人系统的安全性，更适合在通信和定位不完美的实际场景中部署。
...
### [170] [Sugar Shack 4.0: Implementation of a Cyber-Physical System for Logistic and Sanitary Automation in a Maple Syrup Boiling Center](https://arxiv.org/abs/2510.20682)
*Thomas Bernard,François Grondin,Jean-Michel Lavoie*

Main category: eess.SY

TL;DR: 设计并部署了一个过程感知的赛博物理系统，用于自动化枫糖浆生产中心的物流、可追溯性和卫生管理，取代了人工操作，提高了效率和卫生标准。


<details>
  <summary>Details</summary>
Motivation: 传统枫糖浆生产中心依赖人工操作，存在效率低下、卫生风险和管理成本高等问题，需要自动化系统来改善这些方面。

Method: 采用事件驱动的本地服务器编排，使用可重用设备抽象和集中式互锁机制，通过传感器测量流量、温度和糖浓度来驱动路由决策和自动清洗。

Result: 在2025年生产季中，系统安全执行了431次操作，提高了渗透液储备，消除了污染事件，并将管理时间从30多小时减少到约1小时。

Conclusion: 该系统为模块化、工厂规模的自动化提供了实用路径，并为类似工厂或相关行业的可重用组件奠定了基础。
...
### [171] [Learning Optimal Power Flow with Pointwise Constraints](https://arxiv.org/abs/2510.20777)
*Damian Owerko,Anna Scaglione,Alejandro Ribeiro*

Main category: eess.SY

TL;DR: 提出了一种使用逐点约束训练学习参数化来解决最优潮流问题的新方法，该方法在约束要求在所有问题实例上都成立的情况下，直接替代传统OPF问题中的学习参数化。


<details>
  <summary>Details</summary>
Motivation: 现有监督学习方法要求约束在问题实例的平均值上成立，而新方法旨在通过逐点约束训练来减少约束违反，特别是在约束最难满足的极端情况下。

Method: 在双域中使用增广拉格朗日和双梯度上升算法进行逐点约束训练，将学习参数化直接代入具有约束的OPF问题中。

Result: 数值实验表明，逐点约束训练产生的解具有更小的约束违反，在约束最难满足的极端情况下效果最显著，在具有大量母线的电力系统中收益最为明显。

Conclusion: 逐点约束训练方法能有效减少最优潮流问题中的约束违反，特别是在复杂电力系统的极端情况下表现优异。
...
### [172] [Bilevel Analysis of Cost and Emissions Externalities from Data Center Load Shifting](https://arxiv.org/abs/2510.20805)
*Aron Brenner,Rahman Khorramfar,Nathan Engelman Lado,Line Roald,Saurabh Amin*

Main category: eess.SY

TL;DR: 开发了一个双层优化框架，分析数据中心在考虑电力成本和边际排放强度时的负荷转移行为，研究其与电力系统运行和碳排放的相互作用。


<details>
  <summary>Details</summary>
Motivation: 数据中心作为大型灵活电力负荷，能够根据经济和环境信号跨区域转移计算负载，但其灵活性对电力系统运行和碳排放的影响尚不明确，需要研究这种行为如何与电网约束和市场信号相互作用。

Method: 采用双层优化框架，上层是数据中心最小化电力成本和边际排放强度的加权组合，下层是系统运营商在输电和发电约束下进行经济调度。针对三节点电力系统，推导了数据中心和系统目标的闭式分段线性表达式。

Result: 识别了数据中心分散决策与社会最优行为一致或偏离的充分条件，表征了由此产生的外部性。揭示了系统拓扑和发电机不对称性如何影响激励一致性，以及边际价格或排放信号何时可能无法引导灵活负荷实现社会有益结果。

Conclusion: 为分析碳感知激励下的分散灵活性提供了可处理的起点，并提出了改进灵活负荷与系统运行协调的方向。
