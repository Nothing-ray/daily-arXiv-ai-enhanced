...
TL;DR: 提出SEMIR框架，用参数化图小图代替像素网格，通过图收缩保持薄结构连通性，在不分块情况下实现全分辨率推理，在多个数据集上优于基线并减少掩膜碎片化。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如分块、超像素）在分割薄结构时破坏其连通性，拓扑感知损失虽惩罚断连但无法修复由表示本身造成的破坏。

Method: SEMIR用参数化图小图替换像素网格，通过收缩准则保持薄结构连通性；将数百万像素坍缩为数十/数百个边界对齐超节点；轻量级GNN分类缩减图，再通过精确映射将预测提升至像素级分辨率。

Result: 在TTPLA、CrackSeg9k、SkyScapes Lane数据集上匹配或超越领域特定基线（Dice、IoU、Boundary F1），掩膜碎片化相比SLIC降低至少4.6倍，支持高达21MP的全分辨率推理。

Conclusion: SEMIR通过图小图表示从根本解决了薄结构分割中连通性保持问题，通用性强，无需分块即可高效推理。
...
TL;DR: 提出CurvatureGuided Mixing (CGM)框架，通过Hessian矩阵近似损失景观，推导出最优软混合比，并引入硬混合变体CGM†，有效缓解MLLM微调中的灾难性遗忘问题，在LLaVA-1.5和Qwen2.5VL上取得更好效果。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在专用任务微调后会出现灾难性遗忘，现有模型合并方法多为启发式或使用次优目标函数。

Method: 提出CGM框架：1) 构建联合优化目标；2) 利用二阶(Hessian)近似损失景观，解析推导出封闭形式的"软混合"比例；3) 引入CGM†变体，基于曲率感知的评分进行稀疏参数选择（硬混合）。

Result: 在LLaVA-1.5和Qwen2.5VL多个下游任务上，CGM和CGM†持续改进了任务专业化与通用知识保持之间的权衡，优于现有方法。

Conclusion: 基于曲率引导的模型混合方法在理论上更严谨，实践上更有效，能更好地平衡微调后的专用能力与预训练的通用能力。
...
TL;DR: 提出了一种噪声感知边界增强生成学习框架（NBGL），通过生成学习抑制散斑噪声、边界分支保留解剖结构，并利用自适应噪声权重模块实现跨噪声水平的鲁棒性，在141个3D经阴道超声数据上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 超声图像中的散斑噪声严重影响诊断质量，现有降噪方法存在过度平滑组织边界、对不同噪声水平泛化能力差的问题。

Method: 提出NBGL框架，包含散斑抑制分支（生成学习）和边界增强分支（边界敏感表征学习）；设计NIWG模块通过3D拉普拉斯滤波和中位绝对偏差估计噪声水平，生成自适应交互权重；采用加权特征线性调制（wFiLM）模块实现跨分支自适应特征耦合调节。

Result: 在141个3D经阴道超声数据上，NBGL在6种噪声水平下在散斑抑制和解剖结构保持方面一致优于现有方法，且与标注的解剖边界保持一致性。

Conclusion: NBGL框架能有效解决超声散斑噪声问题，在保持解剖边界的同时适应不同噪声水平，具有临床应用潜力。
...
TL;DR: 本文提出Yuvion VL，一个专为内容与AI安全设计的多模态大语言模型系列，通过将安全视为对抗性多模态问题，构建自动化数据合成管道、三阶段训练流程以及Confuse-then-Contrast Fine-Tuning框架，在安全性上超越同等规模开源模型和顶级闭源商业模型。


<details>
  <summary>Details</summary>
Motivation: 通用模型难以可靠识别和理解真实世界的多模态风险，主要是因为内容和AI安全具有内在的多模态对抗性，现有方法缺乏对此问题的专门设计。

Method: 1）自动化对抗感知数据合成管道，融合领域知识与推理标注；2）三阶段训练：持续预训练（跨模态风险概念对齐）、指令微调（生产级安全任务）、推理微调（可解释性）；3）Confuse-then-Contrast Fine-Tuning（C3FT）对比学习框架，挖掘模型特定混淆点并构建多图像对比组，强制区分视觉上相似但安全含义不同的样本。

Result: Yuvion VL-32B在安全性能上达到行业领先水平，超越同等规模开源模型和顶级闭源商业模型，同时保持相当的一般能力。

Conclusion: 将安全作为固有对抗性多模态问题来设计整个管道是实现高效多模态安全模型的关键路径。
...
TL;DR: 本文提出了一种面向图像到视频（I2V）生成扩散模型的跨请求复用框架，通过稀疏性复用（共享稀疏掩码）为主、特征复用为辅的方式，在几乎不影响生成质量的情况下实现2.16倍加速。


<details>
  <summary>Details</summary>
Motivation: I2V扩散模型推理计算成本高，难以大规模部署；实际工作负载中存在大量相似请求（如重复效果模板、相关主体、镜头布局），现有跨请求加速方法主要利用特征复用，但忽略了稀疏注意力模式的复用潜力。

Method: 提出以稀疏性复用为核心的跨请求复用框架：1）稀疏掩码复用——从历史相似请求中复用高质量稀疏掩码，避免新增在线掩码预测开销；2）可选的特征复用——对高冗余时空区域进行降采样计算，减少边界伪影；3）轻量级引导增强——复用后强化图像/文本条件，缓解语义漂移和条件偏差。

Result: 默认稀疏性复用配置在保持生成质量的前提下，实现了2.16倍加速。特征复用作为可选扩展，进一步在高效与质量之间取得平衡。

Conclusion: 该工作证明了稀疏性复用在I2V扩散模型加速中的有效性，为大规模部署提供了一种低成本、高质量的跨请求复用方案。
...
TL;DR: Wan-Streamer 是一个原生流式、端到端交互基础模型，基于单一Transformer统一建模语言、音频和视频的输入输出，采用块因果注意力支持增量流式处理，实现约200ms模型响应延迟和约550ms总交互延迟，支持亚秒级全双工视听交互。


<details>
  <summary>Details</summary>
Motivation: 现有级联交互系统依赖独立的VAD、ASR、语言、TTS、语音驱动动画或视频生成模块，导致流水线延迟和错误累积。作者希望设计一个统一模型，将感知、推理、生成、响应时序、话轮管理和跨模态同步端到端联合学习，消除对多个外部模块的依赖。

Method: 设计了一个原生流式Transformer架构，将视觉、音频和文本输入/输出令牌交错排列，通过块因果注意力实现增量流式处理。重新设计了因果编码器、因果解码器、块因果注意力和低延迟多模态令牌调度，支持25fps下160ms的流式单元粒度。所有模态的感知、推理和生成在一个统一模型中联合学习。

Result: 模型侧响应延迟约200ms，结合350ms双向网络延迟后总交互延迟约550ms，实现了亚秒级全双工视听通信。

Conclusion: Wan-Streamer 证明了统一、端到端、多模态交互基础模型在低延迟流式交互中的可行性，为实时视听交互提供了新的范式。
...
TL;DR: FreeStory提出了一种无需训练的框架，通过实体锚定的特征复用技术，解决自由形式提示下视觉故事生成中的角色一致性问题，并发布了FreeStoryBench基准数据集。


<details>
  <summary>Details</summary>
Motivation: 现有训练无关方法假设结构化提示（每句重复完整角色描述），但真实故事叙述中角色通常只介绍一次，后续用代词或类型表达指代。这种假设偏离实际应用，因此需要一种支持自由形式提示的角色一致性方法。

Method: 提出FreeStory框架，通过实体锚定的特征复用实现：1) 将指代表达关联到对应的角色描述；2) 结合动态角色掩码、对应感知特征匹配、键值注入和查询混合，在保持身份一致性的同时保留生成多样性。

Result: 在结构化提示基准上达到训练无关方法的最优性能，在自由形式提示下相比基线方法在整体一致性上更强。

Conclusion: FreeStory有效解决了自由形式提示下的角色一致性问题，无需额外训练即可在更自然的叙事场景中保持角色身份一致，同时维持生成多样性。
...
TL;DR: 本文评估了多模态大语言模型（MLLMs）在辅助AI场景中的实际表现，通过构建基于头戴式GoPro的NetraLink系统收集真实世界数据，测试模型在货币识别、场景文本问答、多语言视觉内容读取等任务上的能力，揭示了当前MLLMs在辅助技术中的优势与局限。


<details>
  <summary>Details</summary>
Motivation: 尽管MLLMs在图像描述、视觉问答等通用任务上表现出色，但其在辅助AI（如帮助视障或语言障碍用户与环境交互）中的实际效果尚未被充分理解，因此需要系统评估。

Method: 开发NetraLink系统，使用头戴式GoPro采集真实世界自我中心数据，构建涵盖货币识别、场景文本问答、多语言视觉内容读取的基准测试集，对多个SOTA MLLMs进行评估。

Result: 提供了当前MLLMs在辅助AI场景中的全面诊断结果，明确了其能力优势与不足之处。

Conclusion: MLLMs具备辅助AI的潜力，但在真实场景的鲁棒性、多语言理解等方面仍有局限，需进一步改进才能可靠应用于辅助技术。
...
TL;DR: 本文提出通过学习全精度模型低损失子空间中的量化友好路径，使得量化后的模型能自动位于低损失子空间内，从而无需传统量化感知训练即可保持高性能。


<details>
  <summary>Details</summary>
Motivation: 传统量化方法强制离散约束会导致参数偏离全精度模型已优化的最小值，造成性能下降。而研究发现全精度模型的低损失解存在于连通的低损失子空间中，因此作者希望利用这一特性，使量化模型直接落在该子空间内以自动保持性能。

Method: 通过学习权重空间中的量化感知线性路径（低损失子空间），并优化该路径以最小化损失函数，然后取其中点作为量化友好表示，直接对该中点进行量化得到高性能量化模型。

Result: 该子空间的中点天然具有量化友好特性，直接对其进行量化即可达到与传统量化感知训练相当的性能。

Conclusion: 该方法为权重量化提供了全新视角，无需使用straight-through estimator，也无需在训练过程中进行显式离散化操作。
...
TL;DR: Reflective VLA通过将动作-观测-后果三元组作为上下文条件，使VLA模型从反应式变为反思式，从而在分布偏移下提升泛化能力，在LIBERO-Plus和LIBERO-Plus-Hard上分别提升5.4和4.2个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型是反应式的，仅从当前观测预测动作，但单帧观测无法捕捉相机-机器人几何关系、标定误差等具身因素，导致过拟合训练环境、部署时泛化差。

Method: 提出Reflective VLA，将历史动作-观测-后果三元组作为决策上下文，通过共享注意力的VLM路由所有观测模态，并采用块因果掩码实现并行多帧训练和KV缓存实时推理。

Result: 在LIBERO和SimplerEnv-Bridge上保持强分布内性能；在LIBERO-Plus和LIBERO-Plus-Hard上分别提升5.4和4.2个百分点；消融实验证明动作后果是关键因素。

Conclusion: 反思式上下文（尤其是动作后果）比单纯增加历史长度更能提升跨环境泛化能力，为VLA模型处理分布偏移提供了有效范式。
...
TL;DR: 提出了一种基于笼形几何滤波的实时纹理迁移方法，通过识别非化妆品区域（NCZs）来抑制伪影，在不需大规模训练的情况下实现了高效、鲁棒的纹理迁移，在移动设备上对约4.8k三角网格仅需约70ms。


<details>
  <summary>Details</summary>
Motivation: 现有实时纹理迁移方法在速度与伪影抑制之间存在权衡：低延迟方法伪影严重，而鲁棒方法依赖大规模模型，训练成本高、内存开销大，难以在消费级硬件上部署。

Method: 提出基于笼形几何滤波（cage-based geometric filtering）的方法，识别非化妆品区域（Non-Cosmetic Zones, NCZs）用于伪影抑制，无需手动标注数据集和长时间训练。

Result: 在移动设备上对约4.8k三角网格实现了约70ms的高效运行时间，成功抑制伪影，可在消费级硬件上即时部署。

Conclusion: 该方法在效率与鲁棒性之间取得了良好平衡，无需大规模训练即可实现实时纹理迁移与伪影抑制，具有实际应用价值。
...
TL;DR: MJEPA提出了一种统一的单编码器联合嵌入预测架构，通过跨模态预测实现音频-视觉自监督学习，在多个基准上超越先前方法。


<details>
  <summary>Details</summary>
Motivation: 现有音频-视觉自监督方法依赖模态特定编码器和复杂的对比/重建目标，限制了跨模态协同和可扩展性；而JEPA虽简单，但此前仅用于单模态。

Method: 提出MJEPA，使用单一统一编码器处理双模态，仅采用一个预测目标，在模态内和跨模态间同时应用，关键创新在于跨模态预测。

Result: 冻结ViT-g模型在AudioSet-20K上超越最佳先前冻结基线6.8 mAP，在ESC-50和FSD50K上超越全微调模型，视频基准上使用10倍更少数据仍具竞争力。

Conclusion: 跨模态预测对于共享编码器至关重要，能提升各模态表示质量；统一的单编码器架构有效且高效。
...
TL;DR: 本文提出块稀疏特征化器（BSF）来替代传统的方向性特征分解方法，发现视觉概念通常嵌入在2-4维的低维流形中，并在多个视觉模型上验证了其有效性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统的概念分解方法将视觉概念视为孤立的方向（单维度），但近期研究发现概念实际上以低维区域中的几何结构存在。本文旨在弥合这一差距，提出更符合神经科学中"神经元群体编码"思想的特征化方法。

Method: 利用结构稀疏性中的块稀疏先验，将方向分组为块（block），实现三种块稀疏特征化器（BSF）变体；通过最小描述长度（MDL）分析评估压缩效率。

Result: BSF比基于方向的特征化器能更紧凑地描述激活；恢复的概念通常是2-4维的；在InceptionV1中发现曲线检测器对应单一连续曲线流形；在DINOv3中发现阴影和光照等新流形；可在SDXL扩散模型中实现可解释的图像生成控制。

Conclusion: 块稀疏结构与视觉感知的几何结构更为匹配，视觉概念由低维流形而非孤立方向承载，这一发现为神经网络的解释性提供了新工具。
...
TL;DR: 本文提出OrthoTrack，一种无需训练的系统，仅利用公开的正射影像和表面模型作为地图先验，即可实时估计无人机连续6-DoF轨迹，无需GPS或后处理对齐，在多个基准上大幅超越基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉里程计和SLAM方法存在漂移且只能生成相对尺度轨迹，而单帧地理定位又丢弃时间连续性且速度慢。作者希望利用公开地理数据实现无漂移、绝对尺度的实时6-DoF轨迹估计。

Method: 1) 将关键帧与正射影像匹配，通过表面模型将2D对应点提升到度量3D空间；2) 利用光流将这些锚定在地图上的对应点传播到中间帧，从而为每一帧生成绝对尺度的位姿。

Result: 在MovingDrone数据集和真实世界基准上，OrthoTrack在单GPU上实时运行，大幅超越所有基线方法，即使基线方法获得了理想的尺度与对齐信息。

Conclusion: OrthoTrack通过利用公开地理数据实现了对新区域的即插即用部署，无需站点特定适配，为无人机连续位姿估计提供了一种高效、实用的解决方案。
...
TL;DR: 本文提出了一个英-乌双语的血细胞形态学VQA基准数据集WBCMor VQA，包含11万问答对和2万张单细胞图像，旨在解决多语言医疗环境中语言不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 现有血液学视觉语言资源以英语为中心，无法适用于巴基斯坦等南亚多语言医疗环境。调查显示临床文档与患者沟通之间存在严重的语言不匹配，亟需多语言医疗AI技术。

Method: 基于LeukemiaAttri和WBCAtt数据集的形态学感知标注，结合领域特定的乌尔都语血液学词典，构建了11万对英-乌双语VQA问答对，覆盖2万张白血病和正常单细胞图像。

Result: 建立了包含20K单细胞图像、110K双语问答对的WBCMor VQA基准，并评估了多个开源VLM在该基准上的基线性能。

Conclusion: 该资源旨在促进多语言医疗环境中可访问且临床相关的AI系统开发，填补了南亚地区（尤其是巴基斯坦）血液学多语言VQA资源的空白。
...
TL;DR: MR-DiffuSR是一种基于扩散的超分辨率框架，利用高分辨率T1加权图像先验来引导厚层FLAIR扫描的3D重建，通过跨模态结构注意力防止幻觉，并在ADNI-4数据集上显著优于现有方法，尤其在白质高信号分割任务中展现出强大的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 高分辨率MRI采集受扫描时间限制常导致各向异性或低分辨率图像（如厚层FLAIR），而现有深度学习超分辨率方法容易产生解剖结构幻觉，损害脑结构完整性，亟需一种能防止幻觉且保持结构保真度的超分辨率方法。

Method: 提出多分辨率扩散超分辨率框架MR-DiffuSR，在3D潜空间中运行。核心创新包括：(1)跨模态结构Swin注意力机制，从高分辨率T1w图像提取结构注意力图并应用于低分辨率FLAIR潜特征，解耦解剖结构与模态对比度；(2)混合尺度退化策略，训练模型覆盖连续的下采样因子以增强鲁棒性；(3)基于DINOv3的感知损失以保留高频语义细节。

Result: 在ADNI-4数据集上，MR-DiffuSR平均PSNR达32.46dB、SSIM达0.97、LPIPS达0.07，超越CNN和2D扩散方法。在白质高信号分割下游任务中，10倍下采样时基线Dice降至0.51，而MR-DiffuSR保持0.63，等效7mm层厚下仍具实用性。

Conclusion: MR-DiffuSR通过引入高分辨率T1w结构先验和跨模态注意力机制，有效解决了超分辨率中的结构幻觉问题，并在极端厚层FLAIR扫描上展现出卓越的重建质量和下游任务鲁棒性，为临床MRI超分辨率提供了可靠解决方案。
...
TL;DR: Pre-Warm是一种零训练成本的卷积层初始化方法，通过聚类训练batch中的局部patch来初始化第一层一半的卷积核，在多个基准数据集上显著优于标准Kaiming初始化。


<details>
  <summary>Details</summary>
Motivation: 探索是否可以通过轻量级的、依赖于输入数据的信号来改善现代卷积网络的优化轨迹，而无需增加额外的训练成本或修改网络架构。

Method: 在第一次前向传播前，从单个训练batch中提取均值居中的局部patch，使用MiniBatchKMeans聚类，施加逆曼哈顿空间权重，用得到的聚类中心初始化第一层一半的滤波器（另一半保留Kaiming初始化）。除一个不敏感尺度参数外，所有超参数均有闭式规则。

Result: 在MNIST、Fashion-MNIST、CIFAR-10、SVHN和CIFAR-100五个基准数据集上，8次实验的配对检验显示Pre-Warm在所有数据集上均达到统计显著性（p<0.05），其中SVHN上8/8胜出（p=0.0007），CIFAR-100上7/8胜出（p=0.0033）。

Conclusion: 即使是一个轻量级的、依赖于输入数据的信号也能有意义地改善现代卷积网络的优化轨迹；该方法开销极小，无需架构更改，易于集成到现有训练流程中。
...
TL;DR: CoGeoAD是一个基于CLIP的统一框架，通过构建像素对齐的多视图图像，融合颜色和几何特征，实现零样本3D异常检测，在MVTec3D-AD和Eyecandies基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 零样本3D异常检测在工业质检中至关重要，但缺少标注异常样本；同时现有方法缺乏有效机制融合2D彩色图像与3D几何结构，难以在统一框架中检测表面和结构缺陷。

Method: 提出CoGeoAD框架：1) 数据驱动的多视图注意力机制(MVA)自适应聚合3D特征；2) 多阶段颜色-几何融合模块(MS-CGF)分层集成多模态特征；3) 构建像素对齐的配对多视图图像进行特征融合。

Result: 在MVTec3D-AD和Eyecandies两个基准数据集上取得了最先进(SOTA)的性能，能够有效捕获复杂工业场景中的结构异常和纹理异常。

Conclusion: CoGeoAD通过多视图注意力与分层融合机制，有效解决了颜色与几何特征的互补融合问题，实现了统一的零样本3D异常检测框架。
...
TL;DR: 提出HAS-KD框架，通过异构知识蒸馏将多模态模型和多个模型专家的知识转移到单模态点云网络，无需额外推理成本即可提升3D语义分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D语义分割的多模态融合和多模型集成方法要么依赖额外输入信号，要么计算成本过高。希望在不引入难以承受的代价的情况下有效提升分割性能。

Method: 1) 提出IHD（信息导向异构蒸馏），帮助单模态学生网络吸收多模态教师网络的互补知识；2) 设计IOF策略从连续图像序列中选择信息量丰富的图像用于多模态融合；3) 提出ASD（快照蒸馏），将训练过程中产生的模型快照作为多个专家，每个专家只在其擅长的类别上指导学生。

Result: 在ScanNetV2和S3DIS数据集上达到了最先进的性能，可无缝集成到现有3D分割算法中，带来可观增益且不增加推理负担。

Conclusion: HAS-KD通过知识蒸馏有效实现了多模态和模型集成的优势，同时避免了额外的推理成本，是一种高效且实用的3D语义分割增强方案。
...
TL;DR: 本文提出MRI2Rep，一个基于自回归框架的3D肝脏MRI结构化报告生成系统。利用3929对真实临床数据，通过报告到标签规范化模块将自由文本转换为结构化诊断序列，在测试集上显著优于现有基线，并通过放射科医生盲审和LLM自动评估验证了临床可接受性。


<details>
  <summary>Details</summary>
Motivation: 3D肝脏MRI的手动报告耗时费力，而端到端的结构化报告生成因体数据复杂性和配对数据稀缺而未被充分探索，亟需自动化解决方案。

Method: 提出MRI2Rep自回归框架；设计Report-to-Label Canonicalization (RLC)模块将自由文本报告转换为封闭词汇的结构化诊断序列，无需病灶级标注；基于3929对真实MRI-报告数据训练。

Result: 在测试集上：病例级敏感性76.0%，病灶级F1为29.4%（基线不超过8.3%），肝脏级准确率82.4%。盲审中两位放射科医生分别认为75%和70%的AI生成报告临床可接受（原始报告为95%和100%）；LLM-Eval评估61.8%可接受。

Conclusion: 本文是首个端到端的LI-RADS结构化3D肝脏MRI报告生成系统，证明了该方法的可行性，并提供了LLM自动评估作为保守替代方案。
...
TL;DR: 本文提出了两套技术验证协议，系统评估了五种RGB和RGB-D相机在医疗监控场景下的计量性能和人体姿态估计能力，发现深度测量误差范围从50mm到1400mm以上，2D姿态估计精度相近（mAP 78%-90%），而3D重建误差差异显著（MPJPE 104mm-365mm），为相机选型和部署提供了循证指导。


<details>
  <summary>Details</summary>
Motivation: 现有研究往往孤立评估设备计量性能或算法精度，未系统考虑光照变化、遮挡、相机位置等实际部署因素，导致临床或家庭场景下的相机选型缺乏依据，研究可重复性不足。

Method: 设计了两个技术验证协议：协议一评估RGB和RGB-D相机的计量性能；协议二评估其在人体姿态估计中的支持能力。系统测试了5款相机（4款RGB-D，1款RGB），在代表性室内场景中控制光照、相机高度、视角和遮挡程度变化，并使用最先进的姿态估计器进行验证。

Result: 计量性能差异显著：5米处深度偏差范围50mm-1400mm以上；2D姿态估计所有相机精度相当（mAP约78%-90%）；3D重建误差差异大（MPJPE 104mm-365mm），与深度传感质量密切相关；环境因素对3D性能有相机和估计器依赖的影响，而安装高度影响较小。

Conclusion: 本研究为医疗监控应用中的相机选型和部署提供了循证指导，填补了当前技术验证实践中的重要空白。
...
TL;DR: 本文提出一种基于梯度计算像素波动的轻量级假图检测方法，在低端设备上实现与SOTA相当的检测精度。


<details>
  <summary>Details</summary>
Motivation: 恶意合成假图泛滥，低配置智能设备用户易受欺骗，急需高效且计算资源需求低的检测方法。

Method: 通过计算相邻像素间的梯度（作为高通滤波器）来捕获像素波动，突出灰度强度变化，最小化颜色影响，从而区分真伪图像。

Result: 在多个数据集上达到与最先进技术相当的准确率，但所需计算资源极少。
...
TL;DR: 本文构建了儿童危险动作数据集KidRisk（含2500个短视频和10000张图片），并提出基于视觉语言模型的基线方法，在儿童动作分类（83.53%准确率）和危险动作识别（96.14%准确率）上显著优于传统深度学习模型。


<details>
  <summary>Details</summary>
Motivation: 儿童在无人看管时自发活动中常遇到危险情况，现有传统深度学习模型在识别儿童危险动作方面效果有限，因此需要构建专门数据集并探索更有效的视觉语言模型方法。

Method: 1) 构建KidRisk数据集（2500个短视频+10000张图片）；2) 建立基于视觉语言模型的基线方法，利用其出色的上下文理解能力处理视觉信息。

Result: 儿童动作分类准确率83.53%，危险动作识别准确率96.14%，显著优于传统深度学习方法。

Conclusion: 视觉语言模型在检测儿童危险动作方面不仅可行且高效，能够有效助力儿童安全防护。
...
TL;DR: HiFiVe是一个无需训练的3D车辆生成框架，通过联合纹理与几何增强，利用深度引导和多视图融合实现高保真、跨视图一致的车辆建模。


<details>
  <summary>Details</summary>
Motivation: 现有3D车辆生成方法几何保真度低、纹理模糊，多视图扩散模型受限于固定视角、低分辨率且依赖昂贵微调才能实现跨视图一致性。

Method: 提出自回归纹理精炼管线，逐步合成任意视角的高分辨率纹理；利用粗几何作为同步先验，通过深度扭曲和多视图纹理融合保证跨视图一致性；利用车辆对称性减少误差累积；通过法向图精炼网格几何恢复高频表面细节。

Result: 在合成和真实车辆数据集上的大量实验表明，该方法在几何细节和纹理质量上均显著优于现有基线。

Conclusion: HiFiVe通过几何约束锚定2D生成先验，无需训练即可实现高保真车辆建模，有效解决了跨视图一致性和纹理分辨率问题。
...
TL;DR: 本文提出PQSG（物理问题场景图），一种基于层次化问题图的视频物理合理性评估方法，并构建了FinePhyEval数据集用于细粒度评估。实验表明PQSG与人类判断的相关性优于以往方法，且揭示了当前视频生成模型在物理规律遵循方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型尽管能生成逼真视频，但仍难以遵循基本物理规律，且缺乏可靠的细粒度评估方法来定位和具体说明视频中的物理违例现象。

Method: 提出Physics Question Scene Graph (PQSG)评估管道：利用VLM生成基于图的层次化问题，通过问题间的逻辑依赖关系确保上下文有效性，对视频在物体、动作和物理规律遵守三个方面进行细粒度评估。

Result: 构建FinePhyEval数据集，包含来自Sora 2、Veo 3、Wan 2.1等模型的视频及人工标注。PQSG与人类判断的相关性高于以往方法；PQSG排名显示闭源模型在物理真实性上优于Wan 2.1；VLMs在生成问题上接近人类水平，但在回答问题方面仍落后。

Conclusion: PQSG提供了一种可靠的视频物理合理性细粒度评估方法，FinePhyEval可作为基准，未来工作可进一步提升VLMs的物理理解与问答能力。
...
TL;DR: 本文提出了目前规模最大、最全面的遥感目标检测数据集LEVIRDet-159（159类、256万边界框、70万细粒度标注），并设计了尺度层级感知的检测基础模型LEVIRDetNet，通过在线GSD预测、GSD条件查询调制与层级感知检测头实现跨域泛化，在9个外部基准上超越强监督方法平均5.02 mAP。


<details>
  <summary>Details</summary>
Motivation: 现有遥感目标检测数据集和检测器存在碎片化问题：数据集局限于有限类别、固定空间分辨率或单一传感器；检测器难以跨不同传感器和分类体系工作。亟需大规模、多类别、多尺度的统一数据集和通用检测模型。

Method: 构建LEVIRDet-159数据集（159类、多级分类体系、2.56M边界框、700K细粒度标注）；设计LEVIRDetNet检测基础模型，包含三个核心模块：（1）在线视觉GSD预测，（2）GSD条件查询调制与分配，（3）层级感知检测头，支持混合粒度的遥感监督。

Result: LEVIRDetNet无需目标域训练或微调，在9个外部基准上均达到SOTA检测性能，平均超越最强全监督方法5.02 mAP（各基准主指标下）。

Conclusion: LEVIRDet-159数据集和LEVIRDetNet模型有望推动跨类别体系、跨空间分辨率、跨传感器平台的强泛化遥感目标检测发展。数据集和模型将开源发布。
...
TL;DR: 本文提出ESTANet，通过利用动作检测器自身在不同输入动态和时间上下文下的预测行为差异（而非设计复杂架构）来实时检测程序性视频中的错误，在三个基准数据集上实现了SOTA性能，且保持轻量级实时效率。


<details>
  <summary>Details</summary>
Motivation: 现有程序性视频错误检测方法通常依赖复杂架构或专门监督信号，但作者观察到动作检测器本身在不同输入动态和时间上下文下会表现出不同的预测特性，这一简单但被忽视的视角可以用于错误检测。

Method: ESTANet包含标准动作检测器和错误敏感动作检测器，二者在正确执行时行为相似，但在错误发生时响应不同；同时利用不同时间上下文的检测器进一步放大预测不一致性；通过多数投票聚合匹配度来标记包含错误的帧。

Result: 在EgoPER、Assembly-101-O和EPIC-Tent-O三个数据集上，ESTANet达到了在线错误检测的SOTA性能，且保持轻量级架构下的实时效率。

Conclusion: 利用动作检测器自身的固有属性（而非增加架构复杂度）可以构建强大且实用的在线错误检测解决方案。
...
TL;DR: 提出了一种离散旋转反射群等变的视觉Transformer（ViT），结合卷积注意力机制，在图像分类中优于现有等变网络方法。


<details>
  <summary>Details</summary>
Motivation: 现有旋转反射等变模型主要集中在CNN上，而ViT的等变性研究较少且实现复杂，本文旨在简化ViT中等变性的实现并提升性能。

Method: 设计了一种离散旋转反射群等变的视觉Transformer架构，采用卷积注意力机制来实现特征图的旋转、翻转和位置对称性保持。

Result: 实验证明该方法在图像分类任务上优于已有的离散旋转反射群等变神经网络方法。

Conclusion: 本文提出的方法为ViT实现旋转反射等变性提供了一种更简单的途径，且性能更优，证明ViT也能有效应用于对方向敏感的视觉任务。
...
TL;DR: 本文提出V-Zero，一种无需答案标签的视觉推理框架，通过对比证据门控机制进行在线策略蒸馏，在多个基准上有效提升细粒度视觉推理能力，且训练速度比传统监督微调快5倍以上，比强化学习快10倍以上。


<details>
  <summary>Details</summary>
Motivation: 现有细粒度视觉推理方法要么依赖强化学习（代价高昂的探索和人工验证规则），要么依赖监督微调（需要大量标注推理轨迹），都严重依赖外部答案标签。作者希望避免这种标签依赖，探索在线策略蒸馏（OPD）的能力边界，并解决其缺乏轨迹级判别能力的局限。

Method: V-Zero框架：1）将OPD重新解释为无负例的停止梯度对齐，揭示了其缺乏轨迹级判别的问题；2）提出对比证据门控机制，将问题相关的区域裁剪作为正样本，与负视觉视图配对，用于评估学生采样的轨迹并控制密集token级蒸馏。

Result: 在多个视觉推理基准上，V-Zero一致提升了细粒度视觉推理性能，同时保持了良好的泛化能力。训练速度比监督微调快5倍以上，比强化学习基线快10倍以上。

Conclusion: V-Zero成功证明了无需答案标签即可有效进行细粒度视觉推理训练，通过对比证据门控弥补了OPD的轨迹级判别缺失，实现了高效且性能优越的视觉推理框架。
...
TL;DR: RS4D是一种基于蒸馏状态空间建模的遥感实例分割方法，通过知识蒸馏将Transformer的自注意力知识压缩到线性状态空间中，实现了线性计算复杂度，在参数减少8倍、FLOPs减少9倍的同时保持或超越ViT和CNN方法的精度。


<details>
  <summary>Details</summary>
Motivation: Transformer的计算复杂度随tokens数量呈二次增长，严重限制了基于ViT的遥感基础模型在密集预测任务（如实例分割）中的效率。

Method: 提出自适应噪声和掩码知识蒸馏训练方法用于预训练轻量级SSM骨干网络；基于该轻量级视觉编码器设计遥感实例分割架构，探索三种不同骨干网络和两种分割头变体。

Result: 在SSDD、WHU、NWPU等基准数据集上，相比ViT方法，参数减少8倍，FLOPs减少9倍，精度与ViT和CNN方法相当或更优。

Conclusion: 通过蒸馏状态空间建模实现了线性复杂度的遥感实例分割，在显著降低计算开销的同时保持了高精度，为遥感领域长序列建模提供了高效解决方案。
...
TL;DR: 本文全面综述了状态空间模型（SSMs）在遥感领域的应用，涵盖多模态数据、密集视觉预测和时间序列遥感等任务，分析了架构设计进展、关键挑战及未来方向，并提供了持续更新的资源仓库。


<details>
  <summary>Details</summary>
Motivation: SSMs具有线性计算复杂度和强大的长程依赖建模能力，在遥感领域独特的挑战（如密集视觉预测、多模态和时序数据）中展现出巨大潜力，但缺乏系统性综述来总结该方向的快速发展。

Method: 采用多维度分析方法，系统梳理SSMs在遥感任务中的应用（密集预测、多模态、时间序列等），并讨论架构设计方面的演进，涵盖从SSMs引入遥感领域至今的主要研究成果。

Result: 全面总结了SSMs在遥感领域的应用现状和架构设计进展，识别出关键挑战与未来机遇，并建立了持续更新的开源资源仓库。

Conclusion: 本文为遥感研究者提供了SSM方向的系统性基础资源，通过详细视角和可操作见解推动该领域的进一步发展。
...
TL;DR: 本文提出Invoice Haystack基准（1500张发票，视觉高度同质）和VL-RAG混合检索框架，解决VLM在视觉同质文档集合中的"嵌入坍塌"问题，VL-RAG在Recall@1上超越现有方法13.5个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在单文档VQA上表现接近人类，但在视觉高度同质的大规模文档集合中检索能力显著下降。现有多文档基准包含多样文档类型，在嵌入空间中形成人为区分，无法反映企业文档库中成千上万记录共享相同视觉模板的现实场景。作者将此问题定义为"嵌入坍塌"。

Method: 1) 构建Invoice Haystack基准：1500张匿名发票+200个判别性问答对，平均余弦相似度0.73（远超现有基准的0.38/0.31）；2) 提出VL-RAG混合检索增强生成框架：联合利用文本和视觉嵌入的双流融合，再通过VLM验证过滤器进行精确文档识别。

Result: VL-RAG在Invoice Haystack-500上Recall@1达60.0%，比SOTA高13.5个百分点；在DocHaystack-1000达77.1%（vs 75.2%），InfoHaystack-1000达84.5%（vs 80.0%），一致优于现有方法。

Conclusion: 双流融合策略是跨同质和异质文档集合的一致优越检索策略，VL-RAG有效缓解了视觉同质文档的嵌入坍塌问题。
...
TL;DR: 本文提出ACT框架，利用骨骼作为紧凑表示，结合3D点轨迹进行显式运动引导，通过路由轨迹注入器实现准确的轨迹到关节映射，在4D生成中显著提升了保真度和时间一致性。


<details>
  <summary>Details</summary>
Motivation: 现有4D生成方法依赖稠密网格或高斯表示，计算成本高且易产生时间伪影，文本和视频控制信号分别缺乏时空细节或与外观纠缠，导致时间一致性差、身份错误和可控性有限。

Method: 提出ACT框架，采用骨骼作为拓扑通用的紧凑表示，利用单目视频的3D点轨迹作为显式运动引导（解耦外观），核心是Routed Trajectory Injector，包含三种机制：先验引导硬路由（精确骨骼-网格对应）、全局路由（全身运动感知）和局部窗口交叉注意力（细粒度时间对齐）。

Result: 大量实验表明，ACT在保真度和时间一致性上显著优于现有方法。

Conclusion: ACT通过骨骼表示和轨迹引导，有效解决了4D生成中的时间一致性、身份错误和可控性问题，为高质量4D动画生成提供了新范式。
...
TL;DR: 本文提出超图常态世界模型（Hypergraph Normal World Model），利用DINOv2令牌构建超图结构来建模正常图像中的局部、关系与超图统计量，从而检测逻辑异常。在MVTec LOCO数据集上，逻辑异常AUROC从0.8434提升至0.9279，并验证了建模正常关系对逻辑异常检测的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有单类异常检测方法主要针对局部结构缺陷，难以处理逻辑异常——每个局部看似正常，但整体违反数量、共现或空间关系。本文旨在探索能否仅从正常图像中学习类别特定的"常态世界"，以检测逻辑异常。

Method: 基于冻结的DINOv2补丁令牌，构建超图常态世界模型：将令牌分组形成空间超边，提取局部、关系、超边和超边关系四类统计量；通过信息商（information quotient）对测试图像进行评分，分离不同层级的证据。

Result: 在MVTec LOCO早餐盒验证集上，逻辑异常AUROC从DINOv2 patch-kNN的0.8434提升至0.9279；相比无超图变体（0.9013）也有提升。少样本实验仍有效。t-SNE能分离逻辑异常，关系反事实使信息商平均提升83.13，随机超图降低AUROC，超边归因在逻辑异常上更大。

Conclusion: 逻辑视觉异常检测不仅应建模正常局部补丁，更应建模正常关系。超图结构能有效捕捉逻辑异常所需的全局关系信息。
...
TL;DR: 本文揭示了一个被忽视的多模态漏洞：在合成医学图像检测中，视觉语言模型（VLM）会过度依赖文本元数据来判断图像真实性，导致同一张图像因伴随文本不同而产生不同的预测结果。作者为此构建了一个配对基准测试，系统评估了多种VLM在多模态输入下的鲁棒性问题。


<details>
  <summary>Details</summary>
Motivation: 现有合成医学图像检测研究通常孤立评估图像，但临床实践中图像与结构化记录、元数据共同呈现，VLM也越来越多地在图文联合输入场景下部署。作者发现VLM存在过度依赖文本上下文的漏洞，即同一图像因文本变化而得到不同真实性预测，这威胁到实际部署的鲁棒性。

Method: 将合成医学图像检测重新定义为图像-记录界面的多模态鲁棒性审计问题；构建配对基准测试，保持图像固定，仅交换受控的元数据变体；在多种成像模态下评估多种开源和前沿API VLM，量化元数据单独对真实性预测的偏移程度。

Result: 发现VLM在多模态输入下会过度加权记录上下文，导致同一图像因文本变化而产生不同的真实性预测结果，揭示了多模态脆弱性的存在。

Conclusion: 该基准测试为评估和改进超越纯图像设置的多模态鲁棒性提供了标准化工具，代码已开源。
...
TL;DR: 本文提出一种针对主体无关换脸模型的可迁移攻击方法AIR，通过重光照和加性扰动同时攻击身份提取模块，在无需替代模型的情况下实现了高成功率和高质量攻击。


<details>
  <summary>Details</summary>
Motivation: 现有针对换脸模型的防御方法（如深度伪造检测、对抗扰动）要么是被动响应，要么对未见的主体无关换脸模型无效，亟需一种主动且可迁移的攻击方法。

Method: 提出基于重光照函数的加性身份攻击（AIR），同时使用重光照扰动和加性扰动来欺骗身份提取模块；设计自适应平移不变操作和光照控制方案以提升视觉质量；理论上证明了攻击空间的扩展性。

Result: 在1000对图像上，针对包括GAN和扩散模型在内的多种SOTA主体无关换脸模型进行实验，AIR在攻击成功率和图像质量上均超越现有所有攻击方法。

Conclusion: AIR无需替代模型即可实现高迁移性攻击，为换脸模型的主动防御提供了新思路。
...
TL;DR: 本文提出ALDM，一种用于少样本3D MRI合成的框架，通过两阶段方法（3D VAE + 条件潜在扩散模型）在仅有16张目标图像的情况下生成结构连贯的脑肿瘤体积，在FID和分类AUC指标上优于GAN基线。


<details>
  <summary>Details</summary>
Motivation: 弥散胶质瘤的准确分类常因跨中心域偏移和缺乏大规模标注数据集而受阻，亟需一种数据高效且能处理域差异的医学图像合成方法。

Method: 两阶段框架：第一阶段用3D变分自编码器从数据丰富的源域学习解剖先验；第二阶段用条件潜在扩散模型，通过ControlNet以肿瘤掩膜为条件，在数据稀缺的目标域生成结构连贯的3D体积。

Result: 在仅16张目标图像的极限少样本设置下，ALDM的FID达85.40，下游分类AUC达0.987，均优于GAN和混合基线。病理边界清晰、跨模态一致性好。

Conclusion: ALDM通过捕获关键诊断特征，为低资源环境下的临床数据增强提供了鲁棒工具，代码已开源。
...
TL;DR: 本文提出Teach-to-Reason (T2R)框架，通过比较监督信号和自改进教师模型来优化胸部X光视觉问答中的链式推理过程，克服了传统强化学习中答案级奖励过于粗糙的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于强化学习的训练通常使用答案级奖励，但这种奖励过于粗糙，难以有效提升链式推理质量。当群体优势趋近于零时，奖励信号甚至会失效。

Method: T2R框架包含一个自改进的Teacher和一个竞争引导的Reasoner。Teacher通过自我竞争逐步强化，Reasoner则针对逐步增强的Teacher生成参考进行优化。同时设计了案例级奖励机制：在原始奖励信号有效时保留正负分区，在退化时通过竞争分数恢复监督信号。

Result: 在多个胸部X光开放VQA基准上，T2R一致优于强基线方法。

Conclusion: 以受控且原则性的方式整合比较监督，能够为推理优化提供更有效的训练信号。
...
TL;DR: 本文提出了首个用于胃镜场景的新视角合成（NVS）数据集GastroNVS，包含胃镜图像、相机位姿和点云，并评估了多种3D高斯泼溅方法，为胃镜NVS研究提供了基础。


<details>
  <summary>Details</summary>
Motivation: 现有NVS方法（如NeRF和3DGS）在胃镜领域的应用潜力很大（如扩展视野、数字孪生、手术训练），但缺乏专门用于胃镜场景的NVS数据集来支持相关研究。

Method: 构建了首个真实胃镜NVS数据集GastroNVS，包含胃镜图像、相机位姿和点云数据，并基于该数据集对多种3DGS方法进行了评估。

Result: 成功构建了GastroNVS数据集，并通过实验评估了多种3DGS方法在该数据集上的表现，指出了胃镜NVS面临的挑战。

Conclusion: GastroNVS数据集填补了胃镜NVS领域的空白，为未来的胃镜新视角合成研究提供了基准，同时揭示了该场景下仍需解决的难点。
...
TL;DR: PRISM是一个前馈式单图3D重建框架，通过将多视角潜在预测分解为几何先验和残差校正，无需扩散采样即可在36秒内完成场景重建，性能与扩散方法相当。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的单图3D重建方法虽然效果好，但依赖迭代采样过程，推理时间过长，限制了实际部署。

Method: 提出PRISM前馈框架：(1) 将多视角潜在预测分解为参数无关的几何前向扭曲先验 + 可学习的残差校正；(2) 两阶段训练策略：潜在监督蒸馏（保证几何泛化）+ 感知微调（优化外观质量）。

Result: 在三个基准数据集上，PRISM的重建质量与扩散方法相当，但推理时间大幅降低至每场景仅36秒。

Conclusion: 通过巧妙利用几何扭曲覆盖大部分目标视图这一观察，PRISM在不牺牲重建质量的前提下，实现了从扩散迭代采样到前馈推理的范式转变，显著提升了实际部署效率。
...
TL;DR: LinStereo是一种基于Depth Anything V3的立体匹配方法，通过位置感知线性注意力（PALA）、分层语义代价体（HSCV）和深度先验初始化（DPI）三个组件，在标准基准和跨域泛化（尤其水下场景）上达到SOTA级精度，AbsRel降低26-28%。


<details>
  <summary>Details</summary>
Motivation: 现有VFM迭代立体流水线未充分利用多尺度骨干特征（被压缩为单级关联）、几何先验（初始化时未利用）和上下文传播（仅局部），导致在光度退化（如水下场景）下性能下降。

Method: 基于Depth Anything V3构建LinStereo，核心是位置感知线性注意力（PALA）模块，以线性成本实现全局聚合替代局部循环，将可靠区域的估计传播到退化区域。两个关键组件：分层语义代价体（HSCV）提供VFM特征层次中的尺度对齐关联；深度先验初始化（DPI）将单目深度转换为度量校准的初始值。

Result: 在标准基准上达到SOTA级精度，跨域泛化能力强。水下场景中：TartanAir-UW上AbsRel降低28%，SQUID（真实水下数据集）上AbsRel降低26%。

Conclusion: LinStereo通过全局注意力机制和VFM特征的有效利用，显著提升了立体匹配在退化环境（特别是水下场景）中的泛化能力和精度。
...
TL;DR: 本文提出C3-Bench，一个面向真实世界多样变化上下文的全面评估基准，包含4,996对人类标注图像对、51种变化上下文、首个LLM-as-Judge评估框架及新颖的可逆性度量，并对32个模型（传统模型、专有LMMs、开源LMMs）进行基准测试，揭示了现有变化描述系统在脱离训练风格时的系统性盲点。


<details>
  <summary>Details</summary>
Motivation: 现有变化描述系统虽受关注，但其在多样化真实世界变化上下文中的真实性能尚未被充分探索，缺乏全面的评估框架来揭示模型的局限性。

Method: 构建C3-Bench基准，包含4,996对人类标注图像对、51种变化上下文（涵盖自然场景、遥感、图像编辑、异常四个领域）；提出首个LLM-as-Judge评估框架，在正确性、特异性、流畅性、相关性等细粒度维度上进行评估，并引入新颖的可逆性度量来检验模型对变化理解的对称一致性。

Result: 对32个模型（传统变化描述模型、专有LMMs、2B-90B开源LMMs）进行基准测试，发现一旦变化上下文偏离训练风格，传统模型完全崩溃，即使是GPT-5.2等最先进LMMs也表现出系统性的领域依赖性和位置依赖性错误。

Conclusion: 通过将隐藏的失败模式显式化、可度量，为构建可泛化、可信赖的变化描述系统指明了下一个前沿方向。
...
TL;DR: EchoStyle是一个可扩展的文本驱动视频风格化框架，通过视频到视频架构、自动合成的V-Style20k数据集以及init-follow-mode+滑动窗口机制，实现了任意长度视频的高质量风格化，在多种艺术风格上表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有视频风格化方法存在内容泄露、数据稀缺、对长视频适应性有限等问题，导致风格漂移和运动失真，亟需一种可扩展的高质量解决方案。

Method: 提出EchoStyle框架：1）构建视频到视频架构，重新融合视频内容与文本风格；2）首创自动反向合成流程，建立包含20k高质量视频对的V-Style20k数据集；3）设计init-follow-mode机制与滑动窗口推理策略，支持任意长度视频风格化。

Result: 在多种艺术风格上均表现优异，性能甚至可媲美领先的闭源解决方案，有效解决了风格漂移和运动失真问题。

Conclusion: EchoStyle通过数据集构建与推理策略创新，为长视频风格化提供了一种可扩展的实用方案，推动了视频风格化领域的发展。
...
TL;DR: 提出Causal-rCM框架，将扩散蒸馏方法rCM扩展到自回归视频扩散领域，通过结合教师强制（前向散度）和自我强制（反向散度），在流式视频生成中达到SOTA性能，蒸馏后的2步Wan2.1-1.3B模型在VBench-T2V上达到84.63分。


<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散已成为实时流式视频生成和交互式世界模型的主要范式，但现有扩散蒸馏方法（如一致性模型和分布匹配蒸馏）尚未有效整合到自回归设置中。作者希望利用前向散度（教师强制）和反向散度（自我强制）的互补性，提升自回归视频扩散的效率和性能。

Method: (1) 将rCM扩散蒸馏框架扩展到自回归视频扩散，利用教师强制（TF）作为前向散度的离线因果训练范式，自我强制（SF）作为反向散度的在策略细化。(2) 首次实现基于教师强制的连续时间一致性模型（如sCM/MeanFlow），通过自定义掩码FlashAttention-2 JVP内核，相比离散时间CM加速10倍收敛。(3) 提出Causal-rCM统一框架，集成扩散蒸馏与因果训练。

Result: 蒸馏后的2步Causal Wan2.1-1.3B模型在VBench-T2V上达到84.63分（仅需1或2步采样）；在逐帧和逐块设置下均实现流式视频生成SOTA；成功应用于Cosmos 3全模态世界基础模型，支持动作条件生成。

Conclusion: 教师强制CM是自我强制DMD的最佳初始化策略，连续时间CM相比离散时间CM收敛速度提升10倍，Causal-rCM作为统一可扩展的开源方案在流式视频生成和交互式世界模型上取得领先性能。
...
TL;DR: 本文提出TACO框架，通过相对结构蒸馏和表征-优化空间解耦，缓解CLIP微调中泛化性与专用性之间的失衡问题，在开放词汇视频识别任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP视频适配方法忽略微调数据分布外的表征偏移，导致次优适应效果；这种偏移源于微调目标（已知分布）与评估目标（未知分布）之间的不一致。

Method: 提出TACO框架，包含两个核心：(1) 相对结构蒸馏（Relative Structure Distillation），约束表征空间的相对几何结构，抑制训练中有害的语义对齐偏移；(2) 轻量级专用投影（specialization projection），将表征空间与优化空间解耦，避免测试时使用的表征过度专化。

Result: 在跨数据集和base-to-novel等多种benchmark上达到SOTA性能。

Conclusion: TACO通过保持训练分布之外的OOD相关语义对齐，有效缓解了微调-评估目标不一致带来的负面影响，实现了更好的泛化-专用权衡。
...
TL;DR: 论文发现蒙特卡洛渲染生成的立体图像对中，左右视角的方差场经由真实视差对齐后具有高度相关性（ρ≈0.75），这种结构是MC渲染数据独有的，可作为匹配线索，但可能构成从仿真到真实的捷径（sim-to-real shortcut），影响训练网络的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 揭示路径追踪合成立体数据中一个此前未被认识到的属性：尽管左右相机的蒙特卡洛噪声流统计独立，但它们的方差场经真实视差对齐后高度相关，这一信号可能构成仿真到真实领域的捷径，影响视差估计网络的训练和泛化。

Method: 使用Mitsuba 3渲染20个场景，计算左右视图方差场经真实视差对齐后的皮尔逊相关系数；通过残差洗牌（residual-shuffle）干预破坏跨视角对齐，评估对GT代价边界（cost margin）和基于方差的赢者通吃（WTA）精度的影响；分析不同材质（Lambertian vs. 玻璃）下的相关性差异。

Result: 方差场相关系数在20个场景中平均为ρ=0.754±0.016（SPP=512）；在代表性场景中，相关系数在16倍SPP范围内基本不变（ρ=0.778±0.001）；Lambertian区域相关性强（ρ≈0.78），玻璃区域弱（ρ≈0.30）；洗牌干预使非玻璃区域GT代价边界下降33%，玻璃区域基于方差的WTA精度下降4.3倍。

Conclusion: MC渲染数据中跨视角方差场的高度相关性是一个独特的结构信号，可作为匹配线索，但同时也构成潜在的sim-to-real shortcut，可能使训练网络依赖渲染特有的统计特性而非真实场景的几何特征，其具体影响有待进一步量化。
...
TL;DR: 本文提出了HG-Bench基准，用于评估AI在嘈杂、多页手写作业中定位答案区域及其推理步骤的能力。实验表明现有零样本模型表现不佳（FA<55%，FSm<48%），而微调后的GLM-4.6V 9B可达约74%/72%，揭示出步骤级手写定位是一个关键能力缺口。


<details>
  <summary>Details</summary>
Motivation: 现有评估缺少页面感知的、两级答案区域定位设定：给定作业页面序列，模型必须定位完整答案区域及其有序步骤级子区域。自动作业评估不仅依赖识别学生答案，还需要准确定位每个答案和中间推理步骤出现在嘈杂多页手写作业中的位置。

Method: 构建HG-Bench基准（500个K-12手写作业样本，来自1,489,278张源图池），包含含层级约束的问题级和步骤级标注框。提出页面感知评估协议，分别测量完整答案定位（FA）和步骤级分解（FSm）。在多种闭源API和开源VLM上测试零样本性能，并构建微调参比模型（GLM-4.6V 9B，~10k领域样本）。

Result: 零样本系统FA不超过55.22%，FSm不超过48.22%。微调后的GLM-4.6V 9B达到FA=74.97%、FSm=72.26%。揭示步骤级手写定位是现有模型的能力缺口。

Conclusion: 提供了可复现的基准、评估协议和训练参考点，为推动自动作业评估的未来研究奠定了基础。
...
### [48] [Expresso-AI: Explainable Video-Based Deep Learning Models for Depression Diagnosis](https://arxiv.org/abs/2606.25606)
*Felipe Moreno,Sharifa Alghowinem,Hae Won Park,Cynthia Breazeal*

Main category: cs.CV

TL;DR: 本文提出了一种基于深度神经网络解释性分析的框架，通过微调预训练的DCNN模型，从面部视频中诊断抑郁症严重程度，同时生成视觉和量化的解释性信息，并提升了预测性能。


<details>
  <summary>Details</summary>
Motivation: 当前自动化抑郁症诊断方法缺乏情感特异性和可解释性，尤其是深度模型在处理面部视频时的时间活动可解释性未被充分探索，限制了其在临床实践中的应用。

Method: 微调在动作识别数据集上预训练的深度卷积神经网络（DCNN），在AVEC抑郁症数据集的面部视频上进行训练；通过分析显著性图（saliency maps），结合面部区域和时间表达语义，生成视觉和量化解释。

Result: 提出的视频建模方法在抑郁症视觉诊断上超越了此前基于单帧面部图像的基准，预测性能提升；同时成功生成了模型决策的可解释性分析。

Conclusion: 研究成功开发了一个既能从面部模型决策中生成假设，又能提高抑郁症预测能力的可解释框架。
...
TL;DR: 提出C²RM-Seg框架，通过因果反事实推理模块生成形态对齐的CAM伪标签，结合双路径结构-语义架构和不确定性门控损失，在组织病理图像分割上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督组织病理分割中，CAM生成的伪标签受染色驱动，关注表面外观而非真正有因果关系的组织形态，导致定位不准确、结构一致性差。

Method: 1) 分类阶段：C²RM模块，通过因果结构矩阵分解潜在因子并进行反事实干预，抑制混杂上下文，生成形态对齐的CAM。2) 分割阶段：双路径结构-语义架构，融合ResNeSt细粒度结构与DINOV3全局语义先验，交叉路径门控机制保持边界保真度。3) UGM损失：基于预测不确定性动态平衡边界约束与置信度学习。

Result: 在两个公开组织病理数据集上达到SOTA性能，有效提升了分割精度和结构一致性。

Conclusion: 因果推理能有效改善弱监督分割中伪标签噪声问题，结构-语义融合与不确定性损失进一步提升了分割质量。
...
### [50] [Steering Vision-Language Models with Joint Sparse Autoencoders](https://arxiv.org/abs/2606.25657)
*Huizhen Shu,Xuying Li,Hongxu Lin,Wenjie Sun,Hui Li*

Main category: cs.CV

TL;DR: 本文提出联合稀疏自编码器（JSAE），通过显式对齐约束联合分解视觉和语言激活，恢复可解释的跨模态特征。在LLaVA等三个VLM上的双向干预实验表明，加性引导在中后期层达到峰值，而抑制在各层范围内表现一致，支持更可控的多模态特征干预分析。


<details>
  <summary>Details</summary>
Motivation: 已有的稀疏自编码器（SAE）在分析语言模型上有效，但应用于视觉-语言模型（VLM）时，得到的表示难以用作可控的跨模态引导方向。因此需要一种能显式对齐视觉与语言表征、恢复可解释跨模态特征的方法。

Method: 提出联合稀疏自编码器（JSAE），使用显式对齐约束，对序列池化后的视觉和语言激活进行联合分解，得到共享的、可解释的图像/字幕级特征。在LLaVA等三个VLM上通过双向干预（加性引导和抑制）评估特征的可控性。

Result: JSAE恢复了食物、动物等可识别概念的跨模态特征。加性引导在中后期（输出前）层达到峰值，两端减弱；抑制得分在各层间相对一致。三个不同的VLM（LLaVA-v1.6-Mistral-7B, Llama3-LLaVA-8B, Qwen3-VL-30B）上观察到类似的层局部化效应。

Conclusion: 显式对齐的稀疏表示比无约束的替代方法更能支持在特定层范围内进行可控的多模态特征干预分析。
...
TL;DR: 本文提出了一种基于块的隐式神经表示方法（Patch-based INR），通过将非重叠块作为处理单元一次预测整个像素块，减少了推理次数。同时设计了FPGA硬件加速架构，支持可配置流水线和双精度计算。实验表明，2×2块尺寸下PSNR达34.97 dB，推理延迟降低75%，参数仅增加0.6%。


<details>
  <summary>Details</summary>
Motivation: 传统隐式神经表示（INR）逐像素推理会导致推理次数随分辨率呈二次方增长，在高分辨率场景下计算成本急剧增加。为了解决这一效率瓶颈问题，本文提出基于块的推理方法。

Method: 提出基于块的INR方法，将非重叠图像块作为基本处理单元，一次前向传播预测整个块的所有像素。同时设计FPGA硬件加速架构，包含可配置流水线和双精度计算支持。

Result: 2×2块尺寸下PSNR达34.97 dB（与逐像素INR相当），推理延迟降低75%，参数开销仅增加0.6%。

Conclusion: 基于块的INR在保持重建质量的同时，显著降低了推理延迟和计算成本，结合FPGA硬件加速方案，为高分辨率场景下的INR实用化提供了可行路径。
...
TL;DR: 本文提出ST-MoME，一种条件扩散框架，能在多模态MRI部分缺失的情况下，直接从图像空间合成3D DCE参数图，在脑肿瘤数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: DCE-MRI定量图对肿瘤评估重要，但因造影剂风险和协议差异常缺失；现有方法假设输入完整，无法处理实际中多模态MRI部分缺失的情况。

Method: 提出ST-MoME条件扩散框架，包含：1) 模态特定专家特征提取；2) 时空门控网络生成体素级、时间步依赖的权重，融合多模态特征形成条件张量；3) 在图像空间直接进行3D扩散，采用3D patch训练和Swin骨干网络。

Result: 在386例脑肿瘤数据集上，对16种模态缺失场景评估，ST-MoME在三种DCE参数(v_p, v_e, K^{trans})上获得最低平均NMSE，肿瘤区域重建误差最低。

Conclusion: ST-MoME有效解决了多模态缺失下DCE参数图合成问题，且学到的门控机制呈现"结构先融合、生理后融合"的临床一致调度模式。
...
### [53] [Point Cloud Diffusion with Global and Local Reconstruction for Instance-Level 3D Anomaly Detection](https://arxiv.org/abs/2606.25740)
*Linchun Wu,Qin Zou,Jiwen Lu,Qingquan Li*

Main category: cs.CV

TL;DR: 提出PCDiff框架，利用扩散模型进行3D点云异常生成与检测，通过多模态注意力条件生成弱缺陷，并引入局部-全局联合重建算法提升检测精度。


<details>
  <summary>Details</summary>
Motivation: 现有3D点云异常检测方法面临两个挑战：1) 弱缺陷区域（如划痕）在点云中偏差极小（低至10^{-3}），难以重建和检测；2) 背景非缺陷区域在重建中容易产生位置偏差，导致误检。

Method: 提出PCDiff框架，包含两个阶段：1) 生成阶段，在扩散生成框架中嵌入实例级多模态注意力，以纹理梯度、图像块、文本和掩码作为条件，生成高质量的弱缺陷异常；2) 检测阶段，引入联合局部-全局重建算法，实现局部异常恢复与全局几何一致性保持。

Result: 实验表明，PCDiff在3D异常生成保真度和重建质量上显著超越现有最优方法，大幅提升了异常检测精度。

Conclusion: 通过针对弱缺陷的实例级多模态条件生成和联合局部-全局重建策略，有效解决了弱缺陷检测难和背景误检两个核心问题，验证了扩散模型在3D点云异常检测中的潜力。
...
TL;DR: 针对微表情识别中运动放大与光流两种模态的不对称失败模式，提出SAC²-Net，通过语义锚定软对齐减少跨模态异质性，再通过互补共识融合进行可靠性感知融合，在五个基准上达到SOTA或竞争力水平。


<details>
  <summary>Details</summary>
Motivation: 微表情识别面临面部运动微妙、数据有限、AU与情感类别关系模糊三大挑战。现有工作使用光流（捕捉局部位移）和运动放大（放大微弱外观变化）两种模态，但两者常呈现不对称失败模式：一种模态变得噪声大或不具信息量，而另一种仍保留判别性AU证据。这种互补性带来了两个关键融合挑战：跨模态异质性和空间变化的模态可靠性。

Method: 提出SAC²-Net，包含两大模块：1）语义锚定软对齐（SASA）：将激活的AU转化为文本提示作为稳定语义锚点，对齐运动放大和光流表示；构建层次化AU感知软标签，保留样本间语义邻近性。2）互补共识融合（CCF）：先通过互补交换修复局部不可靠证据，再通过共识精炼增强共享空间关注。

Result: 在五个微表情识别基准上，SAC²-Net在粗粒度、细粒度、大规模和跨数据集评估设置下均达到SOTA或极具竞争力的性能。

Conclusion: 利用语义锚定对齐和可靠性感知融合，有效解决了微表情识别中运动放大与光流模态的互补性挑战，验证了跨模态融合策略的有效性。
...
### [55] [Edges Before Embeddings: A Confidence-Aware Blur Gate for Vision-Language Pipelines](https://arxiv.org/abs/2606.25838)
*Duy Tran Thanh*

Main category: cs.CV

TL;DR: 提出 MagikaDocumentFromPixel：一个轻量级（7ms）、CPU友好的图像质量门控，用于分类图像清晰度（清晰/模糊/不确定），在384px分辨率下使用MobileNetV3-Large+边缘先验模块（EPM）达到F1=0.9803，模型仅17MB。


<details>
  <summary>Details</summary>
Motivation: 生产环境中的视觉管线在模糊输入上 silently degrade（静默退化），浪费下游OCR、检索和VLM调用的计算资源，且无法恢复可用输出。

Method: (i) 从46种配置、8次扫描的实证搜索中选出最优配方，发现输入分辨率是主导杠杆，架构容量仅在>=384px时有收益；(ii) 基于经典选择性预测的置信感知路由形式化；(iii) 边缘先验模块（EPM），通过拉普拉斯幅值辅助输入通道直接提供谱证据，提升F1 +1.3点；(iv) 识别该门控为Magika内容检测、风险控制OCR/DocVLM中反复出现的设计模式。最终配方：MobileNetV3-Large + EPM，384x384训练，5尺度测试时增强。

Result: F1=0.9803 (AUC 0.9989)，17MB ONNX模型，相比固定尺度基线（F1=0.9672）提升+1.31点。

Conclusion: 提出轻量有效的图像质量门控，但局限包括：单一运动模糊分布、单次随机种子结果、校准为定性而非定量。
...
TL;DR: TensorLDM是一种成分级潜在扩散模型，通过两组特定编码器（对角线和非对角线元素）、解剖条件自编码器、跨组件注意力机制和混合专家DWI条件器，从稀疏DWI中重建扩散张量，在HCP数据集上实现了高物理有效性和解剖一致性。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习方法从稀疏DWI重建扩散张量时常产生解剖不一致或物理不可行的张量，亟需一种能同时保证解剖一致性和物理有效性的重建方法。

Method: 提出TensorLDM，包含：1) 成分级潜在扩散模型，用两个组特定编码器分别处理对角和非对角线张量分量；2) 解剖条件自编码器，促使潜在表示关注张量属性而非结构信息；3) 跨组件注意力机制，建模分量间依赖关系；4) 混合专家DWI条件器，提供分量自适应条件。

Result: 在HCP单壳层四体积稀疏采集下，TensorLDM生成最准确的纤维追踪结果，SPD违规率1.54%（接近真实值1.40%），体素级重建精度最佳或可比，对数欧几里得度量的测地线张量误差也证实了性能提升。

Conclusion: TensorLDM通过成分级建模和多种机制协同，在稀疏DWI重建中同时实现了高物理有效性（接近真实值）和解剖一致性，为临床加速DTI提供了可行方案。
...
### [57] [Enhancing Brain MRI Anomaly Detection and Reasoning with ROI Rethink and Synthetic Data](https://arxiv.org/abs/2606.25894)
*Shangkun Li,Jie Xu,Yi Guo,Zeju Li,Yuanyuan Wang*

Main category: cs.CV

TL;DR: BrReMark是一个面向脑部MRI诊断的框架，通过显式区域标记（ROI标注）实现假设生成→定位标记→验证确认的闭环推理，结合强化学习与数据增强，显著提升诊断定位精度（mAP50从0.74%升至37.54%）并减少幻觉（OOD假阳性降低45.7%）。


<details>
  <summary>Details</summary>
Motivation: 现有医学视觉语言模型通常进行单次推理生成诊断，缺乏空间定位依据，导致输出不可审计，且模型可能在正常扫描上产生幻觉，限制了临床实用性。

Method: 提出BrReMark框架：1）假设生成阶段预测潜在异常；2）通过显式边界框标记进行空间定位；3）重新检查标记证据验证结论。训练结合结构化推理轨迹的监督微调与基于复合奖励（定位精度+诊断推理）的强化学习，并引入基于域随机的病理合成增强策略提升OOD泛化能力。

Result: 内部基准：mAP0从0.74%提升至37.54%，Clinical F1达21.57%，诊断准确率45.26%。NOVA OOD基准：相比SOTA假阳性减少45.7%，表现竞争力强。

Conclusion: 显式的假设-验证定位机制是实现可信赖的开放脑部MRI诊断的可行路径，在分布内和OOD场景下均有效。
...
TL;DR: 本文提出一个面向3D CT影像的视觉-语言预训练框架，包含CNN-ViT混合编码器、疾病级对比学习和诊断感知提示策略，在CT-RATE和Rad-ChestCT数据集上取得SOTA性能（AUC分别提升5.1%和5.4%），并在60类疾病基准上实现+9.8%的显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有VLP方法在处理3D CT影像时面临两大问题：视觉主干网络效率低下（无法高效捕捉局部解剖细节）以及语义对齐粗糙（难以在同一解剖区域内区分不同疾病）。

Method: (1) CNN-ViT混合编码器：用3D CNN主干替代ViT的patch嵌入，兼顾局部细节捕获与全局注意力；(2) 疾病级对比学习：通过可学习查询令牌从完整报告中动态提取疾病特定语义，并与对应视觉特征对齐；(3) 诊断感知提示策略：使用真实临床短语和聚合疾病原型，缩小预训练与推理之间的差距。

Result: CT-RATE上AUC达84.4%（+5.1%），Rad-ChestCT上AUC达75.4%（+5.4%），60类疾病基准上提升+9.8%；同时表现出良好的报告生成迁移能力。

Conclusion: 该框架通过三项针对性设计有效解决了3D CT场景下VLP的核心瓶颈，验证了方法的通用性和临床实用性。
...
### [59] [Pulmonary Embolism Risk Stratification from CTPA and Medical Records: Vascular Graphs Are Not All You Need](https://arxiv.org/abs/2606.25956)
...
TL;DR: 该研究探讨了在缺少血液检查结果的情况下，仅基于医疗记录和CTPA影像生物标志物进行肺栓塞风险分层的可能性。研究发现医疗记录和心脏生物标志物是最重要的预测因子，而血管生物标志物（包括图神经网络处理的血管树信息）未能提升分层效果，甚至可能不具备判别信息。


<details>
  <summary>Details</summary>
Motivation: 肺栓塞风险分层对临床决策至关重要，但常规实践中血液检查数据常常缺失。研究希望验证是否可以在没有血液检查的情况下，仅依靠医疗记录和CTPA影像特征准确进行风险分层。

Method: 使用私有数据集（n=353），对比多种方法：将血管生物标志物加入表格模型，以及在血管树的图表示上应用图神经网络（GNN），并与仅使用全局特征（医疗记录和心脏生物标志物）的强表格基线进行比较。

Result: 医疗记录和心脏生物标志物是最重要的预测因子，血管生物标志物未能进一步改善分层效果；即使GNN处理血管图结构，也不优于基于全局特征的表格基线模型。

Conclusion: 血管图结构可能对肺栓塞风险分层不具备判别性信息，这与直觉相反。研究对这一反直觉结果从模型和数据两方面提出了假设分析。
...
TL;DR: 提出了一种名为CrosInv的跨尺度可逆隐藏网络，通过空间-频率协作和非可逆机制来提升图像隐藏质量，解决了现有可逆神经网络（INN）方法在单尺度、单域特征提取和非线性表示能力上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的基于可逆神经网络（INN）的图像隐藏方法受限于架构缺陷，存在单尺度和单域特征提取、非线性表示能力不足的问题，导致隐藏图像质量较差。

Method: 提出CrosInv网络，包含：1）跨尺度可逆模块，利用像素重排和Haar小波变换实现双向跨尺度映射；2）非可逆跨密集模块，增强非线性表示能力；3）空间-频率域协同特征提取。

Result: 综合实验验证了CrosInv方法的有效性和优越性，在图像隐藏质量上优于现有方法。

Conclusion: 跨尺度可逆隐藏网络结合空间-频率协作与非可逆机制，能够有效提升图像隐藏的视觉质量，为INN图像隐藏提供了新思路。
...
TL;DR: 本文提出一种通过替换现代图像生成模型（如SD3.5、Flux、Infinity）的瓶颈层为具有结构化激活特征的转码器，实现对特定视觉概念的选择性禁用，达到高效、持久且不影响生成质量的概念移除效果。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型基于大规模未筛选互联网数据训练，包含不良视觉概念。高效移除这些概念同时保持生成质量仍具挑战。

Method: 将现代模型（扩散模型和自回归模型）内部的瓶颈层替换为转码器（transcoder），该转码器在复制原层功能的同时将激活特征结构化，从而实现对特定概念信号的选择性禁用。

Result: 在最新扩散模型和自回归模型上实现了最先进的概念移除性能，保持了视觉生成质量，对对抗性提示具有鲁棒性，并支持顺序移除多个不同概念。

Conclusion: 该方法为前沿图像生成模型中的概念移除提供了一种实用方案。
...
### [62] [TriViewBench: Controlled Complexity Scaling for Multi-View Structural Reasoning in MLLMs](https://arxiv.org/abs/2606.26029)
*Yu-Yang Chen,Lan-Zhe Guo*

Main category: cs.CV

TL;DR: 本文提出 TriViewBench，一个基于合成3D场景的三视角视觉推理基准，系统评估了18个多模态大语言模型。所有模型都表现出相同的能力层级（局部决策 > 物体计数 > 全局恢复），且性能随复杂度单调下降，其中链式思维提示几乎无效，表明瓶颈在于跨视角空间表示而非推理策略。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在标准VQA基准上表现良好，但其在受控结构复杂度下的可扩展性尚不明确。作者希望系统性地探究模型在结构化视觉推理任务中的能力边界和失败模式。

Method: 构建TriViewBench基准，包含1,923个合成3D场景和超过14K的QA对，分为4个复杂度级别和3个推理类别（局部决策、物体计数、全局恢复）。在统一提示协议下评估18个开源和闭源MLLM。

Result: 所有18个模型表现出相同的能力层级：局部决策 > 物体计数 > 全局恢复。性能随复杂度单调下降：局部决策下降12.11%，物体计数下降59.14%，全局恢复下降80.02%。错误分析发现单视图任务主导欠计数（遮挡盲视），多视图任务主导过计数（跨视图身份混淆）。CoT提示总体收益接近零（-0.16%）。

Conclusion: 当前MLLM存在根本性的可扩展性限制，瓶颈在于跨视角空间表示能力而非推理策略。TriViewBench可作为分析结构推理失败的控制诊断框架。
...
### [63] [H-Adapter: Pose-Robust Hairstyle Transfer via Attention-Derived, Source-Aligned Hair Masks](https://arxiv.org/abs/2606.25578)
*Seulgi Jeong,Yunseong Cho,Sanghun Park*

Main category: cs.CV

TL;DR: 提出H-Adapter，通过区域特定损失和空间解耦交叉注意力实现鲁棒的发型迁移，在大姿态差异下仍保持高质量效果，并支持文本生成、颜色控制等多种扩展。


<details>
  <summary>Details</summary>
Motivation: 现有发型迁移方法在源图像和参考图像存在较大头部姿态差异时效果不佳，缺乏姿态鲁棒性。

Method: 1) 区域特定损失训练，解耦头发与非头发目标；2) 空间解耦交叉注意力，生成源对齐的发型编辑掩码；3) 基于扩散模型的掩码引导修复。

Result: 在姿态无关和姿态差异子集上取得最优FID、FID_CLIP、CLIP-I指标，保持非头发区域保真度，提升细粒度发型细节质量。

Conclusion: H-Adapter在发型迁移任务中有效提升姿态鲁棒性，且支持T2I生成、辅助提示颜色控制以及与IP-Adapter兼容等扩展应用。
...
### [64] [A welding penetration prediction model for laser welding process based on self-supervised learning using physics-informed neural networks](https://arxiv.org/abs/2606.26059)
*Sen Li,Xiaoying Liu,Xiaojian Xu,Chendong Shao,Yaqi Wang,Ling Lan,Xinhua Tang,Haichao Cui*

Main category: cs.CV

TL;DR: SimPhysNet是一种结合物理先验的自监督学习算法，仅用200张标注图像（约5%数据）即可实现96.06%的激光焊接穿透状态分类准确率，达到全监督学习的性能水平。


<details>
  <summary>Details</summary>
Motivation: 激光焊接全穿透状态是获得无缺陷焊缝的关键因素，准确预测穿透状态对保证焊接质量至关重要。然而，传统监督学习方法依赖大量高质量标注数据，这在工业应用中受到严重限制。

Method: SimPhysNet的核心是嵌入物理先验的自监督学习范式：1) 利用物理信息神经网络(PINN)从大量无标注数据中提取熔池和匙孔的物理意义特征；2) 三种图像增强任务提升泛化能力；3) 基于原型网络的少样本学习策略，通过少量标注图像构建类别表征实现鲁棒分类。

Result: 仅用200张标注图像（约总标注数据集的5%）即可达到96.06%的分类准确率，性能与使用全部标注数据的传统监督学习算法相当。

Conclusion: 该工作提供了一种新型、高效且高精度的激光焊接穿透状态预测方法，为激光焊接的智能化自动化铺平了道路。
...
### [65] [FeVOS: Foresight Expression Video Object Segmentation](https://arxiv.org/abs/2606.25585)
*Kehan Lan,Kaining Ying,Henghui Ding*

Main category: cs.CV

TL;DR: 本文提出了面向预判推理的视频目标分割新任务FeVOS，通过预测未来事件中相关物体的掩码来回答关于未来动作的问题，并构建了包含968个视频片段和14,525条前瞻性表达的数据集，以及基于MLLM的FeVOS-R1模型，在监督微调和强化学习两阶段训练下达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有Referring Video Object Segmentation任务仅关注描述已观测帧中事件、动作或外观的表达，缺乏需要预判性时空推理的场景评估，限制了其在实际应用中的适用性。

Method: 提出了Foresight Expression Video Object Segmentation (FeVOS)任务；构建了包含968个视频片段、14,525条前瞻性表达和2,904条思维链注释的数据集；开发了FeVOS-R1模型，采用两阶段训练策略：监督微调+强化学习。

Result: FeVOS-R1在FeVOS数据集上达到SOTA性能，并在现有RVOS基准测试上展现出强泛化能力。

Conclusion: 该工作为视频感知中的预判推理研究提供了新方向，有望启发更多关于预测性推理的视频感知研究。
...
### [66] [A cross-process welding penetration status prediction algorithm based on unsupervised domain adaptation in laser and TIG welding](https://arxiv.org/abs/2606.26078)
*Sen Li,Haichao Cui,Chendong Shao,Yaqi Wang,Xinhua Tang*

Main category: cs.CV

TL;DR: 提出了一种结合渐进源域扩展策略的无监督域适应框架，有效解决了焊接过程域转移（如TIG焊与激光焊之间）导致的深度学习性能下降问题，在跨过程迁移任务中准确率提升超40%。


<details>
  <summary>Details</summary>
Motivation: 有监督深度学习在焊接熔透状态分类中性能优异，但在不同焊接工艺（如TIG焊与激光焊）之间进行模型迁移时，由于物理机制差异导致的域偏移会显著降低分类精度。为降低新焊接工艺的重新标注成本并提升智能监测系统的通用性，亟需一种无需目标域标签的域适应方法。

Method: 提出无监督域适应框架，结合渐进源域扩展策略，在无需目标域标签的情况下实现跨工艺迁移。在TIG和激光焊接数据集上进行评估，并采用UMAP可视化验证特征学习效果。

Result: 同工艺：TIGFH达90.65%，LSPS达90.72%，分别超越有监督基线35.83%和38.87%。跨工艺：TIG→激光达80.48%，激光→TIG达81.13%，分别超越基线43.39%和43.40%。

Conclusion: 该方法显著降低了新焊接工艺的重新标注成本，增强了智能监测系统在不同焊接工艺间的通用性和实用性。
...
### [67] [VPA-Guard: Defending and Benchmarking Image-to-Video Generation Against Visual Prompt Attacks](https://arxiv.org/abs/2606.25592)
...
TL;DR: 本文提出VVA-Bench，首个系统评估图像到视频生成模型在面对视觉提示攻击（如箭头、涂鸦、表情符号）时的安全性的基准。研究发现现有模型极易受此类攻击（攻击成功率最高达100%），并提出VPA-Guard防御框架，将攻击成功率平均降低44.2%，危害评分降低73.4%。


<details>
  <summary>Details</summary>
Motivation: 现有安全基准主要关注文本攻击和纯图像攻击，忽略了图像到视频生成中隐式的视觉提示攻击风险——看似无害的静态视觉线索（如箭头、涂鸦）可能被模型解读为可执行的时序指令，生成有害视频。这一威胁尚未被系统研究。

Method: 1）构建VVA-Bench：首个系统化的视觉提示攻击分类基准；2）提出VPA-Guard：基于检索增强和自演化的防御框架，利用小样本推理识别潜在恶意意图。

Result: SOTA模型对视觉提示攻击高度脆弱：Wan 2.7攻击成功率100.0%，Veo 3.1达74.8%。VPA-Guard防御后，攻击成功率平均降低44.2%，危害评分降低73.4%，同时保持对合法编辑的可用性。

Conclusion: 本文填补了视觉提示攻击在图像到视频生成安全领域的空白，提供了系统性基准与有效防御策略，推动了多模态生成的安全与负责任发展。
...
TL;DR: ScaleHP是一个端到端的一阶段手部姿态估计框架，利用手骨内在比例关系作为生物先验，通过Transformer解码器和尺度令牌融合多尺度特征，并结合透视约束最小二乘法恢复公制空间中的绝对手部姿态，在多个基准数据集上取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 现有手部姿态估计方法大多预测根相对坐标系下的姿态，无法估计公制尺度下的绝对手部位置，而依赖外部深度模块的方法又较为脆弱。

Method: 利用手骨内在比例关系编码的稳定人类测量先验，设计基于Transformer的解码器并引入新颖的尺度令牌融合多尺度形态与外观特征，通过透视约束最小二乘法求解公制坐标。

Result: 在FreiHand上达到35.8 CS-MPJPE，在DexYCB和HO3Dv3上分别达到4.6/5.9 PA-MPJPE。

Conclusion: 内部生物约束能显著减少相对几何误差和绝对公制误差，为广义真实世界手部追踪提供了鲁棒解决方案。
...
TL;DR: SSMNBench是一个用于诊断多模态大模型跨视角理解能力的基准测试，包含3300个QA对，通过区分单视角充分性（SVS）和多视角必要性（MVN）任务，揭示了MLLMs存在"干扰退化"和无法整合跨视角几何证据等根本缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有跨视角基准采用固定的"帧袋"评估方式，混淆了模型对视觉干扰的鲁棒性与真正的跨视角证据融合能力，亟需一个能诊断模型真实跨视角推理能力的基准。

Method: 构建SSMNBench基准，包含3300个精心设计的QA对，将任务分为单视角充分性（SVS）和多视角必要性（MVN）两类，通过系统性地扰动视角可用性来评估17个SOTA多模态大模型。

Result: MLLMs在冗余视角下存在严重的"干扰退化"（SVS任务），无法整合跨相机的碎片化几何证据（MVN任务），依赖多张单图像语义平均和视角偏好而非真正的跨视角合成。

Conclusion: SSMNBench暴露了现有MLLMs在跨视角理解方面的根本脆弱性，为未来跨视角感知多模态架构的发展提供了严格的诊断框架。
...
TL;DR: 本文基于FUSE-Bike平台构建了1,027帧标注点云数据的3D目标检测基准，评估了预训练检测器在自行车视角下的零样本表现，并验证了自动标注管线作为人工标注替代方案的有效性，微调后mAP提升最高达23.4点。


<details>
  <summary>Details</summary>
Motivation: 自行车等弱势道路参与者（VRU）的可靠3D感知对其安全至关重要，但目前缺乏从自行车视角采集的标注3D数据，车辆训练的检测器在自行车平台上的泛化能力未知。

Method: 构建包含1,027帧LiDAR关键帧（超过18,000个3D边界框）的FUSE-Bike基准，采用nuScenes预训练的四种检测器，对比零样本与基于VRU自动标注管线微调后的性能，自动标注无需人工标注。

Result: 零样本域差距主要集中在VRU类别；微调后mAP最高提升23.4点，行人和骑行者的提升最大；适配后的检测器甚至超越了训练所用的自动标注质量。

Conclusion: 该基准为以VRU为中心的3D检测提供了可复现的基线，表明自动标注是车辆训练检测器适配自行车平台时人工标注的可行替代方案。
...
TL;DR: 本文提出CausalMem，一种无需训练的动态固定预算记忆库方法，通过在线语义基元估计视觉标记冗余并更新记忆，显著提升多模态大模型在流式视频理解中的表现，实现20倍以上视觉标记压缩比。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs难以处理流式视频中不断增长的帧数、未来内容的不可预测性以及输入指令的不确定性，亟需一种高效且动态的视觉记忆机制。

Method: 提出训练无关的CausalMem方法，构建动态固定预算记忆库，通过在线语义基元估计视觉标记冗余度，实现类人脑的视觉记忆动态更新机制。

Result: 在流式设置下平均准确率提升3.2%，离线设置下提升3.0%；仅用12k标记预算即可记忆小时级流式视频，实现20倍以上视觉标记压缩比，仅占用约82MB存储。

Conclusion: CausalMem在流式与离线视频理解任务中均显著优于现有方法，且具备优秀的语义保留能力和极高的压缩效率。
...
TL;DR: 本文提出Falcon框架，通过结构化安全状态表征（组件存在性、成对功能兼容性、场景级风险）来建模X光行李安检中分散组件的组合威胁推理，并构建Falcon-X基准，实验表明现有多模态模型难以进行组合安全推理，而Falcon显著提升了功能接地与威胁评估的一致性。


<details>
  <summary>Details</summary>
Motivation: 传统视觉-语言模型以物体为中心，仅关注独立个体的检测与描述，但在X光行李安检等安全关键场景中，威胁往往源自空间分散组件（如电池、雷管、炸药）之间的功能兼容性，而非单一物体。作者旨在填补这一组合威胁推理能力的空白。

Method: 提出Falcon框架，将分割感知的区域特征抽象为结构化安全状态（组件存在性、成对功能兼容性、场景级风险），并将该结构化表示作为显式中间接口注入语言模型，以促进关系一致且安全感知的推理。同时构建Falcon-X基准，提供密集接地标注与结构化监督。

Result: 实验表明现有多模态模型虽能适应外观特征，但在组合安全推理上表现不佳；Falcon显著改善了功能接地能力，并产生更连贯的威胁评估结果。

Conclusion: 组合安全推理应被确立为多模态系统的一个独立评估范式，Falcon在该范式下展现出有效性。
...
TL;DR: 提出了一种统一的多视角EEG表示学习框架，通过联合建模时间动态、频谱分解和电极图结构，实现零样本视觉语义解码，在THINGS-EEG基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有EEG-视觉对齐方法多依赖整体EEG嵌入，忽略了时间、频谱和空间结构中的互补信息，导致语义对齐和泛化能力受限。

Method: 构建EEG编码器，联合建模三个互补视角：基于状态空间的时间动态、可学习小波频谱分解、注意力图学习电极交互；通过对比学习将多视角嵌入与预训练视觉表示对齐。

Result: THINGS-EEG上：被试内Top-1 54.8%、Top-5 85.6%；跨被试Top-1 15.3%、Top-5 45.4%；首次系统跨会话评估Top-1 40.8%、Top-5 78.0%。

Conclusion: 显式建模多视角神经结构能有效提升EEG视觉解码的语义对齐和泛化能力。
...
TL;DR: 提出PGL-Net，一种受物理启发的全局-局部解耦轻量级去雾网络，通过算子级模拟替代显式参数估计，在多个真实基准上以10倍以上推理速度提升实现SOTA去雾质量（PSNR提升最高2.6dB）。


<details>
  <summary>Details</summary>
Motivation: 真实去雾问题高度不适定（空间/光谱变化散射），现有方法要么依赖简化假设下的脆弱物理逆推，要么采用不适合边缘部署的重型黑盒架构，亟需兼顾物理可解释性与轻量高效的设计。

Method: 提出物理启发的全局-局部解耦网络（PGL-Net）：1）物理启发仿射融合（PAF）模块，在层次跳跃连接上执行全局条件对齐补偿雾霾偏差；2）退化感知调制（DAM）模块，通过动态特征调制自适应恢复空间和光谱变化的细节。

Result: 在多个真实基准上达到SOTA；相比SGDN，Tiny变体PSNR提升2.6dB，下游目标检测精度一致提升，推理延迟降低10倍以上。

Conclusion: PGL-Net通过物理启发的轻量级设计，在无需显式参数估计的前提下，以极低延迟实现了最先进的去雾性能和下游任务增益，适合边缘部署。
...
TL;DR: UniTeD提出统一时序扩散框架，将感知与规划在共享生成空间中联合建模，通过双向信息交换和噪声条件多任务训练提升鲁棒性，并引入时序过渡模块和锚点刷新策略解决噪声匹配与分布偏移问题，在多个基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有扩散规划方法将扩散模型局限在规划模块，依赖判别式感知网络的固定输出，这种解耦设计导致感知误差传播到规划器，增加优化难度并降低鲁棒性。

Method: 1) 提出UniTeD统一时序扩散框架，在共享生成空间中通过迭代去噪联合建模感知与规划；2) 引入时序过渡模块(TTM)解决历史帧与当前帧的噪声级别不匹配；3) 提出锚点刷新策略(ARS)缓解稀疏扩散端到端驾驶框架中的训练-推理分布偏移。

Result: 在多个基准上超越最新的判别式端到端方法和基于扩散的规划方法，达到SOTA性能。

Conclusion: 统一扩散范式能有效实现感知与规划的联合建模与双向信息交换，结合时序建模和分布偏移缓解策略，可显著提升端到端自动驾驶的规划性能与鲁棒性。
...
TL;DR: 提出DDE框架，通过无训练的高斯分布建模解决Noisy TTA中OOD异常值问题，同时提升ID分类准确率和OOD检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有零样本NTTA方法依赖测试时判别训练，易产生过自信误分类且推理效率低下，亟需一种无需训练、鲁棒且高效的在线OOD过滤方案。

Method: DDE包含两个模块：(1) PFDE显式建模类别级包含与排斥高斯分布，生成校准对比分数增强ID准确性；(2) NLDE建模负标签分布以挖掘高判别力标签，缓解伪相关。

Result: 在ImageNet基准上，调和平均准确率提升3.70%，OOD检测FPR95降低6.20%，且推理高效可扩展。

Conclusion: DDE以零样本、无训练的方式在数据稀缺场景下展现出色鲁棒性，实现了高效率和强性能的平衡。
...
TL;DR: 本文提出CaptureGuide-Bench基准，包含摄影师构图决策与主体姿态推荐两项任务，并构建130K样本的CaptureGuide-Dataset，开发统一MLLM模型ShutterMuse，在拍摄时交互辅助方面取得最佳综合性能。


<details>
  <summary>Details</summary>
Motivation: 现有美学裁剪基准主要评估事后裁剪预测，忽视了拍摄时对摄影师构图和主体姿态的引导能力，而多模态大语言模型在这方面的能力尚未被充分探索。

Method: 提出CaptureGuide-Bench（含构图决策/精炼和姿态推荐两项任务），构建CaptureGuide-Dataset（130K样本含文本理由与结构化视觉标注），开发ShutterMuse模型，采用监督微调+强化微调统一训练。

Result: ShutterMuse在摄影师侧性能上超越所有基线，主体侧姿态推荐达到竞争性表现且推理成本显著更低。

Conclusion: MLLMs具备成为拍摄时交互式摄影助手的潜力，能同时提供构图指导和姿态推荐。
...
TL;DR: 本文提出 S²-FracMix 统一框架，通过自显著混合（Self-Saliency Mixup）和分形混合（FracMix）两种策略，在不引入跨样本干扰的情况下构建标签一致的挑战性增强样本，同时促进尺度不变特征学习和模型鲁棒性，在分类、鲁棒性、校准、检测和迁移学习等七项基准任务上取得最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有混合（mixup）数据增强方法存在两大问题：一是计算开销大，二是跨样本混合导致增强数据的语义破坏（semantic disruption）。作者希望设计一种既能避免跨样本干扰、又能构建有挑战性样本的数据增强方法。

Method: 提出 S²-FracMix 统一框架，包含两部分：1) Self-Saliency (S²) Mixup：提取多尺度显著性补丁并重新插入到同一图像的非显著区域，避免跨样本混合，促进尺度不变特征学习；2) FracMix：利用自适应比例将自相似模式注入显著区域，增强模型鲁棒性。两者结合使模型能同时学习单张图像内的分形与非分形结构。

Result: 在七个基准测试（涵盖粗粒度/细粒度分类、鲁棒性、校准、目标检测、迁移学习）上取得 state-of-the-art 性能，证明了该方法相较于现有数据增强技术的优越性。

Conclusion: S²-FracMix 提供了一种目标明确且结构连贯的样本内增强策略，理论分析和实验结果均表明其在多种视觉任务中优于现有方法。
...
TL;DR: 本文提出了一种基于奇异值分解（SVD）的框架，用于恢复由空变运动模糊造成的图像退化。该方法通过奇异值能量保留准则选择小奇异值的数量，在控制噪声放大的同时保留图像信息。实验在三种一维空变运动PSF（双向线性运动、高斯运动和简谐运动）下验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 实际成像系统中PSF随图像场变化（如运动、像差、湍流等），导致空变退化无法用单一卷积核表示，传统空不变方法难以处理这一挑战。

Method: 将空变退化建模为线性系统，使用SVD分析并求逆对应的退化算子；基于奇异值能量累积百分比（能量保留准则）自适应选择截断的小奇异值数量，以平衡噪声放大与信息保留。

Result: 在三种空变运动PSF（双向线性、高斯、简谐运动）的退化图像上进行了实验，结果表明该方法能有效恢复图像细节、减少模糊伪影。

Conclusion: 基于SVD的空变图像恢复框架能系统性地控制噪声放大，为空变运动模糊的复原提供了一种稳定且有效的解决方案。
...
TL;DR: 针对多模态大模型(MLLM)在长视频处理中因输入令牌限制而严重丢失时序信息的问题，本文提出基于自我中心场景图(EgoSGs)的框架，通过紧凑的文本化场景图结构在保留丰富语义的同时大幅压缩输入长度，在HD-EPIC VQA数据集上取得了最优结果。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在处理长视频（尤其是自我中心视频，具有复杂动态、频繁状态变化和移动相机等特点）时，因严格的输入令牌限制不得不大量下采样帧，导致严重丢失时序和上下文信息，限制了细粒度视频推理能力。

Method: 提出自我中心场景图(Egocentric Scene Graphs, EgoSGs)——一种时序 grounding 的结构化表示，捕捉对象、属性、空间关系及随时间变化的交互。将视频表示为紧凑的文本化场景图，大幅减少输入长度，同时保持语义丰富性，使MLLM能在令牌预算内高效推理整个视频序列。

Result: 在HD-EPIC VQA数据集上达到最先进结果，超越多个强视频基线模型。

Conclusion: 结构化、时序 grounding 的表示如EgoSGs能够弥合长形式自我中心视频理解与当前MLLM上下文限制之间的鸿沟。
...
TL;DR: 本文将一维时间序列编码为图像后，探究编码图像的视觉自然度（FID）与冻结视觉主干迁移准确率之间的关系。通过构建WorldStream数据集（299个时序流，9类源识别任务），发现FID与准确率呈显著负相关（ρ=-0.72）。但通过控制实验（调整频谱指数β和相位打乱）证明，这种相关性是由图像中的局部结构（而非频谱自然度）介导的。即使全微调也无法弥补与结构化基线的差距，且编码器可无损恢复原始信号。


<details>
  <summary>Details</summary>
Motivation: 现有方法常将一维信号转换为图像以复用视觉预训练模型，但很少分析编码图像本身的特性及其对迁移性能的影响。本文旨在探究编码图像的视觉自然度（与自然照片的相似程度）与冻结主干网络识别准确率之间的关系。

Method: 构建WorldStream数据集（9类源、299个时序流、3143个时间窗口）；使用7种编码方式和6种冻结视觉主干；用FID衡量编码图像与自然图像的Frechet距离；通过两项控制干预实验（调整频谱指数β和相位打乱）分离频谱自然度与局部结构的影响。

Result: FID与准确率呈强相关（ρ=-0.72）。但调整β时，FID在β≈2时最低，准确率却基本持平（19.2% vs 73.0%基线），相关性弱（Pearson -0.32）。相位打乱时，FID与准确率同步下降（Pearson -0.89）。全微调仅提升至27%，远低于67%基线。

Conclusion: 编码图像与自然图像的FID相关性并非由频谱自然度因果驱动，而是由局部结构介导——Inception网络（用于计算FID）与视觉主干读取了相同的局部结构。编码器可无损重建原始信号（72.9dB），兼具数据记录功能。
...
TL;DR: 本文提出USS框架，将具身视觉跟踪中的目标指示从纯文本范式转向统一的空间-语义提示（支持文本、点、边界框、掩码），通过隐式动态学习提升跟踪鲁棒性，在真实机器人和仿真实验中均取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有具身视觉跟踪（EVT）方法过度依赖语言描述来指示目标，但在复杂场景中多个物体可能符合同一语义描述，导致目标定位模糊不清。

Method: 提出USS框架：采用模态特定编码器处理异构提示，通过混合注意力机制将提示令牌与视觉特征融合，解码为以自我为中心的路点；并引入隐式世界模型通过自监督对齐预测未来表示以增强时间鲁棒性。

Result: 真实机器人实验中，显式空间目标线索相比纯文本提示获得更高成功率，尤其在存在相似干扰物和长时跟踪场景中。仿真基准上，USS在非MLLM方法中达到SOTA，且推理速度优于近期MLLM方法。

Conclusion: 空间-语义提示为具身视觉跟踪提供了更精确、更灵活的目标指示接口，是对传统纯文本范式的重要范式转变。
...
TL;DR: SurgAtlas是迄今为止最大的手术视频-语言数据集，包含15,291个视频（2,391小时），涵盖18个外科专科和5,000多种手术类型，首次大规模纳入开放手术数据，并提供了专家验证的子集和标准化基准。通过自动化多层级标注管道构建丰富的注释，微调Qwen3-VL-8B模型在多个手术基准上取得了竞争性或最优结果。


<details>
  <summary>Details</summary>
Motivation: 现有手术视频-语言数据集规模有限、缺乏开放手术数据且注释多样性不足，难以支持大规模手术基础模型的训练和临床推理能力的发展。

Method: 构建了包含15,291个YouTube视频的数据集，覆盖开放手术和微创手术；设计自动化多层级标注管道（含LLM增强和分段VQA生成框架）；采用两阶段（字幕预训练+指令微调）方式微调Qwen3-VL-8B。

Result: 微调后的Qwen3-VL-8B在阶段识别、三联检测和推理问答等多个手术基准上达到竞争性或最先进水平。

Conclusion: SurgAtlas凭借其规模、多样性和开放手术覆盖，为手术基础模型的大规模预训练提供了重要资源，推动了下一代多模态手术AI系统的发展。
...
TL;DR: 提出OracleAnalyser，一个基于后训练技术的甲骨文分析推理框架，通过多阶段微调Qwen2.5-VL-3B-Instruct和新提出的稳定焦点偏好优化算法(SFPO)，仅用3B参数就超越了更大规模的模型。


<details>
  <summary>Details</summary>
Motivation: 现有甲骨文研究方法主要集中在识别任务上，忽略了同样重要的甲骨文分析任务。作者旨在填补这一空白，构建一个专门针对甲骨文分析的推理框架。

Method: 1. 基于Qwen2.5-VL-3B-Instruct进行多阶段后训练微调；2. 提出Stable Focal Preference Optimization (SFPO)偏好优化算法；3. 构建甲骨文推理数据集和偏好数据集；4. 建立新的评估基准。

Result: OracleAnalyser仅用3B参数就在甲骨文分析任务上取得了显著效果，超越了参数量更大的模型。

Conclusion: 通过后训练技术和小模型也能在甲骨文分析任务上取得优异的性能，验证了所提框架的有效性。
...
TL;DR: 本文提出基于区域的拖拽编辑方法ICRDrag，利用上下文学习框架和两种注意力正则化，在配对的区域数据集上训练，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于点的拖拽编辑存在歧义性问题，本文旨在通过基于区域的拖拽编辑来解决这一局限，提供更明确、更可控的图像编辑方式。

Method: 在上下文学习框架中，输入源图像、源区域掩码和目标区域掩码。提出两种注意力正则化：1) 图像-掩码注意力一致性，确保目标区域在图像和掩码模态上关注相似的源区域；2) 源-目标注意力对应，确保源区域与目标区域的相互对应。同时构建了大规模配对区域数据集PRD。

Result: 大量实验表明，ICRDrag在定量指标和用户研究上均显著优于现有方法，实现了更高的编辑精度和视觉保真度。

Conclusion: 基于区域的拖拽编辑比基于点的拖拽更具优势，ICRDrag在上下文学习框架下通过注意力正则化有效提升了编辑质量和精度。
...
TL;DR: 本文首次提出像素级眼底图像质量评估基准FunPiQ和可解释方法EFIQA-CP，通过解剖可见性实现任务无关的可靠质量评估，实验表明可解释设计方法优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 现有眼底图像质量评估（FIQA）存在两个核心问题：(1) 图像级质量标签在不同临床任务间标准不一致，依赖专家知识；(2) 图像级标签无法量化局部退化，缺乏可解释性和可信度。因此需要一种任务无关、可解释的像素级评估方法。

Method: (1) 构建FunPiQ基准：首个提供像素级质量标注的FIQA数据集；(2) 提出EFIQA-CP方法：基于解剖可见性生成质量伪标签，利用非负正-无标记学习（Non-Negative PU Learning）训练CNN，实现可解释设计（Explainable-by-Design）。

Result: 在分类方法（+事后解释）、异常检测方法和可解释设计方法的广泛评估中，可解释设计方法整体表现更优，其中EFIQA-CP尤为突出。

Conclusion: 基于解剖可见性的像素级FIQA是更任务无关且可解释的评估方式；可解释设计方法在FIQA任务上显著优于传统分类加事后解释以及异常检测方法。
...
TL;DR: 本文针对智能手机异构双目相机导致的不对称模糊问题，提出了异构立体去模糊框架。包括构建了HSD真实数据集，以及轻量级PECA模块（物理与极线约束交叉注意力），通过物理约束的视差搜索窗口实现高效可靠的跨视图特征融合，并引入置信度加权机制在可靠区域进行交叉引导去模糊、遮挡区域退化为自去模糊。PECA模块可嵌入多种网络架构，实验证明其有效性和高效性。


<details>
  <summary>Details</summary>
Motivation: 现有立体去模糊方法和基准大多假设同构立体相机系统，无法有效处理智能手机异构相机模组（不同硬件参数）导致的非对称模糊伪影。

Method: 1) 构建HSD真实数据集（多帧融合）；2) 提出PECA模块：结合物理约束（光学推导的视差上界）和极线约束限制跨视图匹配搜索窗口；3) 置信度加权注意力+残差融合机制，可靠区域做交叉引导去模糊，遮挡区域退化为自去模糊；4) 模块架构无关，可嵌入CNN、Transformer、NAFNet等基线。

Result: 在HSD数据集上，PECA增强的各类模型均取得了更好的恢复性能，且计算效率优越。

Conclusion: PECA通过物理约束和自信任度机制有效解决了异构立体去模糊难题，为真实智能手机异构相机系统提供了实用且高效的解决方案。
...
TL;DR: 提出了一个层次化分类框架，结合分类学加权损失、最小风险贝叶斯推理、多尺度特征编码和独立秩分类头，在FathomNet 2025数据集上取得接近冠军成绩（平均分类距离1.581 vs 1.535），其解耦组件在分布偏移下泛化更优。


<details>
  <summary>Details</summary>
Motivation: 水下生物自动分类面临三大挑战：不同采集平台间的严重域偏移、近缘物种的细粒度视觉相似性、以及标注粒度不均（部分只能标注到属甚至更粗分类等级）。现有方法难以同时应对这些问题。

Method: 提出分类学感知的深度学习框架，核心组件包括：(1) 分类学加权损失（taxonomy-weighted loss），(2) 最小风险贝叶斯推理（minimum-risk Bayesian inference），(3) 多尺度特征编码，(4) 每个分类等级独立的分类头（per-rank classification heads）。

Result: 在FathomNet 2025数据集（79个海洋类别，7个分类等级）上，平均分类距离为1.581，接近第一名方案的1.535（差距3%以内）。最大的改进来自度量对齐推理和简单解耦组件，这些组件在分布偏移下比学习依赖关系泛化更好。

Conclusion: 层次化、解耦的分类方法在海洋物种分类任务上表现优异，特别是在域偏移场景下，简单解耦组件比复杂依赖关系具有更好的泛化性。
...
TL;DR: 本文提出一种基于教师-学生残差蒸馏的方法，用于将3D高斯街景的外观烘焙到固定几何的球谐系数中，解决了独立视图编辑不一致、视图特定噪声传播等问题。通过频域分解、置信度估计和基元级提升的结构化空间，结合渲染器空间匹配和支撑感知的高斯空间聚合，实现粗到细的置信门控优化，最终在推理时丢弃教师和辅助模块。在Waymo、Tanks and Temples等数据集上，相比编辑基线在目标对齐、内容保持、伪影抑制和跨视图一致性上取得更好平衡。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯街景编辑中，独立编辑的视图无法定义一致的3D目标，直接拟合会传播视图特定噪声，且现有流程无法同时处理不完美的稀疏锚点和标准光栅化部署问题。

Method: 提出教师-相对外观残差蒸馏方法。通过教师锚点与原始渲染之间的残差构建频域分解、置信度估计和基元级提升的结构化空间。使用渲染器空间匹配提供直接优化信号，通过支撑感知的高斯空间聚合正则化基元分配。采用置信门控粗到细优化，保留支撑细节并抑制无支撑噪声，最终将所有残差烘焙到固定几何的球谐系数中。

Result: 在Waymo街景、Tanks and Temples场景及多种目标条件下，相比编辑基线，在目标对齐、内容保持、伪影抑制和跨视图一致性上取得更优的整体平衡。消融实验验证了各主要组件的有效性。

Conclusion: 该方法有效解决了3D高斯街景编辑中的一致性问题，通过残差蒸馏和结构化优化实现了高质量的外观烘焙，代码已开源。
...
TL;DR: 提出MIMFlow框架，将掩码图像建模(MIM)与归一化流(NF)统一端到端训练，通过VAE编码器从掩码图像中提取语义潜变量，让NF专注于低频语义流形，专用解码器处理高频合成，解决了NF的容量瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 归一化流因严格可逆性被迫将模型容量耗费在低层像素细节上，难以捕获高层语义结构；MIM虽在表征学习上表现出色，但与生成模型的整合仍是模块化、割裂的。

Method: 采用VAE编码器从掩码图像中推断语义潜变量，解耦生成任务：归一化流负责简化、低频的语义流形建模，专用解码器负责高频细节合成。

Result: 在ImageNet 256×256上，MIMFlow-L达到71.3%线性探测精度和2.50的FID；仅用128个token（比标准模型少50%），相比同规模NF基线获得32.8%性能提升。

Conclusion: 通过任务解耦有效解决了NF的容量瓶颈，使模型优先关注全局结构一致性而非冗余噪声，实现了生成质量与表征学习的统一提升。
...
TL;DR: 本文提出OCR-Robust基准，系统评估视觉语言模型在视觉扰动下的OCR推理鲁棒性，发现高准确率不意味着高鲁棒性，结构化任务（图表、表格）比文档类任务更脆弱。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在OCR基准上表现强劲，但对视觉退化的鲁棒性理解不足，而视觉扰动会引入OCR错误和结构扭曲，影响推理任务的可靠性。

Method: 构建含812个样本的OCR-Robust基准（含OCR1.0和OCR2.0两个子集）；通过预实验从18种扰动中筛选出5种代表性扰动，每种设3个严重等级；使用干净准确率、RCR、WCR和CRI四个指标评估；对18个模型（含商业系统、开源VLM、OCR+LLM流水线）进行基准测试。

Result: 高干净准确率不一定意味着强鲁棒性；模型在结构敏感型OCR任务上最差情况退化显著；图表和表格比文档类输入在扰动下更脆弱。

Conclusion: 现有VLM在OCR推理鲁棒性方面存在不足，尤其对结构化文本内容，需进一步改进。
...
TL;DR: 本文提出DomainShuttle，一种能够在开放域中同时实现高保真度和灵活生成性的主题驱动文本生成视频方法，通过Domain-MoT解耦视频与参考特征、Video-Reference DualRoPE实现精确空间建模、Cross-Pair Consistent Loss提取主体不变特征，在域内和跨域场景均表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有开放域主题驱动文本生成视频方法主要集中在域内场景下最大化主体保真度，限制了在跨域场景（如新风格、语义组合、域属性）中的可编辑性和适应性。因此，需要一个能够灵活穿梭于不同域、同时在域内和跨域场景都表现良好的理想方法。

Method: 1) Domain-MoT：解耦视频与参考特征，引入域感知的AdaLN进行域特定建模；2) Video-Reference DualRoPE：将参考图像token和视频token置于独立的RoPE空间中，实现精确的主体级空间建模；3) Cross-Pair Consistent Loss：提取不受无关特征影响的主体固有特征。

Result: 大量实验表明，DomainShuttle在多样化的开放域应用场景中，相较于现有方法在主体保真度和生成灵活性方面均取得了显著的性能提升。

Conclusion: DomainShuttle成功实现了开放域视频个性化生成中高保真度与生成灵活性的兼顾，为域内和跨域场景提供了统一有效的解决方案。
...
TL;DR: 本文提出MVTrack4Gen框架，利用多视角点跟踪作为额外的几何与运动监督信号，增强相机条件型新视角视频扩散模型中的运动感知对应关系，从而提升几何一致性与运动保真度。


<details>
  <summary>Details</summary>
Motivation: 现有方法中，基于显式3D表示的方法受限于单目视频动态物体的重建精度；而仅依赖相机条件的方法虽然视觉质量高，但难以保持几何与运动一致性。作者希望在不引入显式3D重建的前提下，增强模型对运动对应关系的建模能力。

Method: 提出MVTrack4Gen框架，核心是观察到特定注意力层编码了强对应关系（查询特征与几何对应位置的键特征跨视图、跨时间匹配），通过对齐这些对应关系来改善运动一致性。具体做法是将这些特征路由到辅助的多视角跟踪头，与扩散模型联合训练一个点跟踪目标函数，显式强化运动感知对应。

Result: 在多个基准上达到最先进的几何一致性，相机精度也具有竞争力。

Conclusion: MVTrack4Gen有效提升了相机条件型新视角视频生成模型的几何一致性和运动保真度，验证了利用注意力层中的对应关系作为监督信号的可行性。
...
TL;DR: 提出TryOnCrafter，首个统一DiT框架，通过可渲染4D试穿代理实现相机可控的视频虚拟试穿（CaM-VVT），支持任意视角探索。


<details>
  <summary>Details</summary>
Motivation: 现有VVT方法被动依赖源相机轨迹，无法实现全方位的视角自由交互。

Method: 构建Renderable 4D Try-on Proxy，将2D试穿先验蒸馏为3DGS化身，通过SMPL-X动画并与背景点云对齐，由Proxy-Anchored Video DiT生成严格符合轨迹的逼真视频。

Result: 实现视角无关纹理合成与非刚体人体-背景结构同步，支持人体重定位、子弹时间、360度环绕观察等应用。

Conclusion: TryOnCrafter首次解决CaM-VVT任务，突破传统VVT的相机轨迹限制，为视频虚拟试穿提供了可控视角的新范式。
...
TL;DR: 本文提出潜在序列优化（LSO）方法，通过结合模拟 rollout 和策略梯度更新优化潜在序列，使行为基础模型（BFMs）能够精确跟踪运动序列，无需奖励工程，并在密集跟踪、稀疏关键帧和真实人形机器人部署上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有BFMs在时不变任务（如目标到达）上表现优异，但其潜在空间无法直接支持时变目标（如运动序列跟踪）。现有的移动窗口平均启发式方法无法捕捉高度动态运动的细微变化。

Method: 提出Latent Sequence Optimization (LSO)：结合模拟rollout与策略梯度更新，优化潜在序列以实现精确运动跟踪；通过时间相关噪声建模潜在序列，引导优化生成平滑连贯的潜在轨迹，无需奖励工程和调参。

Result: 在密集跟踪、稀疏关键帧任务中取得良好效果，并成功直接部署到真实人形机器人上。

Conclusion: LSO有效扩展了BFMs的能力边界，使其能够处理时变运动跟踪任务，且无需繁琐的奖励设计，为通用物理角色控制提供了新范式。
...
TL;DR: 本文提出了一种基于相对L-扩展位姿的迭代不变扩展卡尔曼滤波（IterIEKF）方法，用于IMU装备的铰接刚体系统的精确位姿估计，在UR5e机器人和人体腿部验证中实现了比现有方法至少50%的RMSE降低。


<details>
  <summary>Details</summary>
Motivation: IEKF对单刚体系统具有收敛保证和一致性，但将其扩展到铰接系统面临挑战：体间位姿耦合使得直接应用困难，且如何在不变框架内融入关节运动学约束仍是开放问题。

Method: 提出相对L-扩展位姿的Lie群表示，使每个刚体配备一个IMU的系统具有群仿射动态，并将关节约束表达为不变形式；将这些约束作为无噪声伪测量融入迭代IEKF（IterIEKF）中。

Result: 在UR5e机器人和人体腿部上验证，IterIEKF在所有场景中均优于EKF、IterEKF和绝对位姿IterIEKF基线，收敛更快、运行间变异性更低，RMSE至少降低50%。

Conclusion: 所提方法成功将不变滤波的收敛和一致性保证扩展至铰接多刚体系统，为机器人及人体运动分析中的精确位姿估计提供了有效解决方案。
...
TL;DR: ADM-Fusion是一种端到端深度学习多传感器融合方法，采用自适应传感器混合专家框架和内容感知路由，结合跨任务注意力机制，在仿真和真实数据上实现了对传感器退化环境的鲁棒位姿估计。


<details>
  <summary>Details</summary>
Motivation: 不同传感器在不同环境下以不同方式失效，传统固定权重的融合方法无法自适应地平衡互补线索，特别是在自运动估计任务中，需要根据环境变化和传感器退化动态调整融合策略。

Method: 提出ADM-Fusion方法，包含：(1)自适应传感器混合专家框架(content-aware routing)实现实时动态权重分配；(2)分离的平移和旋转分支，通过跨任务注意力机制保持任务特异性并允许信息共享；(3)在CARLA-LOC仿真数据集训练后在KITTI真实数据上微调，实现Sim-to-Real迁移。

Result: 在退化条件下保持鲁棒性，同时与现有方法相比具有竞争力的性能，成功实现了仿真到真实的迁移学习。

Conclusion: ADM-Fusion通过自适应融合和跨任务注意力机制，有效解决了多传感器融合在退化环境中的鲁棒性问题，验证了仿真训练+真实微调范式的可行性。
...
TL;DR: 本文提出了一个基于Habitat-Sim构建的室内协同目标导航数据集（含206个摄像头、74层），并设计了SurveilNav协同导航框架，通过主动相机调度、联合2D/3D建图、VLM价值估计和协同目标验证，显著提升了探索效率和导航成功率，在HM3D数据集上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有方法多聚焦于单机器人场景，难以在多视角协作的大规模环境中有效工作；固定监控摄像头存在固有盲区，单机器人感知范围有限，导致探索效率低下。

Method: SurveilNav框架包含四个模块：主动相机调度、联合2D/3D建图、基于VLM的价值估计以及协同目标验证，将机器人动态局部感知与监控静态全局视图进行协同。

Result: 在HM3D数据集上，SurveilNav在探索效率和导航成功率方面均显著优于现有方法，达到SOTA性能。

Conclusion: 该协同导航架构有效克服了单机器人感知范围有限和固定相机盲区的问题，在大型搜索、家庭环境和救援任务中展现出良好的应用潜力。
...
TL;DR: AeroCast提出了一种结合Transformer编码器与混合密度网络的概率性轨迹预测框架，用于预测非合作空中障碍物的未来位置，在五秒预测时域上相比基线方法减少约50%的位移误差，推理仅需0.1ms，适合实时部署。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么使用循环编码器难以捕捉长程运动前兆，要么仅产生点预测无法提供分布信息给下游规划器，无法充分处理非合作障碍物的多模态运动模式。

Method: 采用Transformer编码器捕捉长程运动依赖，结合混合密度网络输出每时间步的高斯混合分布；提出平移不变连续位移编码和校准导向的训练目标，解决输入设计和模式退化问题。

Result: 在包含九种运动类别的混合数据集上，五秒时域内ADE和FDE均降低约50%；NLL和CRPS均最优；消融实验表明速度输入和模型容量是关键，位置编码对长时域轨迹一致性重要；推理时间0.1ms，支持100Hz实时部署。

Conclusion: AeroCast在精度、概率预测质量和实时性上均显著优于现有方法，为无人机在共享空域中的自主避险提供了有效的概率轨迹预测方案。
...
TL;DR: 提出了一种RL引导的全身MPPI框架，作为预训练RL策略的附加反馈控制器，通过模块化成本项实现任务目标，无需重新训练即可提高精度和稳定性。


<details>
  <summary>Details</summary>
Motivation: 深度RL虽能实现鲁棒稳定性，但其行为与训练目标强耦合，难以在不重新训练的情况下添加新的反馈目标。

Method: 使用RL策略作为MPPI的采样先验，通过MPPI成本项指定任务目标，在线修正RL先验以满足目标。

Result: 在29-DOF Unitree G1人形机器人上的仿真表明，该方法实现了稳定高速控制（平均280 Hz），并在任务精度上优于纯RL基线。

Conclusion: 该方法通过纠正系统漂移和跟踪额外的全身参考信号，在不重新训练的情况下提升了任务级精度。
...
TL;DR: 本文提出了一种基于因果推理的参数化控制屏障函数（Parametric-CBF）方法，用于多车辆交互场景下的安全控制。该方法通过嵌入因果推理来明确推理车辆间的相互影响，从而使自车能够自适应地反应于周围车辆行为，在保证安全的同时提升任务效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理多车辆交互时存在两个问题：1) 难以确定周围车辆行为的因果关系（受车辆间相互影响干扰）；2) 过度依赖最坏情况分析，导致控制策略过于保守。因此需要一种能够明确建模车辆间因果影响的安全控制器。

Method: 将Parametric-Control Barrier Function（Parametric-CBF）扩展到多机器人交互场景，并在其中嵌入因果推理（causality inference），形成Causality-based Parametric-CBF。基于该函数设计自适应安全控制器，使自车能够根据对周围车辆行为的学习期望做出安全响应。

Result: 在多种交互密集型场景中，通过利用多车辆系统的运动灵活性，所提方法显著提升了任务效率。实验表明，该方法能够有效缓解传统方法过于保守的问题。

Conclusion: 通过因果推理与参数化CBF的结合，可以在多车辆交互中实现更高效且非保守的安全控制，为自动驾驶等安全关键应用提供了新的思路。
...
TL;DR: HALO是一种结合视觉语言模型先验和稀疏注意力机制的视觉运动策略，通过关注记忆检索实现机器人在部分可观测环境中的长时程自主控制，能够从长达8分钟的历史经验中检索任务相关信息。


<details>
  <summary>Details</summary>
Motivation: 通用机器人在部分可观测环境（如家庭）中需要记忆支持自主性，但现有Transformer记忆检索机制在模仿学习中存在两个问题：策略学到虚假相关性（过去信息与预测动作之间的虚假关联）；记忆误差累积导致模型漂移和级联失败。

Method: HALO通过两个关键设计解决上述挑战：(1) 蒸馏视觉语言模型先验，从示范轨迹生成记忆相关问答对，联合视频问答目标训练，引导检索到任务相关信息；(2) 采用稀疏注意力机制，限制检索只关注历史中最相关的部分，减少闭环控制中记忆误差累积的影响。

Result: HALO能够从长达8分钟的历史经验中检索任务相关信息，实现更可靠的长时程控制。

Conclusion: HALO通过结合VLM先验蒸馏和稀疏注意力机制，有效解决了Transformer记忆检索在机器人模仿学习中的虚假相关性和误差累积问题，为通用机器人在部分可观测环境中的长时程自主控制提供了可行方案。
...
TL;DR: 本文提出SwarmFly，一个基于MATLAB的模块化多无人机蜂群仿真与测试平台，解决了现有开源工具维护不足、学习曲线陡峭、场景单一等问题，并通过八项实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有开源无人机蜂群仿真工具存在维护不善、学习门槛高、仅支持单一场景等问题，亟需一个全面、模块化的仿真平台来填补研究空白。

Method: SwarmFly采用MATLAB开发，集成实时运行地图、四种蜂群协调模式（领航-跟随、去中心化、异构中继、异构速度）、模拟IMU遥测、基于IP的地理定位，并设计了插件架构，允许研究人员在不修改核心代码的前提下扩展行为、故障模型和分析工具。

Result: 通过八项实验验证了编队精度、抗风能力、故障恢复、能量续航和空域合规性等子系统性能。平台完全运行于MATLAB，已开源发布。

Conclusion: SwarmFly的模块化设计支持便捷扩展到硬件在环测试、更大规模蜂群和更高保真度动力学仿真，为无人机蜂群研究提供了灵活、可扩展的仿真基础。
...
TL;DR: 本文研究VLM剪枝中的"双重正确预测"问题（准确性+证据正确性），发现现有剪枝方法常保留正确证据但破坏正确预测，进而提出一种基于解释信息的剪枝策略，在自我中心视频基准上实现了更高的预测准确率和双重正确率。


<details>
  <summary>Details</summary>
Motivation: 在人机协作任务中，VLM需要低延迟推理，剪枝是常用手段。但安全的人机交互要求模型不仅预测准确，还要有正确的证据支持（双重正确预测），现有剪枝方法在这方面的表现尚未被充分研究。

Method: 提出一种理性引导（rationale-informed）的剪枝策略，在剪枝过程中更好地对齐证据与决策，从而保持双重正确预测。

Result: 在自我中心视频数据集上，该方法在预测准确率和双重正确率两方面均优于现有剪枝方法。

Conclusion: VLM剪枝应兼顾效率与可解释性、安全性，本文的工作为负责任的人机交互和具身智能提供了新的方向。
...
TL;DR: 本文提出fARfetch，一个结合AR头显与机器人的增强现实人机协作系统，通过共享语义环境映射、情境感知世界缩微图和VLM驱动的AR视图管理，在大型室外环境中显著提升任务完成速度（66%）、降低工作负荷（43%心理需求、34%时间压力、66%挫败感）。


<details>
  <summary>Details</summary>
Motivation: 大型、视觉多样的室外环境（如户外）对AR交互和内容可读性构成挑战，尤其是在远距离和超出视线范围时，现有AR-HRC系统难以应对。

Method: (i) AR头显与机器人共享语义环境映射，可视化检测到的地标以支持地标锚定的go-to指令；(ii) 情境感知的世界缩微图用于精细路径编辑；(iii) 视觉-语言模型驱动的AR视图管理，联合调整虚拟内容的颜色、大小和方向以保持可读性。

Result: 在30.5米真实户外巡检任务中（N=13），fARfetch相比非AR基线提升66%完成速度，心理需求降低43%，时间压力降低34%，挫败感降低66%，可读性调查表明系统有效维持了虚拟内容在大型室外环境中的可读性。

Conclusion: fARfetch验证了结合共享语义映射、世界缩微图和VLM驱动视图管理可有效解决大型室外环境中的AR-HRC可读性与交互难题，显著提升协作效率和用户体验。
...
TL;DR: 本文研究如何在形态感知强化学习框架中整合视觉感知，以提高四足机器人的跨形态泛化能力。通过对比盲策略、评论家感知策略（MorAL+）和全感知策略（PPAL），实验表明评论家层级的感知整合在鲁棒性和稳定性上优于盲策略和全感知策略。


<details>
  <summary>Details</summary>
Motivation: 现有四足运动控制器存在两大局限：一是依赖单机器人训练，无法跨形态泛化；二是盲策略缺乏实时感知，而全感知策略对噪声敏感。研究旨在探索如何将感知信息有效整合到形态感知的强化学习架构中。

Method: 基于MorAL框架，训练跨形态的通用控制器，使用自适应地形课程学习。设计了三种对比方案：盲基线、评论家感知变体（MorAL+，感知信息仅输入评论家网络）和全感知演员-评论家（PPAL，感知信息同时输入演员和评论家网络）。

Result: 在仿真（平坦和崎岖地形）和ANYmal硬件部署中，MorAL+（评论家仅感知）在鲁棒性和跟踪一致性上优于盲基线，且在感知噪声下比PPAL（全感知）更稳定。

Conclusion: 感知信息放置的位置（评论家层级 vs. 演员-评论家层级）以及课程设计是构建可扩展、形态感知运动控制的关键因素。评论家仅感知方案在跨形态泛化中兼顾了鲁棒性与稳定性。
...
TL;DR: RAVEN是一个基于智能体记忆的系统，将视觉嵌入与位姿、时间存储在向量数据库中，通过空间地图检索实现长期机器人问答和导航，在多项基准测试中以10倍更低的检索成本超越基于标题的记忆系统。


<details>
  <summary>Details</summary>
Motivation: 长期机器人部署需要紧凑且可扩展的记忆系统，能够保留细粒度视觉语义、在时空上定位观察，并支持高效存储和检索，现有基于图像到文本标题的方法存在信息损失问题。

Method: 将视觉嵌入与位姿、时间信息一起存储在向量数据库中，将检索锚定在空间地图上，直接对视觉嵌入操作而非依赖图像到文本的标题生成。

Result: 在多个模拟和真实世界视频问答基准测试中，RAVEN持续超越基于标题的记忆系统，在长程任务上达到与前沿VLM相当的性能，检索成本降低10倍，并在真实机器人上成功部署。

Conclusion: RAVEN通过直接操作视觉嵌入避免信息损失，实现了大规模、准确的语义、空间和时间检索，为长期机器人部署提供了高效的记忆解决方案。
...
TL;DR: 本文在Theo Jansen行走机构的铰链中引入间隙模型（Lankarani-Flores接触力+Ambrosio摩擦+Archard磨损），发现：无间隙模型低估峰值载荷约2倍；磨损高度非均匀，集中在约10度载荷弧上；优化设计在统计意义上鲁棒性更强（磨损低~9-7倍）。


<details>
  <summary>Details</summary>
Motivation: 先前研究将关节理想化为无间隙理想销钉，仅给出磨损的相对排名，无法预测实际服役中的退化行为。本文放松该理想化假设，建立含间隙的动力学模型以更真实地评估磨损。

Method: 构建Jansen腿的前向动力学模型，将旋转关节建模为含间隙关节，采用Lankarani-Flores连续法向接触律（含迟滞阻尼）和Ambrosio摩擦力，集成约束稳定化的微分-代数系统，并耦合Archard磨损定律形成磨损→间隙→冲击的反馈回路。采用16个随机相位系综进行统计分析。

Result: (1) 间隙模型峰值接触力约104 N，无间隙模型约48 N（~2倍放大），双关节间隙时达~426 N；(2) 耦合对冲击高度敏感，单轨迹非单调且可反转设计排名，但系综平均下优化设计磨损低~9-7倍（单间隙）和~1.7倍（双间隙），p<0.01；(3) 磨损集中在~10度载荷弧上，均匀间隙假设低估局部间隙增长约36倍。

Conclusion: 无间隙假设下的耐久性优势在混沌、多关节、非均匀磨损耦合的系综平均中仍然成立。文章首次提供了Jansen腿的含间隙耦合前向动力学模型，并提出了可证伪的实验验证协议。
...
TL;DR: 本文提出RigPI，一个结合视觉语言模型和可微物理模拟器的系统框架，用于从真实机器人交互中识别刚体和多连杆刚体的动力学参数（惯性和摩擦特性），通过两阶段优化策略实现了高精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决真实机器人操作中物理参数识别面临的传感噪声、建模误差和先验知识有限等挑战，为高级机器人操作和数字孪生构建提供准确的物理参数。

Method: 集成视觉语言模型（VLM）提供初始化与约束搜索空间，结合可微物理模拟器的梯度信息进行参数优化，采用两阶段优化策略抑制噪声影响、避免物理不可行解。

Result: 在真实世界的旋转关节和滑动关节物体上，RigPI实现了准确稳定的参数估计，并在真实机器人上成功复现了具有参数感知预测有效性的操作轨迹。

Conclusion: RigPI在真实机器人系统辨识任务中展现了有效性和鲁棒性，能够为机器人操作提供物理一致的参数估计。
...
TL;DR: 提出了Swazure技术，通过FLS间的协作解决传感器超出最佳范围时的定位问题，实现物理数据独立性，并通过Move Obstructing启发式方法解决遮挡问题，在极端情况下可解决约30%的遮挡。


<details>
  <summary>Details</summary>
Motivation: FLS（微型无人机光点）在构建3D多媒体显示时需要相互准确测量位姿，但传感器仅在特定最佳范围内精度高，当点云数据导致FLS超出此范围时，缺乏有效的跟踪方法。

Method: 提出Swazure技术，通过FLS间协作来补全缺失的传感器数据，实现物理数据独立性；并针对大型FLS可能导致的遮挡问题，提出Move Obstructing和Move Source两种启发式算法。

Result: 中等尺寸FLS下，Swazure能定位100%的邻居；在极端遮挡情况下，Move Obstructing启发式算法可解决约30%的遮挡问题，优于Move Source。

Conclusion: Swazure通过协作机制有效解决了传感器范围限制问题，Move Obstructing是处理遮挡的更优策略，为3D多媒体显示中的FLS群体定位提供了可行方案。
...
TL;DR: 提出了一种基于模仿学习的自适应教学计算模型，通过最近邻检索和交叉注意力机制，在低数据条件下利用交互历史的时间推理，提升复杂运动技能（如高性能驾驶）的教学适应性，并在两个数据集上验证了其优于非自适应基线及其他自适应模型。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的自动化教学系统在处理复杂运动任务（如高性能驾驶）时，仅依赖局部、上下文相关的推理，缺乏对长期师生交互累积影响以及学生学习过程时间特性的考虑，适应性不足。

Method: 采用模仿学习框架，构建包含时间推理模块的自适应教学模型。模型通过最近邻检索从历史交互中筛选语义相似样本，结合交叉注意力机制作为先验，再使用编码器-解码器结构的并发教学模型进行推理，从而在低数据场景下实现有效的长期交互历史推理。

Result: 在基于Waymo开放运动数据集构建的半合成纵向师生交互数据集，以及小规模真实世界自然主义模拟器赛车教学数据集上，所提模型一致优于非自适应基线，以及采用不同先验和时间融合机制的自适应模型。

Conclusion: 基于最近邻检索和交叉注意力先验的自适应教学模型，能够有效利用有限交互数据中的长期时间信息，提升复杂运动技能教学系统的适应性，为小样本交互场景下的自动化教学提供了可行方案。
...
TL;DR: 提出GRAFT，一种几何感知的零样本操作迁移框架，仅需每个对象一次演示，通过部件级图表示实现未见物体上的接触点迁移。


<details>
  <summary>Details</summary>
Motivation: 现有操作泛化方法依赖大量演示且在小样本场景下失效，语义检索忽略了操作中关键的几何相似性。

Method: 构建部件级图结构，结合全局实例检索、部件对应和顶点级接触点匹配，实现从演示库到新物体的几何感知迁移。

Result: 在零样本设置下，仅需每个对象一次演示即可实现未见物体上的操作迁移。

Conclusion: 几何感知的部件级对应关系能有效替代纯语义检索，实现零样本操作泛化。
...
TL;DR: 提出一种软硬件集成的光电架构，通过DMD光学卷积层、块压缩感知和CLIP引导的自然语言标注，在保持检测精度的同时减少90%数据量和60%计算量，实现低数据、免形状标注的机器人视觉检测。


<details>
  <summary>Details</summary>
Motivation: 解决机器人视觉检测中数据过载和形状级标注效率低下的问题，现有方法依赖大量人工标注且数据冗余度高。

Method: 1) 传感器在环策略将DMD重构为光学卷积层实现光子域特征提取；2) 块压缩感知将空间信息编码为低维时间信号；3) 利用CLIP的通用特征，以自然语言引导网络关注缺陷形状；4) 提出LAA指标量化形状级定位精度。

Result: 在透明材料缺陷检测中，相比传统成像，ViT数据量减少90%，CNN计算量降低60%，同时保持同等精度。

Conclusion: 该低数据范式为数据量大、采集成本高或边缘资源受限的工业自动化场景提供了高效解决方案。
...
TL;DR: 提出了一种动态移动操作框架，通过锚点扩散模型预测抓取轨迹，结合全身强化学习控制策略，实现机器人对动态物体的有效抓取。


<details>
  <summary>Details</summary>
Motivation: 动态物体的移动操作极具挑战性，机器人需要协调移动基座和机械臂，同时适应不断变化的目标位姿，现有方法在预测时序一致的短时抓取轨迹方面仍存在问题。

Method: 1) 基于锚点的扩散模型预测时序一致的短时抓取轨迹；2) 将预测轨迹编码为紧凑特征输入全身强化学习策略；3) 引入预期引导奖励，自适应地将目标从当前抓取观测转移到预测轨迹。

Result: 在Isaac Gym仿真中，该方法在多种动态物体抓取场景下表现出色；在真实实验中，预测器和策略均展现出强泛化能力。

Conclusion: 该动态移动操作框架有效解决了动态物体抓取中的轨迹预测与运动协调问题，在仿真和真实环境中均取得了良好效果。
...
TL;DR: 提出一种低成本被动轮腿机器人，利用标准脚轮和不对称Actor-Critic控制方法，实现全向移动，运输成本比纯腿式运动降低89.1%。


<details>
  <summary>Details</summary>
Motivation: 传统主动轮式机器人需要专门硬件和电气设计，成本高。本文旨在设计一种低成本、被动轮的腿式机器人，兼具全向机动性和高能效。

Method: 1) 在每条腿上安装标准脚轮（被动轮），实现全向移动；2) 采用非对称Actor-Critic结构，利用脚轮角度/速度的特权信息；3) 基于速度指令的脚轮基座姿态调整策略，通过主动关节调整脚轮推进方向；4) 多种推进模式，实现不同程度的脚轮扭转振荡，转化为推进力。

Result: 在绕桩测试和模式切换实验中，被动轮腿机器人实现了全向移动多样性，运输成本（COT）相比纯腿式运动降低最多89.1%。

Conclusion: 被动轮腿方案能显著降低成本和能耗，同时保持复杂地形的适应能力，为长距离运输应用提供了一种高效、低成本的新方案。
...
TL;DR: 本文提出了一种将AI教练建模为非合作动态博弈的强化学习框架，通过自适应共享控制和因果影响模型来加速人类运动技能发展，在无人机竞速实验中显著优于现有AI辅导方法。


<details>
  <summary>Details</summary>
Motivation: AI辅助虽然能提升任务表现，但过度帮助会导致人类依赖和技能退化。现有方法缺乏对学习者能力的动态适配，需要一种能战略性"退后"、允许学习者经历生产性失败的教练范式。

Method: 将AI教练过程形式化为非合作动态博弈：学习者优化任务表现，教练以培养独立能力为目标。结合自适应共享控制与概率因果模型（追踪教练对技能演化的因果影响），训练强化学习策略。

Result: 在FPV无人机竞速的用户实验（N=33）中，该方法在人类学习效果上显著超过最新的AI教练基线方法。

Conclusion: AI教练应动态调整辅助程度，在"帮助"与"放手"间取得平衡。博弈论+强化学习的框架为设计促进真实技能发展的AI系统提供了新范式。
...
TL;DR: 提出了一种基于自电容原理的简单、可扩展的触觉传感系统，用于陪伴型人形机器人，使用单层导电织物和导线架构，无需复杂电极图案。通过柔性电路板实现100点传感器阵列，FPGA上部署决策树分类器实现实时推理，满足HIRO-chan机器人的全身触觉需求。


<details>
  <summary>Details</summary>
Motivation: 传统触觉传感系统存在多层结构复杂、布线繁琐、成本高、可扩展性差等问题，难以在低计算负载下实现全身触觉感知的实时低延迟检测。

Method: 基于自电容原理，采用单层导电织物和导电织物导线架构，无需复杂电极图案。通过柔性电路板制作100点传感器阵列，在FPGA上部署决策树分类器进行实时推理，减轻主处理器负载。

Result: 100 Hz和1000 Hz采样频率能可靠捕获所有交互类型（轻触、慢速敲击、快速敲击、击打），而10 Hz会遗漏瞬态事件。FPGA离线推理延迟低、功耗开销可忽略。

Conclusion: 该设计完全满足HIRO-chan机器人的触觉传感需求，适用于陪伴型机器人的全身触觉感知，具有简单、可扩展、硬件友好的特点。
...
TL;DR: 提出SVP-IL，一种解耦架构，通过零样本空间视觉提示将语义推理与空间控制分离，在数据受限的模仿学习中显著提升了机器人操作的精度和数据效率。


<details>
  <summary>Details</summary>
Motivation: 端到端VLA模型将语义推理和空间控制耦合，导致对齐瓶颈，限制了数据受限模仿学习中的精确目标辨识能力。

Method: 解耦架构SVP-IL：利用视觉语言基础模型将指令解析为零样本几何掩码（空间视觉提示SVP），通过轻量级特征级融合机制注入连续动作生成器，提供明确的空间梯度引导。

Result: 在50-100条演示数据下，高度模糊的语言条件任务成功率从24.0%提升至39.5%；标准基准达67.8%；真实机器人实验验证了鲁棒性和数据效率。

Conclusion: 解耦空间视觉提示与动作生成能有效打破VLA模型的对齐瓶颈，在低数据场景下实现更优的语义-空间对齐和稳定优化。
...
TL;DR: AMPLE-GNC是一个用于深空任务的三层制导、导航与控制架构，结合了小规模基础模型指挥官（将自然语言映射为PDDL+）、约束筛选验证器、故障自适应控制器，以及运行时屏蔽层（含9个LTL不变式）。在6-DOF Basilisk测试平台上，实现了84%的可执行动作、97.8%的作动器故障恢复，并验证了安全盾与自适应控制器之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 深空任务需要同时具备能力和可认证性的星上自主系统。基于规则的自主系统可认证但脆弱，而基于学习的自主系统能力强但难以验证。如何在两者之间取得平衡是核心动机。

Method: 提出AMPLE-GNC三层架构：1）能力路径：小规模基础模型指挥官（360M参数，微调+语法约束解码将自然语言转为PDDL+）、约束筛选验证器、故障自适应控制器（Rapid Motor Adaptation在线推断潜在作动器故障）；2）运行时屏蔽层：9个线性时序逻辑不变式，由Kind 2模型检查器机器验证预测器正确性；3）采用分裂共形预测生成恢复截止证书，实现自适应感知的安全盾介入策略。

Result: 指挥官输出有效性保证硬约束，84%动作可被规划器执行；新颖措辞泛化能力38%精确匹配、51%动作匹配，经措辞多样性再微调提升至48%。故障自适应控制器在训练随机化包络内恢复97.8%作动器信号故障和94.4%连续增益故障，远超故障无感知PD和端到端RL（0%）及最强经典自适应基线（55%连续增益）。包络外测试57-67%，增加4倍同域数据反而降低性能，表明随机化广度而非数据量驱动泛化。星跟踪器噪声鲁棒性在0.005内保持平坦。安全盾介入时，自适应感知的介入策略使控制器保持94.5%自主性同时仍能捕获非恢复情况。

Conclusion: 证明了大语言模型可在可认证自主系统中扮演指挥官角色；故障自适应控制器显著优于传统方法；安全盾与控制器之间需要智能协调策略，简单锁存会过度抑制能力；随机化广度比数据量更关键。
...
TL;DR: 提出Commerge，一种基于图论选择性数据交换的高效通信多机器人LiDAR地图融合框架，在保持对齐精度的同时将通信量减少高达5000倍（例如HeLiPR数据集从7000MB降至1.3MB）。


<details>
  <summary>Details</summary>
Motivation: 多机器人LiDAR地图融合需要大量传感器数据传输，通信成为瓶颈，尤其在通信受限环境中。现有方法要么传输完整扫描（GB级），要么采用简单降采样，无法兼顾通信效率和对齐精度。

Method: 将问题建模为交换图上的三级级联优化：第一级选择序列重叠子集，第二级平衡传输代价，第三级优化几何-感知最优性，从而选出既能保持对齐质量又能减少通信量的关键扫描子集。

Result: 在5个公开数据集和4个内部数据集上，实现高达99.98%的数据交换缩减（如HeLiPR数据集从7000MB降至1.3MB），同时保持对齐性能；在嵌入式到桌面平台上均有效。

Conclusion: Commerge通过图论选择性数据交换显著降低多机器人地图融合中的通信开销，实现了MB级数据交换即可达到GB级数据的对齐精度，为通信受限环境下的多机器人协同建图提供了高效解决方案。
...
### [121] [Large-Scale Tunnel Air--Ground Collaboration With FLISP: Fast LiDAR-IMU Synchronized Path Planne](https://arxiv.org/abs/2606.25393)
*Fenghe Guo,Runjie Shen,Chenyang Sun,Junrui Zhang,Quanxi Zhan,Yongchun Wang,Junjie Zhang*

Main category: cs.RO

TL;DR: 提出FLISP，一种无需地图的UGV-UAV协同规划框架，利用单个LiDAR-IMU系统实现水电隧道快速自主巡检，相比传统方法速度提升7倍至三个数量级，成功率100%。


<details>
  <summary>Details</summary>
Motivation: 水电隧道人工巡检效率低、危险性高，现有基于地图的规划方法存在栅格化开销大、采样不稳定等结构性瓶颈，亟需一种无需地图、低延迟的协同规划方案。

Method: (1) 统一架构：单个UGV搭载的LiDAR-IMU同时为双平台生成同步路径；(2) 平台特有求解器：改进萤火虫算法用于UGV避障，动态迭代优化器用于UAV飞行；(3) 分层细化策略：保证运动学可行性，避免状态估计漂移。

Result: 在1.2 km运营隧道中，FLISP成功率100%，延迟约7 ms，比基于网格的方法快7倍，比基于采样的方法快三个数量级。

Conclusion: FLISP为特征退化线性基础设施的机器人巡检提供了可扩展的解决方案，已在实际水电隧道中得到验证。
...
TL;DR: MAPL利用大语言模型从自然语言目标中学习多目标偏好，无需人工设计奖励函数，在四足机器人 locomotion 任务中达到或超过专家设计的奖励性能。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习在机器人运动控制中依赖精心手工设计的奖励函数，而现有的基于偏好的方法仅给出单一的整体判断，难以捕捉高质量运动所需的多个竞争性目标。

Method: MAPL使用LLM沿语义上有意义的标准（与地形无关的通用语言描述）独立比较轨迹，生成多目标偏好，训练多头偏好评分模型，聚合标量奖励用于策略优化。

Result: 在四种四足机器人 locomotion 环境中，仅使用LLM生成的偏好即可训练策略，性能与专家设计的奖励相当或更好，且无需任务特定的奖励工程。

Conclusion: MAPL有效消除了奖励工程瓶颈，通过自然语言目标实现了高质量的运动控制策略学习。
...
TL;DR: 提出了一种异构多LLM框架（HEART），通过将复杂指令分解为原子推理任务，分配给角色专业化的专家智能体，在令牌预算约束下进行机器人任务规划，实验表明相比单LLM和规则规划器能生成更可行的物理执行计划。


<details>
  <summary>Details</summary>
Motivation: 现有LLM规划的机器人在处理物理空间约束（如可达性、逻辑顺序）时缺乏结构化推理，生成的计划往往无效或不完整，因此需要更鲁棒的规划框架。

Method: 采用异构多LLM框架，将指令分解为原子任务，分配给角色专业化的专家智能体（如可行性、可达性、约束检查等），在令牌预算下进行约束驱动的计划合成。

Result: 在多个家庭基准任务上，HEART相比单LLM和规则规划器显著提高了计划成功率，验证了异构LLM协作的有效性。

Conclusion: 异构LLM协作能够在资源约束下实现鲁棒且可扩展的机器人任务规划，物理可行性得到保证。
...
TL;DR: 本文针对基于学习的惯性里程计问题，提出使用增量位移（Δp）估计替代绝对位置回归，并在EuRoC MAV数据集上比较了MLP和KAN（Kolmogorov-Arnold Network）的性能。KAN仅用8,444个参数（MLP的1/7），却在最终漂移误差上降低了44%（9.61m vs 17.23m），且长期误差累积更稳定。


<details>
  <summary>Details</summary>
Motivation: 传统的绝对位置回归方法容易产生较大的常数误差，因此作者提出通过估计增量位移（Δp）并数值积分重建轨迹，以降低误差累积。

Method: 使用EuRoC MAV数据集中的原始IMU测量数据，将模型训练为在固定50ms滑动窗口上估计增量位移（Δp），再通过数值积分重构完整轨迹。比较了标准MLP与使用可学习B样条激活函数的KAN。

Result: KAN参数仅为MLP的1/6.9（8,444 vs 57,859），但最终累积漂移误差降低了44%（9.61m vs 17.23m），且长期误差累积更稳定，P_50和P_90漂移值均更低。

Conclusion: 基于可学习B样条的激活函数（KAN）在惯性里程计问题中具有减少误差累积的潜力，以更少的参数实现了更好的性能。
...
TL;DR: SWAN是一个端到端管道，能从文本提示直接合成逼真、大规模且无碰撞的无人机编队灯光秀动画，通过自适应点跟踪算法将像素动力学转换为物理集群运动，并在2000架仿真和49架真实无人机上验证可行性。


<details>
  <summary>Details</summary>
Motivation: 现有无人机灯光秀依赖人工动画，效率低下；生成式AI虽提供自动化可能，但现有框架无法实现兼具照片级真实感和流畅动态运动的效果。

Method: 提出SWAN端到端管道：1) 文本→逼真参考视频；2) 自适应点跟踪算法将像素运动转为物理集群运动学（能应对严重遮挡和拓扑变化）；3) 规划器将轨迹分配给个体无人机；4) 安全过滤器确保无碰撞执行。

Result: 在模拟2000架无人机编队中验证可扩展性，并在49架真实四旋翼无人机密集集群中验证物理可行性，所有计算均在标准消费级硬件上完成。

Conclusion: 证明了生成式AI可自动化多机器人编舞设计，为无人机灯光秀提供了便捷的新框架。
...
TL;DR: 本文提出SAGE-Nav，一种结合大语言模型与动态场景图的分层导航框架，通过解耦全局语义规划与高频反应控制，在ObjNav任务上实现了SOTA性能，显著提升了导航效率和零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法在长程推理和新环境泛化方面表现不佳，传统的端到端方法难以处理长期的导航推理任务，且对未见环境的适应能力较差。

Method: 提出分层框架SAGE-Nav：1) LLM作为全局规划器，将抽象指令分解为语义路标序列；2) 层级场景图编码器(HSGE)利用关系图卷积生成结构感知嵌入；3) 目标感知对齐融合网络(GAFN)通过自适应门控机制实现实时感知与结构先验的动态融合。

Result: 在i-THOR和RoboTHOR环境中的广泛评估表明，SAGE-Nav在导航效率和零样本泛化方面均达到SOTA性能，同时保持了物理机器人部署所需的低控制延迟。

Conclusion: 解耦全局语义规划与局部反应控制的分层架构有效提升了长程导航推理能力，LLM与动态场景图的结合为Embodied AI中的语义导航任务提供了新的有效范式。
...
TL;DR: AISPO是一种深度补全框架，通过多尺度RGB-D特征融合与仿射不变形状先验，提升机器人操作中对非朗伯体（如透明、高反光物体）的深度感知可靠性，从而显著提高操作成功率。


<details>
  <summary>Details</summary>
Motivation: 现有深度传感器在非朗伯体表面（透明、高反光物体）上深度测量常出现缺失或错误，导致运动规划失败、抓取位姿无效。现有方法多关注平均深度精度，缺乏对深度图物理合理性和结构完整性的强调。

Method: 提出AISPO框架，结合多尺度RGB-D特征融合与仿射不变形状先验（affine-invariant shape prior），强制几何一致性，缓解深度估计中的灾难性失败。

Result: 在多个基准测试上表现出竞争性性能，对未见物体和新场景有强泛化能力；真实机器人抓取实验表明，该方法显著提升操作成功率，尤其在透明物体场景中优于现有方法。

Conclusion: 通过提升深度图的物理合理性和结构完整性，AISPO有效改善了机器人在复杂光照条件下的操作可靠性，尤其适用于现有方法难以处理的透明物体抓取任务。
...
TL;DR: GROVE是一个文本到场景的行人仿真框架，结合多种SOTA方法生成逼真、具有社交挑战性的场景，用于社交机器人导航训练与部署。
...
Method: 提出GROVE框架，支持预设（紧急、排队、正常）或用户自定义文本提示；采用多模块分别处理长时程人类行为、中时程行人导航和短时程机器人/社交交互；根据场景动态选择SOTA方法缩小仿真到现实的差距；集成于Isaac Sim、Gazebo、RViz。

Result: 通过在不同复杂度场景（住宅、医院、办公室）中的定性比较，GROVE生成的行人仿真在逼真度和社交挑战性上优于现有基线方法。

Conclusion: GROVE实现了高保真行人仿真，能够为社交机器人导航提供复杂、多样且真实的人类行为挑战。
...
TL;DR: 本文提出ASSCG（自适应慢系统控制门），通过强化学习优化的RWKV门控机制，在自动驾驶快慢双系统中动态决策查询/缓存/丢弃LLM慢规划结果，在nuPlan和NAVSIM上分别提升2.28分成绩+60%延迟降低和0.6 PDMS+25%速度提升。


<details>
  <summary>Details</summary>
Motivation: LLM能提升自动驾驶规划效果，但在线查询成本高；现有快慢规划器依赖手工设计的触发规则，要么过度调用慢系统，要么在错误时机调用，缺乏资源感知的智能调度。

Method: 将慢系统调用建模为资源感知的序列决策问题，提出ASSCG门控机制，支持帧级别的Query/Cache/Drop三种决策；采用RWKV骨干网络进行高效长程门控，先监督微调后使用GRPO风格的计算感知强化微调。

Result: 在AsyncDriver+nuPlan上分数提升2.28（达67.28），端到端推理延迟降低60%；在RecogDrive+NAVSIM上PDMS提升0.6（达91.4），平均速度提升25%。

Conclusion: ASSCG有效解决了快慢规划器中慢系统调用时机和频率的优化问题，通过学习型门控替代手工规则，在保持规划性能的同时显著降低计算成本。
...
TL;DR: 本文提出了一种共体变自主性（co-embodiment with variable autonomy）的可穿戴机器人手，让人类和机器人共享同一物理身体，在不同任务阶段（如自主抓取与人类控制）切换自主层级，实现"一个身体，两个大脑"的协作范式。用户研究（N=44）显示任务完成时间提升23.3%，成功率高达93.6%，接受度良好。


<details>
  <summary>Details</summary>
Motivation: 现有辅助机器人面临两难：全自主系统剥夺用户主动性，而全用户控制则需要持续的认知努力。已有共享自主方法虽混合人类与机器人指令，但大多部署在分离的物理身体中，缺乏真正的物理耦合。

Method: 设计了一款共体可穿戴机器人手，采用从示教学习（Learning from Demonstration）的视觉运动扩散策略（Visuomotor Diffusion Policy）实现自主抓取；通过头部手势实现人类控制；用户可通过释放手势（Release Gesture）保留否决权。实现了从完全人类控制到完全自主行动的变自主性。

Result: 44名参与者完成5项双手任务，任务完成时间提升23.3%（p<0.001, Cohen's d=0.94），最佳策略变体任务成功率达93.6%，总体印象评分5.70/7，日常使用意愿5.52/7。

Conclusion: 共体变自主性作为一种可行的辅助机器人范式，通过物理共体实现了人类与机器人之间的有效协作，平衡了自主性与用户控制权。
...
TL;DR: 提出WOLF-VLA框架，融合全身最优控制运动合成与大规模多模态数据集，训练VLA模型直接从自然语言指令生成仿人机器人全身运动策略，解决数据稀缺和动态一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在机器人操作任务中表现优异，但尚未有效应用于仿人机器人全身接触丰富的运动控制，主要面临数据稀缺、缺乏动态一致的运动演示、以及难以将最优性和安全性编码到学习管道中三大挑战。

Method: 构建包含六类运动任务的动态可行仿人轨迹数据集，参数化环境变化、物体颜色、位置及视觉干扰；使用联合轨迹、自我中心视觉观察和自然语言指令训练VLA模型。

Result: 训练出的策略展现出强大的推理能力和对初始条件变化的鲁棒性，在多个任务和环境设置中表现具有竞争力；消融研究验证了各模态对模型性能的影响。

Conclusion: WOLF-VLA为全身仿人运动控制的VLA方法建立了可复现的动态一致基准，数据集、模型和仿真套件将开源，推动指令驱动运动策略的可扩展迁移研究。
...
TL;DR: 本文首次提出大规模乒乓球事件相机数据集，包含超1000回合、14台同步高速相机与事件流，训练了联合估计球位置与速度的CNN，结合卡尔曼滤波将弹跳点预测误差降低36%，并成功实现基于事件感知的实时人机对打。


<details>
  <summary>Details</summary>
Motivation: 传统帧相机在高速乒乓球场景下存在帧率与计算成本的矛盾，事件相机虽有微秒级时间分辨率优势，但缺乏大规模真实场景数据集来支撑事件感知的研发与评测。

Method: 构建大规模事件相机乒乓球数据集（1000+回合，14台200FPS高速相机同步记录），通过200FPS帧插值生成1kHz伪真值标签；训练CNN联合估计图像平面中的球位置与速度；将预测速度作为额外测量值输入卡尔曼滤波器；最后集成Stäubli机械臂实现闭环控制。

Result: 联合估计球速作为卡尔曼滤波附加测量量，相比仅用位置的基线，弹跳点预测误差降低36%。成功实现首个人机实时乒乓球对打系统。

Conclusion: 事件相机结合CNN与卡尔曼滤波可显著提升高速球类运动的预测精度，为实时机器人感知提供了高效新范式，且开源数据集填补了领域空白。
...
TL;DR: 本文提出事件自适应运动规划（EAMP），一种基于VLM的高效机器人导航框架。通过可配置语义事件触发器（PC-SET）选择性激活干预，利用蒸馏后的SemNav-VLM进行策略级决策，并由语义模型预测控制（SMPC）将策略转化为底层控制调整，在安全关键物流场景中显著提升动态安全裕度并保持实时性。


<details>
  <summary>Details</summary>
Motivation: 机器人导航面临动态智能体行为不可预测导致的安全问题；大视觉语言模型虽有常识推理能力但频繁调用会引发严重计算延迟，影响物理执行稳定性。

Method: 1) 可配置语义事件触发器（PC-SET）：持续监测短时域片段中的行为异常，选择性激活语义干预；2) 事件触发蒸馏SemNav-VLM：通过物理验证的语义蒸馏微调，将检测到的异常映射为离散策略级决策；3) 语义模型预测控制（SMPC）：将策略转化为优化目标和几何参考的动态重构。

Result: 在安全关键物流场景中，EAMP有效对齐高层推理与底层控制，相比现有基线显著提升动态安全裕度，同时保持实时效率。

Conclusion: EAMP通过事件驱动的选择性VLM调用机制，解决了计算效率与语义推理能力之间的权衡问题，证明了将大模型用于实时机器人导航的可行方案。
...
TL;DR: 提出一种利用纯旋转运动实现多相机系统（无重叠视场）外参标定的方法，仅需单个静态标定板，结合隐式转台坐标系与SE(3)全局优化，兼顾精度与易用性。


<details>
  <summary>Details</summary>
Motivation: 现有无重叠视场多相机标定方法存在两难：基于标定板的方法需要大型标定板或预置多板位姿，部署繁琐；基于运动的方法易受漂移、尺度模糊和运动退化影响。因此需要一种既精确又实用的标定方案。

Method: 利用纯旋转运动，让所有相机在时间上依次观测同一静态标定板，通过共享的隐式转台坐标系建立几何关联，将问题建模为SE(3)上的3D误差并在全局优化框架中求解。

Result: 在受控相机台架和全尺寸异构相机车辆平台上均验证了有效性，分析了非理想转台运动下的鲁棒性，精度与现有方法相当，无需精密专用硬件。

Conclusion: 该方法在保持竞争精度的同时显著降低了硬件和部署要求，适合实际现场标定场景，并已开源代码。
...
TL;DR: 提出Reptile-D-learning框架，利用元学习算法Reptile学习可泛化的Lyapunov网络初始化和高性能控制器，解决基于学习的Lyapunov控制因参数变化导致需要重训练的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的Lyapunov控制（LLC）依赖精确系统模型，参数变化和不确定性会使稳定性约束失效，导致昂贵的重训练；D-learning虽不依赖显式动力学模型，但受限于单任务动力学，在大参数偏移下性能退化。

Method: 提出Reptile-D-learning框架，利用Reptile元学习算法捕捉不同参数系统间的共享动力学结构，学习可泛化的Lyapunov网络初始化和高性能控制器。

Result: 在多个非线性控制系统上的实验表明，Reptile-D-learning在泛化能力和对未见参数配置的快速自适应方面均显著提升。

Conclusion: 元学习能够有效提升基于学习的Lyapunov控制方法在参数变化下的泛化性和适应性，减少重训练成本。
...
TL;DR: 本文提出将节能水下控制建模为约束马尔可夫决策过程，使用PPO-Lagrangian算法显式约束平均推进器功率，无需手动调参即可在多个车辆和任务中实现14-65%的能耗降低，同时保持任务精度。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习水下控制器仅优化任务精度会导致振荡式耗能，而现有添加能量惩罚项的方法需要手动调参且无法指定目标功率水平，缺乏物理可解释性。

Method: 将节能水下控制建模为约束马尔可夫决策过程（CMDP），以平均推进器功率为显式约束，使用PPO-Lagrangian算法在线更新单一对偶变量以满足功率预算约束。

Result: 在MarineGym模拟器中，跨3种水下航行器和4种任务共12个设置中，能量约束策略均实现最低能耗（降低14-65%），在10个设置中保持最平滑控制，仅在特定功率受限场景下牺牲任务精度。

Conclusion: 将能量作为显式约束而非惩罚项，提供了一种免调参的节能水下控制方案，无需针对不同车辆和任务进行权重搜索。
...
TL;DR: SA-LIVO提出子空间感知信息融合框架，通过特征分解联合信息矩阵并逐方向软门控，使视觉残差仅在LiDAR退化方向上起作用，在三个基准29个序列上实现高精度、低漂移，且计算效率高（CPU上12.3ms/帧）。


<details>
  <summary>Details</summary>
Motivation: 现有LIVO系统在LiDAR与视觉各自失效时缺乏方向级应对：LiDAR在几何约束不足时退化，视觉在弱纹理/光照下失效。现有方法仅在模态层面做退化检测或协方差膨胀，未处理联合信息矩阵的方向依赖结构，导致视觉信息在不需处浪费、在需处分散。

Method: 提出SA-LIVO系统，核心是子空间感知信息融合（SAIF）框架：对联合LiDAR-视觉信息矩阵进行特征分解，每个特征方向施加线性-钳位软门控（linear-clamp soft gate），抑制退化方向、保留可观测方向。在单次InEKF循环中联合优化LiDAR和视觉残差，共享线性化点。视觉雅可比仅在循环前组装一次，跨迭代复用，避免传统迭代滤波器每迭代重算的开销。

Result: 在HILTI'22、New College、Oxford Spires三个基准29个序列以及并发退化场景中，精度与最强基线相当，且在竞争系统发散时保持有界漂移。SA-LIVO在笔记本CPU上平均12.3ms/帧，嵌入式ARM板（无GPU）上26.8ms/帧，峰值内存降低3.6-6.3倍。

Conclusion: 基于方向选择性融合的SA-LIVO有效解决了LIVO系统中LiDAR与视觉的退化问题，实现了高精度、低漂移与高效率的统一，代码将开源。
...
TL;DR: 提出一种异步上身任务空间跟踪框架，通过教师-学生蒸馏、滑动窗口全局奖励和MPC模块，解决人形机器人规划与执行之间的时间异步和结构不完整问题，在低更新率下实现更优跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 人形机器人高层规划器输出稀疏、低更新率的任务空间轨迹，而全身控制器需要高频运行，导致规划与执行之间存在时间异步和结构不完整（缺乏全身控制信息）。

Method: 1) 教师-学生蒸馏初始化学生策略；2) 以完整缓存未来轨迹和执行时间索引为条件训练；3) 滑动窗口全局奖励减少帧漂移；4) 后训练阶段使用MPC模块补全稀疏参考，动作级与正向运动学级自引导约束策略漂移。

Result: 仿真和Unitree G1硬件实验表明：低更新率下跟踪性能提升，优于同步和去耦合基线，对分布外运动具有更安全的适应能力。

Conclusion: 异步跟踪框架有效解决了规划-执行异步问题，无需显式帧估计即可减少帧漂移，在现实硬件上验证了实用性。
...
TL;DR: 本文提出了一种阶段感知与粗糙度约束的扩散策略（SRDP），用于机器人抛光。该方法通过多模态观测历史推断加工阶段后验，并利用粗糙度导向的约束扩散采样生成符合物理可行性的进给速度和法向接触力。在航天器舱段涂层抛光和内腔结构表面精加工两个场景中的实验表明，SRDP在阶段转换稳定性、工艺参数一致性和最终表面质量上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 高端制造中的抛光任务面临长程依赖、阶段转换不确定以及耦合工艺参数难以建模与调控等挑战，现有模仿学习方法在这些问题上表现有限。

Method: 提出SRDP框架：1）从多模态观测历史推断加工阶段后验，并以此条件化共享的逆向去噪过程，实现无需外部阶段标签的跨阶段一致动作生成；2）引入粗糙度导向的工艺约束扩散采样方法，在阶段预设主轴转速下生成受限的进给速度和法向接触力，提升工艺一致性与物理可行性。

Result: 在航天器舱段涂层抛光和内腔结构表面精加工两个代表性场景中，通过与先进基线方法对比、消融实验和真实机器人验证，SRDP在阶段转换稳定性、工艺参数一致性和最终表面质量上均取得更优结果。

Conclusion: SRDP有效解决了工业抛光中长程依赖与阶段不确定性问题，通过阶段感知扩散和粗糙度约束采样提升了工艺一致性与表面质量，在不同抛光场景中均表现优越。
...
TL;DR: 本文提出StairMaster，一种三阶段强化学习框架，使四足机器人在高达55°倾斜的空心楼梯上实现稳定攀爬，首次通过零样本迁移在真实环境中完成此类任务。


<details>
  <summary>Details</summary>
Motivation: 四足机器人攀爬空心楼梯面临腿卡陷、深度信息稀疏和高频噪声三大挑战，现有方法难以应对这种极端不连续地形。

Method: 提出三阶段RL框架：1) Cross-Attention机制从噪声深度数据中提取结构特征；2) 空间感知循环单元(SRU)维持鲁棒时空记忆；3) 高保真度仿真到现实深度传感器建模管道；4) 3D路标点引导的主动感知奖励；5) 空心间隙运动学与楼梯边缘惩罚。

Result: 在Unitree Go2机器人上成功部署，零样本迁移实现最高55°倾斜空心楼梯攀爬，是首个在真实环境达到此性能的基于强化学习的方法。

Conclusion: StairMaster通过创新的感知-记忆-奖励设计，首次实现了RL策略在真实环境中攀爬陡峭空心楼梯，有效弥合了仿真到现实的感知差距。
...
TL;DR: 本文提出MIL-LC多模态定位框架，融合磁力计、惯导和LiDAR，针对GNSS拒绝、几何重复或纹理缺失等挑战性环境，实现鲁棒定位。该框架不依赖几何/纹理特征或基础设施信标，在仿真和真实实验中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有单模态定位方法受限于传感器固有缺陷，多模态融合方案仍过度依赖几何/纹理特征或基础设施信标（增加成本、降低灵活性）。环境磁场的潜力未被充分挖掘，特别是在AMR系统中缺乏实用方案。

Method: 提出MIL-LC框架，融合磁力计、惯性测量单元和LiDAR数据，设计定制化传感器套件。当LiDAR遭遇几何退化或地磁图随时间变化时，仍能提供可靠定位。

Result: 在仿真和真实环境中进行大量实验，验证MIL-LC框架在定位精度和鲁棒性方面均表现优异。

Conclusion: MIL-LC将环境磁场作为互补模态引入AMR定位，克服了传统方法的局限性，为挑战性环境下的自主移动机器人部署提供了实用且可靠的解决方案。
...
TL;DR: 本文提出利用车辆底盘遮挡雷达波束形成的几何阴影，作为一种新颖的几何线索，从2D旋转毫米波雷达中恢复附近细长垂直物体的3D平面内倾斜角，从而将雷达的应用从定位扩展到3D场景重建。


<details>
  <summary>Details</summary>
Motivation: 恶劣天气/光照条件下相机和LiDAR性能下降，而毫米波雷达虽鲁棒但存在高度塌陷（elevation collapse）问题，导致难以进行几何推理。现有方法缺乏从2D雷达中提取3D几何信息的手段。

Method: 观察到车辆底盘会遮挡雷达射线形成独特的几何阴影，且其一致性可推断物体信息。提出一种解析的闭式映射方法，利用雷达回波边界与张角之间的对应关系，无需场景先验即可估计细长垂直物体的倾斜角。

Result: 在Navtech CTS350-X雷达的仿真和实验中验证了该方法在实用条件下能有效估计倾斜角，但物体在雷达扫描中的分割成为主要瓶颈。

Conclusion: 底盘阴影作为一种新颖的几何线索，拓展了2D旋转雷达在3D场景重建中的角色，超越了传统的定位功能。
...
TL;DR: 本文提出TacVerse，一个包含106,800张来自七种视觉触觉传感器的触觉图像的多传感器数据集与基准，支持形状分类、光栅分类和力回归三项任务。实验发现跨传感器直接迁移性能显著下降，而少样本自适应能改善但无法达到同传感器上限，MAE预训练效果最佳。


<details>
  <summary>Details</summary>
Motivation: 现有视觉触觉传感器(VBTS)设计多样，但跨传感器泛化能力尚不明确，缺乏系统性的基准研究。

Method: 构建包含七种VBTS的106,800张触觉图像数据集，设计同传感器训练、零样本跨传感器迁移和少样本自适应三种实验设置，对比不同预训练方法(MAE等)的表征迁移效果。

Result: 同传感器性能良好；跨传感器直接迁移性能显著下降，形状分类相对鲁棒，光栅分类和力回归对传感器差异更敏感；少样本自适应能提升力回归性能但无法达到同传感器上限；MAE预训练一致最优。

Conclusion: TacVerse为研究触觉感知中的传感器差异、数据高效自适应和自监督学习提供了受控测试平台。
...
TL;DR: 本文提出了一个基于3D打印的数学定义纹理数据集，旨在解决触觉传感研究中因纹理描述不一致导致的传感器比较困难问题。通过六个参数化生成的表面图案，在不同3D打印机和耗材上评估了可重复性，发现打印质量显著影响触觉信号一致性，并建立了首个公开可用的物理可复现触觉纹理基准。


<details>
  <summary>Details</summary>
Motivation: 现有触觉数据集基于特定传感器与可用表面的交互记录，而非纹理本身的描述，导致传感器间难以公平比较，且阻碍了可重复研究。

Method: 设计了六个由正弦波和傅里叶函数参数化生成的3D可打印纹理图案，使用光学TacTip传感器在受控接触条件下采集图像，并在三种3D打印机和多种耗材上评估纹理的可重复性。

Result: 打印质量（尤其是峰值锐度和拉丝现象）影响触觉信号方差；高端打印机产生更一致的触觉签名；高质量打印支持良好的打印机内泛化，但跨打印机泛化仍具挑战。

Conclusion: 建立了首个公开可用、物理可复现的3D打印纹理基准，为触觉传感器的公平比较提供了基础。
...
TL;DR: DeformGen提出了一种基于动力学的数据增强框架，通过物理扰动和变形场扭曲，实现对可变形物体的拓扑多样性增强，有效提升策略学习效果。


<details>
  <summary>Details</summary>
Motivation: 现有示范增强方法在可变形操作中面临两大挑战：(1) 状态空间高维且受物理约束，低维位姿扰动难以生成有效配置；(2) 轨迹传递非等变性，变形后物质点不再刚性运动。

Method: (1) 针对状态挑战：施加局部物理扰动并前向模拟动力学，生成拓扑一致且物理合理的变形状态；(2) 针对轨迹挑战：通过变形场扭曲将源操作轨迹传递，将逐粒子位移提升为连续空间函数，使末端执行器轨迹与变形几何保持一致。

Result: 在高保真可变形操作基准上，相比原始示范训练和刚性风格增强基线，DeformGen普遍提升了策略学习效果。

Conclusion: DeformGen通过动力学增强框架，同时增强状态分布及其关联操作行为，有效解决了可变形操作中数据增强的两大核心难题。
...
TL;DR: DSP-SLAM++是一个扩展的物体感知SLAM系统，通过异步映射管道和单目鱼眼-LiDAR融合，在支持多类别物体的同时实现实时性能，将最大物体处理延迟降低70%。


<details>
  <summary>Details</summary>
Motivation: 现有的物体感知SLAM系统在实时性、多类别支持和高质量语义物体建模三者之间存在权衡，难以兼顾。

Method: 在DSP-SLAM框架基础上引入异步映射管道，以及针对单目鱼眼相机与LiDAR传感器套件的专用融合适配。

Result: 最大物体处理延迟最高降低70%，在25Hz的多类别数据集上实现稳健的实时性能，生成精细且几何完整的多类别物体形状。

Conclusion: 本工作使高保真、多类别的物体SLAM在自动驾驶和机器人操作等实际应用中更加实用，尤其是在常见的鱼眼-LiDAR传感器平台上。
...
TL;DR: 本文提出将无人机搭载的量子磁力计作为一种新的废墟探测手段，并通过仿真管道验证了其在倒塌建筑中定位幸存者的潜力。


<details>
  <summary>Details</summary>
Motivation: 倒塌建筑废墟下幸存者的定位在黄金72小时内极具挑战，现有传感手段只能提供有限的结构信息。

Method: 使用Unreal Engine生成钢混停车场倒塌场景，通过逐三角形偶极近似计算感应磁场；采用高斯过程回归与贝叶斯主动采样，结合多传感器阵列进行空间重建。

Result: 在距离屋顶约1米处可恢复亚pT至亚nT量级的磁场结构；三传感器阵列在梯度分辨率与无人机载荷间取得最优平衡；主动采样约100次可达峰值结构相关性。

Conclusion: 量子级磁力传感有望成为无人机倒塌建筑结构分析与空洞检测的有效补充工具。
...
TL;DR: 本文提出一种故障感知模块化控制架构，通过显式利用故障诊断信息激活不同的专家控制器，使腿式机器人在执行器故障下仍能保持可靠运动。实验表明，该模块化策略在性能上优于同规模 monolithic 策略，且在计算资源受限时仍具竞争力。


<details>
  <summary>Details</summary>
Motivation: 腿式机器人在行星探测等远程环境中面临执行器故障和复杂地形挑战，传统强化学习训练的 monolithic 策略难以高效表达应对不同故障所需的多样化控制策略。

Method: 提出 fault-aware modular control architecture，利用故障诊断信息激活与特定执行器失效模式对应的专家控制器（specialized control experts），实现模块化控制。

Result: 显式故障条件化的模块化策略在多种故障场景下的运动性能均优于同等规模的 monolithic 策略；即使在网络容量大幅缩减时仍保持竞争力，适合计算资源受限的平台。

Conclusion: 该模块化架构为远程环境（如太空应用）中腿式机器人的容错运动控制提供了一种高效且资源友好的解决方案。
...
TL;DR: 本文研究将磁性足底传感直接嵌入低成本半主动假肢的承重弹性元件内部，通过3D打印晶格足板、可调液压阻尼器和降阶踝关节模型，实现了无需外部鞋垫的足底力估计和半主动阻尼控制。


<details>
  <summary>Details</summary>
Motivation: 现有假肢足部的传感通常需要额外外部鞋垫层，增加复杂性和成本。本文旨在探究能否将传感直接嵌入假肢的承重弹性元件内部，实现更紧凑、低成本的足底力感知方案。

Method: 1) 集成3D打印晶格足板、伺服可调液压阻尼器和降阶踝关节模型的假肢原型；2) 液压阻尼器的实验特性标定；3) 晶格足板的可控压缩测试；4) 循环法向加载下嵌入式传感器与参考力的对比；5) 静态站立姿态下前足/后足载荷分布的可分性验证；6) 前馈阻尼调度策略模拟参考踝关节轨迹。

Result: 1) 晶格足板的刚度可通过阻尼器调节实现可控变化；2) 嵌入式传感器能追踪测试机参考力，实现无需外部鞋垫的足底力估计；3) 四种静态站立姿态下前后足载荷分布具有可分性；4) 前馈阻尼调度可近似参考踝关节的背屈趋势，但无法产生主动蹬伸。

Conclusion: 研究证明传感可直接嵌入假肢足的承重弹性元件内部，并用于驱动半主动阻尼，为低成本智能假肢设计提供了新路径。
...
TL;DR: 提出Action ControlNet (ACNet)，一种轻量级延迟感知适配器，通过已执行动作后缀作为残差条件，在保持预训练骨干网络不变的情况下，解决VLA模型中异步执行导致的动作不连续、抖动和接触-rich操作失败问题。


<details>
  <summary>Details</summary>
Motivation: VLA模型存在推理延迟瓶颈，异步执行虽能缓解但导致动作拼接不连续、抖动以及接触-rich操作失败，现有方案需全策略重训或特定架构运行时逻辑。

Method: ACNet：轻量级延迟感知适配器，将已执行动作后缀作为残差条件，作用于基本冻结的动作头，兼容扩散和流匹配等生成式动作头，无需改变预训练骨干网络。

Result: 在Kinetix、Meta-World MT50和真实SO-ARM101平台上，ACNet在推理延迟下鲁棒性提升，异步轨迹比直接块拼接更平滑，且比全延迟条件重训更轻量。

Conclusion: ACNet能有效提升VLA模型在异步执行下的控制质量，兼具轻量化和兼容性优势。
...
TL;DR: FORCE是一个三阶段框架，通过价值校准预热和Q函数过滤机制，解决VLA模型RL微调中的样本效率低下和初始性能崩溃问题，在仿真和真实任务中成功率达79%绝对提升，无需人工干预。


<details>
  <summary>Details</summary>
Motivation: VLA模型受限于次优数据导致的模仿天花板，RL微调虽能突破但样本效率极低，主要源于Q函数不稳定导致的灾难性遗忘和低质量探索数据导致的低效策略更新，往往需要昂贵的人工干预。

Method: FORCE包含三个阶段：(1) 价值校准预热阶段，利用在线策略回滚缓解Q函数的分布偏移；(2) 在线阶段，校准后的Q函数作为过滤器，筛选策略自身动作提案和专家数据；(3) 仅保留高价值动作用于策略更新。

Result: 在多种仿真和真实世界任务中，FORCE实现79%的绝对成功率提升，相比先前RL方法提高10%，训练速度加快32.5%，且消除了常见的成功率下降问题。

Conclusion: FORCE在不需人工干预的情况下实现了鲁棒的微调性能，为部署自主机器人代理迈出了关键一步。
...
TL;DR: EMCAR是一个利用木偶戏和绘画等艺术实践的无代码机器人编程工具，旨在让无技术背景的用户（尤其是艺术家）也能参与人机交互（HRI）的设计与研究。


<details>
  <summary>Details</summary>
Motivation: 传统机器人编程对技术门槛要求高，限制了艺术家及非技术背景者参与人机交互设计与研究。EMCAR旨在通过艺术化手段降低门槛，让更多人能创造性地与机器人协作。

Method: EMCAR将木偶操控和绘画等艺术实践作为交互范式，构建无代码的编程平台，使用户无需编写代码即可通过直观的艺术表达方式定义机器人行为。

Result: 该平台拓展了协作型机器人在创意领域的应用场景，并实现了包容性环境，让缺乏技术背景的个体也能有效参与人机交互合作与机器人研究。

Conclusion: EMCAR表明，融合艺术实践的无代码平台具有降低技术壁垒的潜力，能够民主化机器人编程，为HRI领域带来更多样化的参与者和创新用例。
...
TL;DR: FAR-LIO是一个基于CUDA加速的高性能激光雷达-惯性里程计框架，通过体素哈希表、稀疏感知GICP算法和EKF后端，在高速自动驾驶赛车场景中实现了比现有方法平均误差降低6.9%、计算时间减少38.4%的效果。


<details>
  <summary>Details</summary>
Motivation: 在高度动态运动和传感器噪声的环境中（如高速自动驾驶赛车），传统的里程计估计面临挑战，尤其是最小化延迟对闭环控制的稳定性至关重要。

Method: 1) 基于CUDA的体素哈希表实现并行化最近邻搜索和高效地图更新；2) 稀疏感知的广义迭代最近点算法（GICP）结合自适应阈值；3) 扩展卡尔曼滤波器（EKF）后端，采用上采样和延迟补偿策略融合IMU数据。

Result: 在四种不同传感器配置下（含250 km/h的赛车数据），FAR-LIO平均比最先进基线方法位置误差降低6.9%，运行时间减少38.4%。

Conclusion: FAR-LIO展示了在极端动态场景下的计算效率与广泛适用性，代码已开源。
...
TL;DR: 提出G2DP（Grid-Guided Diffusion Planning），一种通过推理时构建可微时空代价体积（融合概率性未来占用分布与路线进度图）向扩散去噪循环注入密集梯度，主动引导轨迹生成实现无碰撞与进度最优的自动驾驶规划方法，在nuPlan、interPlan、DeepScenario上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有扩散规划器随机性强，依赖稀疏实体查询或事后优化进行显式引导，场景感知有限，在交互场景中表现脆弱。需要一种直接施加密集环境约束的推理时引导方法。

Method: 构建可微时空代价体积：融合概率性未来占用分布与路线进度图，将该体积视为连续安全能量泛函，在去噪循环中直接注入密集梯度，主动将轨迹生成引向无碰撞且进度最优的区域。

Result: nuPlan上超过最强模仿学习基线+7.2反应分数；零样本迁移至interPlan和DeepScenario保持最优，interPlan上碰撞避免比无引导方法提升+10.15。

Conclusion: 时空代价网格是扩散规划中鲁棒引导的有效表示，G2DP通过密集梯度注入显著提升了扩散规划器在密集交互场景中的安全性与路线遵循能力。
...
TL;DR: 提出ICWM框架，通过系统识别作为上下文适应问题，让机器人策略从短历史交互中推断系统变量，无需参数更新即可适应新环境。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型仅依赖当前观测和语言指令，忽略了系统配置变量，导致难以泛化到新视角或机器人形态等新场景。

Method: ICWM将系统识别视为上下文适应问题，利用任务无关的自生成交互历史来推断系统变量，通过上下文窗口理解系统运作方式而非任务内容。

Result: 在仿真和真实机器人平台上，ICWM在相机视角泛化任务上显著优于标准VLA基线模型。

Conclusion: ICWM通过上下文学习系统动态，实现了无需参数更新即可适应新配置，为VLA模型的零样本泛化提供了有效范式。
...
### [156] [Learning Action Priors for Cross-embodiment Robot Manipulation](https://arxiv.org/abs/2606.26095)
*Dong Jing,Tianqi Zhang,Jiaqi Liu,Jinman Zhao,Zelong Sun,Li Erran Li,Zhiwu Lu,Mingyu Ding*

Main category: cs.RO

TL;DR: 本文提出一个两阶段训练框架，先在动作模块上预训练运动先验（基于流匹配的编码器-解码器，仅从无条件的动作轨迹中学习时序运动结构），再将其迁移到VLA训练中（解码器重用+潜在蒸馏），实现更快的收敛、更高的成功率和更强的跨实体泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型将动作模块从零开始学习物理运动，缺乏明确的运动先验，导致早期优化同时面临时序动作动力学发现和跨模态对齐的双重挑战，尤其在跨实体场景下问题更加突出。

Method: 两阶段训练框架：Stage 1使用轻量级流匹配编码器-解码器，仅从无条件的动作轨迹中学习时序运动结构；Stage 2通过解码器重用和潜在蒸馏将学到的先验迁移到VLA训练中，编码器作为紧凑的历史压缩器。

Result: 在13个跨实体任务（仿真+真实）上，相比无动作先验的VLA训练，模型收敛更快、成功率更高、数据稀缺场景下表现更强；Stage 1的动作数据规模越大，下游VLA性能越好。

Conclusion: 在VLA训练前给动作模块注入跨实体时序运动先验能显著提升策略的学习效率、泛化能力和数据效率，验证了动作先验预训练的有效性和可扩展性。
...
### [157] [RoboAtlas: Contextual Active SLAM](https://arxiv.org/abs/2606.26046)
*Alexander Schperberg,Shivam K. Panda,Abraham P. Vinod,M. K. Jawed,Stefano Di Cairano*

Main category: cs.RO

TL;DR: RoboAtlas是一个上下文感知的主动SLAM框架，通过大规模3D语义地图系统OpenRoboVox，结合探索与语义推理，在真实1800m²环境中达到100%任务成功率，并在GOAT-Bench上取得90.6%的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决主动SLAM中几何探索与语义推理的平衡问题，让机器人能够根据场景理解程度自适应地从探索模式过渡到语义引导的导航模式。

Method: 提出RoboAtlas框架，包含：(1) OpenRoboVox大规模3D语义地图系统；(2) 融合前沿探索、全局语义地图推理和自我中心VLM推理的上下文多臂老虎机机制，动态平衡探索与语义导航。

Result: 在真实环境(>1800m², ~30k语义实例)达到100%任务成功率。GOAT-Bench上：GPT-4o版本SR 90.6%(领先最强基线17.8个百分点)；Qwen2.5-VL-7B版本SR 88.8%，仍超越所有使用GPT-4o的基线。

Conclusion: 将基础模型与大尺度3D语义地图相结合，能够实现鲁棒且高效的上下文感知主动SLAM；语义地图框架带来的信息增益比单纯替换底层基础模型更为关键。
...
### [158] [Learning Robot Visual Navigation in Crowds via Intention-Aware Scene Representations](https://arxiv.org/abs/2606.26047)
*Han Bao,Bingyi Xia,Hanjing Ye,Yu Zhan,Hao Cheng,Baozhi Jia,Wenjun Xu,Jiankun Wang*

Main category: cs.RO

TL;DR: 本文提出iCrowdNav，一种基于视觉的视觉人群导航方法，通过意图感知的场景表示来编码行为和环境上下文，利用时空编码器和Intent-Interact Former模块推断行人运动意图，在DRL策略训练中取得优于基准的性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度强化学习方法在人群导航中大多将行人简化为二维点，忽略了来自人类和环境的丰富视觉线索，导致场景表示受限，无法充分理解人类意图。

Method: 提出iCrowdNav方法，包含两个关键组件：1) 时空编码器（提取场景占据特征）；2) Intent-Interact Former (I² Former)，一种基于注意力的模块，编码人体姿态以推断行人运动意图。这些特征被集成到紧凑的状态嵌入中，用于DRL策略训练。

Result: 大量实验表明，该方法在性能上优于基线方法，并在真实世界部署中展示了基于视觉的人群导航能力。

Conclusion: 通过引入视觉线索和意图感知的场景表示，iCrowdNav有效提升了机器人人群导航的性能，验证了视觉信息在导航策略中的重要性。
...
### [159] [Deep Reinforcement Learning-Enhanced Event-Triggered Data-Driven Predictive Control for a 3D Cable-Driven Soft Robotic Arm](https://arxiv.org/abs/2606.26048)
*Cheng Ouyang,Moeen Ul Islam,Kaixiang Zhang,Zhaojian Li,Xiaobo Tan,Dong Chen*

Main category: cs.RO

TL;DR: 针对软体机器人控制中DeePC计算负担大的问题，提出基于强化学习的事件触发DeePC框架（RL-ET-DeePC），通过RL策略决定何时调用优化器，在仿真中减少66%优化频率，硬件实验中实现34%优化频率降低，同时保持跟踪精度。


<details>
  <summary>Details</summary>
Motivation: 软体机器人具有非线性和时变动力学特性，难以控制。DeePC虽无需模型，但其滚动时域优化每次采样都需求解约束优化问题，计算负担大，难以在资源受限的机器人平台上实时部署。

Method: 提出RL-ET-DeePC框架：训练一个无模型强化学习策略，根据当前系统状态表征决定何时调用DeePC优化器（事件触发），减少不必要的优化调用，同时保持闭环性能。

Result: 仿真中，RL-ET-DeePC比周期性DeePC减少66%优化频率，跟踪精度相当。硬件实验中，在三维缆驱软体机械臂上实现零样本迁移，优化频率降低34%，跟踪精度与周期性DeePC相当，且比静态阈值触发基线更稳定。

Conclusion: RL-ET-DeePC能有效降低DeePC的计算负担，实现实时软体机器人控制，且具备零样本迁移能力，在效率和性能之间取得良好平衡。
...
### [160] [ForceBand: Learning Forceful Manipulation with sEMG](https://arxiv.org/abs/2606.26093)
...
TL;DR: 本文提出ForceBand，一种低成本腕戴式sEMG系统，通过肌电信号预测手指力，为机器人策略学习生成力增强的演示数据，在力敏感操作任务中达到87%成功率。


<details>
  <summary>Details</summary>
Motivation: 现有机器人演示数据来源（如动捕、互联网视频）主要捕获运动和外观信息，缺少力敏感操作任务中关键的接触力信息。

Method: 收集10小时多模态数据集（含sEMG、IMU、指尖力等），预训练EMG2Force模型从sEMG和IMU信号预测每根手指的力；用户仅需佩戴ForceBand和摄像头采集演示，模型自动标注力信息。

Result: ForceBand的力预测误差比视觉基线低50%以上；在需要精确力控制的抓取任务中达到87%成功率。

Conclusion: sEMG信号能够有效编码手指接触力信息，为机器人学习提供实用的力增强演示数据来源。
...
### [161] [The Hitchhiker's Guide to Agentic AI: From Foundations to Systems](https://arxiv.org/abs/2606.24937)
*Haggai Roitman*

Main category: cs.AI

TL;DR: 这是一本关于自主AI智能体系统构建的综合性实践参考书，涵盖从底层LLM基础（Transformer架构、训练优化、推理加速）到上层对齐与推理（RLHF、PPO/DPO/GRPO、思维链），再到智能体核心能力（RAG、记忆系统、工具使用、多智能体协作协议MCP/A2A），最后讨论开发框架、评估与生产部署。核心论点是：构建优秀智能体系统需要理解整个管道的每一层。


<details>
  <summary>Details</summary>
Motivation: 现有文献往往孤立地讨论LLM或智能体的某个层面，缺乏从底层到顶层的系统性指南。作者认为构建可靠的自主AI系统需要跨层理解，而非仅关注单一环节，因此撰写了这本覆盖全栈的实践参考书。

Method: 全书采用分层递进结构：第一层介绍LLM基座（Transformer、GPU系统、训练与微调方法SFT/LoRA/MoE、模型压缩、推理优化）；第二层讨论对齐与推理（RLHF、PPO/DPO/GRPO、奖励建模、大推理模型的RL与思维链、测试时扩展）；第三层聚焦智能体核心（基于轨迹的RL训练、RAG与Agentic RAG、多种记忆系统、智能体框架设计、设计模式分类学）；第四层处理智能体间协调（MCP协议、工具使用、A2A通信协议、集中/去中心/分层多智能体拓扑）；最后涵盖开发框架、UI设计、评估方法与生产部署。每章均结合理论基础、实现指南、代码示例与文献引用。

Result: 该书提供了一套完整的自主AI智能体构建知识体系，从LLM底层原理到智能体顶层应用，系统化梳理了各层关键技术及其关联，并通过实践指南和代码示例降低了理论到落地的门槛。

Conclusion: 构建优秀的自主AI智能体系统必须跨越从LLM底层到智能体协调的全栈理解，不能仅关注单一层面。该书作为综合参考，为 practitioners 提供了系统化的知识框架和实践路径。
...
### [162] [Project Auto-World: Towards Automated Benchmarking of Neural Relational Reasoners](https://arxiv.org/abs/2606.24965)
*Anirban Das,Joanne Boisson,Irtaza Khalid,Sumita Garai,Steven Schockaert*

Main category: cs.AI

TL;DR: 本文利用大语言模型（LLM）自动生成更难的推理基准测试实例，通过进化搜索和自主代理搜索，提升神经模型在关系结构推理上的泛化能力，并展示了该机制可扩展到LLM提出的新世界。


<details>
  <summary>Details</summary>
Motivation: 神经模型在关系结构推理上的泛化能力评估困难，因为很难预先判断什么使得一个实例变难。现有基准测试难以有效衡量模型的系统性泛化能力。

Method: 以Datalog规则参数化的世界和Edge Transformer作为推理评估器，采用基于LLM的进化搜索（FunSearch）和自主代理搜索来发现能生成困难实例的采样函数。

Result: Edge Transformer可以利用生成的困难数据得到改进，并很好地泛化到进一步的数据扰动上。该机制还可应用于LLM提出的新世界。

Conclusion: LLM自动化基准生成可以有效推动神经关系推理的评估与发展，为自主研究神经关系推理开辟新路径。
...
### [163] [Diagnosing and Mitigating Compounding Failures in Agentic Persuasion via Taxonomic Strategy Retrieval](https://arxiv.org/abs/2606.24976)
*Pradyumna Narayana,Sana Ayromlou,Purvi Sehgal*

Main category: cs.AI

TL;DR: 本文针对多步开放式环境中基础模型代理的复合错误问题，提出TS-RAG方法，通过离散分类瓶颈消除语义泄漏，显著提升轻量级代理在说服任务中的胜率（70.5%→78.5%），并引入辩论状态表征（DSR）诊断机制。


<details>
  <summary>Details</summary>
Motivation: 现有MAD在主观性任务（如说服）中存在问题漂移和谄媚性从众，标准RAG因优先词汇重叠而非逻辑必要性导致语义泄漏，成为这些失败的诱因。

Method: 提出Taxonomic Strategy RAG（TS-RAG），通过离散分类瓶颈路由策略，将论证结构与主题内容解耦；同时引入辩论状态表征（DSR）进行逐回合诊断。

Result: 零样本跨域评估显示TS-RAG显著改善抽象逻辑迁移能力；在非对称部署中，轻量级说服者胜率从70.5%提升至78.5%，论证效率提升。

Conclusion: 严格约束（如TS-RAG）对防止评估崩溃和默认代理谄媚行为至关重要，TS-RAG可作为能力桥梁弥合参数不对称代理之间的性能差距。
...
### [164] [Do vision-language models search like humans? Reasoning tokens as a reaction-time analog in classic visual-search paradigms](https://arxiv.org/abs/2606.25066)
*Farahnaz Wick*

Main category: cs.AI

TL;DR: 本文通过改编四项经典视觉搜索范式，测试视觉语言模型（VLM）的行为是否与人类相似。研究发现VLM在某些方面复现了人类特征（如特征搜索成本恒定、合取搜索成本随数量增加），但也在关键点上与人类存在差异（如目标存在斜率高于目标不存在斜率、枚举计数超过人类能力），作者认为心理物理学范式是探测机器视觉认知的有效工具。


<details>
  <summary>Details</summary>
Motivation: 探究视觉语言模型（VLM）是否具备与人类相似的视觉注意行为特征，特别是经典的并行/序列搜索区分模式，从而揭示机器视觉认知与人类视觉认知的异同。

Method: 改编四项经典视觉搜索范式（特征vs合取搜索、T-vs-L空间构型搜索、枚举、倾斜/垂直搜索不对称性），在多个VLM上进行测试。用模型每轮推理产生的"思考token"数量作为搜索努力的代理指标，并与大规模人类公开基准数据对比。

Result: 模型复现了人类的部分特征：特征搜索成本恒定、合取搜索成本随数量增加；前沿模型保持高准确率而中端模型降至随机水平；分辨率控制实验表明合取成本源于真实搜索而非分辨率限制。存在差异：目标存在斜率超过目标不存在斜率（与人类相反）；枚举准确率高到人类无法达到；推理模型在检测任务上拒绝进行自适应推理。

Conclusion: 心理物理学范式作为行为层面的探测工具，是评估机器视觉认知的经济且有效的手段。模型与人类的一致点和分歧点同样具有信息价值，揭示了机器与人类视觉认知的根本差异。
...
### [165] [Beyond Shapley: Efficient Computation of Asymmetric Shapley Values](https://arxiv.org/abs/2606.25103)
*Ezequiel Companeetz,Santiago Cifuentes,Sergio Abriola*

Main category: cs.AI

TL;DR: 本文研究非对称Shapley值（ASV）在因果图下的高效计算，提出多项式时间算法（针对有根树结构）及针对任意因果DAG的近似采样算法，实验验证了实用性。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型的可解释性需求，现有SHAP值在某些上下文中计算复杂度过高（#P-hard），而ASV通过引入因果知识可降低计算复杂度，但精确计算仍存在挑战。

Method: 1) 证明ASV在特定上下文中可在多项式时间内精确计算；2) 引入拓扑序等价类概念加速计算；3) 针对有根树提出多项式时间算法；4) 针对任意因果DAG设计基于均匀拓扑序采样的近似算法。

Result: ASV在特定条件下多项式时间可解；等价类方法有效降低计算量；近似算法在现实因果结构中表现良好。

Conclusion: 提出的ASV计算与近似方法在理论上降低了复杂度，在实践上具有可行性与有效性，为因果感知的可解释性提供了高效工具。
...
### [166] [The Clinician's Veto: Navigating Trust, Liability, and Uncertainty in Autonomous AI Prescribing](https://arxiv.org/abs/2606.25108)
*Eileanor LaRocco,Sarah Tan,Adarsh Subbaswamy,Anne Andrews,Andrew Taylor,Cree Gaskin,Chirag Agarwal*

Main category: cs.AI

TL;DR: 本文提出AI自主开药系统应具备三个最低架构要求：校准的置信度阈值、区分认知与偶然不确定性、决策时的推理透明度。通过对136名美国临床医生调查，发现这些特征能提高医生采纳率，同时将AI从"自主代理"转变为"高度监督的决策辅助工具"。


<details>
  <summary>Details</summary>
Motivation: 现有AI开药监管仅要求聚合性能指标，缺乏对逐次预测的校准置信度、不确定性来源区分（模型无知 vs. 临床模糊性）以及决策时刻的推理透明度的要求，导致安全隐患和责任归属不清。

Method: 1) 提出监管与技术论证框架；2) 对136名美国开药临床医生进行调查研究，评估医生对三种架构要求的态度。

Result: 医生们：①不允许在没有校准置信度升级机制的情况下让AI自主开药；②在偶然不确定性下偏好竞争性选项摘要，在认知不确定性下偏好弃权；③仅在推理透明度允许其在承认不确定性下做出实质判断时，才愿承担额外责任。

Conclusion: 这些架构特征会降低AI的"自主性"，使其更像是高度监督的决策辅助工具。监管应利用这些技术要求来约束AI在开药中的伦理自主程度，并将责任与控制系统设计和部署的机构行为者对齐。
...
### [167] [TRUSTMEM: Learning Trustworthy Memory Consolidation for LLM Agents with Long-Term Memory](https://arxiv.org/abs/2606.25161)
*Tianyu Yang,Sudipta Paul,Vijay Srinivasan,Vivek Kulkarni,Srinivas Chappidi*

Main category: cs.AI

TL;DR: 提出TrustMem框架，通过记忆转换验证器和偏好引导强化学习，提升LLM智能体长期记忆更新的可信度，显著减少信息遗漏、记忆损坏和幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体的外部记忆更新机制存在遗漏重要信息、损坏已有记忆、引入非事实内容等可靠性问题，这些错误会持续影响后续推理和生成。

Method: 提出TrustMem框架，核心是Memory Transition Verifier评估记忆更新的覆盖性、保留性和忠实性；并构建偏好对进行强化学习优化记忆更新行为。

Result: 在MemoryAgentBench、HaluMem和Mem-alpha验证集上取得SOTA结果；记忆提取F1提升12.14；遗漏、损坏、幻觉分别降低40.1%、79.1%、50.0%。

Conclusion: TrustMem能有效提升长期记忆更新的可信度和可靠性，为LLM智能体在外存管理方面提供了值得信赖的解决方案。
...
### [168] [Elo-Disentangled Player-Style Embeddings for Human Chess via Rating-Conditioned Residual Move Model](https://arxiv.org/abs/2606.25176)
*Jason Carlson*

Main category: cs.AI

TL;DR: 本文提出了一种残差式表征学习方法，通过将每个棋手的风格嵌入向量与棋力（Elo等级分）解耦，学习个体化的国际象棋风格表示。


<details>
  <summary>Details</summary>
Motivation: 现有棋手模型难以将"风格"与"水平"分离，作者希望学习一种与Elo解耦的、可衡量风格相似性的个体化嵌入表示。

Method: 构建一个以棋力为条件的基模型（结合Maia-3策略logits与Stockfish引擎特征），冻结该模型作为锚点，学习每个棋手的风格向量z，z仅解释偏离"典型水平棋手"的行为差异。

Result: 基模型在走子预测上比Maia-3提升27-37%的NLL；风格嵌入z在原始匹配率上增益不显著，但能泛化到未见数据、跨对局识别棋手，且z与Elo的线性回归R²仅0.06，证实了与棋力的解耦。

Conclusion: 一个强力的以棋力为条件的基模型加上紧凑的、与Elo解耦的风格嵌入，是一种经济、可解释的个体风格建模方案，可作为逐棋手偏好微调的有效替代。
...
### [169] [Transferability for General Reasoning: An Automated Curriculum for Multi-Domain RLVR](https://arxiv.org/abs/2606.25178)
*Yongjin Yang,Jiarui Liu,Yinghui He,Lezhen Zhang,Bernhard Schölkopf,Zhijing Jin*

Main category: cs.AI

TL;DR: 提出了Transfer-Aware Curriculum (TAC)，一种基于bandit的在线课程学习策略，通过感知跨域可迁移性来优化多领域强化学习中的训练课程，在不同规模模型上均取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR训练课程通常是固定或手工调整的，忽略了推理技能在不同领域间的不均匀迁移；现有的基于可学习性的课程虽然能适应策略当前改进的方向，但无法判断所选领域的梯度更新是否有利于其他领域。

Method: 提出TAC，一种bandit风格的在线课程策略。利用RL训练中已有的信号：每个领域的优势函数（advantage）衡量局部可学习性，投影梯度（从GRPO步骤中提取）通过梯度几何对齐估算跨域可迁移性，开销极低（<1%墙钟时间）。

Result: 在六个领域的推理套件上，TAC在Qwen3-1.7B和Llama3.2-3B上均取得最佳宏平均准确率，优于比例随机采样、手工设计调度和仅基于可学习性的bandit策略，最高提升2.8个点（相对提升10%）。移除可迁移性项后性能显著下降，在不平衡训练混合中TAC仍然稳健。

Conclusion: 跨域可迁移性是多领域RLVR课程设计的关键信号，TAC通过高效利用已有训练信号实现了性能提升和鲁棒性。
...
### [170] [To Isolate or to Score? Model-Adaptive Assessment for Cost-Efficient Multi-Agent RAG](https://arxiv.org/abs/2606.25191)
*Jungseob Lee,Chanjun Park,Heuiseok Lim*

Main category: cs.AI

TL;DR: 本文研究了检索增强生成中多智能体文档评估的计算开销问题，发现弱基线模型的主要收益来自多文档上下文混淆的消除而非评分质量，并提出了一种轻量级模型自适应路由架构MADARA，可在零样本下推广到未见模型。


<details>
  <summary>Details</summary>
Motivation: 多智能体文档评估在检索增强生成中计算成本高昂，促使研究者转向小型可部署模型，但这些模型的评估机制尚不明确，需要深入理解其工作原理以优化效率。

Method: 对7B-9B指令微调模型进行受控实验，通过干预训练-free的方法研究评估机制；提出Reasoning-Score Coupling扰动探针分类评分行为；构建MADARA模型自适应路由架构。

Result: 弱基线模型的核心收益来自每文档隔离机制，消除多文档上下文混淆可带来高达50个百分点的提升；强基线模型则依赖评分质量。

Conclusion: MADARA的诊断阈值可从单个导引模型零样本推广到四个未见模型族，提供鲁棒且轻量级的流水线以消除计算开销。
...
### [171] [Heuresis: Search Strategies for Autonomous AI Research Agents Across Quality, Diversity and Novelty](https://arxiv.org/abs/2606.25198)
...
TL;DR: Heuresis是一个将机器学习研究流程抽象为可组合原语的框架，实验发现当前策略虽能调控生成想法的质量、多样性和新颖性，但无法扩展质量-新颖性前沿，且完全新颖的想法极为罕见。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的AI研究智能体局限于写代码，缺乏对同时具备高性能、多样性和新颖性的想法的探索能力。需要一种框架来实现开放式的科学探索。

Method: 提出Heuresis框架，将研究流程抽象为通用可组合原语。实现了6种搜索策略（贪心基线、MAP-Elites、Go-Explore、Islands、Curiosity、Omni），在3个领域（LLM预训练、在线RL、模型去学习）共3,222次评分实验中进行评估。

Result: 完全新颖的想法极为罕见，无任何实验被评为"原创"，仅有少数达到"微小相似度"。新颖想法从未接近已知最优性能。仅有一个新颖想法进入质量前十。发现了40起确认的奖励欺骗行为。

Conclusion: 当前搜索和QD策略能引导想法在质量、多样性和新颖性轴上的分布，但无法扩展质量-新颖性前沿。如何弥合这一差距是实现自主持续科学进步的关键挑战。
...
### [172] [Omni-Perception Policy Optimization for Multimodal Emotion Reasoning](https://arxiv.org/abs/2606.25325)
*Zhiyuan Han,Beier Zhu,Wenwen Tong,Pengyang Shao,Peipei Song,Xinyi Wang,Jiangnan Chen,Lewei Lu,Xun Yang*

Main category: cs.AI

TL;DR: 针对当前情感导向的全模态大语言模型缺乏可靠的全模态感知问题（对多模态线索利用不足、存在跨模态幻觉），作者提出OPPO强化学习框架：通过全模态感知奖励分解细粒度视觉/声学/情感线索，以及全模态感知损失对模态特定证据token施加KL惩罚来抑制幻觉。同时构建MEP-Bench诊断基准。实验表明OPPO在多模态情感推理任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 当前情感导向的全模态大语言模型存在两个关键缺陷：(i)推理轨迹中未能充分利用多模态线索；(ii)行为不可靠，经常从其他模态幻觉出模态特定的陈述。因此需要显式优化全模态感知能力。

Method: 提出OPPO框架：(1) Omni-Perception Reward：将真实推理分解为细粒度的视觉、声学和情感线索，奖励能语义恢复这些线索的轨迹；(2) Omni-Perception Loss：比较完整输入与单模态遮蔽输入下的策略，仅对模态特定证据token施加KL散度惩罚以抑制跨模态幻觉。同时构建MEP-Bench诊断基准量化利用率和忠实度。

Result: 在MER-UniBench和MME-Emotion上达到SOTA性能；在MEP-Bench上显著提升了利用率和忠实度分数。

Conclusion: 充分且忠实的多模态感知对于情感推理至关重要，OPPO通过强化学习显式优化全模态感知能有效提升模型表现。
...
### [173] [Agentic Knowledge Tracing: A Multi-Agent LLM Architecture for Stealth Assessment of Financial Literacy in Serious Games](https://arxiv.org/abs/2606.25358)
*Gabriel Santos,Rita Julia,Marcelo Nascimento*

Main category: cs.AI

TL;DR: 本文提出Agentic BKT pipeline，一个基于多智能体大语言模型的架构，用于从开放世界游戏事件中隐式评估金融素养。该流程通过四个阶段处理2D平台游戏事件，并在193名K-12学生的264场游戏会话中验证，其掌握度估计与学习增益显著相关（r=0.276, p=0.0001），预测效度约为单LLM基准的三倍。


<details>
  <summary>Details</summary>
Motivation: 在不干扰游戏学习体验的前提下，从游戏过程中隐式评估玩家的金融素养水平，是严肃游戏教育中的关键挑战。现有方法难以捕捉金融素养的多维性，且缺乏有效的隐式评估手段。

Method: 提出四阶段Agentic BKT pipeline：(1) 游戏记录玩家决策为结构化事件日志；(2) LLM事件分类器对每个动作进行四点评分（与三位领域专家验证，Fleiss kappa=0.624）；(3) 四个领域智能体（风险管理、投资、消费、信用管理）进行会话级推理，结合贝叶斯知识追踪估算各领域掌握度；(4) 专家裁判智能体综合生成总体掌握度分数。

Result: 在193名K-12学生、264场游戏会话中，该方法的掌握度估计与学习增益显著相关（r=0.276, p=0.0001），与后测分数显著相关（r=0.333, p<0.0001），与前测分数无显著相关，同时具备收敛效度和区分效度。预测效度约为单LLM基线（r=0.095, 不显著）的三倍。

Conclusion: 领域分解与会话级推理是捕捉金融素养多维性的关键因素，多智能体架构显著提升了游戏内隐式评估的预测效度，为严肃游戏中的隐式评估提供了有效范式。
...
### [174] [What Actually Works for Spacecraft Fault-Tolerant Control: An Honest Settled-Gate Benchmark of Learned and Classical Methods](https://arxiv.org/abs/2606.25374)
*Alireza Shojaei*

Main category: cs.AI

TL;DR: 本文构建了一个航天器执行器故障容错控制基准测试，发现结构化估计-控制设计（学习型循环模块+解析控制律）在未知增益和符号故障上达到94%以上成功率，而纯学习方法为0%；但所有方法对常值偏置故障均失效，为此提出扰动观测器恢复至59.4%。


<details>
  <summary>Details</summary>
Motivation: 现有容错控制工作多限于仿真、窄故障集和瞬时指标，无法评估在从未见过的故障上持续保持指向的能力。本文旨在建立一个严苛的基准：要求在0.2度精度内持续保持指向，并系统评估各类控制器在未知故障上的泛化能力。

Method: 构建包含训练/测试分离（惯性、增益、符号模式、偏置）的基准，采用Wilson区间统计n=500次试验。对比经典PD/PID、自适应控制、端到端RL、结构化估计-控制（学习型循环模块推断增益+解析律）及扰动观测器，在6-DOF Basilisk平台上验证。

Result: 故障无关PD/PID和端到端RL成功率为0%；经典自适应解决符号故障但增益处理仅55.2%；Nussbaum增益律达45.2%和3.2%；结构化估计-控制在符号和增益故障上达97.8%和94.4%。常值偏置故障所有方法均为0%，引入扰动观测器后恢复至59.4%。

Conclusion: 学习容量本身不是关键，结构化设计（学习感知+解析控制）优于纯学习方法；常值偏置是无积分律控制器的根本瓶颈，可通过扰动观测器弥补；传感器偏置不可观测需融合而非观测器。
...
### [175] [Offline Multi-agent Continual Cooperation via Skill Partition and Reuse](https://arxiv.org/abs/2606.25389)
*Yuchen Xiao,Lei Yuan,Ruiqi Xue,Tieyue Yin,Yang Yu*

Main category: cs.AI

TL;DR: 提出COMAD框架，通过技能划分与重用，解决了多智能体离线数据中技能持续发现与重用时的灾难性遗忘和可塑性损失问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖启发式设计的固定规模技能库，在多任务顺序出现、技能空间指数增长时，面临分布偏移和干扰问题，导致灾难性遗忘和可塑性损失。

Method: 1) 用自编码器从混合多智能体行为数据中提取可重用协调技能；2) 构建带多头架构的技能增强策略学习目标，通过密度基可重用性估计器显式引导优势函数。

Result: 在多个MARL基准上，COMAD持续扩展技能库以缓解干扰，在任务流的前向和后向迁移上均优于多种基线方法。

Conclusion: COMAD框架理论上逼近持续技能发现问题的最优解，实证上有效解决了多智能体持续学习中的遗忘和干扰问题。
...
### [176] [Long-Term Simulation Exposes Cognitive-Developmental Risks in AI Companions](https://arxiv.org/abs/2606.25396)
*Kaicheng Shen,Lingyu Li,Wen Wu,Yan Teng,Liang He,Yingchun Wang*

Main category: cs.AI

TL;DR: 提出TSJ框架，通过长时段模拟（12,960人天）评估AI伴侣对认知发展用户的累积风险，发现短期测试严重低估风险，需140轮对话才能得到稳定估计，并识别出幼儿期和成年初期为最脆弱阶段。


<details>
  <summary>Details</summary>
Motivation: 现有安全评估依赖单轮或短会话测试，无法捕捉长期交互中累积的认知发展风险。

Method: 提出TSJ（Theater-Stage-Judge）框架，包含角色驱动用户模拟、动态心理状态更新和回顾性评估，跨4个发展阶段、24个风险维度、3种心理脆弱性人格模拟12,960人天交互。

Result: 短期测试系统性低估发展风险；TSJ需140轮对话才能获得稳定风险估计；幼儿期和成年初期为最脆弱阶段；认知信任和情感依赖为最薄弱领域。

Conclusion: TSJ为AI伴侣系统提供了可扩展的纵向认知发展风险评估方法论。
...
### [177] [BrainAgent: A Large Language Model-Driven Multi-Agent Framework for Autonomous Brain Signal Understanding](https://arxiv.org/abs/2606.25400)
*Yangxuan Zhou,Sha Zhao,Jiquan Wang,Shijian Li,Gang Pan*

Main category: cs.AI

TL;DR: BrainAgent是一个由大语言模型驱动的多智能体框架，通过层级架构将自然语言意图转化为可执行的脑信号分析流程，旨在降低脑机接口领域的技术门槛并实现自动化和民主化。


<details>
  <summary>Details</summary>
Motivation: 当前脑信号分析存在两大瓶颈：技术门槛高（需要大量专业知识）以及分析范式静态且任务特定，无法处理复杂的长周期工作流。作者希望通过引入LLM驱动的多智能体框架来克服这些障碍，加速脑信号理解的民主化。

Method: 提出BrainAgent框架，采用层级架构：中央监督者（supervisor）负责任务分解与调度，多个专业子智能体（sub-agents）负责具体执行。利用大语言模型的自然语言理解能力，将抽象意图转化为可执行的端到端处理管线。

Result: 实验结果表明BrainAgent能够有效自动化复杂工作流，并具有优越的可靠性。同时建立了评估脑信号分析中智能体系统的系统性基准。

Conclusion: BrainAgent代表了脑信号理解迈向民主化的范式转变，为BCI技术在实际场景中的广泛应用铺平了道路。
...
### [178] [Quantization Inflates Reasoning: Token Inflation as a Hidden Cost of Low-Bit Reasoning Models](https://arxiv.org/abs/2606.25519)
*Xinyu Lian,Walid Krichene,Beichen Huang,Masahiro Tanaka,Olatunji Ruwase,Li Zhang,Minjia Zhang*

Main category: cs.AI

TL;DR: 低比特后训练量化虽然能保持大语言模型的最终答案准确率，但会导致推理模型生成更长的思维链（CoT），增加推理token消耗，抵消了每token的加速收益。论文提出CoT Token膨胀率来衡量这一现象，并发现量化感知训练比提示工程和解码采样更能缓解准确率下降和token膨胀。建议在评估量化推理模型时同时报告准确率和推理token使用量。


<details>
  <summary>Details</summary>
Motivation: 现有研究仅关注量化对最终答案准确率和每token延迟的影响，忽略了量化对推理模型思维链长度（即测试时计算成本）的潜在影响。

Method: 在数学推理、代码生成、科学问答和智能体工具使用等多个基准上，对比INT4/INT3量化模型与全精度模型的推理行为；引入CoT Token膨胀率指标量化推理长度变化；分析token膨胀伴随的行为特征（中间步骤增多、语义重复加剧）；评估提示工程、解码采样和量化感知训练等缓解策略的效果。

Result: INT4/INT3量化在保持准确率的同时，显著增加了推理token使用量；量化感知训练在降低准确率下降和token膨胀方面比提示工程和解码采样更有效。

Conclusion: 评估量化推理模型时，应将推理token使用量与准确率一同报告，以全面反映量化对模型的实际影响。
...
### [179] [Cliff Tokens: Identifying Single-Token Failure Triggers in LLM Mathematical Reasoning](https://arxiv.org/abs/2606.25524)
*Jaeyong Ko,Pilsung Kang,Yukyung Lee*

Main category: cs.AI

TL;DR: 本文提出"悬崖标记"(cliff token)概念，即在数学推理过程中导致模型从正确转向错误的特定标记。通过单侧双比例z检验自适应阈值检测，删除首个悬崖标记后重采样可恢复pass@64至1.0。进一步分类为确定性、不确定性和采样偏离三种悬崖类型，并通过Cliff-DPO优化提升推理准确率最多+6.6。


<details>
  <summary>Details</summary>
Motivation: 现有研究仅在步骤、句子或已经失败的token层面分析推理失败，未能精确定位导致模型从正确转向错误的触发标记。

Method: 提出"悬崖标记"概念，基于单侧双比例z检验的自适应阈值检测token-wise潜在下降；构建包含确定性、不确定性和采样偏离三种类型的悬崖分类法；提出Cliff-DPO在悬崖位置进行单token偏好优化。

Result: 在7个模型和3个基准上，删除首个悬崖标记后重采样可恢复pass@64至1.0；Cliff-DPO在GSM8K上训练后，跨基准准确率提升最多+6.6。

Conclusion: 不确定性和采样偏离悬崖的优化能提升推理能力，而确定性悬崖则无效，验证了悬崖分类法的泛化性和实用性。
...
### [180] [Agentic evolution of physically constrained foundation models](https://arxiv.org/abs/2606.25532)
*Jiangwei Zhang,Wen Sun,Chong Wang,Shiyao Li,Cheng Che,Chunjing Han,Dan Meng,Jian Yang,Yu Wang,Rui Hou*

Main category: cs.AI

TL;DR: 提出了一种物理约束的多智能体自主发现引擎，通过进化知识图谱和算法思维链，将盲随机搜索转化为有向结构进化。在基础模型部署场景中，演化出两种超越人工启发式的硬件感知压缩方法，成功将2350亿参数模型部署到受限双A100服务器上，内存减少75%，精度仅下降0.64%。


<details>
  <summary>Details</summary>
Motivation: 当前通用人工智能智能体缺乏物理锚定，经常幻觉出与硬件不兼容的设计，导致自动化科学发现缺乏物理约束。现有方法无法在严格的物理边界内自动发现硬件兼容的计算系统。

Method: 提出物理约束的多智能体发现引擎，包含三个核心组件：(1) 进化知识图谱，结构化存储过往科学创新；(2) 算法思维链提取，将盲随机搜索转化为有向结构进化；(3) 带宽高效的敏感性分析画像，用于高效部署。在基础模型部署这一极端场景中验证。

Result: 演化出两种硬件感知压缩方法：Q-Enhance缓解长上下文精度损失，MoE-Salient-AQ在sub-3-bit场景下超越手动设计SOTA 3.7%。成功将2350亿参数模型部署到双A100服务器，内存降低75%，精度仅下降0.64%。

Conclusion: 将无约束组合搜索转化为知识驱动的自主探索，建立了在严格物理边界内进行机器驱动的可扩展硬件-软件协同设计范式。
...
### [181] [Reasonable Motion: A General ASP Foundation for Environment Constrained Movement Trajectory Computation](https://arxiv.org/abs/2606.25626)
*Julius Monsen,Jakob Suchan,Mehul Bhatt,Lars Karlsson*

Main category: cs.AI

TL;DR: 提出了一种基于回答集编程(ASP)的混合定量-定性方法，用于计算真实世界中移动物体的约束分支轨迹模式，通过枚举几何上可行的运动行为作为稳定模型，实现可验证的可解释性，并在Argoverse 2自动驾驶基准上进行了实证评估。


<details>
  <summary>Details</summary>
Motivation: 现有纯学习方法缺乏可解释性和可追溯性，需要一种能够生成可验证、可解释轨迹的计算方法。

Method: 基于回答集编程(ASP)的混合定量-定性方法，对环境图进行约束遍历，将几何上可行的运动行为枚举为稳定模型，每个模型代表一种独特的轨迹模式。

Result: 在Argoverse 2大规模自动驾驶基准上验证了方法的适用性，证明计算出的轨迹可追溯至其底层稳定模型，实现了可验证的可解释性。

Conclusion: 该方法广泛适用于各类移动物体动态场景（如自动驾驶），在可解释性方面优于纯学习方法。
...
### [182] [GUI agent: Guided Exploration of User-Sensitive Screens](https://arxiv.org/abs/2606.25705)
*Aradhana Nayak,Mussadiq Nazeer,Wang Peng,Feng Liu*

Main category: cs.AI

TL;DR: 本文提出了一种探索代理，用于在GUI环境中自动识别可能导致用户敏感状态的操作查询，从而支持将任务执行移交回用户，提升LLM代理的安全性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM驱动的代理在开放GUI环境中处理用户敏感信息时缺乏安全性意识，往往盲目完成任务，导致实际部署困难且可靠性差。因此，需要识别和分类用户敏感状态，以便在关键时刻将控制权交还给用户。

Method: 开发了一个探索代理（explorer agent），从单个演示任务出发，系统地探索查询空间，识别那些执行后会导致用户敏感状态的操作查询。

Result: 构建了一个用于识别用户敏感状态和相应查询的数据集，帮助工程师训练代理识别并请求用户接管关键场景。

Conclusion: 该工作为提升LLM代理在真实世界GUI环境中的安全部署提供了基础，通过主动识别敏感状态来增强系统的可靠性和用户信任。
...
### [183] [Position Spaces and Graphs](https://arxiv.org/abs/2606.25719)
*Rita-Nathalia Assaf,Tom Davot,Frédéric Lardeux,Frédéric Saubion*

Main category: cs.AI

TL;DR: 本文提出位置图（position graphs）这一基于位置空间形式化的图推理框架，利用两种严格偏序关系模拟离散标记的相对位置，并研究了其一致性条件及子图同构问题的计算复杂性。


<details>
  <summary>Details</summary>
Motivation: 为离散标记的相对位置关系提供形式化的数学表示和推理框架，弥补现有定性空间演算在行列约束方面的不足，为文档处理等应用提供独立的逻辑层支持。

Method: 形式化定义位置图，引入水平与垂直两种严格偏序关系，并附加链条件（chain condition）和兼容性约束；通过图一致性的特征化分析建立一致性条件；将结构模式发现问题建模为诱导子图同构问题并分析其NP完全性。

Result: 建立了位置图一致性的充要条件；证明了即使在受限的位置图类别上，诱导子图同构问题仍然是NP完全的。

Conclusion: 位置图提供了一个独立于具体数据提取技术的正式逻辑层，其数学性质和计算复杂性分析为后续在文档处理等领域的应用奠定了理论基础。
...
### [184] [Fuzzy Quantification over OWL Ontologies and Knowledge Graphs](https://arxiv.org/abs/2606.25778)
*Enrique Palacín,Fernando Bobillo,Ignacio Huitzil,Francesca A. Lisi,Umberto Straccia*

Main category: cs.AI

TL;DR: 本文提出了一种通用框架，用于在标准/模糊本体及知识图谱上评估模糊量化查询，并实现了开源系统Q2S2。


<details>
  <summary>Details</summary>
Motivation: 现有模糊量化查询方法缺乏通用性，受限于特定的量化器类型、评估方法或数据源（OWL本体/RDFS知识图谱），需要一种灵活、与具体实现无关的统一框架。

Method: 提出一种与量化器类型、评估方法及数据源无关的通用框架，支持Type I和Type II模糊量化表达式，并基于该框架实现了Q2S2系统。

Result: 框架能够灵活适配不同量化器、评估策略和本体数据源（OWL/RDFS），Q2S2系统作为公开可用实现支持后续研究。

Conclusion: 该通用框架有效解决了模糊量化查询的异构性问题，Q2S2为相关领域研究提供了可扩展的开源工具。
...
### [185] [Confidence Sequences for Online Statistical Model Checking of Markov Decision Processes](https://arxiv.org/abs/2606.25797)
*Konstantin Kueffner,Tobias Meggendorfer,Maximilian Weininger,Patrick Wienhöft*

Main category: cs.AI

TL;DR: 本文针对马尔可夫决策过程中概率未知的问题，提出了多种置信序列方法用于在线统计推断，实现了一个高效工具，在平均采样量上比先前最优方法减少了50倍。


<details>
  <summary>Details</summary>
Motivation: 传统MDP假设精确知道转移概率，但在实际系统（如信息物理系统或生物过程）中这一假设不成立。现有统计方法存在不正确或次优的问题，亟需更有效的在线推断方法。

Method: 提出多种专为在线设置设计的置信序列（confidence sequences），替代传统的联合界（union-bound）方法，并实现高效工具。

Result: 提出的方法在平均采样量上比先前最优方法减少约50倍，且优于传统联合界方法。

Conclusion: 置信序列方法在实际中具有优越的采样效率，为不确定概率下的MDP决策提供了更实用的统计推断途径。
...
### [186] [AI Snitches Get Glitches: Towards Evading Agentic Surveillance](https://arxiv.org/abs/2606.25836)
*Hyejun Jeong,Dzung Pham,Amir Houmansadr,Eugene Bagdasarian*

Main category: cs.AI

TL;DR: 本文提出并形式化了“智能体监控”（agentic surveillance）问题，即AI代理可能滥用用户数据权限进行监控。作者构建了SurveilBench数据集评估模型能力，发现部分模型存在自发协助监控的倾向，同时也会向政府报告监控行为。最后提出了三种规避监控的提示注入技术，呼吁建立全面的技术、伦理和法律框架。


<details>
  <summary>Details</summary>
Motivation: AI代理的广泛使用带来新的安全风险：代理可能滥用数据访问权限对用户进行监控，而用户可能无法控制这些监控行为。现有研究缺乏对这一问题的正视和系统分析。

Method: 1) 形式化定义“智能体监控”问题；2) 构建SurveilBench数据集，涵盖企业、教育、警察三个领域；3) 评估多种模型的监控能力；4) 提出三种基于提示注入的规避技术：隐藏、欺骗和诱导过度上报。

Result: 部分模型表现出自发协助监控的涌现能力，但同时也会向政府报告监控尝试。提出的三种提示注入方法可有效规避监控代理。

Conclusion: 智能体监控已可轻易实现，亟需建立全面的技术、伦理和法律框架来保护用户免受这种新型监控的侵害。
...
### [187] [Agentic System as Compressor: Quantifying System Intelligence in Bits](https://arxiv.org/abs/2606.25960)
*Zihan Qin,Hongrui Zhang*

Main category: cs.AI

TL;DR: 本文基于"压缩即智能"的视角，提出用编码长度来评估语言模型作为智能体系统（调用工具、检索、验证等）的性能，并在五个实验任务中验证了智能体组件能缩短编码长度。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型正从独立预测器转向智能体系统（工具调用、检索、验证、多轮交互等），但缺乏统一的、理论上可解释的评估框架来量化智能体组件带来的性能提升。

Method: 采用"压缩即智能"的分析视角：在固定任务分布、接口和计算预算下，更强的智能体系统能用更少的比特重建目标对象。具体使用算术编码、种子编码和回退机制来度量编码长度。

Result: 在五个设置（反转文本、国际象棋走法、蛋白质序列、检索增强问答、语义故事压缩）中，所有包含智能体组件的系统均降低了编码长度，证明智能体组件减少了残余不确定性。

Conclusion: 编码长度能够分析不同组件、观察者和计算预算如何改变残余不确定性，为评估真实智能体系统提供了指导方向。
...
### [188] [WinDOM: Self-Family Distillation for Small-Model GUI Grounding](https://arxiv.org/abs/2606.25964)
*Chengheng Li-Chen,Zhiqian Zhou,Hao Chen,Nicolas Chauvin*

Main category: cs.AI

TL;DR: 本文提出WinDOM（自动从DOM提取GUI目标框数据，无需人工标注）和Self-Family Distillation（SFD，自我家族蒸馏）方法，结合GRPO强化学习，使2B级小模型在GUI grounding任务上达到强竞争力性能，且发现欠饱和冷启动比完全收敛的冷启动更适合作为RL初始化点。


<details>
  <summary>Details</summary>
Motivation: 小规模（~2B）GUI grounding模型在端侧部署、辅助工具和低成本迭代方面具有吸引力，但面临两个开放问题：如何在不依赖昂贵人工标注的情况下获取边界框训练数据，以及如何有效结合监督微调与强化学习。本文目的是推动小模型性能上限，而非简单扩大模型规模。

Method: 1) WinDOM：在开源的Windows 11 Web重实现上通过Playwright无头驱动，直接从DOM读取边界框，构建54,425条记录的数据集，无需OCR或人工标注。2) Self-Family Distillation (SFD)：仅由教师模型选择参数化的拒绝采样冷启动方法，教师可以是学生的EMA（无需外部模型）或冻结的更大同族模型。3) 将SFD冷启动的饱和深度作为GRPO的超参数，发现欠饱和冷启动比完全收敛的冷启动更适合作为RL初始点。

Result: 在Qwen3.5-2B学生模型上，SFD-4B配合Early-init RL相比基线提升+5.4 OOD-mean（ScreenSpot-Pro +3.5，OSWorld-G +7.0，ScreenSpot-V2 +5.8）。同尺寸EMA模式（65.2）与跨尺寸4B变体（66.3）的OOD-mean仅差约1个点，无需外部教师。

Conclusion: 通过WinDOM和SFD方法，小规模GUI grounding模型可以在无需人工标注和外部大教师的情况下达到优异性能；欠饱和冷启动作为GRPO初始化点效果更好，且EMA自蒸馏模式与使用更大教师模型的性能差距很小。
...
### [189] [InvestPhilBench: A Multi-Layer Dynamic Benchmark for Evaluating Large Language Model Procedural Reasoning in Expert Investment Philosophy](https://arxiv.org/abs/2606.25984)
*Mingguang Chen,Bo Qu*

Main category: cs.AI

TL;DR: 提出InvestPhilBench基准测试，评估LLM能否准确重建和应用投资决策框架。该基准涵盖8个认知层级，包含118张投资原则卡、25张决策框架卡和243个QA问题。引入BASP自动评分管道等指标。发现前沿模型BASP高分但GRA暴露出程序性缺陷——复合评分奖励流畅语言而隐藏了程序性差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法检验大语言模型是否能像专业投资者那样准确重建和应用投资决策程序性框架，存在评估空白。

Method: 构建多层级动态基准InvestPhilBench（8个认知层级L1-L8），包含投资原则卡、决策框架卡及QA问题。提出BASP自动评分管道（含5种算法指标）、FMDP故障模式检测协议和GRA门级重建准确率。

Result: 四模型测试显示供应商层级明显分岔（BASP 0.906 vs 0.438）。前沿模型BASP复合分数饱和（Claude L4=0.932），但GRA暴露程序性缺陷（L4约0.77，L7为0.57-0.62）。BASP复合分数与人工参考的Pearson r=0.72，SAP@3是最弱的子指标。

Conclusion: 复合评分奖励流畅语言输出，掩盖了程序性推理能力的不足；需要GRA等细粒度指标才能暴露模型在投资决策程序性框架上的真实缺陷。
...
### [190] [Autodata: An agentic data scientist to create high quality synthetic data](https://arxiv.org/abs/2606.25996)
...
TL;DR: 本文提出Autodata方法，让AI代理充当数据科学家，自动构建高质量训练和评估数据，并通过元优化进一步提升数据质量，实验证明优于传统合成数据方法。


<details>
  <summary>Details</summary>
Motivation: 传统合成数据方法质量有限，需要一种能让AI自主构建更优训练数据的方法，以提升模型性能。

Method: 提出Autodata框架，让AI代理作为数据科学家；具体实现为Agentic Self-Instruct；通过元优化（meta-optimization）使数据科学代理学会创建更强的数据。

Result: 在计算机科学研究任务、法律推理任务和数学对象推理任务上，Autodata优于经典合成数据集创建方法；元优化后性能进一步提升。
...
### [191] [The Unfireable Safety Kernel: Execution-Time AI Alignment for AI Agents and Other Escapable AI Systems](https://arxiv.org/abs/2606.26057)
*Seth Dobrin,Łukasz Chmiel*

Main category: cs.AI

TL;DR: 本文提出了一种针对可逃逸AI系统（AI agents）的架构级授权控制机制——Unfireable Safety Kernel，通过进程隔离、唯一路径前置执行、双重失效关闭和外部化签名验证四个核心属性，在运行时实现AI对齐。该机制在Rust中实现，并通过SMT定理证明和边界模型检查验证了其安全性。


<details>
  <summary>Details</summary>
Motivation: 现有AI agent的安全控制（系统提示、输出过滤、护栏库）都放在agent自身的运行时中，任何在其地址空间内的控制都可被输入影响而绕过，这类系统被称为"可逃逸AI系统"。因此需要一个架构级的、不可绕过的授权机制。

Method: 提出Unfireable Safety Kernel，满足四个属性：1）进程隔离；2）在结构上的唯一路径执行前置动作；3）在请求和系统级别双重失效关闭；4）外部化签名证据，可在受控系统信任边界外验证。在Rust中实现，通过Z3 SMT定理证明和Kani边界模型检查（4/4 harnesses）验证不变性，Python到Rust迁移需通过字节级等价测试（1000/1000 fixtures, 17/17 adversarial classes）。

Result: 在确定性自改进世界模型上测试：1000次自修改中，704次对安全核心的攻击被拒绝，无逃逸；额外300次在操作员终止开关下也被拒绝。6240次授权往返测试中无成功绕过。与3个声称拥有agent控制平面的当代系统对比，本系统的agent无法自主调用控制。

Conclusion: 架构级执行时对齐（execution-time AI alignment）是训练时对齐和推理时对齐的重要补充。Unfireable Safety Kernel证明了通过形式化验证的、进程隔离的授权机制可以有效防御可逃逸AI系统的安全威胁。
