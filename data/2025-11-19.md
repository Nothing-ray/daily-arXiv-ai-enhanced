...
TL;DR: nuCarla是一个基于CARLA模拟器构建的大规模BEV感知数据集，解决了端到端自动驾驶在闭环仿真中缺乏标准化数据集的问题，提供与nuScenes兼容的格式和平衡的类别分布。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶数据集主要来自真实世界的非交互条件，仅支持开环学习，缺乏适用于闭环测试的标准数据集，限制了端到端模型的发展。

Method: 在CARLA模拟器中构建nuScenes风格的大规模BEV感知数据集，保持与nuScenes格式完全兼容，并提供平衡的类别分布和直接可用的闭环仿真部署能力。

Result: nuCarla提供了与nuScenes相当的数据规模但更平衡的类别分布，并实现了最先进的检测性能，加速了闭环端到端自动驾驶的发展。

Conclusion: nuCarla通过提供开放基准的数据和模型，显著加速了闭环端到端自动驾驶的研究，为可靠和安全感知的自动驾驶研究铺平了道路。
...
TL;DR: 提出一个解决开放集识别中过度自信问题的框架，通过参数扰动估计不确定性，并使用两阶段分类器提高已知和未知类别的区分能力。


<details>
  <summary>Details</summary>
Motivation: 开放集识别中，当未知样本与已知类别语义相似时，特征空间的类间重叠会导致模型对未知样本赋予过高置信度，造成错误分类为已知类别，这种现象称为过度自信。

Method: 框架包含两个组件：基于扰动的预测不确定性估计模块（通过可控参数扰动生成多样化预测）和具有不同学习分类器的未知检测模块（作为两阶段过程实现），利用估计的不确定性改进已知和未知类别的区分。

Result: 在三个公共数据集上的实验结果表明，所提框架优于现有的开放集识别方法。

Conclusion: 该框架通过显式缓解类间重叠引起的过度自信问题，有效提升了开放集识别性能。
...
TL;DR: 提出了一种新颖的对象感知时序建模方法，用于少样本视频目标检测，通过选择性传播高置信度对象特征来提高时序一致性和检测精度。


<details>
  <summary>Details</summary>
Motivation: 解决少样本视频目标检测中时序一致性不足和复杂区域提议计算成本高的问题，旨在在有限标注样本下实现更好的新对象泛化能力。

Method: 采用过滤机制选择性传播跨帧的高置信度对象特征，结合少样本训练的检测和分类头，实现高效特征传播而无需显式对象管提议。

Result: 在5-shot设置下，AP提升分别为3.7%(FSVOD-500)、5.3%(FSYTV-40)、4.3%(VidOR)和4.5%(VidVRD)，在1-shot、3-shot和10-shot配置下也均有改进。

Conclusion: 该方法通过对象感知时序建模有效提升了少样本视频目标检测的性能，实现了更好的时序一致性和检测精度，且不依赖计算昂贵的对象管提议。
...
TL;DR: 提出了一种基于流匹配的图像融合方法，通过直接概率传输从源模态到融合图像分布，显著提高了采样效率，并采用伪标签选择和融合精炼模块来提升融合质量。
...
Method: 利用流匹配范式改进采样效率和结构一致性；收集多个SOTA模型的融合结果作为先验，使用任务感知选择函数选择可靠伪标签；引入融合精炼模块系统识别、分解和增强伪标签中的退化组件；集成弹性权重巩固和经验回放机制处理多任务场景。

Result: 在多样化融合任务中达到竞争性性能，显著提高采样效率，保持轻量级模型设计。

Conclusion: 该方法为多模态图像融合提供了一种高效、统一的解决方案，平衡了性能、效率和可扩展性。
...
TL;DR: 提出了一种两阶段无轨迹碰撞检测框架，使用扩散模型生成合理的未来路段地图并识别碰撞，无需车辆轨迹数据


<details>
  <summary>Details</summary>
Motivation: 解决轨迹获取和车辆跟踪的限制，利用记录个体级交通动态数据的地图段直接进行碰撞检测

Method: 两阶段方法：第一阶段使用Mapfusion扩散模型生成路段地图，通过噪声添加和去噪过程，结合时间动态嵌入和背景上下文；第二阶段通过比较监控地图与扩散模型生成结果来检测碰撞

Result: Mapfusion成功生成真实的路段演化地图，在不同采样间隔下保持鲁棒性，真实世界碰撞实验表明该方法能准确检测碰撞

Conclusion: 提出的两阶段方法在碰撞检测方面有效，为主动安全管理策略提供了新途径
...
<details>
  <summary>Details</summary>
Motivation: 由于现有视觉Transformer(ViT)在处理地震数据时忽略了内在模式，无法有效捕捉高频和低频地震信息，需要专门针对地震数据特性的基础模型训练方法。

Method: 使用频谱分解分离高频和低频分量，采用分层希尔伯特编码表示数据，并提出自适应训练策略：先关注粗粒度信息，再逐步细化到细粒度特征。
...
TL;DR: 开发了一种基于面部时序微动态分析的被动痴呆筛查方法，通过分析眨眼动态、嘴部运动、注视变化和头部微调等面部运动特征，无需语言或文本即可进行早期神经认知变化检测。


<details>
  <summary>Details</summary>
Motivation: 现有痴呆筛查方法主要依赖语音或脚本访谈，限制了在临床环境外的应用，且预测结果与语言和转录紧密耦合。本研究旨在开发一种无需语言干预、可跨设备、主题和文化使用的被动筛查方法。

Method: 通过稳定面部信号，将微小运动转换为可解释的面部微动态时间序列，平滑处理后将短窗口汇总为紧凑的片段级统计量。每个窗口通过其活动组合（跨运动流的相对运动份额）进行编码，分析运动分布而非幅度。

Result: 在YT DemTalk数据集（300个片段，150个自报痴呆，150个对照）上，注视不稳定性和嘴部/下颌动态被识别为最有效的线索，轻量级分类器达到AUROC 0.953、AP 0.961、F1分数 0.851、准确率0.857。

Conclusion: 面部时序微动态分析为语言无关的痴呆筛查提供了有效方法，能够在自然环境中进行大规模分析，且不依赖临床干预，具有良好的跨设备和文化适用性。
...
TL;DR: Gen-ViRe是一个新的视觉推理基准测试框架，用于评估视频生成模型作为推理器的能力，重点关注链式帧推理的六个认知维度。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注保真度或对齐性，无法评估链式帧推理能力，导致无法系统理解模型在多步规划、算法逻辑和抽象模式外推方面的核心认知能力。

Method: 基于认知科学和实际AI应用，将链式帧推理分解为六个认知维度和24个子任务，通过多源数据整理、最小提示协议和混合VLM辅助评估进行定量评估。

Result: 对最先进系统的实验显示，视觉质量与推理深度之间存在显著差距，为真正世界模拟器的进步建立了基线和诊断工具。

Conclusion: Gen-ViRe提供了首个对视频模型作为推理器的定量评估，揭示了当前模型在推理能力方面的局限性，为改进提供了原则性指导。
...
TL;DR: 该论文提出基于排序的损失函数来解决热图姿态估计中的三个主要问题：MSE损失无法有效提升关节定位精度、热图的空间和类别不平衡、以及评估指标与损失函数之间的不一致性。


<details>
  <summary>Details</summary>
Motivation: 解决热图姿态估计中的三个关键问题：(1) MSE损失对像素偏差的惩罚不区分，无法有效提升关节定位精度；(2) 热图存在空间和类别不平衡；(3) 评估指标(mAP)与损失函数之间存在差异。

Method: 提出基于排序的损失函数，通过理论分析和实证研究证明其优于常用的热图损失(MSE、KL散度)。该方法提高了置信度分数与定位质量之间的相关性，从而在NMS过程中实现更准确的实例选择。

Result: 在COCO、CrowdPose和MPII三个数据集上验证了RSPose的有效性。RSPose在COCO-val集上达到79.9 mAP，优于之前的最优方法，并将SimCC Resnet-50在COCO-val集上的性能提升了1.5 AP至73.6 AP。

Conclusion: 这是首个提出与姿态估计评估指标(mAP)对齐的损失函数的工作。基于排序的损失函数能够显著提高热图姿态估计的性能，特别是在提高置信度分数与定位质量相关性方面表现出色。
...
TL;DR: 提出碰撞声源分割任务(CS3)，通过音频条件分割视觉输入中的碰撞物体，利用基础模型和自我中心线索在复杂场景中定位声源。


<details>
  <summary>Details</summary>
Motivation: 受人类多感官感知启发，旨在从碰撞声音中识别产生声音的物体，解决碰撞声音来自两个物体交互的独特挑战。

Method: 使用弱监督的音频条件分割方法，结合CLIP和SAM2基础模型，并整合自我中心线索（如手中物体）来定位可能的碰撞声源。

Result: 在两个新基准EPIC-CS3和Ego4D-CS3上，mIoU分别比竞争基线高出3倍和4.7倍。

Conclusion: 该方法在复杂自我中心视频中有效分割碰撞声源，为多模态感知提供了新思路。
...
TL;DR: 提出Geometric Representation Regression (GRR)方法，通过预测解耦的几何表示（射线束方向和点图）来估计相机位姿，而不是直接回归6-DoF位姿。


<details>
  <summary>Details</summary>
Motivation: 传统绝对位姿回归(APR)方法作为黑盒直接回归位姿，容易记忆训练视图而非理解3D场景几何。需要更几何基础的方法。

Method: 将APR重新表述为逆向渲染过程，预测世界坐标系中的两个解耦几何表示：射线束方向（估计旋转）和对应点图（估计平移），然后通过可微分解器恢复6-DoF位姿。

Result: 在7-Scenes和Cambridge Landmarks数据集上达到最先进性能，验证了建模逆向渲染过程是更鲁棒的绝对位姿估计路径。

Conclusion: 解耦旋转和平移预测显著提升性能，将学习到的视觉到几何映射与最终位姿计算分离，为网络引入了强几何先验。
...
TL;DR: 提出了一个用于膀胱癌复发预测的多序列MRI数据集，并开发了H-CNN-ViT模型，该模型通过分层门控注意力多分支架构实现了78.6%的AUC性能。


<details>
  <summary>Details</summary>
Motivation: 膀胱癌复发率高达78%，术后监测至关重要。多序列增强MRI是常用检测方法，但术后组织变化使诊断困难。AI辅助工具有潜力，但缺乏专门的多序列MRI数据集阻碍了进展。

Method: 开发了H-CNN-ViT模型，采用分层门控注意力多分支架构，能够根据上下文需求选择性加权全局(ViT)和局部(CNN)路径的特征，实现平衡和有针对性的特征融合。每个模态独立处理，确保最佳捕获和整合各成像通道的独特特性。

Result: 在自建数据集上评估，H-CNN-ViT达到78.6%的AUC，超越了现有最先进模型。

Conclusion: 提出的数据集为膀胱癌复发预测研究提供了有价值的基准，H-CNN-ViT模型在多序列MRI数据分析中表现出优越性能，为AI辅助膀胱癌复发检测提供了有效解决方案。
...
TL;DR: QwenCLIP 是一个视觉语言框架，用基于大语言模型的嵌入模块替换 CLIP 的文本编码器，通过可学习提示增强跨模态对齐，解决了传统方法处理长文本医学报告的局限性。


<details>
  <summary>Details</summary>
Motivation: CLIP 的文本编码器最多只能处理 77 个 token，无法有效表示信息丰富的长文本放射学报告。现有的领域特定编码器（如 PubMedBERT）虽然有所改进，但仍受限于 512 token 的输入长度和相对浅层的语义理解。

Method: 提出 QwenCLIP 框架，用 LLM 嵌入模块（如 Qwen3-Embedding）替换 CLIP 的文本编码器，引入可学习提示来增强跨模态对齐，利用 LLM 的扩展上下文窗口和更丰富的表示能力。

Result: QwenCLIP 能够从长文本临床报告中捕捉全面的医学语义，显著改善了医学图像-文本对齐和在放射学基准测试中的下游性能。

Conclusion: 通过结合 LLM 的扩展上下文能力和可学习提示机制，QwenCLIP 有效解决了医学领域长文本表示的问题，在放射学任务中取得了显著改进。
...
TL;DR: 提出了一种结合EfficientNet和VGG19的混合模型架构，用于腰椎退变分类，通过伪牛顿增强层和稀疏诱导特征减少层提升性能，在医疗图像分类中取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 传统迁移学习方法在医疗图像高维背景下存在局限性，无法充分利用解剖特征，需要开发更有效的特征选择和表示方法。
...
Conclusion: 该架构克服了传统迁移学习的限制，为医疗图像自动诊断工具的发展做出了贡献。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖手工提示且性能不稳定，限制了在真实场景中的鲁棒性和泛化能力。作者发现现有VLMs在高层次决策任务中存在关键局限性。

Method: 利用VLMs的强大场景理解能力为现有视觉基准添加结构化、语言丰富的场景描述，构建多模态交互架构融合视觉和语言特征，并设计后处理精炼模块提升预测可靠性。
...
TL;DR: 本文研究了医学解剖分割中数据规模与性能的缩放规律，发现更大数据集能显著提升分割性能。提出了基于图像配准的变形增强方法，能有效提高数据利用效率，超越标准幂律缩放趋势。
...
Method: 分析了15个语义任务和4种成像模态的数据缩放规律；评估了随机弹性变形和配准引导变形两种增强策略；提出了基于图像配准生成微分同胚映射的新型可扩展图像增强方法。

Result: 更大数据集显著改善分割性能，遵循相似缩放趋势；配准和生成变形增强均能显著提高数据利用效率；提出的生成变形方法在性能和收敛速度上表现更优，无需额外数据即可超越标准幂律缩放。
...
<details>
  <summary>Details</summary>
Motivation: 现有数字血液病理学方法（包括单任务、视觉语言、WSI优化和单细胞血液学模型）无法提供跨疾病复杂性的统一多任务多模态推理。

Method: 基于Hema-Former多模态模块，在46个公开数据集（含70万+图像和2.1万+问答对）上训练，连接视觉和文本表示，支持检测、分类、分割、形态预测、掩码语言建模和视觉问答等任务。

Result: Uni-Hema在多种血液学任务上达到与单任务单数据集模型相当或更优的性能，同时在单细胞层面提供可解释的形态学相关见解。
...
TL;DR: 提出了首个用于检测短暂沟壑的弱监督管道，利用视觉语言模型减少人工标注工作量，并发布了首个用于半监督检测的遥感图像数据集。


<details>
  <summary>Details</summary>
Motivation: 短暂沟壑是农业领域最令人担忧的土壤侵蚀现象之一，其短暂的时间周期和标注数据的稀缺性使得传统计算机视觉方法和机器学习检测面临挑战。

Method: 使用视觉语言模型的预训练知识，采用师生模型架构，教师模型从VLM生成的噪声标签中学习，学生模型通过弱监督使用教师生成的标签和噪声感知损失函数进行学习。

Result: 实验结果表明该方法优于直接使用VLM和标签模型，在弱监督训练学生模型时表现出更优越的性能。

Conclusion: 该方法有效解决了短暂沟壑自动检测的挑战，通过弱监督方法显著减少了人工标注需求，为土壤侵蚀监测提供了可行解决方案。
...
TL;DR: 提出基于压缩视频运动向量的时间真实性评估框架，通过运动向量统计量量化生成视频与真实视频的差异，并探索运动向量与RGB融合提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 当前生成视频模型在时间真实性方面存在弱点，大多数评估指标主要关注空间外观而对运动敏感性有限，需要更有效的时间行为评估方法。

Method: 使用H.264和HEVC等标准从压缩视频流中提取运动向量，通过KL、JS和Wasserstein散度计算真实与生成视频的运动向量统计差异，并研究多种MV-RGB融合策略。

Result: 实验显示生成视频存在系统性运动偏差，Pika和SVD在熵基散度上最接近真实视频，MV-sum统计量偏好VC2和Text2Video-Zero，CogVideo偏差最大。MV-RGB融合显著提升分类准确率。
...
TL;DR: 提出了SAE-MCVT，首个可扩展的实时多摄像头车辆跟踪框架，解决了现有方法忽视实时性和可扩展性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有MCVT方法过于注重准确性而忽略了实时性能和可扩展性，这两个方面对于实际部署至关重要，特别是在城市规模应用中随着摄像头数量增加而变得更具挑战性。

Method: 采用边缘-中心架构，边缘设备处理RTSP视频流进行目标检测、跟踪、地理映射和特征提取，仅传输轻量级元数据；中心工作站基于自监督相机链接模型计算跨摄像头关联。

Result: 在RoundaboutHD数据集上，SAE-MCVT能在2K 15 FPS视频流上保持实时操作，并达到61.2的IDF1分数。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆的AI系统必须遵守道路安全规范，需要评估多模态大语言模型对道路安全概念的理解能力。

Method: 使用从学校教科书中收集的交通标志和道路安全规范图像数据集，在零样本设置下评估模型能力。

Result: 初步结果显示这些模型在安全推理方面表现不佳，存在人类学习与模型解释之间的性能差距。

Conclusion: 多模态大语言模型在道路安全概念理解方面存在显著缺陷，需要进一步研究来弥合这些差距。
...
TL;DR: 提出CuRPO方法，通过基于课程学习的相对策略优化，利用CoT长度和gIoU奖励作为复杂度指标，从简单到复杂逐步训练视觉定位模型，显著提升性能。
...
Method: CuRPO方法使用CoT长度和gIoU奖励作为复杂度指标，采用课程学习策略从简单到复杂逐步训练数据。

Result: 在RefCOCO、RefCOCO+、RefCOCOg和LISA数据集上表现优异，相比现有方法提升显著，在RefCOCO上达到+12.52 mAP的提升。

Conclusion: CuRPO方法在视觉定位任务中展现出高效性和鲁棒性，特别适用于文本描述模糊复杂的任务，在少样本学习场景下表现突出。
...
<details>
  <summary>Details</summary>
Motivation: 解决视频衍生帧数据集中由于连续帧之间的高度相似性导致的信息泄露问题，确保数据集划分的可靠性和代表性。
...
<details>
  <summary>Details</summary>
Motivation: 探索为视觉Transformer注入跨模态通用归纳偏置的新方法，通过无视觉内容的程序生成数据来培养抽象计算先验

Method: 使用形式语法等简单算法生成无视觉语义内容的数据，在标准图像训练前进行预训练，绕过视觉补丁嵌入机制

Result: 在ImageNet-1k上，仅用1%训练预算的程序生成数据就能提高最终准确率1.7%以上，效果相当于28%的ImageNet数据
...
TL;DR: 提出了一种基于标量场的细胞图像分割方法，通过训练网络学习连续的标量场，使用分水岭方法进行分割，无需正则化即可获得稳健的分割结果。
...
Method: 使用泊松偏微分方程和热方程稳态解的扩散方程作为标量场，通过最小化场残差来获得解，无需正则化。使用单个张量训练U-Net网络，采用分水岭方法进行分割。

Result: 在公共数据集上取得了有竞争力的结果，能够减少训练数据中异常值的不利影响，允许清晰的细胞边界，实现优秀的细胞分割效果。

Conclusion: 这种新颖、简单但几何直观的方法能够实现出色的细胞分割结果，具有实现简化、能耗低、内存占用小等优势，特别适合边缘计算。
...
TL;DR: EchoAgent是一个基于大语言模型控制的框架，通过协调专门的视觉工具实现超声心动图视频的自动化分析，包括时间定位、空间测量和临床解释。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型不支持超声心动图所需的视频级推理和基于指南的测量分析，需要开发能够提供结构化、可解释自动化的解决方案。
...
Conclusion: 这项工作证明了通过特定任务工具和全视频级自动化，实现基于代理的、符合指南的超声心动图视频分析推理的可行性，为心脏超声的可信AI设定了新方向。
...
TL;DR: 提出CrossTrainer方法，通过发现跨运动通用的技能属性（如平衡、控制、手部位置），训练多模态语言模型为视频生成可操作的反馈和熟练度评级，在跨运动迁移和领域内设置中显著优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 解决当前技能评估模型局限于单一运动、专家级监督成本高且稀缺的问题，探索可迁移的视频表示方法以覆盖长尾运动领域。

Method: 发现跨运动通用的技能属性，训练多模态语言模型生成针对新视频的可操作反馈（如"抬高双手以产生更多力量"）和熟练度评级（如早期专家）。

Result: 在多个数据集上的跨运动和领域内设置验证中，相对现有技术获得高达60%的性能提升。
...
TL;DR: 提出基于卷积字典特征解耦的双提示专家网络(CD-DPE)，用于多对比度MRI超分辨率重建，通过分离跨对比度和内部对比度特征来提升重建质量。
...
Method: 采用迭代卷积字典特征解耦模块(CD-FDM)分离特征，并提出双提示特征融合专家模块(DP-FFEM)，使用频率提示和自适应路由提示来选择和融合参考特征。

Result: 在公共多对比度MRI数据集上的实验表明，CD-DPE在重建精细细节方面优于现有最优方法，并在未见数据集上展现出强大的泛化能力。
...
TL;DR: RSD框架利用视觉语言模型增强端到端自动驾驶系统的训练，通过RiskHead模块将风险注意力蒸馏到BEV特征中，提升系统在复杂场景下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决当前自动驾驶系统在泛化能力方面的限制，特别是处理未见场景和不同传感器配置的能力。现有方法存在系统不一致或计算成本过高的问题。

Method: 提出风险语义蒸馏(RSD)框架，通过RiskHead模块从视觉语言模型中提取因果风险估计，并将其蒸馏到鸟瞰图(BEV)特征中，生成可解释的风险注意力图。

Result: 在Bench2Drive基准测试中，RSD显著提升了系统在复杂和不可预测驾驶条件下的性能，增强了感知和规划能力。

Conclusion: RSD通过风险注意力机制使BEV特征学习更丰富的风险表示，更好地模拟人类驾驶行为，有效提升自动驾驶系统的泛化能力。
...
TL;DR: RISE是首个用于单静态雷达室内场景理解的基准和系统，通过利用多路径反射的几何线索，实现了布局重建和物体检测，在保护隐私的同时显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 解决室内场景理解中光学传感器存在的遮挡和隐私风险问题，利用毫米波雷达的隐私保护和穿透能力，但克服其低空间分辨率的限制。

Method: 提出双角度多路径增强方法，显式建模到达角和离开角来恢复次级反射；采用仿真到现实的层次扩散框架，将碎片化的雷达响应转化为完整的布局重建和物体检测。

Result: 在布局重建方面将Chamfer距离降低了60%（降至16厘米），首次实现了基于毫米波的物体检测，达到58%的IoU。
...
<details>
  <summary>Details</summary>
Motivation: 视觉几何Transformer在3D感知和重建中计算量大，难以满足实时性需求，需要一种无需重新训练的高效加速方法。
...
Conclusion: Co-Me为视觉几何Transformer提供了一种有效的加速方案，能够显著提升推理速度而不牺牲性能，适用于多视角和流式视觉几何Transformer。
...
TL;DR: 开发了一个2.5D上下文感知模型，用于自动识别MRI图像的解剖平面方向，准确率达99.49%，比2D模型减少60%错误，并成功应用于脑肿瘤检测任务。


<details>
  <summary>Details</summary>
Motivation: MRI图像中缺失平面方向元数据会导致分析复杂化、数据集合并时的域偏移以及诊断分类器准确性降低，需要自动化解决方案。

Method: 采用2.5D上下文感知模型，利用多切片信息避免单一切片的歧义，同时在3D切片序列和静态2D图像上进行训练。

Result: 2.5D模型准确率达99.49%，比2D参考模型的98.74%提升显著；在脑肿瘤检测中，通过门控策略使用元数据增强预测，准确率从97.0%提升至98.0%，误诊减少33.3%。

Conclusion: 2.5D上下文对于MRI平面方向识别至关重要，生成的元数据能显著提升下游任务的性能，模型已集成到开源交互式web应用中。
...
TL;DR: LINGUAL框架通过自然语言指令和上下文学习，将专家指令转化为可执行程序，自动完成医学图像分割任务，在主动域适应中实现与主动学习基线相当或更优的性能，同时减少约80%的标注时间。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中，主动学习虽然只需标注感兴趣区域而非整张图像，但由于医学图像边界模糊模糊，标注工作仍然具有挑战性、劳动密集且认知要求高。传统主动学习中，标注工作量与ROI大小相关，大区域认知负担轻但标注成本高，小区域需要更精细精度和更多专家注意力。

Method: 提出LINGUAL框架：接收专家的自然语言指令，通过上下文学习将其转化为可执行程序，自动执行相应的子任务序列，无需人工干预。

Result: 在主动域适应任务中，LINGUAL实现了与主动学习基线相当或更优的性能，同时估计标注时间减少了约80%。

Conclusion: 语言引导为医学图像分割提供了一种有效的替代方案，以最小的专家努力绕过了精确边界描绘这一认知要求高的任务。
...
TL;DR: WaRPAD是一种无需训练的AI生成图像检测方法，利用自监督模型对图像裁剪增强的鲁棒性，通过分析图像嵌入对高频扰动和重缩放操作的敏感性来检测AI生成图像。


<details>
  <summary>Details</summary>
Motivation: 随着视觉生成模型的快速发展，AI生成图像检测变得至关重要。传统方法需要针对特定数据集训练检测器，而本研究探索无需训练的方法，利用自监督模型而无需先验数据知识。

Method: WaRPAD首先定义基础评分函数，量化图像嵌入对通过Haar小波分解提取的高频方向扰动的敏感性。为了模拟对裁剪增强的鲁棒性，将图像重缩放到模型输入尺寸的倍数，分割成小块，计算每个块的基础评分，最终检测评分是所有块评分的平均值。

Result: 在真实数据集和23种不同生成模型生成的图像上验证，WaRPAD始终实现竞争性性能，并对测试时损坏表现出强鲁棒性。该方法可应用于各种自监督模型。

Conclusion: WaRPAD提供了一种有效且无需训练的AI生成图像检测方法，利用自监督模型的固有特性，在不同分辨率和领域的图像上表现优异，具有广泛适用性。
...
TL;DR: 提出了一种基于潜在扩散模型的洪水地图超分辨率方法，能够在保持精度的同时显著减少计算时间，并具有良好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统物理水动力模型计算密集，不适用于实时大规模应用；现有卷积神经网络方法泛化能力有限，无法很好地适应未见区域。

Method: 利用潜在扩散模型对粗网格洪水地图进行超分辨率处理，结合物理信息输入，通过迁移学习加速对新地理区域的适应。

Result: 潜在扩散模型显著减少了生成高保真洪水地图的计算时间，且不损失精度，在跨物理位置具有优越的泛化能力。
...
TL;DR: 提出了一种基于无人机图像自动检测、定位和分类混凝土桥梁缺陷的新方法，包含显著性缺陷区域提议和YOLOX深度学习检测器两个主要阶段。
...
Method: 第一阶段使用显著性进行缺陷区域提议，第二阶段采用基于YOLOX的深度学习检测器，对通过边界框级亮度增强获得的显著性增强图像进行操作。
...
TL;DR: SCAR是一种基于自回归模型的语义上下文驱动方法，通过压缩语义预填充和语义对齐引导来提升图像编辑的指令遵循能力和视觉质量。


<details>
  <summary>Details</summary>
Motivation: 自回归模型在图像生成中显示出潜力，但在图像编辑任务中面临条件化弱、效率低的问题，导致指令遵循差和视觉伪影。

Method: 提出SCAR方法，包含两个关键组件：压缩语义预填充（将高层语义编码为紧凑前缀）和语义对齐引导（在自回归解码中对齐视觉隐藏状态与目标语义）。

Result: SCAR在指令编辑和可控生成基准测试中实现了卓越的视觉保真度和语义对齐，优于先前的基于自回归的方法，同时保持可控性。

Conclusion: SCAR方法克服了基于向量量化的预填充方法的语义限制和高成本问题，在保持灵活性的同时显著提升了自回归模型的图像编辑性能。
...
Method: 使用高效分割解码器生成对象掩码作为高层次语义先验，指导将视觉token合并为紧凑的对象中心表示，并采用质心引导排序机制恢复空间顺序。
...
Method: 使用语义引导参数合成器(SGPS)，基于预训练生成引擎，输入单张图像和临床文本描述，直接合成轻量级分类器的全部权重参数，无需任务特定训练。

Result: 在ISIC 2018皮肤病变数据集和自定义罕见疾病数据集上，1-shot和5-shot分类任务中显著优于现有few-shot和zero-shot学习方法，建立新SOTA。
...
TL;DR: 开发并验证了一个全自动深度学习管道，用于在三维CT扫描上测量肩胛盂骨缺损，该方法比传统手动方法更高效且具有更高的医生间一致性。


<details>
  <summary>Details</summary>
Motivation: 当前测量肩胛盂骨缺损的手动和半自动方法耗时且存在读者间变异性，需要开发更可靠、高效的自动化测量工具。

Method: 采用多阶段算法：1）使用U-Net自动分割肩胛盂和肱骨；2）通过第二个网络预测肩胛盂边缘点；3）应用PCA、投影和圆拟合计算骨缺损百分比。

Result: 自动测量与共识读数高度一致（ICC 0.84 vs 0.78），在低和高骨缺损亚组中表现更佳，分类准确率高，无低/高严重程度误分类。

Conclusion: 该方法是一个时间高效且临床可靠的术前规划工具，可用于肩关节不稳患者筛查和术前评估。
...
TL;DR: 提出了DEER-3D框架，通过错误驱动的3D场景编辑生成针对性反事实数据，解决3D-LLMs在空间基础任务中的偏见问题，显著提升模型在3D环境中的语言基础能力。


<details>
  <summary>Details</summary>
Motivation: 现有3D-LLMs在将语言准确基础到3D环境的视觉和空间元素方面存在局限，主要原因是训练数据侧重于语言推理而非空间理解，且缺乏3D资源导致固有的基础偏见未解决。

Method: 提出DEER-3D框架，采用"分解、诊断评估、编辑、重新训练"的工作流程。当识别到3D-LLM的基础失败时，首先诊断具体的谓词级错误（如属性或空间关系），然后执行最小化的谓词对齐3D场景编辑（如重新着色或重新定位），生成针对性反事实监督数据进行迭代模型微调。

Result: 在多个3D基础和场景理解任务的基准测试中，通过迭代精化在所有评估数据集上均表现出持续改进，显著提升了基础准确性。

Conclusion: DEER-3D证明了针对性、错误驱动的场景编辑在弥合3D LLMs中语言推理能力与空间基础之间差距的有效性，为3D-LLMs的改进提供了新途径。
...
TL;DR: GCA-ResUNet是一种高效的医学图像分割网络，通过将分组坐标注意力(GCA)集成到ResNet-50残差块中，在保持计算效率的同时提升全局上下文建模能力。


<details>
  <summary>Details</summary>
Motivation: 传统U-Net架构难以捕捉长距离依赖关系，而基于Transformer的变体需要大量计算资源和训练数据。需要一种既能建模全局上下文又保持计算效率的解决方案。

Method: 在ResNet-50残差块中集成分组坐标注意力(GCA)，通过分组坐标建模联合编码跨通道和空间位置的全局依赖关系，增强特征表示和边界描绘能力。

Result: 在Synapse数据集上达到86.11%的Dice分数，在ACDC数据集上达到92.64%的Dice分数，超越了多个最先进基线方法，同时保持快速推理和良好的计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 传统多阶段'检索-匹配'方法因视角和尺度差异以及复杂背景干扰容易产生累积误差，需要更高效的端到端解决方案。

Method: 采用全Transformer架构，使用Swin-Transformer联合编码无人机和卫星图像特征，引入网格级稀疏专家混合(GMoE)捕获跨模态和视图内依赖关系，使用无锚检测头通过热图监督直接回归坐标。

Result: 在IoU=0.25和mIoU指标上达到87.51%、62.50%和61.45%的领先性能，显著优于DetGeo等方法。
...
TL;DR: 本文提出了一种基于二元交叉熵(BCE)的三方协同学习框架BCE3S，用于解决长尾识别任务中特征紧凑性和分类器平衡性的问题。
...
Conclusion: BCE3S不仅实现了样本特征间更高的紧凑性和分离性，还平衡了分类器的分离性，为长尾识别任务提供了有效的解决方案。
...
TL;DR: FAPE-IR是一个用于图像修复的频率感知规划和执行框架，使用冻结的多模态大语言模型作为规划器生成频率感知修复计划，并通过LoRA-MoE模块在扩散模型执行器中动态选择高频或低频专家，实现统一的多任务图像修复。


<details>
  <summary>Details</summary>
Motivation: 现有的多任务图像修复方法通常依赖任务特定设计或潜在路由策略，难以适应现实世界中各种复杂退化情况，需要开发更统一和可解释的解决方案。

Method: 使用冻结的MLLM作为规划器分析退化图像并生成频率感知修复计划；在扩散模型执行器中采用LoRA-MoE模块动态选择高频或低频专家；引入对抗训练和频率正则化损失来提升修复质量。

Result: 在七个修复任务上实现了最先进的性能，并在混合退化条件下表现出强大的零样本泛化能力。

Conclusion: 通过将语义规划与基于频率的修复相结合，FAPE-IR为多任务图像修复提供了一个统一且可解释的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法需要用户提供精确的空间位置和时间边界描述，这在实际应用中不实用，特别是当用户希望通过语义属性或对象关系的隐式查询来概念化编辑时。

Method: RIVER通过数字孪生表示解耦推理和生成，使用大语言模型进行多跳推理，然后输出结构化指令指导基于扩散的编辑器执行像素级修改。训练采用强化学习，奖励评估推理准确性和生成质量。

Result: RIVER在提出的RVEBenchmark上表现最佳，并在VegGIE和FiVE两个额外视频编辑基准上达到最先进性能，超越了六个基线方法。

Conclusion: RIVER成功解决了基于推理的视频编辑任务，能够有效处理隐式查询，为复杂视频编辑提供了新的解决方案。
...
TL;DR: 提出RTS-Mono，一种实时自监督单目深度估计方法，采用轻量级编码器-解码器架构，在KITTI数据集上实现最先进性能，参数仅3M，在Nvidia Jetson Orin上达到49FPS实时推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有自监督单目深度估计模型计算资源消耗大，轻量化方法性能下降严重，阻碍了在实际场景中的部署应用。

Method: 基于Lite-Encoder的编码器和多尺度稀疏融合框架的解码器，最小化冗余，确保性能并提高推理速度。

Result: 在KITTI数据集上实现最先进性能，低分辨率下Abs Rel和Sq Rel分别提升5.6%和9.8%，高分辨率下Sq Rel和RMSE分别提升6.1%和1.9%。

Conclusion: RTS-Mono在保持高性能的同时实现了实时推理，解决了自监督单目深度估计模型在实际部署中的计算效率问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有最优传输聚合方法对称处理源和目标边际分布，当图像特征和聚类中心分布差异显著时效果受限。

Method: 采用行-列归一化平均与分离边际校准实现不对称匹配，通过可学习坐标嵌入融合几何约束，提升空间感知能力。

Result: 在MSLS、NordLand和Pittsburgh数据集上的实验结果表明该方法在匹配精度和鲁棒性方面表现优越。

Conclusion: 所提出的不对称聚合方法能有效适应视觉位置识别中的分布差异，显著提升识别性能。
...
Method: 提出Cascaded-ViT架构，采用新颖的Cascaded-Chunk Feed Forward Network (CCFFN)，通过分割输入特征提高参数和FLOP效率。

Result: 在ImageNet-1K上，CViT-XL达到75.5% Top-1精度，相比EfficientViT-M5减少15% FLOPs和3.3%能耗。CViT系列在各种模型尺寸下都表现出最低能耗。

Conclusion: CViT在计算效率方面表现优异，特别适合电池受限设备部署，在Accuracy-Per-FLOP指标上达到顶级效率。
...
TL;DR: Coffee是一种通过语言指定不希望学习的概念来正则化扩散模型微调过程的方法，防止模型从微调数据中学习到不需要的概念并与用户提示纠缠。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像扩散模型在微调时容易学习到不需要的概念，并与用户提示产生纠缠，这在下游任务如偏见缓解、防止恶意适应、属性解缠等方面存在问题。

Method: 通过保持用户提示的嵌入与不希望学习的概念不对齐来实现正则化，无需额外训练，只需修改文本描述即可灵活调整不希望学习的概念。

Result: 实验结果表明，Coffee能够有效防止文本到图像模型在微调过程中学习到指定的不希望概念，且性能优于现有方法。

Conclusion: Coffee提供了一种无需额外训练、通过语言灵活控制扩散模型微调过程的有效方法，能够防止模型学习不需要的概念。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频系统只能检测事故是否发生，但无法深入理解事故在不同认知阶段的发展过程，缺乏对事故成因的系统性分析。

Method: 四阶段框架：(1)事件触发的多视角视频采集；(2)行人行为阶段分割；(3)阶段特定的多视角推理；(4)分层合成和诊断推理。使用两个专用VLM模型进行阶段分割和多视角分析。

Result: TG-VLM模型在行为阶段分割上达到mIoU=0.4881，PhaVR-VLM在字幕生成得分33.063，问答准确率最高达64.70%。在Woven Traffic Safety数据集上验证有效。

Conclusion: MP-PVIR成功将多视角视频数据转化为可操作的洞察，推动了车辆-基础设施协同系统的AI驱动交通安全分析。
...
TL;DR: 该论文提出了卷积最近邻(ConvNN)框架，将卷积和自注意力统一在k近邻聚合框架下，揭示了它们都是邻居选择和聚合的特殊情况，只是分别基于空间邻近性和特征相似性。


<details>
  <summary>Details</summary>
Motivation: 虽然卷积神经网络和Transformer重塑了计算机视觉，但这两类架构通常被视为根本不同的。作者认为卷积和自注意力尽管表面差异很大，但可以在统一的k近邻聚合框架下进行统一。

Method: 提出ConvNN框架作为卷积层和注意力层的即插即用替代品，在VGG中实现混合分支架构，在ViT中用ConvNN替换标准注意力，系统探索卷积和注意力之间的中间谱系。

Result: 在CIFAR-10和CIFAR-100上的实验表明：VGG混合分支架构在两个数据集上都提高了准确率；ViT中使用ConvNN优于标准注意力和其他注意力变体；通过k值和架构变体的消融实验显示，在谱系中间插值通过平衡局部和全局感受野提供正则化效益。

Conclusion: 该工作提供了一个统一框架，消除了卷积和注意力之间的表面区别，对设计更有原则性和可解释性的视觉架构具有重要意义。
...
TL;DR: SMART是一个基于多模态大语言模型的视频时刻检索框架，通过集成音频线索和利用镜头级时间结构，结合音频和视觉特征，并应用镜头感知令牌压缩来减少冗余并保留细粒度时间细节。


<details>
  <summary>Details</summary>
Motivation: 现有的视频时刻检索方法主要依赖粗略的时间理解和单一视觉模态，限制了在复杂视频上的性能表现。

Method: 提出SMART框架，集成音频线索，利用镜头级时间结构，结合音频和视觉特征，应用镜头感知令牌压缩选择性地保留高信息令牌，并优化提示设计以更好地利用音频-视觉线索。

Result: 在Charades-STA和QVHighlights上的评估显示，SMART显著优于现有最先进方法，在Charades-STA上R1@0.5提升1.61%，R1@0.7提升2.59%。

Conclusion: SMART通过整合音频线索和镜头级时间结构，有效提升了视频时刻检索的性能，证明了多模态融合和细粒度时间理解的重要性。
...
TL;DR: iGaussian是一个两阶段前馈框架，通过直接3D高斯反演实现实时相机姿态估计，相比传统迭代方法获得10倍加速。
...
Method: 首先通过高斯场景先验的姿态回归网络回归粗略6DoF姿态，然后通过特征匹配和多模型融合进行精炼，使用跨相关模块对齐图像嵌入与3D高斯属性。

Result: 在多个数据集上显著优于先前方法，将中值旋转误差降至0.2°，在移动机器人上达到2.87 FPS跟踪速度。

Conclusion: iGaussian通过前馈框架和直接3D高斯反演，实现了实时高效的相机姿态估计，为机器人视觉导航提供了实用解决方案。
...
TL;DR: 提出了一种面向多模态大语言模型的对象级token压缩策略，通过自适应token合并实现与人类视觉系统一致的高效压缩，仅使用10%的token即可达到原始模型96%的性能。


<details>
  <summary>Details</summary>
Motivation: 传统基于图像块的token化方法导致token数量二次增长，增加了计算和内存负担，且与人类视觉认知系统不匹配，导致幻觉问题和计算冗余。

Method: 提出了对象级token合并策略进行自适应token压缩，该方法与人类视觉系统保持一致，能够显著减少token数量。

Result: 在多个综合基准测试中，该方法平均仅使用10%的token即可达到原始模型96%的性能，在压缩比和性能平衡方面优于相关工作。

Conclusion: 提出的对象级token压缩策略有效解决了多模态大语言模型中的计算效率问题，同时保持了模型性能，为高效多模态理解提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有毫米波重建方法在完全遮挡、多样化日常物体3D重建中面临的覆盖范围有限和噪声高的问题，为机器人、增强现实和物流等领域开辟新应用。

Method: 采用三阶段流水线：提出候选几何表面、使用专为毫米波信号设计的基于transformer的形状补全模型、执行熵引导的表面选择，将原始无线信号与基于视觉的形状补全方法相结合。

Result: 在合成点云数据上训练，在真实世界数据上表现出色，与最先进基线相比，召回率从54%提升到72%，同时保持85%的高精度。

Conclusion: Wave-Former通过物理感知的形状补全模型，成功实现了对完全遮挡物体的高精度3D重建，展示了在真实场景中的良好泛化能力。
...
TL;DR: 提出了RiSe框架解决多模态人脸反欺诈中的跨域泛化问题，通过非对称不变风险最小化和多模态协同解耦来应对表示风险和协同风险。


<details>
  <summary>Details</summary>
Motivation: 多模态人脸反欺诈方法在未见域中性能下降严重，主要由于两个被忽视的风险：模态表示不变风险（表示在域偏移下是否保持泛化性）和模态协同不变风险（模型过拟合域特定的模态间相关性）。

Method: RiSe框架包含两个核心组件：1) AsyIRM：在径向空间学习不变球形决策边界以适应非对称分布；2) MMSD：通过跨样本混合和解耦增强内在、可泛化的模态特征的自监督任务。

Result: 理论分析和实验验证表明，RiSe在跨域性能上达到了最先进水平。

Conclusion: RiSe框架有效解决了多模态人脸反欺诈中的跨域泛化问题，通过处理表示风险和协同风险显著提升了模型在未见域中的性能。
...
TL;DR: 提出UMEG-Net，一个统一的多实体图网络，用于少样本精确事件检测，通过整合人体骨架和运动特定物体关键点，结合GCN和多尺度时序移位，在有限标注数据下实现鲁棒性能。


<details>
  <summary>Details</summary>
Motivation: 精确事件检测面临快速连续事件、运动模糊和细微视觉差异的挑战，现有方法依赖大规模标注数据和像素/姿态输入，在少样本条件下表现不佳，而获取大规模标注数据实际困难。

Method: UMEG-Net整合人体骨架和运动特定物体关键点到统一图中，采用基于GCN和多尺度时序移位的高效时空提取模块，并通过多模态蒸馏将基于关键点的图知识迁移到视觉表示。
...
TL;DR: MVI-Bench是首个专门评估误导性视觉输入对大型视觉语言模型鲁棒性影响的综合基准，包含1,248个标注样本，涵盖三个层次：视觉概念、属性和关系。


<details>
  <summary>Details</summary>
Motivation: 现有鲁棒性基准主要关注幻觉或误导性文本输入，而忽视了误导性视觉输入对视觉理解评估的重要性。

Method: 基于基础视觉基元设计三个层次误导性视觉输入，构建六个代表性类别，引入MVI-Sensitivity细粒度评估指标。
...
TL;DR: 提出MCFormer网络，通过全局和局部相关性建模来解决船舶重识别中的异常样本问题，在三个基准测试中达到最先进性能


<details>
  <summary>Details</summary>
Motivation: 现有船舶重识别方法大多直接迁移自行人重识别算法，无法有效处理船舶图像中更大的类内变化和局部缺失问题，导致同一身份内出现异常样本
...
### [62] [Orion: A Unified Visual Agent for Multimodal Perception, Advanced Visual Reasoning and Execution](https://arxiv.org/abs/2511.14210)
*N Dinesh Reddy,Sudeep Pillai*

Main category: cs.CV

TL;DR: Orion是一个多模态视觉代理框架，通过调用多种计算机视觉工具实现复杂视觉工作流，在多个基准测试中达到先进水平。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型只能产生描述性输出，无法执行复杂的多步骤视觉任务，需要开发能够自主进行视觉推理的智能系统。

Method: 采用代理框架，整合对象检测、关键点定位、全景分割、OCR和几何分析等专业计算机视觉工具，结合神经感知与符号执行。

Result: 在MMMU、MMBench、DocVQA和MMLongBench等基准测试中取得有竞争力的性能表现。

Conclusion: Orion实现了从被动视觉理解到主动、工具驱动的视觉智能的转变，将单体视觉语言模型扩展到生产级视觉智能。
...
TL;DR: DoGCLR是一个基于博弈论的自监督对比学习框架，用于骨架动作识别。它通过时空双权重定位机制识别关键运动区域，并使用熵驱动优势策略管理负样本库，在NTU RGB+D和PKU-MMD数据集上取得了最先进的性能。
...
Method: 1. 将正负样本构建建模为动态优势博弈，达到语义保持和判别力平衡的均衡
2. 时空双权重定位机制识别关键运动区域，指导区域增强
3. 熵驱动优势策略管理记忆库，保留高熵（困难）负样本，替换低熵（弱）负样本
...
Conclusion: DoGCLR通过博弈论方法有效解决了骨架动作识别中的运动信息丢失和负样本选择问题，在多个数据集上表现出强大的鲁棒性和优越性能。
...
TL;DR: Gen3d是一种从单张图像生成高质量、通用3D场景的新方法，通过RGBD图像生成初始点云，维护和扩展世界模型，并优化高斯泼溅表示来生成3D场景。
...
Method: 从RGBD图像生成初始点云，维护和扩展世界模型，通过优化高斯泼溅表示来最终确定3D场景。

Result: 在多样化数据集上的广泛实验表明，该方法在生成世界模型和合成高保真、一致的新视角方面具有强大的泛化能力和优越性能。

Conclusion: Gen3d方法能够从单张图像生成高质量的通用3D场景，在生成世界模型和新视角合成方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法针对单一退化问题开发专业模型，缺乏可扩展性且未能利用恢复问题的共享本质；而通用模型需要详细提示且在这些细粒度任务上表现不佳。

Method: 构建包含380万对数据的大规模数据集，包括新颖的物理可信数据填补公共基准的关键空白；定制训练流程微调扩散变换器，从多样化数据中学习鲁棒恢复先验，集成细粒度掩码和强度控制。

Result: UniSER显著优于专业模型和通用模型，在野外场景中实现鲁棒、高保真度的恢复。

Conclusion: 通过利用软效应的共同本质（半透明遮挡）和协同方法，UniSER能够在单一框架内有效处理多种退化问题，展现出强大的泛化能力和恢复性能。
...
### [66] [SAM-Fed: SAM-Guided Federated Semi-Supervised Learning for Medical Image Segmentation](https://arxiv.org/abs/2511.14302)
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割面临数据隐私和专家标注成本高的限制，联邦半监督学习虽然提供解决方案，但面临伪标签可靠性依赖本地模型强度、客户端设备计算资源有限导致模型轻量化或异构化的问题。
...
### [67] [GloTok: Global Perspective Tokenizer for Image Reconstruction and Generation](https://arxiv.org/abs/2511.14184)
*Xuan Zhao,Zhongyu Zhang,Yuge Huang,Yuxi Mi,Guodong Mu,Shouhong Ding,Jun Wang,Rizen Guo,Shuigeng Zhou*

Main category: cs.CV

TL;DR: GloTok是一种新的图像标记化方法，通过全局关系信息建模更均匀的语义分布，结合码本直方图关系学习和残差学习模块，在无需预训练模型直接访问的情况下实现高质量图像重建和生成。
...
Method: 提出全局视角标记器(GloTok)：1) 码本直方图关系学习方法，将预训练模型在整个数据集上建模的语义转移到语义码本；2) 残差学习模块恢复细粒度细节以减少量化带来的重建误差。

Result: 在标准ImageNet-1k基准测试中，该方法实现了最先进的重建性能和生成质量。

Conclusion: GloTok通过全局关系建模产生更均匀分布的语义潜在表示，有助于自回归模型训练生成高质量图像，且无需在训练过程中直接访问预训练模型。
...
### [68] [LSP-YOLO: A Lightweight Single-Stage Network for Sitting Posture Recognition on Embedded Devices](https://arxiv.org/abs/2511.14322)
*Nanjun Li,Ziyue Hao,Quanqiang Wang,Xuanyin Wang*

Main category: cs.CV

TL;DR: 提出LSP-YOLO轻量级单阶段网络，用于嵌入式设备上的坐姿识别，通过PConv和SimAM模块减少计算成本，在PC上达到94.2%准确率和251FPS，模型仅1.9MB。


<details>
  <summary>Details</summary>
Motivation: 随着久坐行为增加，不良坐姿导致的健康问题日益受到关注。现有方法多为两阶段流水线，存在侵入性强、计算密集、嵌入式设备实时性差等问题。

Method: 基于YOLOv11-Pose，设计轻量模块Light-C3k2（集成PConv和SimAM），在识别头中通过点卷积将关键点直接映射到姿势类别，采用中间监督实现姿态估计与分类的高效融合。构建包含6种姿势类别的5000张图像数据集。

Result: 最小模型LSP-YOLO-n在PC上达到94.2%准确率和251FPS，模型大小仅1.9MB。在SV830C + GC030A平台上展示了受限计算资源下的实时高精度推理。

Conclusion: 该方法具有高效、轻量和可部署的特点，适用于智能教室、康复和人机交互应用。
...
### [69] [PAVE: An End-to-End Dataset for Production Autonomous Vehicle Evaluation](https://arxiv.org/abs/2511.14185)
*Xiangyu Li,Chen Wang,Yumao Liu,Dengbo He,Jiahao Zhang,Ke Ma*

Main category: cs.CV

TL;DR: 首个完全由自动驾驶模式在真实世界收集的端到端基准数据集，包含100+小时自然驾驶数据，用于评估自动驾驶车辆的真实行为安全。


<details>
  <summary>Details</summary>
Motivation: 现有数据集多为人工驾驶模式收集，无法真实评估自动驾驶车辆的行为安全，需要完全由自动驾驶模式收集的数据来评估黑盒控制下的真实安全性。

Method: 收集超过100小时来自市场多个量产自动驾驶车辆的自然驾驶数据，分割为32,727个关键帧，包含同步相机图像、高精度GNSS/IMU数据、车辆轨迹以及详细的2D标注。

Result: 数据集提供丰富的场景级属性，包括驾驶意图、区域类型、光照条件、天气、路面状况等，端到端运动规划模型在自动驾驶帧上的平均位移误差为1.4米。
...
TL;DR: Bapp移动应用使用Google ML Kit进行实时眼睑运动分析，在45个患者视频上验证显示98.4%精确度、96.9%召回率和98.3%总体准确率，为眼睑运动监测提供了便携可靠的解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有眼睑运动评估工具复杂、昂贵且临床应用有限，需要开发便携、可访问的客观监测工具来替代传统手动眨眼计数方法。

Method: 使用Flutter框架开发Bapp移动应用，集成Google ML Kit进行设备端实时眼睑运动分析，通过45个真实患者视频进行验证，由眼科专家手动标注作为基准真值。

Result: 评估显示Bapp达到98.4%精确度、96.9%召回率和98.3%总体准确率，证明其作为眼睑运动监测工具的可靠性。

Conclusion: Bapp是监测正常和异常眼睑运动的便携、可访问且客观的工具，为持续眼部健康监测和术后评估提供了有前景的替代方案。
...
### [71] [Hierarchical Semantic Learning for Multi-Class Aorta Segmentation](https://arxiv.org/abs/2511.14187)
*Pengcheng Shi*

Main category: cs.CV

TL;DR: 提出了一种基于课程学习策略的分形softmax方法，用于主动脉血管的层次语义分割，解决了血管结构中的类别不平衡问题，显著提高了分割精度和效率。


<details>
  <summary>Details</summary>
Motivation: 主动脉血管病理需要及时干预，现有方法往往忽略层次解剖关系，且难以处理血管结构中固有的严重类别不平衡问题。

Method: 采用课程学习策略，结合新颖的分形softmax进行层次语义学习，通过从简单到复杂逐步分解结构来学习解剖约束，并使用两阶段推理策略加速模型。

Result: 在验证集上，层次语义损失将nnU-Net ResEnc M的Dice分数提高了11.65%；在测试集上，提出的模型比基线高出5.6%的Dice分数，推理速度提升了五倍。

Conclusion: 该方法在分割精度和效率方面都有显著提升，适用于实时临床应用，代码已公开。
...
TL;DR: 提出了首个针对自动驾驶中立体匹配模型的纹理化物理对抗攻击方法，使用3D全局伪装纹理而非局部2D补丁，确保视觉一致性和攻击有效性。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击多采用2D补丁，主要针对单目感知，而立体双目深度估计中的物理对抗样本有效性尚未充分探索。

Method: 采用3D物理对抗样本和全局伪装纹理，提出3D立体匹配渲染模块处理双目相机视差效应，并开发细粒度优化的融合攻击方法。

Result: 广泛评估表明，该方法能成功欺骗立体模型产生错误深度信息，显著提升了现有隐藏攻击的隐蔽性和杀伤力。

Conclusion: 该方法在自动驾驶场景下有效攻击立体匹配模型，为立体视觉系统的安全性提供了重要见解。
...
### [73] [Online Data Curation for Object Detection via Marginal Contributions to Dataset-level Average Precision](https://arxiv.org/abs/2511.14197)
*Zitang Sun,Masakazu Yoshimura,Junji Otsuka,Atsushi Irie,Takeshi Ohashi*

Main category: cs.CV

TL;DR: DetGain是一种专门用于目标检测的在线数据筛选方法，通过估计每张图像对数据集平均精度(AP)的边际扰动来选择信息丰富的训练样本。


<details>
  <summary>Details</summary>
Motivation: 高质量数据在规模定律下已成为进步的主要驱动力，但在目标检测领域，由于结构复杂性和领域差距，现有的在线采样策略很少能有效应用。

Method: 通过建模全局分数分布，DetGain高效估计全局AP变化，并计算师生贡献差距来在每次迭代中选择信息丰富的样本。该方法与架构无关且侵入性最小。

Result: 在COCO数据集上的实验表明，使用多个代表性检测器都能获得一致的精度提升。DetGain在低质量数据下表现出强大的鲁棒性，并能与知识蒸馏技术有效结合。
...
### [74] [Agentic Video Intelligence: A Flexible Framework for Advanced Video Exploration and Understanding](https://arxiv.org/abs/2511.14446)
*Hong Gao,Yiming Bao,Xuezhen Tu,Yutong Xu,Yue Jin,Yiyang Mu,Bin Zhong,Linan Yue,Min-Ling Zhang*

Main category: cs.CV

TL;DR: AVI是一个无需训练的视频智能代理框架，通过检索-感知-审查的三阶段推理过程，结合结构化视频知识库和开源模型集成，实现了高效且可解释的视频理解。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解方法存在单次处理、证据重访能力有限、依赖昂贵专有模型或需要大量强化学习训练的问题，需要更灵活高效的解决方案。

Method: 提出三阶段推理过程（检索-感知-审查）、结构化视频知识库（实体图）和多粒度集成工具，使用开源模型集成替代专有API。

Result: 在LVBench、VideoMME-Long、LongVideoBench和Charades-STA等基准测试中取得有竞争力的性能，同时提供更好的可解释性。

Conclusion: AVI框架通过系统级设计和优化，实现了无需训练的高效视频理解，在性能和可解释性方面都表现出色。
...
### [75] [InstantViR: Real-Time Video Inverse Problem Solver with Distilled Diffusion Prior](https://arxiv.org/abs/2511.14208)
*Weimin Bai,Suzhe Xu,Yiwei Ren,Jinhua Hao,Ming Sun,Wenzheng Chen,He Sun*

Main category: cs.CV

TL;DR: InstantViR是一个基于预训练视频扩散先验的超快速视频重建框架，通过将双向视频扩散模型蒸馏为因果自回归学生模型，在单次前向传播中完成视频恢复，实现35+FPS的实时性能，比迭代视频扩散求解器快100倍。


<details>
  <summary>Details</summary>
Motivation: 解决视频逆问题中高质量感知重建与低延迟要求之间的矛盾。现有扩散方法要么存在时间伪影，要么迭代后验采样速度太慢，无法满足实时应用需求。

Method: 将预训练的双向视频扩散模型（教师）蒸馏为因果自回归学生模型，仅需教师扩散模型和已知退化算子，无需外部配对数据。使用LeanVAE替换VAE骨干，通过教师空间正则化蒸馏实现低延迟潜在空间处理。

Result: 在流式随机修复、高斯去模糊和超分辨率任务中，InstantViR匹配或超越扩散基线的重建质量，在NVIDIA A100 GPU上达到35+FPS，比迭代视频扩散求解器快100倍。
...
TL;DR: 提出了一种测量约束采样(MCS)方法，用于盲人脸恢复(BFR)，通过文本提示生成多样化的高质量重建结果，解决了现有方法只能产生确定性结果的问题。
...
Conclusion: MCS方法能够有效实现多样化的盲人脸恢复，通过文本提示控制生成不同的高质量重建结果。
...
### [77] [Deep Learning-Based Regional White Matter Hyperintensity Mapping as a Robust Biomarker for Alzheimer's Disease](https://arxiv.org/abs/2511.14588)
*Julia Machnio,Mads Nielsen,Mostafa Mehdipour Ghazi*

Main category: cs.CV

TL;DR: 提出了一个深度学习框架用于WMH分割和定位，结果显示区域WMH体积在疾病分类中优于全局病变负担，与脑萎缩指标结合可达到AUC 0.97。


<details>
  <summary>Details</summary>
Motivation: 现有WMH分割方法主要提供全局病变负荷，忽略了不同白质区域的空间分布差异，而区域分布可能对神经退行性疾病诊断有重要价值。

Method: 使用深度学习框架进行WMH分割和定位，在解剖学定义区域内量化WMH负荷，并结合脑结构体积评估诊断价值。

Result: 区域WMH体积在疾病分类中始终优于全局病变负担，与脑萎缩指标结合后性能进一步提升，AUC最高达0.97。前部白质束等特定区域与诊断状态可重复相关。
...
TL;DR: 提出了一种自回归扩散模型，用于语音驱动的3D面部动画，通过流式处理音频输入来解决长序列处理问题和延迟问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法一次性处理整个音频序列，在处理超出训练范围的音频序列时性能较差，且在长音频输入时存在显著延迟。

Method: 使用自回归扩散模型，选择有限数量的历史帧作为运动上下文，与音频输入结合形成动态条件，引导扩散过程迭代生成面部运动帧。

Result: 实现了灵活处理不同长度音频的能力，达到与音频时长无关的低延迟，并开发了实时交互演示。

Conclusion: 该方法在保持高质量结果的同时实现了实时合成，解决了长音频序列处理和延迟问题。
...
TL;DR: 本文提出了一种主动感知策略（APS）用于3D人体运动预测，通过商空间表示和辅助学习目标来增强时空建模能力，显著提升了预测性能。
...
Method: 设计了数据感知模块将姿态投影到商空间，解耦运动几何与坐标冗余；引入网络感知模块通过恢复性学习主动学习时空依赖关系；使用专门设计的辅助学习网络从扰动信息中主动适应学习。

Result: 在多个数据集上取得新的最先进性能：H3.6M提升16.3%，CMU Mocap提升13.9%，3DPW提升10.1%。
...
### [80] [CCSD: Cross-Modal Compositional Self-Distillation for Robust Brain Tumor Segmentation with Missing Modalities](https://arxiv.org/abs/2511.14599)
...
<details>
  <summary>Details</summary>
Motivation: 临床中多模态MRI经常存在模态缺失，严重影响深度学习分割模型的性能和泛化性

Method: 采用共享-特定编码器-解码器架构，结合层次模态自蒸馏和渐进模态组合蒸馏两种自蒸馏策略
...
TL;DR: WeSTAR是一个参数高效的弱监督自训练框架，通过正则化增强单目深度估计基础模型在未见领域的鲁棒性，结合密集自训练、语义感知层次归一化、成对深度弱监督和权重正则化。


<details>
  <summary>Details</summary>
Motivation: 基础模型在单目深度估计中展现了强大的零样本泛化能力，但当下游任务有少量数据可用时，能否进一步提升性能？

Method: 采用密集自训练目标作为结构自监督，引入语义感知层次归一化进行多尺度结构归一化，使用成对深度弱监督提供序数约束，并通过权重正则化稳定训练。

Result: 在现实和损坏的外分布数据集上的广泛实验表明，WeSTAR持续改善泛化性能，在多种基准测试中达到最先进水平。

Conclusion: WeSTAR框架有效提升了基础模型在未见和多样化领域的鲁棒性，证明了参数高效适应策略的优越性。
...
### [82] [MRI Embeddings Complement Clinical Predictors for Cognitive Decline Modeling in Alzheimer's Disease Cohorts](https://arxiv.org/abs/2511.14601)
...
<details>
  <summary>Details</summary>
Motivation: 准确建模阿尔茨海默病的认知衰退对于早期分层和个性化管理至关重要，但表格预测因子捕捉细微脑变化的能力有限。

Method: 引入基于动态时间规整聚类的轨迹感知标签策略，通过无监督重建训练3D视觉transformer获得解剖保持的MRI嵌入，并与表格表征和卷积网络基线进行比较。

Result: 临床和体积特征在预测轻度和重度进展时达到约0.70的AUC，而ViT模型的MRI嵌入在区分认知稳定个体时表现最佳（AUC 0.71），但所有方法在异质中度组中表现不佳。

Conclusion: 临床特征在识别高风险极端病例方面表现优异，而基于transformer的MRI嵌入对稳定性标志更敏感，这推动了多模态融合策略的发展。
...
TL;DR: 提出了一种基于LiDAR定位的GNSS-free协作感知框架，包括轻量级姿态生成器PGC和姿态感知时空对齐变换器PASTAT，在GNSS拒绝环境下实现最先进的性能。
...
Method: 1) 轻量级姿态生成器PGC估计紧凑姿态和置信度表示；2) PASTAT进行置信度感知的空间对齐并捕获关键时间上下文；3) 创建V2VLoc仿真数据集支持定位和检测任务。

Result: 在V2VLoc数据集上的广泛实验表明，该方法在GNSS拒绝条件下达到最先进性能，在真实世界V2V4Real数据集上的扩展实验验证了PASTAT的有效性和泛化能力。

Conclusion: 提出的GNSS-free协作感知框架通过LiDAR定位和时空对齐技术，在GNSS拒绝环境中实现了鲁棒的协作感知性能，具有实际应用价值。
...
### [84] [Improving segmentation of retinal arteries and veins using cardiac signal in doppler holograms](https://arxiv.org/abs/2511.14654)
*Marius Dubosc,Yann Fischer,Zacharie Auray,Nicolas Boutry,Edwin Carlinet,Michael Atlan,Thierry Geraud*

Main category: cs.CV

TL;DR: 提出了一种利用标准分割架构对时间多普勒全息图进行动静脉分割的方法，通过结合脉搏分析特征，使传统U-Net能够利用时间动态信息
...
Method: 使用标准分割架构，结合专用脉搏分析流程提取的特征，使传统U-Net能够利用时间动态信息
...
TL;DR: 提出了ManipBench大规模AI图像编辑检测基准和ManipShield统一检测模型，解决了现有基准内容多样性不足、模型覆盖窄、可解释性差的问题。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型的快速发展，AI图像编辑技术已远超传统深度伪造，给检测带来新挑战。现有检测基准存在内容多样性有限、生成模型覆盖范围窄、可解释性不足等问题，限制了检测方法的泛化和解释能力。

Method: 构建包含45万张图像的ManipBench基准，涵盖25个最先进图像编辑模型和12种编辑类别，其中10万张图像带有边界框、判断线索和文本解释标注。基于此提出ManipShield模型，采用多模态大语言模型，结合对比LoRA微调和任务特定解码器，实现统一的图像编辑检测、定位和解释。

Result: 在ManipBench和多个公共数据集上的广泛实验表明，ManipShield实现了最先进的性能，并对未见过的编辑模型表现出强大的泛化能力。

Conclusion: ManipBench基准和ManipShield模型为AI图像编辑检测提供了全面的评估框架和有效的解决方案，两者将在发表时公开。
...
TL;DR: 提出基于高斯泼溅的低秩张量表示框架(GSLR)，通过2D和1D高斯泼溅分别生成潜在张量和变换矩阵，解决了传统t-SVD方法在捕捉局部高频信息方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统张量奇异值分解(t-SVD)方法存在两个关键限制：潜在张量近似粗糙无法准确捕捉空间局部高频信息；变换矩阵由固定基原子组成无法精确捕捉模式3纤维的局部高频信息。

Method: 使用定制的2D高斯泼溅生成潜在张量，1D高斯泼溅生成变换矩阵，两者在该表示框架中不可或缺且互补，具有强大的表示能力，尤其擅长局部高频信息。

Result: 在多维图像恢复的广泛实验中，GSLR始终优于最先进方法，特别是在捕捉局部高频信息方面表现突出。

Conclusion: GSLR框架能够紧凑且连续地表示多维图像，通过高斯泼溅技术有效解决了传统t-SVD方法的局限性，在图像恢复任务中展现出优越性能。
...
### [87] [Impact of Image Resolution on Age Estimation with DeepFace and InsightFace](https://arxiv.org/abs/2511.14689)
*Shiyar Jamo*
...
<details>
  <summary>Details</summary>
Motivation: 年龄验证中图像分辨率差异很大，需要研究分辨率对年龄估计准确性的影响。

Method: 使用IMDB-Clean数据集的1000张图像，处理成7种不同分辨率，共7000个测试样本，评估DeepFace和InsightFace的性能。

Result: 224x224像素时性能最佳，DeepFace MAE为10.83年，InsightFace MAE为7.46年；低分辨率MAE显著增加，过高分辨率也会降低准确性；InsightFace在所有分辨率下都比DeepFace更快。
...
Result: VLM3D在两种不同范式上表现出通用性：作为优化管道的奖励目标，在标准基准测试中显著优于现有方法；作为前馈管道的测试时引导模块，能够主动纠正SOTA原生3D模型的严重空间错误。

Conclusion: VLM3D为将VLM丰富的语言基础语义和空间理解注入多样化3D生成管道提供了一条原则性和可泛化的路径。
...
### [89] [Seeing Beyond the Image: ECG and Anatomical Knowledge-Guided Myocardial Scar Segmentation from Late Gadolinium-Enhanced Images](https://arxiv.org/abs/2511.14702)
*Farheen Ramzan,Yusuf Kiberu,Nikesh Jathanna,Meryem Jabrane,Vicente Grau,Shahnaz Jamil-Copley,Richard H. Clayton,Chen,Chen*

Main category: cs.CV

TL;DR: 提出了一种新颖的多模态框架，将ECG电生理信息与AHA-17图谱的解剖先验相结合，用于LGE-MRI心肌瘢痕分割，通过时间感知特征融合机制处理非同步采集数据，显著提升了分割性能。


<details>
  <summary>Details</summary>
Motivation: LGE心脏MRI中的心肌瘢痕准确分割对于评估组织活性至关重要，但由于对比度变化和成像伪影而具有挑战性。ECG信号提供补充生理信息，传导异常有助于定位或提示瘢痕区域。

Method: 提出多模态框架，集成ECG电生理信息和AHA-17图谱解剖先验。由于ECG和LGE-MRI非同步采集，引入时间感知特征融合(TAFF)机制，基于采集时间差动态加权和融合特征。

Result: 在临床数据集上评估，相比最先进的仅图像基线(nnU-Net)，瘢痕平均Dice分数从0.6149显著提升至0.8463，在精度(0.9115)和灵敏度(0.9043)方面均表现优异。
...
### [90] [Free Lunch to Meet the Gap: Intermediate Domain Reconstruction for Cross-Domain Few-Shot Learning](https://arxiv.org/abs/2511.14279)
*Tong Zhang,Yifan Zhao,Liangyu Wang,Jia Li*

Main category: cs.CV

TL;DR: 提出了一种基于中间域代理的跨域少样本学习方法，通过源域特征嵌入构建代码本，重构目标域特征，并利用中间域属性进行快速域对齐，在8个基准测试中超越现有最佳模型。


<details>
  <summary>Details</summary>
Motivation: 解决跨域少样本学习中的语义不匹配、大域差异和数据稀缺三重挑战，不同于现有方法关注通用表示，尝试构建中间域代理作为学习指导。

Method: 使用源域特征嵌入构建代码本作为中间域代理，重构目标域特征，探索中间域代理的视觉风格和语义内容属性，开发快速域对齐方法指导目标域特征转换。

Result: 在8个跨域少样本学习基准测试中显著超越了当前最先进的模型性能。

Conclusion: 通过中间域重构和目标特征转换的协同学习，有效解决了跨域少样本学习的关键挑战，取得了优越的性能表现。
...
### [91] [Zero-shot Synthetic Video Realism Enhancement via Structure-aware Denoising](https://arxiv.org/abs/2511.14719)
...
Method: 使用扩散视频基础模型，通过辅助模型提取合成视频的结构感知信息（深度图、语义图、边缘图）作为生成过程的约束条件，无需微调。

Result: 在实验中超越了现有基线方法，在保持最先进真实感质量的同时，与原始视频的结构一致性更好。

Conclusion: 该方法是一种简单通用且强大的合成视频真实感增强方法，有效平衡了真实感和结构保持。
...
### [92] [NeuralSSD: A Neural Solver for Signed Distance Surface Reconstruction](https://arxiv.org/abs/2511.14283)
*Zi-Chen Xi,Jiahui Huang,Hao-Xiang Chen,Francis Williams,Qun-Ce Xu,Tai-Jiang Mu,Shi-Min Hu*

Main category: cs.CV

TL;DR: NeuralSSD是一种基于神经Galerkin方法的3D隐式表面重建方法，通过新颖的能量方程和卷积网络从点云数据重建高质量表面。


<details>
  <summary>Details</summary>
Motivation: 现有的隐式场参数化方法缺乏确保表面与输入数据紧密拟合的明确机制，需要解决点云信息可靠性平衡的问题。

Method: 提出新颖的能量方程来平衡点云信息的可靠性，并引入新的卷积网络学习三维信息以实现优越的优化结果。

Result: 在ShapeNet和Matterport等挑战性数据集上评估，在表面重建精度和泛化性方面达到最先进水平。

Conclusion: NeuralSSD能够确保重建表面紧密贴合原始输入点，并从点云中推断有价值的归纳偏差，实现高度准确和稳定的表面重建。
...
### [93] [ARC Is a Vision Problem!](https://arxiv.org/abs/2511.14761)
*Keya Hu,Ali Cy,Linlu Qiu,Xiaoman Delores Ding,Runqian Wang,Yeyin Eva Zhu,Jacob Andreas,Kaiming He*

Main category: cs.CV

TL;DR: 该论文提出了一种视觉中心的ARC问题解决方法，将抽象推理任务视为图像到图像的转换问题，使用Vision Transformer架构，在ARC-1基准上达到60.4%的准确率。


<details>
  <summary>Details</summary>
Motivation: ARC任务本质上是视觉性的，但现有研究多从语言角度处理。作者希望从视觉中心的角度重新构建ARC问题，利用视觉先验知识。

Method: 将ARC输入表示为"画布"，应用标准视觉架构（如ViT）进行图像到图像映射，仅使用ARC数据进行从头训练，通过测试时训练实现泛化。

Result: 在ARC-1基准上达到60.4%的准确率，显著优于其他从头训练的方法，与领先的LLMs竞争，接近人类平均表现水平。

Conclusion: 视觉中心的方法在抽象推理任务上表现出色，证明了将ARC视为视觉问题的有效性，为抽象推理研究提供了新视角。
...
TL;DR: 提出NeuralBoneReg，一种自监督的表面配准框架，用于解决计算机辅助骨科手术中多模态骨表面配准的挑战，使用3D点云作为模态无关表示，无需跨主体训练数据。


<details>
  <summary>Details</summary>
Motivation: 计算机辅助骨科手术中，术前和术中数据的模态异质性使得配准具有挑战性且容易出错，需要鲁棒、自动和模态无关的骨表面配准方法。

Method: 包含两个模块：学习术前骨模型的隐式神经无符号距离场(UDF)，以及基于MLP的配准模块，通过生成变换假设来执行全局初始化和局部细化，对齐术中点云与神经UDF。
...
TL;DR: 提出了Diff-NAF框架，通过结合神经衰减场和条件扩散模型，在超稀疏视角条件下迭代优化多源静态CT重建质量
...
Result: 在多个模拟3D CT体积和真实投影数据上，Diff-NAF在超稀疏视角条件下取得了最佳性能

Conclusion: Diff-NAF通过迭代细化策略有效提升了超稀疏视角条件下CT重建的完整性和保真度
...
TL;DR: 提出了Dental3R方法，用于从稀疏的口内照片进行3D重建，解决了传统方法在远程正畸中无法使用口内扫描仪的局限性。通过几何感知配对策略和基于小波的3D高斯溅射，实现了高质量的重建效果。


<details>
  <summary>Details</summary>
Motivation: 传统口内扫描方法无法用于远程正畸，而稀疏的智能手机图像重建面临大视角基线、光照不一致和镜面反射等挑战，现有方法容易产生过度平滑的重建结果，丢失关键诊断细节。

Method: 1. 几何感知配对策略(GAPS)选择高价值图像对；2. 基于恢复的位姿和点云训练3D高斯溅射模型；3. 使用小波正则化目标函数，通过离散小波变换保持频带保真度。

Result: 在950个临床病例和195个视频测试集上验证，Dental3R能有效处理稀疏、无位姿输入，在牙齿咬合可视化方面实现了优异的新视角合成质量，优于现有最先进方法。

Conclusion: Dental3R为远程正畸提供了一种鲁棒、高保真的3D重建解决方案，能够从稀疏的口内照片中恢复精细的牙齿结构细节。
...
TL;DR: 提出Step by Step Network (StepsNet)解决深度网络中的短路退化和宽度限制问题，通过通道维度特征分离和逐步学习机制提升深度模型性能


<details>
  <summary>Details</summary>
Motivation: 尽管理论表明更深网络具有指数级能力提升，但现有残差架构在深度扩展时面临短路退化和宽度限制问题，无法充分发挥理论潜力

Method: 提出StepsNet架构，沿通道维度分离特征，通过堆叠宽度递增的块实现渐进式学习，缓解短路退化和宽度限制

Result: 在图像分类、目标检测、语义分割和语言建模等任务中均优于传统残差模型

Conclusion: StepsNet作为残差架构的泛化改进，在多种任务中展现出优越性能
...
TL;DR: ArchMap是一个无需训练、基于知识引导的框架，用于3D口腔扫描的结构化理解，通过几何感知的牙弓展平模块和牙科知识库实现稳健的牙齿计数、解剖分割、牙列阶段分类等功能。
...
Result: 在1060个正畸前后病例上验证，在牙齿计数、解剖分割、牙列阶段分类及临床状况识别方面表现稳健，相比监督方法和VLM基线具有更高准确性和稳定性。
...
TL;DR: DentalSCR是一个用于口腔内扫描模型与侧位头颅X光片3D-2D配准的稳定轮廓引导框架，通过构建统一解剖坐标系和优化轮廓匹配来解决临床X光片中常见的投影畸变和低对比度问题。


<details>
  <summary>Details</summary>
Motivation: 传统的基于强度的配准方法在真实临床条件下（如投影放大、几何畸变、低对比度牙冠等）表现不稳定，容易导致配准失败或解剖学上不合理的对齐结果。

Method: 首先构建U-Midline Dental Axis建立统一跨弓解剖坐标系，然后通过表面DRR和Gaussian splatting生成X光片样投影，最后使用对称双向Chamfer距离在分层粗到细策略下优化2D相似变换。

Result: 在34个专家标注的临床病例上评估，显示显著减少了标志点误差（特别是后牙区域），下颌骨误差分布更集中，曲线级别的Chamfer和Hausdorff距离表现良好。
...
TL;DR: ARC-Chapter是首个基于百万级长视频章节数据训练的大规模视频分章模型，通过双语、时序定位和分层标注，在视频分章任务上实现了显著的性能提升。
...
Method: 构建了双语英中章节数据集，统一ASR转录、场景文本和视觉描述为多级标注；设计了GRACE评估指标，结合多对一段落重叠和语义相似度；通过数据扩展（数据量和标注强度）实现性能提升。

Result: ARC-Chapter在F1分数上比之前最佳方法提升14.0%，在SODA分数上提升11.3%；在YouCook2密集视频描述等下游任务上也表现出优异的迁移能力。

Conclusion: ARC-Chapter通过大规模数据训练和精心设计的评估指标，在视频分章任务上建立了新的最先进水平，并展示了良好的泛化能力。
...
TL;DR: 提出Image-Based Gaussian Splatting方法，通过结合3DGS基础颜色和从训练图像学习的高频残差，显著提升渲染质量，特别是高光等视角相关效果，且不增加存储开销。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法使用低阶球谐函数难以捕捉空间变化的颜色和视角相关效果（如高光），而现有增强方法要么全局纹理图处理复杂场景困难，要么逐高斯纹理图存储开销大。

Method: 将像素颜色建模为标准3DGS渲染的基础颜色与从邻近训练图像学习的残差组合，利用高分辨率源图像进行精细细节和视角特定颜色建模。

Result: 在标准NVS基准测试中，该方法在渲染质量上显著优于先前的Gaussian Splatting方法。

Conclusion: 提出的Image-Based Gaussian Splatting方法能够渲染高频细节和准确的视角相关效果，同时保持存储效率。
...
TL;DR: O3SLM是一个大型视觉语言模型，通过新构建的大规模图像-草图-指令三元组数据集，显著提升了LVLM对抽象手绘草图的解释能力，在多个草图任务上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型在解释抽象视觉输入（特别是手绘草图）方面能力有限，主要瓶颈是缺乏同时建模草图、真实图像和自然语言指令的大规模数据集。

Method: 提出两个关键贡献：(1) 构建大规模图像-草图-指令三元组数据集用于预训练和指令调优；(2) 在该数据集上训练O3SLM模型，并在多个草图任务上进行评估。

Result: 在对象定位、计数、图像检索（SBIR和细粒度SBIR）和视觉问答等任务上，O3SLM显著优于现有LVLM，在草图理解和推理方面达到最先进性能。

Conclusion: 通过专门设计的数据集和训练方法，O3SLM成功解决了LVLM在草图理解方面的局限性，为抽象视觉输入的解释提供了有效解决方案。
...
TL;DR: 提出JFD3框架，联合进行特征域去模糊和检测，通过双分支架构和特征一致性监督提升红外无人机模糊目标的检测性能。
...
Method: 设计共享权重的双分支架构，清晰分支引导模糊分支：1）轻量级特征恢复网络；2）频率结构引导模块；3）特征一致性自监督损失。

Result: 在IRBlurUAV基准上，JFD3实现了优越的检测性能，同时保持实时效率。

Conclusion: JFD3通过联合特征域去模糊和检测，有效提升了模糊红外无人机目标的检测性能，证明了该方法在真实场景中的有效性。
...
TL;DR: 提出了一种肩部姿态评估算法，用于自动评估生物识别证件中肩部姿态的合规性，仅使用两个肩部标志点的3D坐标来量化肩部偏转和滚动角度。


<details>
  <summary>Details</summary>
Motivation: 国际生物识别证件标准要求严格符合姿态要求，但现有自动化质量评估方法缺乏针对肩部姿态的定量评估方法。

Method: 使用常见姿态估计框架提供的两个肩部标志点的3D坐标，量化肩部偏转和滚动角度，提出肩部姿态评估算法。
...
Method: 开发了TLS-Assist框架，将检测结果转换为结构化自然语言消息注入LLM输入，强制关注安全关键提示。该框架即插即用、模型无关，支持单视图和多视图相机设置。

Result: 在CARLA的LangAuto基准测试中，相对LMDrive性能提升达14%，相对BEVDriver提升7%，同时持续减少交通灯和标志违规。

Conclusion: TLS-Assist有效解决了LLM驾驶代理在交通规则执行和小物体检测方面的局限性，显著提升了自动驾驶的安全性和性能。
...
TL;DR: BEDLAM2.0是一个新的数据集，在BEDLAM基础上显著提升了多样性、真实性和功能，特别适用于训练世界坐标系下的人体运动估计方法。


<details>
  <summary>Details</summary>
Motivation: 从视频中推断3D人体运动是一个具有挑战性的问题，许多应用需要在世界坐标系中估计人体运动，特别是在同时存在人体和相机运动的情况下。该领域进展受到缺乏包含真实人体和相机运动的地面真实视频数据的限制。

Method: 开发了BEDLAM2.0数据集，相比BEDLAM引入了更多样化和真实的相机及相机运动，增加了身体形状、动作、服装、头发和3D环境的多样性和真实性，并添加了之前缺失的鞋子。

Result: BEDLAM2.0在训练世界坐标系下的人体估计方法方面显著优于BEDLAM。比较在BEDLAM和BEDLAM2.0上训练的最先进方法发现，BEDLAM2.0显著提高了准确性。

Conclusion: BEDLAM2.0是一个比BEDLAM更好的数据集，为训练3D人体姿态和运动回归器提供了更丰富、更真实的资源，特别适用于世界坐标系下的人体运动估计任务。
...
TL;DR: 提出了一种基于序数回归的糖尿病视网膜病变检测框架，在APTOS-2019数据集上取得了0.8992的QWK分数，创造了新的基准。
...
Method: 使用绿通道提取、噪声掩蔽和CLAHE等预处理方法提取相关特征，采用序数回归方法进行DR分类。
...
<details>
  <summary>Details</summary>
Motivation: 现有领域增量学习方法面临两难：将所有域投影到统一视觉空间会导致域间干扰和语义扭曲，而隔离域特定参数会导致知识碎片化，形成阻碍知识重用的"知识孤岛"。

Method: LAVA使用语言锚定的视觉对齐方法，通过文本参考锚点驱动相对对齐，引导每个域的可视表示保持一致的相对几何结构，该结构由类名之间的成对语义相似性定义。

Result: 在标准DIL基准测试上的广泛实验表明，LAVA相比最先进方法实现了显著的性能提升。

Conclusion: LAVA通过语言锚定的相对对齐方法有效解决了领域增量学习中的知识保持和知识重用问题，为跨域知识迁移提供了稳健的解决方案。
...
TL;DR: 提出Cranio-ID框架，通过YOLO-pose模型自动标注2D头骨X射线图像和光学图像中的标志点，然后使用图表示和跨模态注意力机制进行颅骨到面部的匹配。


<details>
  <summary>Details</summary>
Motivation: 传统颅骨标志点定位方法耗时且需要专业知识，现有深度学习方法缺乏大规模验证研究，可靠性不足。

Method: 1. 使用训练的YOLO-pose模型自动标注2D头骨X射线和光学图像中的标志点；2. 将标志点表示为图结构，使用跨注意力机制和最优传输框架进行跨模态图匹配。

Result: 在S2F和CUHK数据集上的实验表明，该框架在可靠性和准确性方面均有显著提升，在法医学颅骨到面部和素描到面部匹配中表现有效。

Conclusion: Cranio-ID框架为法医颅面识别提供了一种可靠且准确的跨模态匹配解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 探索婴儿阶段性视觉发展（低分辨率、色彩退化但时间连续）对构建鲁棒视觉智能的生态优势，为理解视觉智能涌现提供逆向工程框架。

Method: 使用CATDiet方法训练自监督学习模型：包括灰度到彩色(C)、模糊到清晰(A)、保持时间连续性(T)三个约束条件，并在对象中心视频上进行训练。

Result: 所有CATDiet变体在对象识别中表现出增强的鲁棒性，模型还显示出与生物发育模式一致的行为，包括类似猕猴V1区突触密度的神经可塑性变化和婴儿视觉悬崖反应。

Conclusion: 婴儿早期视觉经验的发展进程为理解机器中鲁棒视觉智能的涌现提供了有力的逆向工程框架，CombDiet方法在对象识别和深度感知任务上优于标准自监督学习。
...
Method: 提出DIR-TIR框架，包含两个专门模块：对话精炼模块主动询问用户提取关键信息并生成更精确的描述；图像精炼模块识别生成图像与用户意图之间的感知差距，策略性地减少视觉-语义差异。

Result: 在多个图像数据集上的综合实验表明，该对话式方法显著优于仅使用初始描述的基线方法，模块协同集成实现了更高的检索精度和更好的交互体验。

Conclusion: DIR-TIR框架通过多轮对话提供了比传统单查询方法更优越的可控性和容错性，在目标图像命中精度方面取得了显著提升。
...
TL;DR: CompEvent是一个基于复数神经网络的框架，通过全流程融合事件数据和RGB帧来增强低光视频去模糊能力，包含复数时序对齐GRU和复数空间-频率学习模块。


<details>
  <summary>Details</summary>
Motivation: 解决低光视频去模糊的挑战，特别是在夜间监控和自动驾驶等应用中，现有方法采用分阶段策略限制了对抗低光和运动模糊联合退化的效果。

Method: 提出CompEvent框架，包含两个核心组件：1) 复数时序对齐GRU，使用复数卷积并通过GRU迭代处理视频和事件流实现时序对齐和连续融合；2) 复数空间-频率学习模块，在空间和频域进行统一的复数信号处理。

Result: 广泛的实验表明，CompEvent在处理这一具有挑战性的任务上优于最先进的方法。

Conclusion: 通过利用复数神经网络的全方位表示能力，CompEvent实现了全流程时空融合，最大化模态间的互补学习，显著增强了低光视频去模糊能力。
...
TL;DR: 提出了一种物理引导的残差学习框架，通过预测BedMachine先验上的床厚度残差来重建冰下地形，结合多尺度质量守恒等物理约束，在格陵兰冰盖区域实现了优于传统方法的精度和物理一致性。


<details>
  <summary>Details</summary>
Motivation: 冰下地形对冰盖建模至关重要，但雷达观测数据稀疏且分布不均，需要开发能够有效利用物理约束和先验知识的方法来准确重建地形。

Method: 采用DeepLabV3+解码器和标准编码器（如ResNet-50）的架构，结合多尺度质量守恒、流向总变分、拉普拉斯阻尼、厚度非负性、斜坡先验一致性项以及基于置信度图的掩蔽Huber拟合等轻量级物理和数据项进行训练。

Result: 在格陵兰两个子区域上，该方法在测试核心区域实现了高精度和高结构保真度，优于U-Net、Attention U-Net、FPN和普通CNN等对比方法。

Conclusion: 残差先验设计结合物理约束能够产生空间连贯、物理合理的冰下地形，适用于领域偏移下的操作制图。
...
TL;DR: Gaussian Spatial Transport (GST) 是一个利用高斯泼溅技术从图像坐标空间到标注图的概率度量传输框架，通过贝叶斯概率计算传输计划，并推导出用于标准网络优化的损失函数，在人群计数和地标检测等任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统最优传输方案在训练过程中需要迭代计算传输计划，效率较低。本文旨在开发一个无需迭代计算传输计划的高效传输框架。
...
Conclusion: GST 框架成功消除了训练过程中迭代传输计划计算的需求，在保持性能的同时显著提升了效率。
...
TL;DR: SEED-SR是一种新的农场边界分割方法，通过在分割感知的潜在空间中执行超分辨率，而不是像素空间，实现了20倍尺度因子的分割图生成，显著提升了小农户农场边界分割的准确性。


<details>
  <summary>Details</summary>
Motivation: 解决小农户农场边界分割的挑战：高分辨率图像更新频率低，而低分辨率图像更新频率高但细节不足。现有参考超分辨率方法无法满足大尺度因子需求，且会平滑关键特征。

Method: 使用条件潜在扩散模型和大规模多光谱、多源地理空间基础模型，在分割感知的潜在空间中执行超分辨率，绕过像素空间的显式超分辨率任务。

Result: 在两个大型真实数据集上的实验显示，相比基于最先进参考超分辨率方法的方法，在实例分割和语义分割指标上分别实现了25.5%和12.9%的相对提升。

Conclusion: SEED-SR通过分割感知潜在空间超分辨率的方法，成功解决了大尺度因子下的农场边界分割问题，为农业监测提供了更有效的解决方案。
...
Conclusion: PAMM框架通过状态空间模型有效增强了多任务密集预测中的任务互联性，实现了精确的多任务交互和全局任务先验集成。
...
TL;DR: 提出D-PerceptCT架构，基于人类视觉系统原理增强低剂量CT图像质量，通过视觉双路径提取器和全局-局部状态空间块保留关键解剖结构和病理细节，并引入深度感知相关性损失函数。


<details>
  <summary>Details</summary>
Motivation: 低剂量CT图像质量较差，现有方法容易过度平滑导致关键细节丢失，需要开发能保留感知相关特征的增强方法。

Method: D-PerceptCT包含视觉双路径提取器（整合DINOv2语义先验与局部空间特征）和全局-局部状态空间块（捕获长距离信息和多尺度特征），并提出深度感知相关性损失函数。

Result: 在Mayo2016数据集上的实验表明，相比现有最优方法，D-PerceptCT能更好地保留LDCT图像的结构和纹理信息。

Conclusion: D-PerceptCT通过模拟人类视觉系统原理，有效增强了低剂量CT图像质量，保留了诊断所需的关键细节。
...
<details>
  <summary>Details</summary>
Motivation: 当前水下图像恢复和增强的深度学习方法受限于高质量配对数据集的稀缺，现有基准数据集通常依赖增强算法的手动选择结果，缺乏全局一致的颜色和真实监督信号，限制了模型在颜色恢复、图像增强和泛化方面的能力。

Method: 建立基于非配对图像到图像翻译的生成数据框架，将自然空中图像转换为6种代表性水下退化类型的图像，构建具有精确真实标签的大规模合成数据集。

Result: 在6种代表性网络架构和3个独立测试集上的广泛定量和定性实验表明，使用合成数据训练的模型在颜色恢复和泛化性能方面达到或优于现有基准数据集训练的模型。

Conclusion: 该研究为水下图像恢复和增强提供了可靠且可扩展的数据驱动解决方案，生成的合成数据集可有效促进模型学习从退化水下图像到原始场景外观的准确映射。
...
TL;DR: DeCo-VAE通过将视频内容解耦为关键帧、运动和残差三个组件，并分别为每个组件学习专用潜在表示，实现了紧凑的视频潜在表示，避免了现有VAE中帧内容相似性导致的冗余建模问题。
...
Method: 将视频内容分解为关键帧、运动和残差三个组件，为每个组件设计专用编码器，使用共享的3D解码器保持时空一致性，并采用解耦适应策略依次训练编码器。

Result: 广泛的定量和定性实验表明，DeCo-VAE实现了卓越的视频重建性能。

Conclusion: 通过显式解耦视频内容并学习专用潜在表示，DeCo-VAE能够有效减少冗余建模，实现紧凑且高质量的视频重建。
...
TL;DR: 提出了一种在共享空间中表示多个SDF的方法，通过结合泛化学习和过拟合学习策略的优势，以更紧凑的潜在表示恢复高保真几何细节。
...
Conclusion: 该方法能够以更紧凑的潜在表示恢复更多高保真几何细节，为多SDF分析提供了有效解决方案。
...
TL;DR: 提出了一种无需物体先验知识的手-物体交互场景几何与外观建模方法，基于动态3D高斯泼溅技术，通过交互感知的高斯模型、动态场和渐进式策略解决复杂交互中的遮挡和运动问题。


<details>
  <summary>Details</summary>
Motivation: 解决在没有任何物体先验知识的情况下，同时建模手-物体交互场景的几何和外观的挑战性问题，特别关注复杂的相互遮挡和边缘模糊情况。

Method: 使用交互感知的手-物体高斯模型引入可优化参数，采用分段线性假设；将手部信息融入物体变形场构建交互感知动态场；采用渐进式策略分别处理动态区域和静态背景；设计显式正则化确保平滑运动过渡和物理交互真实性。

Result: 实验表明该方法超越了现有的动态3D-GS方法，在手-物体交互重建方面达到了最先进的性能。

Conclusion: 该方法成功解决了无先验知识下的手-物体交互场景建模问题，通过创新的高斯模型、动态场设计和优化策略，实现了高质量的动态重建效果。
...
<details>
  <summary>Details</summary>
Motivation: 单一CNN流无法捕捉多尺度伪造伪影，限制了Deepfake检测的鲁棒性和泛化能力

Method: 三模态框架：RGB分支提取全局视觉不一致性，纹理分支检测细粒度混合伪影，频率分支识别周期性频谱噪声；使用注意力时序池化和自适应注意力融合

Result: 在Celeb-DF数据集上AUC达0.9752，F1分数0.9408，准确率0.9208，优于单流基线

Conclusion: 全面的特征融合提供了对细微伪造的优越鲁棒性，分支协同作用得到验证
...
<details>
  <summary>Details</summary>
Motivation: 现有基于显著性图的方法（如遮挡、GradCAM、相关性传播）只能突出单个切片中对预测贡献最大的区域，缺乏对模型全局行为的理解。

Method: 采用聚类技术分析卷积神经网络模型，通过聚类结果可视化来理解模型行为。
...
<details>
  <summary>Details</summary>
Motivation: 处理音视频令牌序列在OmniLLMs中造成了显著的计算瓶颈，现有令牌压缩方法尚未满足联合压缩多模态令牌的需求。

Method: 首先识别关键音频令牌，计算每个时间组的音频保留分数来捕捉信息密度，动态指导视频令牌剪枝，并使用交错时空方案压缩视频令牌。

Result: 实现了3.42倍推理加速和1.4倍内存减少，同时保持性能且无需训练。

Conclusion: OmniZip有效解决了音视频令牌压缩问题，显著提升了推理效率，是多模态理解领域的重要进展。
...
TL;DR: XAttn-BMD是一个多模态深度学习框架，通过双向交叉注意力机制融合髋部X射线图像和临床元数据来预测股骨颈骨密度，在回归和分类任务中均优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 骨健康不良是重要的公共卫生问题，低骨密度会增加骨折风险。现有方法需要专门的骨密度测量设备，而XAttn-BMD旨在利用常规髋部X射线和临床数据提供更便捷的骨密度评估方案。

Method: 提出双向交叉注意力机制动态整合图像和元数据特征，实现跨模态相互增强；使用加权平滑L1损失函数解决骨密度不平衡问题并优先处理临床重要病例。

Result: 在Hertfordshire队列研究数据上的实验表明，模型在回归泛化性和鲁棒性方面优于基线。交叉注意力融合相比简单特征拼接，MSE降低16.7%，MAE降低6.03%，R2分数提高16.4%。

Conclusion: XAttn-BMD通过有效的多模态融合和定制化损失函数，在股骨颈骨密度估计方面表现出色，具有在真实临床场景中应用的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的预测算法要么独立处理每个切片忽略3D结构，要么3D感知方法不可生成且扩展性差，需要一种能够生成可扩展3D组织转录组学图谱的方法。

Method: 在共享特征空间中检索相邻切片上的形态对应斑点，将跨切片上下文融合到轻量级ControlNet中；引入结合学习零膨胀负二项先验和空间经验先验的3D一致先验；使用全局注意力块实现线性扩展的3D H&E处理。
...
TL;DR: 提出BioST-GCN双流模型，结合姿态和生物力学信息进行跌倒预测，在模拟数据上表现优异但存在显著的模拟-现实差距，零样本泛化性能大幅下降。
...
TL;DR: 提出了一种名为\net{}的新方法，通过立体几何-纹理对齐和伪特征增强几何一致性来解决稀疏视图下高斯溅射的过拟合问题，实现更准确的表面重建和高质量的新视角合成。


<details>
  <summary>Details</summary>
Motivation: 稀疏视图下优化高斯溅射容易过拟合，导致重建质量下降。现有的扁平化高斯基元方法虽然能更好地拟合表面几何，但其增加的各项异性在稀疏视图场景中会加剧过拟合，阻碍准确的表面拟合并降低新视角合成性能。

Method: 1. 引入立体几何-纹理对齐，连接渲染质量和几何估计；2. 提出伪特征增强几何一致性，通过结合训练视图和未见视图来强制多视角几何一致性，有效缓解稀疏监督导致的过拟合。

Result: 在DTU、BlendedMVS和Mip-NeRF360数据集上的大量实验表明，该方法达到了最先进的性能水平。

Conclusion: 该方法能够重建更准确和详细的表面，同时保持高质量的新视角渲染，在稀疏视图场景中显著提升了表面重建和视图合成的性能。
...
TL;DR: SLAM-AGS是一个用于计算细胞学的多任务预训练框架，通过联合优化弱监督相似性目标和自监督对比目标，在低见证率下提升下游任务性能。
...
Result: 在公开骨髓细胞学数据集上，SLAM-AGS在0.5%-10%的模拟见证率下，相比其他预训练方法提升了袋级F1分数和Top 400阳性细胞检索性能，在低见证率下增益最大。
...
TL;DR: RepAir是一个用于3D气道分割的三阶段框架，结合nnU-Net网络和基于解剖学的拓扑校正，在健康和有严重气道病理的CT扫描上都优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 手动标注胸部CT扫描中的气道不切实际，而许多基于U-Net的自动方法会产生不连续的分割结果，阻碍可靠的生物标志物提取。

Method: 三阶段框架：1) 基于nnU-Net的网络生成初始气道掩码；2) 基于骨架的算法识别潜在不连续性并提议重新连接；3) 1D卷积分类器确定哪些候选连接对应真实解剖分支。

Result: 在ATM'22（健康受试者）和AeroPath（严重气道病理）两个数据集上，RepAir在体素级和拓扑指标上都优于现有的3D U-Net方法，产生更完整和解剖一致的气道树。

Conclusion: RepAir框架能够生成更完整和解剖一致的气道分割结果，在保持高分割精度的同时显著改善拓扑完整性。
...
TL;DR: HyMAD是一种基于时空特征融合的深度神经网络架构，用于从地震信号中检测和区分同时发生的多种活动（如人类入侵、动物移动和车辆行驶），解决了边境监控中重叠活动识别困难的问题。


<details>
  <summary>Details</summary>
Motivation: 地震传感器在边境监控中具有隐蔽性优势，但复杂嘈杂的地震信号使得准确检测和区分同时发生的多种活动成为主要挑战，错误识别会导致误分类、漏检和监控系统可靠性降低。

Method: 提出HyMAD框架，集成SincNet提取的频谱特征和RNN建模的时间依赖关系，使用自注意力层增强模态内表示，并通过跨模态融合模块实现稳健的多标签分类。

Result: 在真实边境监控场景收集的数据集上评估，该方法能够泛化到涉及人类、动物和车辆的复杂同时活动场景，取得了有竞争力的性能。

Conclusion: HyMAD提供了一个模块化框架，可在现实世界安全应用中扩展基于地震的活动识别能力。
...
TL;DR: 提出了一种无需训练的扩散变换器视频生成方法，通过向内滑动窗口注意力机制和双路径管道，实现超高分辨率视频合成，无需额外训练。
...
Method: 采用向内滑动窗口注意力机制，结合双路径管道和交叉注意力覆盖策略，通过局部注意力与全局感受野分支的协同工作确保整体一致性，并引入交叉注意力缓存策略提高效率。

Result: 实验表明该方法能够生成具有精细视觉细节的超高分辨率视频，在VBench上表现优于基于训练的方法，且具有竞争性或改进的效率。

Conclusion: 该方法在无需训练的情况下实现了高质量超高分辨率视频生成，在效率和性能方面都表现出色。
...
TL;DR: 提出了DSD框架，将潜在扩散模型的编码器、解码器和扩散网络统一为单个端到端可训练网络，解决了传统三部分架构的计算效率低下和性能不佳问题。


<details>
  <summary>Details</summary>
Motivation: 传统潜在扩散模型采用复杂的三部分架构（编码器、解码器、扩散网络），需要分阶段训练，计算效率低且性能次优，阻碍了与视觉基础模型的单网络架构统一。

Method: 提出扩散即自蒸馏（DSD）框架，通过修改训练目标来稳定潜在空间，防止"潜在崩溃"问题，实现单个网络的端到端训练。

Result: 在ImageNet 256×256条件生成任务上取得优异性能：FID=13.44/6.38/4.25，参数量仅42M/118M/205M，训练50个epoch，无需分类器自由引导。

Conclusion: DSD首次实现了稳定端到端训练单个网络同时学习编码、解码和扩散，为扩散模型与视觉基础模型的统一架构开辟了新途径。
...
TL;DR: 提出了一种基于神经场的3D城市环境隐式表示方法，通过向量场编码视图，支持快速查询和逆查询，用于城市分析任务如可见性评估、太阳能暴露分析等。


<details>
  <summary>Details</summary>
Motivation: 3D城市数据集日益增多，但由于计算瓶颈和交互复杂性，提取洞察仍然困难。复杂的几何结构导致高度遮挡，需要大量手动视角调整，使得大规模探索效率低下。

Method: 采用基于神经场的方法构建3D环境的隐式表示，通过向量场编码环境视图，支持直接查询（计算视图评估指标）和逆查询（避免遮挡、搜索匹配数据模式的视图）。

Result: 通过定量实验、真实世界案例研究和领域专家反馈验证了方法的有效性，在寻找理想视点、分析建筑立面可见性和评估室外空间视野方面表现出色。

Conclusion: 该方法为3D城市数据分析提供了高效的探索工具，支持关键城市分析任务，代码和数据已公开。
...
<details>
  <summary>Details</summary>
Motivation: 视频参与度识别面临主观标签和噪声问题，限制了模型性能。需要解决标签主观性和噪声对模型训练的负面影响。

Method: 使用VLMs优化标注并指导训练过程，通过问卷提取行为线索将数据分为高可靠性和低可靠性子集，结合课程学习和软标签精炼的训练策略，逐步引入模糊样本并调整监督以反映不确定性。

Result: 在EngageNet基准测试中（6个特征设置中的3个）最大提升+1.21%，在DREAMS/PAFE基准测试中F1分数分别提升+0.22/+0.06，超越了现有最佳方法。

Conclusion: 该方法证明了利用VLMs处理标签主观性的有效性，经典计算机视觉模型在精炼的高可靠性子集上训练并结合课程学习策略能够显著提升参与度识别性能。
...
<details>
  <summary>Details</summary>
Motivation: 构建一个统一的模型来同时处理图像理解、生成和编辑任务，通过共享奖励模型和指令对齐来提升整体性能。

Method: 1. 改进模型架构和训练流程；2. 提出统一的强化学习策略，通过共享奖励模型联合优化图像生成和编辑；3. 引入轻量级编辑指令对齐阶段以增强编辑指令理解。

Result: 在GenEval上获得0.89分，在ImgEdit上获得4.31分，超越了BAGEL等最先进模型，性能可与GPT-Image-1等专有模型相媲美。

Conclusion: UniGen-1.5展示了在图像理解、生成和编辑方面的竞争优势，通过统一的强化学习策略和指令对齐实现了显著的性能提升。
...
### [137] [Imagine in Space: Exploring the Frontier of Spatial Intelligence and Reasoning Efficiency in Vision Language Models](https://arxiv.org/abs/2511.13782)
*Xiaoxing Lian,Aidong Yang,Jun Zhu,Peng Wang,Yue Zhang*

Main category: cs.AI

TL;DR: SpatiaLite是一个合成基准测试，用于评估视觉语言模型的空间推理能力，发现当前先进VLMs主要依赖语言表示，在视觉中心任务中存在显著缺陷，并提出Imagery Driven Framework来改进空间推理。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型和视觉语言模型在逻辑推理、问题解决和决策方面表现出色，但空间推理（包括心理旋转、导航和空间关系理解）仍然是当前先进VLMs的重大挑战。研究者假设想象力（空间状态的内部模拟）是空间世界模型中的主导推理机制。

Method: 引入SpatiaLite合成基准测试，联合测量空间推理准确性和效率；通过综合实验分析VLMs的空间推理机制；提出Imagery Driven Framework (IDF)用于数据合成和训练，隐式构建内部世界模型。

Result: 三个关键发现：1) 先进VLMs主要依赖语言表示推理和想象力，在需要感知空间关系和3D几何变换的视觉中心任务中存在显著缺陷；2) 当前空间推理机制效率低下，随着变换复杂性增加，token使用量快速增长；3) IDF框架可以隐式构建对空间推理至关重要的内部世界模型。

Conclusion: 这项工作通过SpatiaLite基准测试描绘了先进VLMs的空间推理限制和模式，识别了关键缺陷，并为未来进展提供了信息。IDF框架展示了构建内部世界模型对改进VLMs空间推理能力的重要性。
...
### [138] [KANGURA: Kolmogorov-Arnold Network-Based Geometry-Aware Learning with Unified Representation Attention for 3D Modeling of Complex Structures](https://arxiv.org/abs/2511.13798)
...
<details>
  <summary>Details</summary>
Motivation: 现有预测模型难以捕捉微生物燃料电池阳极结构的复杂几何依赖性，影响性能优化。

Method: 使用Kolmogorov-Arnold网络进行函数分解，通过几何解耦表示学习和统一注意力机制来重构几何关系。

Result: 在ModelNet40数据集上达到92.7%准确率，在真实MFC阳极结构问题上达到97%准确率，优于15个SOTA模型。

Conclusion: KANGURA为三维几何建模提供了强大框架，为先进制造和质量驱动工程应用中的复杂结构优化开辟了新可能。
...
### [139] [When AI Does Science: Evaluating the Autonomous AI Scientist KOSMOS in Radiation Biology](https://arxiv.org/abs/2511.13825)
...
Method: 使用简单随机基因零基准测试KOSMOS在三个辐射生物学问题上的表现：DDR-p53关联、OGT/CDO1预测辐射响应模块、12基因特征预测前列腺癌放疗预后

Result: 假设1不支持（DDR与p53响应弱负相关），假设2中CDO1是明确异常值（r=0.70,p=0.0039），假设3获得一致性指数0.61但效应量不唯一

Conclusion: AI科学家能产生有用想法但需要严格审计，KOSMOS产生一个明确发现、一个不确定结果和一个错误假设
...
TL;DR: 本文研究了将Bjøru等人提出的反事实概率边界计算方法从马尔可夫模型扩展到半马尔可夫结构因果模型的方法，解决了外生变量可能影响多个内生变量的情况。


<details>
  <summary>Details</summary>
Motivation: 现有的反事实概率边界计算方法仅适用于马尔可夫模型，而半马尔可夫模型能够表示马尔可夫模型无法表示的混杂关系，因此需要扩展该方法以处理更一般的因果模型。

Method: 通过最小示例说明扩展的挑战，提出替代解决方案策略，并从理论和计算角度评估这些策略。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在智能交通系统中应用广泛，但极易受到越狱攻击，这可能导致模型生成有害内容，对交通安全构成严重威胁。

Method: 构建交通相关有害查询数据集；提出基于图像排版操纵和多轮提示的新型越狱攻击；开发多层响应过滤防御技术；使用GPT-4判断和人工验证评估攻击与防御效果。

Result: 实验证明所提出的越狱攻击方法在开源和闭源LVLMs上均有效，揭示了图像排版操纵和多轮提示带来的严重安全风险。

Conclusion: LVLMs在ITS中集成存在严重安全漏洞，需要更强大的防御机制来防止越狱攻击，确保模型安全可靠地应用于关键基础设施。
...
<details>
  <summary>Details</summary>
Motivation: 解决实时AI系统和关系数据库查询中，规则匹配可能遇到的指数级时间和空间复杂度问题，特别是当系统自动生成规则时容易产生最坏情况匹配模式。

Method: 采用两步法：前向传递构建/维护接地关系图，后向迭代器按需生成匹配，避免传统RETE算法中β-内存收集部分匹配的问题。

Result: 在性能评估中，CORGI在简单组合匹配任务上显著优于SOAR和OPS5的RETE实现。

Conclusion: CORGI算法使在线AI系统能够实用化，无需手工工程约束，同时避免不可预测的故障模式。
...
<details>
  <summary>Details</summary>
Motivation: 由于难以实时捕捉事故触发场景，获取真实的工作场所危险图像数据集具有挑战性，需要一种能够生成逼真危险场景图像的方法。
...
Conclusion: 该框架能够有效生成逼真的工作场所危险场景图像，为训练视觉模型检测工作场所危险提供了可行的数据增强方案。
...
<details>
  <summary>Details</summary>
Motivation: 整合AI智能体在音乐分析和教育领域的历史发展，评估其教学意义，并解决透明度、文化偏见和混合评估指标等关键挑战。

Method: 采用双案例方法：在中等教育中使用生成式AI平台培养分析和创造技能；设计用于符号音乐分析的多智能体系统，实现模块化、可扩展和可解释的工作流程。

Result: 实验结果表明，AI智能体在音乐模式识别、创作参数化和教育反馈方面优于传统自动化方法，具有更好的可解释性和适应性。
...
TL;DR: Syn-STARTS框架使用LLM生成大规模分类案例，解决了真实世界分类数据稀缺的问题，生成的合成数据与人工整理的真实数据在质量上难以区分。


<details>
  <summary>Details</summary>
Motivation: 大规模伤亡事件中分类决策至关重要，但真实分类数据难以收集，阻碍了AI模型的开发。需要合成数据来弥补数据稀缺问题。

Method: 开发Syn-STARTS框架，利用LLM生成分类案例，并与人工整理的TRIAGE开放数据集进行质量对比验证。

Result: 生成的分类案例在质量上与人工整理数据无法区分，且在不同分类类别（绿色、黄色、红色、黑色）中LLM评估结果高度稳定。

Conclusion: 合成数据在开发高性能医疗紧急情况AI模型方面具有巨大潜力，为严重和关键医疗场景的AI发展提供了可行方案。
...
### [147] [AISAC: An Integrated multi-agent System for Transparent, Retrieval-Grounded Scientific Assistance](https://arxiv.org/abs/2511.14043)
*Chandrachur Bhattacharya,Sibendu Som*

Main category: cs.AI

TL;DR: AISAC是一个在阿贡国家实验室开发的多智能体科学助手系统，集成了LangGraph、FAISS和SQLite等技术，专注于科学工作流的透明性、溯源跟踪和适应性。


<details>
  <summary>Details</summary>
Motivation: 开发一个集成化的多智能体系统来支持科学和工程工作流，强调透明度、溯源跟踪和科学适应性。

Method: 采用Router-Planner-Coordinator工作流和可选Evaluator角色，使用提示工程智能体通过LangGraph协调，结合FAISS向量搜索和SQLite持久化的混合内存方法，以及基于文件哈希的增量索引策略。

Result: 系统已在阿贡国家实验室的多个研究领域成功应用，包括废物转化产品和能源过程安全等专门部署，以及通用科学辅助，展示了其跨领域适用性。

Conclusion: AISAC证明了多智能体系统在科学工作流中的有效性，通过配置驱动的项目引导层和透明的工作流可视化，为研究团队提供了可定制且可追溯的科学助手解决方案。
...
### [148] [Making Evidence Actionable in Adaptive Learning](https://arxiv.org/abs/2511.14052)
*Amirreza Mehrabi,Jason W. Morphew,Breejha Quezada,N. Sanjay Rebello*

Main category: cs.AI

TL;DR: 该研究提出了一个由教师主导的反馈循环系统，将概念级评估证据转化为经过验证的微干预措施，通过三个保障机制（充分性、注意力预算、多样性）实现精准的个性化学习干预。


<details>
  <summary>Details</summary>
Motivation: 自适应学习系统通常诊断准确但干预效果弱，导致帮助时机不当或与学习需求不匹配。需要一种能够将诊断证据有效转化为教学干预的系统。

Method: 将干预分配形式化为带约束的二进制整数规划问题，包含覆盖度、时间、难度窗口、概念矩阵先决条件和反冗余多样性约束。采用贪婪选择、梯度松弛和混合方法三种求解策略。

Result: 在1204名学生的物理课程部署中，两种求解器都实现了几乎对所有学习者的完整技能覆盖，梯度方法比贪婪方法减少了约12%的冗余覆盖，并更好地协调了难度分布。
...
### [149] [Collaborative QA using Interacting LLMs. Impact of Network Structure, Node Capability and Distributed Data](https://arxiv.org/abs/2511.14098)
*Adit Jain,Vikram Krishnamurthy,Yiming Zhang*

Main category: cs.AI

TL;DR: 本文研究了LLM网络中的协同问答问题，分析了幻觉在LLM网络中的传播机制及其对真相估计的影响。


<details>
  <summary>Details</summary>
Motivation: LLM在缺乏直接证据时会产生幻觉，这种效应在交互的LLM网络中更加显著，导致原本准确的LLM也开始产生幻觉。

Method: 结合网络科学的平均场动力学和经济学中的随机效用模型，构建生成模型，将LLM建模为具有潜在状态（真实或幻觉）的节点，分析有向网络中的信息扩散。

Result: 提出了固定点存在性和唯一性的充分条件，分析了激励对固定点行为的影响，并在100个开源LLM网络上进行了实验验证。

Conclusion: LLM网络中的幻觉传播是一个重要问题，需要从网络动力学角度理解和管理，激励策略可以影响网络的整体行为。
...
### [150] [APD-Agents: A Large Language Model-Driven Multi-Agents Collaborative Framework for Automated Page Design](https://arxiv.org/abs/2511.14101)
*Xinpeng Chen,Xiaofeng Han,Kaihao Zhang,Guochao Ren,Yujie Wang,Wenhao Cao,Yang Zhou,Jianfeng Lu,Zhenbo Song*

Main category: cs.AI

TL;DR: APD-agents是一个基于大语言模型的多智能体框架，用于自动化移动应用页面设计，通过多个专业智能体的协作将用户描述转换为完整的页面布局。
...
Method: 采用多智能体框架，包含OrchestratorAgent、SemanticParserAgent、PrimaryLayoutAgent、TemplateRetrievalAgent和RecursiveComponentAgent，通过智能体协作将用户描述解析为结构化数据并生成从粗到细的页面布局。
...
### [151] [PRISM: Prompt-Refined In-Context System Modelling for Financial Retrieval](https://arxiv.org/abs/2511.14130)
*Chun Chet Ng,Jia Yu Lim,Wei Zeng Low*

Main category: cs.AI

TL;DR: 提出了PRISM框架，用于金融信息检索任务，通过系统提示、上下文学习和轻量级多代理系统实现文档和片段排序，在FinAgentBench数据集上取得良好性能。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的发展，从冗长金融文件中提取任务相关信息对运营和分析决策至关重要，需要高效的金融信息检索方法。

Method: PRISM训练免费框架，整合精炼系统提示、上下文学习和轻量级多代理系统，各组件协同工作：提示工程提供精确任务指令，上下文学习提供语义相关示例，多代理系统建模协调评分行为。

Result: 最佳配置在受限验证集上达到NDCG@5为0.71818，证明PRISM在生产规模金融检索中可行且稳健。

Conclusion: PRISM的模块化、仅推理设计使其适用于实际应用场景，为金融信息检索提供了实用解决方案。
...
### [152] [Run, Ruminate, and Regulate: A Dual-process Thinking System for Vision-and-Language Navigation](https://arxiv.org/abs/2511.14131)
...
TL;DR: 提出R3双过程思维框架，将LLM的泛化能力与VLN专业模型结合，通过Runner、Ruminator和Regulator三个模块实现高效导航。
...
Method: R3框架包含：Runner（轻量级专家模型）、Ruminator（多模态LLM进行结构化推理）、Regulator（监控导航进度并控制思维模式）。
...
### [153] [Beyond Accuracy: A Multi-Dimensional Framework for Evaluating Enterprise Agentic AI Systems](https://arxiv.org/abs/2511.14136)
*Sushant Mehta*

Main category: cs.AI

TL;DR: 提出了CLEAR评估框架，针对企业AI代理部署需求，弥补现有基准在成本效率、可靠性和运营稳定性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有AI代理基准主要关注任务完成准确率，忽视了企业部署所需的关键要求如成本效率、可靠性和运营稳定性。

Method: 通过系统分析12个主要基准和实证评估最先进代理，识别了三个根本局限性，并提出CLEAR（成本、延迟、效能、保证、可靠性）这一整体评估框架。

Result: 在300个企业任务上评估6个领先代理，发现仅优化准确率的代理比成本感知替代方案贵4.4-10.8倍，专家评估确认CLEAR能更好预测生产成功（相关性ρ=0.83 vs 仅准确率评估的ρ=0.41）。
...
### [154] [HFL-FlowLLM: Large Language Models for Network Traffic Flow Classification in Heterogeneous Federated Learning](https://arxiv.org/abs/2511.14199)
...
<details>
  <summary>Details</summary>
Motivation: 解决5G和物联网环境下传统集中式机器学习面临的分布式数据和隐私问题，以及现有联邦学习方法的高成本和泛化能力差的问题。

Method: 提出HFL-FlowLLM框架，将大语言模型应用于异构联邦学习的网络流量分类中。
...
<details>
  <summary>Details</summary>
Motivation: 测试LLMs是否真正理解时间顺序，因为金融应用中需要避免前瞻性偏差，而现有方法隐含假设模型理解时间顺序。

Method: 设计三类时间顺序任务：时间排序、条件排序（先筛选后排序）和时代错误检测，在多种推理设置下评估GPT-4.1、Claude-3.7 Sonnet和GPT-5。

Result: 随着序列长度增加，精确匹配率急剧下降；条件排序失败主要来自筛选步骤；时代错误检测是最简单的任务；增加推理预算能显著提升GPT-5和Claude-3.7 Sonnet的性能。

Conclusion: 当前LLMs在时间顺序任务上存在局限，明确分配推理预算有助于改善时间排序，GPT-5在高推理努力下能实现完美排序，这对金融实时应用具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: Whisper模型在噪声条件下经常出现幻觉错误，而现有方法主要关注音频预处理或转录后处理，对模型本身的修改研究不足。
...
Conclusion: ALA和KD提供了一种改进Whisper在真实世界噪声条件下可靠性的原则性策略。
...
Method: 1) 通过持续预训练和多任务微调为LLM注入物联网操作领域知识；2) 使用直接偏好优化使微调后的LLM与具体用户偏好对齐；3) 设计基于置信度的曝光控制机制避免低质量推荐。

Result: 在所有数据集上显著超越基线方法，各项指标平均提升69.5%。已在小米家庭APP中部署一个季度，为25.5万用户提供日常操作推荐，在线实验显示独立访客设备覆盖率提升21.6%，页面浏览接受率提升29.1%。

Conclusion: DevPiolt通过结合领域知识注入、用户偏好对齐和置信度控制，有效解决了物联网设备操作推荐中的挑战，在实际部署中取得了显著效果。
...
TL;DR: 提出基于LLM的区域Airbnb市场预测框架，通过整合房源特征和外部环境因素生成区域嵌入，结合时间序列模型预测未来1-3个月的收入、预订天数和预订数量，在首尔数据集上相比基线方法将RMSE和MAE降低了约48%。


<details>
  <summary>Details</summary>
Motivation: 短期租赁平台如Airbnb的扩张扰乱了当地住房市场，导致租金上涨和住房负担能力问题。准确预测区域Airbnb市场趋势可为政策制定者和城市规划者提供关键见解以减轻这些影响。
...
TL;DR: PathMind是一个增强知识图谱推理的框架，通过选择性引导LLM使用重要推理路径来提高忠实性和可解释性，采用"检索-优先排序-推理"范式，在复杂推理任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的知识图谱推理方法存在两个关键限制：1）不加区分地提取推理路径，可能引入误导性噪声；2）动态探索推理路径需要高检索需求和频繁LLM调用。

Method: PathMind采用"检索-优先排序-推理"范式：1）检索模块从KG中获取查询子图；2）路径优先排序机制使用语义感知路径优先级函数识别重要推理路径；3）通过双阶段训练策略（任务特定指令微调和路径偏好对齐）生成准确响应。
...
### [160] [DataSage: Multi-agent Collaboration for Insight Discovery with External Knowledge Retrieval, Multi-role Debating, and Multi-path Reasoning](https://arxiv.org/abs/2511.14299)
*Xiaochuan Liu,Yuanfeng Song,Xiaoming Yin,Xing Chen*

Main category: cs.AI

TL;DR: DataSage是一个多智能体框架，通过外部知识检索、多角色辩论机制和多路径推理来解决现有数据洞察代理在领域知识利用不足、分析深度浅和代码生成易错的问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的数据洞察代理存在三个主要限制：领域知识利用不足、分析深度浅、代码生成易错，导致无法提供满意的洞察发现结果。
...
### [161] [When Words Change the Model: Sensitivity of LLMs for Constraint Programming Modelling](https://arxiv.org/abs/2511.14334)
*Alessio Pellegrino,Jacopo Mauro*

Main category: cs.AI

TL;DR: 本文通过系统性地改写和扰动经典CSPLib问题，测试LLMs在自动生成优化模型时的真实推理能力，发现其表现受数据污染影响较大，对上下文和语言变化敏感。


<details>
  <summary>Details</summary>
Motivation: 检验LLMs在自动生成优化模型时的真实推理能力，而非仅仅依赖训练数据中的标准问题。

Method: 对CSPLib问题进行系统性改写和扰动，保持结构不变但改变上下文并引入误导元素，比较三个代表性LLM在原始和修改描述下的模型生成表现。

Result: LLMs能生成语法有效且语义合理的模型，但在上下文和语言变化下表现急剧下降，显示出浅层理解和对措辞的敏感性。

Conclusion: LLMs在自动生成优化模型方面的成功可能更多源于数据污染而非真正的推理能力，对问题的理解较为肤浅且易受语言表述影响。
...
### [162] [Operationalizing Pluralistic Values in Large Language Model Alignment Reveals Trade-offs in Safety, Inclusivity, and Model Behavior](https://arxiv.org/abs/2511.14476)
*Dalia Ali,Dora Zhao,Allison Koenecke,Orestis Papakyriakopoulos*

Main category: cs.AI

TL;DR: 本研究探讨了在LLM对齐过程中考虑多元社会价值观的重要性，通过分析不同人口群体的偏好差异和设计参数对模型行为的影响。
...
Method: 收集了来自美国和德国参与者的27,375个评分数据，在五个维度上评估LLM响应；使用不同社会群体的偏好对多个大语言模型进行微调，同时改变评分尺度、分歧处理方法和优化技术。

Result: 发现系统性人口效应：男性参与者对毒性的评分比女性低18%；保守派和黑人参与者对情感意识的评分分别比自由派和白人参与者高27.9%和44%。技术设计选择也产生显著影响：保留评分者分歧比多数投票在毒性降低方面效果高53%；5点量表比二元格式效果高22%；DPO在多值优化中持续优于GRPO。
...
### [163] [A Neuro-Symbolic Framework for Reasoning under Perceptual Uncertainty: Bridging Continuous Perception and Discrete Symbolic Planning](https://arxiv.org/abs/2511.14533)
*Jiahao Wu,Shengwen Yu*

Main category: cs.AI

TL;DR: 提出了一个神经符号框架，明确建模和传播从感知到规划的不确定性，通过概率符号状态提取和不确定性感知符号规划，在桌面机器人操作任务中实现高效规划和高成功率。
...
Result: 在10,047个PyBullet生成场景上，翻译器输出校准置信度的概率谓词（总体F1=0.68）；在规划器中，系统在Simple Stack、Deep Stack和Clear+Stack基准上分别达到94%/90%/88%成功率（平均90.7%），比最强POMDP基线高出10-14个百分点，规划时间在15ms内。

Conclusion: 该框架建立了校准不确定性与规划收敛之间的定量联系，提供理论保证并通过实证验证，是通用的可应用于任何需要从感知输入到符号规划的不确定性感知推理的领域。
...
TL;DR: 提出基于率失真理论和最优传输几何的知识图谱构建与优化框架，通过FGW耦合量化语义失真，使用精炼操作最小化率失真拉格朗日量，生成紧凑且信息保留的知识图谱，在数据科学讲座应用中显著提升多选题生成质量。


<details>
  <summary>Details</summary>
Motivation: 将非结构化教育材料（如讲义和幻灯片）转化为捕捉关键教学内容的知识图谱存在困难，需要建立理论基础的KG优化方法用于AI辅助教育。

Method: 将讲座内容建模为度量-测度空间，使用Fused Gromov-Wasserstein耦合量化语义失真，通过添加、合并、拆分、移除、重连等精炼操作最小化率失真拉格朗日量。

Result: 在数据科学讲座应用中产生可解释的率失真曲线，从优化KG生成的多选题在15个质量标准上持续优于原始笔记生成的题目。
...
### [165] [AutoTool: Efficient Tool Selection for Large Language Model Agents](https://arxiv.org/abs/2511.14650)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM代理框架在工具选择时存在高推理成本问题，特别是像ReAct这样的方法需要反复调用LLM来确定每个步骤使用的工具。

Method: 构建有向图表示历史代理轨迹，节点代表工具，边捕获转移概率，建模工具选择惯性，并集成参数级信息来优化工具输入生成。

Result: 实验表明AutoTool将推理成本降低高达30%，同时保持竞争力的任务完成率。

Conclusion: 将统计结构集成到LLM代理设计中可以在不牺牲性能的情况下实现更高效率。
...
### [166] [SkillGen: Learning Domain Skills for In-Context Sequential Decision Making](https://arxiv.org/abs/2511.14670)
*Ruomeng Ding,Wei Cheng,Minglai Shao,Chen Zhao*

Main category: cs.AI

TL;DR: SkillGen是一个基于技能的上下文学习框架，通过构建动作中心图、识别高效用动作和检索步骤技能，为顺序决策任务生成细粒度的上下文感知提示，在多个基准测试中显著提升了性能。
...
Result: 在ALFWorld、BabyAI和ScienceWorld基准测试中，使用开源和专有LLM，SkillGen平均提高了5.9%-16.5%的进度率。
...
### [167] [Heterogeneous Multi-Agent Proximal Policy Optimization for Power Distribution System Restoration](https://arxiv.org/abs/2511.14730)
*Parya Dolatyabi,Mahdi Khodayar*

Main category: cs.AI

TL;DR: 本文提出了一种基于异构智能体强化学习（HARL）的配电网恢复方法，通过HAPPO算法协调多个微电网的分布式能源资源，解决了传统优化方法在非线性约束下的计算效率问题。


<details>
  <summary>Details</summary>
Motivation: 大规模停电后配电网恢复需要处理复杂的开关操作和分布式能源协调，传统优化方法和基于价值的强化学习方法在非线性约束下计算效率低且难以扩展。

Method: 采用异构智能体近端策略优化（HAPPO）框架，每个智能体控制具有不同负载、DER容量和开关数量的微电网，使用分散的演员策略和集中的评论家进行训练，通过物理信息化的OpenDSS环境提供功率流反馈。

Result: 在IEEE 123总线和IEEE 8500节点系统上的实验表明，HAPPO相比DQN、PPO等算法具有更快的收敛速度、更高的恢复功率和更平滑的多种子训练效果。
...
### [168] [FICO: Finite-Horizon Closed-Loop Factorization for Unified Multi-Agent Path Finding](https://arxiv.org/abs/2511.13961)
*Jiarui Li,Alessandro Zanardi,Runyu Zhang,Gioele Zardini*

Main category: cs.RO

TL;DR: 提出了一个系统级的多智能体路径规划框架，将规划与执行集成，通过闭环因子化算法实现实时响应和大规模扩展
...
Method: 引入MAPF系统作为控制设计问题的形式模型，提出有限时域闭环因子化算法，利用组合结构实现高效闭环操作

Result: 算法在毫秒级内开始执行，可扩展到数千个智能体，计算时间比开环基线减少两个数量级，在随机延迟和智能体到达情况下吞吐量显著提高

Conclusion: 为通过系统级建模、因子化和闭环设计来分析和推进MAPF建立了原则性基础
...
### [169] [LIO-MARS: Non-uniform Continuous-time Trajectories for Real-time LiDAR-Inertial-Odometry](https://arxiv.org/abs/2511.13985)
*Jan Quenzel,Sven Behnke*

Main category: cs.RO

TL;DR: 提出了一种基于LiDAR-IMU融合的里程计系统LIO-MARS，通过多分辨率面元地图与高斯混合模型的联合配准，结合连续时间B样条轨迹，实现了高精度的实时定位。


<details>
  <summary>Details</summary>
Motivation: 自主机器人系统需要鲁棒的实时感知能力，特别是在搜救等任务中。LiDAR提供精确距离测量，IMU约束加速度和旋转，两者互补可提高导航的鲁棒性和准确性。

Method: 使用连续时间B样条轨迹，通过非均匀时间节点放置确保轨迹连续性；采用克罗内克和与积加速协方差和GMM计算；使用无迹变换去偏斜面元；通过分割扫描段进行运动补偿；结合相对位姿和预积分IMU伪测量的软约束。

Result: 在多种手持、地面和空中车辆数据集上的评估表明，LIO-MARS相比现有LIO系统具有最先进的性能，计算速度提升了3.3倍。

Conclusion: LIO-MARS通过LiDAR和IMU的紧密融合，结合创新的算法设计，实现了高精度、高效率的里程计系统，在多种应用场景下表现出色。
...
### [170] [Searching in Space and Time: Unified Memory-Action Loops for Open-World Object Retrieval](https://arxiv.org/abs/2511.14004)
*Taijing Chen,Sateesh Kumar,Junhong Xu,George Pavlakos,J oydeep Biswas,Roberto Martín-Martín*

Main category: cs.RO

TL;DR: STAR是一个统一时空检索的框架，将记忆查询和具体行动结合在单一决策循环中，在动态开放环境中显著优于基于场景图和纯记忆的方法。


<details>
  <summary>Details</summary>
Motivation: 服务机器人在动态开放环境中检索物体时，需要处理属性、空间上下文和时间状态等多种查询方式，现有方法只能解决部分问题，缺乏统一的解决方案。

Method: STAR框架利用非参数长期记忆和工作记忆支持高效召回，使用视觉语言模型在每一步选择时间或空间行动，将时空搜索作为统一问题处理。

Result: 在STARBench基准测试和Tiago机器人实验中，STAR始终优于场景图和纯记忆基线方法。

Conclusion: 将时间搜索和空间搜索作为统一问题处理具有显著优势，STAR框架为动态开放环境中的物体检索提供了有效解决方案。
...
### [171] [FACA: Fair and Agile Multi-Robot Collision Avoidance in Constrained Environments with Dynamic Priorities](https://arxiv.org/abs/2511.14024)
*Jaskirat Singh,Rohan Chandra*

Main category: cs.RO

TL;DR: FACA是一种公平敏捷的碰撞避免方法，通过自然语言协调多机器人系统，在受限空间中实现高效安全导航


<details>
  <summary>Details</summary>
Motivation: 多机器人系统在救援、配送等关键应用中需要在拥挤的受限空间中高速导航，且角色和优先级会动态变化，需要既安全又敏捷的解决方案

Method: 提出FACA方法，机器人通过自然语言通信协调任务，采用新型人工势场算法在冲突时自动创建"环岛"效应

Result: 实验显示FACA效率提升显著，任务完成速度比基线快3.5倍以上，时间减少超过70%，同时保持鲁棒的安全边界

Conclusion: FACA方法在多机器人系统中有效平衡了安全性和敏捷性，在动态变化的环境中实现了高效协调
...
### [172] [BIM-Discrepancy-Driven Active Sensing for Risk-Aware UAV-UGV Navigation](https://arxiv.org/abs/2511.14037)
*Hesam Mojtahedi,Reza Akhavian*

Main category: cs.RO

TL;DR: 提出了一个BIM差异驱动的主动感知框架，用于无人机和无人地面车辆在动态建筑环境中的协同导航，通过风险触发重新扫描显著降低导航风险和地图不确定性。


<details>
  <summary>Details</summary>
Motivation: 传统导航方法依赖静态BIM先验或有限的机载感知，无法有效应对动态建筑环境中的不确定性。需要一种能够融合实时感知数据和BIM先验的主动感知框架来提高导航安全性。

Method: 框架持续融合空中和地面机器人的实时LiDAR数据与BIM先验，维护演化的2D占据地图。通过统一的走廊风险度量（整合占据不确定性、BIM地图差异和间隙）量化导航安全性，当风险超过安全阈值时，无人机自主重新扫描受影响区域以降低不确定性。

Result: 在PX4-Gazebo仿真中验证，风险触发重新扫描相比静态BIM导航将平均走廊风险降低58%，地图熵降低43%，同时保持间隙裕度在0.4米以上。相比前沿探索方法，在任务时间减半的情况下实现了相似的不确定性降低。

Conclusion: 将BIM先验与风险自适应空中感知相结合，为建筑机器人实现了可扩展的、不确定性感知的自主性。
...
### [173] [FlexiCup: Wireless Multimodal Suction Cup with Dual-Zone Vision-Tactile Sensing](https://arxiv.org/abs/2511.14139)
...
TL;DR: FlexiCup是一种完全无线多模态吸盘，集成了双区域视觉触觉传感，支持真空和伯努利两种吸力模式，通过模块化机械配置实现自主操作。
...
Method: 中央区域通过照明控制在视觉和触觉模态间动态切换进行接触检测，外围区域提供连续空间感知用于接近规划。采用模块化机械配置支持两种吸力模式，配备板载计算和电源实现完全无线自主。

Result: 在结构化表面上的模块化感知驱动抓取中，真空模式平均成功率90.0%，伯努利模式86.7%。基于扩散的端到端学习在倾斜运输任务中成功率73.3%，橙子提取任务66.7%。消融研究证实双区域观测协调提供13%的接触感知操作改进。

Conclusion: FlexiCup通过集成多模态传感和无线自主性，显著提升了吸盘在非结构化环境中的操作能力。
...
### [174] [AsyncVLA: Asynchronous Flow Matching for Vision-Language-Action Models](https://arxiv.org/abs/2511.14148)
*Yuhua Jiang,Shuang Cheng,Yan Ding,Feifei Gao,Biqing Qi*

Main category: cs.RO

TL;DR: 提出AsyncVLA框架，通过异步流匹配和时间感知的动作生成，解决传统同步流匹配在长时程任务中的不稳定问题，实现动作自校正。


<details>
  <summary>Details</summary>
Motivation: 传统VLA模型使用同步流匹配生成动作，依赖固定的时间调度，缺乏动作上下文感知和异步自校正能力，在长时程任务中容易因单个动作错误导致级联失败。

Method: 引入异步流匹配，在非均匀时间调度中生成动作令牌，具有动作上下文感知能力；添加置信度评估器提取初始生成动作的置信度，选择性修正不准确的动作令牌；提出SFM和AFM的统一训练程序。

Result: 在机器人操作基准测试中，AsyncVLA表现出数据效率和自校正能力，在通用具身评估中达到最先进的结果。

Conclusion: AsyncVLA通过异步生成和自校正机制，显著提升了VLA模型在长时程任务中的稳定性和性能。
...
### [175] [RoboTidy : A 3D Gaussian Splatting Household Tidying Benchmark for Embodied Navigation and Action](https://arxiv.org/abs/2511.14161)
*Xiaoquan Sun,Ruijian Zhang,Kang Pang,Bingchen Miao,Yuxiang Tan,Zhen Yang,Ming Li,Jiayu Chen*

Main category: cs.RO

TL;DR: 提出了RoboTidy基准测试，用于语言引导的家庭整理任务，支持视觉-语言-动作和视觉-语言导航的训练与评估。
...
Method: 提供500个真实感3D高斯溅射家庭场景，包含500个物体和容器，将整理任务表述为"动作(物体,容器)"列表，并提供6.4k个高质量操作演示轨迹和1.5k个导航轨迹。

Result: 构建了统一的家庭整理基准测试平台，并在真实世界中部署用于物体整理，建立了端到端的评估标准。

Conclusion: RoboTidy为语言引导机器人提供了可扩展平台，通过实现全面和现实的评估，弥补了具身AI中的关键空白。
...
### [176] [Towards Deploying VLA without Fine-Tuning: Plug-and-Play Inference-Time VLA Policy Steering via Embodied Evolutionary Diffusion](https://arxiv.org/abs/2511.14178)
*Zhuo Li,Junjia Liu,Zhipeng Dong,Tao Teng,Quentin Rouxel,Darwin Caldwell,Fei Chen*

Main category: cs.RO

TL;DR: VLA-Pilot是一种即插即用的推理时策略引导方法，无需微调或额外数据收集即可实现预训练视觉-语言-动作模型的零样本部署，显著提升下游机器人操作任务的性能。
...
Method: 提出VLA-Pilot方法，这是一种即插即用的推理时策略引导技术，无需额外微调或数据收集，直接对预训练VLA模型进行零样本部署。

Result: 在6个真实世界下游操作任务和2种不同机器人平台上进行评估，涵盖分布内和分布外场景，实验结果表明VLA-Pilot显著提升了现成预训练VLA策略的成功率。
...
### [177] [Dual-Variable Force Characterisation method for Human-Robot Interaction in Wearable Robotics](https://arxiv.org/abs/2511.14327)
*Felipe Ballen-Moreno,Pasquale Ferrentino,Milan Amighi,Bram Vanderborght,Tom Verstraten*

Main category: cs.RO

TL;DR: 提出了一种双变量表征方法，用于分析可穿戴机器人物理交互中的法向和切向力，解决了现有单变量方法的局限性。
...
Method: 引入双变量表征方法，同时考虑法向力和切向力，通过分析不同场景和材料模型下的归一化均方误差来评估单变量拟合的影响。

Result: 该方法证明了在表征过程中纳入两个变量的重要性，能够更准确地识别材料参数并评估力与扭矩响应。

Conclusion: 双变量表征方法为可穿戴机器人物理交互模拟提供了更精确的基础，特别关注袖带与人体肢体之间的相互作用。
...
### [178] [MA-SLAM: Active SLAM in Large-Scale Unknown Environment using Map Aware Deep Reinforcement Learning](https://arxiv.org/abs/2511.14330)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有主动SLAM方法在大规模多样化环境中面临的探索时间长、路径优化不足的问题。

Method: 提出结构化地图表示方法，将空间数据离散化并整合边界点和历史轨迹；使用深度强化学习决策模块和全局规划器优化探索路径。

Result: 在三个仿真环境和真实无人地面车辆上的实验表明，相比最先进方法显著减少了探索时间和距离。

Conclusion: MA-SLAM系统能够有效解决大规模环境中的主动SLAM探索效率问题，具有实际应用价值。
...
### [179] [Simultaneous Localization and 3D-Semi Dense Mapping for Micro Drones Using Monocular Camera and Inertial Sensors](https://arxiv.org/abs/2511.14335)
...
<details>
  <summary>Details</summary>
Motivation: 当前单目SLAM算法存在稀疏方法缺乏详细几何信息、学习驱动方法计算量大、以及尺度模糊影响精度等问题，需要一种轻量级且准确的解决方案。
...
Conclusion: 该方法为资源受限环境中的实时建图和导航提供了一种有效实用的解决方案。
...
### [180] [Going Places: Place Recognition in Artificial and Natural Systems](https://arxiv.org/abs/2511.14341)
*Michael Milford,Tobias Fischer*

Main category: cs.RO

TL;DR: 这篇综述整合了机器人系统、动物研究和人类研究，探讨了不同系统如何编码和回忆地点，提出了统一的场所识别概念框架。


<details>
  <summary>Details</summary>
Motivation: 场所识别对于生物导航和自主系统都至关重要，需要从多学科角度理解不同系统如何实现这一能力。

Method: 通过综合机器人系统、动物研究和人类研究的发现，分析计算和表征策略，包括拓扑映射、线索整合和记忆管理。

Result: 揭示了动物系统的多模态导航机制、人类研究的语义场所概念和文化影响，以及人工系统的可扩展架构。
...
### [181] [Perception-aware Exploration for Consumer-grade UAVs](https://arxiv.org/abs/2511.14393)
*Svetlana Seliunina,Daniel Schleich,Sven Behnke*

Main category: cs.RO

TL;DR: 将多无人机自主探索技术扩展到消费级无人机（如DJI Mini 3 Pro），提出包含视点选择、轨迹规划和半分布式通信的完整管道


<details>
  <summary>Details</summary>
Motivation: 将先进的多无人机自主探索技术适配到硬件受限的消费级无人机上，解决其在深度估计和运动约束方面的挑战

Method: 1) 选择可估计深度的视点对；2) 规划满足运动约束的轨迹以支持里程计估计；3) 采用半分布式通信方案平衡工作负载

Result: 通过仿真验证了不同数量无人机下的性能，证明系统能够安全探索环境并重建地图，即使使用消费级无人机硬件

Conclusion: 提出的方法成功将多无人机自主探索扩展到消费级硬件，在硬件限制下仍能实现安全探索和地图重建
...
### [182] [Continuous Vision-Language-Action Co-Learning with Semantic-Physical Alignment for Behavioral Cloning](https://arxiv.org/abs/2511.14396)
*Xiuxiu Qi,Yu Yang,Jiannong Cao,Luyao Bai,Chongshan Fan,Chengtai Cao,Hongpeng Wang*

Main category: cs.RO

TL;DR: 提出CCoL框架，通过视觉-语言-动作的连续协同学习和语义-物理对齐，解决行为克隆中的复合误差问题，实现更平滑准确的动作执行


<details>
  <summary>Details</summary>
Motivation: 现有方法在缓解行为克隆中的复合误差时存在物理不连续性和语义-物理错位问题，导致动作克隆不准确和执行中断

Method: CCoL框架通过视觉、语言和本体感觉输入的连续协同学习生成鲁棒平滑的动作轨迹，使用双向交叉注意力将语言语义锚定到视觉运动表示中

Result: 在三个仿真套件中平均相对提升8.0%，在双手插入任务中最高相对提升19.2%，在7自由度机器人真实测试中表现出对未见和噪声物体状态的泛化能力

Conclusion: CCoL通过连续协同学习和语义-物理对齐有效解决了行为克隆中的复合误差问题，实现了更准确和一致的动作执行
...
### [183] [Self-Supervised Multisensory Pretraining for Contact-Rich Robot Reinforcement Learning](https://arxiv.org/abs/2511.14427)
*Rickmer Krohn,Vignesh Prasad,Gabriele Tiboni,Georgia Chalvatzaki*

Main category: cs.RO

TL;DR: 提出MSDP框架，通过掩码自编码学习多感官表征，结合非对称架构实现鲁棒的多感官机器人操作


<details>
  <summary>Details</summary>
Motivation: 解决强化学习在接触丰富的多感官操作任务中，面对感官噪声和动态变化时的学习困难

Method: 基于掩码自编码训练Transformer编码器重建多感官观测，下游策略学习采用非对称架构，评论家使用交叉注意力提取动态特征，执行器接收稳定池化表征

Result: 在仿真和真实机器人任务中展示加速学习和鲁棒性能，对传感器噪声和物体动态变化具有强鲁棒性，真实机器人仅需6000次在线交互即达高成功率
...
### [184] [Mutation Testing for Industrial Robotic Systems](https://arxiv.org/abs/2511.14432)
*Marcela Gonçalves dos Santos,Sylvain Hallé,Fábio Petrillo*

Main category: cs.RO

TL;DR: 本文提出了针对工业机器人系统的领域特定变异测试方法，通过定义捕获机器人动作和传感器语义的变异算子，提高了测试套件的有效性评估能力。


<details>
  <summary>Details</summary>
Motivation: 工业机器人系统在多样化环境中部署，故障可能导致严重事故和高昂停机时间。传统变异测试算子不适合机器人程序，因为它们涉及基于消息的命令和物理世界交互。

Method: 提出在高层读写操作级别生成有意义变异体的方法，包括运动、夹爪动作和传感器噪声注入等域特定变异算子。
...
### [185] [Achieving Safe Control Online through Integration of Harmonic Control Lyapunov-Barrier Functions with Unsafe Object-Centric Action Policies](https://arxiv.org/abs/2511.14434)
*Marlow Fawn,Matthias Scheutz*

Main category: cs.RO

TL;DR: 提出了一种将谐波控制李雅普诺夫-屏障函数与任意机器人策略结合的方法，将不安全策略转化为具有形式化保证的安全策略。
...
Method: 使用从信号时序逻辑规范导出的谐波控制李雅普诺夫-屏障函数与给定机器人策略结合，通过安全证书产生保持安全和任务行为的指令。

Result: 在静止机器人臂的运动任务中，经过安全约束结合后，能够成功避开桌面障碍物。

Conclusion: 该方法可将不安全策略转化为安全策略，并适用于更复杂规范和动态任务场景。
...
### [186] [Advancing Minimally Invasive Precision Surgery in Open Cavities with Robotic Flexible Endoscopy](https://arxiv.org/abs/2511.14458)
...
TL;DR: 开发了一个用于开放腔体微创手术的机器人平台，结合磁驱动柔性内窥镜和自主导航能力，在羊模型中进行体内验证。


<details>
  <summary>Details</summary>
Motivation: 柔性机器人在微创手术中具有优势，但在开放腔体中的内窥镜干预仍面临挑战，包括缺乏解剖约束、设备灵活性导致的控制困难以及内窥镜视野受限。

Method: 结合磁驱动柔性内窥镜与远程操作和半自主导航能力，实时重建内窥镜场景的拼接图像以提供扩展的视觉上下文。

Result: 在羊模型中进行了体内验证，展示了该系统在开放空间中解决微创手术关键限制的能力。

Conclusion: 该机器人平台能够克服开放腔体微创手术中的挑战，特别是在复杂的胎儿镜激光凝固手术中具有应用潜力。
...
### [187] [Aerial Assistance System for Automated Firefighting during Turntable Ladder Operations](https://arxiv.org/abs/2511.14504)
*Jan Quenzel,Valerij Sekin,Daniel Schleich,Alexander Miller,Merlin Stampa,Norbert Pahlke,Christof Röhrig,Sven Behnke*

Main category: cs.RO

TL;DR: 提出了一种利用无人机和机动消防监控器的自动化消防辅助系统，通过无人机自主检测和定位热源，系统自动控制消防监控器将水流导向火源。


<details>
  <summary>Details</summary>
Motivation: 工业设施火灾因建筑规模大、视觉遮挡严重，导致消防员难以准确定位火源，影响灭火精度和效率，增加损失和救援时间。

Method: 使用无人机在障碍物自由飞行区域内自主飞行检测热源，操作员选择目标后，无人机自动规划三角测量路径持续定位火源，同时系统控制消防监控器将水流对准检测到的热源。

Result: 初步测试中，该系统成功定位了多个热源，并能将水流导向火源。

Conclusion: 该自动化消防辅助系统能够有效解决工业火灾中的火源定位难题，提高灭火精度和效率。
...
### [188] [Masked IRL: LLM-Guided Reward Disambiguation from Demonstrations and Language](https://arxiv.org/abs/2511.14565)
*Minyoung Hwang,Alexandra Forsey-Smerek,Nathaniel Dennler,Andreea Bobu*

Main category: cs.RO

TL;DR: 提出Masked IRL框架，利用LLM结合语言指令和演示数据，通过推断状态相关性掩码来提升奖励学习的样本效率和泛化能力


<details>
  <summary>Details</summary>
Motivation: 传统基于演示的奖励学习容易过拟合到虚假相关性，而语言指令虽然能直接指定重要内容，但现有方法未能充分利用其消除歧义的潜力，且真实指令本身往往存在歧义

Method: 使用LLM从语言指令推断状态相关性掩码，强制模型对无关状态分量保持不变性；当指令存在歧义时，利用LLM在演示上下文中的推理来澄清指令

Result: 在仿真和真实机器人实验中，Masked IRL比现有语言条件IRL方法性能提升高达15%，同时数据使用量减少4.7倍，显示出更好的样本效率、泛化能力和对模糊语言的鲁棒性

Conclusion: 语言指令和演示数据携带互补信息，通过LLM有效结合两者可以显著提升奖励学习的性能，特别是在数据有限和指令模糊的情况下
...
### [189] [Is Your VLM for Autonomous Driving Safety-Ready? A Comprehensive Benchmark for Evaluating External and In-Cabin Risks](https://arxiv.org/abs/2511.14592)
...
TL;DR: DSBench是首个综合性的驾驶安全基准测试，用于评估视觉语言模型在外部环境风险和车内驾驶行为安全方面的表现，包含10大类28小类安全场景，通过大规模数据集微调可显著提升模型安全性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在自动驾驶中的应用前景广阔，但其在安全关键场景下的适用性尚未充分验证，缺乏同时评估外部环境风险和车内驾驶行为安全的综合基准测试。

Method: 构建DSBench基准测试，涵盖外部环境风险和车内驾驶行为安全两大类别，细分为10个关键类别和28个子类别，创建包含98K实例的大规模安全场景数据集用于模型微调。

Result: 对主流开源和闭源视觉语言模型的广泛评估显示，在复杂安全关键场景下模型性能显著下降，突显了安全担忧；通过在该数据集上微调，现有视觉语言模型的安全性能得到显著提升。

Conclusion: DSBench填补了自动驾驶安全评估的关键空白，为推进自动驾驶技术提供了重要的基准工具和数据集，通过针对性微调可有效提升视觉语言模型在安全关键场景下的表现。
...
### [190] [Gallant: Voxel Grid-based Humanoid Locomotion and Local-navigation across 3D Constrained Terrains](https://arxiv.org/abs/2511.14625)
*Qingwei Ben,Botian Xu,Kailin Li,Feiyu Jia,Wentao Zhang,Jingping Wang,Jingbo Wang,Dahua Lin,Jiangmiao Pang*

Main category: cs.RO

TL;DR: Gallant是一个基于体素网格的人形机器人3D约束地形运动框架，使用体素化LiDAR数据作为感知表示，通过z分组的2D CNN实现端到端控制策略优化，在复杂3D环境中实现高成功率运动。
...
Method: 使用体素化LiDAR数据作为轻量级结构化感知表示，开发高保真LiDAR模拟器，采用z分组的2D CNN将感知映射到控制策略，实现完全端到端优化。

Result: Gallant的广泛感知覆盖使单一策略能够处理地面障碍物、侧向杂物、顶部约束、多层结构和狭窄通道等复杂场景，在楼梯攀爬和平台登高等挑战性场景中实现接近100%的成功率。

Conclusion: 基于体素网格的LiDAR感知框架显著提升了人形机器人在3D约束地形中的运动能力，通过端到端优化实现了超越现有方法的性能表现。
...
### [191] [NORA-1.5: A Vision-Language-Action Model Trained using World Model- and Action-based Preference Rewards](https://arxiv.org/abs/2511.14659)
...
<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型在跨不同具身系统和真实环境部署时，在可靠性和泛化能力方面仍存在不足，需要更可靠的具身智能体。

Method: 1. 在预训练NORA骨干基础上添加基于流匹配的动作专家；2. 开发奖励模型结合动作条件世界模型和偏离地面真值启发式；3. 通过直接偏好优化对NORA-1.5进行后训练。

Result: NORA-1.5在模拟和真实世界基准测试中均优于NORA和多个最先进的VLA模型，奖励驱动的后训练在仿真和真实机器人设置中持续提升性能。

Conclusion: NORA-1.5和奖励指导的后训练为实现更可靠的、适合真实世界部署的具身智能体提供了可行路径。
...
### [192] [Robust Verification of Controllers under State Uncertainty via Hamilton-Jacobi Reachability Analysis](https://arxiv.org/abs/2511.14755)
*Albert Lin,Alessandro Pinto,Somil Bansal*

Main category: cs.RO

TL;DR: 提出了RoVer-CoRe框架，这是首个基于Hamilton-Jacobi可达性分析的方法，用于在感知不确定性下验证基于感知的自主系统控制器的安全性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 随着基于感知的控制器在现实世界中越来越流行，需要形式化验证其在感知不确定性下的安全性和性能。现有验证方法要么限制控制器类型，要么分析过于保守，而HJ可达性分析在基于感知系统中的应用尚未充分探索。

Method: 提出RoVer-CoRe框架，通过将系统控制器、观测函数和状态估计模块串联，获得等效的闭环系统，使其与现有可达性框架兼容。框架包含形式化安全验证和鲁棒控制器设计的新方法。

Result: 在飞机滑行和基于神经网络的漫游车导航案例研究中验证了框架的有效性。

Conclusion: RoVer-CoRe是首个基于HJ可达性分析的框架，能够验证感知不确定性下的基于感知系统，为自主系统的形式化验证提供了新途径。
...
### [193] [HMC: Learning Heterogeneous Meta-Control for Contact-Rich Loco-Manipulation](https://arxiv.org/abs/2511.14756)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界机器人演示学习面临交互动态复杂多变的问题，纯位置控制器在处理接触或变负载时表现不佳。

Method: 开发了HMC-Controller接口在扭矩空间连续混合不同控制模式的动作，并设计了HMC-Policy使用专家混合风格路由从大规模位置数据和精细力感知演示中学习。

Result: 在真实人形机器人上的实验显示，在挑战性任务如柔性桌面擦拭和抽屉开启中，相比基线方法有超过50%的相对改进。

Conclusion: HMC框架有效提升了机器人在复杂交互任务中的性能，证明了异构控制方法的优势。
...
### [194] [A Deep Learning Density Shaping Model Predictive Gust Load Alleviation Control of a Compliant Wing Subjected to Atmospheric Turbulence](https://arxiv.org/abs/2511.13745)
*Seid H. Pourtakdoust,Amir H. Khodabakhsh*

Main category: eess.SY

TL;DR: 提出了一种基于深度学习的随机阵风载荷减缓方法，通过主动调节机翼弯度来改善柔性机翼在随机阵风下的性能。


<details>
  <summary>Details</summary>
Motivation: 柔性机翼在随机阵风条件下需要有效的载荷减缓控制策略，传统方法难以处理随机性和非线性问题。

Method: 使用基于深度学习的模型预测控制器，结合物理信息神经网络求解概率密度演化方程，并采用自动微分进行MPC优化。

Result: 在柔性机翼模型上的数值仿真表明，该方法能有效减缓随机阵风载荷并减小翼尖挠度。

Conclusion: 所提出的概率密度成形模型预测控制方法在随机阵风载荷减缓方面具有显著效果。
...
### [195] [Deep reinforcement learning-based spacecraft attitude control with pointing keep-out constraint](https://arxiv.org/abs/2511.13746)
*Juntang Yang,Mohamed Khalil Ben-Larbi*

Main category: eess.SY

TL;DR: 使用深度强化学习（DRL）实现带单指向禁区的航天器重定向控制，采用SAC算法处理连续状态和动作空间，设计了包含姿态约束区的新状态表示，通过课程学习方法训练智能体。


<details>
  <summary>Details</summary>
Motivation: 解决航天器在重定向过程中需要避开特定指向禁区的姿态控制问题，传统方法在处理复杂约束时存在局限性。

Method: 采用Soft Actor-Critic（SAC）算法，设计包含姿态约束区紧凑表示的新状态表示，制定奖励函数实现控制目标并强制执行姿态约束，使用课程学习方法进行智能体训练。

Result: 仿真结果表明，所提出的基于DRL的方法在航天器指向约束姿态控制方面具有有效性。

Conclusion: 深度强化学习方法能够有效解决带指向禁区的航天器姿态控制问题，为复杂约束下的航天器控制提供了新思路。
...
### [196] [Game-theoretic Decentralized Coordination for Airspace Sector Overload Mitigation](https://arxiv.org/abs/2511.13770)
*Jaehan Im,Daniel Delahaye,David Fridovich-Keil,Ufuk Topcu*

Main category: eess.SY

TL;DR: 提出基于最佳响应动态的分散式空中交通管理机制，通过可调合作因子建模自利行为，减少扇区过载，无需共享决策。


<details>
  <summary>Details</summary>
Motivation: 分散式空中交通管理系统通常假设高度合作，但实践中扇区独立运营且优先考虑本地目标，导致合作假设失效。

Method: 每个扇区调整其控制航班的起飞时间以减少自身拥堵，使用可调合作因子建模对其他扇区过载的容忍度，基于最佳响应动态。

Result: 数值实验使用24小时欧洲航班数据，显示即使最小合作下也能显著减少过载，保持可扩展性且与集中式求解器质量相当。

Conclusion: 该机制满足势博弈结构，确保最佳响应动态收敛到纯纳什均衡，在轻度限制下过载无解对应势函数全局最小化。
...
### [197] [Tri-Level Stochastic-Robust Co-Planning of Distribution Networks and Renewable Charging Stations With an Adaptive iC&CG Algorithm](https://arxiv.org/abs/2511.13776)
*Yongheng Wang,Xiemin Mo,Tao Liu*

Main category: eess.SY

TL;DR: 提出一个三层次两阶段随机鲁棒优化框架，用于联合规划可再生能源充电站选址和配电网扩建，考虑交通流量和人口密度，并区分决策相关和决策无关的不确定性。


<details>
  <summary>Details</summary>
Motivation: 可再生能源充电站（RCS）将电动汽车充电与分布式发电相结合，可以提高可再生能源利用率和配电网效率，但其可变性和选址依赖的充电需求如果选址不当可能导致馈线过载。

Method: 采用三层次两阶段随机鲁棒优化框架：上层选择RCS站点和配电网扩建；中层根据RCS选址调度电动汽车路由和充电；下层在决策无关不确定性最坏情况下最小化年度损耗成本。开发了自适应不精确列和约束生成算法求解。

Result: 在中国深圳47节点配电网和68枢纽交通网络的案例研究中，A-iC&CG算法优于基准算法，光伏-电动汽车混合站是成本最优的，RCS选址集中在变电站和高流量枢纽附近。

Conclusion: 该框架能有效解决RCS和配电网的联合规划问题，考虑交通网络影响和不同类型的不确定性，为可再生能源充电站的合理选址和配电网扩建提供了优化方法。
...
### [198] [Quantifying Distribution Shift in Traffic Signal Control with Histogram-Based GEH Distance](https://arxiv.org/abs/2511.13785)
...
<details>
  <summary>Details</summary>
Motivation: 交通信号控制算法容易受到分布偏移的影响，在不同于训练条件的交通状况下性能会下降，需要一种原则性的方法来量化这种偏移

Method: 将交通场景表示为需求直方图，使用GEH距离函数进行比较，在20个模拟场景中验证了NEMA驱动控制器和强化学习控制器(FRAP++)
...
### [199] [Stochastic framework for scheduling preemptive upgrades of distribution transformers](https://arxiv.org/abs/2511.13846)
*William A Wheeler,Samuel Chevalier,Amritanshu Pandey*

Main category: eess.SY

TL;DR: 提出一个基于蒙特卡洛方法的变压器升级优先级框架，通过预测住宅电热化和电动汽车充电负载来避免配电网变压器过载故障。


<details>
  <summary>Details</summary>
Motivation: 住宅供暖和交通电气化可能导致配电网变压器过载，需要战略性规划变压器升级以避免运行故障和供电短缺风险。

Method: 采用蒙特卡洛方法预测每个电表的负载，包括热泵和电动汽车充电器，基于整体增长率和客户特定因素随机添加负载，结合网络拓扑和变压器故障模型计算故障概率。

Result: 在佛蒙特州农村真实配电网上的测试显示，在20年模拟中，通常少于20%的变压器存在显著故障风险。

Conclusion: 开发了基于故障概率的成本优化升级调度方法，能够有效管理变压器升级并讨论预期故障数量。
...
### [200] [Targeted Algorithmic Purpose-Driven Cyber Attacks in Distributed Multi-Agent Optimization](https://arxiv.org/abs/2511.13859)
*Mahan FakouriFard,Mingxi Liu*

Main category: eess.SY

TL;DR: 本文提出了一类新型的战略性目的驱动算法攻击，由参与代理发起并与分布式多代理优化系统交互，以实现自利的攻击目的。建立了这些攻击向量的理论基础，并通过电动汽车充电控制仿真验证了攻击效果。


<details>
  <summary>Details</summary>
Motivation: 分布式多代理优化虽然具有很好的可扩展性，但由于依赖频繁的点对点通信，容易受到恶意数据注入和篡改等网络攻击。现有网络安全研究主要关注破坏整体环境的广谱攻击，但无法持续实现特定或针对性目标。

Method: 开发了由参与代理发起的战略性目的驱动算法攻击，这些攻击与分布式多代理优化系统交互。建立了这些攻击向量的理论基础，包括原始和对偶意义下的分析，包含具有和不具有隐蔽特性的攻击。

Result: 通过电动汽车充电控制的仿真验证了所提出算法攻击的有效性，并展示了此类攻击对配电网络的影响。
...
### [201] [Data-Driven EV Charging Load Profile Estimation and Typical EV Daily Load Dataset Generation](https://arxiv.org/abs/2511.13861)
*Linhan Fang,Jesus Silva-Rodriguez,Xingpeng Li*

Main category: eess.SY

TL;DR: 提出两种基于实际电表数据的数据驱动方法来估计住宅电动汽车充电曲线，为电力公司提供可扩展工具来预测EV驱动的需求增长。


<details>
  <summary>Details</summary>
Motivation: 电动汽车的广泛采用给配电网带来新挑战，包括大容量局部负荷增加、随机充电行为和有限的数据可用性。

Method: 第一种方法使用最小二乘估计比较聚合的EV和非EV电表数据来提取平均充电速率；第二种方法从电表曲线中分离EV负荷并应用核密度估计开发概率充电模型。

Result: 两种方法都产生了独特的"U形"日充电曲线，大部分充电发生在夜间。

Conclusion: 验证后的充电曲线为电力公司提供了可扩展工具，能更好地预测EV驱动的需求增长并支持主动电网规划。
...
### [202] [Just Few States are Enough: Randomized Sparse Feedback for Stability of Dynamical Systems](https://arxiv.org/abs/2511.13870)
*Zaid Hadach,Hajar El Hammouti,El Houcine Bergou,Adnane Saoud*

Main category: eess.SY

TL;DR: 研究在控制器只能随机访问状态向量子集的情况下，如何设计反馈增益矩阵和测量稀疏化策略，以最小化所需状态分量数量同时保证闭环系统的渐近均方稳定性。


<details>
  <summary>Details</summary>
Motivation: 经典控制理论假设控制器能访问完整状态测量，但实际应用中可能只能获取随机选择的状态子集。本文研究这种随机稀疏化反馈下的系统稳定性问题。

Method: 提出基于线性矩阵不等式(LMI)的算法，联合计算稳定增益矩阵和随机稀疏化策略，最小化期望测量状态坐标数同时保持AMSS。方法扩展到状态分量稀疏化概率可变的情况。

Result: 给出了系统动力学条件，证明存在稀疏化策略的可能性。数值模拟显示在某些设置下，系统性能可与全状态反馈相媲美，仅需测量0.3%的状态坐标。

Conclusion: 这是首个研究仅依赖随机选择状态测量的控制系统稳定性的工作，为资源受限的控制系统设计提供了新思路。
...
### [203] [Dynamic state estimation of hybrid systems: Inverters that switch between grid-following and grid-forming control schemes](https://arxiv.org/abs/2511.13872)
*Bukunmi G. Odunlami,Marcos Netto*

Main category: eess.SY

TL;DR: 提出了一种用于逆变器在电网跟随和电网形成控制模式间切换的混合系统建模框架，通过混合自动机和扩展卡尔曼滤波器确保模式切换时的稳定性和状态估计性能。


<details>
  <summary>Details</summary>
Motivation: 逆变器需要在电网跟随和电网形成控制模式之间切换，但传统连续模型无法准确描述这种切换行为，需要建立能够处理模式切换的混合系统模型。

Method: 使用混合自动机建模逆变器，设置基于电压和频率的守卫条件，在模式切换时保持相位、频率和下垂参考的一致性，并将混合模型嵌入扩展卡尔曼滤波器进行状态估计。

Result: 该框架确保了稳定且行为良好的动态特性，与平滑连续模型相比，在切换时刻附近显著改善了状态估计性能。

Conclusion: 所提出的混合系统建模框架能够有效处理逆变器控制模式切换问题，提供更准确的状态估计和稳定的系统动态。
...
### [204] [A congestion-dependent imbalance pricing mechanism for regions allowing passive balancing](https://arxiv.org/abs/2511.13873)
*Hang Thanh Nguyen,Bart Van Der Holst,Phuong Hong Nguyen,Koen Kok*

Main category: eess.SY

TL;DR: 提出了一种依赖拥塞的不平衡定价机制，在保持被动平衡激励的同时满足电网约束，缓解了拥塞区域的意外峰值流量问题。


<details>
  <summary>Details</summary>
Motivation: 随着可再生能源份额增加，市场设计和电网容量增强滞后，维持系统平衡变得更具挑战性。平衡责任方（BRPs）专注于利润最大化而非最小化投资组合差异，可能对价格信号产生同时响应，在输配电接口处造成问题。

Method: 开发了一个两阶段随机模型，捕捉BRPs在日前和平衡市场价格不确定性下的动态行为及其对电网的影响，涵盖三种不平衡定价机制：单一、双重和两价。然后提出了一种依赖拥塞的不平衡定价机制。

Result: 通过对荷兰部分配电网的仿真验证，结果表明所提出的方法缓解了拥塞区域的意外峰值流量问题，同时保留了非拥塞区域其他BRPs的被动平衡贡献。

Conclusion: 提出的拥塞依赖不平衡定价机制能够在满足电网约束的同时保持被动平衡激励，有效解决BRPs行为对电网的影响问题。
...
### [205] [L-Functions Certify Set Attractivity for Discrete-Time Uncertain Nonlinear Switched Systems](https://arxiv.org/abs/2511.13906)
*Alejandro Anderson,Esteban A. Hernandez-Vargas,Giulia Giordano*

Main category: eess.SY

TL;DR: 提出L函数类来验证不确定非线性切换系统在离散时间下的集合吸引性，基于收缩集构造分段连续L函数，并应用于抗菌素耐药性建模。


<details>
  <summary>Details</summary>
Motivation: 为不确定非线性切换系统提供验证集合吸引性的系统化方法，特别关注生物系统等实际应用场景的分析需求。

Method: 基于系统收缩集构造分段连续L函数，证明鲁棒控制收缩集的存在性可推出相应L函数的存在性。

Result: 建立了L函数与集合鲁棒局部吸引性的等价关系，并通过抗菌素耐药性案例验证了方法的有效性。

Conclusion: L函数框架为不确定非线性切换系统的集合吸引性分析提供了有效的验证工具，在生物系统建模中具有应用价值。
...
### [206] [Dissipativity-Based Distributed Stability Analysis for Networks with Heterogeneous Nonlinear Agents](https://arxiv.org/abs/2511.13925)
*Ingyu Jang,Ethan J. LoCicero,Leila Bridgeman*

Main category: eess.SY

TL;DR: 结合Vidyasagar网络耗散性定理与ADMM，开发了非线性异构智能体网络的分布式稳定性分析方法，提升信息安全性和计算效率。


<details>
  <summary>Details</summary>
Motivation: 大型非线性智能体网络的稳定性分析具有挑战性，需要分解和分布式分析以实现计算可行性和信息安全。

Method: 将Vidyasagar网络耗散性定理与交替方向乘子法(ADMM)结合，开发两种分布式算法：一种仅需共享耗散性特征而非动力学模型；另一种进一步将信息共享限制在团内，支持弦分解以降低计算负担。

Result: 证明了算法收敛性，识别了支持弦分解的网络条件，并通过线性化无人机群和其他网络验证了方法的有效性。

Conclusion: 提出的分布式方法在保持信息安全的同时，有效解决了非线性异构智能体网络的稳定性分析问题，特别适用于支持弦分解的网络结构。
...
### [207] [Consensus-Based Stability Analysis of Multi-Agent Networks](https://arxiv.org/abs/2511.13926)
*Ingyu Jang,Ethan J. LoCicero,Leila Bridgeman*

Main category: eess.SY

TL;DR: 提出基于共识的分布式稳定性分析方法，利用网络耗散定理和ADMM算法进行一般稳定性分析，并通过无人机群案例验证算法收敛性


<details>
  <summary>Details</summary>
Motivation: 大规模多智能体系统的涌现需要稀疏通信的控制器合成方法，但现有方法多为集中式，需要信息交换和较高计算成本，因此需要仅使用局部动态信息的分布式算法

Method: 基于共识的分布式稳定性分析，结合Vidyasagar的网络耗散定理和交替方向乘子法(ADMM)进行一般稳定性分析

Result: 通过2D无人机群的数值示例验证了所提算法的收敛性

Conclusion: 提出的分布式稳定性分析方法能够有效解决多智能体系统的控制器设计问题，仅需局部动态信息即可实现稳定分析
...
### [208] [dkpy: Robust Control with Structured Uncertainty in Python](https://arxiv.org/abs/2511.13927)
*Timothy Everett Adams,Steven Dahdah,James Richard Forbes*

Main category: eess.SY

TL;DR: 开发了一个名为dkpy的开源Python包，用于对受结构化不确定性影响的系统进行鲁棒控制器分析和综合。


<details>
  <summary>Details</summary>
Motivation: 控制系统设计中的模型存在不确定性，需要确保闭环系统的鲁棒性。μ-分析和μ-综合方法能够处理结构化不确定性下的控制器分析和设计问题。

Method: 开发了dkpy开源Python包，提供鲁棒控制器分析和综合工具，以及使用扰动系统数据执行模型不确定性表征的工具。

Result: 成功创建了dkpy开源项目，可在GitHub上获取：https://github.com/decargroup/dkpy

Conclusion: dkpy为处理结构化不确定性的鲁棒控制问题提供了一个实用的开源工具，能够支持控制器分析和综合以及模型不确定性表征。
...
### [209] [Power Delivery for Cryogenic Scalable Quantum Applications: Challenges and Opportunities](https://arxiv.org/abs/2511.13965)
...
TL;DR: 本文评估了稀释制冷机中可扩展功率传输的几种候选架构，包括高压有线传输、辐射无线传输、非辐射无线传输以及高压与非辐射混合传输，通过热负载、功率损耗、加热、耦合噪声等指标分析，发现高压非辐射传输是量子系统可扩展性的有前景方案。


<details>
  <summary>Details</summary>
Motivation: 随着量子系统扩展到数百万量子比特，控制电子设备需要在低温下运行以避免布线瓶颈，但传统的有线功率传输会带来热负载、焦耳热、噪声耦合和布线可扩展性等严重挑战。

Method: 评估并比较了四种功率传输架构：高压有线传输、辐射无线传输、非辐射无线传输以及高压与非辐射混合传输，从热负载、功率损耗、加热、耦合噪声、功率密度、可扩展性、可靠性和复杂性等方面进行分析。

Result: 比较分析显示了这些架构之间的权衡关系，高压非辐射传输在可扩展量子系统中表现出良好前景。

Conclusion: 高压非辐射传输是解决量子系统功率传输挑战的有前景候选方案，能够满足大规模量子系统对可扩展功率传输的需求。
...
### [210] [On the Impact of Voltage Unbalance on Distribution Locational Marginal Prices](https://arxiv.org/abs/2511.13971)
*Alireza Zabihi,Luis Badesa,Araceli Hernandez*

Main category: eess.SY

TL;DR: 将电压不平衡作为软限制处理，通过在最优潮流中加入惩罚项来反映电压不平衡对设备寿命的影响，从而产生包含不平衡成本的分区边际电价。


<details>
  <summary>Details</summary>
Motivation: 传统电网标准对电压不平衡采用不同的硬性限制，缺乏动态经济激励且无法达到最优。随着单相负载和分布式能源增加，电压不平衡问题日益严重，导致设备绝缘老化加速和网络损耗增加。

Method: 在三相最优潮流中加入电压不平衡惩罚项，开发了包含能量、损耗、拥堵和不平衡四个组成部分的分区边际电价数学分解方法。

Result: 案例研究表明，不平衡惩罚重塑了节点电价，产生了意外的相级效应，甚至出现增加负载反而降低不平衡和成本的情况。
...
### [211] [Data Center Control Against Sub-Synchronous Resonance: A Data-Driven Approach](https://arxiv.org/abs/2511.14141)
*Grant Ruan,Marija D. Ilic,Le Xie*

Main category: eess.SY

TL;DR: 该论文研究了数据中心与电网之间的次同步谐振风险，通过阻抗建模和数据驱动的预防控制方法，为大规模数据中心并网提供安全保障。


<details>
  <summary>Details</summary>
Motivation: 电网运营商对数据中心并网的可靠性风险了解有限，特别是次同步谐振可能导致设备损坏和级联故障。数据中心独特的运行特性（如功率因数校正转换器）与电网交互可能引发谐振问题。

Method: 1. 对通过PFC转换器并网的数据中心进行双频扫描，分析其阻抗特性（幅值和相位角随频率变化）；2. 建立阻抗模型评估SSR风险；3. 开发数据驱动的预防控制器，提供早期预警和灵活的工作负载管理建议。

Result: 阻抗建模能有效捕捉数据中心独特的阻抗凹陷特征，并跟踪不同工作负载场景下的阻抗变化。早期预警和预防控制方法能显著提高安全裕度，且只需最小的工作负载重新调度。

Conclusion: 该研究为电网运营商和数据中心管理者提供了有价值的见解，支持未来大规模数据中心并网场景的准备，通过阻抗分析和预防控制可有效避免谐振风险。
...
### [212] [Uncertainty Discounting in Deterministic Black Box Price Predictions for Energy Arbitrage](https://arxiv.org/abs/2511.14158)
*Arnab Bhattacharjee*

Main category: eess.SY

TL;DR: 本研究提出了一种简单的后验不确定性折扣方法，用于改善电池能量套利中的经济表现，在保持相同操作约束下实现了超过20%的经济回报提升。


<details>
  <summary>Details</summary>
Motivation: 传统的模型预测控制框架依赖确定性电价预测来优化电池调度，但这些预测缺乏不确定性意识，使套利策略容易受到极端价格波动的影响。

Method: 提出了简单的启发式不确定性折扣方法，无需访问预测模型的架构或输入，将这些策略集成到现有的MPC框架中。

Result: 在2.2 MWh、1.1 MW的特斯拉电池测试系统中，展示了在相同操作约束下经济回报超过20%的改善。

Conclusion: 该方法增强了能量套利中的决策能力，同时保持实用性、可扩展性，并且独立于特定的预测模型。
...
### [213] [A Receding Horizon Reinforcement Learning Framework for Campus Chiller Energy Management - A case study from an Australian University](https://arxiv.org/abs/2511.14160)
*Laura Musgrave,Arnab Bhattacharjee,Tapan Kumar Saha*

Main category: eess.SY

TL;DR: 使用强化学习优化澳大利亚大学校园大型HVAC系统的能源管理，相比传统基于规则的方法节省28%电力消耗


<details>
  <summary>Details</summary>
Motivation: 传统基于规则的HVAC控制方法效率低下，需要更智能的能源管理方案来降低大型校园建筑的能耗

Method: 开发预测性建筑冷却需求模型，使用近端策略优化强化学习算法在滚动时域控制框架中优化冷水机组调度

Result: 通过控制冷水机组质量流量，在最小约束违反的情况下节省高达28%的电力消耗

Conclusion: 强化学习方法在大型HVAC系统能源管理中具有显著优势，能有效降低能耗并满足运行约束
...
### [214] [Entirely Transformerless Universal Direct-Injection Power-Flow Controller](https://arxiv.org/abs/2511.14209)
*Davood Keshavarzi,Alexander Koehler,Wolfram H. Wellssow,Stefan M. Goetz*

Main category: eess.SY

TL;DR: 提出了一种紧凑的无变压器部分功率转换电路，用于解决低压电网中可再生能源、电动汽车充电器和储能系统带来的功率管理和稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 传统功率流和软开点解决方案体积庞大且昂贵，需要变压器和大型磁性元件，而高频变压器方案仍面临成本和尺寸挑战。

Method: 结合硅和碳化硅器件的优势，采用无变压器设计，通过非隔离逆变器将并联转换器与低压串联模块双向连接，实现高电流直接注入。

Result: 电路比先前概念需要更少的半导体器件，能够在低压电网的不同馈线之间提供服务。

Conclusion: 该电路通过数学分析、仿真和实验验证了其运行性能，提供了一种紧凑且成本效益高的功率流控制解决方案。
...
### [215] [Improved Decoupled Control of Modular Multilevel Converter under Constaint of Nearest Level Modulation via Disturbance Observer Design](https://arxiv.org/abs/2511.14217)
*Jaeyeon Park,Dongjoon Kim,Seungjun Lee,Shenghui Cui*

Main category: eess.SY

TL;DR: 提出了一种基于扰动观测器的新控制策略，用于解决NLM在SM数量较少时引起的电压电流畸变问题，实现间接调制MMC的完全解耦控制。


<details>
  <summary>Details</summary>
Motivation: NLM在SM数量较少时会产生显著的电压电流畸变，而间接调制虽然能实现完全解耦控制，但其性能依赖于精确的臂电压合成，与NLM固有的量化误差不兼容。

Method: 使用扰动观测器估计并主动补偿NLM引起的臂电压合成误差，无需修改传统NLM和解耦控制结构。

Result: 通过仿真和实验验证了该方法在改善电流质量和实现解耦SM能量控制方面的有效性和可行性。

Conclusion: 该方法成功解决了NLM与间接调制之间的冲突，为SM数量较少的MMC系统提供了有效的控制解决方案。
...
### [216] [Secure parameter identification of ARX systems with CKKS cryptosystem](https://arxiv.org/abs/2511.14267)
*Jialong Chen,Jimin Wang,Ji-Feng Zhang*

Main category: eess.SY

TL;DR: 提出基于CKKS加密系统的ARX系统参数识别算法，保护系统输入输出隐私，通过投影映射和随机逼近方法实现收敛


<details>
  <summary>Details</summary>
Motivation: 解决云环境下ARX系统参数识别时的输入输出隐私保护问题

Method: 基于CKKS加密系统，利用投影映射和随机逼近方法，严格证明高斯分布与截断离散分布统计距离可忽略

Result: 算法具有与标准CKKS加密系统相同的安全级别，通过数值示例验证了有效性

Conclusion: 提出的加密参数识别算法能有效保护隐私，同时实现几乎必然和均方收敛
...
### [217] [A graph-informed regret metric for optimal distributed control](https://arxiv.org/abs/2511.14280)
*Daniele Martinelli,Andrea Martin,Giancarlo Ferrari-Trecate,Luca Furieri*

Main category: eess.SY

TL;DR: 提出了一种名为空间遗憾的图信息度量，用于评估分布式控制器与具有额外传感器信息的oracle之间的性能差距。通过最小化空间遗憾，可以得到能有效应对大规模网络局部扰动的分布式控制器。


<details>
  <summary>Details</summary>
Motivation: 传统分布式控制器在大规模系统中对局部扰动的处理效果不佳，需要一种新的度量标准来评估控制器性能，使其能更好地模拟具有额外信息的oracle对局部扰动的响应。

Method: 引入空间遗憾度量，将其转化为凸优化问题，并推导出可计算的上界，使优化问题能够以分布式方式求解。

Result: 在电力系统模型上的数值实验表明，基于空间遗憾优化的控制器比使用传统度量的控制器能更有效地缓解局部扰动。

Conclusion: 空间遗憾是一种有效的图信息度量，能够指导设计出在大规模网络中更好处理局部扰动的分布式控制器。
...
### [218] [Overtourism to Equilibrium: A System Dynamics & Multi-Objective Model for Sustainable Destinations](https://arxiv.org/abs/2511.14288)
*Huanzhu Lyu,Xiao Yang,Xintong Ji*

Main category: eess.SY

TL;DR: 开发了一个结合系统动力学和多目标进化算法(NSGA-II)的决策模型，用于平衡旅游经济收益、环境保护和社会满意度，通过朱诺和冰岛的案例研究验证了模型有效性。


<details>
  <summary>Details</summary>
Motivation: 过度旅游对热门目的地造成严重挑战，威胁自然环境和当地社区，需要开发综合决策模型来平衡经济、环境和社会目标。

Method: 集成系统动力学与NSGA-II多目标进化算法，收集2008-2024年多源数据，建立包含游客行为、政府财政、环境演变和社会福祉四个模块的动态系统。

Result: 朱诺案例优化结果显示净收入达16.4亿美元，环境指数0.93，社会满意度0.86；冰岛案例显示帕累托前沿覆盖收入1.5-2亿美元，环境指数达0.92，社会满意度超0.80。

Conclusion: 该研究贡献了可持续旅游管理的综合框架，通过案例研究证明了模型的可移植性，并通过敏感性分析识别了影响政策结果的关键杠杆因素。
...
### [219] [Multi-Timescale Model Predictive Control for Slow-Fast Systems](https://arxiv.org/abs/2511.14311)
*Lukas Schroth,Daniel Morton,Amon Lahr,Daniele Gammelli,Andrea Carron,Marco Pavone*

Main category: eess.SY

TL;DR: 该论文提出了一种多时间尺度的模型预测控制方案，通过切换至仅捕捉慢速主导动态的简化模型，并沿预测时域指数增加积分步长来降低模型细节，从而显著提高计算效率。


<details>
  <summary>Details</summary>
Motivation: 模型预测控制在约束控制中占据主导地位，但将长时域与高保真模型结合时，实时求解优化问题仍然具有挑战性。受灵敏度指数衰减结果的启发，该研究旨在解决快速采样控制中的计算效率问题。

Method: 针对具有快慢动态的系统，提出多时间尺度MPC方案：i) 切换到仅捕捉慢速主导动态的简化模型；ii) 沿预测时域指数增加积分步长以逐步减少模型细节。

Result: 在三个实际机器人控制问题的仿真中评估该方法，观察到速度提升可达一个数量级。

Conclusion: 所提出的多时间尺度MPC方案能够显著提高计算效率，同时保持控制性能，为具有多时间尺度动态的系统提供了实用的解决方案。
...
### [220] [An adaptive extension to robust data-driven predictive control under parametric uncertainty](https://arxiv.org/abs/2511.14319)
*Ignacio Sanchez,Filiberto Fele,Daniel Limon*

Main category: eess.SY

TL;DR: 提出了一种结合离线数据和在线滚动窗口数据的鲁棒数据驱动控制方法，用于稳定参数在有限多面体集合内变化的时变线性系统。


<details>
  <summary>Details</summary>
Motivation: 传统数据驱动控制器依赖历史实验数据来覆盖系统参数变化，而在线数据能针对当前系统条件提升性能，但无法处理整体时变系统运行。需要同时利用这两种数据源来设计稳定控制器。

Method: 基于数据信息性框架，同时利用离线数据集（覆盖系统参数极端变化）和在线滚动窗口数据，通过半定优化实现状态反馈控制律设计，无需持续激励条件。

Result: 数值实验验证了所提出控制器的有效性，能够为与在线数据一致的系统类提供成本上界，同时保证与离线数据兼容的所有系统的成本递减。

Conclusion: 该方法成功结合了离线和在线数据源，在放松持续激励要求的同时实现了时变线性系统的稳定控制，为数据驱动控制提供了新的设计思路。
...
### [221] [Identifying Time-varying Costs in Finite-horizon Linear Quadratic Gaussian Games](https://arxiv.org/abs/2511.14358)
*Kai Ren,Maryam Kamgarpour*

Main category: eess.SY

TL;DR: 提出了一个在有限时域线性二次高斯博弈中识别成本参数的算法，通过反向传播方法识别时变成本参数，并推导了有限轨迹下的概率误差界。


<details>
  <summary>Details</summary>
Motivation: 解决在有限时域线性二次高斯博弈中成本参数的识别问题，即给定纳什均衡策略，确定生成该策略的成本参数集合。

Method: 使用反向传播算法识别时变成本参数，并推导了从有限轨迹识别成本参数时的概率误差界。

Result: 在数值和驾驶仿真中测试，算法能够识别出能够重现纳什均衡策略和轨迹观测的成本参数。

Conclusion: 所提出的算法有效解决了线性二次高斯博弈中的成本识别问题，能够准确识别生成给定纳什均衡策略的成本参数。
...
### [222] [Offset-free Data-Driven Predictive Control for Grid-Connected Power Converters in Weak Grid Faults](https://arxiv.org/abs/2511.14337)
*Ivo Kraayeveld,Thomas de Jong,Mircea Lazar*

Main category: eess.SY

TL;DR: 用无偏数据驱动预测控制器替代传统PI控制器，显著提升弱电网故障期间并网变流器的稳定性和故障穿越能力。


<details>
  <summary>Details</summary>
Motivation: 弱电网故障时，传统PI控制器会产生振荡响应和较差的故障穿越性能，需要更鲁棒的控制方案。

Method: 开发无偏数据驱动预测控制器，利用故障前或故障时数据构建输入输出预测器，无需物理建模即可实现无偏控制。

Result: 预故障无偏DPC可将处理的关键等效电网阻抗翻倍，故障期间均方根误差降低40倍，计算时间与传统PI控制相当。

Conclusion: 该无偏数据预测控制器为传统控制提供了简单、鲁棒且计算高效的替代方案，显著增强了弱电网中变流器的故障穿越能力。
...
### [223] [Accelerating Automatic Differentiation of Direct Form Digital Filters](https://arxiv.org/abs/2511.14390)
*Chin-Yun Yu,György Fazekas*

Main category: eess.SY

TL;DR: 提出了一种通过直接形式滤波器进行自动微分的通用公式，得到了包含初始条件梯度的闭式反向传播。该方法支持并行计算，在PyTorch中的C++/CUDA实现比原生Python实现快1000倍以上，在GPU上运行最快。


<details>
  <summary>Details</summary>
Motivation: 为直接形式滤波器开发高效的自动微分方法，支持初始条件梯度计算，并实现并行化处理。

Method: 使用通用公式进行直接形式滤波器的自动微分，得到闭式反向传播表达式，支持初始条件梯度计算。采用C++/CUDA实现并行计算。

Result: 在PyTorch中的实现比原生Python快1000倍以上，在GPU上运行最快。对于实践中常用的低阶滤波器，时域滤波方法在速度上优于频域方法。

Conclusion: 该方法为直接形式滤波器提供了高效的自动微分解决方案，支持并行计算和初始条件梯度，在GPU上表现出色。
...
### [224] [Ultra-Low Insertion Loss Stepped Impedance Resonator Topology for HTSC RF Front-End](https://arxiv.org/abs/2511.14447)
*Ilan Kurtser,Yoav Koral,Eldad Holdengreber,Shmuel E. Schacham,Eliyahu Farber*

Main category: eess.SY

TL;DR: 本文设计并制造了一种用于S波段应用的高温超导阶梯阻抗谐振器带通滤波器，并将其集成到低温接收机级联中。该11极滤波器在3.39GHz处实现了-0.1dB的超低插入损耗和0.34dB的接收机噪声系数，相比传统铜基前端可将雷达探测距离提升20%。


<details>
  <summary>Details</summary>
Motivation: 开发具有超低插入损耗的高温超导滤波器，以提升下一代通信和雷达系统的性能，特别是通过降低接收机噪声系数来增加雷达探测距离。

Method: 采用YBa2Cu3O(7-x) (YBCO)薄膜在蓝宝石衬底上制造11极阶梯阻抗谐振器带通滤波器，并将其与低温低噪声放大器集成到接收机级联中。

Result: 滤波器表现出-0.1dB的超低插入损耗、100MHz的陡峭滚降和超过-80dB的抑制水平。系统级测量预测接收机噪声系数为0.34dB，相比传统铜基前端可将雷达探测距离提升20%。

Conclusion: 这项工作证明了基于高温超导的实用射频前端在下一代通信和雷达系统中的可行性，实现了目前报道的S波段滤波器中最低的插入损耗。
...
### [225] [Agentic AI Systems in Electrical Power Systems Engineering: Current State-of-the-Art and Challenges](https://arxiv.org/abs/2511.14478)
*Soham Ghosh,Gaurav Mittal*

Main category: eess.SY

TL;DR: 本文对"代理AI"这一新兴AI范式进行了概念定义和分类学梳理，通过电气工程领域的四个先进应用案例展示了其实际影响，并提出了确保系统安全可靠部署的设计建议。


<details>
  <summary>Details</summary>
Motivation: 代理AI系统作为人工智能领域的关键变革方法，其能力远超传统AI代理和当代生成式AI模型。这种快速演进需要清晰的概念和分类学理解来区分这一新范式。

Method: 通过建立代理AI的精确定义和分类学，逐步介绍概念，重点展示其在工程领域的多样化应用，特别提供了电气工程中四个详细的先进应用案例研究。

Result: 案例研究展示了实际影响，包括用于简化复杂电力系统研究和基准测试的先进代理框架，以及为电池交换站动态定价策略生存分析开发的新系统。

Conclusion: 基于故障模式调查结果，提出了可操作的建议，用于设计和实施安全、可靠和负责任的代理AI系统，为研究人员和从业者提供了关键资源。
...
### [226] [Stratospheric Grid: A Wireless Power Transfer Enabled HAP Network with Integrated Generation-Grid-Load-Storage Functions](https://arxiv.org/abs/2511.14527)
*Peng Wang,Eros Kuikel,Jia Ye,Mohamed-Slim Alouini*

Main category: eess.SY

TL;DR: 提出平流层能量网格系统，通过无线电力传输连接高空平台作为可重构的发电-电网-负载-存储节点，实现能量交换和时空优化。
...
Method: 构建平流层能量网格，将高空平台作为动态可重构的IGGLS节点，通过无线电力传输实现点对点能量传输和协同调度。

Result: 系统验证了通过能量网格可实现高空平台间的能量互补支持，并扩展到卫星和地面电网，提升整体能量利用率和运行韧性。

Conclusion: 该平流层能量网格系统为解决高空平台连续运行问题提供了可行方案，并指出了未来研究方向。
...
### [227] [Robust Offset-free Kernelized Data-Driven Predictive Control for Nonlinear Systems](https://arxiv.org/abs/2511.14652)
*Mahmood Mazare,Hossein Ramezani*

Main category: eess.SY

TL;DR: 提出了一种新颖的核化数据驱动预测控制方案，用于非线性系统的鲁棒无偏跟踪。该方法结合核岭回归和核映射的解析线性化，将控制器表示为标准二次规划问题，实现高效实时控制。


<details>
  <summary>Details</summary>
Motivation: 传统模型预测控制需要精确的系统模型，而数据驱动方法在处理非线性系统时面临计算复杂性和实时性挑战。本文旨在开发一种计算高效、能实现无偏跟踪的数据驱动预测控制方法。

Method: 采用混合方法：1) 使用核岭回归从历史轨迹学习非线性映射；2) 对核映射进行解析线性化来近似未来输入的影响。这种线性化使得控制器可以表示为标准二次规划问题。

Result: 在非线性Van der Pol振荡器上的验证表明，该方法能成功抑制未测量扰动、消除稳态误差，性能优于标准模型控制器。

Conclusion: 所提出的KDPC方案为非线性系统提供了一种计算高效、理论保证的数据驱动预测控制方法，能够实现鲁棒的无偏跟踪控制。
...
### [228] [Towards AC Feasibility of DCOPF Dispatch](https://arxiv.org/abs/2511.14725)
*Michael A. Boateng,Russell Bent,Sidhant Misra,Parikshit Pareek,Pascal Van Hentenryck,Daniel Molzahn*

Main category: eess.SY

TL;DR: 开发了一个统一的DCOPF-ACPF管道，从DCOPF调度中恢复AC可行解，显著提高了AC可行性并减少了成本差异。


<details>
  <summary>Details</summary>
Motivation: DCOPF因其简单性和计算效率在电力系统中广泛应用，但其无损耗、忽略无功功率的模型在实际ACPF场景下往往产生不可行调度。

Method: 使用四种DCOPF变体，通过分布式松弛分配和PV/PQ切换进行AC可行性恢复，构建统一的DCOPF-ACPF管道。

Result: 在13,659节点案例中，DCOPF与ACOPF的平均绝对误差和成本差异分别减少了75%和93%；在极端负载条件下，不等式约束违规减少了3-5倍。

Conclusion: 该管道能有效恢复AC可行性，满足ACPF方程和所有工程不等式约束，比传统单松弛总线方法表现更优。
