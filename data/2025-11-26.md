...
TL;DR: 提出了一种名为PuzzlePole的新型圆柱形标记器，基于PuzzleBoard校准模式，能够从360度视角进行可靠识别和姿态估计。


<details>
  <summary>Details</summary>
Motivation: 自主系统需要可靠的环境感知能力，而校准和定位任务通常依赖于鲁棒的视觉标记。现有标记器在视角范围和抗遮挡方面存在局限。

Method: 利用PuzzleBoard模式的独特组合结构设计圆柱形标记器，通过其几何特性实现360度视角的识别和姿态估计。

Result: PuzzlePole在定位和定向方面提供高精度，同时对遮挡具有鲁棒性。

Conclusion: PuzzlePole为自主系统（如机器人导航、SLAM和实体界面）提供了一种灵活且可靠的视觉标记解决方案。
...
TL;DR: PIGReward是一个个性化奖励模型，通过动态生成用户条件评估维度和CoT推理来评估文本到图像生成模型，无需用户特定训练即可实现个性化评估和提示优化。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型的评估方法无法捕捉个人视觉偏好的多样性和复杂性，需要开发能够适应个体用户偏好的个性化评估方法。

Method: 采用自引导策略，通过有限的参考数据进行推理构建丰富的用户上下文，使用CoT推理动态生成用户条件评估维度，无需用户特定训练即可实现个性化。
...
<details>
  <summary>Details</summary>
Motivation: 深度视觉模型中训练点对测试预测影响估计面临挑战：逆曲率计算昂贵，训练非平稳性使静态近似失效，现有方法离线计算滞后且置信度校准缺失导致脆弱排序。

Method: SG-OIF框架将算法稳定性作为实时控制器，维护轻量级锚点IHVP，提出模块化曲率后端，使用稳定性引导的残差阈值、异常门控和置信度来调节每样本影响分数。

Result: 在CIFAR-10(20%不对称噪声)上top 1%预测样本准确率达91.1%，MNIST上AUPR得分99.8%，在多个数据集的各种损坏上达到SOTA。

Conclusion: 该框架是用于在线影响估计的实用控制器，有效解决了深度视觉模型中训练点影响估计的挑战。
...
TL;DR: Pistachio是一个新的视频异常检测与理解基准，通过生成式流水线构建，解决了现有基准在场景多样性、异常覆盖度和时间复杂性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有视频异常检测基准缺乏场景多样性、平衡的异常覆盖和时间复杂性，而视频异常理解需要更深层的语义和因果推理但难以标注。

Method: 利用视频生成模型，通过场景条件异常分配、多步骤故事情节生成和时间一致的长格式合成策略，构建41秒的连贯视频。

Result: 实验证明了Pistachio的规模、多样性和复杂性，揭示了现有方法的新挑战。

Conclusion: Pistachio为动态和多事件异常理解提供了新的研究动力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理跟踪和分割任务时使用专门架构或模态特定参数，限制了泛化能力和可扩展性。现有统一方法忽视了跨模态分布差距和跨任务特征表示差距这两个关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 姿态估计是计算机视觉中的重要问题，现有方法如SVD和四元数特征系统能精确解决带噪声的3D-3D对齐，但对于带噪声的3D-2D正交投影问题尚无类似闭式解。

Method: 使用行列式比值矩阵（DRaM）方法求解无误差EnP和OnP问题的最小二乘系统，并通过直接旋转校正方案处理带噪声数据。

Result: 提出了EnP和OnP旋转估计问题的DRaM家族解决方案，并与现有方法进行比较分析。

Conclusion: 这项工作不仅为3D和2D正交投影姿态估计提供了新解决方案，还深入揭示了这类问题的本质特征，并证明DRaM方法可推广到所有N维欧几里得姿态估计问题。
...
TL;DR: LatentDreamer是一个从单张图像生成3D物体的新框架，使用预训练的变分自编码器将3D几何映射到潜在特征，通过粗几何生成、精细几何生成和纹理生成三个步骤，在70秒内完成高保真3D生成。


<details>
  <summary>Details</summary>
Motivation: 当前自动3D生成方法难以同时实现快速、详细和高保真的生成效果，需要一种能平衡这些要求的新方法。

Method: 使用预训练VAE将3D几何映射到潜在特征，通过三阶段流水线：粗几何生成→精细几何生成→真实纹理生成，降低3D生成难度。

Result: 生成的3D物体对输入图像具有高保真度，整个生成过程在70秒内完成，与当代方法相比具有竞争力。

Conclusion: LatentDreamer通过潜在特征映射和三阶段生成流程，实现了快速、高质量的3D物体生成，仅需少量训练即可达到竞争性性能。
...
<details>
  <summary>Details</summary>
Motivation: 受最小描述长度原则启发，研究模型容量与视觉语义空间所需最小token数量之间的基本关系，探索如何更高效地表示图像语义。

Method: 将图像token重新解释为视觉语义空间中的向量，定义图像内在语义复杂度为跨越该空间所需的最小基向量集合。提出正交过滤方法，自适应地将冗余token聚类为紧凑的正交基集合。

Result: 通过广泛的ViT模型实验，揭示了token与模型规模的缩放规律：更大的模型需要显著更少的token来跨越视觉语义空间。同时贡献了一个视觉长上下文数据集。

Conclusion: 模型容量与所需视觉token数量之间存在明确的缩放关系，更大的模型能够用更少的token有效表示图像语义，正交过滤方法能够有效压缩冗余token。
...
TL;DR: 提出了GroundingAgent，一种无需任务特定微调的代理式视觉定位框架，通过集成预训练的开词汇目标检测器、多模态大语言模型和大语言模型，实现零样本视觉定位。


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位方法依赖大量任务特定标注和微调，难以泛化到新场景或分布外情况，需要一种无需微调就能有效工作的解决方案。

Method: 采用结构化迭代推理机制，结合预训练的开词汇目标检测器、多模态大语言模型和大语言模型，通过联合语义和空间分析逐步优化候选区域。

Result: 在RefCOCO、RefCOCO+、RefCOCOg基准测试中达到65.1%的平均零样本定位准确率；仅使用选择阶段时准确率可达约90%，接近监督方法性能。

Conclusion: GroundingAgent证明了无需微调的视觉定位可行性，突出了大语言模型推理能力的关键作用，同时提供了良好的可解释性。
...
TL;DR: 提出InfoPrune信息理论框架，用于视觉语言模型的自适应结构压缩，基于信息瓶颈原理，通过注意力头剪枝和FFN低秩近似实现高效压缩。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型规模不断增长带来部署挑战，传统压缩方法缺乏理论保证，需要一种基于信息理论的结构化压缩框架。

Method: 基于信息瓶颈原理，引入熵有效秩和KS距离量化注意力头贡献，提出训练式注意力头剪枝和训练自由FFN低秩近似两种压缩方案。
...
TL;DR: 提出了一种基于3D面部标志的眼睑角度(ELA)新指标，用于检测驾驶员疲劳状态，该指标比传统的2D眼部特征更稳定，并可用于生成合成数据集。


<details>
  <summary>Details</summary>
Motivation: 可靠检测驾驶员疲劳对道路安全至关重要，但现有方法如眼宽比(EAR)对视角变化敏感，且自然疲劳数据收集困难且危险。
...
Conclusion: ELA既是可靠的生物特征度量，也是驾驶员状态监测中生成可扩展数据集的强大工具。
...
TL;DR: VideoChat-M1是一个多智能体视频理解系统，采用协作策略规划范式，通过多个策略智能体动态生成、执行和更新工具调用策略，结合多智能体强化学习优化性能，在多个基准测试中达到SOTA水平。
...
Method: 提出协作策略规划范式：1) 策略生成：每个智能体根据用户查询生成独特的工具调用策略；2) 策略执行：智能体顺序调用相关工具执行策略；3) 策略通信：执行过程中智能体相互交互更新策略。结合多智能体强化学习进行联合优化。
...
TL;DR: 提出了感知分类学基准，用于评估基于物理的视觉推理能力，发现现有视觉语言模型在属性驱动的多步推理任务上表现较差，但通过感知分类学引导的提示可以改善性能。
...
Method: 构建了包含3173个对象、84个精细属性的感知分类学基准，包含28033个模板问题和50个专家设计问题，涵盖对象描述、空间推理、属性匹配和分类学推理四种类型。

Result: 领先的视觉语言模型在识别任务上表现良好，但在属性驱动问题上性能下降10-20%，特别是在需要结构化属性多步推理的任务上。

Conclusion: 当前模型在结构化视觉理解方面存在持续差距，主要依赖模式匹配，而感知分类学引导的提示能有效提升真实世界和专家问题的推理性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有在线HD地图构建方法依赖昂贵的3D地图标注进行训练，限制了在不同驾驶环境中的泛化性和可扩展性。

Method: 提出基于NeRF的条件模块生成视图一致的3D几何和语义伪标签，采用自训练方式迭代优化地图网络，并引入Map-to-Ray匹配策略减轻误差累积。

Result: 在Argoverse 2和nuScenes数据集上的实验表明，MapRF达到全监督方法约75%的性能，超越了多个仅使用2D标签的方法。

Conclusion: MapRF展示了实现可扩展且成本效益高的在线HD地图构建的潜力。
...
TL;DR: Vidi2模型在视频理解方面取得重大进展，具备细粒度时空定位能力，扩展至视频问答，在时空检索和时空定位基准测试中超越领先专有系统。


<details>
  <summary>Details</summary>
Motivation: 视频已成为互联网主要通信和创作媒介，需要可扩展、高质量的视频生产解决方案。现有模型在复杂编辑场景中的时空定位能力不足。

Method: Vidi2模型实现端到端时空定位，给定文本查询可识别对应时间戳和边界框。引入新基准VUE-STG和升级版VUE-TR-V2进行综合评估。

Result: Vidi2在VUE-TR-V2和VUE-STG基准上大幅超越Gemini 3 Pro和GPT-5等专有系统，在视频问答基准上与类似规模开源模型竞争。

Conclusion: Vidi2通过时空定位能力为复杂编辑应用提供支持，新基准为实际场景中的时空定位评估提供更全面的框架。
...
TL;DR: 本研究探讨了多模态大语言模型在分布式光伏系统跨域泛化中的应用，通过结构化提示和微调，在未见区域实现了优于传统计算机视觉方法的性能表现。
...
TL;DR: 该论文提出了大规模分析制图遗产的方法和数据集，通过77万条地图记录和近10万张数字化图像，揭示了地图作为文化对象反映政治和认知期望的特性，并开发了语义分割和目标检测技术来分析地图内容。


<details>
  <summary>Details</summary>
Motivation: 虽然全球遗产机构已数字化超过100万张地图，自动化技术也能大规模识别地图内容，但这些方法很少结合制图史视角，未能将地图视为反映政治和认知期望的语义符号系统和文化对象。

Method: 利用来自38个数字目录的771,561条地图记录和99,715张数字化图像，经过标准化处理后包含236,925位贡献者；引入语义分割技术和目标检测模型，使用标注数据和合成图像进行训练，识别土地类别和制图符号。

Result: 分析了制图的空间焦点与政治动态的关系，揭示了地图作为设计图像具有中心化和语义对称的特征；编码了6300万个符号和2500万个片段，发现了从阴影线到地形等高线的图形演变；证明了符号倾向于形成局部一致系统，并突出了合法性和主要城市在图形规范和符号文化传播中的作用。

Conclusion: 该研究通过大规模数据分析揭示了地图作为文化产品的复杂性，证明了制图实践与政治、经济和社会因素的紧密联系，为理解制图遗产提供了新的文化视角和方法论框架。
...
TL;DR: SpLap是一种基于表面感知splat图的拉普拉斯算子的无代理高斯splat变形方法，无需依赖变形代理如笼子或网格，能更好地保持细节和拓扑结构。


<details>
  <summary>Details</summary>
Motivation: 现有高斯splat变形方法依赖变形代理，存在代理质量依赖和额外计算开销问题；直接应用拉普拉斯变形技术又因缺乏显式结构而无法正确捕捉表面信息。

Method: 构建表面感知splat图，基于splat的交集而非中心距离定义邻域关系；使用拉普拉斯算子支持合理变形；引入高斯核自适应技术保持变形后的表面结构。

Result: 在ShapeNet、Objaverse、Sketchfab和NeRF-Synthetic数据集的50个挑战性对象上，相比基于代理和无代理基线方法表现出优越性能。

Conclusion: SpLap方法通过表面感知splat图和拉普拉斯算子实现了更合理的高斯splat变形，能有效保持细节和拓扑结构，提升变形后的渲染质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI灾害评估方法依赖昂贵的数据标注和固定答案空间，生成模型存在幻觉输出和通用回答问题，需要更灵活、准确且可解释的解决方案。

Method: 两阶段推理框架：首先生成结构化推理轨迹（思维链提示+上下文学习），然后通过答案选择模块评估并选择最一致和准确的答案，结合定制信息检索系统和领域特定提示。

Result: 在FloodNet和RescueNet-VQA数据集上实验表明，ThiFAN-VQA在准确性、可解释性和适应性方面表现优异，适用于真实世界的灾后损害评估任务。

Conclusion: ThiFAN-VQA在零样本和监督方法之间架起桥梁，结合灵活性和一致性，为灾害场景下的视觉问答提供了有效的解决方案。
...
TL;DR: HunyuanOCR是一个商业级、开源、轻量级（10亿参数）的视觉语言模型，专用于OCR任务。它在感知任务（文本检测、解析）和语义任务（信息提取、文本图像翻译）上表现出色，在ICDAR 2025 DIMT挑战赛小型模型赛道获得第一名，并在OCRBench上达到SOTA水平。


<details>
  <summary>Details</summary>
Motivation: 解决传统"OCR专家模型"功能单一和"通用VLM"效率低下的问题，统一多功能性和效率，提供端到端的解决方案，避免传统流程中的错误传播。

Method: 采用原生视觉Transformer和轻量级LLM通过MLP适配器连接的架构，纯端到端范式消除对预处理模块的依赖，结合高质量数据和强化学习策略。

Result: 超越商业API、传统流程和更大模型（如Qwen3-VL-4B），在ICDAR 2025 DIMT挑战赛小型模型赛道获得第一名，在OCRBench上达到SOTA水平。

Conclusion: HunyuanOCR在多功能性与效率统一、端到端架构和数据驱动策略方面取得突破，为前沿研究和工业应用提供坚实基础，已在HuggingFace开源并提供高性能部署方案。
...
<details>
  <summary>Details</summary>
Motivation: 缺血性卒中是一种时间紧迫的医疗紧急情况，非对比增强CT作为一线成像工具，在早期超急性期往往无法显示细微的缺血变化，这会延迟关键干预措施。

Method: 使用生成对抗网络框架，结合少量标注的NCCT扫描和大量未标注扫描，采用Dice损失、交叉熵损失、特征匹配损失和自训练损失来学习识别和描绘早期梗死区域。

Result: 在公开可用的急性缺血性卒中数据集（AISD）上的实验表明，该方法有潜力提升诊断能力，减少手动标注负担，并支持更高效的卒中护理临床决策。

Conclusion: 该方法能够有效解决早期缺血性卒中在NCCT上的诊断挑战，为临床提供更准确的病灶分割结果。
...
TL;DR: 提出了一种基于VAE的多尺度向量量化变分自编码器(MSVQ-VAE)方法，用于生成无线胶囊内窥镜(WCE)图像，特别是能够无缝地将异常引入正常WCE图像中，并支持条件生成不同类型的异常。


<details>
  <summary>Details</summary>
Motivation: 由于隐私约束和标注成本，医疗数据稀缺阻碍了基于深度学习的临床决策支持系统的发展，需要生成合成数据来解决这一限制。

Method: 开发了多尺度向量量化变分自编码器(MSVQ-VAE)，能够条件生成包含不同类型异常(如息肉、血管和炎症)的合成WCE图像。

Result: 实验表明，使用该方法生成的异常图像训练的CDS分类器与仅使用真实数据训练的分类器性能相当。

Conclusion: 该方法为医疗多媒体领域提供了一种有效的合成数据生成解决方案，具有良好的泛化能力。
...
TL;DR: SkillSight是一个用于第一人称视角技能评估的节能系统，通过结合注视数据和视频来预测技能水平，最终仅使用注视数据就能高效评估技能，功耗降低73倍。


<details>
  <summary>Details</summary>
Motivation: 智能眼镜上的自我中心感知可以改变我们在物理世界中学习新技能的方式，但自动技能评估仍然是一个基本的技术挑战。需要开发节能的方法来在真实环境中进行技能评估。

Method: 采用两阶段框架：首先学习联合建模注视和自我中心视频来预测技能水平，然后蒸馏出一个仅使用注视数据的学生模型。推理时只需注视输入，大幅降低功耗。

Result: 在烹饪、音乐和体育三个数据集上的实验首次确立了注视在不同真实世界环境中对技能理解的重要价值。教师模型达到最先进性能，仅使用注视的学生模型在保持高精度的同时功耗降低73倍。
...
TL;DR: 该研究提出了一种语义分布引导的MRI重建框架，利用预训练的视觉语言基础模型将重建图像和辅助信息编码为高层语义特征，通过对比目标确保重建表示与目标语义分布一致，从而提升欠采样MRI重建质量。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索视觉语言基础模型能否通过提供超越传统先验的高层上下文信息来增强欠采样MRI重建效果，解决传统方法在保留解剖结构和感知质量方面的局限性。

Method: 提出语义分布引导重建框架，使用预训练视觉语言基础模型编码重建图像和辅助信息为高层语义特征，采用对比目标对齐重建表示与目标语义分布，可灵活整合多模态语义先验。

Result: 在膝部和脑部数据集上的实验表明，图像语义先验能保留精细解剖结构并获得更优感知质量（更低LPIPS值、更高Tenengrad分数、更好的读者研究评分），图像语言信息进一步扩展语义分布并实现重建属性的高层控制。

Conclusion: 研究表明视觉语言基础模型通过语义空间优化可以显著改善欠采样MRI重建，为医学图像重建提供了新的语义引导优化框架。
...
TL;DR: GIANT框架让大型多模态模型能够像病理学家一样迭代导航全切片图像，在病理图像解释任务中显著优于传统方法，接近或超过专门训练的专业模型性能。


<details>
  <summary>Details</summary>
Motivation: 通用大型多模态模型在医学图像解释特别是病理学领域表现不佳，先前研究使用低分辨率缩略图或随机图像块可能低估了模型性能。

Method: 提出GIANT框架，允许LMMs迭代导航全切片图像，并发布MultiPathQA基准测试，包含934个WSI级别问题和128个病理学家编写的问题。
...
TL;DR: 本文提出了CodeV视觉代理和TAPO训练框架，通过过程级强化学习监督工具使用行为，显著提高了视觉推理的忠实性，在保持高准确率的同时解决了工具使用不忠实的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在调用图像工具时存在不忠实推理问题——可能在无关区域调用工具或忽略工具输出但仍能猜对答案，这掩盖了模型真实推理能力。

Method: 提出CodeV代码化视觉代理和TAPO训练框架，将视觉工具表示为可执行Python代码，通过过程级RL直接对工具输入输出定义密集奖励，避免奖励黑客问题。

Result: CodeV在视觉搜索基准上达到竞争性或更优的准确率，同时显著提高了忠实工具使用率，在多模态推理和数学基准上也表现强劲。

Conclusion: 明确监督中间工具行为对于构建可信赖的代理视觉推理系统至关重要，TAPO框架有效提升了视觉推理的忠实性。
...
TL;DR: OncoVision是一个多模态AI管道，结合乳腺X光图像和临床数据，通过注意力编码器-解码器分割四个关键区域，并预测十项临床特征，采用后期融合策略提高诊断精度。


<details>
  <summary>Details</summary>
Motivation: 通过结合影像和临床数据来提高乳腺癌诊断准确性，减少观察者间变异性，为医疗资源匮乏地区提供可扩展的筛查解决方案。

Method: 使用基于注意力的编码器-解码器架构分割肿块、钙化、腋窝发现和乳腺组织四个ROI，预测十项临床特征，开发两种后期融合策略整合多模态数据。

Result: 实现了最先进的分割精度，能够稳健预测临床特征，通过后期融合提高诊断精度，降低观察者间变异性。

Conclusion: OncoVision通过准确分割与临床直觉的结合，为AI乳腺X光诊断设定了新标准，提供可扩展的公平解决方案，有助于早期发现乳腺癌和及时干预治疗。
...
### [28] [IndEgo: A Dataset of Industrial Scenarios and Collaborative Work for Egocentric Assistants](https://arxiv.org/abs/2511.19684)
*Vivek Chavan,Yasmina Imgrund,Tung Dao,Sanwantri Bai,Bosong Wang,Ze Lu,Oliver Heimann,Jörg Krüger*

Main category: cs.CV

TL;DR: IndEgo是一个多模态工业任务数据集，包含3,460个第一人称视角和1,092个第三人称视角的录制视频，专注于协作工业任务，提供丰富的注释和基准测试。


<details>
  <summary>Details</summary>
Motivation: 解决工业环境中协作任务的复杂理解需求，提供包含多模态数据和详细注释的数据集来支持工业AI应用。

Method: 收集了约197小时的第一人称视角和97小时第三人称视角的工业任务录制，包含组装/拆卸、物流组织、检查维修、木工等多种任务，特别关注两人协作场景。

Result: 数据集包含丰富的多模态数据（眼动追踪、语音、声音、动作等）和详细注释（动作、摘要、错误标注、叙述），基准测试显示对当前最先进的多模态模型具有挑战性。

Conclusion: IndEgo数据集为工业任务理解提供了有价值的资源，特别是在协作任务和错误检测方面，推动了工业AI应用的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有的层剪枝方法应用于视觉语言模型时会导致显著的性能下降，需要一种更有效的方法来剪枝冗余层而不损失性能。

Method: 分析连续三层中的局部冗余性，移除前两层中最冗余的层，微调剩余层以补偿损失的能力，并冻结第三层作为微调期间的稳定锚点。采用交错微调-冻结设计，实现剪枝后的快速收敛。

Result: 仅使用FineVision数据集的1%数据进行一个epoch的微调，在剪除25%网络层后达到88.9%的平均性能保持率，实现了最先进的性能。
...
### [30] [Maritime Small Object Detection from UAVs using Deep Learning with Altitude-Aware Dynamic Tiling](https://arxiv.org/abs/2511.19728)
*Sakib Ahmed,Oscar Pizarro*

Main category: cs.CV

TL;DR: 提出了一种基于高度感知的动态分块方法，通过根据无人机高度自适应调整图像分块大小，显著提升了小型物体检测性能，同时提高了推理速度。


<details>
  <summary>Details</summary>
Motivation: 无人机在搜救任务中面临小型物体检测困难的问题，特别是在高海拔下物体与背景像素比率较低，导致检测性能下降。

Method: 采用高度感知的动态分块方法，结合高度相关的缩放因子和自适应分块因子，在保持检测性能的同时减少不必要的计算。

Result: 在SeaDronesSee数据集上测试，使用YOLOv5和SAHI框架，小物体检测的平均精度提高了38%，推理速度比静态分块方法快两倍以上。
...
TL;DR: Prune-Then-Plan框架通过步骤级校准解决VLM在具身问答中的边界振荡问题，使用统计修剪和覆盖规划器提升导航效率和答案质量。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在具身问答中由于过度自信和校准不当导致边界振荡（不稳定的来回移动），造成导航效率低下和答案质量下降。

Method: 提出Prune-Then-Plan框架：首先使用Holm-Bonferroni启发式修剪程序剔除不可信的边界选择，然后将最终决策委托给基于覆盖率的规划器，将过度自信的预测转化为保守可解释的动作。

Result: 在3D-Mem EQA框架中集成该方法，视觉基础SPL指标相对提升49%，LLM-Match指标相对提升33%，在同等探索预算下获得更好的场景覆盖。

Conclusion: 该方法通过分离修剪和规划步骤，有效校准VLM的步骤级行为，显著提升具身问答的性能和效率。
...
TL;DR: 提出了一种基于原型的方法用于可解释的细胞计数，通过密度图估计实现。该方法在密度估计网络中集成原型层，学习细胞和背景伪影的代表性视觉模式，在保持计数效果的同时提供可解释性。


<details>
  <summary>Details</summary>
Motivation: 生物医学成像中的细胞计数对各种临床应用至关重要，但深度学习模型在该领域的可解释性仍然是一个重大挑战。需要为研究人员和临床医生提供透明可靠的工具，增加信任并加速深度学习在关键生物医学应用中的采用。

Method: 在密度估计网络中集成原型层，学习细胞和背景伪影的代表性视觉模式。通过生成突出显示输入图像中与每个原型最相似区域的解释，提供模型如何识别和计数细胞的清晰理解。

Result: 在两个公共数据集上的广泛实验表明，该方法在不影响计数效果的情况下实现了可解释性。生物学家调查确认了所识别视觉模式的相关性，进一步验证了模型的可解释性。
...
### [33] [GigaWorld-0: World Models as Data Engine to Empower Embodied AI](https://arxiv.org/abs/2511.19861)
...
TL;DR: GigaWorld-0是一个统一的世界模型框架，通过视频生成和3D生成建模的协同优化，为视觉-语言-动作学习提供高质量合成数据，使VLA模型无需真实世界交互即可在物理机器人上取得优异表现。


<details>
  <summary>Details</summary>
Motivation: 解决具身AI中高质量训练数据稀缺的问题，通过世界模型生成视觉吸引人、空间一致、物理合理且指令对齐的具身交互数据，实现数据高效的可扩展学习。

Method: 包含两个协同组件：GigaWorld-0-Video（大规模视频生成）和GigaWorld-0-3D（3D生成建模+高斯泼溅重建+物理系统识别+运动规划），通过GigaTrain框架实现高效训练。

Result: 生成高质量、多样化、可控的数据，基于该数据训练的VLA模型（如GigaBrain-0）在物理机器人上表现出强大的泛化能力和任务成功率，无需真实世界训练交互。

Conclusion: GigaWorld-0作为数据引擎，为具身AI提供了可扩展的数据合成解决方案，证明了合成数据可以替代真实交互训练，实现零真实交互的机器人学习。
...
TL;DR: RADSeg利用RADIO视觉基础模型，通过自相关递归注意力、自相关全局聚合和高效掩码细化，在零样本开放词汇语义分割中同时提升mIoU、降低延迟和减少参数数量，以更少的计算资源实现最先进性能。
...
Method: 利用RADIO视觉基础模型，采用自相关递归注意力、自相关全局聚合和计算高效的掩码细化技术来增强零样本开放词汇语义分割性能。

Result: RADSeg在基础ViT类别中mIoU提升6-30%，速度快3.95倍，参数减少2.5倍。RADSeg-base（105M）在mIoU上超越了之前850-1350M的大型视觉模型组合。
...
TL;DR: MAPS是一个用于视觉语言动作模型的微调框架，通过模块化邻近度调度来平衡稳定性和灵活性，在保持预训练先验的同时提升泛化性能。
...
Method: 提出MAPS框架，通过系统分析发现模块邻近度约束的放松顺序，线性调度这种放松，让视觉编码器保持接近预训练先验，而面向动作的语言层更自由地适应。
...
<details>
  <summary>Details</summary>
Motivation: 现有ViT加速方法主要关注算法级优化，但深层架构带来的计算开销仍未充分解决。研究能否在减少堆叠层数的同时保持表征能力。

Method: 在训练阶段引入并行分支，通过非线性组件入口点逐步合并分支，实现FFN和MHSA模块的精确数学重参数化，避免推理时的近似误差。

Result: 在ViT-Tiny上成功将12层架构压缩到6、4甚至3层，在ImageNet-1K上保持分类精度，移动CPU推理速度提升最高37%。
...
### [37] [Reasoning-VLA: A Fast and General Vision-Language-Action Reasoning Model for Autonomous Driving](https://arxiv.org/abs/2511.19912)
...
Method: 使用高斯采样从训练数据中初始化可学习动作查询，与推理增强的视觉语言特征交互，并行生成连续动作轨迹；整合8个自动驾驶数据集为标准化Chain-of-Thought推理格式；结合监督学习和强化学习微调。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶规划系统依赖手工锚点或强化学习来选择单一最佳模式，这丢弃了替代未来信息并复杂化优化过程。需要一种能处理多模态未来并保持计算效率的方法。

Method: 使用掩码动作规划(MAP)模块将未来自车运动视为掩码序列补全，通过注入噪声生成多样化轨迹查询。轻量级世界模型基于候选轨迹展开未来BEV语义，训练时使用轨迹概率作为离散路径权重计算语义损失。

Result: 在NAVSIM上，该方法与基于锚点的方法相当，并在基于世界模型的方法中达到最先进性能，同时避免强化学习并保持实时推理延迟。

Conclusion: MAP-World提供了一种有效的多模态规划框架，能够从完整可信未来分布中学习，无需手工锚点或强化学习，在保持实时性能的同时实现先进规划能力。
...
TL;DR: 本文研究了切片最优传输(min-STP)框架中优化切片器的可迁移性，证明在数据分布轻微扰动下切片器仍能保持有效，并提出了小批量min-STP以提升可扩展性。


<details>
  <summary>Details</summary>
Motivation: 切片传输方法虽然能降低最优传输的计算成本，但优化切片器是否能迁移到新分布对仍未知，这在数据演化或重复计算相关分布OT的场景中至关重要。

Method: 提出可迁移的min-STP框架，理论分析切片器在分布扰动下的稳定性，并引入小批量min-STP以提升可扩展性，提供统计精度保证。

Result: 理论证明优化切片器在数据分布轻微变化时保持接近，实验显示可迁移min-STP在点云对齐和流生成建模中实现强的一次性匹配性能和摊销训练。

Conclusion: 切片最优传输中的优化切片器具有良好的可迁移性，能够在相关任务间高效转移，为处理演化数据或重复OT计算提供了有效解决方案。
...
Method: 开发了PathFMTools工具包，用于评估CONCH和MUSK两种视觉语言基础模型在440个皮肤鳞状细胞癌H&E全切片图像上的组织学分级性能，比较了多种适应策略。

Result: 基准测试展示了不同预测方法之间的权衡，验证了使用基础模型嵌入训练小型专家模型的可行性。
...
### [41] [Material-informed Gaussian Splatting for 3D World Reconstruction in a Digital Twin](https://arxiv.org/abs/2511.20348)
*João Malheiro Silva,Andy Huynh,Tong Duy Son,Holger Caesar*

Main category: cs.CV

TL;DR: 提出仅使用相机的数字孪生3D重建方法，通过多视角图像进行3D高斯溅射重建，结合视觉模型提取语义材质掩码，并转换为带物理材质属性的网格，实现高保真传感器模拟。


<details>
  <summary>Details</summary>
Motivation: 传统LiDAR-相机融合方法需要复杂校准，且对玻璃等材料处理不佳。相机方法能自然捕捉语义和纹理，但缺乏物理材质属性。

Method: 使用多视角图像进行3D高斯溅射重建，通过视觉模型提取语义材质掩码，将高斯表示转换为带材质标签的网格表面，并分配基于物理的材质属性。

Result: 在仪器化测试车辆内部数据集上验证，利用LiDAR作为反射率真值，结合图像相似度指标，显示传感器模拟保真度与LiDAR-相机融合相当。

Conclusion: 相机专用方法结合了照片级真实重建与物理材质分配，在保持传感器模拟精度的同时消除了硬件复杂性和校准要求。
...
TL;DR: 本文提出了一种多模态原型网络方法，通过集成来自不同模态的原型来解决物种检测中的两个主要问题：模型不可解释性和基因数据收集成本高的问题。该方法能智能分配昂贵的基因数据用于精细分类，同时利用丰富的图像数据进行清晰视觉分类。


<details>
  <summary>Details</summary>
Motivation: 物种检测对于生态系统健康监测和入侵物种识别至关重要。多模态神经网络虽然被广泛使用，但存在两个主要缺陷：决策过程不可解释，以及基因数据收集成本高且需要侵入性操作。

Method: 扩展原型网络(ProtoPNets)到多模态、成本感知设置，集成来自每个模态的原型，使用相关权重确定预测对每个模态的依赖程度，并引入方法来识别不需要昂贵基因信息即可做出自信预测的情况。

Result: 该方法能够智能分配昂贵的基因数据用于精细区分，同时利用丰富的图像数据进行清晰视觉分类，达到与始终使用两种模态的模型相当的准确率。
...
### [43] [Vision--Language Enhanced Foundation Model for Semi-supervised Medical Image Segmentation](https://arxiv.org/abs/2511.19759)
...
<details>
  <summary>Details</summary>
Motivation: 结合视觉语言模型的泛化能力和少样本学习优势，解决医学图像分割中专家标注稀缺的问题，提升半监督学习效果
...
### [44] [BRIC: Bridging Kinematic Plans and Physical Control at Test Time](https://arxiv.org/abs/2511.20431)
...
Method: BRIC在测试时动态调整物理控制器以适应噪声运动计划，同时通过损失函数保留预训练技能；并引入轻量级测试时引导机制在信号空间引导扩散模型。

Result: BRIC在各种长期任务（包括运动组合、避障和人-场景交互）上实现了最先进的性能。

Conclusion: 通过结合两种自适应策略，BRIC能够以高效的方式确保跨不同环境的一致且物理合理的长期执行。
...
Conclusion: 相对角度特征能够在资源受限的硬件上实现更大批量的执行，无需对模型架构进行修改。
...
### [46] [Wanderland: Geometrically Grounded Simulation for Open-World Embodied AI](https://arxiv.org/abs/2511.20620)
*Xinhao Liu,Jiaqi Li,Youming Deng,Ruxin Chen,Yingjia Zhang,Yifei Ma,Li Guo,Yiming Li,Jing Zhang,Chen Feng*

Main category: cs.CV

TL;DR: Wanderland是一个真实到模拟的框架，用于解决具身AI中可复现闭环评估的瓶颈，通过多传感器采集、可靠重建、精确几何和鲁棒视图合成，为开放世界具身导航提供可信测试平台。


<details>
  <summary>Details</summary>
Motivation: 解决具身AI（如视觉导航）中可复现闭环评估的主要瓶颈，当前视频-3DGS方法存在较大的视觉和几何模拟到真实差距，不适合基准测试。
...
TL;DR: CrispFormer通过三个协同改进增强SegFormer解码器：边界分支监督物体轮廓、不确定性引导的细化器以及动态多尺度融合层，显著提升弱监督语义分割效果。


<details>
  <summary>Details</summary>
Motivation: 弱监督语义分割需要从噪声和欠指定的线索中学习密集掩码，现有方法在边界保持和小物体分割方面存在不足。

Method: 在SegFormer解码器中添加：(1)边界分支使用轻量级边缘头和边界感知损失；(2)不确定性引导的细化器预测像素级不确定性并加权损失；(3)动态多尺度融合层用空间softmax门控替换静态连接。
...
TL;DR: 本文提出了CRPA方法，解决了在混合分辨率去噪中使用RoPE位置编码时的注意力机制崩溃问题，通过相位对齐实现稳定的跨分辨率生成。


<details>
  <summary>Details</summary>
Motivation: 当在混合分辨率去噪中使用线性插值的RoPE位置编码时，不同空间网格的token混合会导致注意力机制崩溃，这是由于相位混叠破坏了DiT依赖的精确相位模式。

Method: 提出了Cross-Resolution Phase-Aligned Attention (CRPA)，一种无需训练的即插即用修复方法，通过修改每个注意力调用的RoPE索引映射，使所有Q/K位置在查询的步幅上表达，确保相同物理距离产生相同的相位增量。

Result: CRPA能够稳定所有注意力头和层，实现高保真度和高效的混合分辨率生成，在图像和视频生成任务上优于现有最先进方法。

Conclusion: CRPA通过相位对齐从根本上解决了RoPE在混合分辨率设置中的结构性问题，为预训练DiT提供了稳定的跨分辨率生成能力。
...
TL;DR: 提出Latent Representation Probing (LRP)方法，通过分析VLM内部表示来检测不确定性，在STVQA任务中显著提升拒绝回答的准确性。


<details>
  <summary>Details</summary>
Motivation: VLM在安全关键应用中需要具备在不确定时拒绝回答的能力，现有方法依赖不可靠的输出概率或语义一致性，无法有效处理OCR错误等场景。

Method: 训练轻量级探针分析VLM的隐藏状态或注意力模式，包括跨层表示拼接、视觉标记注意力聚合和单层探针集成投票三种设计。

Result: 在四个图像和视频基准测试中，LRP比最佳基线方法提升拒绝准确率7.6%，探针能泛化到不同不确定性来源和数据集，最优信号来自中间层而非最终层。

Conclusion: 建立了一个基于内部状态而非不可靠输出的原则性框架，用于构建可部署的AI系统，通过检测内部置信信号提高系统可靠性。
...
TL;DR: TPSO是一种无需训练、模型无关的模块，通过优化token嵌入空间来提升文本到图像扩散模型的生成多样性，避免模型重复生成强模式样本。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型存在生成多样性不足的问题，容易重复生成相似输出，限制了创造性探索和下游应用。现有方法如噪声重采样、提示重写等仍会坍缩到主导模式或引入失真。

Method: 提出Token-Prompt嵌入空间优化(TPSO)，引入可学习参数探索token嵌入空间中未被充分表示的区域，同时利用提示级空间提供全局语义约束来调节分布偏移。

Result: 在MS-COCO数据集和三个扩散骨干上的实验表明，TPSO显著提升了生成多样性，将基线性能从1.10提高到4.18分，且不牺牲图像质量。

Conclusion: TPSO有效解决了扩散模型生成多样性不足的问题，通过探索token嵌入空间的欠表示区域，在保持高质量的同时显著提升多样性。
...
TL;DR: CropVLM是一种低成本的外部方法，通过强化学习训练模型动态放大图像相关区域，提升视觉语言模型在细粒度图像理解任务上的性能，无需修改或微调原始VLM。
...
Method: 使用强化学习训练CropVLM模型，无需人工标注的边界框监督信号或昂贵的合成评估，使模型能够动态放大图像相关区域来捕捉细节。

Result: 在需要高分辨率图像理解的任务上，特别是对于目标VLM域外基准测试，该方法带来了显著性能提升。

Conclusion: CropVLM是一种通用且低成本的方法，可搭配开源和专有VLM使用，提升细粒度图像理解能力，同时避免灾难性遗忘问题。
...
TL;DR: ReDirector是一种新颖的相机控制视频重拍生成方法，通过改进RoPE在时空位置对齐上的应用，并引入Rotary Camera Encoding来捕捉多视角关系，实现了对分布外相机轨迹和视频长度的泛化。


<details>
  <summary>Details</summary>
Motivation: 解决先前工作中RoPE在输入视频和目标重拍时空位置对齐上的误用问题，提升动态对象定位和静态背景保持能力。

Method: 提出Rotary Camera Encoding (RoCE)，这是一种相机条件的RoPE相位偏移，能够捕捉并整合输入和目标视频内部及之间的多视角关系。

Result: 实验显示在相机可控性、几何一致性和视频质量方面都有显著提升，能够泛化到分布外的相机轨迹和视频长度。

Conclusion: ReDirector通过将相机条件整合到RoPE中，有效提升了视频重拍生成的质量和泛化能力。
...
TL;DR: 提出BHD-RAG框架，通过整合DCLD专业知识与多模态大语言模型，提升Birt-Hogg-Dube综合征的CT诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习在BHD诊断中面临的临床样本有限、疾病间差异小的挑战，以及MLLMs在罕见疾病诊断中因缺乏领域知识而产生幻觉风险的问题。

Method: 采用多模态检索增强生成框架，包括：专业代理生成CT图像描述构建多模态语料库、基于余弦相似度的检索器匹配相关图像-描述对、MLLM结合检索证据进行诊断。

Result: 在包含四种DCLD的数据集上验证，取得了优越的准确率，生成的基于证据的描述与专家见解高度一致。

Conclusion: BHD-RAG框架有效整合领域专业知识与MLLMs，显著提升BHD诊断准确性，为罕见疾病的多模态诊断提供了可行方案。
...
Method: 提出Rectified SpaAttn方法：1) 对关键token使用孤立池化注意力重分配，计算准确的修正因子；2) 对非关键token使用增益感知池化修正，确保修正增益始终超过引入的误差。
...
<details>
  <summary>Details</summary>
Motivation: 传统基准强调不同评估维度，缺乏对世界真实感能力的统一评估，需要系统评估世界生成模型在空间、时间、物理和指令控制方面的连贯性。

Method: 引入自适应多模态条件评估，将所有模态条件映射到统一文本空间，结合LLM-as-judge、MLLM-as-judge和传统网络方法进行综合评估。

Result: 初步人类研究表明自适应工具选择与主观人类判断更接近，为客观比较和改进提供了基础。

Conclusion: 该基准将加速从"视觉生成"到"世界生成"的转变，为下一代多模态智能系统提供评估基础。
...
TL;DR: 提出了IMIC方法，在单一嵌入空间中同时处理四个视觉任务（物体识别、高低质量人脸识别、全身图像人物识别），避免了灾难性遗忘，并在多个基础模型上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 解决视觉基础模型在微调时出现的灾难性遗忘问题，创建能同时处理多个任务的统一模型。

Method: 引入IMIC（交错多域身份课程）方法，通过梯度耦合的交错训练计划，在四个任务上同时微调基础模型骨干网络。

Result: EVA-02和CLIP模型在所有四个任务上表现与领域专家相当，在跨人脸、身体和物体数据集的多任务处理中比人类更准确，且不影响分布外泛化能力。

Conclusion: IMIC方法成功创建了统一嵌入空间，四个任务表示线性可分但特征大量共享，少数主成分即可完成所有任务而性能几乎不下降。
...
<details>
  <summary>Details</summary>
Motivation: 现有道路网络提取方法依赖折线表示，难以准确建模弯曲几何形状，而道路几何本质上是基于曲线的。同时，获取向量标注数据困难，成为基于曲线表示的瓶颈。

Method: 提出贝塞尔图表示法，将任务重构为贝塞尔图上的全局优化问题。DOGE框架包含两个交替模块：DiffAlign通过可微分渲染连续优化几何，TopoAdapt使用离散算子优化拓扑结构。

Result: 在大型SpaceNet和CityScale基准测试中达到了新的最先进水平，能够生成高保真度的道路网络向量地图。

Conclusion: 该方法为道路网络向量地图生成提供了新范式，通过直接从分割掩码学习参数化贝塞尔图，绕过了曲线标注数据的需求。
...
TL;DR: STAvatar提出了一种从单目视频重建高保真可动画3D头部化身的方法，通过UV自适应软绑定框架和时间自适应密度控制策略，解决了现有方法运动僵硬、表达能力有限以及遮挡区域处理不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯泼溅的方法通常将高斯绑定到网格三角形，仅通过线性混合蒙皮建模变形，导致运动僵硬和表达能力有限，且缺乏专门处理频繁遮挡区域（如口腔内部、眼睑）的策略。

Method: STAvatar包含两个关键组件：(1) UV自适应软绑定框架，利用图像和几何先验学习UV空间中每个高斯特征偏移，支持动态重采样；(2) 时间自适应密度控制策略，首先聚类结构相似帧，然后引入融合感知误差作为克隆准则，联合捕捉几何和纹理差异。

Result: 在四个基准数据集上的广泛实验表明，STAvatar实现了最先进的重建性能，特别是在捕捉细粒度细节和重建频繁遮挡区域方面表现优异。

Conclusion: STAvatar通过创新的UV自适应软绑定和时间自适应密度控制策略，显著提升了3D头部化身的重建质量和动画表达能力，代码将公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将时间序列转换为"伪图像"进行时间预测，但未能建立语义级对齐。探索非视觉连续序列作为条件信号进行高保真图像生成的潜力。
...
TL;DR: ChessMamba是一个用于多时相遥感影像变化检测的结构感知框架，通过状态空间建模解决异质性和时空不对齐问题，在多个变化检测任务中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 多时相遥感影像变化检测面临异质性和时空不对齐的挑战，现有方法在时间序列化过程中会破坏局部结构一致性，导致在不对齐情况下难以准确识别变化。

Method: 提出ChessMamba框架，集成SpatialMamba编码器和轻量级跨源交互模块，采用棋盘交错和蛇形扫描顺序将多时相特征序列化，通过多膨胀卷积实现结构感知融合。

Result: 在二元变化检测、语义变化检测和多模态建筑损伤评估三个任务上的综合评估表明，ChessMamba能有效融合异质特征，相比最先进方法实现显著精度提升。

Conclusion: ChessMamba通过结构感知的状态空间建模，在多时相遥感影像变化检测中实现了鲁棒的性能，为精细变化识别提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏在跨模态场景下效果不佳，因为不同模态间的表示不一致导致知识迁移困难

Method: 利用频域特征，对低频特征实施强对齐损失，对高频特征引入松弛对齐，并提出尺度一致性损失和共享分类器

Result: 在多个基准数据集上的实验表明，该方法显著优于传统KD和最先进的跨模态KD方法

Conclusion: 频率解耦方法有效解决了跨模态知识蒸馏中的表示不一致问题，实现了更好的知识迁移
...
<details>
  <summary>Details</summary>
Motivation: 现有计算机辅助诊断(CAD)数据集通常只支持单一任务，限制了CAD技术的发展，需要构建能够探索任务间相关性的多任务数据集。

Method: 基于动脉期增强CT构建包含150个病例的多任务肝脏数据集，涵盖四种肝脏疾病和正常病例，每个病例都由经验丰富的临床医生仔细标注和校准。

Result: 提供了公开的多任务数据集LiMT，包含相关基线实验结果，并回顾了现有肝脏相关任务的数据集和方法。

Conclusion: LiMT数据集可能成为医学影像研究社区未来有价值的资源，为探索任务间相关性提供了解决方案，无需担心训练过程中任务特定数据集间的异质性。
...
TL;DR: 提出了VeriSciQA数据集，包含20,351个科学视觉问答对，涵盖20个科学领域和12种图表类型，通过生成-验证框架解决现有数据质量问题
...
Method: 采用生成-验证框架：首先生成带图表文本上下文的问答对，然后通过跨模态一致性检查和辅助过滤器消除错误对

Result: VeriSciQA对开源模型构成挑战性基准，领先开源模型准确率64%，专有模型82%；在该数据集上微调的模型在SVQA基准上获得一致提升

Conclusion: 验证框架的可扩展性表明，持续的数据扩展可以进一步推进开源社区的科学视觉问答能力
...
TL;DR: Agent0-VL是一个自演进视觉语言代理，通过工具集成推理实现持续自我改进，无需人工标注或外部奖励模型，在几何问题解决和视觉科学分析任务上比基础模型提升12.5%。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言代理的学习受限于人工标注监督，纯文本自评估难以验证复杂视觉推理步骤且易产生评估幻觉。

Method: 将工具使用整合到推理、自评估和自我修复中，在单一LVLM内统一求解器和验证器两个协同角色，通过工具基础批判生成结构化反馈和细粒度自奖励。

Result: 在几何问题解决和视觉科学分析任务上，Agent0-VL比基础模型提升12.5%。

Conclusion: 通过零外部奖励演进，Agent0-VL无需人工标注或外部奖励模型即可对齐其推理和验证行为，实现持续自我改进。
...
TL;DR: 提出了一种多模态连续手语识别方法，结合3D骨骼特征和手形分类来检测手语边界，然后进行手语识别，在ASLLRP语料库上表现出显著改进。


<details>
  <summary>Details</summary>
Motivation: 提高连续手语识别的鲁棒性，通过检测手语边界和结合手形信息来改善分割和识别性能。

Method: 使用3D骨骼特征捕捉手语动态特性，预训练87种标准手形分类器，通过多模态融合模块整合视频分割和手形分类模型，在包含孤立手语和连续手语的大型数据库上训练识别模型。
...
TL;DR: MambaEye是一个新颖的因果序列视觉编码器，基于Mamba2架构，采用严格单向处理，支持任意图像分辨率和扫描模式，在ImageNet-1K分类任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决现有视觉编码器无法真正实现输入尺寸无关性的问题，这是人类视觉的基本特性。

Method: 使用因果状态空间模型作为纯Mamba2主干，采用相对移动嵌入编码空间偏移，引入扩散启发的损失函数进行密集步进监督。

Result: 在ImageNet-1K分类任务中，特别是在1536^2等高分辨率下表现出稳健性能，同时保持相对于补丁数量的线性时间和内存复杂度。

Conclusion: MambaEye实现了真正输入尺寸无关的视觉编码，在保持线性复杂度的同时，在高分辨率图像上表现出色。
...
TL;DR: Motion Marionette是一个零样本框架，用于从单目源视频到单视图目标图像的刚性运动迁移，通过内部时空先验实现无需外部约束的运动转移。
...
Conclusion: 该方法通过内部时空先验实现了无需外部约束的刚性运动迁移，在泛化性和时间一致性方面表现出色。
...
TL;DR: 提出EmoFeedback2方法，通过生成-理解-反馈的强化范式，利用大视觉语言模型为连续情感图像生成提供奖励和文本反馈，解决现有方法缺乏情感反馈和情感保真度不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有连续情感图像生成方法缺乏从生成图像中获得情感反馈，限制了情感连续性的控制，且简单的文本-情感对齐无法根据图像内容自适应调整情感提示，导致情感保真度不足。

Method: 提出生成-理解-反馈强化范式，包括：1）情感感知奖励反馈策略，用LVLM评估生成图像的情感值并计算奖励；2）自我促进文本反馈框架，LVLM迭代分析图像情感内容并自适应生成提示改进建议。
...
TL;DR: 论文揭示了一种新型威胁：单个扰动可以劫持整个决策链，使模型输出多个预定义错误结果，而不仅仅是单一错误分类。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的模型通常进行一系列决策，孤立错误容易被纠正，但级联错误可能导致严重风险。传统对抗攻击只关注操纵单一决策，无法反映真实威胁。

Method: 提出语义感知通用扰动(SAUPs)，基于输入语义诱导不同结果。开发有效算法，在归一化空间搜索扰动，采用语义分离策略解决优化挑战。

Result: 在三个多模态大语言模型上的广泛实验显示，仅使用一个对抗帧就能控制五个不同目标，达到70%的攻击成功率。

Conclusion: 多模态大语言模型对SAUPs攻击存在脆弱性，单个扰动能够劫持整个决策链，这揭示了现实世界中的严重安全威胁。
...
TL;DR: 提出了一个预测3D打印物体光化学转化的框架，通过C-PGA架构从3D视觉数据预测密集的非视觉体积物理性质，无需传统后打印测量。
...
Method: 提出耦合物理门控适应（C-PGA）架构，使用稀疏几何和工艺参数作为查询，通过特征线性调制（FiLM）动态门控和适应密集视觉特征，并行处理原始投影堆栈及其扩散-衍射校正对应物。

Result: 该方法在虚拟化学表征方面取得突破，能够精确控制化学转化状态，无需传统后打印测量。

Conclusion: C-PGA框架通过显式建模物理耦合，成功解决了从3D视觉数据预测体积物理性质的挑战，为复杂3D打印物体的化学状态预测提供了有效解决方案。
...
### [71] [Pedestrian Crossing Intention Prediction Using Multimodal Fusion Network](https://arxiv.org/abs/2511.20008)
...
Result: 在JAAD数据集上的广泛实验验证了所提出网络的有效性，相比基线方法取得了更优越的性能。

Conclusion: 该多模态融合网络通过有效提取和整合不同模态的互补线索，在行人过街意图预测任务中表现出色，为自动驾驶车辆提供了关键的环境线索。
...
TL;DR: LoTTS是首个无需训练的局部测试时缩放框架，通过在缺陷区域进行自适应重采样来提升图像质量，同时保持高质量区域不变，显著减少计算成本。
...
Method: LoTTS通过对比质量和低质量提示下的交叉和自注意力信号来定位缺陷区域，然后仅在缺陷区域进行局部扰动和去噪，确保全局一致性。

Result: 在SD2.1、SDXL和FLUX上的实验表明，LoTTS在提升局部质量和全局保真度的同时，相比Best-of-N采样将GPU成本降低了2-4倍。

Conclusion: 局部测试时缩放是扩散模型在推理时进行缩放的一个有前景的新方向。
...
### [73] [Multi-Context Fusion Transformer for Pedestrian Crossing Intention Prediction in Urban Environments](https://arxiv.org/abs/2511.20011)
...
Method: 采用渐进式融合策略：首先通过互注意力机制在各上下文内部进行特征融合，然后通过跨上下文注意力整合全局信息，最后通过引导注意力机制进一步优化上下文表示。
...
### [74] [HybriDLA: Hybrid Generation for Document Layout Analysis](https://arxiv.org/abs/2511.19919)
*Yufan Chen,Omar Moured,Ruiping Liu,Junwei Zheng,Kunyu Peng,Jiaming Zhang,Rainer Stiefelhagen*

Main category: cs.CV

TL;DR: HybriDLA是一个新颖的文档布局分析生成框架，结合扩散和自回归解码，在单层中统一处理，通过多尺度特征融合编码器提升检测质量，在DocLayNet和M$^6$Doc基准测试中达到83.5% mAP的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统文档布局分析方法依赖经验先验或固定可学习查询，在单次前向传播中执行，适用于早期文档的少量预定义区域，但难以处理现代文档中元素数量多样且布局复杂的情况。
...
Conclusion: HybriDLA通过结合扩散和自回归解码的统一框架，有效解决了现代文档布局分析的挑战，在复杂布局下实现了精确的区域预测，所有数据和模型将公开可用。
...
### [75] [WaymoQA: A Multi-View Visual Question Answering Dataset for Safety-Critical Reasoning in Autonomous Driving](https://arxiv.org/abs/2511.20022)
*Seungjun Yu,Seonho Lee,Namho Kim,Jaeyo Shin,Junsung Park,Wonjeong Ryu,Raehyuk Jung,Hyunjung Shim*

Main category: cs.CV

TL;DR: 提出安全关键推理任务，通过多视角输入解决自动驾驶中避免一个风险可能引发另一个风险的复杂场景推理问题，并引入WaymoQA数据集来提升多模态大语言模型的安全推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在自动驾驶场景理解方面表现出色，但在安全关键场景中的高级推理能力不足，特别是当避免一个交通风险可能引发另一个风险时的复杂决策问题。

Method: 将安全关键推理分为两个阶段：先解决即时风险，再缓解决策引发的下游风险；引入包含35,000个人工标注问答对的WaymoQA数据集，涵盖复杂高风险驾驶场景。

Result: 实验显示现有MLLMs在安全关键场景中表现不佳，但使用WaymoQA微调后其推理能力显著提升。
...
### [76] [Intelligent Image Search Algorithms Fusing Visual Large Models](https://arxiv.org/abs/2511.19920)
*Kehan Wang,Tingqiong Cui,Yang Zhang,Yu Chen,Shifeng Wu,Zhenzhang Li*

Main category: cs.CV

TL;DR: DetVLM是一个融合目标检测和视觉大模型的细粒度图像检索框架，通过两阶段流程实现组件状态检索和零样本搜索，在车辆组件数据集上达到94.82%的检索准确率。
...
Result: 在车辆组件数据集上，DetVLM达到94.82%的总体检索准确率，显著优于仅使用检测器的基线方法。在零样本搜索中达到94.95%的准确率，在状态搜索任务中平均准确率超过90%。

Conclusion: DetVLM成功融合了目标检测和视觉大模型的优势，实现了高效的组件状态检索和零样本搜索能力，为细粒度图像检索提供了有效的解决方案。
...
### [77] [MFM-point: Multi-scale Flow Matching for Point Cloud Generation](https://arxiv.org/abs/2511.20041)
...
Conclusion: MFM-Point框架成功提升了基于点的点云生成方法的性能，证明了多尺度流匹配在保持简单性和效率的同时能够实现高质量的生成结果。
...
### [78] [CounterVQA: Evaluating and Improving Counterfactual Reasoning in Vision-Language Models for Video Understanding](https://arxiv.org/abs/2511.19923)
*Yuefei Chen,Jiang Liu,Xiaodong Lin,Ruixiang Tang*

Main category: cs.CV

TL;DR: 提出了CounterVQA基准来评估视觉语言模型在视频反事实推理方面的能力，并开发了CFGPT方法来提升模型在这方面的表现。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在视频理解方面取得显著进展，但在反事实推理能力方面研究不足，这种能力对于鲁棒的视频理解至关重要。

Method: 引入CounterVQA基准，包含三个难度级别来系统评估反事实推理能力；开发CFGPT后训练方法，通过从语言模态中蒸馏反事实推理能力来增强模型的视觉反事实推理能力。

Result: 评估发现现有模型在简单反事实问题上表现尚可，但在复杂多跳因果链问题上性能显著下降；CFGPT方法在所有CounterVQA难度级别上都带来了持续改进。

Conclusion: 视觉语言模型在反事实推理方面存在显著能力差距，CFGPT方法能有效提升这种能力，为更鲁棒的视频理解系统提供了解决方案。
...
TL;DR: 将概念瓶颈模型(CBMs)扩展到视觉异常检测(VAD)领域，提出CONVAD方法，通过概念学习提供人类可解释的异常描述，同时生成概念和视觉解释。


<details>
  <summary>Details</summary>
Motivation: 现有VAD模型虽然能提供视觉异常区域定位，但缺乏直接且语义上有意义的解释，限制了用户对异常的理解和系统信任度。

Method: 开发概念数据集支持CBM研究；改进CBM架构同时生成概念和视觉解释；引入人工异常合成管道，减少对稀有异常样本的依赖。

Result: CONVAD在性能上与经典VAD方法相当，但提供了更丰富的概念驱动解释，增强了VAD系统的可解释性和可信度。

Conclusion: 概念瓶颈模型能有效提升视觉异常检测的可解释性，通过概念学习提供语义丰富的异常描述，是VAD领域的重要进展。
...
### [80] [Context-Aware Token Pruning and Discriminative Selective Attention for Transformer Tracking](https://arxiv.org/abs/2511.19928)
*Janani Kugarajeevan,Thanikasalam Kokul,Amirthalingam Ramanan,Subha Fernando*

Main category: cs.CV

TL;DR: CPDATrack是一种新型跟踪框架，通过概率驱动的token剪枝和选择性注意力机制来抑制背景和干扰物干扰，同时保持目标上下文信息，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 传统单流Transformer跟踪器中，过多背景搜索token关注目标模板token会削弱跟踪器的判别能力。现有token剪枝方法往往会剪掉目标附近的token，导致丢失关键上下文信息。此外，搜索token中的干扰物进一步降低了准确识别目标的能力。

Method: 1. 在指定编码器层之间集成可学习模块来估计每个搜索token与目标关联的概率；2. 基于概率估计剪除信息量少的背景token，同时保留目标周围的上下文线索；3. 在早期层使用判别性选择性注意力机制完全阻断搜索到模板的注意力；4. 在后续编码器层中，从局部区域选择性提取高概率目标token来关注模板token。
...
TL;DR: LungEvaty是一个基于Transformer的框架，用于从单次低剂量CT扫描预测1-6年肺癌风险，无需像素级注释，在9万多CT扫描上训练，性能达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 随着更多国家引入基于低剂量CT的肺癌筛查项目，需要可扩展的方法来高效处理整个肺部体积，充分利用大规模筛查数据集的潜力。现有方法要么过度依赖像素级注释，要么将肺部分析为片段，限制了性能和可扩展性。

Method: 提出LungEvaty框架，基于Transformer处理整个肺部输入，直接从大规模筛查数据中学习，捕捉与恶性肿瘤风险相关的全面解剖和病理线索。采用可选解剖感知注意力引导损失来鼓励解剖聚焦的注意力。

Result: 仅使用影像数据且无区域监督的情况下，LungEvaty达到最先进性能，在超过9万CT扫描（包括2.8万微调和6千评估）上训练。
...
### [82] [Image Diffusion Models Exhibit Emergent Temporal Propagation in Videos](https://arxiv.org/abs/2511.19936)
*Youngseo Kim,Dohyun Kim,Geonhee Han,Paul Hongsuck Seo*

Main category: cs.CV

TL;DR: 本文提出DRIFT框架，利用预训练图像扩散模型的自注意力机制实现零样本视频目标跟踪，通过重新解释注意力图为语义标签传播核，并结合SAM引导的掩码优化，在标准视频目标分割基准上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 图像扩散模型在生成过程中隐含地捕获了丰富的语义结构，这些结构可以用于识别和定位任务，而不仅仅是合成。本文旨在探索如何利用扩散模型的自注意力机制来实现零样本目标跟踪。

Method: 将扩散模型的自注意力图重新解释为语义标签传播核，建立相关图像区域之间的像素级对应关系。跨帧扩展形成时间传播核，实现视频中的零样本目标跟踪。采用测试时优化策略（DDIM反转、文本反转和自适应头权重）来适应扩散特征，并引入DRIFT框架结合SAM引导的掩码优化。

Result: 在标准视频目标分割基准上实现了最先进的零样本性能，证明了扩散模型注意力机制在目标跟踪任务中的有效性。

Conclusion: 扩散模型的自注意力机制可以作为强大的语义标签传播工具，通过适当的优化策略和框架设计，能够实现高效的零样本视频目标跟踪，为扩散模型在识别任务中的应用开辟了新途径。
...
### [83] [While recognizing actions, LMMs struggle to detect core interaction events](https://arxiv.org/abs/2511.20162)
*Daniel Harari,Michael Sidorov,Liel David,Chen Shterental,Abrham Kahsay Gebreselasie,Muhammad Haris Khan*

Main category: cs.CV

TL;DR: 论文研究了大型多模态模型在视频交互事件定位中的表现，发现虽然模型能准确描述物体和动作，但无法精确定位交互开始或结束的时刻和位置。
...
Method: 创建了首个大规模数据集，包含2万多个标注的交互事件，让250名人工标注者标记接触和分离的关键时刻，然后测试两个LMM模型在短视频中定位这些事件的能力。

Result: 模型能可靠地命名目标物体、识别动作并提供连贯推理，但持续无法识别交互开始或结束的帧，也无法在场景中定位事件。

Conclusion: 模型缺乏感知基础，无法精确定义交互的物理接触时刻和位置，表明其对动态场景的深层理解存在局限。
...
### [84] [Low-Resolution Editing is All You Need for High-Resolution Editing](https://arxiv.org/abs/2511.19945)
*Junsung Lee,Hyunsoo Lee,Yong Jae Lee,Bohyung Han*

Main category: cs.CV

TL;DR: 提出了一种用于高分辨率图像编辑的测试时优化框架，通过分块优化、细节传递和同步策略实现高质量编辑


<details>
  <summary>Details</summary>
Motivation: 现有方法仅限于低分辨率设置（通常只支持1K分辨率），而高分辨率内容创建是视觉和图形社区的核心挑战，需要有效的可控高分辨率图像编辑机制

Method: 采用分块优化方法处理高分辨率源图像，包含细粒度细节传递模块和新的同步策略来保持块间一致性

Result: 大量实验表明该方法能产生高质量编辑结果

Conclusion: 该方法为高分辨率内容创建铺平了道路
...
### [85] [OmniAlpha: A Sequence-to-Sequence Framework for Unified Multi-Task RGBA Generation](https://arxiv.org/abs/2511.20211)
*Hao Yu,Jiabo Zhan,Zile Wang,Jinglin Wang,Huaisong Zhang,Hongyu Li,Xinrui Chen,Yongxian Wei,Chun Yuan*

Main category: cs.CV

TL;DR: OmniAlpha是首个统一的序列到序列RGBA图像生成和编辑多任务框架，通过新颖的MSRoPE-BiL方法和AlphaLayers数据集，在21个多样化任务上超越了专门的基线模型。
...
Result: 在21个多样化任务上联合训练，统一方法持续优于强大的专门基线。在AIM-500上实现无遮罩抠图的SAD相对减少84.8%，在层条件完成任务中赢得超过90%的人类偏好。
...
### [86] [Supervise Less, See More: Training-free Nuclear Instance Segmentation with Prototype-Guided Prompting](https://arxiv.org/abs/2511.19953)
*Wen Zhang,Qin Ren,Wenjing Liu,Haibin Ling,Chenyu You*

Main category: cs.CV

TL;DR: SPROUT是一个完全无需训练和标注的核实例分割框架，利用组织学先验构建特定切片参考原型，通过部分最优传输方案进行特征对齐，将特征转换为点提示使SAM模型无需参数更新即可实现精确核分割。
...
Method: 利用组织学先验构建切片特定参考原型来缓解领域差距，通过部分最优传输方案逐步指导特征对齐，将前景和背景特征转换为正负点提示，使SAM模型无需参数更新即可生成精确核分割。

Result: 在多个组织病理学基准测试中的广泛实验表明，SPROUT在无需监督或重新训练的情况下实现了有竞争力的性能。

Conclusion: SPROUT为病理学中可扩展的训练免费核实例分割建立了一个新的范式。
...
### [87] [Uplifting Table Tennis: A Robust, Real-World Application for 3D Trajectory and Spin Estimation](https://arxiv.org/abs/2511.20250)
...
Method: 两阶段流水线：前端使用新创建的TTHQ数据集进行2D监督训练，后端提升网络仅在物理正确的合成数据上训练，并重新设计以抵抗真实世界常见伪影。

Result: 通过整合球检测器和桌面关键点检测器，将概念验证的提升方法转化为实用、鲁棒且高性能的端到端应用。

Conclusion: 该方法成功实现了对乒乓球3D轨迹和旋转的稳健分析，克服了真实世界视频中的检测噪声和帧率变化问题。
...
### [88] [GFT-GCN: Privacy-Preserving 3D Face Mesh Recognition with Spectral Diffusion](https://arxiv.org/abs/2511.19958)
*Hichem Felouat,Hanrui Wang,Isao Echizen*

Main category: cs.CV

TL;DR: 提出GFT-GCN隐私保护3D人脸识别框架，结合谱图学习和扩散模板保护，实现高精度识别和强隐私保护


<details>
  <summary>Details</summary>
Motivation: 3D人脸识别具有抗光照、姿态变化和呈现攻击的鲁棒性，但需要保护存储的生物特征模板，特别是在高安全应用中

Method: 集成图傅里叶变换和图卷积网络从3D人脸网格提取紧凑谱特征，引入谱扩散机制生成不可逆、可更新、不可链接的模板，采用轻量级客户端-服务器架构

Result: 在BU-3DFE和FaceScape数据集上展示高识别精度和强重构攻击抵抗力

Conclusion: GFT-GCN有效平衡隐私和性能，为安全3D人脸认证提供实用解决方案
...
### [89] [XiCAD: Camera Activation Detection in the Da Vinci Xi User Interface](https://arxiv.org/abs/2511.20254)
*Alexander C. Jenke,Gregor Just,Claas de Boer,Martin Wagner,Sebastian Bodenstedt,Stefanie Speidel*

Main category: cs.CV

TL;DR: 开发基于ResNet18的轻量级管道，自动检测达芬奇Xi系统中摄像头图块的位置和激活状态，为手术数据分析提供摄像头运动元数据


<details>
  <summary>Details</summary>
Motivation: 机器人辅助微创手术仅依赖内窥镜视频作为术中视觉反馈，检测摄像头激活状态可提供有价值的元数据，支持工具追踪、技能评估和摄像头控制自动化等下游任务

Method: 使用ResNet18卷积神经网络构建轻量级管道，在SurgToolLoc数据集的手动标注数据上进行微调，并在包含7万多帧的三个公共数据集上评估

Result: 模型在检测激活摄像头方面F1分数达到0.993-1.000，正确定位摄像头图块且无多摄像头误检

Conclusion: 该管道能够从手术视频中可靠、实时提取摄像头激活元数据，为各种下游应用的自动化预处理和分析提供便利
...
### [90] [Prompting Lipschitz-constrained network for multiple-in-one sparse-view CT reconstruction](https://arxiv.org/abs/2511.20296)
*Baoshun Shi,Ke Jiang,Qiusheng Lian,Xinran Yu,Huazhu Fu*

Main category: cs.CV

TL;DR: 提出PromptCT框架，通过LipNet网络满足Lipschitz约束并嵌入显式提示模块，实现了多视图稀疏CT重建的单一模型解决方案，降低了存储成本并确保算法收敛。


<details>
  <summary>Details</summary>
Motivation: 解决稀疏视图CT重建中两个主要问题：深度展开算法的先验网络难以明确证明满足Lipschitz约束，以及多视图设置下需要训练多个模型导致存储成本过高。

Method: 开发LipNet网络确保Lipschitz连续性，集成显式提示模块提供不同稀疏采样设置的判别知识，构建PromptCT深度展开框架实现多合一SVCT重建。

Result: 在模拟和真实数据实验中，PromptCT在多项指标上优于基准重建算法，实现了更高质量的重建结果和更低的存储成本。

Conclusion: 提出的PromptCT框架有效解决了稀疏视图CT重建中的理论约束和实际存储问题，为临床应用提供了可行的解决方案。
...
### [91] [HiCoGen: Hierarchical Compositional Text-to-Image Generation in Diffusion Models via Reinforcement Learning](https://arxiv.org/abs/2511.19965)
...
### [92] [3D Motion Perception of Binocular Vision Target with PID-CNN](https://arxiv.org/abs/2511.20332)
...
<details>
  <summary>Details</summary>
Motivation: 从PID角度理解神经网络拟合非线性问题的能力，将单层神经网络视为使用二阶差分方程和非线性来描述局部问题，多层网络通过多个这样的组合逐步将原始表示转换为所需表示。

Method: 设计了一个相对较小的PID卷积神经网络，共17层413千参数，通过连接和池化实现了简单实用的特征复用方法，使用模拟随机移动球数据集进行训练和测试。

Result: 实验结果显示预测精度接近输入图像分辨率所能表示的上限，分析了实验结果、误差以及现有不足和改进方向。
...
### [93] [VGGT4D: Mining Motion Cues in Visual Geometry Transformers for 4D Scene Reconstruction](https://arxiv.org/abs/2511.19971)
*Yu Hu,Chong Cheng,Sicheng Yu,Xiaoyang Guo,Hao Wang*

Main category: cs.CV

TL;DR: VGGT4D是一个无需训练的框架，将3D基础模型VGGT扩展到4D场景重建，通过挖掘VGGT全局注意力层中的动态线索，生成精确的静态-动态分离掩码，有效解决动态物体干扰问题。


<details>
  <summary>Details</summary>
Motivation: 现有4D方法依赖外部先验、繁重的后优化或需要在4D数据集上微调，而VGGT等3D基础模型在动态物体主导时性能显著下降，需要一种无需训练的方法来扩展3D模型到4D场景重建。

Method: 通过挖掘VGGT全局注意力层中的层间动态线索，使用Gram相似性放大全局动态线索并跨时间窗口聚合，引入基于投影梯度的细化策略锐化掩码边界，将精确掩码集成到VGGT早期推理中。

Result: 在六个数据集上，该方法在动态物体分割、相机姿态估计和密集重建方面均取得优越性能，支持单次推理超过500帧的长序列。

Conclusion: VGGT4D证明了3D基础模型的全局注意力层已隐含丰富的动态线索，通过挖掘这些线索可以实现无需训练的鲁棒4D场景重建。
...
### [94] [From Passive Perception to Active Memory: A Weakly Supervised Image Manipulation Localization Framework Driven by Coarse-Grained Annotations](https://arxiv.org/abs/2511.20359)
*Zhiqing Guo,Dongdong Xi,Songlin Li,Gaobo Yang*

Main category: cs.CV

TL;DR: BoxPromptIML是一个弱监督图像篡改定位框架，通过粗粒度区域标注策略和知识蒸馏，在降低标注成本的同时实现精细定位，性能媲美全监督方法。


<details>
  <summary>Details</summary>
Motivation: 解决图像篡改定位中标注成本与定位精度之间的权衡问题。全监督方法依赖密集像素级标注成本高，而弱监督方法基于图像级标签缺乏空间定位精度。

Method: 提出粗粒度区域标注策略降低标注成本；设计轻量级学生模型通过SAM教师模型进行知识蒸馏；基于人类潜意识记忆机制设计双引导特征融合模块，实现动态知识回忆。
...
### [95] [Boosting Reasoning in Large Multimodal Models via Activation Replay](https://arxiv.org/abs/2511.19972)
*Yun Xing,Xiaobin Hu,Qingdong He,Jiangning Zhang,Shuicheng Yan,Shijian Lu,Yu-Gang Jiang*

Main category: cs.CV

TL;DR: 本文研究了RLVR（可验证奖励的强化学习）对大型多模态模型推理能力的影响机制，发现RLVR会意外地改变低熵激活，并提出了一种无需训练的新方法——激活重放，通过重放基础模型的低熵激活来提升RLVR模型的推理性能。


<details>
  <summary>Details</summary>
Motivation: 理解RLVR后训练范式背后的机制，特别是它如何影响模型激活模式，从而提升多模态推理能力。

Method: 提出激活重放方法：在测试时通过操纵视觉token，重放基础模型的低熵激活来调节RLVR模型的对应激活，无需昂贵的策略优化。

Result: 激活重放显著提升了多种场景下的推理能力，包括数学推理、视觉代理和视频推理，同时提高了Pass@K指标并缓解了RLVR推理覆盖范围较窄的问题。

Conclusion: 激活重放是一种简单有效的训练免费方法，通过调节低熵激活能够显著提升RLVR后训练模型的多模态推理性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决多目标跟踪在计算资源受限条件下性能下降的问题，特别是低频检测场景下的跟踪质量稳定问题。

Method: 引入两阶段匹配策略，提出Bbox-Based距离替代传统马氏距离，将视觉跟踪集成到卡尔曼滤波和整体跟踪流程中。

Result: 在MOT17-val数据集1Hz检测频率下HOTA提升11.6%，在标准MOT17、MOT20和DanceTrack基准测试中与最佳方法保持相当性能。

Conclusion: StableTrack方法有效提升了低频检测条件下的跟踪稳定性，同时在全频检测场景下保持竞争力。
...
### [97] [Block Cascading: Training Free Acceleration of Block-Causal Video Models](https://arxiv.org/abs/2511.20426)
*Hmrishav Bandyopadhyay,Nikhil Pinnaparaju,Rahim Entezari,Jim Scott,Yi-Zhe Song,Varun Jampani*

Main category: cs.CV

TL;DR: Block Cascading通过训练无关的并行化方法，显著缓解了块因果视频生成中的速度-质量权衡，在5个GPU上实现约2倍加速，且不损失生成质量。


<details>
  <summary>Details</summary>
Motivation: 解决块因果视频生成中速度与质量的严重权衡问题：小模型(1.3B)只能达到16FPS，大模型(14B)仅4.5FPS，用户必须在响应性和质量之间做出选择。

Method: 利用关键洞察：未来视频块不需要完全去噪的当前块即可开始生成。通过使用部分去噪的上下文启动块生成，将顺序流水线转换为并行级联，多个块同时去噪。

Result: 在5个GPU上实现约2倍加速：1.3B模型从16FPS加速到30FPS，14B模型从4.5FPS加速到12.5FPS。同时消除了KV重缓存带来的约200ms开销。

Conclusion: Block Cascading在保持生成质量的同时，显著提高了块因果视频生成的推理速度，解决了速度-质量权衡问题。
...
### [98] [SONIC: Spectral Optimization of Noise for Inpainting with Consistency](https://arxiv.org/abs/2511.19985)
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于引导的方法理论上可用于图像修复等逆问题，但实际效果有限，导致需要专门的修复模型。本文旨在开发无需训练即可使用现成文生图模型进行图像修复的方法。

Method: 优化初始种子噪声以近似匹配未掩码区域数据，采用线性近似避免计算成本高昂的反向展开，并在频域进行优化以提高稳定性。
...
### [99] [Object-Centric Vision Token Pruning for Vision Language Models](https://arxiv.org/abs/2511.20439)
*Guangyuan Li,Rongzhen Zhao,Jinhong Deng,Yanbo Wang,Joni Pajarinen*

Main category: cs.CV

TL;DR: OC-VTP是一种直接且保证性的方法，通过选择最具代表性的视觉token来提高VLM推理效率，同时保持准确性。该方法仅需轻量级预训练一个小型对象中心视觉token剪枝器，无需在任何数据集上微调任何模型。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型中的视觉token数量多但信息分散，消耗过多不必要的计算。现有方法采用间接且无保证的方式来剪枝冗余视觉token，需要更直接有效的方法。

Method: 提出OC-VTP方法，通过最小化从选定token重建原始未剪枝token的误差，保证保留最具代表性的视觉token。仅需预训练一个小型对象中心视觉token剪枝器，可插入现有VLM中。

Result: 在各种视觉剪枝比例下，OC-VTP始终帮助主流VLM保持最高的推理准确率，同时展示了有趣的可解释性。

Conclusion: OC-VTP提供了一种直接且保证性的视觉token剪枝方法，显著提高VLM推理效率而不损失准确性，具有很好的通用性和可解释性。
...
### [100] [GazeProphetV2: Head-Movement-Based Gaze Prediction Enabling Efficient Foveated Rendering on Mobile VR](https://arxiv.org/abs/2511.19988)
*Farhaan Ebadulla,Chiraag Mudlpaur,Shreya Chaurasia,Gaurav BV*

Main category: cs.CV

TL;DR: 提出了一种结合时间注视模式、头部运动数据和视觉场景信息的多模态VR注视预测方法，通过门控融合机制和跨模态注意力自适应加权不同模态，在22个VR场景的530万注视样本数据集上验证了优于单模态方法的预测准确性。


<details>
  <summary>Details</summary>
Motivation: VR环境中的注视行为预测对于渲染优化和界面设计具有重要意义，但现有方法面临挑战。该研究旨在通过多模态融合提高预测准确性，为更高效的VR系统提供支持。

Method: 采用多模态方法，结合时间注视模式、头部运动数据和视觉场景信息，使用门控融合机制和跨模态注意力来自适应加权不同模态的贡献。

Result: 在22个VR场景的530万注视样本数据集上，多模态方法相比单模态方法在1-3个未来帧的预测准确性有显著提升，跨场景泛化测试显示93.1%的验证准确率和时间一致的预测轨迹。

Conclusion: 该研究证明了结合过去注视轨迹、头部方向和场景内容能有效提升VR注视预测准确性，为渲染优化、交互设计和用户体验评估提供了潜在应用，是实现无需昂贵眼动追踪硬件的高效VR系统的重要一步。
...
### [101] [DesignPref: Capturing Personal Preferences in Visual Design Generation](https://arxiv.org/abs/2511.20513)
*Yi-Hao Peng,Jeffrey P. Bigham,Jason Wu*

Main category: cs.CV

TL;DR: 本文介绍了DesignPref数据集，包含12k个UI设计对比标注，发现设计师之间存在显著偏好分歧，并探索了基于个性化策略的设计偏好预测方法。


<details>
  <summary>Details</summary>
Motivation: 由于视觉设计具有主观性和高度个性化特征，传统基于多数投票的生成模型评估方法无法准确反映个体偏好，需要研究个性化设计评估方法。

Method: 构建包含12k对UI设计对比的DesignPref数据集，由20位专业设计师标注多级偏好评分；研究多种个性化策略，包括微调和将设计师特定标注整合到RAG流程中。

Result: 设计师之间存在显著分歧（Krippendorff's alpha = 0.25）；个性化模型在使用少20倍样本的情况下，仍能持续优于聚合基线模型来预测个体设计师偏好。

Conclusion: 该研究为个性化视觉设计评估提供了首个数据集，支持未来对个体设计品味的建模研究，证明个性化方法在预测设计偏好方面的有效性。
...
### [102] [OmniRefiner: Reinforcement-Guided Local Diffusion Refinement](https://arxiv.org/abs/2511.19990)
*Yaoli Liu,Ziheng Ouyang,Shengtao Lou,Yiren Song*

Main category: cs.CV

TL;DR: 提出了一种细节感知的参考图像引导生成框架，通过两阶段参考驱动修正来增强像素级一致性，解决现有扩散模型在保留细粒度视觉细节方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前基于VAE的潜在压缩方法会丢弃细微纹理信息，导致身份和属性特定线索消失；现有的后编辑方法在放大局部细节时往往与原始图像在光照、纹理或形状上不一致。

Method: 采用两阶段参考驱动修正：首先微调单图像扩散编辑器，使其同时接收草稿图像和参考图像进行全局一致性细化；然后应用强化学习进一步强化局部编辑能力，明确优化细节准确性和语义一致性。

Result: 在具有挑战性的参考引导恢复基准测试中，该方法显著提高了参考对齐和细粒度细节保留能力，生成忠实且视觉连贯的编辑结果，超越了开源和商业模型。

Conclusion: 该方法通过细节感知的细化框架有效解决了参考图像引导生成中的细节保留问题，为高质量的图像编辑提供了有效解决方案。
...
### [103] [Automated Monitoring of Cultural Heritage Artifacts Using Semantic Segmentation](https://arxiv.org/abs/2511.20541)
*Andrea Ranieri,Giorgio Palmieri,Silvia Biasotti*

Main category: cs.CV

TL;DR: 比较不同CNN编码器的U-Net架构在文化遗产裂缝检测中的语义分割性能，评估模型在未见过的雕像和纪念碑上的泛化能力。
...
Method: 使用不同CNN编码器的U-Net架构进行语义分割，在OmniCrack30k数据集上进行定量评估（mIoU、Dice系数、Jaccard指数），并在真实世界未标记的裂缝雕像和纪念碑上进行定性评估。

Result: 模型展现出对未见过的文化遗产环境的良好泛化能力，尽管从未在雕像或纪念碑图像上明确训练过。

Conclusion: 研究为不同CNN编码器在细粒度裂缝分割中的能力提供了有价值的见解，证明了其在文化遗产保护中的潜力。
...
### [104] [CREward: A Type-Specific Creativity Reward Model](https://arxiv.org/abs/2511.19995)
...
Method: 首先进行人类基准评估来捕捉人类对各类创意图像的感知，分析人类判断与大视觉语言模型预测的相关性，然后收集LVLM生成的标签来训练CREward模型。

Result: 确认LVLM与人类感知有很强的对齐性，成功训练出适用于创意图像评估和生成的CREward模型。

Conclusion: CREward模型在创造力评估、可解释创造力和创意样本获取三个应用方面具有实用价值，可用于人类设计灵感和指导创意生成。
...
### [105] [New York Smells: A Large Multimodal Dataset for Olfaction](https://arxiv.org/abs/2511.20544)
...
TL;DR: 本文介绍了纽约气味数据集，这是一个包含7000个气味-图像对的大规模多模态数据集，用于解决机器嗅觉感知的瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 动物依赖嗅觉感知世界，但这一丰富的化学感官模式在机器中仍难以实现，主要瓶颈是缺乏在自然环境中收集的多样化多模态嗅觉训练数据。

Method: 构建了包含7000个气味-图像对的数据集，涵盖室内外环境的3500个不同物体，比现有嗅觉数据集多约70倍。设计了三个基准任务：跨模态气味-图像检索、仅凭气味识别场景/物体/材料、草种细粒度区分。

Result: 实验表明视觉数据能够实现跨模态嗅觉表征学习，学习到的嗅觉表征优于广泛使用的手工特征。

Conclusion: 纽约气味数据集为机器嗅觉研究提供了重要的数据基础，证明了跨模态学习在嗅觉感知中的有效性。
...
### [106] [Flash-DMD: Towards High-Fidelity Few-Step Image Generation with Efficient Distillation and Joint Reinforcement Learning](https://arxiv.org/abs/2511.20549)
*Guanjie Chen,Shirui Huang,Kai Liu,Jianchen Zhu,Xiaoye Qu,Peng Chen,Yu Cheng,Yifu Sun*

Main category: cs.CV

TL;DR: Flash-DMD是一个新颖的框架，通过时间步蒸馏和联合RL优化来加速扩散模型生成，显著降低训练成本并提升稳定性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型生成过程计算昂贵，现有时间步蒸馏方法需要大量训练且导致图像质量下降，RL微调过程不稳定且容易陷入奖励黑客问题。

Method: 提出高效的时间步感知蒸馏策略减少训练成本，并引入联合训练方案，在蒸馏训练的同时进行RL微调，利用蒸馏损失作为正则化器稳定RL训练。

Result: 仅用DMD2训练成本的2.1%就超越其性能，在少步采样机制下实现最先进的生成质量，在视觉质量、人类偏好和文本图像对齐指标上优于现有方法。

Conclusion: Flash-DMD为训练高效、高保真和稳定的生成模型提供了有效范式，显著加速收敛并提升生成质量。
...
### [107] [Evaluating the Performance of Deep Learning Models in Whole-body Dynamic 3D Posture Prediction During Load-reaching Activities](https://arxiv.org/abs/2511.20615)
*Seyede Niloofar Hosseini,Ali Mojibi,Mahdi Mohseni,Navid Arjmand,Alireza Taheri*

Main category: cs.CV

TL;DR: 本研究探索了使用双向长短期记忆网络和Transformer架构来预测动态负重活动中的全身人体姿态，提出了通过优化新成本函数来强制保持身体段长度不变的新方法，显著提高了预测精度。


<details>
  <summary>Details</summary>
Motivation: 探索深度神经网络在动态负重活动中全身姿态预测的应用，为理解手动物料搬运活动中的运动动力学提供新方法。

Method: 使用BLSTM和Transformer两种时间序列模型，输入包括手部负载位置、举重技术、身体特征和前25%任务时长的姿态数据，预测剩余75%任务时长的身体坐标。提出通过优化新成本函数来强制保持身体段长度不变的方法。

Result: 新成本函数使手臂和腿部模型的预测误差分别降低约8%和21%。Transformer架构的均方根误差为47.0毫米，比BLSTM模型的长时性能准确约58%。

Conclusion: 研究表明利用捕捉时间序列依赖性的神经网络在3D运动帧中具有优势，为理解和预测手动物料搬运活动中的运动动力学提供了独特方法。
...
### [108] [ACIT: Attention-Guided Cross-Modal Interaction Transformer for Pedestrian Crossing Intention Prediction](https://arxiv.org/abs/2511.20020)
*Yuanzhe Li,Steffen Müller*

Main category: cs.CV

TL;DR: 提出了一种注意力引导的跨模态交互Transformer（ACIT）用于行人过街意图预测，通过六种视觉和运动模态的分组交互，结合双路径注意力机制和Transformer时序建模，在JAAD数据集上取得了优于现有方法的表现。


<details>
  <summary>Details</summary>
Motivation: 有效提取和整合不同类型数据的互补线索是行人过街意图预测的主要挑战，需要解决多模态数据的深度交互问题。

Method: ACIT将六种模态分为三组交互对：全局语义图和全局光流、局部RGB图像和局部光流、自车速度和行人边界框。采用双路径注意力机制增强主模态显著区域，并通过光流引导注意力与辅助模态深度交互。使用跨模态注意力建模运动特征，多模态特征融合模块促进跨模态交互，Transformer模块捕获时序依赖。

Result: 在JAADbeh和JAADall数据集上分别达到70%和89%的准确率，优于现有最先进方法。消融研究验证了各模块的贡献。

Conclusion: ACIT通过有效的跨模态交互和时序建模，显著提升了行人过街意图预测性能，证明了多模态深度交互的重要性。
...
### [109] [MapReduce LoRA: Advancing the Pareto Front in Multi-Preference Optimization for Generative Models](https://arxiv.org/abs/2511.20629)
...
TL;DR: 提出了MapReduce LoRA和Reward-aware Token Embedding (RaTE)两种方法，解决多奖励联合优化中的对齐税问题，在文本到图像、文本到视频和语言任务中均取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统RLHF方法在联合优化多个奖励时会产生对齐税，即改善一个维度的同时会降低其他维度，需要解决这一多奖励对齐的挑战。

Method: MapReduce LoRA并行训练偏好特定的LoRA专家并迭代合并；RaTE学习奖励特定的token嵌入，在推理时组合以实现灵活的偏好控制。
...
Conclusion: 该框架在多模态多偏好对齐方面达到了新的最先进水平，为多奖励优化提供了有效的解决方案。
...
### [110] [MotionV2V: Editing Motion in a Video](https://arxiv.org/abs/2511.20640)
...
<details>
  <summary>Details</summary>
Motivation: 虽然生成式视频模型在保真度和一致性方面取得了显著进展，但将这些能力应用于视频编辑仍然是一个复杂挑战。现有研究主要关注运动可控性来增强文本到视频生成或图像动画，而精确的运动控制作为编辑现有视频的有前景但尚未充分探索的范式。

Method: 提出了一个生成"运动反事实"视频对的流程，这些视频对具有相同内容但不同运动，并在该数据集上微调了运动条件视频扩散架构。通过直接编辑从输入中提取的稀疏轨迹来实现视频运动修改。

Result: 在四向头对头用户研究中，该模型相对于先前工作获得了超过65%的偏好率。该方法允许编辑从任何时间戳开始并自然传播。

Conclusion: 运动编辑表示与生成式主干网络结合，能够实现强大的视频编辑能力，特别是在精确控制视频运动方面表现出色。
...
### [111] [SAM-MI: A Mask-Injected Framework for Enhancing Open-Vocabulary Semantic Segmentation with SAM](https://arxiv.org/abs/2511.20027)
...
### [112] [MedROV: Towards Real-Time Open-Vocabulary Detection Across Diverse Medical Imaging Modalities](https://arxiv.org/abs/2511.20650)
...
<details>
  <summary>Details</summary>
Motivation: 传统医学影像目标检测模型采用闭集范式，无法检测新型标签目标。开放词汇目标检测在医学影像中研究不足，主要受限于数据集稀缺和文本-图像对齐弱的问题。

Method: 构建大规模数据集Omnis（60万检测样本，覆盖9种成像模态）；提出伪标签策略处理多源数据集的缺失标注；利用预训练基础模型增强泛化能力；采用对比学习和跨模态表示。

Result: MedROV在医学图像检测中平均绝对提升40 mAP50，超越闭集检测器3+ mAP50，运行速度达70 FPS，创下医学检测新基准。

Conclusion: MedROV成功解决了医学影像中开放词汇检测的挑战，通过大规模数据集和先进学习方法，实现了对已知和未知结构的高效检测，为医学影像分析开辟了新方向。
...
### [113] [Tell Model Where to Look: Mitigating Hallucinations in MLLMs by Vision-Guided Attention](https://arxiv.org/abs/2511.20032)
*Jianfei Zhao,Feng Zhang,Xin Sun,Chong Feng,Zhixing Tan*

Main category: cs.CV

TL;DR: 提出了Vision-Guided Attention (VGA)，一种无需训练的方法，通过利用视觉token的语义内容构建精确的视觉基础，引导模型关注相关视觉区域，有效减少MLLMs的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: MLLMs虽然能准确提取视觉语义，但在后续推理中未能充分利用这一优势，导致有限的定位能力和幻觉问题。

Method: VGA首先构建精确的视觉基础，然后使用该基础引导模型关注相关视觉区域。在图像描述任务中，VGA在生成过程中动态细化指导，抑制已描述区域。

Result: VGA在多个幻觉基准测试中实现了最先进的去幻觉性能，仅引入4.36%的延迟开销，且与FlashAttention等高效注意力实现完全兼容。
...
### [114] [Clair Obscur: an Illumination-Aware Method for Real-World Image Vectorization](https://arxiv.org/abs/2511.20034)
*Xingyue Lin,Shuai Peng,Xiangyu Xie,Jianhua Zhu,Yuxuan Zhou,Liangcai Gao*

Main category: cs.CV

TL;DR: COVec是一个基于明暗对比原理的图像矢量化框架，首次在矢量域引入本征图像分解，将图像分离为反照率、阴影和光照层，通过语义引导初始化和两阶段优化实现高保真度和可编辑性。
...
Method: 基于Clair-Obscur明暗对比原理，引入矢量域的本征图像分解，将图像分离为反照率、阴影和光照层，采用语义引导初始化和两阶段优化结合可微分渲染。

Result: 在多个数据集上的实验表明，COVec相比现有方法实现了更高的视觉保真度和显著改进的可编辑性。

Conclusion: COVec通过引入本征图像分解和明暗对比原理，成功解决了复杂真实图像矢量化中的保真度和可编辑性问题。
...
### [115] [History-Augmented Contrastive Meta-Learning for Unsupervised Blind Super-Resolution of Planetary Remote Sensing Images](https://arxiv.org/abs/2511.20045)
*Huijia Zhao,Jie Lu,Yunqing Jiang,Xiao-Ping Lu,Kaichang Di*

Main category: cs.CV

TL;DR: 提出HACBSR框架，一种无需地面真值和外部核先验的无监督盲超分辨率方法，通过对比核采样和历史增强对比学习解决行星遥感图像的质量问题。


<details>
  <summary>Details</summary>
Motivation: 行星遥感图像受成像环境和硬件限制影响，存在多样且未知的退化，缺乏地面真值图像限制了监督盲超分辨率的发展。

Method: HACBSR包含两个组件：对比核采样机制（缓解高斯采样的分布偏差）和历史增强对比学习（使用历史模型生成负样本，实现非贪婪优化和强凸性）。

Result: 在多个放大因子下，HACBSR与最先进的无监督方法相比具有竞争力表现，并发布了Ceres-50数据集用于评估。
...
### [116] [DeLightMono: Enhancing Self-Supervised Monocular Depth Estimation in Endoscopy by Decoupling Uneven Illumination](https://arxiv.org/abs/2511.20058)
*Mingyang Ou,Haojin Li,Yifeng Zhang,Ke Niu,Zhongxi Qiu,Heng Li,Jiang Liu*

Main category: cs.CV

TL;DR: 提出了DeLight-Mono框架，通过光照解耦解决内窥镜图像中不均匀光照对自监督单目深度估计的影响。


<details>
  <summary>Details</summary>
Motivation: 内窥镜图像存在不均匀光照问题，特别是在低强度区域，导致深度估计性能下降。现有低光增强技术无法有效指导深度网络，而其他领域的解决方案需要良好光照条件，不适用于内窥镜场景。
...
Conclusion: DeLight-Mono框架通过光照解耦有效解决了内窥镜图像中不均匀光照对深度估计的负面影响，为内窥镜导航系统的发展提供了有力支持。
...
### [117] [FLaTEC: Frequency-Disentangled Latent Triplanes for Efficient Compression of LiDAR Point Clouds](https://arxiv.org/abs/2511.20065)
*Xiaoge Zhang,Zijie Wu,Mingtao Feng,Zichen Geng,Mehwish Nasim,Saeed Anwar,Ajmal Mian*

Main category: cs.CV

TL;DR: FLaTEC是一种频率感知的点云压缩模型，通过解耦低频结构和高频纹理，使用混合潜在三平面作为紧凑代理，实现高压缩比的全扫描压缩。


<details>
  <summary>Details</summary>
Motivation: 点云压缩方法需要平衡比特率和重建失真，但低频和高频组件在同一分辨率下的贡献不同，这使得平衡压缩比和重建质量变得困难。

Method: 将体素化嵌入转换为三平面表示以减少稀疏性、计算成本和存储需求；设计频率解耦技术提取紧凑低频内容并收集跨尺度高频细节；使用基于频率的高效注意力机制补偿3D相关性损失。

Result: 在SemanticKITTI和Ford数据集上，BD-rate分别比标准编解码器提高了78%和94%，实现了最先进的率失真性能。

Conclusion: FLaTEC通过频率感知机制和紧凑的三平面表示，有效解决了点云压缩中低频和高频组件的平衡问题，显著提升了压缩性能。
...
### [118] [PRADA: Probability-Ratio-Based Attribution and Detection of Autoregressive-Generated Images](https://arxiv.org/abs/2511.20068)
*Simon Damm,Jonas Ricker,Henning Petzka,Asja Fischer*

Main category: cs.CV

TL;DR: 提出了PRADA方法，通过分析自回归图像生成模型的概率比来检测和溯源AI生成的图像


<details>
  <summary>Details</summary>
Motivation: 自回归图像生成模型能生成逼真图像，但目前缺乏专门检测此类模型生成图像的方法

Method: 检查模型对图像token序列的条件概率和无条件概率之比，利用该概率比的独特特征进行阈值检测和溯源

Result: PRADA对8个类别到图像和4个文本到图像模型高度有效

Conclusion: 概率比分析为自回归生成图像的可靠检测和溯源提供了简单可解释的方法
...
### [119] [Learning Procedural-aware Video Representations through State-Grounded Hierarchy Unfolding](https://arxiv.org/abs/2511.20073)
*Jinghan Zhao,Yifei Huang,Feng Lu*

Main category: cs.CV

TL;DR: 提出Task-Step-State (TSS)框架，通过引入状态作为视觉基础语义层，将抽象程序与可观察细节关联，采用渐进式预训练策略在多个下游任务上取得优越性能。
...
Method: 引入状态作为文本快照，构建任务-步骤-状态层次框架，提出渐进式预训练策略逐步展开TSS层次结构。

Result: 在COIN和CrossTask数据集上的实验表明，该方法在任务识别、步骤识别和下一步预测等下游任务上优于基线模型，状态监督是性能提升的关键因素。

Conclusion: 状态层提供了视觉基础语义，渐进式预训练策略比标准联合训练更有效地强制层次结构，是学习程序感知视频表示的有效方法。
...
### [120] [Blind Adaptive Local Denoising for CEST Imaging](https://arxiv.org/abs/2511.20081)
*Chu Chen,Aitor Artola,Yang Liu,Se Weon Park,Raymond H. Chan,Jean-Michel Morel,Kannie W. Y. Chan*

Main category: cs.CV

TL;DR: 提出了一种新的盲自适应局部去噪方法BALD，用于解决CEST MRI中的异方差噪声问题，无需先验噪声知识即可稳定噪声分布并提高定量对比映射的准确性。


<details>
  <summary>Details</summary>
Motivation: CEST MRI在临床应用中面临空间变化噪声和复杂成像协议导致的异方差性问题，传统去噪方法无法处理这种复杂噪声且会改变关键的生物医学信息。

Method: BALD利用CEST数据的自相似性推导自适应方差稳定变换，通过两阶段去噪在线性变换数据上分离分子信号与噪声，使用局部SVD分解防止空间和频谱去噪伪影。
...
### [121] [WPT: World-to-Policy Transfer via Online World Model Distillation](https://arxiv.org/abs/2511.20095)
*Guangfeng Jiang,Yueru Luo,Jun Liu,Yi Huang,Yiyao Zhu,Zhan Qu,Dave Zhenyu Chen,Bingbing Liu,Xu Yan*

Main category: cs.CV

TL;DR: WPT是一种世界模型到策略的转移训练范式，通过在线蒸馏在端到端世界模型指导下，将教师策略的推理能力转移到轻量级学生策略中，实现高性能和实时部署的平衡。
...
Result: 在开环和闭环基准测试中达到最先进性能：开环碰撞率0.11，闭环驾驶分数79.23，超越基于世界模型和模仿学习方法；学生策略推理速度提升4.9倍。

Conclusion: WPT训练范式成功实现了高性能规划与实时部署性的平衡，在准确性和安全性方面均表现优异。
...
TL;DR: 本文提出了一个用于早期森林火灾检测的数据集，通过游戏模拟器合成烟雾和初始火灾图像，并比较了YOLOv7和检测变换器模型在火灾识别任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏针对早期森林火灾检测的大规模数据集和专门模型，现有方法存在漏检问题，需要开发更有效的早期预警系统。

Method: 使用游戏模拟器（如Red Dead Redemption 2）合成烟雾羽流和初始火灾图像构建数据集，结合已有公开图像形成更全面的数据集，并比较YOLOv7和检测变换器模型的图像分类和定位性能。

Result: 创建了一个专门用于早期森林火灾识别的合成数据集，包含多个烟雾羽流和初始火灾实例，为火灾检测提供了更丰富的训练数据。

Conclusion: 通过游戏模拟器合成的数据集能够有效补充现有火灾检测数据，YOLOv7和检测变换器模型在早期火灾识别任务上表现出良好的应用潜力。
...
Method: 基于数据收集和预处理阶段，使用CNN配置和Inception V3模型作为核心模块，并集成多层注意力机制，特别是多头注意力机制来自动学习特征并选择性关注输入图像的特定区域。

Result: 模型在评估阶段表现出色：准确率95.6%，精确率95.2%，召回率96.2%，灵敏度95.7%，特异性96.1%，AUC值为96.0。

Conclusion: 该方法能够敏感地识别心脏肥大，验证了模型在临床诊断中的有效性，展示了深度学习结合注意力机制在医疗影像分析中的潜力。
...
Method: 从注意力图角度分析问题根源，发现两个失败模式都源于注意力分散。提出UltraViCo方法，通过常数衰减因子抑制训练窗口外token的注意力，无需训练即可使用。

Result: 在多种模型和外推比例上超越现有基线方法，将外推限制从2倍提升到4倍。在4倍外推时，动态程度和图像质量分别比之前最佳方法提升233%和40.5%。

Conclusion: 通过统一解决注意力分散问题，UltraViCo有效解决了视频长度外推的两个核心挑战，并可无缝推广到可控视频合成和编辑等下游任务。
...
TL;DR: 提出了PETRG-3D，一种端到端的3D双分支框架，用于自动生成PET/CT报告，特别针对淋巴瘤诊断，通过体积双模态建模和风格自适应提示来解决PET/CT报告生成的独特挑战。


<details>
  <summary>Details</summary>
Motivation: PET/CT在肿瘤学中至关重要，但扫描仪的快速扩张超过了训练有素专家的可用性，使得自动PET/CT报告生成对于减少临床工作量越来越重要。与结构成像相比，功能性PET面临独特挑战：代谢模式随示踪剂生理学变化，需要全身3D上下文信息而非局部区域解释。

Method: 提出PETRG-3D，一个端到端的3D双分支框架，分别编码PET和CT体积，并融入风格自适应提示以减轻医院间报告实践的变异性。构建了PETRG-Lym多中心淋巴瘤数据集和AutoPET-RG-Lym公开基准。

Result: 实验表明，PETRG-3D在自然语言指标（如ROUGE-L提升31.49%）和临床效能指标（如PET-All提升8.18%）上显著优于现有方法，突出了体积双模态建模和风格感知提示的益处。
...
TL;DR: HCFSSNet是一个用于学习图像压缩的混合卷积和频率状态空间网络，结合CNN的局部高频细节提取能力和状态空间模型的长程低频建模能力，通过频率调制和方向扫描实现高效压缩。


<details>
  <summary>Details</summary>
Motivation: 现有的Transformer和状态空间模型在图像压缩中虽然具有强大的长程建模能力，但可能导致结构信息丢失或忽略对压缩至关重要的频率特性。需要结合CNN的局部细节捕捉优势。

Method: 提出HCFSSNet：1) 使用CNN提取局部高频结构；2) 视觉频率状态空间块结合全向邻域状态空间模块和自适应频率调制模块；3) 频率Swin Transformer注意力模块用于频率感知的边信息建模。

Result: 在Kodak、Tecnick和CLIC数据集上，HCFSSNet相比VTM锚点分别减少BD率18.06%、24.56%和22.44%，与MambaIC等SSM编解码器相比具有竞争性的率失真性能，且参数数量显著减少。

Conclusion: HCFSSNet为未来学习图像压缩系统提供了一个高效且可解释的混合架构，成功结合了CNN的局部建模和状态空间模型的长程建模优势。
...
TL;DR: Restora-Flow是一种无需训练的流匹配方法，通过退化掩码引导采样并加入轨迹校正机制，在图像修复任务中实现高质量结果和快速处理。


<details>
  <summary>Details</summary>
Motivation: 当前基于流模型的修复方法存在处理时间长或结果过度平滑的问题，需要一种既能保持高质量生成又能快速处理的方法。

Method: 使用退化掩码引导流匹配采样，并加入轨迹校正机制来确保与退化输入的一致性，无需额外训练。

Result: 在自然和医学数据集上的多种图像修复任务（修复、超分辨率、去噪）中，相比扩散模型和流匹配方法，显示出更优的感知质量和处理时间。

Conclusion: Restora-Flow通过掩码引导和轨迹校正，为基于流匹配的图像修复提供了一种高效且高质量的解决方案。
...
Method: 首先将sMRI特征投影到流形空间保留疾病进展的固有几何结构；使用时间感知神经ODE建模观测间潜在状态的连续演化；注意力黎曼门控循环单元自适应整合历史和当前信息处理不规则间隔。

Result: 在疾病状态预测和认知评分回归任务中始终优于最先进模型，消融研究验证各模块的贡献，在不同序列长度和缺失数据率下表现稳定，跨数据集验证证实其鲁棒性。

Conclusion: 该联合设计提高了时间一致性，在不规则采样下产生稳健的AD轨迹预测，在多样化临床环境中具有强适用性。
...
TL;DR: SKEL-CF是一个从粗到细的框架，用于估计SKEL参数，通过Transformer编码器-解码器架构预测和逐步优化相机和SKEL参数，在MOYO数据集上显著优于之前的SOTA方法。


<details>
  <summary>Details</summary>
Motivation: 现有的参数化3D人体模型如SMPL在人体姿态和形状估计方面取得了进展，但其简化的运动学限制了生物力学真实性。SKEL模型通过重新绑定解剖学准确的骨骼解决了这一限制，但直接估计SKEL参数仍然具有挑战性。

Method: 采用基于Transformer的编码器-解码器架构，编码器预测粗略的相机和SKEL参数，解码器在连续层中逐步优化它们。将现有的SMPL数据集4DHuman转换为SKEL对齐版本4DHuman-SKEL，提供高质量训练数据。
...
<details>
  <summary>Details</summary>
Motivation: 现有持续视觉指令调优研究主要关注未安全对齐的模型，忽视了现实世界多模态大语言模型需要安全机制来降低潜在风险。在持续适应过程中，模型不仅遭受任务遗忘，还会出现安全性退化。

Method: 提出HPA后训练框架，包括基于关注度的参数划分、平衡参数选择和正交参数调整。将参数按关注安全或任务性能划分，从平衡角度选择关键参数进行保护，并对参数更新施加正交性约束。

Result: 在CVIT基准测试和安全评估数据集上的广泛实验表明，HPA比现有基线更好地保持高安全性并减轻遗忘。

Conclusion: HPA框架有效解决了安全对齐多模态大语言模型在持续学习中的安全性与任务性能平衡挑战。
...
TL;DR: ADNet是一个大规模多领域异常检测基准，包含380个类别、196,294张图像，涵盖电子、工业、农业食品、基础设施和医疗领域。实验显示现有方法在扩展到380个类别时性能显著下降，作者提出了Dinomaly-m方法来解决可扩展性问题。
...
Method: 构建ADNet基准，聚合49个公开数据集的380个类别，提供标准化像素级标注和结构化文本描述。提出Dinomaly-m方法，这是Dinomaly的上下文引导混合专家扩展，在不增加推理成本的情况下扩展解码器容量。

Result: 现有最先进方法在单类别设置下达到90.6% I-AUROC，但在扩展到380个类别的多类别设置下降至78.5%。Dinomaly-m达到83.2% I-AUROC和93.1% P-AUROC，优于现有方法。

Conclusion: ADNet为异常检测提供了标准化和可扩展的基准，支持社区在不同领域扩展异常检测数据集，并为未来异常检测基础模型提供可扩展基础。
...
TL;DR: 首个电池供电的可穿戴瞳孔中心追踪系统，结合事件视觉传感器和神经形态处理，实现100Hz双目光学追踪，单眼功耗低于5mW。


<details>
  <summary>Details</summary>
Motivation: 可穿戴平台需要实现鲁棒、高频的眼动追踪，同时保持超低功耗。事件视觉传感器虽然具有微秒级分辨率和稀疏数据流，但缺乏完全集成的低功耗实时处理解决方案。

Method: 在商用Speck2f系统芯片上结合事件感知和神经形态处理，使用低功耗微控制器进行轻量级坐标解码。采用新型不确定性量化脉冲神经网络和门控时间解码，针对严格的内存和带宽限制进行优化，并辅以系统部署机制。

Result: 在新型多用户数据集上验证，可穿戴原型配备双神经形态设备，实现100Hz鲁棒双目光学追踪，单眼平均功耗低于5mW。

Conclusion: 端到端神经形态计算能够实现实用的、始终在线的眼动追踪，为下一代高能效可穿戴系统提供支持。
...
Method: 提出了三个关键模块：EgoExo-Align实现视角对齐，MultiExoCon聚合多视角条件信号，PoseInj注入相机姿态信息。

Result: 在ExoEgo4D数据集上的实验表明，Exo2EgoSyn显著提升了Ego2Exo合成质量。

Conclusion: 该工作为基础模型的跨视角视频生成提供了可扩展的解决方案，无需从头训练。
...
TL;DR: SFA是一个无需训练的Video-LLM框架，专门用于视频文本视觉问答任务，通过自适应扫描视频帧、选择性关注关键区域并直接放大它们，引导模型注意力生成更准确的答案。
...
Method: 提出SFA训练免费框架，基于人类回答问题过程的启发，通过自适应扫描视频帧、选择性关注关键区域并直接放大它们，有效引导Video-LLM的注意力朝向关键线索。

Result: SFA在多个公开Video TextVQA数据集上实现了新的最先进结果，大幅超越先前方法，证明了其有效性和泛化能力。

Conclusion: SFA框架通过模拟人类认知过程，成功解决了视频文本视觉问答中的关键挑战，在多个数据集上取得了显著性能提升。
...
TL;DR: GHR-VQA是一个基于场景图的人为中心的视频问答框架，通过图神经网络处理视频级图结构，实现跨帧的人类-物体交互推理，在AGQA数据集上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统基于像素的方法难以捕捉视频中复杂的人类-物体交互关系，需要更结构化、可解释的方法来理解视频中的时空动态。

Method: 将视频帧表示为场景图，跨帧连接人类节点到全局根节点形成视频级图，使用图神经网络处理图结构，并与问题特征在多抽象层次上进行分层集成。
...
TL;DR: 本文介绍了一个在BraTS-Inpainting 2025挑战赛中获得第一名的深度学习框架，用于在3D脑部MRI扫描中合成健康组织，通过U-Net架构和随机掩码增强策略实现了优异的图像修复性能。
...
Conclusion: 该方法在脑部MRI图像修复任务中表现出色，超越了2023和2024年的获胜方案，为解决数据集偏差问题提供了有效解决方案。
...
TL;DR: 提出CT-CIG方法，通过可控文本引导生成逻辑合理且逼真的伪装图像，利用大视觉语言模型标注高质量文本提示，并设计频率交互细化模块提升伪装模式学习效果。


<details>
  <summary>Details</summary>
Motivation: 现有伪装图像生成方法往往忽略伪装对象与背景环境之间的逻辑关系，导致结果不自然，需要开发能够生成逻辑合理且逼真伪装图像的方法。
...
Conclusion: CT-CIG方法能够有效生成逻辑合理且逼真的伪装图像，解决了现有方法忽略逻辑关系的问题，在语义对齐和伪装效果方面表现优异。
...
TL;DR: 该研究开发了一种基于Vision Transformer的深度学习模型，用于胶质母细胞瘤的病理图像分析，在BraTS-Path 2025挑战赛中获得了第二名。
...
Conclusion: 通过绕过纠缠的patch特征直接操作值特征，V-Attack实现了高效的语义控制，揭示了现代视觉语言理解中的关键漏洞。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在多模光纤成像中主要关注简单数据集，难以处理复杂的真实世界成像任务，且通常需要大量数据。本文旨在解决这些限制，使多模光纤成像更接近实际临床应用。

Method: 开发了光学实验装置，使用空间光调制器和多模光纤捕获散斑模式；提出HistoSpeckle-Net架构，包含直方图计算单元和三级特征细化模块；采用基于直方图的互信息损失和多尺度结构相似性损失。
...
<details>
  <summary>Details</summary>
Motivation: 长提示词包含丰富的内容、空间和风格信息，能提高生成图像的保真度，但往往会抑制多样性，导致输出重复且缺乏创意。现有模型在长提示词下的行为尚未得到充分研究。

Method: 提出了LPD-Bench基准来评估长提示词生成的保真度和多样性；开发了理论框架通过提示词重构增加采样熵；提出了PromptMoG方法，在嵌入空间中使用高斯混合模型采样提示词嵌入来增强多样性。

Result: 在四个最先进模型（SD3.5-Large、Flux.1-Krea-Dev、CogView4和Qwen-Image）上的广泛实验表明，PromptMoG能一致地提高长提示词生成的多样性，且不会导致语义漂移。

Conclusion: PromptMoG是一种有效的训练免费方法，能够解决长提示词生成中的保真度-多样性困境，在保持语义一致性的同时显著提升生成多样性。
...
<details>
  <summary>Details</summary>
Motivation: 现实环境需要能够识别多样化、前所未见物体的模型，而闭集方法在这方面存在重大限制。现有开放词汇3D检测器虽然放宽了标注要求，但仍依赖训练场景。

Method: 通过2D实例掩码的图聚类构建3D边界框，使用最佳视图选择和视图一致性掩码生成的开放词汇模块分配语义标签。提供两种模式：零样本Zoo3D₀（完全无需训练）和自监督Zoo3D₁（在Zoo3D₀生成的伪标签上训练类无关检测器）。
...
TL;DR: Adv-GRPO是一个对抗性强化学习框架，通过迭代更新奖励模型和生成器来解决传统预训练偏好模型的奖励黑客问题，使用参考图像和视觉基础模型提供密集视觉奖励，显著提升图像质量。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习在图像生成中依赖的预训练偏好模型存在奖励黑客问题，即高分不一定对应更好的图像，且这些标量奖励难以捕捉人类感知。

Method: 提出Adv-GRPO框架：1）使用对抗性奖励迭代更新奖励模型和生成器；2）以参考图像作为正样本监督奖励模型；3）采用图像本身作为奖励，结合视觉基础模型（如DINO）提供密集视觉信号。

Result: 在人类评估中，Adv-GRPO在图像质量和美学方面分别以70.0%和72.4%的胜率优于Flow-GRPO和SD3，在图像质量、美学和任务特定指标上均取得一致提升。

Conclusion: 结合参考样本和基础模型奖励的方法能够实现分布迁移和灵活风格定制，有效解决奖励黑客问题并生成更高质量的图像。
...
TL;DR: MBCD是一个多模态领域泛化的协作蒸馏框架，通过自适应模态丢弃、梯度一致性约束和基于权重平均的跨模态蒸馏，解决多模态优化速度差异导致的模态偏差问题，实现更平坦的损失曲面和更好的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 在多模态领域泛化中，直接应用权重平均会因为不同模态优化速度差异而偏向快速收敛的模态，抑制慢速但互补模态的贡献，导致模态融合效果差和损失曲面尖锐化，影响泛化能力。

Method: 1) 学生模型中的自适应模态丢弃控制早期对主导模态的偏向；2) 梯度一致性约束对齐单模态分支和融合表示的学习信号；3) 基于权重平均的教师模型进行跨模态蒸馏，将融合知识传递到各单模态分支。

Result: 在MMDG基准测试上的广泛实验表明，MBCD持续优于现有方法，在不同未见域上实现了更优的准确性和鲁棒性。

Conclusion: MBCD框架成功保留了权重平均的平坦化优势，同时克服了其在多模态上下文中的缺陷，实现了更有效的模态融合和更好的领域泛化性能。
...
Method: 使用离散扩散过程在类别概率或离散类别标签上进行扩散预测，提供计算和内存权衡的灵活性
...
TL;DR: 提出了一种半监督深度强化学习框架，通过自适应采样、自编码器和预测器的协同工作，在仅有少量标注数据的情况下有效检测工业视觉中的细微缺陷。


<details>
  <summary>Details</summary>
Motivation: 工业视觉检测中缺陷样本稀缺，现有无监督重建方法容易过拟合且难以检测细微缺陷，需要一种能有效利用有限标注数据的解决方案。

Method: 结合神经批量采样器、自编码器和预测器的半监督深度强化学习框架，RL采样器通过复合奖励平衡探索与利用，自编码器生成损失分布图，预测器在损失分布空间进行分割。
...
TL;DR: 提出了VKnowU基准和VideoKnow+模型，用于评估和改进多模态大语言模型的视觉知识理解能力，涵盖物理和社会常识


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs虽然擅长物体识别，但缺乏对人类世界物理和社会原则的直观理解，这种视觉知识是连接感知和推理的关键桥梁

Method: 构建VKnowU基准（1680个问题，1249个视频，8类视觉知识），提出VideoKnow+模型采用See-Think-Answer范式和基于视觉知识奖励的强化学习

Result: 评估23个SOTA MLLMs发现与人类表现存在差距，VideoKnow+在VKnowU上提升3.7%，在多个基准上取得一致改进
...
TL;DR: ScenarioCLIP是一个解决CLIP模型在复杂场景理解中缺乏关系建模能力的模型，通过引入基于关系的场景分析，在跨模态检索和细粒度视觉理解任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP模型主要处理单对象分类或短描述检索，但真实场景包含丰富的组合结构（多对象和动作关系）。现有方法局限于预定义类别列表，缺乏对关系和组合结构的显式建模。

Method: 提出ScenarioCLIP模型，接受输入文本、基础关系、输入图像以及突出关系的聚焦区域。模型在精心策划的场景数据上预训练，并在下游任务上微调。构建新数据集，使用语言模型管道来基础化动作、对象和关系。

Result: ScenarioCLIP在各种领域特定任务中表现出强大的零样本和微调性能，在多个场景基准任务上优于基线方法。

Conclusion: ScenarioCLIP通过显式建模场景中的关系结构，显著提升了CLIP模型在复杂场景理解任务中的表现，为场景分析提供了有效解决方案。
...
TL;DR: 提出了DAPointMamba框架，这是首个在领域自适应点云补全中应用状态空间模型的工作，通过跨域补丁级扫描、空间SSM对齐和通道SSM对齐三个模块，有效解决几何和语义域差异问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在领域自适应点云补全中存在有限感受野或二次复杂度问题，直接应用状态空间模型会破坏空间拓扑和局部几何特征，且缺乏领域无关表示设计。

Method: 提出DAPointMamba框架，包含三个核心模块：跨域补丁级扫描建立局部几何对应关系，跨域空间SSM对齐增强空间一致性，跨域通道SSM对齐解决全局语义差距。

Result: 在合成和真实世界基准测试中，DAPointMamba在计算复杂度和推理延迟更低的情况下，性能优于现有最先进方法。

Conclusion: DAPointMamba展示了在领域自适应点云补全中的强适应性，具有全局感受野和高效线性复杂度的优势。
...
<details>
  <summary>Details</summary>
Motivation: 尽管transformer架构在端到端跟踪方面取得进展，但检测性能差以及检测与关联在联合架构中的冲突仍是关键问题。现有方法通过使用高级去噪策略或引入外部检测器先验来缓解这些问题。

Method: 引入SelfMOTR，一种依赖自生成检测先验的新型跟踪transformer。通过广泛分析和消融研究，揭示并证明了MOTR类模型的隐藏检测能力，并提供了有效利用这些能力的实用工具集。

Result: 在DanceTrack数据集上，SelfMOTR实现了强大的性能，与最近最先进的端到端跟踪方法竞争。

Conclusion: 通过挖掘MOTR类模型的隐藏检测能力并利用自生成检测先验，SelfMOTR成功解决了端到端跟踪中检测性能差和检测-关联冲突的问题。
...
Method: 采用多模态思维链过程，基于物理不一致性反馈迭代优化提示词，逐步提升生成质量。该方法无需训练且即插即用，适用于多种视频生成模型。

Result: 在PhyIQ基准测试中，物理智商分数从56.31提升至62.38，显著改善了生成视频的物理一致性。
...
TL;DR: 提出了BTTF框架，用于为视频分类器生成反事实解释，解决了现有图像方法无法生成时间一致、平滑且物理合理的视频反事实的问题。
...
Method: 提出BTTF优化框架，包含两个新特征：1）基于输入视频第一帧的条件化初始潜在噪声优化方案；2）在输入视频附近搜索反事实视频的两阶段优化策略，采用渐进式优化策略加速收敛。

Result: 在Shape-Moving（运动分类）、MEAD（情感分类）和NTU RGB+D（动作分类）等视频数据集上的实验表明，BTTF能有效生成有效、视觉相似且逼真的反事实视频。

Conclusion: BTTF框架能够为视频分类器生成高质量的反事实解释，为理解分类器决策机制提供了具体洞察。
...
TL;DR: 提出了CrossEarth-Gate方法，通过建立遥感模块工具箱和Fisher引导的自适应选择机制，有效处理遥感数据中的多面域差距问题，在16个跨域遥感语义分割基准上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有的参数高效微调方法在应用于大规模地球观测任务时经常失败，因为它们无法完全处理遥感数据中固有的多面性和不可预测的域差距（如空间、语义和频率偏移）。

Method: 1. 建立全面的遥感模块工具箱，包含空间、语义和频率模块来处理多面域差距；2. 开发Fisher引导的自适应选择机制，基于Fisher信息量化每个模块的重要性，动态激活最关键模块以最大化适应效果和效率。
...
TL;DR: 提出了TaCo网络，通过时空语义联合约束增强遥感变化检测，将变化视为双时态状态间的语义转换，无需推理时额外计算开销即可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅依赖掩码监督，虽然能有效指导空间定位，但对时态语义转换的约束有限，导致预测结果存在语义不一致问题。

Method: 提出时空语义一致网络TaCo，包含文本引导的转换生成器和时空语义联合约束（双时态重构约束和转换约束），将文本语义与视觉特征结合构建跨时态转换特征。

Result: 在六个公开数据集上的广泛实验表明，TaCo在二进制和语义变化检测任务中均达到最先进的性能。

Conclusion: TaCo通过引入时空语义联合约束，有效解决了遥感变化检测中的语义不一致问题，在多个数据集上实现了SOTA性能。
...
Method: 直接使用预训练DiT或UNet预测的速度作为输出，采用内存高效的潜在循环一致性和身份损失，以及轻量级架构简化。

Result: 在多个图像翻译数据集上达到与最先进方法相当的性能，同时支持实时推理。

Conclusion: TReFT成功解决了RF模型在一步图像翻译中的收敛问题，为实时应用提供了可行方案。
...
TL;DR: 提出IrisNet框架，通过元学习动态适应不同红外场景，使用图像到解码器的变换器生成自适应解码参数，提升红外小目标检测的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决红外小目标检测中由于低信噪比、复杂背景和缺乏可识别特征导致的挑战，以及现有深度学习方法在跨场景（如昼夜变化、天空/海洋/地面域）中的模式漂移问题。

Method: 建立红外图像特征与整个解码器参数之间的动态映射，通过图像到解码器变换器生成自适应解码模式，并集成高频组件增强红外图像感知能力。

Result: 在NUDT-SIRST、NUAA-SIRST和IRSTD-1K数据集上实现了最先进的性能。

Conclusion: IrisNet通过动态适应不同红外场景，显著提升了红外小目标检测的鲁棒性和性能。
...
TL;DR: 提出了一种基于公正世界模型的端到端自动驾驶强化学习框架，通过反事实合成方法教会模型诚实预测危险，显著提升了安全性能。
...
Method: 引入反事实合成管道系统生成碰撞和偏离道路事件的数据，构建公正世界模型作为内部批评器，在策略精炼过程中让智能体查询候选动作的后果。
...
TL;DR: 提出基于深度学习的单图像单应性估计框架，使用ConvNeXt骨干网络和归一化坐标回归，通过合成单应性增强策略解决数据稀缺问题，在货架图像矫正任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决零售领域单视角货架监控和产品对齐的挑战，传统方法在单图像单应性估计方面存在困难，需要更稳健高效的解决方案。

Method: 使用ConvNeXt骨干网络进行特征提取，采用归一化坐标回归提高稳定性，提出合成单应性建模和采样的数据增强策略来应对数据稀缺问题。

Result: 在测试集上达到1.298像素的平均角点误差，相比传统计算机视觉和深度学习方法，在准确性和推理速度方面都表现出竞争力。

Conclusion: 该方法为现实世界单视角图像矫正提供了稳健高效的解决方案，并公开数据集和代码以促进该领域进一步研究。
...
Method: 基于多视图重建训练的稀疏体素场景表示方法，使用前馈网络架构。

Result: 在相机姿态、深度和度量尺度估计、3D重建方面达到最先进性能，甚至超过基于优化的SLAM和SfM方法。

Conclusion: AMB3R展示了单一模型在多种3D视觉任务中的通用性和优越性能，无需任务特定调整。
...
TL;DR: 提出了人形视觉搜索任务，让代理在360°全景图像中主动旋转头部搜索物体或路径，并构建了H* Bench基准测试，展示了当前模型在此任务上的局限性及改进方法。


<details>
  <summary>Details</summary>
Motivation: 人类通过头眼协同在360°环境中高效搜索视觉信息，但现有方法局限于静态图像，忽略了物理具身性与3D世界的交互。需要开发能绕过真实世界硬件限制的高效具身视觉搜索代理。

Method: 提出人形视觉搜索任务，构建H* Bench基准测试包含交通枢纽、零售空间等复杂场景，使用后训练技术增强开源模型Qwen2.5-VL的性能。

Result: 顶级专有模型在物体和路径搜索中成功率仅约30%。通过后训练，Qwen2.5-VL的物体搜索成功率从14.83%提升至47.38%，路径搜索从6.44%提升至24.94%。

Conclusion: 路径搜索的较低上限揭示了其固有难度，需要复杂的空间常识。研究展示了构建能无缝融入人类日常生活的MLLM代理的前进道路，但也量化了剩余的巨大挑战。
...
TL;DR: GS-Checker是一种无需3D标注监督的3DGS模型篡改定位方法，通过引入3D篡改属性和对比机制来精确定位被篡改区域
...
Method: 在3D高斯参数中集成3D篡改属性，设计3D对比机制比较高斯间的关键属性相似性，并采用循环优化策略来精炼篡改属性
...
TL;DR: VGGTFace是一种从多视角图像自动重建拓扑一致面部几何的方法，利用VGGT 3D基础模型并注入拓扑信息，在10秒内完成高质量重建。
...
Method: 结合VGGT和Pixel3DMM，通过像素对齐的UV值注入拓扑信息，提出拓扑感知的束调整策略，构建拉普拉斯能量项。

Result: 在基准测试中达到最先进结果，对野外数据具有出色的泛化能力，16个视图在RTX 4090上10秒内完成重建。
...
TL;DR: FREE框架通过特征级自回归和并行验证，为DiTs提供无损加速，结合不确定性引导的松弛策略进一步提升速度，在ImageNet-512²上实现最高2.25倍加速。


<details>
  <summary>Details</summary>
Motivation: DiTs虽然生成质量优秀，但需要长序列去噪过程导致推理延迟高。现有推测推理方法在DiTs上加速有限，因为验证阶段的草稿准确性不足。

Method: 分析DiTs特征动态，发现顶层transformer层特征具有强时序一致性和丰富语义抽象。提出FREE框架：使用轻量级草稿器进行特征级自回归并行验证；引入不确定性引导松弛策略动态调整接受概率。

Result: 在ImageNet-512²上，FREE实现1.86倍加速，FREE（松弛）达到2.25倍加速，同时保持高质量生成保真度。

Conclusion: FREE通过特征级推测推理和不确定性感知的松弛策略，有效解决了DiTs推理延迟问题，实现了显著加速而不损失生成质量。
...
TL;DR: 提出MultiID方法解决多ID定制化任务，通过ID解耦交叉注意力机制和三种控制策略，在无需训练的情况下实现高质量的多身份图像生成。


<details>
  <summary>Details</summary>
Motivation: 多ID定制化相比单ID定制化面临两个主要挑战：复制粘贴问题导致生成质量下降，以及文本控制能力不足导致生成结果与文本提示不一致。

Method: 采用ID解耦交叉注意力机制将不同ID嵌入注入对应图像区域，结合局部提示、深度引导空间控制和扩展自注意力三种策略增强生成可控性。

Result: 在IDBench基准测试中，MultiID在定性和定量评估中表现出色，性能与基于训练的多ID定制方法相当甚至更好。

Conclusion: MultiID有效解决了多ID定制化的两大挑战，无需训练即可实现高质量的多身份图像生成，具有良好的文本控制能力。
...
TL;DR: 提出轨迹反向一致性模型(TBCM)，通过直接从教师模型的生成轨迹中提取潜在表示，消除对外部训练数据的依赖，显著提高扩散模型的时间步蒸馏效率。


<details>
  <summary>Details</summary>
Motivation: 当前连续时间一致性蒸馏方法严重依赖训练数据和计算资源，限制了在资源受限场景的部署和向多样化领域的扩展。

Method: 采用自包含的蒸馏范式，从教师模型的生成轨迹中提取潜在表示，无需VAE编码和大规模数据集，有效弥合训练与推理之间的分布差距。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以平衡基于文本生成的创意灵活性与显式结构表示提供的对象级可编辑性，需要开发能同时满足风格多样性、细粒度控制和结构一致性的3D城市生成方法。

Method: 将城市表示为可控布局、资产和材料的组合，采用四阶段流程；集成MajutsuAgent支持五种对象级操作；构建包含2D语义布局、高度图、3D建筑资产、PBR材料和天空盒的多模态数据集。

Result: 布局FID比CityDreamer降低83.7%，比CityCraft降低20.1%；在所有AQS和RDR评分中排名第一，在几何保真度、风格适应性和语义可控性方面达到最新水平。

Conclusion: MajutsuCity在3D城市生成的几何保真度、风格适应性和语义可控性方面确立了新的最先进水平，有望激发3D城市生成研究的新方向。
...
<details>
  <summary>Details</summary>
Motivation: 为了理解人类在复杂场景中的交互行为，需要建模多人与周围环境的关联，特别是两人共享物体交互的情况。

Method: 使用图像生成模型合成HHOI数据，分别训练text-to-HOI和text-to-HHI扩散模型，然后整合成统一生成框架。

Result: 实验表明该方法能生成逼真的HHOIs，优于仅关注单人HOI的方法，并能扩展到多人交互场景。

Conclusion: 提出的统一生成框架能有效合成多人-物体交互，为多人体运动生成提供了应用基础。
...
TL;DR: ZoomSearch是一个无需训练、即插即用的超高分遥感视觉问答流水线，通过自适应多分支缩放搜索和布局感知补丁重组来解决超高分辨率图像处理问题，在保持推理效率的同时显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前遥感基础模型不适合处理超高分辨率图像：全图编码会耗尽token和内存预算，而基于缩放的预处理会丢失细粒度和关键细节。因此需要在预测前引导模型关注重要区域。

Method: ZoomSearch结合了自适应多分支缩放搜索（在图像块上进行分层搜索以定位查询相关区域）和布局感知补丁重组（将选定块重新组织成紧凑、布局忠实的画布）。

Result: 与LLaVA-ov集成时，在LRS-VQA上比基线提升26.3%，在MME-RealWorld-RS上提升114.8%，同时推理速度比之前基于搜索的方法快20%~44%。

Conclusion: ZoomSearch在超高分遥感视觉问答任务中实现了最先进的准确性和更高的推理效率，证明了将'看哪里'与'如何回答'解耦的有效性。
...
TL;DR: STARFlow-V是一个基于归一化流的视频生成模型，通过全局-局部架构和流-分数匹配技术，实现了端到端学习、因果预测和原生似然估计，在视频生成质量上可与扩散模型媲美。


<details>
  <summary>Details</summary>
Motivation: 在视频生成领域，当前最先进的方法几乎完全依赖扩散模型，而归一化流(NFs)虽然在其他连续数据生成任务中表现出色，但在视频生成中尚未得到充分探索。本研究旨在重新审视归一化流在视频生成中的潜力。

Method: 1. 在时空潜在空间中构建全局-局部架构，限制因果依赖到全局潜在空间，同时保留帧内丰富的局部交互；2. 提出流-分数匹配技术，为模型配备轻量级因果去噪器；3. 采用视频感知的Jacobi迭代方案提高采样效率。

Result: STARFlow-V在视觉保真度和时间一致性方面取得了强劲表现，采样吞吐量相对于基于扩散的基线具有实际优势。该模型原生支持文本到视频、图像到视频以及视频到视频生成任务。
...
TL;DR: 提出了一种轻量级舞蹈风格分类框架，使用从视频中提取的姿态估计数据，结合Laban运动分析的时空描述符和快速傅里叶变换特征来识别舞蹈风格。
...
Method: 使用姿态估计提取运动特征，提出受Laban运动分析启发的时空描述符来捕捉局部关节动力学（速度、加速度、上肢角度运动），并集成快速傅里叶变换特征来编码运动的节奏和周期性方面。

Result: 该方法能够以低计算成本对不同舞蹈风格进行稳健分类，无需复杂模型架构，表明可解释的运动表示可以有效捕捉风格细微差别。

Conclusion: 提出的轻量级框架通过结合时空描述符和频域特征，成功实现了舞蹈风格的准确分类，证明了基于运动特征的方法在舞蹈识别中的有效性。
...
TL;DR: 本研究评估了受感知系统启发的模块化架构，开发了三个独立感知模块：CNN用于眼状态检测、深度CNN用于面部表情识别、LSTM用于语音识别，在各自数据集上分别达到93.0%、97.8%和96.89%的准确率。
...
Method: 提出模块化架构，包含三个独立感知模块：CNN用于眼状态检测（困倦/注意力）、深度CNN用于面部表情识别、LSTM用于语音说话人识别。

Result: 在Eyes Image、FER2013和自定义音频数据集上，三个模型分别达到93.0%、97.8%和96.89%的准确率。
...
TL;DR: 提出了一种基于物理学的损失函数(PIL)，用于脑动脉DSA序列分割，通过弹性过程建模预测边界与真实边界之间的相互作用，提高血管分割的精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统的像素级重叠损失函数忽视了血管边界的几何和物理一致性，导致血管预测出现碎片化或不稳定。需要一种能更好捕捉精细血管几何结构的损失函数。

Method: 提出物理信息损失(PIL)，基于材料物理中的位错理论，将边界相互作用建模为弹性过程，引入物理正则化项以强制平滑轮廓演化和结构一致性。

Result: 在DIAS和DSCA两个公开基准上测试，PIL在U-Net、U-Net++、SegFormer和MedFormer等架构中均优于交叉熵、Dice、主动轮廓和表面损失等传统损失函数，实现了更高的敏感性、F1分数和边界一致性。
...
TL;DR: AlignBench是一个新的图像-文本对齐评估基准，通过评估多样化模型生成的详细图像-描述对来提供新的对齐指标，揭示CLIP模型在细粒度对齐评估中的局限性。
...
Method: 引入AlignBench基准，使用多样化图像到文本和文本到图像模型生成详细图像-描述对，并对每个句子进行正确性标注，直接评估视觉语言模型作为对齐评估器的能力。

Result: 评估发现：CLIP模型对细粒度对齐几乎盲视；检测器系统性地高估早期句子得分；模型表现出强烈的自偏好倾向，偏向自身输出并损害检测性能。

Conclusion: AlignBench为图像-文本对齐提供了更全面的评估框架，揭示了现有模型在细粒度对齐评估中的重要局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的统一模型采用对称设计，由于模态差异而效果欠佳。HBridge旨在让异构专家充分利用各自模态域的预训练先验知识。

Method: 采用非对称H形架构，选择性桥接中间层，解耦浅层和深层，引入语义重建令牌来增强跨模态一致性。

Result: 在多个基准测试中的广泛实验证明了HBridge的有效性和优越性能。

Conclusion: HBridge为统一多模态生成建立了新的范式。
...
TL;DR: 提出了Mistake Attribution (MATT)任务，用于在自我中心视频中细粒度理解人类错误，通过MisEngine数据引擎自动构建错误样本，并开发了统一的MisFormer模型进行错误归因。


<details>
  <summary>Details</summary>
Motivation: 现有错误理解工作缺乏细粒度输出，需要更具体地将错误归因到指令文本或尝试视频中。

Method: 开发MisEngine数据引擎自动构建错误样本，提出统一的MisFormer模型进行语义、时间和空间维度的错误归因。

Result: 在EPIC-KITCHENS-M和Ego4D-M数据集上，MisFormer优于现有的视频-语言、时间定位、手-物交互和错误检测基线方法。

Conclusion: MATT任务和MisEngine/MisFormer框架为细粒度错误理解提供了有效解决方案，在多个基准测试中表现出色。
...
Result: 发现理解与生成之间存在显著差距，显式思维链能有效弥合推理生成差距，自训练可内化推理能力；思维链辅助知识检索，基于查询的架构具有潜在思维链特性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型缺乏明确的物理可控性和合理性，基于物理渲染的方法在建模复杂物理属性和长期时间序列控制方面存在挑战。

Method: 采用两阶段方法：1) 通过部件感知的物理属性重建估计图像中所有物体的静态初始物理属性；2) 通过时间指令和物理编辑模拟合成高质量视频。

Result: 实验结果表明PhysChoreo能生成具有丰富行为和物理真实感的视频，在多个评估指标上优于最先进方法。

Conclusion: PhysChoreo框架成功解决了视频生成中物理可控性和真实性的问题，为物理引导的视频生成提供了有效解决方案。
...
TL;DR: ReaDe是一个通用的模型无关解释器，通过先推理后描述的方式将用户指令转换为精确的视频生成规范，提升指令忠实度和视频质量


<details>
  <summary>Details</summary>
Motivation: 解决扩散变换器在视频生成中用户输入简洁模糊与训练时详细提示不匹配的问题，改善意图-输出不匹配

Method: 采用推理-描述范式：先分析用户请求识别核心需求并消除歧义，然后生成详细指导；通过两阶段优化训练：推理增强监督和多维奖励分配器

Result: 在单条件和多条件场景下均显著提升指令忠实度、字幕准确性和下游视频质量，对推理密集型和新输入有强泛化能力
...
TL;DR: DINO-Tok是一个基于DINO的视觉分词器，通过整合浅层和深层特征构建信息完整的潜在空间，解决了现有分词器在语义表示和重建保真度之间的平衡问题，并在高维空间中通过全局PCA重加权机制稳定向量量化。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉分词器通常从头训练，难以在高维潜在空间中平衡语义表示和重建保真度，导致关键信息丢失和码本崩溃。

Method: 提出DINO-Tok分词器，整合DINO的浅层特征（保留细粒度细节）和深层特征（编码全局语义），并采用全局PCA重加权机制来稳定高维空间中的向量量化。

Result: 在ImageNet 256×256上，DINO-Tok实现了最先进的重建性能：自编码达到28.54 PSNR，基于VQ的建模达到23.98 PSNR，显著优于先前分词器，与亿级数据训练模型相当。
...
TL;DR: 该论文提出了VQ-VA World框架，通过大规模数据构建和训练，使开源模型能够实现视觉问答-视觉回答能力，即在回答视觉问题时生成图像而非文本。


<details>
  <summary>Details</summary>
Motivation: 解决开源模型在视觉问答-视觉回答(VQ-VA)能力上的缺失，缩小与专有系统(如NanoBanana、GPT-Image)之间的差距。

Method: 构建VQ-VA World数据框架，采用代理式流水线进行大规模目标数据构建，爬取约180万高质量图像-文本交错样本用于模型训练。

Result: 训练后的LightFusion模型在IntelligentBench基准上达到53.06分，显著超越先前最佳开源基线(7.78分)，大幅缩小与领先专有系统的差距(81.67-82.64分)。

Conclusion: 通过发布完整的模型权重、数据集和流水线，旨在推动VQ-VA领域的未来研究。
...
Method: 构建参考-退化-目标三元组数据集，通过VLM选择和显式退化模拟生成模型常见的不准确问题；设计注意力对齐损失和细节编码器来精确纠正不一致性；集成到智能体框架中进行多轮局部编辑。
...
TL;DR: ShapeGen通过改进3D表示和监督、提升分辨率以及利用线性变换器的优势，实现了高质量的图像到3D形状生成，显著提升了生成质量并达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前3D形状生成方法存在缺乏细节、表面过度平滑和薄壳结构碎片化等问题，无法满足艺术家标准。

Method: 采用3D表示和监督改进、分辨率提升以及线性变换器优势相结合的方法。

Result: ShapeGen在图像到3D生成方面实现了显著飞跃，建立了新的最先进性能，生成的3D资产可以无缝集成到3D流程中。

Conclusion: 通过这些增强措施的协同效应，ShapeGen在图像到3D生成方面取得了重大突破。
...
Result: iMontage在多个主流多输入多输出任务中表现出色，不仅保持强大的跨图像上下文一致性，还能生成超出传统范围的具有非凡动态的场景。

Conclusion: iMontage成功将强大的视频模型重新用作一体化图像生成器，统一了多种图像生成和编辑任务，在保持时间连贯性的同时显著扩展了动态范围。
...
TL;DR: CAPNET是一个针对长尾多标签视觉识别的新框架，通过显式建模CLIP文本编码器中的标签相关性，结合图卷积网络和可学习软提示，在多个基准数据集上实现了最先进的性能。
...
Method: 提出相关性适应提示网络(CAPNET)，包含：图卷积网络进行标签感知传播、可学习软提示优化嵌入、分布平衡的Focal损失进行类别感知重加权、测试时集成提高泛化能力、参数高效微调对齐视觉-文本模态。

Result: 在VOC-LT、COCO-LT和NUS-WIDE等基准数据集上的大量实验表明，CAPNET相比现有最先进方法实现了显著改进。

Conclusion: CAPNET通过显式建模标签相关性和多模态对齐，有效解决了长尾多标签视觉识别问题，验证了其在现实世界应用中的有效性。
...
TL;DR: 本文提出了DataConcept数据集和概念感知批量采样框架(CABS)，通过在线概念化数据筛选方法改进视觉语言模型的训练数据选择。


<details>
  <summary>Details</summary>
Motivation: 现有数据筛选方法多为离线和概念无关的，存在数据偏见问题，需要更灵活的任务自适应在线概念化筛选方法。

Method: 构建DataConcept数据集(1.28亿图像文本对)，并提出CABS框架，包括多样性最大化(CABS-DM)和频率最大化(CABS-FM)两种批量采样策略。
...
TL;DR: VLM²是一个具有持久记忆的视觉语言模型，通过双记忆模块解决视频空间推理中的语义-几何不对齐和缺乏持久3D表示的问题，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在基于视频的空间推理中仍达不到人类水平，主要存在语义-几何不对齐和缺乏持久记忆来保持3D表示的问题。
...
Conclusion: VLM²通过持久记忆和3D感知表示有效解决了视频空间推理的关键挑战，为智能机器人的空间推理能力提供了重要进展。
...
TL;DR: 提出PixelDiT，一种单阶段端到端的像素空间扩散变换器模型，无需预训练自编码器，通过双层级设计在像素空间直接学习扩散过程，在图像生成任务上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 解决传统潜在空间扩散模型依赖两阶段流程的问题，其中预训练自编码器引入有损重建，导致误差累积并阻碍联合优化。

Method: 采用完全基于Transformer的架构，包含补丁级DiT捕获全局语义和像素级DiT细化纹理细节的双层级设计，直接在像素空间进行扩散建模。

Result: 在ImageNet 256x256上达到1.61 FID，大幅超越现有像素生成模型；在1024x1024分辨率文本到图像生成中，GenEval得分为0.74，DPG-bench得分为83.5，接近最佳潜在扩散模型性能。

Conclusion: PixelDiT证明了像素级token建模对像素扩散成功的重要性，提供了一种无需自编码器的端到端像素空间扩散解决方案，在图像生成任务上表现出色。
...
Method: 引入轻量级跨视图模块（CvM），在任务间共享，通过跨视图信息交换捕捉跨视图相关性，并与多任务编码器特征集成进行多任务预测

Result: 在NYUv2和PASCAL-Context数据集上的广泛实验表明，该方法能有效将几何一致性注入现有多任务学习方法中以提高性能

Conclusion: 通过集成跨视图相关性作为几何一致性，能够显著提升多任务学习网络在密集预测任务中的表现
...
TL;DR: 提出DPP-GRPO框架，通过结合行列式点过程和分组相对策略优化理论，解决文本到视频生成模型输出多样性不足的问题，在保持提示对齐和感知质量的同时提升视频多样性。
...
Method: 将问题建模为集合级策略优化问题，使用DPP对冗余样本施加递减回报，通过GRPO提供候选集的分组反馈，框架即插即用且模型无关。

Result: 在WAN和CogVideoX上实现，在VBench、VideoScore和人类偏好研究中一致提升视频多样性，涵盖视觉外观、相机运动和场景结构。

Conclusion: DPP-GRPO框架有效解决了文本到视频生成的多样性问题，发布了代码和包含30,000个多样化提示的新基准数据集以支持未来研究。
...
TL;DR: LocateAnything3D将3D检测转化为下一个token预测问题，通过Chain-of-Sight序列实现从2D检测到3D框预测的渐进推理，在Omni3D基准上达到49.89 AP_3D，比之前最佳方法提升15.51。
...
Method: 采用Chain-of-Sight序列，先进行2D检测作为视觉思维链，然后按从近到远的顺序预测3D框，采用中心-尺寸-旋转的因子化策略，按信息稳定性和可学习性排序。
...
<details>
  <summary>Details</summary>
Motivation: 当前自回归视频扩散模型存在三个核心限制：受限于3D-RoPE的有限时间范围、长序列生成中提示响应缓慢、以及无法在单一生成流中实现不连续的电影场景转换。
...
Result: 实验表明∞-RoPE在整体VBench评分上持续超越先前的自回归模型，实现了无限范围、可控和电影化的视频扩散。

Conclusion: ∞-RoPE为无限范围、可控和电影化视频扩散提供了一个无需训练的基础框架，解决了自回归视频扩散模型的关键限制。
...
TL;DR: RubricRL是一个基于评分标准的强化学习框架，用于对齐文本到图像生成模型与人类偏好，通过结构化、可解释的评分标准代替黑盒标量奖励。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖固定权重的复合指标或人类偏好模型蒸馏的单一标量奖励，限制了可解释性和灵活性。
...
### [193] [Discover, Learn, and Reinforce: Scaling Vision-Language-Action Pretraining with Diverse RL-Generated Trajectories](https://arxiv.org/abs/2511.19528)
*Rushuai Yang,Zhiyuan Feng,Tianxiang Zhang,Kaixin Wang,Chuheng Zhang,Li Zhao,Xiu Su,Yi Chen,Jiang Bian*

Main category: cs.RO

TL;DR: 提出了DLR框架，通过信息论模式发现生成多种不同的高成功率行为模式，用于视觉语言动作模型的预训练，解决了标准强化学习生成数据多样性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言动作模型预训练需要大量多样化、高质量的操作轨迹数据，但人类远程操作获取成本高且难以扩展。强化学习虽然能通过自主探索学习有用技能，但标准RL训练会收敛到单一执行模式，限制了其在大规模预训练中的实用性。

Method: 提出了DLR（Discover, Learn and Reinforce）框架，这是一个基于信息论的模式发现框架，能够生成多种不同的高成功率行为模式。该方法在LIBERO基准上生成明显更多样化的轨迹语料库。

Result: DLR能够为同一任务学习多种不同的高成功率策略，而标准RL只能发现一种策略，因此覆盖了更广泛的状态-动作空间。在下游未见任务套件上，使用DLR多样化RL数据预训练的VLA模型优于使用等量标准RL数据集训练的模型。

Conclusion: 多模式RL可以作为具身基础模型的实用、可扩展数据引擎，DLR展现出单模式RL所缺乏的正向数据扩展行为。
...
### [194] [A Virtual Mechanical Interaction Layer Enables Resilient Human-to-Robot Object Handovers](https://arxiv.org/abs/2511.19543)
*Omar Faris,Sławomir Tadeja,Fulvio Forni*

Main category: cs.RO

TL;DR: 提出了一种基于虚拟模型控制和增强现实的人机物体交接方法，能够适应交接过程中的动态变化并实现双向通信。


<details>
  <summary>Details</summary>
Motivation: 物体交接是协作任务中的常见交互形式，但实现高效交接仍具挑战性，需要确保机器人动作能够适应交接过程中物体姿态的复杂变化。

Method: 使用虚拟模型控制创建交互层来控制机器人并适应交接过程的动态变化，同时利用增强现实促进人机双向通信。

Result: 控制器在各种不确定性条件下表现出韧性，包括交接过程中物体姿态的复杂变化。用户研究显示参与者普遍偏好所提出的方法。

Conclusion: 该方法在人机物体交接中表现良好，研究结果为指导进一步开发适应用户交互的方法提供了见解。
...
### [195] [Robot-Powered Data Flywheels: Deploying Robots in the Wild for Continual Data Collection and Foundation Model Adaptation](https://arxiv.org/abs/2511.19647)
*Jennifer Grannen,Michelle Pan,Kenneth Llontop,Cherie Ho,Mark Zolotas,Jeannette Bohg,Dorsa Sadigh*

Main category: cs.RO

TL;DR: 提出了机器人驱动的数据飞轮框架，将机器人从基础模型消费者转变为数据生成器，通过在真实环境中部署机器人收集数据来持续改进基础模型性能


<details>
  <summary>Details</summary>
Motivation: 基础模型在真实世界非结构化环境中表现脆弱，因为训练数据缺乏真实世界的混乱数据（如遮挡或多语言文本）。机器人作为具身代理能够收集大规模真实世界数据来弥补这一差距

Method: 开发了Scanford移动机械臂系统，在图书馆部署2周自主扫描书架，使用视觉语言模型识别书籍，并利用图书馆目录自动标注图像，无需人工标注

Result: 从2103个书架收集数据，将书籍识别准确率从32.0%提升至71.8%，多语言OCR性能从24.8%提升至46.6%（英语）和30.8%提升至38.0%（中文），节省约18.7小时人工时间
...
### [196] [Online Learning-Enhanced High Order Adaptive Safety Control](https://arxiv.org/abs/2511.19651)
*Lishuo Pan,Mattia Catellani,Thales C. Silva,Lorenzo Sabattini,Nora Ayanian*

Main category: cs.RO

TL;DR: 提出了一种基于神经ODE的在线学习增强高阶自适应控制屏障函数，用于在复杂时变模型扰动下实时提高CBF认证系统的安全性，并在38g纳米四旋翼上成功验证。


<details>
  <summary>Details</summary>
Motivation: 控制屏障函数(CBFs)是保证系统安全性的有效工具，但其安全保证严重依赖于模型准确性。实际应用中如载荷变化或风扰动等复杂时变扰动会显著影响系统动力学，使安全保证失效。

Method: 使用神经ODE开发高效的在线学习增强高阶自适应控制屏障函数，结合混合自适应CBF控制器，在复杂时变模型扰动下实时调整控制策略。

Result: 在38g纳米四旋翼上成功部署，在18km/h风速下保持与障碍物的安全距离，验证了方法的有效性。
...
### [197] [Flow-Based Path Planning for Multiple Homogenous UAVs for Outdoor Formation-Flying](https://arxiv.org/abs/2511.19653)
*Mahmud Suhaimi Ibrahim,Shantanu Rahman,Muhammad Samin Hasan,Minhaj Uddin Ahmad,Abdullah Abrar*

Main category: cs.RO

TL;DR: 使用流网络方法为多无人机编队飞行规划无碰撞路径，包括构建流网络图、寻找最小成本路径和实现最大流路径规划，通过仿真和实验验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 多无人机编队飞行中，无碰撞路径规划是最关键的部分，需要解决无人机间的碰撞问题。

Method: 1) 从物理GPS坐标构建流网络图；2) 使用图路径算法寻找最小成本路径；3) 应用Ford-Fulkerson方法寻找最大流无碰撞路径。

Result: 对最多64架无人机进行了各种编队仿真，并通过3架四旋翼无人机的物理实验验证了方法的可行性和有效性。

Conclusion: 该方法能够生成安全、无碰撞的路径，在多无人机编队飞行中具有实际应用价值。
...
### [198] [Development of a Testbed for Autonomous Vehicles: Integrating MPC Control with Monocular Camera Lane Detection](https://arxiv.org/abs/2511.19655)
*Shantanu Rahman,Nayeb Hasin,Mainul Islam*

Main category: cs.RO

TL;DR: 提出了一种结合车道识别与模型预测控制(MPC)的新方法，用于提高自动驾驶汽车轨迹跟踪的精度和稳定性。该方法使用边缘识别、滑动窗口直线识别等技术提取车道线，并基于自行车车辆动力学模型构建MPC控制器。


<details>
  <summary>Details</summary>
Motivation: 随着自动驾驶车辆在道路、工业自动化、农业和军事等领域的普及，需要提高轨迹跟踪的精度和稳定性。大多数标准车辆采用阿克曼转向机制，需要优化底层控制器性能。

Method: 使用边缘识别、滑动窗口直线识别进行车道线提取，结合动态感兴趣区域(ROI)提取。基于自行车车辆动力学模型构建模型预测控制(MPC)控制器，并在ROS Gazebo中构建单车道道路仿真模型进行验证。

Result: 仿真结果显示，最优跟踪轨迹与目标轨迹之间的均方根误差降低了27.65%，证明了所开发控制器的高鲁棒性和灵活性。

Conclusion: 结合车道识别与MPC的方法能有效提高自动驾驶车辆的轨迹跟踪性能，在仿真环境中表现出良好的鲁棒性和灵活性。
...
### [199] [Multi-Agent gatekeeper: Safe Flight Planning and Formation Control for Urban Air Mobility](https://arxiv.org/abs/2511.19691)
*Thomas Marshall Vielmetti,Devansh R Agrawal,Dimitra Panagou*

Main category: cs.RO

TL;DR: 提出Multi-Agent gatekeeper框架，为3D复杂环境中的领导者-跟随者编队控制提供可证明的安全保证，通过领导者安全轨迹作为共享备份集，确保跟随者始终有安全备份机动。


<details>
  <summary>Details</summary>
Motivation: 现有方法面临权衡：在线规划器和控制器缺乏形式化安全保证，而离线规划器无法适应代理数量或期望编队的变化。需要解决这一差距。

Method: 采用混合架构，单个领导者跟踪预计算的安全轨迹作为共享轨迹备份集，跟随者执行名义编队跟踪控制器，始终拥有沿领导者路径的已知安全备份机动。

Result: 在模拟3D城市环境中，100次随机试验中实现了100%的碰撞避免成功率，显著优于基线CBF和NMPC方法，并在四旋翼团队上验证了轨迹的物理可行性。

Conclusion: 该方法为多智能体系统提供了可证明的安全保证，是门控框架在3D环境中的首次应用，成功解决了领导者-跟随者编队控制的安全协调问题。
...
### [200] [Whole-Body Inverse Dynamics MPC for Legged Loco-Manipulation](https://arxiv.org/abs/2511.19709)
*Lukas Molnar,Jin Cheng,Gabriele Fadini,Dongho Kang,Fatemeh Zargarbashi,Stelian Coros*

Main category: cs.RO

TL;DR: 提出了一种全身模型预测控制框架，通过全阶逆动力学直接优化关节扭矩，在单一预测层内实现运动与力的统一规划与执行，在四足机器人上实现80Hz实时性能。


<details>
  <summary>Details</summary>
Motivation: 全身运动操作需要协调全身运动来有效操纵物体同时保持运动稳定性，这对规划和控制都提出了重大挑战。

Method: 使用全身模型预测控制框架，通过全阶逆动力学直接优化关节扭矩，采用Pinocchio、CasADi软件框架和Fatrop求解器实现。

Result: 在配备机械臂的Unitree B2四足机器人上实现80Hz实时性能，成功完成拉重物、推箱子、擦白板等需要精确控制末端位置和力的操作任务。

Conclusion: 该MPC框架能够实现物理一致的全身行为，统一处理运动与力的规划执行，在复杂操作任务中表现出色。
...
### [201] [Unifying Perception and Action: A Hybrid-Modality Pipeline with Implicit Visual Chain-of-Thought for Robotic Action Generation](https://arxiv.org/abs/2511.19859)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于思维链的视觉-语言-动作模型在复杂空间环境中难以充分捕捉场景细节，且面临视觉观察与低级动作之间的模态差距，以及视觉预测与动作生成目标冲突导致训练不稳定的挑战。
...
Conclusion: VITA展示了作为通用机器人操作模型的潜力，通过联合建模感知和运动控制，有效解决了视觉-动作模态对齐和训练稳定性问题。
...
### [202] [Human-Centered Cooperative Control Coupling Autonomous and Haptic Shared Control via Control Barrier Function](https://arxiv.org/abs/2511.19869)
*Eito Sato,Takahiro Wada*

Main category: cs.RO

TL;DR: 提出了一种结合独立自主控制器与触觉共享控制的合作框架，通过控制屏障函数在安全区域内忽略操纵杆输入，在虚拟环境实验中相比传统触觉共享控制提高了精度并减少了所需时间。


<details>
  <summary>Details</summary>
Motivation: 当完全自主控制因不确定性或感知限制而受限时，触觉共享控制在遥操作中很有效。然而，最大化触觉共享控制强度的自主控制性能有限，因为操纵杆和人类手臂的动力学会影响机器人的行为。

Method: 提出了一个合作框架，将独立于操纵杆的自主控制器与触觉共享控制耦合。控制屏障函数在由人类操作员实时确定的安全区域内忽略操纵杆输入，而在其他情况下则启用触觉共享控制。

Result: 在虚拟环境中对遥操作水下机器人进行的模拟任务初步实验表明，相比传统触觉共享控制，该方法提高了精度并减少了所需时间。

Conclusion: 所提出的合作框架通过结合自主控制和触觉共享控制的优势，在保持操作员控制的同时提高了遥操作性能。
...
### [203] [CoC-VLA: Delving into Adversarial Domain Transfer for Explainable Autonomous Driving via Chain-of-Causality Visual-Language-Action Model](https://arxiv.org/abs/2511.19914)
*Dapeng Zhang,Fei Shen,Rui Zhao,Yinda Chen,Peng Zhi,Chenyang Li,Rui Zhou,Qingguo Zhou*

Main category: cs.RO

TL;DR: 提出CoC-VLA框架，通过对抗迁移学习将模拟环境中的长尾场景处理能力转移到真实世界自动驾驶系统


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖真实世界数据，要么依赖模拟数据，但很少能有效整合两者的互补优势。需要解决模拟到真实世界的长尾场景能力迁移问题

Method: 基于VLM的端到端对抗迁移框架，包含教师VLM、学生VLM和判别器。使用共享的CoC VLM架构，集成时间信息并通过端到端文本适配器支持链式因果推理

Result: 框架能够将模拟环境中的长尾场景处理能力有效迁移到真实世界部署

Conclusion: CoC-VLA框架成功解决了模拟到真实世界的长尾场景能力迁移问题，为自动驾驶系统在复杂场景下的性能提升提供了有效方案
...
### [204] [Collaborate sim and real: Robot Bin Packing Learning in Real-world and Physical Engine](https://arxiv.org/abs/2511.19932)
*Lidi Zhang,Han Wu,Liyu Zhang,Ruofeng Liu,Haotian Wang,Chao Li,Desheng Zhang,Yunhuai Liu,Tian He*

Main category: cs.RO

TL;DR: 提出了一种结合物理仿真和真实世界数据反馈的混合强化学习框架，用于解决3D装箱问题中的仿真到现实差距，显著降低了包装坍塌率。


<details>
  <summary>Details</summary>
Motivation: 现有方法将3D装箱问题建模为离散静态过程，而实际应用涉及连续重力驱动交互，这种理想化简化导致实际部署不可行（如不稳定包装）。物理仿真与真实世界物理属性变化之间存在差距。

Method: 采用混合强化学习框架：1）在仿真中应用领域随机化，让智能体接触各种物理参数；2）基于真实世界部署反馈对RL智能体进行微调。

Result: 在仿真和真实场景中均实现了更低的坍塌率。在物流系统中大规模部署验证了实际有效性，相比基线方法包装坍塌率降低了35%。

Conclusion: 该混合RL框架成功弥合了仿真与现实之间的差距，通过领域随机化和真实反馈微调，显著提高了3D装箱的稳定性和实际部署可行性。
...
### [205] [ShapeForce: Low-Cost Soft Robotic Wrist for Contact-Rich Manipulation](https://arxiv.org/abs/2511.19955)
...
Method: 设计软腕部装置，将外力扭矩转换为柔性核心的可测量变形，通过基于标记的位姿跟踪估计变形并转换为力类信号。无需校准或专用电子设备。

Result: 在多种接触丰富任务和操作策略的广泛实验中，ShapeForce性能与六维力扭矩传感器相当，但成本极低。

Conclusion: ShapeForce提供了一种经济有效的接触反馈解决方案，能够替代昂贵的六维力扭矩传感器，使接触丰富的机器人操作更加普及。
...
### [206] [Active3D: Active High-Fidelity 3D Reconstruction via Hierarchical Uncertainty Quantification](https://arxiv.org/abs/2511.20050)
*Yan Li,Yingzhao Li,Gim Hee Lee*

Main category: cs.RO

TL;DR: 提出了一种用于高保真3D重建的主动探索框架，通过构建多级不确定性空间和不确定性驱动的运动规划来选择最佳视角，结合隐式-显式混合表示和分层不确定性量化，在挑战性基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D重建方法在主动探索和不确定性建模方面存在不足，需要一种能够联合捕获全局结构先验和局部观测细节，并能有效量化不确定性以指导探索的方法。

Method: 采用隐式-显式混合表示融合神经场和高斯基元；构建分层不确定性体积量化全局结构质量和局部表面置信度；提出不确定性驱动的关键帧选择策略和视角空间滑动窗口；将最佳视角选择建模为期望混合信息增益问题，并集成风险敏感路径规划器。

Result: 在挑战性基准测试中，该方法在准确性、完整性和渲染质量方面均达到最先进水平，证明了其在真实世界主动重建和机器人感知任务中的有效性。

Conclusion: 所提出的主动探索框架通过有效的混合表示和不确定性驱动规划，能够实现高质量3D重建，为机器人感知和主动重建任务提供了有力工具。
...
### [207] [Hibikino-Musashi@Home 2025 Team Description Paper](https://arxiv.org/abs/2511.20180)
...
TL;DR: Hibikino-Musashi@Home团队开发了用于训练机器人视觉系统的数据集生成器、开源开发环境、基于大语言模型的任务规划器，以及脑启发记忆模型，旨在为家庭环境提供个性化机器人辅助服务。


<details>
  <summary>Details</summary>
Motivation: 设计能够在家中为人类提供辅助服务的家庭服务机器人，并通过持续参加比赛来评估和改进开发的系统。

Method: 开发数据集生成器训练机器人视觉系统；创建开源开发环境运行在HSR模拟器上；使用大语言模型驱动的任务规划器选择原始技能；研究脑启发记忆模型以适应个体家庭环境。

Result: 团队为Pumas在RoboCup2024中开发的导航系统提供了可重用性贡献，构建了完整的家庭服务机器人系统框架。

Conclusion: 该研究通过多技术融合的方法，为家庭服务机器人提供了从感知到决策的完整解决方案，支持个性化辅助服务。
...
### [208] [Toward generic control for soft robotic systems](https://arxiv.org/abs/2511.20226)
...
TL;DR: 提出了一种基于控制顺应性的通用软体机器人控制框架，通过利用而非抑制顺应性来实现鲁棒性、安全性和跨平台可迁移性。


<details>
  <summary>Details</summary>
Motivation: 软体机器人控制方法碎片化，需要任务特定的控制器，阻碍理论整合和大规模部署。刚性控制逻辑依赖精确模型和严格执行，不适用于软体机器人。

Method: 受人类运动控制启发，提出基于控制顺应性的通用框架，通过高层运动倾向表达意图，让反射和生物力学机制自主处理局部细节。

Result: 在多种形态和驱动机制的机器人上验证，展示了稳定、安全且跨平台可迁移的行为。

Conclusion: 拥抱而非抵抗控制顺应性，可能为统一的软体机器人控制提供广泛应用基础。
...
### [209] [HAFO: Humanoid Force-Adaptive Control for Intense External Force Interaction Environments](https://arxiv.org/abs/2511.20275)
*Chenhui Dong,Haozhe Xu,Wenhao Feng,Zhipeng Wang,Yanmin Zhou,Yifei Zhao,Bin He*

Main category: cs.RO

TL;DR: HAFO是一个双智能体强化学习控制框架，通过耦合训练同时优化鲁棒的运动策略和精确的上半身操作策略，在强外力交互环境下实现人形机器人的稳定控制。


<details>
  <summary>Details</summary>
Motivation: 强化学习控制器在人形机器人运动和轻负载操作方面取得了显著进展，但在强外力交互下实现鲁棒精确的运动仍然是一个重大挑战。

Method: 提出HAFO双智能体强化学习框架，通过弹簧-阻尼系统显式建模外部拉力扰动，利用虚拟弹簧实现精细力控制，并采用非对称Actor-Critic框架，其中Critic网络访问特权弹簧阻尼力来指导Actor网络学习可泛化的鲁棒策略。

Result: 实验结果表明HAFO在各种强外力交互下实现了人形机器人的稳定控制，在负载任务中表现出色，并确保机器人在绳索拉力扰动下的稳定运行。

Conclusion: HAFO框架成功解决了强外力交互下人形机器人的控制挑战，实现了鲁棒的运动和精确的操作控制，为复杂交互环境下的机器人控制提供了有效解决方案。
...
### [210] [Dynamic-ICP: Doppler-Aware Iterative Closest Point Registration for Dynamic Scenes](https://arxiv.org/abs/2511.20292)
*Dong Wang,Daniel Casado Herraez,Stefan May,Andreas Nüchter*

Main category: cs.RO

TL;DR: Dynamic-ICP是一种多普勒感知的雷达里程计方法，通过结合几何残差和旋转不变的多普勒残差，在高度动态环境中实现稳健的扫描配准。


<details>
  <summary>Details</summary>
Motivation: 传统ICP方法假设场景静态，在动态、重复或低纹理环境中性能下降，需要一种能在动态环境中可靠工作的里程计方法。

Method: 使用多普勒速度估计自车运动，聚类动态物体，预测动态点运动，结合点对面几何残差和旋转不变多普勒残差进行扫描配准。

Result: 在三个数据集上评估，Dynamic-ICP在旋转稳定性和平移精度上均优于现有方法，实时运行且易于集成。

Conclusion: Dynamic-ICP为动态环境中的稳健配准提供了轻量级解决方案，无需外部传感器或传感器-车辆标定。
...
### [211] [How Robot Kinematics Influence Human Performance in Virtual Robot-to-Human Handover Tasks](https://arxiv.org/abs/2511.20299)
*Róisín Keenan,Joost C. Dessing*

Main category: cs.RO

TL;DR: 本研究使用VR技术探索人机协作中的物体传递任务，发现人类受益于机器人提供早期视觉信息和类人平滑轨迹，这能提高预测准确性和同步性。


<details>
  <summary>Details</summary>
Motivation: 随着机器人系统越来越多地融入人类工作场所，需要优化人机协调协作，特别是在物体传递等交互任务中。

Method: 通过VR机器人传递模拟，在受控环境中分别测试四个因素对人类表现的影响：任务启动控制和机器人运动同步性、伙伴外观、机器人速度曲线、旋转物体运动时机。

Result: 实验发现人类能从机器人提供的早期视觉信息和类人平滑轨迹中获益，这些因素不同程度地提高了交互中的预测准确性和同步性。

Conclusion: 人机交互设计应让人类能够利用其检测生物运动的自然能力，这反过来可以减少昂贵的机器人计算需求或人类额外的认知适应负担。
...
### [212] [ArtiBench and ArtiBrain: Benchmarking Generalizable Vision-Language Articulated Object Manipulation](https://arxiv.org/abs/2511.20330)
*Yuhan Wu,Tiantian Wei,Shuo Wang,ZhiChao Wang,Yanyong Zhang,Daniel Cremers,Yan Xia*

Main category: cs.RO

TL;DR: 提出了ArtiBench基准测试和ArtiBrain框架，用于解决交互式关节物体操作中的泛化挑战，通过模块化设计结合高层推理和自适应底层控制，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言和基于扩散的策略在跨部件、实例和类别的关节物体操作中泛化能力不足，需要解决长时程多步骤交互的物理一致性挑战。

Method: 提出ArtiBrain模块化框架，包含VLM任务推理器进行子目标分解和验证，混合控制器结合几何感知关键帧执行和可供性引导扩散，以及持续积累执行经验的Affordance Memory Bank。

Result: 在ArtiBench基准测试上的广泛实验表明，ArtiBrain在鲁棒性和泛化能力上显著优于最先进的多模态和基于扩散的方法。

Conclusion: ArtiBrain通过统一高层推理和自适应底层控制，有效解决了关节物体操作中的泛化挑战，为交互式操作提供了可解释和精确的解决方案。
...
### [213] [Quality-guided UAV Surface Exploration for 3D Reconstruction](https://arxiv.org/abs/2511.20353)
*Benjamin Sportich,Kenza Boubakri,Olivier Simonin,Alessandro Renzaglia*

Main category: cs.RO

TL;DR: 提出了一种新型模块化最佳视点规划框架，利用重建质量目标指导无人机探索规划，通过自适应视点生成和选择方法，在覆盖范围、3D地图质量和路径效率方面优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 自主机器人在未知环境中的建图需求广泛，但现有规划策略往往忽视不同应用场景的特定要求。快速信息收集和建筑结构综合评估需要不同的方法。

Method: 提出模块化最佳视点规划框架，使用重建质量目标指导探索规划。引入高效的自适应视点生成和候选视点选择方法，充分利用TSDF环境表示中的不确定性信息。

Result: 在真实环境模拟中验证，该方法成功根据用户目标调整行为，在覆盖范围、最终3D地图质量和路径效率方面持续优于传统最佳视点策略。

Conclusion: 所提出的框架能够根据预定义目标做出知情且高效的探索决策，为不同质量要求的应用场景提供定制化解决方案。
...
### [214] [Improved adaptive wind driven optimization algorithm for real-time path planning](https://arxiv.org/abs/2511.20394)
*Shiqian Liu,Azlan Mohd Zain,Le-le Mao*

Main category: cs.RO

TL;DR: 提出了一种改进的风驱动优化算法MAWDO，通过分层引导机制在动态环境中实现更好的路径规划效果，相比现有方法缩短路径长度3.51%-14.93%，获得更平滑的轨迹。


<details>
  <summary>Details</summary>
Motivation: 动态环境中的实时适应性是自主导航的关键挑战，风驱动优化算法因其物理可解释的搜索动态而成为有前景的框架。

Method: 基于风驱动优化原理，提出多分层自适应风驱动优化(MAWDO)，采用分层引导机制将种群划分为多个组，由个体、区域和全局领导者引导，平衡探索与利用。

Result: 在16个基准函数上评估显示MAWDO获得优越的优化精度、收敛稳定性和适应性。在动态路径规划中，路径长度缩短至469.28像素，相比MEWDO、AWDO和WDO分别提升3.51%、11.63%和14.93%，最优性差距最小(1.01)，平滑度为0.71。

Conclusion: MAWDO在复杂环境中生成更平滑、更短且无碰撞的轨迹，证实了其在实时路径规划中的有效性。
...
### [215] [Power-Efficient Autonomous Mobile Robots](https://arxiv.org/abs/2511.20467)
*Liangkai Liu,Weisong Shi,Kang G. Shin*

Main category: cs.RO

TL;DR: pNav是一个新型的功率管理系统，通过联合优化自主移动机器人的物理/机械和网络子系统，显著提高了功率/能量效率。


<details>
  <summary>Details</summary>
Motivation: 通过分析AMR的功耗特性，发现实现CPS功率效率面临三个挑战：系统功耗分解的可变性、环境感知导航的局部性、以及网络和物理子系统的协调问题。
...
Conclusion: pNav系统有效解决了AMR的功率效率问题，通过联合优化网络和物理子系统实现了显著的功耗降低。
...
### [216] [Kleinkram: Open Robotic Data Management](https://arxiv.org/abs/2511.20492)
*Cyrill Püntener,Johann Schwabe,Dominique Garmier,Jonas Frey,Marco Hutter*

Main category: cs.RO

TL;DR: Kleinkram是一个开源的机器人数据管理系统，提供可扩展的存储、索引和共享功能，支持ROS bags和MCAP格式，集成Docker工作流进行数据处理。


<details>
  <summary>Details</summary>
Motivation: 解决大规模非结构化机器人数据集管理的挑战，支持从单个实验到大规模研究数据集的存储和共享需求。

Method: 采用模块化本地云解决方案，集成S3兼容存储，内置Action Runner执行基于Docker的自定义工作流，提供Web界面和CLI。

Result: 成功管理超过30TB来自各种机器人系统的数据，简化了研究生命周期。

Conclusion: Kleinkram为机器人研究社区提供了一个高效、灵活的数据管理解决方案。
...
### [217] [Metric, inertially aligned monocular state estimation via kinetodynamic priors](https://arxiv.org/abs/2511.20496)
*Jiaxin Liu,Min Li,Wanting Xu,Liang Li,Jiaqi Yang,Laurent Kneip*

Main category: cs.RO

TL;DR: 提出了一种将现有刚体姿态估计方法扩展到非刚性系统的框架，通过结合学习变形力模型和连续时间B样条运动模型，建立视觉轨迹加速度与变形诱导加速度之间的物理联系。


<details>
  <summary>Details</summary>
Motivation: 柔性机器人系统的精确状态估计面临重大挑战，特别是对于动态变形结构使刚体假设失效的平台。需要将现有刚体姿态估计方法扩展到非刚性系统。

Method: 使用多层感知机学习注入式变形-力模型捕捉弹性特性；采用连续时间B样条运动模型求解平台平滑运动；通过牛顿第二定律建立视觉轨迹加速度与预测变形诱导加速度的物理联系。

Result: 方法不仅能在非刚性平台上实现鲁棒准确的姿态估计，而且正确建模的平台物理特性激发了惯性传感特性。在简单弹簧相机系统上验证了可行性，解决了单目视觉里程计中度量尺度和重力恢复的典型病态问题。

Conclusion: 该方法成功将刚体姿态估计扩展到非刚性系统，通过物理建模实现了鲁棒的状态估计，并解决了单目视觉里程计中的关键挑战。
...
### [218] [Gated Uncertainty-Aware Runtime Dual Invariants for Neural Signal-Controlled Robotics](https://arxiv.org/abs/2511.20570)
*Tasha Kim,Oiwi Parker Jones*

Main category: cs.RO

TL;DR: GUARDIAN是一个用于神经信号控制机器人的实时神经符号验证框架，通过结合置信度校准的脑信号解码和符号目标基础，在低精度解码器下实现94-97%的安全率。


<details>
  <summary>Details</summary>
Motivation: 安全关键的辅助系统需要可靠性和可信度的严格保证，特别是直接从神经信号解码用户意图的系统。

Method: GUARDIAN框架结合了置信度校准的脑信号解码、符号目标基础和双层运行时监控，强制执行逻辑安全和生理信任。

Result: 在BNCI2014 EEG数据集上，即使使用轻量级解码器（测试精度27-46%，ECE置信度校准误差0.22-0.41），系统仍能达到94-97%的安全率，在模拟噪声测试中比基线提高1.7倍正确干预。

Conclusion: GUARDIAN在100Hz频率和亚毫秒延迟下运行，对信号退化具有渐进响应，并产生从意图到行动的可审计轨迹，将神经证据与可验证的机器人行动联系起来。
...
### [219] [Safe and Stable Neural Network Dynamical Systems for Robot Motion Planning](https://arxiv.org/abs/2511.20593)
*Allen Emmanuel Binny,Mahathi Anand,Hugo T. M. Kussaba,Lingyun Chen,Shreenabh Agrawal,Fares J. Abu-Dakka,Abdalla Swikir*

Main category: cs.RO

TL;DR: 提出S²-NNDS框架，通过神经网络学习动态系统，同时学习Lyapunov稳定性和屏障安全证书，为复杂非线性任务提供安全稳定的运动学习。
...
Method: 使用神经网络学习动态系统，同时学习神经Lyapunov稳定性和屏障安全证书，通过分割保形预测提供概率保证。

Result: 在2D和3D数据集（包括LASA手写和Franka Emika Panda机器人演示）上的实验验证了S²-NNDS在学习鲁棒、安全、稳定运动方面的有效性。

Conclusion: S²-NNDS能够从可能不安全的演示中学习到安全稳定的机器人运动，优于传统的限制性多项式参数化方法。
...
### [220] [Reinforcing Action Policies by Prophesying](https://arxiv.org/abs/2511.20633)
...
<details>
  <summary>Details</summary>
Motivation: 现有的VLA策略主要通过模仿学习训练，容易过拟合演示数据且在分布偏移时表现脆弱。强化学习能直接优化任务奖励但真实机器人交互成本高，传统仿真器难以构建和迁移。

Method: 提出ProphRL框架：1) Prophet - 在大规模异构机器人数据上预训练的动作到视频世界模型，可快速适应新机器人、物体和环境；2) FA-GRPO - 针对流式动作头的强化学习算法；3) FlowScale - 逐步重新加权方法，重新缩放流式头中的每步梯度。

Result: 在公开基准测试中成功率提升5-17%，在真实机器人上提升24-30%，验证了方法的有效性和泛化能力。

Conclusion: ProphRL为VLA后训练提供了一个实用、数据和计算效率高的路径，通过世界模型和定制化强化学习解决了模仿学习的局限性。
...
### [221] [Using Wearable Devices to Improve Chronic PainTreatment among Patients with Opioid Use Disorder](https://arxiv.org/abs/2511.19577)
...
<details>
  <summary>Details</summary>
Motivation: 慢性疼痛和鸦片使用障碍是相互关联的常见慢性疾病，目前缺乏针对接受药物治疗的鸦片使用障碍患者的循证综合治疗方案。可穿戴设备有潜力监测复杂患者信息并为治疗开发提供信息。

Method: 使用可穿戴设备监测患者数据，应用多种AI方法（包括机器学习模型和大语言模型）来预测疼痛峰值。

Result: 机器学习模型在预测疼痛峰值方面取得了相对较高的准确率（>0.7），而大语言模型在提供疼痛峰值见解方面表现有限。

Conclusion: 通过可穿戴设备的实时监测结合先进的AI模型，可以促进疼痛峰值的早期检测，支持个性化干预措施。鉴于大语言模型整体表现有限，这些发现强调了开发能够在此领域提供可操作见解的大语言模型的必要性。
...
### [222] [Fara-7B: An Efficient Agentic Model for Computer Use](https://arxiv.org/abs/2511.19663)
...
<details>
  <summary>Details</summary>
Motivation: 解决计算机使用代理（CUA）发展中缺乏大规模高质量数据集的问题，填补了与LLM文本数据相当的CUA轨迹数据空白。

Method: 开发FaraGen系统生成合成数据：从常用网站提出多样化任务，生成多个解决方案尝试，使用多个验证器过滤成功轨迹。基于这些数据训练Fara-7B模型，该模型仅通过截图感知计算机，通过预测坐标执行动作。

Result: FaraGen以约1美元的成本生成每个验证轨迹，实现高吞吐量、产量和多样性。Fara-7B在WebVoyager、Online-Mind2Web和WebTailBench等基准测试中优于同类规模的CUA模型，且能与更大的前沿模型竞争。

Conclusion: 可扩展的数据生成系统在推进小型高效代理模型方面具有关键优势，Fara-7B模型已开源发布，同时发布了新的WebTailBench基准测试。
...
### [223] [HeaRT: A Hierarchical Circuit Reasoning Tree-Based Agentic Framework for AMS Design Optimization](https://arxiv.org/abs/2511.19669)
*Souradip Poddar,Chia-Tung Ho,Ziming Wei,Weidong Cao,Haoxing Ren,David Z. Pan*

Main category: cs.AI

TL;DR: HeaRT是一个基础推理引擎，用于自动化设计循环，在电路复杂性增加时仍能保持>97%的推理准确率和>98%的Pass@1性能，同时使用比SOTA基线少50%的计算资源。
...
Result: 在40个电路基准测试中，HeaRT在电路复杂性增加时仍保持>97%的推理准确率和>98%的Pass@1性能，计算资源消耗仅为SOTA基线的0.5倍，在尺寸和拓扑设计适应任务中实现3倍以上的收敛速度提升。

Conclusion: HeaRT是向智能自适应设计优化迈出的重要一步，能够保持先前设计意图的同时显著提高设计效率。
...
### [224] [FISCAL: Financial Synthetic Claim-document Augmented Learning for Efficient Fact-Checking](https://arxiv.org/abs/2511.19671)
...
<details>
  <summary>Details</summary>
Motivation: 当前金融应用中的大语言模型存在事实可靠性不足和计算效率低下的问题，需要开发更准确且高效的解决方案。

Method: 提出FISCAL框架生成金融合成数据，并训练MiniCheck-FISCAL轻量级验证器用于金融数字声明的事实核查。

Result: MiniCheck-FISCAL在多个金融数据集上表现优异，超越GPT-3.5 Turbo等模型，接近Mixtral-8x22B等更大模型的准确性，在外部数据集上可与GPT-4o和Claude-3.5相媲美。
...
### [225] [Scaling Item-to-Standard Alignment with Large Language Models: Accuracy, Limits, and Solutions](https://arxiv.org/abs/2511.19749)
*Farzan Karimi-Malekabadi,Pooya Razavi,Sonya Powers*

Main category: cs.AI

TL;DR: 研究表明大型语言模型（特别是GPT-4o-mini）在识别教育评估项目与内容标准对齐方面表现良好，准确率达83-94%，结合候选技能预筛选策略可显著减轻人工审核负担。


<details>
  <summary>Details</summary>
Motivation: 传统人工对齐审核准确但耗时费力，特别是在大规模项目库中。研究旨在探索LLMs是否能加速这一过程而不牺牲准确性。

Method: 使用超过12,000个K-5年级项目-技能对，测试三种LLM模型（GPT-3.5 Turbo、GPT-4o-mini和GPT-4o）在三个任务上的表现：识别未对齐项目、从完整标准集中选择正确技能、以及在分类前缩小候选列表。

Result: GPT-4o-mini在识别对齐状态方面正确率约83-94%；数学表现强劲，阅读表现较低；预筛选候选技能使正确技能出现在前五建议中的概率超过95%。

Conclusion: LLMs，特别是结合候选筛选策略，能显著减少项目审核的人工负担同时保持对齐准确性。建议开发混合流程，将基于LLM的筛选与人工审核结合，为持续项目验证和教学对齐提供可扩展解决方案。
...
### [226] [Scaling Agentic Reinforcement Learning for Tool-Integrated Reasoning in VLMs](https://arxiv.org/abs/2511.19773)
...
TL;DR: VISTA-Gym是一个可扩展的训练环境，通过工具集成的视觉推理能力激励视觉语言模型进行多步视觉交互推理。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在图像理解方面表现出色，但在多步视觉交互推理方面能力有限，特别是在工具选择、调用和协调方面存在困难。

Method: VISTA-Gym统一了多样化的多模态推理任务，提供标准化的视觉工具接口、可执行交互循环、可验证反馈信号和高效轨迹记录，通过多轮轨迹采样和端到端强化学习训练VISTA-R1模型。
...
### [227] [NOEM$^{3}$A: A Neuro-Symbolic Ontology-Enhanced Method for Multi-Intent Understanding in Mobile Agents](https://arxiv.org/abs/2511.19780)
...
<details>
  <summary>Details</summary>
Motivation: 解决移动AI代理中多意图理解的问题，通过符号对齐策略实现准确高效的设备端自然语言理解，减少对大型语言模型的依赖。
...
### [228] [KOM: A Multi-Agent Artificial Intelligence System for Precision Management of Knee Osteoarthritis (KOA)](https://arxiv.org/abs/2511.19798)
...
Method: 开发KOM多智能体系统，自动化KOA评估、风险预测和治疗处方生成，基于患者个体特征、疾病状态、风险因素和禁忌症制定个性化管理计划。

Result: 基准实验显示KOM在影像分析和处方生成方面优于通用大语言模型；三臂随机模拟研究表明KOM与临床医生协作可将诊断和规划时间减少38.5%，并提高治疗质量。

Conclusion: KOM有助于实现KOA管理的自动化，集成到临床工作流程中可提升护理效率，其模块化架构为开发其他慢性病的AI辅助管理系统提供了宝贵见解。
...
### [229] [A Unified Evaluation-Instructed Framework for Query-Dependent Prompt Optimization](https://arxiv.org/abs/2511.19829)
*Ke Chen,Yifeng Wang,Hassan Almosapeeh,Haohan Wang*

Main category: cs.AI

TL;DR: 本文提出了一个基于评估指导的提示优化框架，通过系统化的提示质量评估和多维度评分来指导查询相关的提示重写，解决了传统静态模板优化在复杂动态场景中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法主要依赖静态模板，在复杂动态用户场景中效果有限；现有查询相关方法依赖不稳定的文本反馈或黑盒奖励模型，提供弱且不可解释的优化信号；提示质量本身缺乏统一系统定义，导致评估信号碎片化不可靠。

Method: 首先建立性能导向的系统化提示评估框架，开发并微调无需执行的评估器，直接从文本预测多维度质量分数；然后使用评估器指导指标感知的优化器，以可解释、查询相关的方式诊断失败模式并重写提示。
...
### [230] [Reinforcement Learning with $ω$-Regular Objectives and Constraints](https://arxiv.org/abs/2511.19849)
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习依赖标量奖励，表达能力有限，容易导致奖励破解，且无法明确表达时间性、条件性或安全关键目标。同时，单一标量性能指标掩盖了安全性-性能之间的权衡。

Method: 开发了基于线性规划的模型强化学习算法，将ω-正则目标与约束分离处理，在极限情况下生成满足ω-正则约束阈值的同时最大化目标满足概率的策略。

Result: 建立了到约束极限平均问题的转换，并提供了最优性保证。

Conclusion: 该方法能够同时处理ω-正则目标和约束，有效解决安全性-性能权衡问题，为强化学习提供了更丰富的表达能力。
...
### [231] [MicroSims: A Framework for AI-Generated, Scalable Educational Simulations with Universal Embedding and Adaptive Learning Support](https://arxiv.org/abs/2511.19864)
*Valerie Lockhart,Dan McCreary,Troy A. Peterson*

Main category: cs.AI

TL;DR: MicroSims是一个用于创建轻量级交互式教育模拟的框架，通过AI辅助生成、iframe架构和可定制代码，解决了传统模拟开发成本高、技术复杂的问题。


<details>
  <summary>Details</summary>
Motivation: 传统教育模拟需要大量资源和技术专长，限制了其广泛应用。该研究旨在降低创建交互式教育模拟的门槛，提高教育公平性。

Method: 提出标准化设计模式支持AI生成，使用iframe架构实现通用嵌入和安全沙盒，提供透明可修改代码支持定制。

Result: 基于物理教育和STEM研究的实证表明，交互模拟相比传统教学可将概念理解提升30-40%。MicroSims在保持这些益处的同时解决了成本、技术复杂性和平台依赖等障碍。

Conclusion: MicroSims框架为全球教育工作者提供了按需创建定制化、与课程对齐的模拟的能力，对教育公平和低成本智能交互教科书发展具有重要意义。
...
### [232] [Agentic AI-Empowered Conversational Embodied Intelligence Networks in 6G](https://arxiv.org/abs/2511.19865)
...
<details>
  <summary>Details</summary>
Motivation: 6G时代多具身智能设备需要语义协作执行复杂任务，但现有系统在多模态信息融合、自适应通信和决策可解释性方面存在局限。

Method: CC-EIN包含四个核心模块：PerceptiNet进行图像和雷达数据的跨模态融合生成统一语义表示；自适应语义通信策略根据任务紧急性和信道质量动态调整编码方案和传输功率；语义驱动协作机制支持任务分解和异构设备无冲突协调；InDec模块通过Grad-CAM可视化增强决策透明度。
...
### [233] [Simulated Self-Assessment in Large Language Models: A Psychometric Approach to AI Self-Efficacy](https://arxiv.org/abs/2511.19872)
*Daniel I Jackson,Emma L Jensen,Syed-Amad Hussain,Emre Sezgin*

Main category: cs.AI

TL;DR: 本文通过改编通用自我效能感量表，评估了10个大型语言模型在四种条件下的模拟自我评估，发现模型自我评估与真实能力不匹配，且高自我效能感对应更拟人化的推理风格。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型评估主要关注任务准确性，而忽视了自我评估这一可靠智能的关键方面。研究旨在探索LLMs的模拟自我评估能力及其与真实表现的关系。

Method: 改编10项通用自我效能感量表，在四种条件下（无任务、计算推理、社交推理、摘要）对10个LLMs进行评估，分析自我评估的稳定性、与任务表现的关系，并进行定性分析。

Result: 模型自我评估在不同条件下差异显著，总体低于人类标准；所有模型在计算和社交任务中准确率完美，但摘要表现差异大；自我评估不能可靠反映能力，高自信模型可能表现差；后续信心提示导致适度下调，表明初步评估存在轻度高估。

Conclusion: 心理测量提示为LLM沟通行为提供了结构化洞察，但不能提供校准的性能估计；高自我效能感对应更自信、拟人化的推理风格，低分则反映谨慎、去拟人化的解释。
...
### [234] [RPM-MCTS: Knowledge-Retrieval as Process Reward Model with Monte Carlo Tree Search for Code Generation](https://arxiv.org/abs/2511.19895)
...
Method: 提出RPM-MCTS方法：1）利用知识检索作为过程奖励模型评估中间步骤；2）扩展阶段使用相似性过滤去除冗余节点；3）利用沙箱执行反馈定位和纠正错误步骤。

Result: 在四个公开代码生成基准测试中，RPM-MCTS优于当前最先进方法，同时减少约15%的token消耗。使用RPM-MCTS构建的数据进行全微调可显著提升基础模型的代码能力。

Conclusion: RPM-MCTS提供了一种有效的方法来提升代码生成质量，通过避免复杂的过程奖励模型训练和及时的错误纠正，在减少计算成本的同时实现了更好的性能。
...
### [235] [Semantic-KG: Using Knowledge Graphs to Construct Benchmarks for Measuring Semantic Similarity](https://arxiv.org/abs/2511.19925)
...
<details>
  <summary>Details</summary>
Motivation: 当前评估LLM文本输出的语义相似性方法存在缺陷，可能更关注句法或词汇形式而非语义内容，且现有基准依赖主观人工判断、生成成本高、领域适用性有限、等价定义不清晰。

Method: 利用知识图谱生成语义相似或不相似的自然语言陈述对，将不相似对分为四种子类型，在四个不同领域（通用知识、生物医学、金融、生物学）生成基准数据集，并比较传统NLP评分和LLM-as-a-judge预测等语义相似性方法。

Result: 研究发现语义变化的子类型以及基准领域都会影响语义相似性方法的性能，没有哪种方法始终表现最优。

Conclusion: 研究结果对使用LLM-as-a-judge检测文本语义内容具有重要启示，表明需要根据具体应用场景选择合适的语义相似性评估方法。
...
### [236] [A System-Level Taxonomy of Failure Modes in Large Language Model Applications](https://arxiv.org/abs/2511.19933)
*Vaishali Vinay*

Main category: cs.AI

TL;DR: 本文提出了LLM在实际应用中的15种隐藏故障模式分类法，分析了现有评估方法的不足，并提供了构建可靠LLM系统的设计原则。
...
Method: 提出系统级的故障模式分类法，分析现有评估与监控实践的差距，并研究LLM部署的生产挑战。

Result: 识别了15种隐藏故障模式，揭示了评估方法在稳定性、可重现性和工作流集成方面的不足，并提出了可靠LLM系统的设计原则。
...
### [237] [M$^3$Prune: Hierarchical Communication Graph Pruning for Efficient Multi-Modal Multi-Agent Retrieval-Augmented Generation](https://arxiv.org/abs/2511.19969)
*Weizi Shao,Taolin Zhang,Zijie Zhou,Chen Chen,Chengyu Wang,Xiaofeng He*

Main category: cs.AI

TL;DR: 提出M³Prune框架，通过层次化图剪枝消除多模态多智能体系统中的冗余通信边，在保持任务性能的同时显著降低token开销。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统虽然性能出色，但存在显著的token开销和计算成本问题，难以大规模部署。

Method: 采用层次化图剪枝：先进行模态内图稀疏化识别关键边，然后构建动态通信拓扑进行模态间图稀疏化，最后逐步剪枝冗余边。

Result: 在通用和领域特定mRAG基准测试中，该方法始终优于单智能体和鲁棒多智能体系统，同时显著减少token消耗。

Conclusion: M³Prune框架实现了任务性能和token开销之间的最佳平衡，为多模态多智能体系统的高效部署提供了有效解决方案。
...
### [238] [Reducing Latency of LLM Search Agent via Speculation-based Algorithm-System Co-Design](https://arxiv.org/abs/2511.20048)
...
TL;DR: SPAgent是一个算法-系统协同设计框架，通过推测执行减少基于LLM的搜索代理的延迟，在保持准确性的同时实现最高1.65倍的端到端加速。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的搜索代理虽然性能强大，但存在严重的延迟问题，因为每个步骤都需要串行的LLM推理和工具执行。传统推测执行范式虽然能打破串行执行，但收益有限，因为它保留了完整的工作负载并增加了额外的推理开销。

Method: SPAgent引入了两阶段自适应推测机制，在安全时选择性省略验证；系统层面采用两级调度器根据引擎负载调节推测请求，确保推测始终有益。

Result: 在广泛的实验设置中，SPAgent实现了最高1.65倍的端到端加速，同时保持相同甚至更高的准确性。

Conclusion: SPAgent通过算法-系统协同设计，显著降低了多步骤搜索代理的延迟，使其能够实际部署使用。
...
### [239] ["Are We Done Yet?": A Vision-Based Judge for Autonomous Task Completion of Computer Use Agents](https://arxiv.org/abs/2511.20067)
*Marta Sumyk,Oleksandr Kosovan*

Main category: cs.AI

TL;DR: 提出了一个基于视觉语言模型的自主评估框架，通过屏幕截图直接评估计算机使用代理的任务完成情况，在macOS应用中达到73%的检测准确率，使任务成功率相对提高27%。


<details>
  <summary>Details</summary>
Motivation: 计算机使用代理在自主操作数字界面时，往往难以可靠判断任务是否完成，需要有效的评估反馈机制来提高可靠性。

Method: 使用视觉语言模型直接从屏幕截图和任务描述中评估任务完成情况，构建了涵盖42个macOS应用和1260个人工标注任务的数据集。

Result: 任务成功检测准确率达到73%，应用评估器反馈后整体任务成功率相对提高27%。
...
### [240] [VICoT-Agent: A Vision-Interleaved Chain-of-Thought Framework for Interpretable Multimodal Reasoning and Scalable Remote Sensing Analysis](https://arxiv.org/abs/2511.20085)
...
Method: 采用基于堆栈的推理结构和模块化MCP兼容工具套件，使LLM能高效执行多轮交错视觉语言推理任务；提出推理堆栈蒸馏方法将复杂代理行为迁移到轻量模型中。
...
### [241] [From data to concepts via wiring diagrams](https://arxiv.org/abs/2511.20138)
*Jason Lo,Mohammadnima Jafari*

Main category: cs.AI

TL;DR: 本文介绍了准骨架布线图的概念，证明其与Hasse图对应，并设计了从序列数据中提取布线图的算法，在分析自主代理游戏行为中成功识别获胜策略。


<details>
  <summary>Details</summary>
Motivation: 布线图是表示抽象概念（如时间过程）的标记有向图，需要开发从序列数据中自动提取布线图的方法来理解复杂系统的行为模式。

Method: 提出准骨架布线图概念并证明其与Hasse图的对应关系，设计基于此的布线图提取算法，与DBSCAN和凝聚层次聚类算法进行对比。

Result: 算法在分析自主代理游戏行为时正确识别了获胜策略，在数据扰动情况下仍保持良好性能。

Conclusion: 本文成功将范畴论、图论、聚类、强化学习和数据工程技术相结合，为从序列数据中提取结构化表示提供了有效方法。
...
### [242] [Towards Benign Memory Forgetting for Selective Multimodal Large Language Model Unlearning](https://arxiv.org/abs/2511.20196)
*Zhen Zeng,Leijiang Gu,Zhangling Duan,Feng Li,Zenglin Shi,Cees G. M. Snoek,Meng Wang*

Main category: cs.AI

TL;DR: 提出了Sculpted Memory Forgetting Adapter (SMFA)方法，通过微调模型将敏感响应替换为拒绝回答，并使用保留锚点引导的掩蔽机制，实现精确可控的多模态大语言模型隐私信息遗忘，同时保持模型的通用图像理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型(MLLMs)可能会无意中记忆隐私敏感信息，而现有的遗忘方法虽然可以移除这些知识，但往往会降低模型的通用图像理解性能，无法实现良性遗忘。

Method: SMFA首先微调模型将敏感响应替换为拒绝回答，得到记忆遗忘适配器，然后应用保留锚点引导的掩蔽机制，防止对无关知识和理解能力的干扰。

Result: 广泛的实验表明，与先前方法不同，SMFA实现了精确可控的遗忘，同时保持了模型的基础图像理解能力。

Conclusion: SMFA方法能够精确控制多模态大语言模型的隐私信息遗忘，在移除敏感知识的同时有效保留模型的通用视觉理解能力，为解决MLLMs隐私保护问题提供了有效方案。
...
### [243] [Interactive AI NPCs Powered by LLMs: Technical Report for the CPDC Challenge 2025](https://arxiv.org/abs/2511.20200)
...
Conclusion: 提出的简单有效框架在常识角色对话挑战中表现出色，证明了方法的有效性。
...
### [244] [CostNav: A Navigation Benchmark for Cost-Aware Evaluation of Embodied Agents](https://arxiv.org/abs/2511.20216)
...
TL;DR: CostNav是首个将导航研究指标与商业可行性联系起来的经济测试平台，通过成本收益分析评估自主送货机器人的经济可行性，揭示了任务成功率优化与经济部署优化的根本差异。


<details>
  <summary>Details</summary>
Motivation: 现有导航基准只关注任务成功率，忽略了商业部署所需的经济可行性。自主送货机器人在商业部署中需要考虑完整的成本收益分析。

Method: CostNav建立了一个微观导航经济测试平台，模拟完整的经济生命周期，包括硬件、训练、能源、维护成本和送货收入，使用行业参数进行成本收益分析。

Result: 基准测试显示43.0%的服务水平协议合规率，但商业上不可行：每次运行亏损30.009美元，碰撞导致的维护成本占总成本的99.7%。

Conclusion: CostNav填补了导航研究与商业部署之间的差距，为基于规则的导航、模仿学习和成本感知强化学习训练提供了评估基础，使研究人员能够做出关于经济权衡的数据驱动决策。
...
### [245] [Actionable and diverse counterfactual explanations incorporating domain knowledge and causal constraints](https://arxiv.org/abs/2511.20236)
*Szymon Bobek,Łukasz Bałec,Grzegorz J. Nalepa*

Main category: cs.AI

TL;DR: 提出DANCE方法，通过整合特征依赖关系和因果约束来生成多样、可操作且知识约束的反事实解释，确保反事实的合理性和现实可行性。


<details>
  <summary>Details</summary>
Motivation: 现有反事实解释方法往往忽略现实数据集中的复杂依赖关系，导致生成不现实或不可行的修改。受网络安全和电子邮件营销应用的启发，需要确保反事实的合理性和现实可行性。

Method: DANCE方法从数据中学习线性和非线性约束，或整合专家提供的依赖图，确保反事实合理且可操作。通过保持特征关系的一致性，生成符合现实约束的解释，并平衡合理性、多样性和稀疏性。

Result: 在140个公共数据集上的广泛评估表明，该方法能够生成有意义、领域相关的反事实，在广泛使用的指标上优于其他现有方法。

Conclusion: DANCE方法有效解决了现有算法在生成合理、多样且稀疏反事实方面的关键限制，为网络安全和电子邮件营销等实际应用提供了可行的解决方案。
...
### [246] [SMoG: Schema Matching on Graph](https://arxiv.org/abs/2511.20285)
*Mingyu Jeon,Jaeyoung Suh,Suwan Cho*

Main category: cs.AI

TL;DR: SMoG是一个基于知识图谱的schema匹配框架，通过迭代执行简单的1跳SPARQL查询来增强LLM在医疗数据集成中的性能，同时提高可解释性和降低存储需求。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在schema匹配中的幻觉问题和缺乏最新领域知识的问题，同时克服现有KG增强方法中复杂多跳查询和存储密集型检索的局限性。

Method: 采用迭代执行简单1跳SPARQL查询的方法，受KGQA成功策略启发，直接查询SPARQL端点，生成可验证的查询路径。

Result: 在真实医疗数据集上的实验表明，SMoG达到了与最先进基线方法相当的性能，验证了其在KG增强schema匹配中的有效性和效率。

Conclusion: SMoG框架通过简单高效的KG查询策略，成功解决了LLM在医疗schema匹配中的关键挑战，为数据集成提供了可靠且可解释的解决方案。
...
### [247] [Improving Language Agents through BREW](https://arxiv.org/abs/2511.20297)
*Shashank Kirtania,Param Biyani,Priyanshu Gupta,Yasharth Bajpai,Roshni Iyer,Sumit Gulwani,Gustavo Soares*

Main category: cs.AI

TL;DR: BREW框架通过构建和精化经验学习知识库来优化LLM智能体，在保持计算效率的同时提升任务精度和减少API调用。
...
Method: 引入BREW框架，通过知识库构建和精化来优化智能体，采用任务评分器和行为准则学习洞察，利用状态空间搜索确保鲁棒性。

Result: 在真实世界基准测试中，BREW实现10-20%的任务精度提升，10-15%的API调用减少，执行时间更快，计算效率与基础模型相当。
...
### [248] [Data Augmentation Techniques to Reverse-Engineer Neural Network Weights from Input-Output Queries](https://arxiv.org/abs/2511.20312)
*Alexander Beiser,Flavio Martinelli,Wulfram Gerstner,Johanni Brea*

Main category: cs.AI

TL;DR: 本文提出新的数据增强技术来改进神经网络参数逆向工程，能够恢复比训练数据参数多100倍的网络。


<details>
  <summary>Details</summary>
Motivation: 现有方法在教师网络参数数量超过训练数据时失败，因为学生会过拟合查询数据而不是对齐参数。需要更好的采样技术来获取教师网络隐藏层的丰富表示。

Method: 设计专门针对网络隐藏层表示空间采样的新数据增强技术，相比标准增强技术（旋转、翻转、加噪）效果更好。

Result: 新增强技术扩展了可恢复网络大小的最先进范围，能够恢复参数比训练数据点多100倍的网络。

Conclusion: 专门设计的表示空间增强技术能显著提升网络参数逆向工程的性能，突破了现有方法的规模限制。
...
Conclusion: 主动推理可以在不依赖期望自由能的情况下实现，通过约束散度最小化框架提供更清晰的数学基础。
...
### [250] [NNGPT: Rethinking AutoML with Large Language Models](https://arxiv.org/abs/2511.20333)
...
TL;DR: NNGPT是一个开源框架，将大语言模型转变为自改进的AutoML引擎，用于神经网络开发，通过生成新模型、评估和自改进的闭环系统实现持续优化。
...
### [251] [VibraVerse: A Large-Scale Geometry-Acoustics Alignment Dataset for Physically-Consistent Multimodal Learning](https://arxiv.org/abs/2511.20422)
*Bo Pang,Chenxi Xu,Jierui Ren,Guoping Wang,Sheng Li*

Main category: cs.AI

TL;DR: VibraVerse是一个大规模几何-声学对齐数据集，通过明确连接3D几何→物理属性→模态参数→声学信号的因果链，为物理一致的多模态学习提供基准。


<details>
  <summary>Details</summary>
Motivation: 现有多模态学习框架缺乏物理一致性，忽视了物体几何、材料、振动模态和产生声音之间的内在因果关系。需要建立基于物理定律而非统计相关性的感知模型。

Method: 引入VibraVerse数据集，每个3D模型都有明确的物理属性和体积几何，从中计算模态特征用于冲击声合成。提出CLASP对比学习框架，在跨模态对齐中保持物理结构到声学响应的因果对应关系。
...
### [252] [DRAFT-RL: Multi-Agent Chain-of-Draft Reasoning for Reinforcement Learning-Enhanced LLMs](https://arxiv.org/abs/2511.20468)
*Yuanhao Li,Mingshan Liu,Hongbo Wang,Yiding Zhang,Yifei Ma,Wei Tan*

Main category: cs.AI

TL;DR: DRAFT-RL是一个新颖的多智能体强化学习框架，通过集成链式草稿推理来改进LLM的多步推理能力，在代码合成、符号数学和知识密集型问答任务中显著优于现有方法。
...
Method: 提出DRAFT-RL框架：每个智能体为每个查询生成多个草稿，由同伴智能体和学习的奖励模型评估，选择最有希望的轨迹，通过actor-critic学习优化未来推理策略。

Result: 在复杂推理任务（代码合成、符号数学、知识密集型QA）上，DRAFT-RL在准确性和收敛速度方面显著优于现有的反思和基于RL的智能体。

Conclusion: DRAFT-RL通过显式的多路径探索、同伴引导反思和奖励对齐选择，实现了更鲁棒和可解释的LLM智能体行为。
...
### [253] [Universe of Thoughts: Enabling Creative Reasoning with Large Language Models](https://arxiv.org/abs/2511.20471)
*Yuto Suzuki,Farnoush Banaei-Kashani*

Main category: cs.AI

TL;DR: 该论文提出了一个基于大语言模型的创造性推理框架，包含组合、探索和转化三种推理范式，并通过Universe of Thoughts方法实现，在需要创新解决方案的领域表现出色。
...
Method: 提出了基于认知科学原理的计算框架，包含组合、探索和转化三种创造性推理范式，并开发了Universe of Thoughts方法来实现这些过程。

Result: 与现有最先进的推理技术和商业模型相比，UoT在创造性推理方面表现出优越性能。

Conclusion: 该研究填补了LLM在创造性推理方面的空白，为需要创新解决方案的领域提供了有效的推理方法。
...
### [254] [Quantifying the Privacy Implications of High-Fidelity Synthetic Network Traffic](https://arxiv.org/abs/2511.20497)
*Van Tran,Shinan Liu,Tian Li,Nick Feamster*

Main category: cs.AI

TL;DR: 本文提出了针对合成网络流量的隐私度量方法，评估了不同生成模型的隐私风险，发现MIA成功率可达88%，网络标识符恢复率可达100%，并识别了影响攻击成功的关键因素。


<details>
  <summary>Details</summary>
Motivation: 解决合成网络流量数据稀缺和隐私问题，但目前缺乏对合成流量泄露敏感信息的程度和测量方法的研究。

Method: 引入了一套全面的隐私度量指标，结合标准方法（如MIA和数据提取攻击）与网络特定标识符和属性，系统评估不同代表性生成模型的脆弱性。

Result: 结果显示不同模型和数据集间的隐私风险存在显著差异：MIA成功率从0%到88%，网络标识符恢复率可达100%，揭示了严重的隐私漏洞。

Conclusion: 识别了影响攻击结果的关键因素（训练数据多样性和生成模型拟合度），为设计最小化隐私泄露的生成模型提供了可行指导，建立了更安全的合成网络流量生成基础。
...
### [255] [FRAGMENTA: End-to-end Fragmentation-based Generative Model with Agentic Tuning for Drug Lead Optimization](https://arxiv.org/abs/2511.20510)
*Yuto Suzuki,Paul Awolade,Daniel V. LaBarbera,Farnoush Banaei-Kashani*

Main category: cs.AI

TL;DR: FRAGMENTA是一个用于药物先导化合物优化的端到端框架，包含生成模型和智能体AI系统，通过动态Q学习和对话反馈解决小数据集分子生成问题，在癌症药物发现实验中表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 分子生成在药物发现中至关重要，但类别特定数据集通常少于100个训练样本。现有基于片段的模型虽然比基于原子的方法更好处理有限数据，但启发式片段化限制了多样性且遗漏关键片段。此外，模型调优需要药物化学家和AI工程师之间缓慢的间接协作。

Method: 1) 新颖的生成模型，将片段化重构为"词汇选择"问题，使用动态Q学习联合优化片段化和生成；2) 智能体AI系统，通过领域专家的对话反馈精炼目标，从循环中移除AI工程师，逐步学习领域知识最终实现自动化调优。

Result: 在真实世界癌症药物发现实验中，FRAGMENTA的人类-智能体配置识别的高分分子数量几乎是基线的两倍。完全自主的智能体-智能体系统优于传统的人类-人类调优，证明了智能体调优在捕捉专家意图方面的有效性。

Conclusion: FRAGMENTA框架通过创新的片段化方法和智能体调优系统，在小数据集分子生成和药物先导化合物优化方面取得了显著进展，展示了智能体调优在捕捉专家意图方面的潜力，有望实现药物发现过程的自动化。
...
### [256] [Assessing LLMs' Performance: Insights from the Chinese Pharmacist Exam](https://arxiv.org/abs/2511.20526)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在数字健康教育和评估工作流程中的集成度提高，需要探索其在支持高风险、领域特定认证任务方面的能力。中国国家药师执业考试为评估药师临床和理论能力提供了标准化基准。

Method: 从官方考试、培训材料和公共数据库中收集了2,306道纯文本选择题（2017-2021年），排除含表格或图片的题目。以原始中文格式输入问题，评估模型响应的准确率，使用皮尔逊卡方检验比较整体表现，费舍尔精确检验分析年度选择题准确率。

Result: DeepSeek-R1整体准确率显著高于ChatGPT-4o（90.0% vs 76.1%，p < 0.001）。单元级分析显示DeepSeek-R1在基础和临床综合模块中具有一致优势。虽然年度选择题表现也倾向于DeepSeek-R1，但在任何特定单元-年份中这种性能差距未达到统计学显著性。

Conclusion: DeepSeek-R1在药师执业考试的结构和语义要求方面表现出强大的对齐能力。这些发现表明领域特定模型在此背景下值得进一步研究，同时也强调了在法律和伦理敏感情境中人类监督的必要性。
...
### [257] [Beyond Generation: Multi-Hop Reasoning for Factual Accuracy in Vision-Language Models](https://arxiv.org/abs/2511.20531)
*Shamima Hossain*

Main category: cs.AI

TL;DR: 提出了一个基于知识图谱的视觉语言模型推理框架，通过多跳验证提升事实准确性，在初步实验中事实准确性提高了约31%。
...
Method: 使用结构化知识图谱进行多跳验证，通过图像描述任务展示框架。包括视觉实体识别、知识图谱遍历和基于事实的描述优化等系统推理步骤，评估了层次化、三元组和项目符号三种知识表示方法。

Result: 在Google Landmarks v2、Conceptual captions和Coco captions混合数据集上的初步实验显示，事实准确性提高了约31%，揭示了推理模式和失败模式的关键见解。

Conclusion: 这项工作展示了整合外部知识推进视觉语言模型推理的潜力，为构建更可靠、知识更丰富的多模态系统铺平了道路。
...
### [258] [PaTAS: A Parallel System for Trust Propagation in Neural Networks Using Subjective Logic](https://arxiv.org/abs/2511.20586)
...
<details>
  <summary>Details</summary>
Motivation: 传统评估指标如准确率无法捕捉模型预测的不确定性或可靠性，特别是在对抗性或退化条件下。需要可解释、可量化的信任评估系统来确保AI系统在安全关键应用中的可信度。
...
Conclusion: PaTAS为在神经网络架构中实现透明和可量化的信任推理提供了原则性基础，能够在AI生命周期中评估模型可靠性。
...
### [259] [Building a Foundation Model for Trajectory from Scratch](https://arxiv.org/abs/2511.20610)
*Gaspard Merten,Mahmoud Sakr,Gilles Dejaegere*

Main category: cs.AI

TL;DR: 本教程通过从GPT-2开始的最小实现，展示了构建轨迹基础模型的步骤和代码，并比较了TrajFM、TrajGPT等代表性模型，旨在帮助SIGSPATIAL社区理解和构建移动性基础模型。
...
Method: 采用代码驱动的分步过程，将GPT-2适配到时空数据，并比较代表性轨迹基础模型的架构创新和差异，同时引入TimesFM的补丁方法等补充技术。

Result: 成功演示了轨迹基础模型的最小实现，提供了概念和术语的解释，支持社区构建和评估移动性基础模型。

Conclusion: 创建这一教育材料对于支持SIGSPATIAL社区构建和评估移动性基础模型至关重要，能够提高移动性AI研究的清晰度和同行评审的有效性。
...
### [260] [Copyright Detection in Large Language Models: An Ethical Approach to Generative AI Development](https://arxiv.org/abs/2511.20623)
...
<details>
  <summary>Details</summary>
Motivation: LLMs广泛使用引发版权内容未经授权纳入训练数据的担忧，现有检测框架计算密集且对独立创作者不可及，需要可扩展、透明、用户友好的解决方案。

Method: 通过优化API调用效率，改进相似性检测和数据集验证，提供直观用户界面和可扩展后端。

Result: 实现了计算开销减少10-30%，提高了易用性和相似性检测能力。
...
### [261] [Fighting AI with AI: Leveraging Foundation Models for Assuring AI-Enabled Safety-Critical Systems](https://arxiv.org/abs/2511.20627)
*Anastasia Mavridou,Divya Gopinath,Corina S. Păsăreanu*

Main category: cs.AI

TL;DR: 提出了REACT和SemaLens两个互补组件，利用AI技术解决安全关键系统中AI组件集成的验证挑战，构建从非正式需求到验证实现的全流程解决方案。


<details>
  <summary>Details</summary>
Motivation: AI组件（特别是深度神经网络）在安全关键系统中的集成面临验证困难，AI系统的不透明性以及高层需求与低层网络表示之间的语义鸿沟阻碍了传统验证方法。

Method: REACT使用大语言模型连接非正式自然语言需求和形式化规范，支持早期验证；SemaLens利用视觉语言模型基于人类可理解概念对基于DNN的感知系统进行推理、测试和监控。

Result: 构建了从非正式需求到验证实现的完整流程，解决了AI系统验证中的语义鸿沟和可扩展性问题。

Conclusion: 通过AI技术本身来解决AI系统验证挑战是可行的，REACT和SemaLens为安全关键系统中AI组件的可信集成提供了有效途径。
...
### [262] [Power sector models featuring individual BEV profiles: Assessing the time-accuracy trade-off](https://arxiv.org/abs/2511.19449)
*Adeline Guéret*
...
### [263] [Strong Duality and Dual Ascent Approach to Continuous-Time Chance-Constrained Stochastic Optimal Control](https://arxiv.org/abs/2511.19451)
...
<details>
  <summary>Details</summary>
Motivation: 解决连续时间连续空间随机最优控制问题中状态约束的违反概率限制问题，避免保守近似，直接处理机会约束。

Method: 利用退出时间概念将机会约束转化为期望函数，通过Hamilton-Jacobi-Bellman偏微分方程的对偶公式，采用路径积分方法进行梯度上升求解。

Result: 在特定系统动态和成本函数假设下，证明了原始机会约束问题与其对偶问题之间的强对偶性，并通过移动机器人空间导航的仿真验证了方法的有效性。
...
### [264] [A Data-Driven Model Predictive Control Framework for Multi-Aircraft TMA Routing Under Travel Time Uncertainty](https://arxiv.org/abs/2511.19452)
*Yi Zhang,Yushen Long,Liping Huang,Yicheng Zhang,Sheng Zhang,Yifang Yin*

Main category: eess.SY

TL;DR: 提出了一个用于终端机动区域多机无冲突路由和调度的闭环框架，通过混合整数优化和模型预测控制来减少拥堵、提高着陆效率。
...
Method: 采用混合整数优化模型和滚动时域模型预测控制策略，结合扩展的50海里半径终端机动区域网络，整合安全间隔、速度调整和等待时间约束。

Result: 在高峰拥堵期间计算时间减少7倍，使用新加坡ADS-B数据集验证了计算效率，蒙特卡洛模拟证实了框架的鲁棒性。

Conclusion: 该框架具有操作弹性和计算可扩展性，通过实时优化和自适应重新规划为空中交通管制员提供可行的决策支持。
...
### [265] [A K-means Inspired Solution Framework for Large-Scale Multi-Traveling Salesman Problems](https://arxiv.org/abs/2511.19454)
*Xiubin Chen*

Main category: eess.SY

TL;DR: 提出基于K-means的任务分配框架，将多旅行商问题重构为空间约束分类过程，显著降低计算复杂度，适用于大规模多智能体系统。


<details>
  <summary>Details</summary>
Motivation: 传统优化方法在处理大规模多智能体任务分配时计算成本过高，难以满足无人系统大规模协调需求。

Method: 采用"先聚类后路由"的分解策略，利用空间一致性实现快速路径成本估计和高效任务分组。

Result: 在1000个智能体和5000个目标的大规模场景中仍能保持高质量解，计算效率显著提升。

Conclusion: 该聚类优先的分解策略为大规模多智能体任务分配提供了高效可靠的解决方案。
...
### [266] [Active Secure Neighbor Selection in Multi-Agent Systems with Byzantine Attacks](https://arxiv.org/abs/2511.19522)
*Jinming Gao,Yijing Wang,Wentao Zhang,Rui Zhao,Yang Shi,Zhiqiang Zuo*

Main category: eess.SY

TL;DR: 提出一种基于主动安全邻居选择框架的多智能体系统弹性控制方法，通过构建预判别图和动态邻居选择策略，在拜占庭攻击下实现弹性共识，显著降低通信负担。


<details>
  <summary>Details</summary>
Motivation: 解决多智能体系统在拜占庭攻击下的弹性控制问题，传统方法通常假设正常智能体初始连通性，且通信开销较大，需要更高效且不依赖初始连通性的解决方案。

Method: 首先构建预判别图定义候选邻居集合，然后提出动态入邻居选择策略，每个智能体主动选择预判别邻居的子集，选择数量可调节以平衡通信开销和鲁棒性。

Result: 该方法能够在检测和隔离拜占庭智能体后重建正常智能体间的有向生成树，实现弹性共识，且不要求正常智能体初始连通性，显著降低通信负担。

Conclusion: 所提出的主动安全邻居选择框架有效解决了多智能体系统在拜占庭攻击下的弹性控制问题，在降低通信开销的同时保持了对攻击的鲁棒性。
...
### [267] [State Feedback Controllers with Operational Constraints](https://arxiv.org/abs/2511.19683)
*Eugene Lavretsky*

Main category: eess.SY

TL;DR: 本文为多输入多输出线性时不变系统开发了具有最小/最大运行限制约束的状态反馈控制设计，解决了带有输入和输出约束的伺服跟踪控制问题。


<details>
  <summary>Details</summary>
Motivation: 解决多输入多输出线性系统在伺服跟踪控制中面临的输入和输出约束问题，特别是为航空飞行关键系统提供可靠的控制解决方案。

Method: 基于Nagumo定理的前向不变性、比较引理处理输入/输出不等式约束，以及最小范数最优控制器进行综合设计，包含静态和动态伺服控制器两种方案。

Result: 提出的控制方法能够有效处理运行限制约束，并为具有积分作用的动态伺服控制器提供抗饱和保护逻辑。

Conclusion: 该方法与控制屏障函数方法相似且直接相关，仿真研究表明其在航空飞行关键系统中具有显著优势。
...
### [268] [Understanding Risk and Revenue in the Nordic 15-minute mFRR market: An EV Aggregation Study](https://arxiv.org/abs/2511.19715)
*Theodor Hagström,Lars Herre*

Main category: eess.SY

TL;DR: 本文研究了电动汽车车队参与北欧15分钟mFRR能源激活市场的商业模式，通过风险感知随机优化协调日前调度与季度mFRR投标，提高了预期利润并降低了下跌风险。


<details>
  <summary>Details</summary>
Motivation: 脱碳化、去中心化和间歇性推动了灵活性市场向更短市场时间单位发展，更短的MTU和关闸时间降低了需求侧聚合商的进入壁垒。

Method: 将车队灵活性表示为具有时变功率和能量包络的虚拟电池，并制定风险感知随机优化，协调日前调度与季度mFRR投标。

Result: 在两种价格情景下，协同优化增加了预期利润并降低了下跌风险：模型在日前购买更少能源，将采购转向mFRR下调，同时平整充电计划以保持mFRR上调资格。

Conclusion: 利润分解显示提升主要来自更高的mFRR下调收入和减少对平仓日前头寸的依赖，讨论了投标的操作影响并提出了两个扩展：滚动45分钟重新优化和V2G框架。
...
### [269] [Multi-Hypotheses Ego-Tracking for Resilient Navigation](https://arxiv.org/abs/2511.19770)
*Peter Iwer Hoedt Karstensen,Roberto Galeazzi*

Main category: eess.SY

TL;DR: 提出了一种结合多假设估计和泊松二项窗口计数检测器的弹性导航架构，用于识别和隔离异常，通过状态机协调操作、诊断和缓解之间的转换，在检测到攻击时基于微分平坦度进行轨迹重规划。


<details>
  <summary>Details</summary>
Motivation: 依赖射频定位的自主机器人容易受到欺骗和传感器操纵攻击，需要开发能够抵御此类攻击的弹性导航系统。

Method: 采用多假设估计与泊松二项窗口计数检测器相结合的方法进行异常识别和隔离，使用状态机管理操作模式转换，在攻击检测时基于微分平坦度进行轨迹重规划。

Result: 案例研究表明该系统能有效检测偏置传感器，保持状态估计，并在持续欺骗攻击下恢复正常运行。

Conclusion: 所提出的弹性导航架构能够有效应对射频定位系统的欺骗攻击，通过自适应响应机制确保机器人在对抗环境下的可靠运行。
...
### [270] [An Exact Solution Algorithm for the Bi-Level Optimization Problem of Electric Vehicles Charging Station Placement](https://arxiv.org/abs/2511.19884)
*Mobina Nankali,Michael W. Levin*

Main category: eess.SY

TL;DR: 本文提出了首个用于大规模电动汽车充电站布局问题的精确求解算法，采用分支定价割平面法，在合理时间内获得全局最优解，相比现有方法计算速度提升两个数量级。


<details>
  <summary>Details</summary>
Motivation: 现有研究依赖启发式方法无法保证最优性，或精确算法运行时间过长，需要开发能在合理时间内提供数学保证的全局最优解的精确算法。

Method: 建立双层优化模型，上层规划者在预算约束下选择充电站位置最大化净收益，下层EV用户选择路线和充电站最小化出行和充电成本。提出分支定价割平面算法，结合价值函数割和列生成技术。

Result: 在多个真实网络上的计算实验显示，算法在几分钟内终止，所有实例的最优性差距均低于1%，相比现有方法计算速度提升超过两个数量级，成功处理超过30万个可行组合的问题。

Conclusion: 该算法将电动汽车充电基础设施规划从计算上不可行的问题转变为可处理的优化任务，适用于实际决策问题。
...
### [271] [Toward Trustworthy Digital Twins in Agentic AI-based Wireless Network Optimization: Challenges, Solutions, and Opportunities](https://arxiv.org/abs/2511.19961)
*Zhenyu Tao,Wei Xu,Xiaohu You*

Main category: eess.SY

TL;DR: 提出了一个统一的数字孪生评估框架，用于确保基于智能AI的网络优化中可信赖的数字孪生，从传统的物理精度指标转向更全面的任务中心评估。


<details>
  <summary>Details</summary>
Motivation: 现代无线网络优化面临高动态性和复杂性挑战，智能AI在真实环境中训练成本高且风险大，而数字孪生技术提供了安全训练环境，但其有效性取决于保真度。

Method: 引入统一数字孪生评估框架，从孤立物理精度指标转向任务中心评估，作为设计、选择和生命周期管理的指导原则。

Result: 在真实无线网络测试平台上的案例研究表明，该评估框架能有效预筛选候选数字孪生，显著降低训练和测试成本而不牺牲部署性能。

Conclusion: 该评估框架为无线网络数字孪生提供了有效的设计和选择指导，并讨论了潜在的研究机会。
...
### [272] [Assessing the Technical and Environmental Impacts of Energy Management Systems in Smart Ports](https://arxiv.org/abs/2511.20043)
*Youzhe Yang,Hafiz Majid Hussain,Juha Haakana,Pedro Nardelli*

Main category: eess.SY

TL;DR: 本文评估了智能港口能源管理系统(EMS)的实施效果，发现其能显著降低能耗、碳排放和运营成本，但也面临系统集成、数据质量等挑战。


<details>
  <summary>Details</summary>
Motivation: 港口需要应对海事行业的环境影响，遵守欧洲绿色协议和可持续发展目标等框架，因此需要系统实施全面的能源管理解决方案。

Method: 首先对汉堡、热那亚、裕廊和上海洋山四期等主要港口的案例研究进行系统文献回顾，然后构建优化模型模拟负荷调度、碳减排和运输调度。

Result: EMS部署使年能耗降低约7%-8%，碳排放减少11%-12%，同时实现30%的成本节约。
...
### [273] [Quantum-Enhanced Reinforcement Learning for Accelerating Newton-Raphson Convergence with Ising Machines: A Case Study for Power Flow Analysis](https://arxiv.org/abs/2511.20237)
*Zeynab Kaseb,Matthias Moller,Lindsay Spoor,Jerry J. Guo,Yu Xiang,Peter Palensky,Pedro P. Vergara*

Main category: eess.SY

TL;DR: 提出了一种结合强化学习和量子退火的新方法，用于优化牛顿-拉夫逊法在电力潮流计算中的初始化过程，显著提高了收敛速度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统牛顿-拉夫逊法在不良初始化或极端运行场景下性能下降，特别是在高可再生能源渗透率的情况下，传统初始化策略往往导致收敛缓慢甚至发散。

Method: 使用强化学习优化NR初始化，并引入量子增强的RL环境更新机制，将电压调整任务建模为二次无约束二进制优化问题，利用量子/数字退火器评估状态转移。

Result: 结果显示收敛速度显著提升，NR迭代次数减少，在不同运行条件下具有更强的鲁棒性。

Conclusion: 量子增强的强化学习方法能够有效解决传统NR方法在复杂电力系统场景下的初始化问题，为电力系统分析提供了更高效的解决方案。
...
### [274] [Occlusion-Aware Multi-Object Tracking via Expected Probability of Detection](https://arxiv.org/abs/2511.20239)
*Jan Krejčí,Oliver Kost,Yuxuan Xia,Lennart Svensson,Ondřej Straka*

Main category: eess.SY

TL;DR: 提出了一种考虑物体间遮挡的多目标系统跟踪方法，通过增强标准点目标模型，为每个对象分配基于减少Palm密度的期望检测概率，系统性地处理所有对象的不确定性。


<details>
  <summary>Details</summary>
Motivation: 在多目标系统中，物体之间可能相互遮挡，影响传感器的检测概率。现有方法未能系统性地考虑所有物体存在带来的不确定性。

Method: 增强标准点目标检测模型，为每个对象分配基于减少Palm密度的期望检测概率，使用带标记的多伯努利混合(MBM)滤波器进行视觉跟踪。

Result: 该方法能够清晰且可管理地考虑所有物体的不确定性，相比现有方法更系统地处理遮挡问题。

Conclusion: 提出的方法为多目标跟踪中的遮挡问题提供了原则性的解决方案，通过期望检测概率有效处理物体间的相互影响。
...
### [275] [LLM-Driven Transient Stability Assessment: From Automated Simulation to Neural Architecture Design](https://arxiv.org/abs/2511.20276)
*Lianzhe Hu,Yu Wang,Bikash Pal*

Main category: eess.SY

TL;DR: 本文提出了一个基于LLM的端到端工作流，用于电力系统暂态稳定评估(TSA)，通过LLM驱动框架自动生成扰动场景并优化TSA模型，在IEEE 39节点系统中实现了93.71%的测试精度和实时推理能力。
...
Method: 结合大型语言模型(LLMs)与专业仿真器(ANDES)，通过LLM驱动的神经网络设计(LLM-NND)管道自动设计和优化TSA模型，采用性能引导的闭环反馈机制。

Result: 在IEEE 39节点系统上，LLM-NND模型仅用4.78M参数就实现了93.71%的四类TSA测试精度，推理延迟小于0.95ms/样本，优于手动设计的DenseNet(25.9M参数，80.05%精度)。

Conclusion: LLM代理能够可靠地加速TSA研究，从场景生成到模型设计，提供了一个可扩展的范式，可扩展到其他电力系统任务。
...
### [276] [SAFE-IMM: Robust and Lightweight Radar-Based Object Tracking on Mobile Platforms](https://arxiv.org/abs/2511.20294)
*Dnyandeep Mandaokar,Bernhard Rinner*

Main category: eess.SY

TL;DR: 提出了SAFE-IMM方法，一种轻量级的IMM变体，通过安全协方差感知门控机制，在资源受限平台上实现快速响应且平滑的目标跟踪


<details>
  <summary>Details</summary>
Motivation: 传统IMM滤波器在目标机动时存在滞后问题，而winner-takes-all方法响应快但会产生不连续性，需要在响应性和平滑性之间找到平衡

Method: 使用安全协方差感知门控，仅当从混合模型到获胜模型的跳跃被证明有界时才允许WTA，保持数值稳定性和抗干扰性

Result: 在仿真和nuScenes前雷达数据上，SAFE-IMM实现了高精度实时跟踪，减少了ID切换，同时保持竞争性能

Conclusion: SAFE-IMM提供了一种在响应性和平滑性之间实用的平衡方案，易于集成且计算高效
...
### [277] [Accelerating Time-Optimal Trajectory Planning for Connected and Automated Vehicles with Graph Neural Networks](https://arxiv.org/abs/2511.20383)
*Viet-Anh Le,Andreas A. Malikopoulos*

Main category: eess.SY

TL;DR: 提出基于图神经网络的学习框架，加速联网自动驾驶车辆的时间和能量最优轨迹规划，通过离线学习生成在线预测作为数值优化的热启动解。


<details>
  <summary>Details</summary>
Motivation: 解决多智能体协调问题中的实时轨迹规划挑战，需要在保证安全约束的同时实现计算效率。

Method: 使用图神经网络学习离线生成的时间最优轨迹规划解，在线预测作为数值优化的热启动，结合重规划机制融入新观测信息。

Result: 学习增强方法显著减少计算时间，同时确保满足所有状态、输入和安全约束。

Conclusion: 该框架实现了联网自动驾驶车辆在多智能体场景下的实时时间和能量最优轨迹规划。
...
### [278] [Adaptive Meshing for CPA Lyapunov Function Synthesis](https://arxiv.org/abs/2511.20443)
*Amy K. Strong,Samuel Akinwande,Leila Bridgeman*

Main category: eess.SY

TL;DR: 本文研究了更高效的网格划分方法，以减少连续分段仿射Lyapunov函数合成的计算成本。


<details>
  <summary>Details</summary>
Motivation: 连续分段仿射Lyapunov函数合成方法在更细的网格上计算成本较高，特别是在高维系统中，需要寻找更高效的网格划分策略。

Method: 探索了三种网格划分方法：自适应网格划分、利用系统模型知识的网格划分，以及两者的组合方法。

Result: 通过二维和三维非线性动力系统的数值例子比较了三种方法的有效性。

Conclusion: 提出了更高效的网格划分策略，能够降低Lyapunov函数合成的计算复杂度。
...
### [279] [Learning Control Barrier Functions with Deterministic Safety Guarantees](https://arxiv.org/abs/2511.20463)
*Amy K. Strong,Ali Kashani,Claus Danielson,Leila Bridgeman*

Main category: eess.SY

TL;DR: 本文提出了一种使用ReLU神经网络构建连续分段仿射障碍函数的方法，通过数据驱动优化为非线性动力系统提供确定性安全保证。


<details>
  <summary>Details</summary>
Motivation: 为非线性（可能未建模）非自治动力系统计算有效的安全集和障碍函数是一个困难任务，需要数据驱动的方法来获得具有确定性控制不变性保证的安全集。

Method: 利用ReLU神经网络构建连续分段仿射障碍函数，通过采样的一步轨迹数据，采用迭代凸上界方法求解非凸优化问题，包含新颖的分类器项来创建松弛的障碍函数条件。

Result: 在二维自治和非自治动力系统上验证了方法的有效性，能够获得具有确定性安全保证的安全集。

Conclusion: 该方法成功地将神经网络与凸优化技术结合，为非线性动力系统提供了数据驱动的安全验证框架。
...
### [280] [Causal Feature Selection for Weather-Driven Residential Load Forecasting](https://arxiv.org/abs/2511.20508)
*Elise Zhang,François Mirallès,Stéphane Dellacherie,Di Wu,Benoit Boulet*

Main category: eess.SY

TL;DR: 本文研究因果特征选择在短期负荷预测中的应用，比较不同特征选择方法对预测精度和鲁棒性的影响。


<details>
  <summary>Details</summary>
Motivation: 天气是居民用电需求的主要外部驱动因素，但添加过多气象协变量会增加模型复杂度并可能降低精度。需要建立原则性的特征选择框架来优化预测性能。

Method: 使用因果特征选择方法PCMCI，与无特征选择和非因果选择方法进行比较，在三个时间序列预测模型（GRU、TCN、PatchTST）上评估，基于南安大略地区的IESO用电数据和ERA5天气再分析数据。

Result: 非因果选择优先选择辐射和湿度等相关性变量，而因果选择强调更直接的热驱动因素并修剪间接协变量，在预测精度和极端天气鲁棒性方面表现良好。

Conclusion: 因果特征选择是现代预测器在集成天气信息到居民负荷预测时的实用补充方法，能够提高模型的简洁性和鲁棒性。
...
### [281] [From Features to States: Data-Driven Selection of Measured State Variables via RFE-DMDc](https://arxiv.org/abs/2511.20552)
*Haoyu Wang,Andrea Alfonsi,Roberto Ponciroli,Richard Vilim*

Main category: eess.SY

TL;DR: RFE-DMDc是一种数据驱动的工作流，使用递归特征消除选择最小化的物理意义变量集，并通过动态模态分解构建线性状态空间模型，在多组件系统中有效避免特征遮蔽问题。
...
Method: 使用递归特征消除选择最小变量集，然后通过动态模态分解构建线性状态空间模型，包含跨子系统选择步骤以缓解多组件系统中的特征遮蔽问题。

Result: 在RLC基准测试和集成能源系统上，RFE-DMDc能够恢复紧凑的状态集（约10个变量），测试误差与GA-DMDc相当，但计算时间减少一个数量级。

Conclusion: 所选变量在子系统间保持清晰的物理解释，生成的模型具有竞争力的预测精度、计算效率和抗过拟合鲁棒性。
...
### [282] [Exploring Urban Air Mobility Adoption Potential in San Francisco Bay Area Region: A Systems of Systems Level Case Study on Passenger Waiting Times and Travel Efficiency](https://arxiv.org/abs/2511.20603)
*Winfrey Paul Sagayam Dennis*

Main category: eess.SY

TL;DR: 该研究评估了旧金山湾区城市空中交通(UAM)的可行性，通过多智能体仿真比较了UAM与传统地面交通的出行时间，发现在高峰时段使用UAM可将总出行时间减少高达80%。
...
Result: 在高峰需求时段使用UAM可将区域内的总出行时间减少高达80%。机队规模、乘客请求量和周转时间直接影响等待时间、运营成本和用户接受度。

Conclusion: 研究确定了机队调度的关键运营因素，为UAM在旧金山湾区的系统级采用提供了可行性分析。
