...
TL;DR: 提出一种针对目标检测模型的动态掩码后门攻击方法，利用SAM分割模型生成动态触发器，在蘑菇检测数据集上实现高攻击成功率


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在计算机视觉任务中广泛应用，但面临各种对抗攻击威胁，特别是后门攻击。传统后门攻击使用静态触发器模式，容易被检测。需要开发更隐蔽的动态攻击方法来揭示外包实践中的安全风险

Method: 提出动态掩码后门攻击方法：1）使用数据集投毒技术嵌入恶意触发器；2）利用SAM图像分割模型生成掩码进行动态触发器放置；3）在蘑菇检测数据集上构建详细攻击场景；4）使用YOLOv7目标检测模型进行评估

Result: 实验表明该方法在干净数据上保持高精度，在投毒样本上实现高攻击成功率。动态掩码方法超越传统的静态模式后门注入，展示了更隐蔽的攻击效果

Conclusion: 动态掩码后门攻击对目标检测模型构成严重威胁，特别是对蘑菇检测等关键领域。研究强调了保护深度学习模型免受此类进化对抗攻击的紧迫性，需要开发鲁棒的对策
...
<details>
  <summary>Details</summary>
Motivation: 尽管动态NeRF在说话人像建模方面取得了进展，但在准确高效捕捉口部运动方面仍存在挑战，需要提升说话人像的保真度
...
Conclusion: 提出的基于眨眼嵌入和哈希网格地标编码的方法显著提升了说话人像的保真度，解决了口部运动捕捉的准确性和效率问题
...
TL;DR: 提出一种结合3DMM和StyleGAN的方法，仅用自拍视频实现高细节的头像重演，能恢复皱纹、发丝等高频细节。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在三个主要问题：1) 3DMM方法无法实时捕捉整个头部（包括非面部区域和背景）；2) GAN方法难以再现皱纹、发丝等细粒度细节；3) 现有方法通常需要大量训练数据，很少关注仅用简单自拍视频实现头像重演。

Method: 结合3DMM和基于StyleGAN的生成器，提出详细重建模型，在对抗训练中采用混合损失函数进行前景重建和头像图像生成，以恢复高频细节。

Result: 在自重演和交叉重演任务上的定性和定量评估表明，该方法相比现有方法实现了更优越的头像重建，具有更丰富和复杂的纹理细节。

Conclusion: 该方法成功解决了现有头像重演方法的局限性，仅用自拍视频就能实现高质量、高细节的头像重演，在纹理细节恢复方面表现优异。
...
TL;DR: 提出基于检测网络架构的弱监督补丁级预训练方法，用于增强北极驯鹿检测的鲁棒性，在多个数据集上取得优于通用权重初始化的性能。


<details>
  <summary>Details</summary>
Motivation: 北极驯鹿数量下降需要可扩展的监测方法，但人工解释航空图像劳动密集且易错，自动检测面临背景异质性、类别不平衡、目标小/遮挡、密度和尺度变化等挑战。

Method: 提出弱监督补丁级预训练方法，基于检测网络架构，使用空与非空标签进行预训练，为检测网络提供早期弱监督知识，然后迁移到检测任务。

Result: 在多个驯鹿群数据集上，弱监督预训练在补丁级检测（F1: 92.6%/93.5% vs 89.3%/88.6%）和全图像计数（F1: 95.5%/93.3% vs 91.5%/90.4%）上均优于ImageNet权重初始化。

Conclusion: 在检测前进行粗标签预训练，即使在标注数据有限的情况下，也能依赖弱监督预训练权重获得与通用权重初始化相当甚至更好的结果，为大规模航空图像监测提供可靠方法。
...
Method: 利用多个现有检测器的优势，结合免训练统计量，计算多个测试统计量的p值，使用经典统计集成方法评估与真实图像分布的匹配度

Result: 提出通用、灵活、免训练的统计框架，适用于多样化和不断演化的场景中的鲁棒假图像检测
...
TL;DR: 该研究通过大规模实证分析发现，在手术视觉基础模型中引入深度信息进行多模态预训练，能显著提升各项任务性能，并实现卓越的数据效率，且无需推理时的架构修改。


<details>
  <summary>Details</summary>
Motivation: 当前手术视觉基础模型主要依赖单模态RGB预训练，忽略了手术环境的复杂3D几何结构。虽然通用计算机视觉中有多模态或几何感知输入架构，但在手术场景中深度信息的益处尚未充分探索。

Method: 研究比较了8种基于ViT的视觉基础模型，这些模型在预训练领域、学习目标和输入模态（RGB vs RGB-D）上有所不同。使用包含140万张机器人手术图像及深度图的数据集进行预训练，并在8个手术数据集上评估，涵盖目标检测、分割、深度估计和姿态估计任务。

Result: 采用显式几何标记化的模型（如MultiMAE）在所有任务上显著优于单模态基线。几何感知预训练实现了卓越的数据效率：仅用25%标注数据微调的模型就超过了使用全部数据的RGB模型。这些优势无需推理时的架构或运行时修改。
...
<details>
  <summary>Details</summary>
Motivation: Split-Federated学习在客户端计算要求低，但通信信道中的噪声会影响学习过程和最终模型质量，需要提高对信道噪声的鲁棒性

Method: 提出智能平均策略用于SplitFed学习，通过改进平均机制来增强对通信信道噪声的抵抗能力

Result: 在胚胎图像分割模型实验中，智能平均策略相比传统平均能容忍两个数量级更强的通信信道噪声，同时保持最终模型的准确性
...
Conclusion: 相机权重机制能够有效调整源视图的重要性，提升少样本新视角合成的质量，该方法是可适应的，可集成到多种NVS算法中。
...
TL;DR: FreeOrbit4D：无需训练的视频重定向框架，通过重建几何完整的4D代理解决大角度视角变化下的几何模糊问题


<details>
  <summary>Details</summary>
Motivation: 单目视频重定向在大角度视角变化时存在严重问题：单目视频仅捕捉动态3D场景的狭窄时空视图，导致几何模糊和时间不一致。现有基于扩散的方法在远离原始轨迹的大角度视角变化下容易失效。

Method: 提出训练免费框架FreeOrbit4D：1）解耦前景和背景重建，将单目视频反投影到统一全局空间；2）利用物体中心的多视角扩散模型合成多视角图像，在规范物体空间重建几何完整的前景点云；3）通过密集像素同步的3D-3D对应将规范前景点云对齐到全局场景空间；4）将几何完整的4D代理投影到目标相机视角，为条件视频扩散模型提供几何支架。

Result: 实验表明FreeOrbit4D在挑战性的大角度轨迹下能产生更忠实可靠的重定向视频，其几何完整的4D代理为编辑传播和4D数据生成等实际应用开辟了潜在途径。

Conclusion: FreeOrbit4D通过恢复几何完整的4D代理作为视频生成的结构基础，有效解决了单目视频大角度重定向中的几何模糊问题，实现了更一致和可靠的重定向效果。
...
TL;DR: RW-CP是一种模型无关的随机游走共形预测框架，通过在预训练视觉基础模型特征上构建k近邻图并应用随机游走扩散不确定性，为医学图像分割生成空间连贯、解剖有效的预测集。


<details>
  <summary>Details</summary>
Motivation: 医学影像中深度学习部署需要不确定性量化，但标准共形预测方法在分割中常忽略解剖上下文，导致预测集碎片化、空间不连贯和过度分割，限制了临床实用性。

Method: 提出随机游走共形预测(RW-CP)，在预训练视觉基础模型特征上构建k近邻图，应用随机游走扩散不确定性，正则化非共形分数，使预测集对共形校准参数λ更不敏感，确保更稳定连续的解剖边界。

Result: 在多种模态公共数据集上评估，相比标准CP基线，在允许错误率α=0.1下，分割质量提升高达35.4%，同时保持严格的边际覆盖。

Conclusion: RW-CP通过强制空间连贯性生成解剖有效的预测集，显著改善分割质量，为医学图像分割提供了更稳定、临床实用的不确定性量化框架。
...
<details>
  <summary>Details</summary>
Motivation: 慢性伤口监测和管理需要准确高效的伤口测量方法，传统方法存在侵入性、效率低或精度不足的问题

Method: 结合RGB-D里程计和B样条曲面重建技术，从RGB-D图像生成详细的3D伤口网格，自动计算临床相关参数如周长、表面积和尺寸

Result: 在硅胶伤口模型上验证，3D重建精度达到亚毫米级，测量结果重复性高且与人工评估高度一致，性能优于现有方法，运行时间适合实时临床部署

Conclusion: 该方法为临床和远程医疗环境中的自动伤口评估提供了有前景的工具，具有高精度、高效率和非侵入性的优势
...
<details>
  <summary>Details</summary>
Motivation: 传统皮层特征映射使用离散网格结构存在局限性，需要一种连续、紧凑的神经表示方法，能够从任意大小的网格中学习并在任意分辨率下提供特征

Method: 引入神经皮层映射作为连续神经表示，提出NC-Reg算法：结合神经皮层特征映射、梯度下降优化和模拟退火策略的迭代配准方法

Result: 相比经典重心插值，运行时提升30倍；在皮层表面刚性配准中实现亚度精度（<1°误差），作为临床环境中鲁棒的预对齐策略
...
TL;DR: NuiWorld是一个解决世界生成中可控性、可扩展性和效率问题的框架，通过生成式引导策略从少量图像生成训练数据，使用可变场景块和扁平化向量集表示来提升性能。
...
Method: 提出生成式引导策略，从少量输入图像开始，利用3D重建和可扩展场景生成技术合成不同大小和布局的场景，生成足够数据训练端到端模型。将场景表示为可变场景块的集合，压缩为扁平化向量集表示，减少大场景的标记长度。通过伪草图标签实现可控性。

Result: 框架能够生成不同大小和布局的场景，保持跨场景尺寸的一致几何保真度，同时提高训练和推理效率。展示了对未见草图的泛化能力。

Conclusion: NuiWorld框架通过生成式引导策略解决了数据稀缺问题，使用可变场景块和扁平化向量集表示提高了可扩展性和效率，并通过伪草图标签实现了可控性，为视频游戏、仿真和机器人等应用提供了有效的世界生成解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有MM-RAG系统缺乏内部策略来决定何时以及如何进行检索，依赖模块化流水线（检测器、分割器、字幕生成器等），限制了检索增强推理的效果。

Method: 提出PixSearch：1）编码时发射<search>标记触发检索；2）选择查询模态（文本、图像或区域）；3）生成像素级掩码直接作为视觉查询；4）采用两阶段监督微调策略，通过搜索交错监督学习检索时机和查询选择，同时保持分割能力。

Result: 在自我中心和实体中心VQA基准测试中，PixSearch显著提升事实一致性和泛化能力，在CRAG-MM上相比全图像检索获得19.7%的相对准确率提升，同时在各种VQA和纯文本QA任务上保持竞争力的推理性能。

Conclusion: PixSearch通过统一区域级感知和检索增强推理，实现了端到端的多模态检索增强生成，消除了对模块化流水线的依赖，在事实一致性和泛化能力方面取得显著改进。
...
TL;DR: m2sv是一个用于地图到街景空间推理的基准测试，评估视觉语言模型在将抽象俯视图与第一人称视图对齐的能力，发现现有模型表现远低于人类水平。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在多模态基准测试中表现良好，但在需要将抽象俯视图与第一人称视图对齐的空间推理任务上仍然脆弱，需要专门的评估工具来揭示这些差距。

Method: 引入m2sv基准测试，包含m2sv-20k地理多样化数据集和m2sv-sft-11k监督微调数据集，要求模型通过对齐北向俯视图和同一交叉口的街景图像来推断相机拍摄方向。

Result: 最佳视觉语言模型在m2sv上仅达到65.2%准确率，远低于人类基线95%。监督微调和强化学习带来改进但跨基准迁移有限，分析揭示了模型在几何对齐、证据聚合和推理一致性方面的持续差距。

Conclusion: 研究突出了视觉语言模型在跨视角空间推理方面的持续挑战，特别是在几何对齐、证据聚合和推理一致性方面，为未来研究提供了方向。
...
TL;DR: GF-Screen：一种基于"扫视-聚焦"强化学习的全癌筛查框架，通过Glance模型定位病灶区域，Focus模型精确分割，利用强化学习优化病灶选择，解决CT大体积扫描中微小病灶检测的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有AI方法在全癌CT筛查中面临挑战，主要因为大体积CT中多样微小病灶定位困难，极端的前景-背景不平衡阻碍模型关注病变区域，而对健康区域的冗余关注不仅降低效率还增加假阳性。

Method: 受放射科医生"扫视-聚焦"诊断策略启发，提出GF-Screen框架：1) Glance模型从整个CT体积中裁剪子体积组并学习选择含病灶的子体积；2) Focus模型精确分割病灶；3) 通过强化学习，利用分割结果奖励Glance模型；4) 引入组相对学习范式，通过组内相对比较优先处理高优势预测，丢弃低优势预测。

Result: 在16个内部和7个外部数据集（涵盖9种病灶类型）上的广泛实验验证了GF-Screen的有效性。在MICCAI FLARE25全癌挑战赛公开验证排行榜上领先，大幅超越FLARE24冠军方案（DSC +25.6%，NSD +28.2%）。

Conclusion: GF-Screen首次有效扩展前沿强化学习技术解决全癌筛查中的特定挑战，通过"扫视-聚焦"策略显著提升病灶检测效率和准确性，减少假阳性，为大规模CT全癌筛查提供了创新解决方案。
...
TL;DR: Reg-TTR：一种测试时优化框架，通过结合深度学习与传统配准技术的优势，在推理时优化预训练模型预测，显著提升配准精度，仅需21%额外推理时间（0.56秒）。


<details>
  <summary>Details</summary>
Motivation: 传统图像配准方法稳健但迭代过程缓慢；深度学习加速推理但易受领域偏移影响；新兴配准基础模型在速度与稳健性间取得平衡，但难以匹敌在特定数据集上训练的专用模型的峰值精度。

Method: 提出Reg-TTR测试时优化框架，在推理阶段对预训练模型的预测结果进行优化，结合深度学习与传统配准技术的互补优势，以适度计算成本提升配准精度。
...
<details>
  <summary>Details</summary>
Motivation: 随着大规模文生图扩散模型的显著进展，其自然扩展到文本驱动的图像到图像转换受到关注。现有方法需要源图像作为视觉指导，但如何有效结合文本提示和视觉指导进行可控转换仍具挑战性。

Method: 从频率域新视角出发，通过动态替换扩散特征的频率带来实现图像转换。FBSDiff通过替换低频带实现外观引导，中频带实现布局引导，高频带实现轮廓引导。FBSDiff++进一步优化：1) 大幅加速推理速度(8.9倍)；2) 改进频率带替换模块支持任意分辨率和宽高比；3) 扩展功能支持局部图像操作和风格特定内容创建。

Result: 广泛的定性和定量实验验证了FBSDiff++在图像转换视觉质量、效率、多功能性和可控性方面优于相关先进方法。该框架实现了即插即用式操作，无需模型训练、微调或在线优化。

Conclusion: FBSDiff/FBSDiff++提供了一个新颖的频率域视角，将现成的文生图扩散模型适配到图像到图像转换范式，实现了高度可控、多功能且高效的文本驱动图像翻译，为图像编辑和内容创作提供了灵活工具。
...
TL;DR: 提出GB-DAL方法，通过原型粒度球分割和模拟非因果因子模块，解决开放世界目标检测中的领域不变表示问题，提升泛化性能。


<details>
  <summary>Details</summary>
Motivation: 开放世界目标检测面临领域不变表示的挑战，特别是隐式非因果因子问题。现有基于领域对抗学习的方法过度关注领域不变信息，但忽略了潜在的非因果因子。主要原因是：1）基于领域判别器的方法受限于稀疏的领域标签；2）非因果因子过于隐式，传统对抗学习方法难以识别。

Method: 提出GB-DAL方法，包含两个核心模块：1）原型粒度球分割模块，从有限数据集中生成更密集的领域（类似更细粒度的粒度球），揭示更多潜在非因果因子；2）模拟非因果因子模块，通过类似对抗扰动的数据增强减少非因果因子的隐式性，促进GB-DAL训练。

Result: 在多个基准测试上的对比实验表明，该方法在新型环境下实现了更好的泛化性能。

Conclusion: 通过粒度球视角改进领域对抗学习，有效解决了开放世界目标检测中的领域不变表示问题，特别是针对隐式非因果因子的挑战，提升了模型在新环境下的泛化能力。
...
TL;DR: 提出空间上下文约束方法解决工业点云分割中类别极度不平衡（215:1）与几何模糊性（尾类与头类共享圆柱基元）的双重危机，通过边界和密度约束提升安全关键组件的识别精度。


<details>
  <summary>Details</summary>
Motivation: 工业点云分割在数字孪生构建中面临安全关键组件（如减速器、阀门）系统性误分类的问题。这源于两个叠加因素：训练数据中这些组件稀少，且它们与主导结构（如管道）具有相同的局部几何特征。本文识别出工业3D数据特有的双重危机：极端的类别不平衡（215:1比例）与几何模糊性（大多数尾类与头类共享圆柱基元）。
...
Result: 在Industrial3D数据集（来自水处理设施的6.1亿个点）上，该方法达到55.74% mIoU，尾类性能相对提升21.7%（29.59% vs. 24.32%基线），同时保持头类精度（88.14%）。具有基元共享模糊性的组件表现显著提升：减速器IoU从0%提升至21.12%；阀门相对提升24.3%。

Conclusion: 该方法解决了几何模糊性问题，避免了典型的头尾类别权衡，为数字孪生应用中的自动化知识提取实现了安全关键组件的可靠识别。
...
TL;DR: 提出一种无监督语义感知曝光校正网络，通过自适应语义融合模块和伪真值生成器解决曝光校正中的语义信息缺失和无标签数据问题。


<details>
  <summary>Details</summary>
Motivation: 曝光不当会导致细节丢失、颜色失真和对比度降低。现有方法面临两个关键挑战：1) 忽视物体级区域语义信息导致颜色偏移伪影；2) 真实世界曝光图像通常没有真值标签，手动标注需要大量编辑工作。

Method: 提出无监督语义感知曝光校正网络，包含：1) 自适应语义感知融合模块，将预训练FastSAM提取的语义信息融合到共享图像特征空间；2) 多尺度残差空间Mamba组恢复细节和调整曝光；3) 基于CLIP的伪真值生成器自动识别曝光情况并指导校正；4) 语义提示一致性损失确保语义一致性和图像-提示对齐。

Result: 综合实验结果表明，该方法在真实世界曝光图像校正方面有效，在数值和视觉上都优于最先进的无监督方法。

Conclusion: 该方法成功解决了曝光校正中的语义信息缺失和无标签数据问题，通过结合FastSAM和CLIP的丰富先验知识，实现了有效的无监督曝光校正。
...
TL;DR: QA-ReID：一种用于衣物变化行人重识别的质量感知双分支匹配方法，通过融合RGB特征和解析特征，并结合质量感知查询自适应卷积，显著提升了跨衣物场景下的识别性能。


<details>
  <summary>Details</summary>
Motivation: 传统行人重识别在衣物变化场景下面临严重挑战，因为衣物变化会引入巨大的外观变化。需要开发能够同时建模全局外观和衣物不变结构线索的方法。

Method: 提出质量感知双分支匹配框架：1）联合利用RGB特征和解析特征分别建模全局外观和衣物不变结构线索；2）通过多模态注意力模块自适应融合异构特征；3）在匹配阶段设计质量感知查询自适应卷积，包含像素级重要性加权和双向一致性约束。

Result: 在PRCC、LTCC和VC-Clothes等多个基准测试中达到最先进性能，在跨衣物场景下显著优于现有方法。

Conclusion: QA-ReID通过融合RGB和解析特征，结合质量感知匹配机制，有效解决了衣物变化行人重识别的挑战，为跨衣物场景下的鲁棒识别提供了有效解决方案。
...
TL;DR: 提出拓扑感知框架，通过图注意力网络和混合损失函数解决视网膜血管分割中的拓扑断裂问题，显著减少血管碎片化。


<details>
  <summary>Details</summary>
Motivation: 传统卷积网络在视网膜动静脉分割中会产生拓扑断裂（间隙和不连续），尽管像素级精度高，但无法进行可靠的基于图的临床分析，影响系统性心血管疾病诊断。

Method: 提出拓扑感知框架：1) 拓扑特征融合模块(TFFM)，将局部特征映射到潜在图空间，使用图注意力网络捕捉全局结构依赖；2) 混合损失函数，结合Tversky损失处理类别不平衡和soft clDice损失显式惩罚拓扑断裂。

Result: 在Fundus-AVSeg数据集上取得SOTA性能：综合Dice分数90.97%，95% Hausdorff距离3.50像素。血管碎片化相对基线减少约38%，产生可用于自动生物标志物量化的拓扑连贯血管树。

Conclusion: 提出的拓扑感知框架能有效保持血管连通性，显著减少分割中的拓扑断裂，为基于图的临床分析提供可靠基础，代码已开源。
...
TL;DR: GTFMN网络通过解耦低光图像超分为光照估计和纹理恢复两个子问题，使用光照图指导纹理特征调制，实现空间自适应增强。


<details>
  <summary>Details</summary>
Motivation: 低光图像超分辨率任务面临分辨率和光照双重退化的挑战，现有方法难以同时处理这两个耦合问题。

Method: 提出GTFMN框架，包含光照流和纹理流两个分支。光照流预测空间变化的光照图，纹理流使用IGM块根据光照图动态调制特征，实现空间自适应恢复。

Result: 在OmniNormal5和OmniNormal15数据集上，GTFMN在定量指标和视觉质量上均优于现有方法，达到最佳性能。

Conclusion: 通过解耦光照估计和纹理恢复，并利用光照图指导特征调制，GTFMN能有效处理低光图像超分辨率问题，实现高质量恢复。
...
TL;DR: SNR-Edit：无需训练的流式生成模型图像编辑框架，通过自适应噪声控制实现精确的潜在轨迹校正，解决现有方法因固定高斯噪声导致的轨迹偏差和结构退化问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于流式生成模型的免反演图像编辑方法依赖固定高斯噪声构建源轨迹，导致轨迹动力学偏差，造成结构退化或质量损失，需要更精确的轨迹控制机制。

Method: 提出SNR-Edit框架，采用结构感知的噪声校正技术，将分割约束注入初始噪声，将源轨迹的随机分量锚定到真实图像的隐式反演位置，减少源-目标传输过程中的轨迹漂移。

Result: 在SD3和FLUX模型上，通过PIE-Bench和SNR-Bench评估显示，SNR-Edit在像素级指标和VLM评分上表现优异，每张图像仅增加约1秒处理时间，无需模型调优或反演。

Conclusion: SNR-Edit通过轻量级的噪声修正实现了更平滑的潜在轨迹和高保真结构保留，为免反演的流式生成模型图像编辑提供了有效的轨迹校正解决方案。
...
Method: 提出对比频谱校正(CSR)：利用频谱引导的对比目标优化校正扰动，使输入与自然流形重新对齐，采用输入自适应方式应用

Result: 在16个分类基准测试中，CSR对抗强AutoAttack平均优于SOTA方法18.1%，推理开销适中，且在不同视觉任务中具有广泛适用性

Conclusion: CSR通过利用对抗样本的频谱特性，提供了一种高效、通用的测试时防御方法，显著提升了视觉语言模型的对抗鲁棒性
...
TL;DR: 本文提出了UniPCB——首个用于PCB质量检测的统一视觉语言基准，以及PCB-GPT模型，在复杂PCB检测任务上显著优于现有MLLMs。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在工业质量检测方面有潜力，但在PCB检测等复杂场景中表现不足。PCB检测面临组件密集、布线复杂、缺陷模式细微等独特挑战，需要专业领域知识。目前缺乏高质量、统一的视觉语言基准来定量评估MLLMs在PCB检测任务上的表现，主要原因是数据有限、数据集碎片化且标准化不一致。
...
Conclusion: UniPCB基准填补了PCB质量检测领域统一评估标准的空白，PCB-GPT模型通过渐进式课程学习有效提升了复杂工业检测任务的性能。作者将发布指令数据、基准和模型以促进未来研究。
...
TL;DR: SimpleSeg是一种简单而有效的多模态大语言模型像素级感知方法，将分割重构为序列生成问题，直接预测物体边界点坐标，无需专用架构。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要复杂的任务特定设计或辅助组件来实现多模态大语言模型的像素级感知，作者希望探索更简单、更统一的方法。

Method: 将分割重构为序列生成问题，模型直接预测物体边界点坐标；采用两阶段训练流程：SF→RL，其中强化学习使用IoU奖励来精炼点序列以匹配真实轮廓。

Result: 在分割基准测试中，SimpleSeg的性能与依赖复杂任务特定设计的方法相当甚至超越，表明标准MLLM架构具有强大的低级感知能力。
...
TL;DR: 提出视频连接新任务及VC-Bench基准，包含1579个高质量视频，覆盖15个主类别和72个子类别，评估视频质量、首尾一致性和过渡平滑度三个核心指标。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成主要关注文本或图像条件，但实际应用如视频编辑和vlog制作需要将独立片段无缝连接。视频连接任务缺乏标准化评估基准，阻碍了该领域发展。

Method: 提出VC-Bench基准，包含从公共平台收集的1579个高质量视频，覆盖15个主类别和72个子类别确保多样性。建立三方面评估框架：视频质量评分(VQS)、首尾一致性评分(SECS)和过渡平滑度评分(TSS)。

Result: 在VC-Bench上评估多个最先进的视频生成模型，结果显示现有模型在保持首尾一致性和过渡平滑度方面存在显著局限性，导致整体连贯性和流畅性较低。
...
TL;DR: 系统评估了多种手工特征描述符在CIFAKE数据集上检测合成图像的性能，发现LightGBM结合混合特征表现最佳，证实了手工特征在可解释性和计算效率方面的持续价值。


<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展使得合成图像高度逼真，引发了对数字媒体真实性和信任的担忧。虽然深度学习方法主导当前文献，但手工特征因其可解释性、效率和泛化能力仍具吸引力。本研究旨在系统评估手工特征描述符在检测合成图像方面的有效性。

Method: 在CIFAKE数据集（5万训练样本，1万测试样本）上系统评估了多种手工特征描述符：原始像素、颜色直方图、离散余弦变换(DCT)、方向梯度直方图(HOG)、局部二值模式(LBP)、灰度共生矩阵(GLCM)和小波特征。使用七种分类器进行基准测试，包括逻辑回归到先进的梯度提升集成方法（LightGBM、XGBoost、CatBoost）。
...
Conclusion: 精心设计的手工特征和集成学习在检测合成图像方面仍然具有重要价值，特别是在需要可解释性和计算效率的场景中。混合特征方法能显著提升性能，为合成图像检测提供了有效的替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 半监督域适应中，目标域标记样本有限导致特征空间内类别相似性高，即使使用平衡数据集训练也会产生预测偏差，需要解决这一问题

Method: 提出多视图一致性框架：1) 使用强增强数据训练的两个视图；2) 基于模型预测性能的类别概率去偏策略；3) 利用模型预测生成伪负标签；4) 跨域亲和性学习对齐不同域中相同类别的特征

Result: 在DomainNet和Office-Home两个标准域适应数据集上超越了竞争方法，证明了方法的有效性
...
TL;DR: ProMist-5K是一个专门用于模拟Pro-Mist滤镜电影风格的数据集，包含20,000张高分辨率图像对，涵盖不同滤镜密度和焦距配置，支持图像翻译模型学习真实的光晕和扩散效果。


<details>
  <summary>Details</summary>
Motivation: Pro-Mist滤镜在电影摄影中能创造柔和光晕、降低对比度并产生独特的氛围风格，但这些效果难以通过数字方式重现，因为光扩散行为复杂。需要专门的数据集来支持电影风格仿真。

Method: 在场景参考线性空间中构建物理启发的采集流程，包含20,000张高分辨率图像对，涵盖两种滤镜密度（1/2和1/8）和两种焦距（20mm和50mm）。使用多层模糊和精心调校的权重来建模光学扩散的不同强度和扩散程度。

Result: 数据集在不同训练设置下表现良好，能够捕捉从细微到强烈的电影外观效果。提供了可控的目标域，支持多种图像翻译模型和学习范式。
...
Method: 1) 创建ASFW数据集：通过专业Photoshop工作流程制作1081对真实世界阴影和阴影去除图像；2) 提出Face Shadow Eraser方法展示数据集有效性。

Result: ASFW数据集提供逼真的阴影变化和准确地面真值，弥合了合成域和真实域之间的差距。在ASFW上训练的深度模型在真实世界条件下表现出更好的阴影去除效果。

Conclusion: ASFW数据集显著提升了人脸阴影去除模型的性能，为该任务设定了新标准，Face Shadow Eraser方法验证了数据集的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 单目相机在3D检测中存在深度模糊和恶劣条件下鲁棒性差的问题，雷达虽然对光照和天气变化鲁棒，但稀疏性和低分辨率限制了其直接应用，需要有效的雷达-相机融合方法。

Method: 1. InstaRadar：基于实例分割引导的雷达扩展方法，利用预训练分割掩码增强雷达密度和语义对齐；2. 将预训练RCDPT深度估计模块集成到BEVDepth框架中，替换其原始深度模块。

Result: InstaRadar在雷达引导深度估计中达到SOTA，RCDPT集成持续提升3D检测性能，相比基线BEVDepth模型获得稳定增益，但落后于直接提取BEV特征的雷达-相机融合模型。

Conclusion: InstaRadar和显式深度监督在3D目标检测中有效，未来将扩展InstaRadar到点云表示，并集成带时序线索的专用雷达分支以增强BEV融合。
...
TL;DR: 该论文首次系统评估了八种立体深度估计方法在植被密集环境中的零样本泛化能力，发现不同方法在不同场景下表现各异，其中DEFOM在植被深度估计中表现最佳，被确立为黄金标准基线。
...
Result: 结果显示场景依赖模式：基础模型在结构化场景表现优异（BridgeDepth：ETH3D上0.23px；DEFOM：Middlebury上4.65px），而迭代方法在不同基准间表现不一（IGEV++：ETH3D上0.36px但Middlebury上6.77px）。在Tree Branches数据集上，DEFOM被确立为植被深度估计的黄金标准基线，具有优越的跨域一致性（在所有基准中排名1-2，平均排名1.75）。

Conclusion: DEFOM在植被深度估计中表现最佳，其预测将作为未来基准测试的伪地面真值，为自主无人机林业操作提供了重要的深度估计基准。
...
Conclusion: 即使没有大规模数据，也能构建高效、可复现、高性能的科学多模态模型，为未来研究提供实用基础，强调原则性数据选择的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 为SLAM和地点识别算法的开发和基准测试提供多模态数据集，特别是针对视觉和LiDAR模态的算法，填补在火山环境等特殊地形数据集的空白

Method: 在意大利西西里火山岛采集多序列数据，包含各种环境、纹理和地形（玄武岩/富铁岩石、古老熔岩通道、干燥植被和水域），提供开源工具包用于生成地面真实位姿和准备地点识别任务的标记样本

Result: 发布了S3LI Vulcano数据集和开源工具包，数据集包含多模态数据（视觉+LiDAR），覆盖多种环境条件，可用于SLAM和地点识别算法的开发和评估

Conclusion: 该数据集为SLAM和地点识别研究提供了有价值的基准测试资源，特别是在具有挑战性的火山环境中，开源工具包进一步增强了其实用性和可重复性
...
Conclusion: 通过区域课程学习和模糊标签机制处理分割中的非均匀不确定性，能够稳定优化过程，引导模型收敛到帕累托近似解，显著提升医学图像分割性能。
...
TL;DR: VGGT-SLAM 2.0 是一个实时RGB前馈SLAM系统，通过改进因子图设计消除漂移和平面退化，利用VGGT注意力层进行图像检索验证，在多种环境中实现实时性能，并在TUM数据集上比VGGT-SLAM减少约23%的位姿误差。
...
Method: 1. 设计新的因子图结构来消除漂移和平面退化；2. 利用VGGT的注意力层进行图像检索验证，无需额外训练即可拒绝误匹配并完成更多闭环；3. 在Jetson Thor上实现实时在线运行。

Result: 1. 在TUM数据集上达到最高精度，比VGGT-SLAM减少约23%的位姿误差；2. 在杂乱室内公寓、办公室场景和4200平方英尺谷仓等多种环境中有效运行；3. 可轻松适配开放集目标检测；4. 在地面机器人上实现实时性能。

Conclusion: VGGT-SLAM 2.0通过改进因子图设计和利用VGGT注意力层，显著提升了SLAM系统的精度和鲁棒性，在多种现实环境中实现了实时高性能的定位与建图。
...
Method: 采用混合工作流程：1) 使用关键词从PubMed Central提取相关图像；2) 结合深度学习图像模态分类和图像标题分析的新颖混合方法进行分类；3) 由认证皮肤病理学家审核

Result: 验证651张手动标注图像显示：深度学习F-score 89.6%，关键词检索61.0%，混合方法90.4%；获得7,772张图像覆盖166种诊断；发现当前OpenAI图像分析算法不适合皮肤病理学图像分析

Conclusion: 开发了大型、同行评审、开放获取的皮肤病理学图像数据集DermpathNet，具有半自动策展工作流程，可用于教育、交叉参考和机器学习目的
...
TL;DR: Tri-Reader是一个免费的三阶段肺癌筛查流水线，集成了肺部分割、结节检测和恶性分类，优先考虑敏感性同时减少标注负担。


<details>
  <summary>Details</summary>
Motivation: 开发一个全面、免费的肺癌筛查工具，整合多个公开数据集训练的模型，优先保证敏感性并减少标注人员的工作负担。

Method: 使用多个基于公开数据集训练的开源模型，开发Tri-Reader三阶段工作流：肺部分割→结节检测→恶性分类，形成统一流水线。

Result: 在多个内部和外部数据集上评估Tri-Reader，与专家标注和数据集提供的参考标准进行比较，确保准确性和跨不同实践的泛化能力。

Conclusion: Tri-Reader是一个有效的免费肺癌筛查流水线，在保证敏感性的同时减少了标注负担，并在多样化的数据集上表现出良好的准确性和泛化能力。
...
TL;DR: 提出了X-AIGD基准测试，用于可解释的AI生成图像检测，提供像素级分类标注，发现现有检测器很少依赖感知伪影，但通过显式对齐注意力可以提高可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前AI生成图像检测方法主要依赖二元分类，缺乏可解释的证据支持其决策。现有基准测试虽然包含大量合成图像，但覆盖的伪影多样性有限且缺乏详细的局部标注。

Method: 提出了X-AIGD基准测试，提供像素级、分类的感知伪影标注，涵盖低层失真、高层语义和认知层反事实。通过该基准进行广泛研究，分析现有检测器的行为。

Result: 研究发现：(1)现有检测器对感知伪影的依赖微乎其微；(2)虽然可以训练检测器识别特定伪影，但判断仍主要基于不可解释特征；(3)显式对齐模型注意力与伪影区域可以提高检测器的可解释性和泛化能力。
...
TL;DR: RoamScene3D：通过语义推理和几何约束从文本生成一致且逼真的3D场景，解决了现有方法的空间盲点和轨迹固定问题。


<details>
  <summary>Details</summary>
Motivation: 现有文本到3D场景生成方法存在空间盲点，依赖预定义轨迹无法利用对象间内在关系，且2D修复模型难以处理相机运动造成的空洞问题。
...
Conclusion: RoamScene3D通过语义引导和空间生成的结合，解决了文本到3D场景生成中的关键挑战，实现了自适应场景探索和逼真内容生成。
...
TL;DR: 提出基于CNN和SAM的双学生-教师框架(DSTCS)，用于超声图像中耻骨联合和胎头分割，通过协同学习机制提升分割精度，在MICCAI基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 耻骨联合和胎头分割在产程监测中至关重要，但现有方法面临类别不平衡、边界模糊、噪声干扰以及标注数据稀缺等挑战。当前研究主要依赖CNN和Transformer架构，更强大模型的潜力尚未充分探索。
...
Conclusion: 提出的DSTCS框架通过结合CNN和SAM的优势，有效解决了PSFH分割中的挑战，为产程监测提供了准确可靠的分割解决方案，具有重要的临床应用价值。
...
TL;DR: Dyn-HSI：首个动态人-场景交互认知架构，赋予虚拟人视觉、记忆、控制三大类人组件，在动态场景中生成高质量交互动作


<details>
  <summary>Details</summary>
Motivation: 现实世界中场景持续动态变化，但现有的人-场景交互生成方法通常将场景视为静态，这与实际情况不符。需要开发能够处理动态场景的交互生成方法。

Method: 提出Dyn-HSI认知架构，包含三个类人组件：1)视觉（动态场景感知导航）-持续感知环境变化并自适应预测下一个路径点；2)记忆（分层经验记忆）-存储和更新训练期间积累的经验数据，在推理时利用先验知识进行上下文感知的动作引导；3)控制（人-场景交互扩散模型）-基于多模态输入生成高保真交互动作。构建动态基准数据集Dyn-Scenes进行评估。

Result: 在静态和动态设置下，Dyn-HSI方法始终优于现有方法，生成高质量的人-场景交互动作。通过广泛的定性和定量实验验证了方法的有效性。

Conclusion: Dyn-HSI是首个动态人-场景交互认知架构，通过模拟人类的视觉、记忆和控制能力，成功解决了动态场景中的交互生成问题，在静态和动态场景中均表现出色。
...
TL;DR: 提出ENkG采样策略，通过熵引导的自适应候选集大小解决视频生成中静态top-k/top-p策略的局限性，提升长序列生成质量。


<details>
  <summary>Details</summary>
Motivation: 视频token与语言token存在本质差异：视频token语义密度低、时空冗余度高，导致静态top-k/top-p策略在视频解码中效果不佳。低不确定性区域（静态背景）引入不必要随机性，高不确定性区域（前景物体）早期错误会累积，最终严重降低长序列生成质量。

Method: 提出熵引导的k-Guard采样策略，基于每个token预测分布的熵值量化token分散度，自适应调整候选集大小：低熵区域使用较少候选以抑制冗余噪声并保持结构完整性；高熵区域使用更多候选以减轻错误累积。

Result: 实验表明ENkG相比静态top-k/top-p策略在感知质量和结构稳定性方面有持续改进。该方法模型无关、无需训练、计算开销可忽略。

Conclusion: ENkG采样策略通过自适应候选集大小有效解决了视频生成中静态采样策略的局限性，为自回归视频解码提供了更优的采样方案。
...
TL;DR: 提出一个快速3DGS重建管道，能在1分钟内收敛，在SIGGRAPH Asia 3DGS快速重建挑战赛中取得第一名（PSNR 28.43）。针对不同相机姿态质量采用两阶段策略：第一阶段处理SLAM噪声轨迹，第二阶段处理准确COLMAP姿态。


<details>
  <summary>Details</summary>
Motivation: 解决SIGGRAPH Asia 3DGS快速重建挑战中的时间限制问题（1分钟收敛），同时需要处理两种不同的相机姿态质量：SLAM生成的噪声轨迹和COLMAP生成的高精度姿态。

Method: 采用两阶段解决方案：第一阶段针对SLAM噪声轨迹，使用反向逐高斯并行优化、紧凑前向溅射、负载均衡分块、基于锚点的Neural-Gaussian表示、单目深度初始化、全局姿态细化模块。第二阶段针对准确COLMAP姿态，禁用姿态细化、回归标准3DGS、引入多视角一致性引导的高斯分裂、深度估计器监督渲染深度。

Result: 在严格的一分钟时间预算下实现高保真重建，在挑战赛中取得最佳性能，PSNR达到28.43，排名第一。

Conclusion: 提出的快速3DGS重建管道成功解决了在1分钟内收敛的挑战，通过针对不同相机姿态质量的自适应两阶段策略，在SIGGRAPH Asia 3DGS快速重建挑战中取得了第一名。
...
TL;DR: Cor2Vox是一个基于皮层结构的脑MRI生成框架，利用皮层表面先验引导3D形状到图像的布朗桥扩散过程，实现解剖学上一致的脑图像合成。


<details>
  <summary>Details</summary>
Motivation: 现有合成神经影像数据生成模型主要依赖标签或文本等弱条件信号，缺乏解剖学基础，常产生生物学上不可信的输出。需要一种能生成解剖学一致、高质量脑MRI的方法。

Method: 提出Cor2Vox框架，利用从33,000多个UK Biobank扫描中提取的大规模皮层形态统计形状模型作为先验，通过皮层表面引导的3D形状到图像布朗桥扩散过程进行生成。

Result: 在传统图像质量指标、高级皮层表面重建和全脑分割质量方面优于多个基线方法。在三个应用中（解剖一致合成、渐进灰质萎缩模拟、数据集协调）均能保持亚体素级别的精细皮层形态。

Conclusion: Cor2Vox通过将图像生成与连续皮层结构先验绑定，实现了拓扑忠实、解剖学可控的脑MRI合成，对皮层几何变化和疾病表型具有显著鲁棒性。
...
TL;DR: Pref-Restore：一种分层盲人脸修复框架，通过离散语义逻辑与连续纹理生成结合，实现确定性偏好对齐的修复，解决信息不对称问题。


<details>
  <summary>Details</summary>
Motivation: 当前生成式方法在盲人脸修复中存在信息不对称问题：低质量输入信息稀疏，高质量输出信息密集，导致一对多映射、随机不确定性和幻觉伪影。需要解决这种信息不平衡以实现确定性修复。

Method: 提出分层框架Pref-Restore，采用两种互补策略：1) 增强输入密度：使用自回归积分器将文本指令转化为密集潜在查询，注入高级语义稳定性约束退化信号；2) 修剪输出分布：将策略强化学习直接集成到扩散修复循环中，将人类偏好转化为可微分约束，惩罚随机偏差，锐化后验分布。

Result: 在合成和真实世界基准测试中达到最先进性能。经验分析证实偏好对齐策略显著降低解决方案熵，为可靠确定性盲修复建立稳健路径。

Conclusion: Pref-Restore通过整合离散语义逻辑和连续纹理生成，有效解决盲人脸修复中的信息不对称问题，实现确定性偏好对齐的修复，显著减少随机不确定性。
...
TL;DR: 提出基于超宽带传感器稀疏成对距离测量的实时全身运动捕捉系统Wild-Poser，无需外部相机，适用于野外环境


<details>
  <summary>Details</summary>
Motivation: 传统光学或惯性运动捕捉系统受环境限制（光照、磁场干扰），需要外部设备，难以在野外环境中使用。需要一种能在非受控环境中工作的形状无关、环境鲁棒的运动捕捉方法。

Method: 使用身体佩戴的超宽带传感器进行飞行时间测距，获取稀疏成对距离测量。提出Wild-Poser（WiP）——基于Transformer的紧凑实时架构，直接从噪声或损坏的距离测量预测3D关节位置，然后通过学习方法重建关节旋转。

Result: WiP实现低关节位置误差，在人类和动物受试者上展示准确的3D运动重建，无需个体身体测量或形状拟合，实时运行，适用于野外环境。

Conclusion: 该系统为现实世界环境中的可扩展、低成本、通用运动捕捉提供了潜力，特别适用于野外和非受控环境。
...
<details>
  <summary>Details</summary>
Motivation: 当前重度抑郁症的评估主要依赖主观方法，特别是精神运动迟缓的临床评估缺乏客观性。3D运动捕捉虽然客观但需要专业设备，不适合常规临床使用。需要开发非侵入式、可扩展的客观监测工具。

Method: 1) 提出计算框架将单目RGB视频转换为3D步态运动学特征；2) 使用重力视图坐标和新的轨迹校正算法，利用改进的Timed Up and Go协议闭环拓扑缓解单目深度误差；3) 从单摄像头提取297个明确的步态生物力学标志物；4) 引入基于稳定性的机器学习框架处理小临床数据集，防止过拟合。

Result: 在CALYPSO数据集上验证：1) 检测精神运动迟缓准确率达83.3%；2) 解释总体抑郁严重程度64%的方差(R²=0.64)；3) 发现踝关节推进力减少和骨盆活动受限与抑郁运动表型强相关；4) 证明身体运动可作为认知状态的可靠代理。

Conclusion: 物理运动可作为认知状态的稳健代理，为抑郁症的客观监测提供了透明且可扩展的工具，适用于标准临床环境。该方法揭示了特定运动特征与抑郁表型的关联，为非侵入式精神疾病评估开辟了新途径。
...
TL;DR: MaDiS：基于掩码扩散的语言模型，用于手语生成，通过双向依赖建模和并行多标记生成，在多个数据集上实现SOTA性能，推理延迟降低30%


<details>
  <summary>Details</summary>
Motivation: 现有手语生成方法采用自回归语言模型，存在单向上下文建模和逐标记推理慢的问题，需要更高效的双向依赖建模和并行生成方案

Method: 提出掩码扩散语言模型MaDiS，包含三层次跨模态预训练（标记、潜在、3D物理空间）、带时间检查点的解掩码策略、以及混合部件嵌入层

Result: 在CSL-Daily、Phoenix-2014T和How2Sign数据集上实现最优性能，包括DTW误差和两个新指标SiBLEU和SiCLIP，推理延迟降低近30%

Conclusion: MaDiS通过掩码扩散框架有效解决了手语生成中的双向依赖建模和推理效率问题，为聋哑和听力障碍社区提供了更高效的沟通桥梁
...
TL;DR: 提出QuaMo方法，使用四元数微分方程进行人体运动捕捉，解决传统方法中欧拉角不连续性问题，实现更准确、连续的3D运动估计


<details>
  <summary>Details</summary>
Motivation: 传统3D姿态估计方法忽略帧间时间一致性，导致运动不自然和抖动。现有基于运动学的方法依赖欧拉角，存在不连续性问题，特别是在在线设置中无法进行轨迹优化。四元数没有不连续性，能产生连续的姿态过渡

Method: 提出QuaMo方法，使用四元数微分方程进行人体运动学捕捉。采用状态空间模型描述实时运动学估计，状态为四元数，QDE描述四元数速度。通过带有加速度增强的元PD控制器计算相应的角加速度，自适应调节控制信号。在四元数单位球约束下求解QDE

Result: 实验结果表明，QuaMo的QDE公式配合加速度增强能够准确估计3D人体运动学，没有不连续性且最小化不合理性。在Human3.6M、Fit3D、SportsPose和AIST等多个数据集上优于现有最先进方法

Conclusion: QuaMo通过四元数微分方程解决了传统运动捕捉中的不连续性问题，实现了更准确、连续的3D人体运动估计，在多个基准数据集上表现出优越性能
...
### [55] [GMS-CAVP: Improving Audio-Video Correspondence with Multi-Scale Contrastive and Generative Pretraining](https://arxiv.org/abs/2601.19606)
*Shentong Mo,Zehua Chen,Jun Zhu*

Main category: cs.CV

TL;DR: GMS-CAVP提出多尺度视频-音频对齐与扩散预训练框架，通过结合判别式-生成式学习提升跨模态理解与生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频-音频联合嵌入方法（如CAVP）对多尺度时空结构的建模不足，限制了跨模态检索和生成任务的性能提升。

Method: 1) 多尺度对比学习策略，捕捉不同粒度的语义和时间关系；2) 引入扩散生成目标，实现视频-音频模态间的转换与合成。

Result: 在VGGSound、AudioSet和Panda70M数据集上，GMS-CAVP在生成和检索任务上均优于先前方法。

Conclusion: 统一的判别式-生成式框架能更深入建模跨模态对应关系，为高质量生成任务奠定基础。
...
### [56] [ScenePilot-Bench: A Large-Scale Dataset and Benchmark for Evaluation of Vision-Language Models in Autonomous Driving](https://arxiv.org/abs/2601.19582)
...
Result: 对代表性VLM进行基准测试，提供了实证分析，明确了当前性能边界，识别了驾驶导向推理的差距，为安全关键自动驾驶环境中的VLM评估提供了全面框架。

Conclusion: ScenePilot-Bench为评估和推进视觉语言模型在安全关键自动驾驶环境中的应用提供了全面的评估框架，有助于推动自动驾驶领域VLM技术的发展。
...
TL;DR: 大规模评估医疗影像差分隐私分析中的初始化策略，发现领域特定的监督预训练在隐私保护下表现最佳，而自监督初始化优于ImageNet初始化但不及领域特定方法。


<details>
  <summary>Details</summary>
Motivation: 差分隐私(DP)在保护敏感医疗数据时通常导致诊断性能显著下降，而模型初始化是缓解这种性能退化的关键因素。然而，现代自监督学习在全模型DP下的作用尚未被充分理解。

Method: 使用超过80万张胸部X光片作为基准，通过最先进的ConvNeXt模型在现实隐私机制下使用DP-SGD训练，比较三种初始化策略：非领域特定的监督ImageNet初始化、非领域特定的自监督DINOv3初始化、以及领域特定的监督预训练（在MIMIC-CXR数据集上）。在五个外部数据集上进行评估，涵盖不同机构和采集设置。

Result: DINOv3初始化在DP下相对于ImageNet初始化始终提高诊断效用，但仍不及领域特定的监督预训练，后者性能最接近非隐私基线。初始化选择强烈影响人口统计学公平性、跨数据集泛化能力，以及在隐私约束下对数据规模和模型容量的鲁棒性。

Conclusion: 初始化策略是差分隐私医疗影像中效用、公平性和泛化能力的核心决定因素。领域特定的监督预训练在隐私保护下表现最佳，为医疗影像的隐私保护分析提供了重要指导。
...
### [58] [Localized Latent Editing for Dose-Response Modeling in Botulinum Toxin Injection Planning](https://arxiv.org/abs/2601.19593)
...
<details>
  <summary>Details</summary>
Motivation: 肉毒杆菌注射是面部不对称和美容年轻化的黄金标准，但确定最佳剂量主要依赖直觉，常导致次优结果，需要更科学的注射规划方法

Method: 提出区域特异性潜在轴发现方法，在StyleGAN2潜在空间中学习局部肌肉松弛轨迹，避免全局副作用；通过将局部潜在轨迹与注射毒素单位关联，建立预测性剂量-响应模型；比较直接度量回归与基于图像的生成模拟两种方法

Result: 在包含46名患者360张图像的临床数据集上，在测试集上显示中度到强结构相关性，生成模型正确捕捉形态变化方向；引入混合"人在回路"工作流，让临床医生交互式优化模拟

Conclusion: 该框架通过剂量-响应建模模拟肉毒杆菌注射效果，虽然生物变异性限制了绝对精度，但"人在回路"工作流弥合了病理重建与美容规划之间的差距
...
TL;DR: 论文提出了首个面向城市管理的低空智能多模态基准数据集GovLA-10K和统一视觉语言推理框架GovLA-Reasoner，用于支持城市治理中的异常理解和管理决策。


<details>
  <summary>Details</summary>
Motivation: 现有以物体为中心的感知范式和松散的视觉语言管道难以满足真实城市治理中面向管理的异常理解需求，需要建立管理导向的低空智能系统。

Method: 1) 构建GovLA-10K基准数据集，围绕功能显著目标设计，提供可操作管理建议；2) 提出GovLA-Reasoner框架，通过高效特征适配器隐式协调视觉检测器与大语言模型之间的表征共享。

Result: 实验表明该方法显著提升性能，同时避免了针对特定任务组件进行微调的需求，为管理感知的低空视觉语言系统提供了新视角和基础。

Conclusion: 该工作填补了城市治理中管理导向异常理解的空白，为未来低空视觉语言系统的研究提供了新的方向和基础框架。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型持续学习需要平衡三个相互竞争的目标：保留预训练知识、保持已学任务序列的知识、以及维持获取新知识的可塑性。现有方法难以有效平衡这些目标。

Method: 通过分析模型参数空间的知识保留机制，发现通用知识主要编码在主成分子空间，而任务特定知识编码在残差子空间。KeepLoRA通过将新任务的梯度投影到与预训练模型主成分子空间和先前任务特征主导方向都正交的子空间，将LoRA参数更新限制在残差子空间。

Result: 理论和实证分析证实KeepLoRA能有效平衡三个目标，并在持续学习任务中实现了最先进的性能。

Conclusion: KeepLoRA提供了一种简单而有效的持续学习方法，通过参数空间分析指导的梯度投影策略，在视觉语言模型持续学习中实现了预训练知识保留、任务知识保持和学习可塑性的良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像相似度度量方法不是感知度量，难以评估包含纹理失真的图像，且深度学习方法作为黑盒只能提供相似度分数而无法解释差异。

Method: 提出由两部分组成的新感知度量：1) 使用Earth Mover's Distance评估图像纹理差异；2) 在Oklab感知颜色空间中评估颜色差异。

Result: 在Berkeley-Adobe Perceptual Patch Similarity数据集上评估，新度量方法优于现有技术，特别是在包含形状失真的图像上表现更好，证实了其更强的感知能力。

Conclusion: 新提出的感知度量不仅性能优于现有方法，还提供可视化解释来支持相似度评分，使相似度评估更加透明和可解释。
...
Method: 通过引入辅助特征函数（定义为具有跳跃Neumann边界条件的泊松方程的解），使用可微的局部积分评估，联合优化特征位置和MLP参数。

Result: 在2D问题和3D CAD模型重建任务中，SharpNet能准确恢复尖锐边缘和角点，同时保持其他区域的平滑性，优于现有基线方法。

Conclusion: SharpNet提供了一种精确可控的C^0连续性方法，能够有效表示具有尖锐特征的函数，在保持平滑区域的同时准确捕捉梯度不连续性。
...
Result: 在五个具有挑战性的基准测试中达到最先进或极具竞争力的结果，在Video-Holmes上达到42.7%（超越GPT-4o），在推理和通用视频理解任务上均获得一致提升。消融研究验证了归因信号的互补作用和目标token级更新的鲁棒性。
...
TL;DR: 提出DSVM-UNet，通过双重自蒸馏方法在全局和局部层面对齐特征，提升VM-UNet的医学图像分割性能，无需复杂架构设计。
...
Method: 提出DSVM-UNet，采用双重自蒸馏方法：1) 全局自蒸馏对齐全局特征；2) 局部自蒸馏对齐局部特征，无需改变VM-UNet的基础架构。
...
<details>
  <summary>Details</summary>
Motivation: 现代模型参数规模日益增大，预训练模型变得重要，但实际部署需要不同大小的架构，传统预训练和微调方法存在局限性，需要能支持可变尺寸模型初始化的方法。

Method: 提出SWEET框架：1）学习共享权重模板和尺寸特定权重缩放器，基于Tucker分解实现模块化；2）目标模型通过组合和重加权模板进行初始化；3）引入宽度随机缩放，在宽度维度上正则化模板，提升跨宽度泛化能力。

Result: 在分类、检测、分割和生成任务上的大量实验表明，SWEET在初始化可变尺寸视觉模型方面达到了最先进的性能。

Conclusion: SWEET通过约束式预训练和模块化设计，有效解决了可变尺寸视觉模型的初始化问题，为不同规模架构的部署提供了灵活高效的解决方案。
...
TL;DR: DiffStyle3D：一种新颖的基于扩散的3D高斯溅射风格迁移方法，通过潜在空间直接优化，使用注意力感知损失和几何引导的多视图一致性，解决了现有方法的多视图一致性和训练稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D风格迁移方法存在局限性：基于VGG和CLIP的方法难以在模型内部建模多视图一致性，而基于扩散的方法虽然能捕捉一致性但依赖去噪方向导致训练不稳定。需要一种既能保持多视图一致性又训练稳定的新方法。

Method: 提出DiffStyle3D框架：1）在潜在空间直接优化；2）引入注意力感知损失，在自注意力空间对齐风格特征同时通过内容特征对齐保持原始内容；3）提出几何引导的多视图一致性方法，将几何信息整合到自注意力中以实现跨视图对应建模；4）基于几何信息构建几何感知掩码，防止跨视图重叠区域的冗余优化。

Result: 大量实验表明，DiffStyle3D在风格化质量和视觉真实感方面优于现有最先进方法，实现了更高的多视图一致性和更好的风格迁移效果。

Conclusion: DiffStyle3D通过潜在空间直接优化、注意力感知损失和几何引导的多视图一致性，成功解决了3D风格迁移中的多视图一致性和训练稳定性问题，为3D内容创作提供了高质量的视觉表达工具。
...
TL;DR: WaterClear-GS：首个纯3D高斯泼溅框架，通过将水下光学特性（局部衰减和散射）显式集成到高斯基元中，实现实时渲染的水下3D重建和外观恢复


<details>
  <summary>Details</summary>
Motivation: 现有NeRF方法渲染速度慢且颜色恢复不理想，而3DGS缺乏建模复杂体积散射效应的能力。水下3D重建和外观恢复受到水复杂光学特性（波长相关衰减和散射）的阻碍。

Method: 提出WaterClear-GS框架：1）将水下光学特性（局部衰减和散射）显式集成到高斯基元中，无需辅助介质网络；2）采用双分支优化策略确保水下光度一致性并自然恢复无水外观；3）结合深度引导几何正则化、感知驱动图像损失、曝光约束、空间自适应正则化和物理引导光谱正则化，增强局部3D一致性和自然视觉感知。

Result: 在标准基准和新收集的数据集上，WaterClear-GS在新视角合成（NVS）和水下图像恢复（UIR）任务上均取得优异性能，同时保持实时渲染速度。

Conclusion: WaterClear-GS是首个纯3DGS框架，成功将水下光学特性集成到高斯表示中，实现了高质量的水下3D重建和外观恢复，同时保持实时渲染能力，为水下视觉任务提供了有效解决方案。
...
TL;DR: PaW-ViT是一种基于解剖知识的预处理方法，通过将token边界与耳部特征边界对齐来增强视觉Transformer在耳部识别中的性能，提高对形状、大小和姿态变化的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 视觉Transformer中常见的矩形token会包含识别对象之外的信息，这会影响识别性能。特别是在耳部生物识别中，耳部形态变化与Transformer架构的位置敏感性之间存在不匹配问题。

Method: 提出PaW-ViT（基于补丁的扭曲视觉Transformer），这是一种基于解剖知识的预处理方法。通过准确地将token边界与检测到的耳部特征边界对齐，并按照自然耳部曲率对齐特征边界，为不同形态生成更一致的token表示。
...
TL;DR: GeoDiff3D是一个自监督的3D场景生成框架，使用粗几何作为结构锚点，通过几何约束的2D扩散模型生成纹理丰富的参考图像，无需严格的多视角一致性，实现了高效高质量的3D场景生成。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景生成方法存在两个主要问题：间接2D到3D重建和直接3D生成方法都受到弱结构建模和严重依赖大规模真实监督的限制，导致结构伪影、几何不一致和复杂场景中高频细节退化。需要一种更高效、更少依赖标注数据的解决方案。

Method: 提出GeoDiff3D框架：1) 使用粗几何作为结构锚点；2) 采用几何约束的2D扩散模型生成纹理丰富的参考图像；3) 无需严格多视角一致性，对噪声和不一致指导保持鲁棒性；4) 引入体素对齐的3D特征聚合和双重自监督机制；5) 低计算成本训练，支持快速高质量生成。

Result: 在具有挑战性的场景上进行广泛实验，相比现有基线方法，GeoDiff3D展现出更好的泛化能力和生成质量，同时大幅减少对标注数据的依赖，为可访问和高效的3D场景构建提供了实用解决方案。

Conclusion: GeoDiff3D通过结合粗几何结构锚点和几何约束的2D扩散模型，实现了自监督的高质量3D场景生成，解决了现有方法的结构建模弱、依赖大规模监督的问题，为游戏、影视特效和VR/AR领域的3D内容创作提供了高效实用的工具。
...
<details>
  <summary>Details</summary>
Motivation: 耳饰遮挡（如耳环、耳机）会严重影响耳部生物识别系统的性能，特别是在非受控成像条件下，需要有效方法来处理这种遮挡问题

Method: 使用扩散模型进行耳部修复作为预处理辅助，输入耳部图像和自动生成的遮挡掩码，模型重建干净且解剖学合理的耳部区域，合成缺失像素同时保持关键耳部结构的局部几何一致性

Result: 实验表明，扩散模型修复可以有效缓解耳饰遮挡问题，提高基于Transformer的耳部识别系统的整体识别性能

Conclusion: 扩散模型耳部修复是一种有效的预处理辅助方法，能够改善耳饰遮挡对耳部识别系统的影响，提升识别性能
...
TL;DR: Youtu-VL采用视觉-语言统一自回归监督范式，将视觉信号从"条件输入"转变为"监督目标"，提升细粒度视觉信息保留能力


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型存在细粒度视觉信息保留不足的问题，主要原因是训练范式存在文本主导的优化偏差，将视觉信号仅视为被动条件输入而非监督目标

Method: 提出Youtu-VL框架，采用视觉-语言统一自回归监督范式，将视觉标记直接整合到预测流中，对视觉细节和语言内容进行统一自回归监督，并扩展到视觉中心任务

Result: 在通用多模态任务和视觉中心任务上均取得有竞争力的性能，为开发全面通用视觉智能体奠定坚实基础

Conclusion: 通过将视觉信号从"输入"转变为"目标"的范式转变，Youtu-VL有效解决了现有VLM的细粒度视觉信息保留问题，实现了更全面的多模态理解能力
...
<details>
  <summary>Details</summary>
Motivation: 现有AVQA方法主要关注视觉信息处理，音频仅作为视频分析的补充，文本问题信息在推理后期才整合，未能充分利用音频的频域特性和问题引导的线索。

Method: 提出查询引导的时空频交互方法(QSTar)，结合问题引导线索并利用音频信号的频域特性；引入查询上下文推理块(QCR)，引导模型更精确关注语义相关的音频和视觉特征。
...
TL;DR: HexFormer是一种用于图像分类的双曲视觉Transformer，通过指数映射聚合注意力机制，在多个数据集上超越了欧几里得基线和先前双曲ViT，混合变体表现最佳，同时双曲模型展现出更稳定的梯度和训练鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 图像、文本和图等多模态数据常包含层次和关系结构，这些结构在欧几里得几何中难以建模。双曲几何为表示此类结构提供了自然框架，但需要设计适合视觉任务的Transformer架构。
...
Result: 在多个数据集上实验显示，HexFormer相比欧几里得基线和先前双曲ViT获得一致性能提升，混合变体整体表现最强。双曲模型展现出更稳定的梯度和对预热策略的敏感性降低，训练更鲁棒高效。

Conclusion: 双曲几何能通过改善梯度稳定性和准确性来增强视觉Transformer架构，相对简单的指数映射聚合机制能提供强大的实际效益，为层次结构数据建模提供了有前景的方向。
...
Result: 在ARCTIC和EgoExo4D数据集上实验显示，相比最先进方法取得一致性能提升。同时展示了真实世界泛化能力，并通过重建手部作为视觉提示改进了EgoVLM的手物交互推理。

Conclusion: EgoHandICL是首个将上下文学习应用于3D手部重建的框架，通过多模态上下文理解和示例检索，显著提升了在挑战性第一人称视角条件下的重建质量和鲁棒性。
...
TL;DR: SONIC提出了一种连续频谱参数化的卷积算子，使用少量共享的、方向选择性的组件来建模卷积操作，实现了全局感受野和跨分辨率的自然适应。


<details>
  <summary>Details</summary>
Motivation: CNN受限于固定大小的局部核，难以捕获全局上下文；ViT虽然提供全局连接但缺乏空间归纳偏置，依赖显式位置编码且受限于初始patch大小。需要一种既结构化又全局的表征方法。

Method: 引入SONIC（频谱导向神经不变卷积），使用连续频谱参数化建模卷积算子，通过少量共享的方向选择性组件定义全频域上的平滑响应。

Result: 在合成基准测试、大规模图像分类和3D医学数据集上，SONIC对几何变换、噪声和分辨率变化表现出更好的鲁棒性，以数量级更少的参数匹配或超越了卷积、基于注意力和先前频谱架构的性能。

Conclusion: 连续、方向感知的频谱参数化为传统空间和频谱算子提供了一种原则性且可扩展的替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态模型在处理阿拉伯文字，特别是艺术化和风格化的书法形式方面能力不足，缺乏专门的评估基准来测试模型对复杂阿拉伯书法的理解能力。

Method: 创建DuwatBench数据集，包含1,272个精心策划的样本，涵盖约1,475个独特词汇，覆盖六种古典和现代阿拉伯书法风格，每个样本都配有句子级检测标注。使用该基准评估了13个领先的阿拉伯和多语言多模态模型。

Result: 评估显示，虽然这些模型在处理干净文本时表现良好，但在处理书法变体、艺术扭曲和精确的视觉-文本对齐方面存在困难，表明当前模型对阿拉伯艺术文字的理解能力有限。

Conclusion: DuwatBench的发布旨在推进文化基础的多模态研究，促进阿拉伯语言和视觉遗产在AI系统中的公平包容，支持该领域的持续进步。数据集和评估套件已公开可用。
...
TL;DR: 论文提出基于自然轨迹数据的自动驾驶多目标平衡学习框架，通过帕累托前沿识别安全、效率和交互的最优平衡状态


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统需要在安全、效率和交互三个目标之间取得平衡，但现有方法难以直接从真实驾驶数据中学习这些权衡关系

Method: 提出经验学习框架：1) 构建统一目标空间，用复合分数表示每个时间步的安全、效率和交互表现；2) 应用帕累托支配识别非支配状态，形成经验前沿；3) 使用TGSIM数据集进行验证

Result: 仅有0.23%的自动驾驶实例达到帕累托最优，表明同时优化多个目标非常罕见；帕累托最优状态在三个目标上的平均分数显著更高，其中交互目标的改进潜力最大

Conclusion: 该框架仅需运动学和位置数据，具有最小侵入性和模块化特点，可直接应用于多目标学习表面的推导和可视化，为自动驾驶系统设计提供数据驱动的平衡策略
...
TL;DR: DeFM是一个专门为机器人应用设计的自监督深度图像基础模型，使用60M深度图像数据集训练，在多种机器人任务中实现SOTA性能，并支持从仿真到真实环境的泛化。


<details>
  <summary>Details</summary>
Motivation: 深度传感器在机器人平台中广泛应用，但相比RGB模态，深度模态的表征学习研究不足。尽管快速高保真深度仿真促进了机器人策略的sim-to-real迁移，但缺乏专门针对深度图像的大规模基础模型。

Method: 采用DINO风格的自蒸馏目标，在6000万深度图像数据集上训练；引入新颖的输入归一化策略以保持多尺度下的度量感知；将DeFM蒸馏为适合资源受限机器人系统的紧凑模型。

Result: 在深度分类、分割、导航、运动控制和操作等基准测试中达到最先进性能；展示了从仿真到真实环境的强泛化能力；模型无需任务特定微调即可用于深度机器人学习。

Conclusion: DeFM填补了深度模态表征学习的空白，为机器人应用提供了首个专门针对深度图像的自监督基础模型，实现了跨环境、任务和传感器的泛化能力，并支持资源受限部署。
...
TL;DR: Sprout是一个面向开发者的安全、可表达的人形机器人平台，旨在解决现有机器人系统难以在人类环境中长期部署的问题。
...
Method: Sprout采用轻量化设计，具有柔性控制、有限关节扭矩和柔软外壳，确保在共享人类空间中的安全操作。平台集成了全身控制、集成夹持器操作和基于虚拟现实的遥操作，形成统一的硬件-软件栈。还配备了可表达头部以支持社交互动。

Result: 通过降低部署的物理和技术障碍，Sprout扩展了有能力的人形机器人平台的访问范围，为在真实人类环境中开发具身智能提供了实用基础。

Conclusion: Sprout作为一个开发者平台，通过强调安全性、可表达性和开发者可访问性，解决了当前人形机器人部署的局限性，为在人类环境中开发实用的机器人应用提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 水下自主车辆在执行干预任务时可能面临碰撞风险，需要安全处理策略来保护车辆敏感区域

Method: 采用切换非线性模型预测控制策略，当无法避免碰撞时利用机械臂推离障碍物来偏转车辆

Result: 虚拟实验证明算法能成功检测碰撞，并能避免碰撞或适当使用机械臂处理碰撞而不损坏车辆敏感区域

Conclusion: 提出的控制策略能有效处理水下车辆-机械臂系统的碰撞情况，提高水下干预任务的安全性
...
<details>
  <summary>Details</summary>
Motivation: 传统盲文阅读器采用离散字符扫描，速度慢且破坏自然阅读流程；基于视觉的方法计算量大、延迟高且在真实环境中性能下降

Method: 使用开源神经形态事件触觉传感器Evetac，结合时空分割和轻量级ResNet分类器处理稀疏事件流，支持连续滑动识别

Result: 在标准深度下达到接近完美准确率（≥98%），支持多种盲文板布局，快速扫描下保持良好性能，日常词汇单词级准确率超过90%
...
TL;DR: SimTO框架通过从物理模拟器中自动提取负载情况，实现高分辨率拓扑优化，为特征丰富的物体生成定制化软体夹爪。


<details>
  <summary>Details</summary>
Motivation: 现有软体夹爪难以抓取具有高拓扑变异性的特征丰富物体（如齿轮、珊瑚、西兰花等），这些物体缺乏明确的"最优"接触表面，传统夹爪设计容易造成损坏。拓扑优化需要预定义负载情况，但对于特征丰富物体，抓取过程中的接触力是未知且不可预测的。

Method: 提出SimTO框架，通过接触式物理模拟器自动提取负载情况，无需手动指定负载。给定任意特征丰富的物体，SimTO能够进行高分辨率拓扑优化，生成针对物体几何形态定制的软体夹爪。

Result: 数值结果显示，SimTO生成的设计不仅高度专业化于特征丰富物体，还能泛化到未见过的物体。

Conclusion: SimTO框架通过自动提取负载情况，解决了拓扑优化在软体夹爪设计中的关键限制，能够为特征丰富物体生成高度定制化的软体夹爪，同时保持泛化能力。
...
TL;DR: 提出一种新的群体控制方法，允许智能体通过局部观察协商不同的期望间距，无需全局信息或通信，适用于目标部分冲突的半信任场景。


<details>
  <summary>Details</summary>
Motivation: 传统群体控制假设智能体具有统一的期望间距，但实际应用中智能体类型和配置多样，且常在没有信任保证或安全通信的环境下工作，需要更灵活的方法处理部分对齐或冲突的目标。

Method: 通过新的约束集体势函数，放松共享间距假设，允许智能体通过纯局部观察协商间距参数，无需全局信息或通信，适用于半信任场景。

Result: 通过一系列仿真验证了方法的有效性，能够处理相邻智能体追求冲突目标的情况。

Conclusion: 更新了传统群体控制框架，引入间距参数协商机制，增强了多智能体系统在多样化配置和半信任环境下的适应性和鲁棒性。
...
TL;DR: 提出一个处理不确定性的自动化仓储系统框架，通过k有界扰动模型和Θ(k)网格宽度设计，在最大容量下消除或减少货物重定位。


<details>
  <summary>Details</summary>
Motivation: 实际物流应用中，存储后的检索顺序可能发生变化，而现有方法假设已知存储和检索序列，无法处理这种不确定性。

Method: 引入k有界扰动模型，证明Θ(k)网格宽度在最大容量下消除重定位的必要性和充分性，提供高效求解器计算鲁棒存储布局，并提出应对超k扰动的策略。

Result: 实验表明：当k不超过网格宽度一半时，框架基本消除重定位；当k达到整个网格宽度时，重定位减少50%以上。

Conclusion: 提出的框架能有效处理检索顺序不确定性，显著减少自动化仓储系统中的货物重定位，提高运营效率。
...
TL;DR: 开发了iFAN生态系统作为核设施数字孪生框架，提供高保真虚拟测试平台，用于验证自主和网络弹性核操作


<details>
  <summary>Details</summary>
Motivation: 随着核设施数字化转型和先进反应堆发展，AI集成、网络物理安全和自主机器人操作等新兴技术日益发展，但缺乏专用虚拟测试平台阻碍了评估和部署

Method: 开发了Immersive Framework for Advanced Nuclear (iFAN)生态系统，这是一个具有真实3D环境和基于物理仿真的综合数字孪生框架，提供实时数据交换用于部署前验证

Result: iFAN生态系统作为高保真虚拟测试平台，支持工厂操作、网络安全、物理安全和机器人操作，具备虚拟现实、强化学习、辐射模拟和网络物理安全等核心功能

Conclusion: iFAN生态系统为验证下一代自主和网络弹性核操作提供了多功能且安全的架构，通过潜在操作场景展示了各种应用
...
TL;DR: 提出TaMeSo-bot系统，结合软手腕和触觉记忆，通过MAT³模型实现安全鲁棒的接触式操作，在多种孔洞任务中表现优异
...
Method: 提出TaMeSo-bot系统：1）软手腕实现安全接触探索；2）触觉记忆通过检索重用过往演示；3）核心MAT³模型通过掩码令牌预测联合建模机器人动作、分布式触觉反馈、力扭矩测量和本体感觉信号的时空交互

Result: 在真实机器人实验中，MAT³在所有条件下的成功率均优于基线方法，并展现出对未见过的孔洞和条件的显著适应能力

Conclusion: TaMeSo-bot系统通过软手腕和触觉记忆实现了安全鲁棒的操作，MAT³模型能够学习丰富的时空表征并自主提取任务相关特征，为接触式操作提供了有效解决方案
...
TL;DR: 提出Perception-to-Pursuit (P2P)框架，通过8维运动令牌和因果Transformer实现无人机轨迹预测与可执行拦截规划，在Anti-UAV-RGBT数据集上显著提升预测精度和拦截成功率。


<details>
  <summary>Details</summary>
Motivation: 现有无人机跟踪方法只优化预测精度，忽略拦截可行性，导致99.9%的情况下无法实际拦截。需要将检测与可执行的拦截规划相结合。

Method: 提出P2P框架，将无人机运动表示为包含速度、加速度、尺度和平滑度的8维令牌，使用12帧因果Transformer进行时序推理，引入拦截成功率(ISR)指标评估拦截可行性。

Result: 在Anti-UAV-RGBT数据集（226个真实无人机序列）上，P2P达到28.12像素平均位移误差和0.597 ISR，相比仅跟踪基线提升77%轨迹预测精度和597倍拦截可行性，同时保持100%无人机分类准确率。

Conclusion: 通过对运动模式的时序推理，既能实现准确预测，又能支持可执行的拦截规划，为自主无人机追击提供了有效解决方案。
...
TL;DR: 提出自监督框架，通过可微分硬约束投影层实现运行时保证，解决自动驾驶在非结构化环境中的安全、数据稀缺和计算资源限制问题。


<details>
  <summary>Details</summary>
Motivation: 非结构化环境中部署深度学习自动驾驶代理面临安全、数据稀缺和计算资源限制三大挑战。传统求解器延迟高，基于学习的方法难以保证确定性可行性。

Method: 1) 构建全局引导人工势场(G-APF)提供密集监督信号；2) 采用自适应神经投影层迭代修正网络输出到可行流形；3) 结合可微分硬约束投影层确保运行时安全。

Result: 在20,000个场景测试中达到88.75%成功率，CARLA闭环实验验证动态约束下路径物理可实现性，NVIDIA Jetson Orin NX上推理延迟94ms，满足实时性要求。

Conclusion: 该框架为将物理定律嵌入神经架构提供了通用范式，为机电系统中的约束优化问题提供了可行方向，实现了从具身智能到嵌入式智能的跨越。
...
TL;DR: 基于网页的平台Machine Learning with Bricks通过无编程的机器人活动向12-17岁学生教授机器学习概念，结合可视化与乐高机器人教学KNN、线性回归和Q-learning算法。


<details>
  <summary>Details</summary>
Motivation: 让年轻学习者（12-17岁）能够通过直观、有形的方式接触和理解机器学习概念，降低技术门槛，提高学习兴趣和可及性。

Method: 开发开源网页平台，结合交互式可视化与乐高机器人，设计为期两天的课程，学生通过收集数据、训练模型、与机器人交互来学习三种核心算法，无需编程基础。

Result: 对14名学生的前后测调查显示：机器学习概念理解显著提升，对AI的认知态度积极转变，平台可用性高，学习动机增强。

Conclusion: 基于有形化和可视化的教学方法能够有效向年轻学习者传授机器学习概念，既保持技术深度又提高参与度，为青少年AI教育提供了可行方案。
...
TL;DR: Judgelight是一种后优化方法，通过消除多智能体路径规划中不必要的振荡和冗余运动来改进轨迹质量，将解决方案成本降低约20%。


<details>
  <summary>Details</summary>
Motivation: 现有的学习型MAPF求解器虽然快速且可扩展，但生成的可行轨迹往往包含不必要的或振荡的运动，不适合实际部署。

Method: 提出Judgelight后优化方法，将智能体轨迹中的封闭子路径折叠以消除冗余运动，同时保持所有可行性约束。将该过程形式化为MAPF-Collapse问题，证明其NP难，并通过整数线性规划（ILP）实现精确优化。

Result: 实验结果表明，Judgelight能持续将解决方案成本降低约20%，特别是对学习型求解器效果显著，产生的轨迹更适合实际部署。

Conclusion: Judgelight作为一种有效的后优化方法，能够显著改进MAPF解决方案的质量，消除不必要的运动，使学习型求解器生成的轨迹更适合实际应用。
...
TL;DR: SimHum：通过协同训练从仿真机器人动作中提取运动先验，从真实人类观测中提取视觉先验，实现数据高效且可泛化的机器人操作


<details>
  <summary>Details</summary>
Motivation: 仿真数据和真实人类数据各有局限：仿真存在视觉差距（sim-to-real gap），人类数据存在本体差距（embodiment gap）。研究发现两者具有天然互补性：仿真提供机器人动作，人类数据提供真实世界观测

Method: 提出SimHum协同训练框架，同时从仿真机器人动作中提取运动先验，从真实人类观测中提取视觉先验，基于这两个互补先验进行训练

Result: 在相同数据收集预算下，SimHum比基线方法提升高达40%；仅用80个真实数据就能达到62.5%的OOD成功率，比仅用真实数据的基线提升7.1倍

Conclusion: 通过利用仿真和人类数据的互补性，SimHum框架能够实现数据高效且可泛化的机器人操作，有效解决了仿真到真实和人类到机器人的差距问题
...
TL;DR: TCMP提出任务优先对抗模仿学习框架，将模仿作为条件正则化器而非平等目标，在保持任务性能的同时获得自然运动风格


<details>
  <summary>Details</summary>
Motivation: 传统方法面临两难：单纯模仿人类演示会因演示次优、具身差异、重定向误差等导致任务性能下降；而纯任务强化学习会产生不自然或不稳定的运动。线性奖励混合在对抗模仿学习中存在根本局限。

Method: 提出任务中心运动先验(TCMP)，将模仿视为条件正则化器而非平等目标。框架最大化任务改进，仅在模仿信号与任务进展兼容时才纳入模仿信号，产生自适应、几何感知的更新，保持任务可行下降并抑制不对齐时的有害模仿。

Result: 通过人形控制实验验证了方法有效性，在噪声演示下实现了鲁棒的任务性能和一致的运动风格。提供了梯度冲突和任务优先稳定点的理论分析。

Conclusion: TCMP框架成功解决了任务性能与运动自然性之间的权衡问题，通过任务优先的对抗模仿学习实现了在保持任务性能的同时获得自然运动风格，特别适用于演示次优或不对齐的场景。
...
<details>
  <summary>Details</summary>
Motivation: 对于晶格型模块化自重构机器人，在重构过程中保持稳定连接对物理可行性和部署性至关重要。现有方法在保证连接稳定性方面存在不足。

Method: 首先使用虚拟图表示构建可行的连接/断开动作，然后通过依赖反向树（DRTree）组织这些动作形成有效执行序列，解决动作间的相互依赖关系。

Result: 证明对于7个以上模块（排除线性拓扑）的任何配置对都存在满足运动特性的重构序列。与改进的BiRRT算法相比，该方法在效率和稳定性方面表现更优，并在物理机器人平台上验证了实际可行性。

Conclusion: 提出了一种能够保证稳定连接的四边形模块化自重构机器人自重构规划算法，通过理论证明和实验验证了方法的有效性、高效性和实际可行性。
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习虽然能学习目标到达策略，但缺乏形式化保证目标一定能到达。现有的屏蔽机制虽然能提供安全约束，但容易导致学习过于保守，影响探索效率。需要在保证形式化目标到达的同时，保持有效的学习和探索。

Method: 1. 设计实时RL策略，包含15个精心定义的奖励项，鼓励机器人到达静态和动态目标，同时生成平滑的控制信号并满足安全规范。2. 在基准RL框架中集成李雅普诺夫类稳定器作为策略监督器，形式化加强目标到达控制，同时保持状态动作空间的有意义探索。

Result: 实验结果表明，提出的李雅普诺夫类稳定器持续改进了基准RL策略：目标到达率从84.6%提升到99.0%，显著减少失败，提高效率。框架适合在挑战性环境中实时部署，提供形式化的收敛保证。

Conclusion: 该框架成功地将强化学习与形式化保证相结合，通过李雅普诺夫稳定器监督RL策略，在保持有效探索的同时提供形式化的目标到达保证，显著提升了轮式移动机器人在非结构化环境中的控制性能。
...
TL;DR: 该论文提出了一种改进的SINS/DVL松耦合导航方法，通过引入姿态误差感知的DVL速度转换模型和协方差矩阵方差传播方法，有效抑制了长期导航误差发散。


<details>
  <summary>Details</summary>
Motivation: 传统SINS/DVL导航系统使用SINS推导的姿态将DVL速度从机体坐标系转换到导航坐标系，但姿态估计误差会随时间累积，导致速度投影偏差，从而降低长期导航性能。

Method: 1. 提出车辆姿态误差感知的DVL速度转换模型，在观测方程中引入姿态误差项以减少投影引起的速度偏差；2. 开发基于协方差矩阵的方差传播方法，通过期望姿态误差补偿项实现统计一致的噪声建模。

Result: 仿真和现场实验表明：两种改进方法单独使用时都能提高导航精度；联合应用时能有效抑制长期误差发散。现场实验显示，相比基线IMU+DVL方法，3D位置RMSE提升78.3%，最大分量位置误差降低71.8%。

Conclusion: 提出的方法通过同时考虑姿态误差对投影速度和测量不确定性的影响，为改善SINS/DVL长期导航性能提供了鲁棒解决方案，显著提高了水下导航系统的精度和稳定性。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM在机器人控制中存在两个主要局限：1）缺乏模块化的智能体执行机制，无法实现闭环的规划-反思-修订；2）现有操作任务基准主要关注低级控制，未能系统评估多步推理和语言多样性。

Method: 提出ALRM框架，通过ReAct式推理循环整合策略生成与智能体执行，支持两种互补模式：Code-asPolicy（CaP）直接生成可执行控制代码，Tool-as-Policy（TaP）进行迭代规划和基于工具的动作执行。同时创建包含56个任务的新仿真基准。

Result: 在10个LLM上的实验表明，ALRM提供了可扩展、可解释、模块化的方法，能够将自然语言推理与可靠的机器人执行相连接。Claude-4.1-Opus在CaP模式下表现最佳（闭源模型），Falcon-H1-7B在开源模型中表现最佳。

Conclusion: ALRM框架成功解决了现有LLM机器人控制方法的局限性，通过智能体执行机制和新基准测试，为自然语言推理与机器人执行的可靠连接提供了有效解决方案。
...
TL;DR: PALM通过利用局部动作分布在OOD与演示域之间的不变性，同时处理工作空间偏移、视角变化和跨具身迁移等OOD问题，无需额外模态、模型修改或数据收集。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常孤立地处理图像行为克隆中的泛化问题（工作空间偏移、视角变化、跨具身迁移），依赖复杂流程，难以同时应对多种OOD偏移。

Method: PALM将操作策略模块化为粗粒度全局组件和细粒度局部策略。通过强制局部视觉聚焦和一致的本体感知表示，减少局部策略层面的域间差异，使策略能在OOD条件下检索不变的局部动作。

Result: 实验显示PALM将OOD性能下降限制在模拟中8%、现实世界24%，而基线方法分别为45%和77%。

Conclusion: PALM利用局部动作分布的不变性，有效解决了图像行为克隆中的多种OOD泛化问题，显著优于现有方法。
...
TL;DR: Rhombot是一种新型可变形平面晶格模块化自重构机器人，采用菱形模块设计，通过单个中央执行器实现对角线折叠/展开，具有简化控制、连续稳定重构的特点，可在不同环境中形成多种构型。


<details>
  <summary>Details</summary>
Motivation: 开发一种具有最小控制复杂度的模块化自重构机器人系统，能够实现基本的MSRR功能（变形、对接、运动），并在不同环境中实现连续稳定的重构过程。

Method: 设计菱形模块，采用平行四边形骨架和单个中央执行器，实现沿对角线折叠/展开。提出morphpivoting运动基元用于重构，并制定连续执行策略。

Result: 物理实验验证了模块的稳定重构能力、位置精度和对接精度，系统能够在不同环境中可靠形成各种构型。

Conclusion: Rhombot通过简化设计和新型运动基元，实现了高效稳定的模块化自重构，为MSRR系统提供了新的设计思路和实现方法。
...
TL;DR: 提出低成本统一框架，利用增强逆透视映射进行矢量化道路制图，通过优化IPM矩阵和车辆姿态，实现厘米级精度的自动高精度地图生成。


<details>
  <summary>Details</summary>
Motivation: 传统逆透视映射在矢量化道路制图中存在误差大、共面性假设限制等问题，需要低成本、高精度的自动制图解决方案。

Method: 使用Catmull-Rom样条表征车道线，多边形表征其他地面标记；通过实例分割结果优化三维控制点和多边形角点位置；同时优化IPM单应矩阵和车辆姿态。

Result: 显著降低IPM制图误差，提高IPM矩阵和车辆姿态精度；在两种实际场景中实现厘米级精度的自动高精度地图生成；优化后的IPM矩阵达到人工校准精度。

Conclusion: 该框架有效解决了IPM的共面性限制，为矢量化道路制图提供了低成本、高精度解决方案，能够统一处理所有常见地面标记和车道线。
...
TL;DR: AC²-VLA：基于动作上下文的自适应计算框架，通过认知重用、token剪枝和选择性执行，显著降低VLA模型在机器人操作中的计算开销，同时保持任务成功率。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在机器人操作中表现出色，但闭环部署时存在高延迟和高计算成本问题。现有效率方法忽略了动作上下文的重要性，而VLA推理在时间、空间和深度维度上存在结构化冗余。

Method: 提出AC²-VLA框架，基于当前视觉观察、语言指令和先前动作状态进行自适应计算。通过统一的机制实现跨时间步的认知重用、token剪枝和模型组件选择性执行。采用动作引导的自蒸馏训练策略，在保持密集VLA策略行为的同时实现结构化稀疏化。

Result: 在机器人操作基准测试中，AC²-VLA实现了高达1.79倍的加速，将FLOPs降低到密集基线的29.4%，同时保持可比较的任务成功率。

Conclusion: AC²-VLA通过动作上下文感知的自适应计算，有效解决了VLA模型在机器人操作中的计算效率问题，为实际部署提供了可行的解决方案。
...
TL;DR: 提出使用四足机器人进行港口基础设施检查的初步方案，包括关键区域识别和机器人检查分析


<details>
  <summary>Details</summary>
Motivation: 基础设施检查对保障工人安全日益重要，港口环境复杂，需要机器人解决方案来应对日常运营中的挑战

Method: 首先识别港口环境中的关键区域，然后分析使用四足机器人检查这些关键区域的初步解决方案

Result: 提出了港口基础设施检查的初步方案，包括关键区域识别和四足机器人检查分析

Conclusion: 港口基础设施检查需要专门设计的机器人解决方案，四足机器人是可行的初步选择，但需要进一步研究和开发
...
TL;DR: SCOPE是一个快速高效的柔性线性物体建模与操纵框架，使用凸近似替代传统能量方法，在保持物理合理形变的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统基于能量的柔性线性物体建模方法计算成本高，难以满足实时或近实时应用需求。需要一种能在速度和精度之间取得更好平衡的解决方案。

Method: 采用凸近似方法替代传统的能量最小化方法，通过这种近似显著降低计算复杂度，同时保持形变的平滑性和物理合理性。

Result: 通过全面的仿真实验验证了框架的有效性，能够生成满足几何和长度约束的平滑形状轨迹，实现了速度与精度的良好平衡。

Conclusion: SCOPE框架通过凸近似实现了柔性线性物体的高效建模与操纵，特别适用于需要实时或近实时响应的应用场景。
...
TL;DR: 论文提出将推荐系统技术整合到社交机器人中，以解决现有个性化方法的局限性，建立了一个模块化框架来增强社交机器人的个性化能力。


<details>
  <summary>Details</summary>
Motivation: 现有社交机器人个性化方法（如大语言模型和强化学习）无法全面捕捉用户偏好（包括长期、短期和细粒度偏好），也无法有效利用这些偏好进行动作排序选择、主动个性化交互和确保伦理适应性。
...
Conclusion: 推荐系统技术能有效解决社交机器人个性化中的关键挑战，提出的模块化框架为实现更全面、主动和伦理的个性化交互提供了可行方案，促进了跨领域合作。
...
TL;DR: 研究探讨机器人拟人化程度和道德基础如何影响人们对机器人受虐的反应，发现拟人化决定是否给予道德关怀，而道德基础影响关怀的推理方式。


<details>
  <summary>Details</summary>
Motivation: 随着机器人日益融入日常生活，理解人们对机器人受虐的反应具有重要的伦理和设计意义。需要探究拟人化水平和道德基础如何塑造对机器人虐待的反应。

Method: 采用混合方法研究（N=201），参与者观看不同拟人化程度机器人（蜘蛛型、双足型、人形）受物理虐待的视频，完成道德基础、愤怒和社会距离测量。结合定量和定性分析。

Result: 拟人化程度决定人们是否将道德关怀延伸至机器人，而道德基础塑造了这种关怀的推理方式。低进步主义个体使用基于性格的判断，高进步主义个体则进行面向未来的道德审议。

Conclusion: 研究结果为机器人设计和政策沟通提供了启示，强调需要考虑拟人化程度和个体道德基础差异来设计更符合伦理的机器人交互。
...
<details>
  <summary>Details</summary>
Motivation: 当前演示编程方法在双臂任务中存在应用不足的问题，主要由于手部协调的复杂性阻碍了数据记录，需要简化双臂机器人编程过程

Method: 使用单次RGB视频演示，应用香农信息论分析场景元素间的信息流，利用场景图特性检测手部协调策略，生成基于不同手臂协调结构的模块化行为树

Result: 通过多个主题视频演示和公开数据集验证，相比现有方法在生成双臂系统集中式执行计划方面有显著改进

Conclusion: 该方法能够有效处理单次双臂任务演示视频，生成协调双臂系统的执行计划，为简化双臂机器人编程提供了新方案
...
<details>
  <summary>Details</summary>
Motivation: 现有的人机交互系统缺乏在多用户环境中持续个性化和动态适应的机制，限制了其在真实世界部署中的有效性。需要一种能够管理长期多用户交互的框架。
...
Conclusion: HARMONI框架通过整合多模态感知、世界建模、用户建模和生成模块，实现了社交辅助机器人在多用户环境中的长期个性化交互，为解决现有系统在真实世界部署中的局限性提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 工业5.0强调以人为中心的人机协作，需要确保安全、舒适和信任。现有方法缺乏对信任的量化评估，需要数据驱动的方法来评估和预测人机协作中的信任水平

Method: 1. 基于偏好优化的算法生成增强信任的机器人轨迹；2. 操作员反馈作为训练数据；3. 机器学习模型从行为指标预测信任水平；4. 在化学工业混合化学品场景中测试

Result: 机器学习模型分类信任准确率超过80%，其中投票分类器达到84.07%准确率和0.90的AUC-ROC分数，表明行为指标能有效预测人机信任动态

Conclusion: 数据驱动方法能有效评估人机协作中的信任，行为指标在预测人类信任动态方面发挥重要作用，为工业5.0的人机协作提供了实用的信任评估框架
...
### [108] [Agentic Business Process Management Systems](https://arxiv.org/abs/2601.18833)
*Marlon Dumas,Fredrik Milani,David Chapela-Campa*

Main category: cs.AI

TL;DR: 论文提出基于生成式和代理式AI的自主业务流程管理系统(A-BPMS)，将流程管理从自动化转向自主化，从设计驱动转向数据驱动，通过流程挖掘技术实现代理感知、推理和优化。


<details>
  <summary>Details</summary>
Motivation: 业务流程管理(BPM)领域自90年代以来经历了多轮自动化技术浪潮，但当前生成式和代理式AI的出现带来了新的变革机会。传统BPM技术主要关注任务自动化和流程编排，而AI技术能够实现从自动化到自主化的转变，从设计驱动到数据驱动的管理方式变革。

Method: 论文提出基于流程挖掘技术构建自主业务流程管理系统(A-BPMS)的架构愿景。该系统利用流程挖掘作为基础，使代理能够：1)感知流程状态，2)推理改进机会，3)执行优化行动。系统需要支持从人工驱动到完全自主的连续流程谱系。

Result: 论文提出了A-BPMS的架构概念，这是一种整合自主性、推理和学习能力的新型流程管理平台。该系统重新定义了流程自动化和治理的边界，能够支持不同自主程度的业务流程。

Conclusion: 生成式和代理式AI正在引领BPM领域的新一波变革，从自动化转向自主化。流程挖掘技术为代理式BPM系统奠定了基础，未来的A-BPMS将整合自主性、推理和学习能力，支持从人工到完全自主的连续流程管理，重新定义业务流程的自动化和治理方式。
...
TL;DR: 使用LLaMEA框架，通过进化循环中的大语言模型生成具有明确高层景观特征的优化问题，扩展了BBOB基准测试的多样性


<details>
  <summary>Details</summary>
Motivation: 现有基准测试套件（如BBOB）的结构多样性有限，阻碍了连续黑盒优化的基准测试。需要能够生成具有明确高层景观特征的优化问题的方法

Method: 使用LLaMEA框架，通过进化循环中的LLM从自然语言描述生成问题代码，利用ELA属性预测器评分候选问题，引入ELA空间适应度共享机制增加种群多样性

Result: 生成的函数确实表现出预期的结构特征，t-SNE嵌入显示它们扩展了BBOB实例空间而非形成无关集群，创建了广泛、可解释、可复现的基准问题库

Conclusion: 该方法成功生成了具有明确景观特征的优化问题，扩展了基准测试的多样性，为景观分析和自动算法选择等下游任务提供了有价值的资源
...
TL;DR: 开发IT2-ANFIS模型用于污水处理厂能耗预测，提供可解释的不确定性量化，通过模糊规则结构分解不确定性到特征、规则和实例三个层次。
...
Result: 在墨尔本水务东部处理厂数据集上验证，IT2-ANFIS与一阶ANFIS相比具有相当的预测性能，同时显著减少训练运行的方差，并提供可解释的不确定性估计，将预测置信度直接与运营条件和输入变量关联。

Conclusion: IT2-ANFIS为污水处理厂能耗预测提供了可解释的不确定性量化框架，支持风险感知决策，有助于提高运营效率和可持续性。
...
TL;DR: 论文提出RIFT测试平台，通过重新排序指令来评估大语言模型的指令跟随能力，发现模型对指令顺序有强烈依赖，跳跃式指令会导致性能大幅下降。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在复杂工作流中的应用日益增多，但它们在保持指令流方面的能力尚未得到充分探索。现有基准测试将任务复杂性与结构顺序混为一谈，难以分离提示结构对性能的影响。

Method: 引入RIFT（重新排序指令跟随测试平台），使用重新表述的Jeopardy!问答对，测试两种提示结构：线性提示（顺序进行）和跳跃提示（内容相同但需要非顺序遍历）。在六个最先进的开源LLM上进行了10,000次评估。

Result: 在跳跃条件下，准确率相比基线下降了高达72%，显示出对位置连续性的强烈依赖。约50%的失败源于指令顺序违反和语义漂移，表明当前架构将指令跟随内化为顺序模式而非推理技能。

Conclusion: 结构敏感性是当前架构的基本限制，对需要非顺序控制流的应用（如工作流自动化和多智能体系统）有直接影响。当前模型将指令跟随视为顺序模式而非推理能力。
...
### [112] [Neural Theorem Proving for Verification Conditions: A Real-World Benchmark](https://arxiv.org/abs/2601.18944)
*Qiyuan Xu,Xiaokun Luan,Renxi Wang,Joshua Ong Jun Leang,Peixin Wang,Haonan Li,Wenda Li,Conrad Watt*

Main category: cs.AI

TL;DR: 提出了首个针对程序验证中验证条件自动证明的多语言基准测试NTP4VC，评估了大语言模型在VC证明任务上的表现，发现虽然LLM显示出潜力，但程序验证仍面临重大挑战。


<details>
  <summary>Details</summary>
Motivation: 程序验证中的验证条件自动证明是主要瓶颈，现有自动定理证明器无法处理困难的VC，导致需要大量手动证明。虽然神经定理证明在数学竞赛中取得成功，但在程序验证中的应用尚未充分探索，缺乏专门针对VC证明的基准测试。

Method: 从Linux和Contiki-OS内核等真实项目收集数据，使用工业级工具链（Why3和Frama-C）生成在Isabelle、Lean和Rocq等不同形式语言中语义等价的测试用例，构建了首个多语言VC证明基准测试NTP4VC。

Result: 评估了通用大语言模型和针对定理证明微调的模型在NTP4VC上的表现，结果显示LLM在VC证明中显示出潜力，但程序验证仍面临重大挑战，存在很大的研究差距和机会。

Conclusion: NTP4VC是首个针对程序验证中验证条件自动证明的基准测试，揭示了LLM在该任务上的潜力和局限性，为未来研究指明了重要方向。
...
### [113] [More at Stake: How Payoff and Language Shape LLM Agent Strategies in Cooperation Dilemmas](https://arxiv.org/abs/2601.19082)
...
TL;DR: 研究LLM在重复社会困境中的战略行为，发现激励强度和语言环境显著影响其策略选择，揭示了模型和语言依赖的行为意图模式


<details>
  <summary>Details</summary>
Motivation: 随着LLM在交互和多智能体环境中作为自主智能体，理解其战略行为对于安全、协调以及AI驱动的社会和经济系统至关重要

Method: 使用收益缩放的囚徒困境来分离对激励强度的敏感性，训练监督分类器识别经典重复博弈策略，并将其应用于LLM决策分析

Result: 观察到一致的行为模式，包括对激励敏感的调节策略和跨语言差异；语言框架有时匹配或超过架构效应的影响；揭示了系统性的模型和语言依赖行为意图
...
<details>
  <summary>Details</summary>
Motivation: 现有3D情感说话人脸合成方法存在两个关键问题：1) 音频-视觉情感对齐不佳，表现为音频情感提取困难和对情感微表情控制不足；2) 多视角融合策略采用一刀切方法，忽视了不确定性和特征质量差异，损害了渲染质量。

Method: 提出UA-3DTalk框架，包含三个核心模块：1) 先验提取模块将音频解耦为内容同步特征和个性化特征；2) 情感蒸馏模块采用多模态注意力加权融合机制和4D高斯编码，实现细粒度音频情感提取和情感微表情控制；3) 基于不确定性的形变模块通过不确定性块估计视角特定不确定性，实现自适应多视角融合，并使用多头解码器优化高斯基元。

Result: 在常规和情感数据集上的实验表明，UA-3DTalk在情感对齐的E-FID指标上优于DEGSTalk和EDTalk 5.2%，在唇部同步的SyncC指标上提升3.1%，在渲染质量的LPIPS指标上提升0.015。

Conclusion: UA-3DTalk通过不确定性感知的多视角融合和情感先验蒸馏，有效解决了3D情感说话人脸合成中的情感对齐和渲染质量问题，在多个关键指标上超越了现有最佳方法。
...
Result: 该方法能够系统性地识别和针对函数调用LLM的弱点，推动开发更鲁棒的函数调用模型，为识别和纠正LLM与外部工具交互能力中的弱点提供系统化方法。

Conclusion: 通过强化学习驱动的对抗性数据增强方法，能够有效提升LLM函数调用能力的泛化性和鲁棒性，克服现有方法在数据分布和模式固定方面的局限性。
...
### [116] [Length-Adaptive Interest Network for Balancing Long and Short Sequence Modeling in CTR Prediction](https://arxiv.org/abs/2601.19142)
...
TL;DR: LAIN是一个长度自适应的兴趣网络框架，通过将序列长度作为条件信号，平衡长短序列建模，解决推荐系统中序列长度异质性导致的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 现代推荐系统中的用户行为序列存在显著的长度异质性，从稀疏的短期交互到丰富的长期历史。现有CTR模型增加最大输入序列长度反而会因注意力极化和训练数据长度不平衡而降低短序列用户的性能。

Method: 提出LAIN框架，包含三个轻量级组件：1) 谱长度编码器将长度映射为连续表示；2) 长度条件提示将全局上下文线索注入长短行为分支；3) 长度调制注意力根据序列长度自适应调整注意力锐度。

Result: 在三个真实世界基准测试和五个强CTR骨干网络上，LAIN持续提升整体性能，达到最高1.15%的AUC增益和2.25%的log损失减少。显著提高了短序列用户的准确性，同时不牺牲长序列效果。

Conclusion: LAIN提供了一个通用、高效且可部署的解决方案，用于缓解顺序推荐中由长度引起的偏差，平衡长短序列建模，提升推荐系统性能。
...
### [117] [TS-Debate: Multimodal Collaborative Debate for Zero-Shot Time Series Reasoning](https://arxiv.org/abs/2601.19151)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在时间序列分析中表现出潜力但存在脆弱性，包括数值保真度差、模态干扰和跨模态集成不足等问题，需要更好的零样本推理方法
...
Result: 在涵盖三个公共基准测试的20个任务上，TS-Debate相比强基线（包括所有智能体观察所有输入的标准多模态辩论）实现了持续且显著的性能提升

Conclusion: TS-Debate框架通过模态专业化、结构化辩论和程序化验证，有效保持了模态保真度、暴露了冲突证据并减轻了数值幻觉，无需任务特定微调即可改善时间序列推理
...
### [118] [LocationAgent: A Hierarchical Agent for Image Geolocation via Decoupling Strategy and Evidence from Parametric Knowledge](https://arxiv.org/abs/2601.19155)
*Qiujun Li,Zijin Xiao,Xulin Wang,Zhidan Ma,Cheng Yang,Haifeng Li*

Main category: cs.AI

TL;DR: 论文提出LocationAgent，一种分层定位代理，通过RER架构（推理器-执行器-记录器）实现分层推理，将地理证据验证卸载到外部工具，显著提升零样本性能。
...
Method: 提出分层定位代理LocationAgent，核心思想是在模型中保留分层推理逻辑，同时将地理证据验证卸载到外部工具。采用RER架构（推理器-执行器-记录器）实现角色分离和上下文压缩以防止多步推理中的漂移问题，并构建了一套线索探索工具来提供多样化的证据支持位置推理。

Result: 在零样本设置下，LocationAgent显著优于现有方法至少30%。同时为了解决现有数据集中数据泄露和中文数据稀缺的问题，引入了CCL-Bench（中国城市定位基准），涵盖不同场景粒度和难度级别。

Conclusion: 通过将分层推理逻辑保留在模型中，同时将地理证据验证卸载到外部工具，LocationAgent能够有效解决现有方法在开放世界场景中的事实幻觉和泛化瓶颈问题，在零样本设置下取得了显著性能提升。
...
### [119] [Multi-Agent Procedural Graph Extraction with Structural and Logical Refinement](https://arxiv.org/abs/2601.19170)
*Wangyang Ying,Yanchi Liu,Xujiang Zhao,Wei Cheng,Zhengzhang Chen,Wenchao Yu,Yanjie Fu,Haifeng Chen*

Main category: cs.AI

TL;DR: 提出MULTI-AGENT框架，通过结构化和逻辑反馈迭代优化从自然语言提取工作流程图


<details>
  <summary>Details</summary>
Motivation: 从自然语言自动提取工作流程图很有前景但研究不足，需要保证结构有效性和逻辑一致性。现有大语言模型常产生结构错误或逻辑误解。

Method: 采用多智能体框架，将过程图提取建模为多轮推理：1) 图构建智能体提取初始图；2) 模拟智能体诊断结构缺陷；3) 语义智能体对齐流程逻辑与文本语义。通过自然语言反馈进行迭代优化。

Result: 实验表明，该方法在结构正确性和逻辑一致性方面显著优于现有基线方法。

Conclusion: 提出的多智能体框架通过结构化反馈机制，无需监督或参数更新即可有效提升工作流程图提取的质量。
...
### [120] [CollectiveKV: Decoupling and Sharing Collaborative Information in Sequential Recommendation](https://arxiv.org/abs/2601.19178)
*Jingyu Li,Zhaocheng Du,Qianhui Zhu,kaiyuan Li,Zhicheng Zhang,Song-Li Wu,Chaolang Li,Pengwen Dai*

Main category: cs.AI

TL;DR: 提出CollectiveKV方法，通过跨用户KV共享机制将KV缓存压缩到原始大小的0.8%，同时保持或提升模型性能


<details>
  <summary>Details</summary>
Motivation: 序列推荐系统中Transformer注意力机制的计算复杂度随序列长度增长，KV缓存技术虽然能降低推理延迟，但在大规模用户和长历史序列场景下会带来巨大的存储开销。研究发现不同用户的KV序列存在显著相似性，表明KV中存在协作信号

Method: 通过SVD分析发现KV信息可分为可跨用户共享的大部分信息和用户特定的少部分信息。提出CollectiveKV方法：1）通过可学习的全局KV池捕获跨用户共享信息；2）推理时每个用户从池中检索高维共享KV，并与低维用户特定KV拼接得到最终KV

Result: 在5个序列推荐模型和3个数据集上的实验表明，该方法能将KV缓存压缩到原始大小的0.8%，同时保持甚至提升模型性能

Conclusion: CollectiveKV通过利用KV中的协作信号，实现了高效的跨用户KV共享，显著减少了序列推荐系统中的存储开销，同时不损害模型性能
...
### [121] [CoReTab: Improving Multimodal Table Understanding with Code-driven Reasoning](https://arxiv.org/abs/2601.19193)
*Van-Quang Nguyen,Takayuki Okatani*

Main category: cs.AI

TL;DR: CoReTab：一个通过可执行Python代码实现多步推理的代码驱动框架，显著提升多模态表格理解的准确性和可解释性


<details>
  <summary>Details</summary>
Motivation: 现有多模态表格理解数据集（如MMTab）主要提供简短的事实性答案，缺乏明确的多步推理监督。基于这些数据集训练的模型生成简短回答，准确率不足且推理过程缺乏可解释性。

Method: 提出CoReTab代码驱动推理框架，将多步推理与可执行Python代码结合，生成可扩展、可解释且可自动验证的标注。使用该框架构建了115K个验证样本的数据集，并通过三阶段管道微调开源MLLMs。

Result: 在17个MMTab基准测试（包括表格问答、事实验证和表格结构理解）中，CoReTab训练的模型相比MMTab训练基线分别获得+6.2%、+5.7%和+25.6%的显著提升，同时产生透明且可验证的推理轨迹。

Conclusion: CoReTab作为一个稳健且可泛化的监督框架，有效提升了多模态表格理解中的多步推理能力，实现了更高的准确性和可解释性。
...
### [122] [MAGNET: Towards Adaptive GUI Agents with Memory-Driven Knowledge Evolution](https://arxiv.org/abs/2601.19199)
*Libo Sun,Jiwen Zhang,Siyuan Wang,Zhongyu Wei*

Main category: cs.AI

TL;DR: MAGNET是一个记忆驱动的自适应GUI代理框架，通过双级记忆系统解决移动界面频繁更新导致的代理失效问题，利用界面变化中的稳定功能语义和任务意图来提升代理的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 移动GUI代理经常因为界面频繁更新（UI外观改变和工作流重组）而失效，尽管界面表面变化，但功能语义和任务意图保持稳定。需要一种能适应界面变化、保持任务执行能力的代理框架。

Method: 提出MAGNET框架，包含双级记忆系统：1) 静态记忆将多样化视觉特征链接到稳定功能语义，实现鲁棒的动作定位；2) 程序记忆捕获不同工作流中的稳定任务意图。采用动态记忆进化机制，通过优先访问频繁使用的知识来持续优化两种记忆。

Result: 在AndroidWorld在线基准测试中相比基线有显著提升，离线基准测试在分布偏移下也显示出一致的性能增益。验证了利用界面变化中的稳定结构可以改善代理性能和泛化能力。

Conclusion: 通过挖掘界面变化中的稳定功能语义和任务意图，MAGNET框架能够有效应对移动GUI的频繁更新，提升代理在演化软件环境中的适应性和性能，为构建更鲁棒的自主GUI代理提供了新思路。
...
### [123] [MATA: A Trainable Hierarchical Automaton System for Multi-Agent Visual Reasoning](https://arxiv.org/abs/2601.19204)
*Zhixi Cai,Fucai Ke,Kevin Leo,Sukai Huang,Maria Garcia de la Banda,Peter J. Stuckey,Hamid Rezatofighi*

Main category: cs.AI

TL;DR: MATA是一个基于分层有限状态自动机的多智能体视觉推理系统，通过可训练的超智能体选择最优代理来完成任务，在多个视觉推理基准上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型虽然感知能力强，但隐式推理难以解释且容易在复杂查询上产生幻觉。组合方法虽然提高了可解释性，但大多依赖单一智能体或手工设计的流程，无法决定何时在互补智能体之间协作或在重叠智能体之间竞争。

Method: 提出MATA（多智能体分层可训练自动机），将多智能体系统表示为分层有限状态自动机。每个智能体对应超自动机中的一个状态，运行基于规则的小型子自动机进行可靠微控制。所有智能体读写共享内存，产生透明的执行历史。通过构建转移轨迹树并转换为内存到下一状态对，创建MATA-SFT-90K数据集用于监督微调，训练LLM作为转移策略。

Result: 在多个视觉推理基准测试中，MATA相比单一模型和组合基线方法取得了最先进的结果。微调后的LLM作为转移策略能够理解查询和智能体能力，高效选择最优智能体解决任务。

Conclusion: MATA通过分层多智能体系统实现了可解释且高效的视觉推理，结合了感知能力和显式推理的优势，在保持透明执行历史的同时达到了最先进的性能。
...
### [124] [Beyond In-Domain Detection: SpikeScore for Cross-Domain Hallucination Detection](https://arxiv.org/abs/2601.19245)
*Yongxin Deng,Zhen Fang,Yixuan Li,Ling Chen*

Main category: cs.AI

TL;DR: 提出SpikeScore方法，通过量化多轮对话中的不确定性波动来检测幻觉，在跨域幻觉检测任务中表现优异


<details>
  <summary>Details</summary>
Motivation: 现有幻觉检测方法在同域表现良好但跨域泛化能力差，需要解决跨域可泛化的幻觉检测问题

Method: 提出SpikeScore评分，基于观察到的现象：幻觉引发的多轮对话比事实性对话表现出更大的不确定性波动，通过量化对话中的突变波动来检测幻觉

Result: 实验表明SpikeScore在多个LLM和基准测试中优于代表性基线方法，在跨域泛化方面表现优异

Conclusion: SpikeScore方法能有效解决跨域幻觉检测问题，为实际应用中部署LLM提供了可靠的幻觉检测方案
...
### [125] [GLOVE: Global Verifier for LLM Memory-Environment Realignment](https://arxiv.org/abs/2601.19249)
...
Method: 提出GLOVE框架，通过主动探测检索记忆与新鲜观察之间的不一致性，建立相对真理概念，无需真实监督或强依赖模型内省即可验证和更新记忆。

Result: 在网页导航、规划和控制等多样化基准测试中，加入受控环境漂移后，GLOVE显著提高了智能体成功率，展现了强大的环境适应能力。

Conclusion: GLOVE为LLM记忆系统提供了新的设计维度，通过相对真理验证实现记忆-环境重新对齐，为构建能够自我演化的认知智能体提供了稳健途径。
...
### [126] [Curiosity Driven Knowledge Retrieval for Mobile Agents](https://arxiv.org/abs/2601.19306)
...
<details>
  <summary>Details</summary>
Motivation: 移动代理在复杂应用中表现受限，主要原因是知识不完整和对未见环境的泛化能力弱

Method: 引入好奇心驱动框架，将执行不确定性量化为好奇心分数，超过阈值时从文档、代码库和历史轨迹检索信息，组织成结构化AppCards（包含功能语义、参数约定、界面映射和交互模式），在执行时选择性集成到推理过程中

Result: 在AndroidWorld基准测试中，平均提升6个百分点，结合GPT-5时达到88.8%的新SOTA成功率，AppCards对多步骤和跨应用任务特别有效

Conclusion: AppCards框架能有效减少模糊性、缩短探索时间、支持稳定执行轨迹，显著提升移动代理在复杂应用中的性能
...
### [127] [Balancing Sustainability And Performance: The Role Of Small-Scale Llms In Agentic Artificial Intelligence Systems](https://arxiv.org/abs/2601.19311)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型成为智能代理系统的核心，其推理能耗可能带来严重的可持续性挑战。研究旨在探索部署较小规模语言模型是否能在多智能体真实环境中降低能耗而不影响响应性和输出质量。

Method: 对不同规模的语言模型进行对比分析，量化效率与性能之间的权衡关系，并基于研究发现提出可持续AI设计的实用指南。

Result: 结果显示，较小的开源权重模型能够降低能耗同时保持任务质量。研究还提出了包括优化批量大小配置和计算资源分配在内的实用设计指南。

Conclusion: 这些发现为开发可扩展、环境友好的AI系统提供了可行的策略，表明在保持性能的同时实现能源效率是可能的。
...
### [128] [SETA: Statistical Fault Attribution for Compound AI Systems](https://arxiv.org/abs/2601.19337)
...
<details>
  <summary>Details</summary>
Motivation: 现有鲁棒性测试技术主要针对单网络模型，无法有效扩展到多网络系统，需要新的测试方法来应对复杂AI系统的安全挑战

Method: 开发模块化鲁棒性测试框架，支持组件级系统分析和跨神经网络模块的错误传播推理，框架与架构和模态无关

Result: 在真实世界铁路自主巡检系统上成功应用，实现了超越传统端到端指标的细粒度鲁棒性分析

Conclusion: 该框架为多网络AI系统提供了有效的鲁棒性测试解决方案，支持组件级错误隔离和跨模块错误传播分析
...
### [129] [PROTEUS: SLA-Aware Routing via Lagrangian RL for Multi-LLM Serving Systems](https://arxiv.org/abs/2601.19402)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM路由器需要离线调参并猜测准确率结果，参数与结果关系间接、非单调且依赖数据集。运营商需要直接指定准确率目标，而不是从模糊设置中推断。

Method: 使用拉格朗日对偶控制，学习对偶变量λ跟踪训练期间的约束违反情况，并调节策略网络，使路由器能将指定的τ值转换为满足要求的路由决策。

Result: 在RouterBench和SPROUT数据集上，PROTEUS实现一致的底线合规性（准确率≥τ），目标-响应相关性达0.97-0.98。相比OmniRouter仅22%时间满足底线，PROTEUS在τ∈[0.85,0.95]范围内工作，准确率接近oracle，成本节省达89.8%。

Conclusion: PROTEUS通过接受准确率目标作为输入，解决了LLM路由中参数设置不透明的问题，实现了准确率目标的运行时控制，显著提升了路由系统的实用性和效率。
...
### [130] [RPO:Reinforcement Fine-Tuning with Partial Reasoning Optimization](https://arxiv.org/abs/2601.19404)
...
TL;DR: RPO是一种创新的强化微调算法，通过仅生成推理路径的后缀来大幅减少训练时的token生成量，实现90%的训练时间加速，同时保持与完整路径方法相当的性能。
...
Method: 提出RPO（Partial Reasoning Optimization）算法，与传统方法生成完整推理路径不同，RPO通过经验缓存生成推理路径的后缀进行训练。在rollout阶段，这种方法能减少约95%的token生成，显著降低理论时间开销。

Result: RPO将1.5B模型的训练时间减少90%，7B模型减少72%。同时可以与GRPO、DAPO等典型算法集成，在保持与原算法相当性能的同时实现训练加速。

Conclusion: RPO是一种即插即用的强化微调算法，通过部分推理优化显著减少了训练计算开销，实现了高效的训练加速，同时保持了模型性能。
...
### [131] [Fuzzy expert system for the process of collecting and purifying acidic water: a digital twin approach](https://arxiv.org/abs/2601.19527)
*Temirbolat Maratuly,Pakizar Shamoi,Timur Samigulin*

Main category: cs.AI

TL;DR: 本文提出了一种结合数字孪生的模糊专家系统，用于自动化净化酸性水处理过程，通过模拟人类推理保持关键参数在期望水平，降低操作风险并提高效率。


<details>
  <summary>Details</summary>
Motivation: 酸性水净化对于减少排放、降低腐蚀风险、实现处理水在工业或家庭应用中的再利用以及最终降低运营成本至关重要。自动化净化过程有助于通过限制人工参与来降低工人伤害风险。原油中的酸性成分在加工过程中会释放到酸性水中，如果不妥善处理，会对环境造成严重威胁并加速管道和设备腐蚀。

Method: 开发了结合数字孪生的模糊专家系统，使用Honeywell UniSim Design R492开发数字孪生模拟真实工业行为，通过MATLAB进行阀门动态建模，使用OPC DA实现模拟器与控制器之间的实时数据交换。模糊控制器采用分程控制策略控制两个阀门，在21种不同初始压力条件下测试了5种去模糊化策略，共105个测试场景。
...
### [132] [Benchmarks Saturate When The Model Gets Smarter Than The Judge](https://arxiv.org/abs/2601.19532)
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型基准测试存在数据集和评估方法的不准确问题，这些问题削弱了基准测试的有效性。作者旨在创建一个更干净、更可靠的数学数据集，以更精确地评估模型性能。

Method: 创建Omni-MATH-2数据集，对原始Omni-MATH数据集进行人工修订：1）确保LaTeX可编译性；2）验证问题可解性和可验证性；3）添加缺失的图形或信息；4）标记需要证明、估计或图像的问题；5）移除杂乱内容。数据集分为精确答案子集（4181个问题）和标记的非标准子集（247个问题）。使用该数据集评估裁判引起的噪声，比较GPT-5 mini和原始Omni-Judge的表现。

Result: 1）修订后的数据集显著减少了数据集引起的噪声；2）在裁判分歧中，Omni-Judge在96.4%的情况下是错误的，表明其无法区分模型能力；3）随着问题难度增加，需要更胜任的裁判来防止裁判错误掩盖模型间的真实差异；4）对于标记的非标准问题子集，两种裁判都未能识别当前的失败模式。

Conclusion: 数据集质量和裁判可靠性对于开发准确的模型性能基准都至关重要。即使基准测试尚未饱和，不准确的裁判也可能无法区分模型能力。需要同时改进数据集质量和评估方法，才能获得可靠的模型性能评估。
...
### [133] [Learning Adaptive Parallel Execution for Efficient Code Localization](https://arxiv.org/abs/2601.19568)
*Ke Xu,Siyang Xiao,Ming Liang,Yichen Yu,Zhixiang Wang,Jingxuan Xu,Dajun Chen,Wei Jiang,Yong Li*

Main category: cs.AI

TL;DR: FuseSearch通过联合质量-效率优化，动态调整并行搜索广度，显著提升代码定位效率，减少冗余调用，在保持SOTA性能的同时实现93.6%的加速。


<details>
  <summary>Details</summary>
Motivation: 当前自动化软件开发管道中，代码定位是关键瓶颈。虽然并行工具执行可以提升发现速度，但现有代理存在34.9%的冗余调用率，抵消了并行化的优势。

Method: 提出FuseSearch，将并行代码定位重新定义为联合质量-效率优化任务。定义工具效率（独特信息增益与调用次数的比率），采用两阶段SFT和RL训练方法学习自适应并行策略，动态根据任务上下文调整搜索广度，从探索阶段演进到精炼阶段。

Result: 在SWE-bench Verified上评估，FuseSearch-4B达到SOTA性能（84.7%文件级和56.4%函数级F1分数），同时实现93.6%的加速，使用次数减少67.7%，token使用减少68.9%。

Conclusion: 效率感知训练通过消除噪声冗余信号自然提升质量，实现了高性能且成本效益高的代码定位代理。动态并行策略优化能同时提升质量和效率。
...
### [134] [ComAgent: Multi-LLM based Agentic AI Empowered Intelligent Wireless Networks](https://arxiv.org/abs/2601.19607)
*Haoyun Li,Ming Xiao,Kezhi Wang,Robert Schober,Dong In Kim,Yong Liang Guan*

Main category: cs.AI

TL;DR: ComAgent是一个多LLM智能体框架，通过感知-规划-行动-反思循环，协调专门代理自动生成求解器就绪的优化公式和可重复仿真，用于6G网络跨层优化。


<details>
  <summary>Details</summary>
Motivation: 6G网络依赖复杂的跨层优化，但将高层意图手动转化为数学公式仍是瓶颈。现有LLM方法缺乏足够的领域基础、约束意识和验证能力。

Method: 采用多LLM智能体框架，通过感知-规划-行动-反思闭环循环，协调文献搜索、编码和评分等专门代理，迭代分解问题并自我纠正错误。

Result: 在复杂波束成形优化中达到专家可比性能，在多样化无线任务中优于单体LLM，展示了在无线网络自动化设计中的潜力。
...
### [135] [Algorithmic Prompt-Augmentation for Efficient LLM-Based Heuristic Design for A* Search](https://arxiv.org/abs/2601.19622)
*Thomas Bömer,Nico Koltermann,Max Disselnmeyer,Bastian Amberg,Anne Meyer*

Main category: cs.AI

TL;DR: 提出A-CEoH框架，通过将A*算法代码融入提示词，利用上下文学习自动生成启发式函数，在UPMP和SPP问题上超越人工设计的启发式函数


<details>
  <summary>Details</summary>
Motivation: 传统启发式函数需要人工设计，依赖专家知识且耗时。随着大语言模型和进化框架的发展，自动化启发式设计成为可能。本文旨在扩展EoH框架，探索A*搜索中引导启发式的自动生成

Method: 提出算法-上下文EoH（A-CEoH）框架，采用领域无关的提示增强策略，将A*算法代码融入提示词以利用上下文学习。在单元装载预调配问题（UPMP）和经典滑块拼图问题（SPP）两个领域进行评估

Result: 计算实验表明，A-CEoH能显著提高生成启发式的质量，甚至超越专家设计的启发式函数

Conclusion: A-CEoH框架通过将算法代码融入提示词，有效利用上下文学习自动生成高质量的启发式函数，为自动化启发式设计提供了新方法
...
### [136] [Agentic Design Patterns: A System-Theoretic Framework](https://arxiv.org/abs/2601.19752)
*Minh-Dung Dao,Quy Minh Le,Hoang Thanh Lam,Duc-Trong Le,Quoc-Viet Pham,Barry O'Sullivan,Hoang D. Nguyen*

Main category: cs.AI

TL;DR: 提出基于系统理论的AI智能体工程化方法，包括五子系统框架和12个设计模式，用于构建更可靠、模块化的自主系统。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型驱动的智能体系统存在幻觉、推理能力差等问题，且设计模式缺乏严谨的系统理论基础，导致应用不可靠、脆弱。需要建立原则性的工程方法来构建稳健的AI智能体。

Method: 1) 提出系统理论框架，将智能体系统解构为五个核心交互功能子系统：推理与世界模型、感知与接地、动作执行、学习与适应、智能体间通信；2) 基于该架构和智能体挑战分类，提出12个智能体设计模式，分为基础模式、认知与决策模式、执行与交互模式、适应与学习模式。

Result: 通过ReAct框架的案例研究展示了该框架的实用性，证明所提出的设计模式可以纠正系统性架构缺陷。为研究人员和工程师提供了标准化的智能体设计交流语言和方法。
...
### [137] [GAVEL: Towards rule-based safety through activation monitoring](https://arxiv.org/abs/2601.19768)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于激活的安全方法存在精度低、灵活性差、缺乏可解释性等问题，无法有效检测表面文本不明显的危险行为。受网络安全中规则共享实践的启发，需要一种更精确、可定制、可解释的安全框架。

Method: 提出基于规则的激活安全范式：1) 将激活建模为认知元素（CEs），如"发出威胁"、"支付处理"等细粒度可解释因子；2) 基于CEs定义谓词规则；3) 实时检测违规行为；4) 提供无需重新训练模型或检测器的配置更新机制。

Result: 基于规则的激活安全方法提高了检测精度，支持领域定制化，为可扩展、可解释、可审计的AI治理奠定了基础。将开源发布GAVEL框架及配套的自动规则创建工具。

Conclusion: 基于规则的激活安全通过认知元素的可组合表示和谓词规则检测，实现了比传统方法更高的精度、灵活性和可解释性，为实际AI安全治理提供了实用框架。
...
### [138] [CASTER: Breaking the Cost-Performance Barrier in Multi-Agent Orchestration via Context-Aware Strategy for Task Efficient Routing](https://arxiv.org/abs/2601.19793)
*Shanyv Liu,Xuyang Yuan,Tao Chen,Zijun Zhan,Zhu Han,Danyang Zheng,Weishan Zhang,Shaohua Cao*

Main category: cs.AI

TL;DR: CASTER是一个用于图基多智能体系统的轻量级路由器，通过双信号路由机制动态选择模型，在保持任务成功率的同时大幅降低推理成本


<details>
  <summary>Details</summary>
Motivation: 图基多智能体系统存在静态模型分配的效率问题，统一部署强模型在简单子任务上浪费计算资源，需要动态模型选择机制来优化资源分配

Method: CASTER采用双信号路由器，结合语义嵌入和结构元特征来估计任务难度；通过冷启动到迭代演化的训练范式，利用自身路由失败的负反馈进行自优化

Result: 在软件工程、数据分析、科学发现和网络安全四个领域的实验中，CASTER相比强模型基线减少高达72.4%的推理成本，同时保持相同的成功率，优于启发式路由和FrugalGPT

Conclusion: CASTER为图基多智能体系统提供了一种高效的动态模型选择方案，通过智能路由在保持性能的同时显著降低计算成本
...
### [139] [An Interpretable Recommendation Model for Psychometric Data, With an Application to Gerontological Primary Care](https://arxiv.org/abs/2601.19824)
...
TL;DR: 提出一个用于老年初级保健的推荐系统，利用心理测量数据结构提供可视化解释，帮助专业人员制定个性化护理计划
...
Method: 开发了一种推荐模型，利用心理测量数据结构生成可视化解释。这些解释既忠实于模型，又能被护理专业人员理解。研究聚焦于老年初级保健这一特定医疗领域，通过离线性能评估和用户研究验证模型效果。

Result: 在巴西研究合作伙伴收集的医疗数据集上进行了比较离线性能评估，结果显示模型表现良好。用户研究评估了模型生成的可视化解释的可解释性，结果表明模型能够提供专业人员可理解的解释。

Conclusion: 该推荐模型能够推进推荐系统在老年初级保健领域的应用。随着人口结构变化，这一领域的需求、机会和信息技术需求预计将增长，该模型为此提供了可行的解决方案。
...
### [140] [Routing End User Queries to Enterprise Databases](https://arxiv.org/abs/2601.19825)
*Saikrishna Sudarshan,Tanay Kulkarni,Manasi Patwardhan,Lovekesh Vig,Ashwin Srinivasan,Tanmay Tulsidas Verlekar*

Main category: cs.AI

TL;DR: 该论文提出了一种用于多数据库企业环境中自然语言查询路由的模块化、推理驱动的重排序策略，通过显式建模模式覆盖、结构连接性和细粒度语义对齐，在扩展的NL-to-SQL基准测试中优于嵌入方法和直接LLM提示方法。


<details>
  <summary>Details</summary>
Motivation: 在多数据库企业环境中，随着数据库存储库规模增大、领域重叠以及查询歧义性增加，自然语言查询路由变得越来越具有挑战性，需要更结构化、更鲁棒的基于推理的解决方案。

Method: 提出了一种模块化、推理驱动的重排序策略，显式建模三个关键方面：模式覆盖（schema coverage）、结构连接性（structural connectivity）和细粒度语义对齐（fine-grained semantic alignment）。通过扩展现有NL-to-SQL数据集构建了现实的基准测试。

Result: 提出的方法在所有指标上一致地优于仅使用嵌入的方法和直接LLM提示的基线方法，特别是在大规模、领域重叠的数据库存储库和歧义查询场景下表现更优。

Conclusion: 在多数据库企业环境中，显式建模模式覆盖、结构连接性和语义对齐的推理驱动方法比简单的嵌入或直接LLM提示方法更有效，为自然语言查询路由提供了更鲁棒的解决方案。
...
### [141] [Visual Generation Unlocks Human-Like Reasoning through Multimodal World Models](https://arxiv.org/abs/2601.19834)
...
TL;DR: 该论文提出视觉优势假说：对于物理世界相关任务，视觉生成能更好地作为世界模型，而纯语言模型存在表示限制和先验知识不足的瓶颈。通过构建VisWorld-Eval评估套件，实验证明交错视觉-语言思维链推理在需要视觉世界建模的任务上显著优于纯语言推理。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统在数学、编程等抽象领域已达到专家水平，但在物理和空间智能等需要丰富表示和先验知识的领域仍远落后于人类。统一多模态模型的出现引发了人们对基于互补多模态路径的类人推理的兴趣，但其优势尚不明确。本文旨在从世界模型角度，系统研究视觉生成何时以及如何有益于推理。

Method: 1. 提出视觉优势假说：对于某些任务（特别是物理世界相关任务），视觉生成更自然地作为世界模型；2. 从理论上将内部世界建模形式化为思维链推理的核心组成部分；3. 构建新的评估套件VisWorld-Eval，识别需要交错视觉-语言思维链推理的任务；4. 在最先进的统一多模态模型上进行对照实验，比较交错视觉-语言思维链与纯语言思维链的性能。

Result: 实验表明，交错视觉-语言思维链推理在有利于视觉世界建模的任务上显著优于纯语言思维链推理，但在其他任务上没有明显优势。这验证了视觉优势假说，并明确了多模态世界建模的潜力边界。

Conclusion: 该研究阐明了多模态世界建模对于构建更强大、更类人的多模态AI的潜力。视觉生成在物理世界相关任务中能更自然地作为世界模型，而纯语言世界模型存在表示限制和先验知识不足的瓶颈。这项工作为理解何时以及如何利用多模态推理提供了原则性指导。
